 BEGIN BEGIN TRY 
			...
			WHERE ixp_rule_hash = 'CEF4955B_F4E0_4323_A204_E5BF3949B111'

			if @old_ixp_rule_id IS NULL
			BEGIN
				SELECT @old_ixp_rule_id = ixp_rules_id
			FROM ixp_rules ir
			WHERE ir.ixp_rules_name = 'Recovery Rate'
			END
		...
		END
			...
			BEGIN
				...
				VALUES( 
					...
					23505,
					1,
					'CEF4955B_F4E0_4323_A204_E5BF3949B111'
					 )
		...
		BEGIN
			...
			UPDATE
			ixp_rules
			SET ixp_rules_name = 'Recovery Rate'
...
INSERT INTO ixp_import_data_source (rules_id, data_source_type, connection_string, data_source_location, destination_table, delimiter, source_system_id, data_source_alias, is_customized, customizing_query, is_header_less, no_of_columns, folder_location, custom_import, use_parameter, excel_sheet, ssis_package, soap_function_id, is_ftp, ftp_url, ftp_username, ftp_password, clr_function_id, ws_function_name)
					SELECT @ixp_rules_id_new,
						   ...
						   '',
						   '',
						   0x01000000E66C805A1574E897B5B20747AED69330787D5A71C77AD643,
...
INSERT INTO ixp_import_data_mapping(ixp_rules_id, dest_table_id, source_column_name, dest_column, column_function, column_aggregation, repeat_number, where_clause )   SELECT @ixp_rules_id_new, it.ixp_tables_id, 'RR.[Effective Date]', ic.ixp_columns_id, NULL, NULL, 0, NULL 
									   FROM ixp_tables it 
									   INNER JOIN ixp_tables it2 ON it2.ixp_tables_name = 'ixp_recovery_rate'
									   ...
									   WHERE it.ixp_tables_name = 'ixp_recovery_rate' UNION ALL  SELECT @ixp_rules_id_new, it.ixp_tables_id, 'RR.[Debt Rating]', ic.ixp_columns_id, NULL, NULL, 0, NULL 
									   FROM ixp_tables it 
									   INNER JOIN ixp_tables it2 ON it2.ixp_tables_name = 'ixp_recovery_rate'
									   ...
									   WHERE it.ixp_tables_name = 'ixp_recovery_rate' UNION ALL  SELECT @ixp_rules_id_new, it.ixp_tables_id, 'RR.[Months]', ic.ixp_columns_id, NULL, NULL, 0, NULL 
									   FROM ixp_tables it 
									   INNER JOIN ixp_tables it2 ON it2.ixp_tables_name = 'ixp_recovery_rate'
									   ...
									   WHERE it.ixp_tables_name = 'ixp_recovery_rate' UNION ALL  SELECT @ixp_rules_id_new, it.ixp_tables_id, 'RR.[Recovery Rate]', ic.ixp_columns_id, NULL, NULL, 0, NULL 
									   FROM ixp_tables it 
									   INNER JOIN ixp_tables it2 ON it2.ixp_tables_name = 'ixp_recovery_rate'
									   ...
									   WHERE it.ixp_tables_name = 'ixp_recovery_rate' UNION ALL  SELECT @ixp_rules_id_new, it.ixp_tables_id, 'RR.[Rating Type]', ic.ixp_columns_id, NULL, NULL, 0, NULL 
									   FROM ixp_tables it 
									   INNER JOIN ixp_tables it2 ON it2.ixp_tables_name = 'ixp_recovery_rate'
