BEGIN BEGIN TRY 
			...
			WHERE ixp_rule_hash = 'B7415574_5DB6_4098_95A8_DDCD2BDDB058'

			if @old_ixp_rule_id IS NULL
			BEGIN
				SELECT @old_ixp_rule_id = ixp_rules_id
			FROM ixp_rules ir
			WHERE ir.ixp_rules_name = 'Commodity Definition'
			END
		...
		END
			...
			BEGIN
				...
				VALUES( 
					...
					23501,
					1,
					'B7415574_5DB6_4098_95A8_DDCD2BDDB058'
					 )
		...
		BEGIN
			...
			UPDATE
			ixp_rules
			SET ixp_rules_name = 'Commodity Definition'
...
INSERT INTO ixp_import_data_source (rules_id, data_source_type, connection_string, data_source_location, destination_table, delimiter, source_system_id, data_source_alias, is_customized, customizing_query, is_header_less, no_of_columns, folder_location, custom_import, use_parameter, excel_sheet, ssis_package, soap_function_id, is_ftp, ftp_url, ftp_username, ftp_password, clr_function_id, ws_function_name)
					SELECT @ixp_rules_id_new,
						   ...
						   '',
						   '',
						   0x01000000E50C276C80D1C575409AC2C9A00A8498518F381CD4788C8A,
...
INSERT INTO ixp_import_data_mapping(ixp_rules_id, dest_table_id, source_column_name, dest_column, column_function, column_aggregation, repeat_number, where_clause )   SELECT @ixp_rules_id_new, it.ixp_tables_id, 'cc.[Commodity ID]', ic.ixp_columns_id, NULL, NULL, 0, NULL 
									   FROM ixp_tables it 
									   INNER JOIN ixp_tables it2 ON it2.ixp_tables_name = 'ixp_source_commodity_template'
									   ...
									   WHERE it.ixp_tables_name = 'ixp_source_commodity_template' UNION ALL  SELECT @ixp_rules_id_new, it.ixp_tables_id, 'cc.[Name]', ic.ixp_columns_id, NULL, NULL, 0, NULL 
									   FROM ixp_tables it 
									   INNER JOIN ixp_tables it2 ON it2.ixp_tables_name = 'ixp_source_commodity_template'
									   INNER JOIN ixp_columns ic ON ic.ixp_columns_name = 'commodity_name' AND ic.ixp_table_id = it2.ixp_tables_id AND (ic.header_detail = 'NULL' OR ic.header_detail IS NULL)
									   WHERE it.ixp_tables_name = 'ixp_source_commodity_template' UNION ALL  SELECT @ixp_rules_id_new, it.ixp_tables_id, 'cc.[Description]', ic.ixp_columns_id, NULL, NULL, 0, NULL 
									   FROM ixp_tables it 
									   INNER JOIN ixp_tables it2 ON it2.ixp_tables_name = 'ixp_source_commodity_template'
									   INNER JOIN ixp_columns ic ON ic.ixp_columns_name = 'commodity_desc' AND ic.ixp_table_id = it2.ixp_tables_id AND (ic.header_detail = 'NULL' OR ic.header_detail IS NULL)
									   WHERE it.ixp_tables_name = 'ixp_source_commodity_template'
