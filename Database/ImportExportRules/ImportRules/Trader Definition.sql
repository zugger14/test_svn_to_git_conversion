BEGIN 
	BEGIN TRY 
			...
			WHERE ixp_rule_hash = '34EE820B_3351_47C0_A2A3_CD69851D09DB'

			if @old_ixp_rule_id IS NULL
			BEGIN
				SELECT @old_ixp_rule_id = ixp_rules_id
			FROM ixp_rules ir
			WHERE ir.ixp_rules_name = 'Trader Definition'
			END
		...
		END
			...
			BEGIN
				...
				VALUES( 
					...
					23501,
					1,
					'34EE820B_3351_47C0_A2A3_CD69851D09DB'
					 )
		...
		BEGIN
			...
			UPDATE
			ixp_rules
			SET ixp_rules_name = 'Trader Definition'
...
INSERT INTO ixp_import_filter (ixp_rules_id,filter_group,filter_id,filter_value,ixp_import_data_source)
					SELECT @ixp_rules_id_new,
						   'New..',
						   112200,
						   'rshrestha@pioneersolutionsglobal.com',
						   21409
					FROM ixp_rules ir 
					WHERE ir.ixp_rules_id = @ixp_rules_id_new 
INSERT INTO ixp_import_filter (ixp_rules_id,filter_group,filter_id,filter_value,ixp_import_data_source)
					SELECT @ixp_rules_id_new,
						   'Kiran',
						   112200,
						   'kgautam@pioneersolutionsglobal.com;keyrungautm@gmail.com',
						   21409
					FROM ixp_rules ir 
					WHERE ir.ixp_rules_id = @ixp_rules_id_new 
INSERT INTO ixp_import_filter (ixp_rules_id,filter_group,filter_id,filter_value,ixp_import_data_source)
...
INSERT INTO ixp_import_data_mapping(ixp_rules_id, dest_table_id, source_column_name, dest_column, column_function, column_aggregation, repeat_number, where_clause ,udf_field_id)   SELECT @ixp_rules_id_new, it.ixp_tables_id, 't.[Code]', ic.ixp_columns_id, NULL, NULL, 0, NULL, NULL 
									   FROM ixp_tables it 
									   INNER JOIN ixp_tables it2 ON it2.ixp_tables_name = 'ixp_source_trader_template'
									   ...
									   WHERE it.ixp_tables_name = 'ixp_source_trader_template' UNION ALL  SELECT @ixp_rules_id_new, it.ixp_tables_id, 't.[Name]', ic.ixp_columns_id, NULL, NULL, 0, NULL, NULL 
									   FROM ixp_tables it 
									   INNER JOIN ixp_tables it2 ON it2.ixp_tables_name = 'ixp_source_trader_template'
									   INNER JOIN ixp_columns ic ON ic.ixp_columns_name = 'trader_name' AND ic.ixp_table_id = it2.ixp_tables_id AND (ic.header_detail = 'NULL' OR ic.header_detail IS NULL)
									   WHERE it.ixp_tables_name = 'ixp_source_trader_template' UNION ALL  SELECT @ixp_rules_id_new, it.ixp_tables_id, 't.[Description]', ic.ixp_columns_id, NULL, NULL, 0, NULL, NULL 
									   FROM ixp_tables it 
									   INNER JOIN ixp_tables it2 ON it2.ixp_tables_name = 'ixp_source_trader_template'
									   INNER JOIN ixp_columns ic ON ic.ixp_columns_name = 'trader_desc' AND ic.ixp_table_id = it2.ixp_tables_id AND (ic.header_detail = 'NULL' OR ic.header_detail IS NULL)
									   WHERE it.ixp_tables_name = 'ixp_source_trader_template' UNION ALL  SELECT @ixp_rules_id_new, it.ixp_tables_id, 't.[User]', ic.ixp_columns_id, NULL, NULL, 0, NULL, NULL 
									   FROM ixp_tables it 
									   INNER JOIN ixp_tables it2 ON it2.ixp_tables_name = 'ixp_source_trader_template'
									   INNER JOIN ixp_columns ic ON ic.ixp_columns_name = 'user_login_id' AND ic.ixp_table_id = it2.ixp_tables_id AND (ic.header_detail = 'NULL' OR ic.header_detail IS NULL)
									   WHERE it.ixp_tables_name = 'ixp_source_trader_template'
