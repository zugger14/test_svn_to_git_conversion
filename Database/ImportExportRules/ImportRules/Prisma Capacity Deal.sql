...
OUTER APPLY (
	SELECT source_uom_id FROM source_uom WHERE uom_name = ''''Mwh''''
) uom
OUTER APPLY (
	SELECT source_currency_id FROM source_currency WHERE currency_name = ''''Eur''''
) curr
...
INSERT INTO ixp_import_data_source (rules_id, data_source_type, connection_string, data_source_location, destination_table, delimiter, source_system_id, data_source_alias, is_customized, customizing_query, is_header_less, no_of_columns, folder_location, custom_import, use_parameter
 ...
 [counter], dealID, auctionId, balancingGroup_balancingGroupNumber, bookingDate, bundled
  ...
  , quantity_unit, quantity_value, runtime_end, runtime_start, shipper_user_email, tso_name, undiscounted, bundleInformation_splitFactor_entry
INTO #test_data
FROM  
(
...
, bundleInformation_splitFactor_entry shipper_user_alloc_price
INTO #final_data
FROM #test_data
...
INSERT INTO ixp_import_data_mapping(ixp_rules_id, dest_table_id, source_column_name, dest_column, column_function, column_aggregation, repeat_number, where_clause ,udf_field_id)   SELECT @ixp_rules_id_new, it.ixp_tables_id, 'pa.[dealID]', ic.ixp_columns_id, NULL, NULL, 0, NULL, NULL 
									   FROM ixp_tables it 
									   ...
									   WHERE it.ixp_tables_name = 'ixp_source_deal_template' UNION ALL  SELECT @ixp_rules_id_new, it.ixp_tables_id, 'pa.[quantity_value]', ic.ixp_columns_id, NULL, NULL, 0, NULL, NULL 
									   FROM ixp_tables it 
									   INNER JOIN ixp_tables it2 ON it2.ixp_tables_name = 'ixp_source_deal_template'
