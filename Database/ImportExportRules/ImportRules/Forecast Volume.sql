BEGIN BEGIN TRY 
			...
			WHERE ixp_rule_hash = '8AEB347F_A902_4C3A_9114_FC4BCF5C52B6'

			if @old_ixp_rule_id IS NULL
			BEGIN
				SELECT @old_ixp_rule_id = ixp_rules_id
			FROM ixp_rules ir
			WHERE ir.ixp_rules_name = 'Forecast Volume'
			END
		...
		END
			...
			BEGIN
				...
				VALUES( 
					...
					23502,
					1,
					'8AEB347F_A902_4C3A_9114_FC4BCF5C52B6'
					 )
		...
		BEGIN
			...
			UPDATE
			ixp_rules
			SET ixp_rules_name = 'Forecast Volume'
...
INSERT INTO ixp_import_data_source (rules_id, data_source_type, connection_string, data_source_location, destination_table, delimiter, source_system_id, data_source_alias, is_customized, customizing_query, is_header_less, no_of_columns, folder_location, custom_import, use_parameter, excel_sheet, ssis_package, soap_function_id, is_ftp, ftp_url, ftp_username, ftp_password)
					SELECT @ixp_rules_id_new,
						   ...
						   '',
						   '',
						   0x010000006CF39A4AD9E835729F010A9840D72FB28E998925291F40CA
					FROM ixp_rules ir 
...
INSERT INTO ixp_import_data_mapping(ixp_rules_id, dest_table_id, source_column_name, dest_column, column_function, column_aggregation, repeat_number, where_clause )   SELECT @ixp_rules_id_new, it.ixp_tables_id, 'fv.[Profile Name]', ic.ixp_columns_id, NULL, NULL, 0, NULL 
									   FROM ixp_tables it 
									   ...
									   WHERE it.ixp_tables_name = 'ixp_deal_detail_hour_template' UNION ALL  SELECT @ixp_rules_id_new, it.ixp_tables_id, 'fv.[Term]', ic.ixp_columns_id, NULL, NULL, 0, NULL 
									   FROM ixp_tables it 
									   INNER JOIN ixp_tables it2 ON it2.ixp_tables_name = 'ixp_deal_detail_hour_template'
									   ...
									   WHERE it.ixp_tables_name = 'ixp_deal_detail_hour_template' UNION ALL  SELECT @ixp_rules_id_new, it.ixp_tables_id, 'fv.[Hour]', ic.ixp_columns_id, NULL, NULL, 0, NULL 
									   FROM ixp_tables it 
									   INNER JOIN ixp_tables it2 ON it2.ixp_tables_name = 'ixp_deal_detail_hour_template'
									   ...
									   WHERE it.ixp_tables_name = 'ixp_deal_detail_hour_template' UNION ALL  SELECT @ixp_rules_id_new, it.ixp_tables_id, 'fv.[Volume]', ic.ixp_columns_id, NULL, NULL, 0, NULL 
									   FROM ixp_tables it 
									   INNER JOIN ixp_tables it2 ON it2.ixp_tables_name = 'ixp_deal_detail_hour_template'
									   ...
									   WHERE it.ixp_tables_name = 'ixp_deal_detail_hour_template' UNION ALL  SELECT @ixp_rules_id_new, it.ixp_tables_id, 'fv.[Is DST]', ic.ixp_columns_id, NULL, NULL, 0, NULL 
									   FROM ixp_tables it 
									   INNER JOIN ixp_tables it2 ON it2.ixp_tables_name = 'ixp_deal_detail_hour_template'
									   ...
									   WHERE it.ixp_tables_name = 'ixp_deal_detail_hour_template' UNION ALL  SELECT @ixp_rules_id_new, it.ixp_tables_id, 'fv.[Minute]', ic.ixp_columns_id, NULL, NULL, 0, NULL 
									   FROM ixp_tables it 
									   INNER JOIN ixp_tables it2 ON it2.ixp_tables_name = 'ixp_deal_detail_hour_template'
