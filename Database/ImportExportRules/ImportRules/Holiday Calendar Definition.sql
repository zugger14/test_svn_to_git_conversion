 BEGIN BEGIN TRY 
			...
			WHERE ixp_rule_hash = 'C517BCDB_A749_4713_B065_6C41541FE2EF'

			if @old_ixp_rule_id IS NULL
			BEGIN
				SELECT @old_ixp_rule_id = ixp_rules_id
			FROM ixp_rules ir
			WHERE ir.ixp_rules_name = 'Holiday Calendar Definition'
			END
		...
		END
			...
			BEGIN
				...
				VALUES( 
					...
					23501,
					1,
					'C517BCDB_A749_4713_B065_6C41541FE2EF'
					 )
		...
		BEGIN
			...
			UPDATE
			ixp_rules
			SET ixp_rules_name = 'Holiday Calendar Definition'
...
INSERT INTO ixp_import_data_source (rules_id, data_source_type, connection_string, data_source_location, destination_table, delimiter, source_system_id, data_source_alias, is_customized, customizing_query, is_header_less, no_of_columns, folder_location, custom_import, use_parameter, excel_sheet, ssis_package, soap_function_id, is_ftp, ftp_url, ftp_username, ftp_password, clr_function_id, ws_function_name)
					SELECT @ixp_rules_id_new,
						   ...
						   '',
						   '',
						   0x0100000051D416029E3BB4472B1F488543BB9FCC9C571E41764249DE,
...
INSERT INTO ixp_import_data_mapping(ixp_rules_id, dest_table_id, source_column_name, dest_column, column_function, column_aggregation, repeat_number, where_clause )   SELECT @ixp_rules_id_new, it.ixp_tables_id, 'H.[Holiday Group]', ic.ixp_columns_id, NULL, NULL, 0, NULL 
									   ...
									   WHERE it.ixp_tables_name = 'ixp_holiday_calendar_template' UNION ALL  SELECT @ixp_rules_id_new, it.ixp_tables_id, 'H.[Date]', ic.ixp_columns_id, NULL, NULL, 0, NULL 
									   FROM ixp_tables it 
									   INNER JOIN ixp_tables it2 ON it2.ixp_tables_name = 'ixp_holiday_calendar_template'
