BEGIN 
	BEGIN TRY 
		...
		DECLARE @ixp_rules_id_new INT
			...
			WHERE ixp_rule_hash = 'E525FBD0_9616_42F1_89C7_F174C50572B5'

			if @old_ixp_rule_id IS NULL
			BEGIN
				SELECT @old_ixp_rule_id = ixp_rules_id
			FROM ixp_rules ir
			WHERE ir.ixp_rules_name = 'Capacity PFC'
			END
...
DECLARE @set_process_id VARCHAR(40), @scr_table NVARCHAR(200), @output_table  NVARCHAR(200)
SELECT @set_process_id = REVERSE(SUBSTRING(REVERSE(''[temp_process_table]''), 0,37)) 
SET @scr_table = ''adiha_process.dbo.temp_src_details_''+ @set_process_id 
SET @output_table = ''adiha_process.dbo.temp_output_details_''+ @set_process_id 


EXEC(''DROP TABLE IF EXISTS ''+ @scr_table + '' 
	SELECT [deal ref id] deal_id, [term date] term, [hour] , [minute], [is dst]  is_dst, volume, [Actual Volume] actual_volume,
	[Schedule Volume] schedule_volume,	Price
	INTO ''+ @scr_table + '' 
	FROM [temp_process_table]'') 

EXEC spa_calc_weighted_average_price @flag = ''c'',
    @source_process_table= @scr_table
    , @calc_process_table= @output_table
	, @generic_mapping_process = ''Transportation Capacity''
	--, @debug_mode BIT = 1

EXEC(''
	UPDATE a
	SET volume = rs.volume
		, price = rs.price
	FROM [temp_process_table] a
...
',
					...
					23502,
					1,
					'E525FBD0_9616_42F1_89C7_F174C50572B5'
					 )
		...
		BEGIN
			...
			UPDATE
			ixp_rules
			SET ixp_rules_name = 'Capacity PFC'
