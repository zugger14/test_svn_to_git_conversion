BEGIN 
	BEGIN TRY 
		...
		DECLARE @ixp_rules_id_new INT
			...
			WHERE ixp_rule_hash = '496CD549_43E6_4C7F_89E1_121D470EC179'

			if @old_ixp_rule_id IS NULL
			BEGIN
				SELECT @old_ixp_rule_id = ixp_rules_id
			FROM ixp_rules ir
			WHERE ir.ixp_rules_name = 'Deal Volume Update'
			END
		...
		END
			...
			BEGIN
				...
				VALUES( 
					...
					23502,
					1,
					'496CD549_43E6_4C7F_89E1_121D470EC179'
					 )
		...
		BEGIN
			...
			UPDATE
			ixp_rules
			SET ixp_rules_name = 'Deal Volume Update'
...
INSERT INTO ixp_import_data_mapping(ixp_rules_id, dest_table_id, source_column_name, dest_column, column_function, column_aggregation, repeat_number, where_clause ,udf_field_id)   SELECT @ixp_rules_id_new, it.ixp_tables_id, 'du.[term start]', ic.ixp_columns_id, NULL, NULL, 0, NULL, NULL 
									   FROM ixp_tables it 
									   INNER JOIN ixp_tables it2 ON it2.ixp_tables_name = 'ixp_source_deal_volume_update_template'
									   ...
									   WHERE it.ixp_tables_name = 'ixp_source_deal_volume_update_template' UNION ALL  SELECT @ixp_rules_id_new, it.ixp_tables_id, 'du.[deal id]', ic.ixp_columns_id, NULL, NULL, 0, NULL, NULL 
									   FROM ixp_tables it 
									   INNER JOIN ixp_tables it2 ON it2.ixp_tables_name = 'ixp_source_deal_volume_update_template'
									   ...
									   WHERE it.ixp_tables_name = 'ixp_source_deal_volume_update_template' UNION ALL  SELECT @ixp_rules_id_new, it.ixp_tables_id, 'du.[deal ref id]', ic.ixp_columns_id, NULL, NULL, 0, NULL, NULL 
									   FROM ixp_tables it 
									   INNER JOIN ixp_tables it2 ON it2.ixp_tables_name = 'ixp_source_deal_volume_update_template'
									   ...
									   WHERE it.ixp_tables_name = 'ixp_source_deal_volume_update_template' UNION ALL  SELECT @ixp_rules_id_new, it.ixp_tables_id, 'du.[volume]', ic.ixp_columns_id, NULL, NULL, 0, NULL, NULL 
									   FROM ixp_tables it 
									   INNER JOIN ixp_tables it2 ON it2.ixp_tables_name = 'ixp_source_deal_volume_update_template'
									   INNER JOIN ixp_columns ic ON ic.ixp_columns_name = 'deal_volume' AND ic.ixp_table_id = it2.ixp_tables_id AND (ic.header_detail = 'NULL' OR ic.header_detail IS NULL)
									   WHERE it.ixp_tables_name = 'ixp_source_deal_volume_update_template' UNION ALL  SELECT @ixp_rules_id_new, it.ixp_tables_id, 'du.[Actual Volume]', ic.ixp_columns_id, NULL, NULL, 0, NULL, NULL 
									   FROM ixp_tables it 
									   INNER JOIN ixp_tables it2 ON it2.ixp_tables_name = 'ixp_source_deal_volume_update_template'
									   ...
									   WHERE it.ixp_tables_name = 'ixp_source_deal_volume_update_template' UNION ALL  SELECT @ixp_rules_id_new, it.ixp_tables_id, 'du.[Schedule Volume]', ic.ixp_columns_id, NULL, NULL, 0, NULL, NULL 
									   FROM ixp_tables it 
									   INNER JOIN ixp_tables it2 ON it2.ixp_tables_name = 'ixp_source_deal_volume_update_template'
