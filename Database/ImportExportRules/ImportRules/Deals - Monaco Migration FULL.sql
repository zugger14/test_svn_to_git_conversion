BEGIN 
	BEGIN TRY 
		...
		DECLARE @ixp_rules_id_new INT
			...
			WHERE ixp_rule_hash = 'DE89E58B_460F_4DCF_A7BE_1CB00023C4E5'

			if @old_ixp_rule_id IS NULL
			BEGIN
				SELECT @old_ixp_rule_id = ixp_rules_id
			FROM ixp_rules ir
			WHERE ir.ixp_rules_name = 'Deals - Monaco Migration FULL'
			END
		...
		END
			...
			BEGIN
				...
				VALUES( 
					'Deals - Monaco Migration FULL' ,
					'N' ,
					NULL ,
					'IF OBJECT_ID(''tempdb..#temp_deal'') IS NOT NULL DROP TABLE #temp_deal
DECLARE @_process_id VARCHAR(500),@_delete_table VARCHAR(500),@_user_login_id VARCHAR(100)

SET @_process_id = dbo.FNAGetNewID()
SET @_user_login_id = dbo.FNAdbuser()
SET @_delete_table =  dbo.FNAProcessTableName(''delete_deals'', @_user_login_id,@_process_id)
EXEC(''CREATE TABLE ''+@_delete_table+''(source_deal_header_id INT, status VARCHAR(100),description VARCHAR(500))'') 

create table #temp_deal([deal id] VARCHAR(100) COLLATE DATABASE_DEFAULT)
INSERT INTO #temp_deal SELECT [deal id] FROM [EU-T-SQL01\INSTANCE2019].MONACO_MIGRATION.dbo.temp_import_deal_full_void
EXEC(''INSERT INTO ''+@_delete_table+''(source_deal_header_id) SELECT sdh.source_deal_header_id FROM source_deal_header sdh INNER JOIN #temp_deal t ON sdh.[deal_id] = t.[deal id]'')

EXEC spa_source_deal_header ''d'','''','''','''',@_process_id
--[final_process_table]
',
					'DELETE uddf
FROM [temp_process_table] TEMP
INNER JOIN source_deal_header_template sdht ON sdht.template_name = TEMP.template_id 
INNER JOIN source_deal_header sdh ON sdh.deal_id = TEMP.deal_id
INNER JOIN user_defined_deal_fields_template uddft ON uddft.template_id = sdht.template_id AND uddft.udf_type = ''h''
...
WHERE udft.Field_label IN (''Positive Price Commodity VAT'')  
	AND uddf.udf_deal_id IS NOT NULL
...
LEFT JOIN user_defined_deal_fields uddf ON uddf.source_deal_header_id = sdh.source_deal_header_id
	AND uddf.udf_template_id = uddft.udf_template_id
WHERE udft.Field_label IN (''Negative Price Commodity VAT'')
	AND uddf.udf_deal_id IS NOT NULL
...
LEFT JOIN user_defined_deal_fields uddf ON uddf.source_deal_header_id = sdh.source_deal_header_id
	AND uddf.udf_template_id = uddft.udf_template_id
WHERE udft.Field_label IN (''Commodity Energy Tax'')
	AND uddf.udf_deal_id IS NOT NULL
...
SELECT DISTINCT sdh.source_deal_header_id
	...
	,uddft.default_value
FROM [temp_process_table] TEMP
INNER JOIN source_deal_header_template sdht ON sdht.template_name = TEMP.template_id AND uddft.udf_type = ''h''
INNER JOIN source_deal_header sdh ON sdh.deal_id = TEMP.deal_id
INNER JOIN user_defined_deal_fields_template uddft ON uddft.template_id = sdht.template_id
INNER JOIN user_defined_fields_template udft ON udft.field_name = uddft.field_name
LEFT JOIN user_defined_deal_fields uddf ON uddf.source_deal_header_id = sdh.source_deal_header_id
	AND uddf.udf_template_id = uddft.udf_template_id
WHERE udft.Field_label IN (''Commodity Energy Tax'')
	AND uddf.udf_deal_id IS NULL',
					...
					23502,
					1,
					'DE89E58B_460F_4DCF_A7BE_1CB00023C4E5'
					 )
		...
		BEGIN
			...
			UPDATE
			ixp_rules
			SET ixp_rules_name = 'Deals - Monaco Migration FULL'
				, individuals_script_per_ojbect = 'N'
				, limit_rows_to = NULL
				, before_insert_trigger = 'IF OBJECT_ID(''tempdb..#temp_deal'') IS NOT NULL DROP TABLE #temp_deal
DECLARE @_process_id VARCHAR(500),@_delete_table VARCHAR(500),@_user_login_id VARCHAR(100)
...
--[final_process_table]
'
				, after_insert_trigger = 'DELETE uddf
FROM [temp_process_table] TEMP
INNER JOIN source_deal_header_template sdht ON sdht.template_name = TEMP.template_id 
INNER JOIN source_deal_header sdh ON sdh.deal_id = TEMP.deal_id
INNER JOIN user_defined_deal_fields_template uddft ON uddft.template_id = sdht.template_id AND uddft.udf_type = ''h''
...
INSERT INTO ixp_import_data_source (rules_id, data_source_type, connection_string, data_source_location, destination_table, delimiter, source_system_id, data_source_alias, is_customized, customizing_query, is_header_less, no_of_columns, folder_location, custom_import, use_parameter
					...
					SELECT @ixp_rules_id_new,
						   NULL,
						   '[EU-T-SQL01\INSTANCE2019].MONACO_MIGRATION.dbo.temp_import_deal_full_new',
...
INSERT INTO ixp_import_data_mapping(ixp_rules_id, dest_table_id, source_column_name, dest_column, column_function, column_aggregation, repeat_number, where_clause ,udf_field_id)   SELECT @ixp_rules_id_new, it.ixp_tables_id, 'd.[Deal ID]', ic.ixp_columns_id, NULL, NULL, 0, NULL, NULL 
									   ...
									   WHERE it.ixp_tables_name = 'ixp_source_deal_template' UNION ALL  SELECT @ixp_rules_id_new, it.ixp_tables_id, 'd.[Internal Deal Type]', ic.ixp_columns_id, NULL, NULL, 0, NULL, NULL 
									   FROM ixp_tables it 
									   INNER JOIN ixp_tables it2 ON it2.ixp_tables_name = 'ixp_source_deal_template'
									   ...
									   WHERE it.ixp_tables_name = 'ixp_source_deal_template' UNION ALL  SELECT @ixp_rules_id_new, it.ixp_tables_id, 'd.[Internal Deal Subtype]', ic.ixp_columns_id, NULL, NULL, 0, NULL, NULL 
									   FROM ixp_tables it 
									   INNER JOIN ixp_tables it2 ON it2.ixp_tables_name = 'ixp_source_deal_template'
									   ...
									   WHERE it.ixp_tables_name = 'ixp_source_deal_template' UNION ALL  SELECT @ixp_rules_id_new, it.ixp_tables_id, 'd.[Premium Fees]', ic.ixp_columns_id, NULL, NULL, 0, NULL, NULL 
									   FROM ixp_tables it 
									   INNER JOIN ixp_tables it2 ON it2.ixp_tables_name = 'ixp_source_deal_template'
									   ...
									   WHERE it.ixp_tables_name = 'ixp_source_deal_template' UNION ALL  SELECT @ixp_rules_id_new, it.ixp_tables_id, 'd.[Broker Fees]', ic.ixp_columns_id, NULL, NULL, 0, NULL, NULL 
									   FROM ixp_tables it 
									   INNER JOIN ixp_tables it2 ON it2.ixp_tables_name = 'ixp_source_deal_template'
									   ...
									   WHERE it.ixp_tables_name = 'ixp_source_deal_template' UNION ALL  SELECT @ixp_rules_id_new, it.ixp_tables_id, 'd.[Clearing Fees]', ic.ixp_columns_id, NULL, NULL, 0, NULL, NULL 
									   FROM ixp_tables it 
									   INNER JOIN ixp_tables it2 ON it2.ixp_tables_name = 'ixp_source_deal_template'
									   ...
									   WHERE it.ixp_tables_name = 'ixp_source_deal_template' UNION ALL  SELECT @ixp_rules_id_new, it.ixp_tables_id, 'd.[Trans Fees]', ic.ixp_columns_id, NULL, NULL, 0, NULL, NULL 
									   FROM ixp_tables it 
									   INNER JOIN ixp_tables it2 ON it2.ixp_tables_name = 'ixp_source_deal_template'
