BEGIN 
	BEGIN TRY 
		...
		DECLARE @ixp_rules_id_new INT
			...
			WHERE ixp_rule_hash = 'A185CCCF_B1C4_4569_9918_2B66EE5C0345'

			if @old_ixp_rule_id IS NULL
			BEGIN
				SELECT @old_ixp_rule_id = ixp_rules_id
			FROM ixp_rules ir
			WHERE ir.ixp_rules_name = 'UOM Definition'
			END
		...
		END
			...
			BEGIN
				...
				VALUES( 
					...
					23501,
					1,
					'A185CCCF_B1C4_4569_9918_2B66EE5C0345'
					 )
		...
		BEGIN
			...
			UPDATE
			ixp_rules
			SET ixp_rules_name = 'UOM Definition'
...
INSERT INTO ixp_import_data_source (rules_id, data_source_type, connection_string, data_source_location, destination_table, delimiter, source_system_id, data_source_alias, is_customized, customizing_query, is_header_less, no_of_columns, folder_location, custom_import, use_parameter, excel_sheet, ssis_package, soap_function_id, is_ftp, ftp_url, ftp_username, ftp_password, clr_function_id, ws_function_name, use_sftp, enable_email_import, send_email_import_reply)
					SELECT @ixp_rules_id_new,
						   21400,
						   '',
						   '\\PSSJCDEV01\shared_docs_TRMTracker_ATCO\temp_Note\UOM Definition.csv',
						   NULL,
						   ',',
						   2,
						   'UOM',
...
INSERT INTO ixp_import_data_mapping(ixp_rules_id, dest_table_id, source_column_name, dest_column, column_function, column_aggregation, repeat_number, where_clause ,udf_field_id)   SELECT @ixp_rules_id_new, it.ixp_tables_id, 'UOM.[uom id]', ic.ixp_columns_id, NULL, NULL, 0, NULL, NULL 
									   FROM ixp_tables it 
									   INNER JOIN ixp_tables it2 ON it2.ixp_tables_name = 'ixp_source_uom_template'
									   ...
									   WHERE it.ixp_tables_name = 'ixp_source_uom_template' UNION ALL  SELECT @ixp_rules_id_new, it.ixp_tables_id, 'UOM.[name]', ic.ixp_columns_id, NULL, NULL, 0, NULL, NULL 
									   FROM ixp_tables it 
									   INNER JOIN ixp_tables it2 ON it2.ixp_tables_name = 'ixp_source_uom_template'
									   INNER JOIN ixp_columns ic ON ic.ixp_columns_name = 'uom_name' AND ic.ixp_table_id = it2.ixp_tables_id AND (ic.header_detail = 'NULL' OR ic.header_detail IS NULL)
									   WHERE it.ixp_tables_name = 'ixp_source_uom_template' UNION ALL  SELECT @ixp_rules_id_new, it.ixp_tables_id, 'UOM.[description]', ic.ixp_columns_id, NULL, NULL, 0, NULL, NULL 
									   FROM ixp_tables it 
									   INNER JOIN ixp_tables it2 ON it2.ixp_tables_name = 'ixp_source_uom_template'
									   INNER JOIN ixp_columns ic ON ic.ixp_columns_name = 'uom_desc' AND ic.ixp_table_id = it2.ixp_tables_id AND (ic.header_detail = 'NULL' OR ic.header_detail IS NULL)
									   WHERE it.ixp_tables_name = 'ixp_source_uom_template'
