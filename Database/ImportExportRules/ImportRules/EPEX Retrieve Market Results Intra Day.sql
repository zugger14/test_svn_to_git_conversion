BEGIN 
	BEGIN TRY 
		...
		DECLARE @ixp_rules_id_new INT
			...
			WHERE ixp_rule_hash = '68CCFA17_5294_45A8_B619_EA96A079EA7B'

			if @old_ixp_rule_id IS NULL
			BEGIN
				SELECT @old_ixp_rule_id = ixp_rules_id
			FROM ixp_rules ir
			WHERE ir.ixp_rules_name = 'EPEX Retrieve Market Results Intra Day'
			END
		...
		END
			...
			BEGIN
				...
				VALUES( 
					'EPEX Retrieve Market Results Intra Day' ,
					'N' ,
					NULL ,
					'DELETE udt FROM udt_epex_market_results_intraday udt 
INNER JOIN [temp_process_table] a
INNER JOIN static_data_value sdv_area ON sdv_area.code = a.[Area] AND sdv_area.[type_id] = 112500
ON sdv_area.value_id = udt.area AND a.[date] = udt.utc_date AND a.participant = udt.participant
...
INSERT INTO udt_epex_market_results_intraday
(
	area
	,participant
	,utc_date
...
SELECT 
	...
	, DATEPART(MINUTE, [dbo].[FNAGetLOCALTime]([Date], @default_code_value)) 
...
--UPDATE udt_epex_market_results_intraday
--SET std_date = ''2021-10-31 02:15:00.000'', hour = 2
--WHERE utc_date = ''2021-10-31T00:15:00Z'' AND std_date = ''2021-10-31 01:15:00.000''

--UPDATE udt_epex_market_results_intraday
--set std_date = ''2021-10-31 02:30:00.000'', hour = 2
--WHERE utc_date = ''2021-10-31T00:30:00Z'' AND std_date = ''2021-10-31 01:30:00.000''

--UPDATE udt_epex_market_results_intraday
--SET std_date = ''2021-10-31 02:45:00.000'', hour = 2
--WHERE utc_date = ''2021-10-31T00:45:00Z'' AND std_date = ''2021-10-31 01:45:00.000''


--SET @run_date = ''2021-10-31''
SELECT market_results_id, area, utc_date
,mcp
,mcv
,total_sched_net
,total_sched_purchase
...
INTO #udt_epex_market_results_intraday
FROM udt_epex_market_results_intraday WHERE CAST(std_date AS DATE) = @run_date

UPDATE a
SET [hour] = 24		
FROM #udt_epex_market_results_intraday a
INNER JOIN (
SELECT ROW_NUMBER() OVER (PARTITION BY area, area_set, auction_name, participant, std_date  
ORDER BY market_results_id) rn, market_results_id  FROM #udt_epex_market_results_intraday
) b ON b.market_results_id = a.market_results_id AND rn =2
...
DECLARE profile_cursor CURSOR FOR
	SELECT DISTINCT a.auction_name, a.participant, a.column_name,  a.profile_id, a.curve_id 
	FROM #udt_epex_market_results_intraday upmr
	OUTER APPLY (
...
BEGIN
	SET @sql = ''			
			...
			FROM (
			SELECT auction_name, CAST(std_date AS DATE) std_date, '' + @column_name + '', [hour], [min],participant
				FROM #udt_epex_market_results_intraday 
			) p
			PIVOT
			(
			  SUM('' + @column_name + '')
	...
	SET @sql += '' 
			...
			FROM #udt_epex_market_results_intraday piv WHERE 1 = 1		
...
',
					...
					23500,
					1,
					'68CCFA17_5294_45A8_B619_EA96A079EA7B'
					 )
		...
		BEGIN
			...
			UPDATE
			ixp_rules
			SET ixp_rules_name = 'EPEX Retrieve Market Results Intra Day'
				, individuals_script_per_ojbect = 'N'
				, limit_rows_to = NULL
				, before_insert_trigger = 'DELETE udt FROM udt_epex_market_results_intraday udt 
INNER JOIN [temp_process_table] a
INNER JOIN static_data_value sdv_area ON sdv_area.code = a.[Area] AND sdv_area.[type_id] = 112500
ON sdv_area.value_id = udt.area AND a.[date] = udt.utc_date AND a.participant = udt.participant
...
BEGIN
	EXEC spa_build_epex_data_table @process_table, ''DE-TPS-15min'', @response 
END
EXEC(''
SELECT * 
--[__custom_table__]
