BEGIN BEGIN TRY 
			...
			WHERE ixp_rule_hash = '0F1D4BE6_10F0_433D_A6DA_409B4826BF42'

			if @old_ixp_rule_id IS NULL
			BEGIN
				SELECT @old_ixp_rule_id = ixp_rules_id
			FROM ixp_rules ir
			WHERE ir.ixp_rules_name = 'Static Data'
			END
		...
		END
			...
			BEGIN
				...
				VALUES( 
					...
					23501,
					1,
					'0F1D4BE6_10F0_433D_A6DA_409B4826BF42'
					 )
		...
		BEGIN
			...
			UPDATE
			ixp_rules
			SET ixp_rules_name = 'Static Data'
...
INSERT INTO ixp_import_data_source (rules_id, data_source_type, connection_string, data_source_location, destination_table, delimiter, source_system_id, data_source_alias, is_customized, customizing_query, is_header_less, no_of_columns, folder_location, custom_import, use_parameter, excel_sheet, ssis_package, soap_function_id, is_ftp, ftp_url, ftp_username, ftp_password, clr_function_id, ws_function_name)
					SELECT @ixp_rules_id_new,
						   21405,
						   '',
						   '\\SG-S-STG01\shared_docs_TRMTracker_PROD\temp_Note\0',
						   NULL,
						   ',',
						   2,
						   'sd',
						   ...
						   '',
						   '',
						   0x01000000ABD345DE382CED8B59CD9EC1E5DCCB4A3DCE5ADA51A72D3F,
...
INSERT INTO ixp_import_data_mapping(ixp_rules_id, dest_table_id, source_column_name, dest_column, column_function, column_aggregation, repeat_number, where_clause )   SELECT @ixp_rules_id_new, it.ixp_tables_id, 'sd.[Data Type]', ic.ixp_columns_id, NULL, NULL, 0, NULL 
									   FROM ixp_tables it 
									   INNER JOIN ixp_tables it2 ON it2.ixp_tables_name = 'ixp_static_data_value_template'
									   INNER JOIN ixp_columns ic ON ic.ixp_columns_name = 'type_id' AND ic.ixp_table_id = it2.ixp_tables_id AND (ic.header_detail = 'NULL' OR ic.header_detail IS NULL)
									   WHERE it.ixp_tables_name = 'ixp_static_data_value_template' UNION ALL  SELECT @ixp_rules_id_new, it.ixp_tables_id, 'sd.[Code]', ic.ixp_columns_id, NULL, NULL, 0, NULL 
									   FROM ixp_tables it 
									   INNER JOIN ixp_tables it2 ON it2.ixp_tables_name = 'ixp_static_data_value_template'
									   ...
									   WHERE it.ixp_tables_name = 'ixp_static_data_value_template' UNION ALL  SELECT @ixp_rules_id_new, it.ixp_tables_id, 'sd.[Description]', ic.ixp_columns_id, NULL, NULL, 0, NULL 
									   FROM ixp_tables it 
									   INNER JOIN ixp_tables it2 ON it2.ixp_tables_name = 'ixp_static_data_value_template'
