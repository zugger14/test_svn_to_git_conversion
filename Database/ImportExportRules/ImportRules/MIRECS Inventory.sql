BEGIN 
	BEGIN TRY 
		...
		DECLARE @ixp_rules_id_new INT
			...
			WHERE ixp_rule_hash = 'B54FC358_B0AE_41CD_B220_F6FE0F3D68C7'

			if @old_ixp_rule_id IS NULL
			BEGIN
				SELECT @old_ixp_rule_id = ixp_rules_id
			FROM ixp_rules ir
			WHERE ir.ixp_rules_name = 'MIRECS Inventory'
			END
...
/******************/',
					...
					23502,
					1,
					'B54FC358_B0AE_41CD_B220_F6FE0F3D68C7'
					 )
		...
		BEGIN
			...
			UPDATE
			ixp_rules
			SET ixp_rules_name = 'MIRECS Inventory'
...
SELECT [Mirecs id] generator,  
	   [Month] vintage_month,
	   [YEAR] vintage_year,
	   [Transferor] AS counterparty,
	   [Credit Serial Numbers],
	   LTRIM(RTRIM(REVERSE(
		   LEFT( 
			   REVERSE(
					 SUBSTRING([Credit Serial Numbers], 
							0, 
							CHARINDEX(''to'', [Credit Serial Numbers])
				     )
			   ),
			   CHARINDEX(''-'',
						 REVERSE(
	   ...
	   LTRIM(RTRIM(SUBSTRING([Credit Serial Numbers], CHARINDEX(''to'', [Credit Serial Numbers]) + 2, 16))) certificate_seq_to,
	   [Quantity] AS volume,
	   [Date of Transfer] AS issue_date,
	   ''0'' price,
	   [Project] [facility_name],
	   ''MI'' [generation_state],
	   [Fuel/Project Type] [technology],
	   [action],
	   LTRIM(RTRIM(REVERSE(
		  LEFT(
			  SUBSTRING(REVERSE(SUBSTRING([Credit Serial Numbers], 
										  0, 
										  CHARINDEX(''to'', [Credit Serial Numbers])
								)
						), 
						CHARINDEX(''-'', REVERSE(SUBSTRING([Credit Serial Numbers], 0, CHARINDEX(''to'', [Credit Serial Numbers])))) + 1,
						50
			  ),
			  CHARINDEX(''-'',
						SUBSTRING(REVERSE(SUBSTRING([Credit Serial Numbers], 
	   ...
	   LTRIM(RTRIM(
		 SUBSTRING(
			SUBSTRING([Credit Serial Numbers], 
					  CHARINDEX(''-'',[Credit Serial Numbers]) + 1, 
					  50
			), 
			0,
			CHARINDEX(''-'', 
...
INTO #temp_table  
--FROM adiha_process.dbo.temp_import_data_table_ri_EE50272B_8D18_4758_A8A7_E591610A7F96
...
SELECT [Generator] 
	  ...
	  , SUBSTRING([Credit Serial Numbers], 
				  0,
				  CHARINDEX([source_certificate_number], [Credit Serial Numbers]) - 1
		) + ''-'' + [source_certificate_number] + ''-'' + [certificate_seq_from] [Certificate Serial Numbers From]
	  , SUBSTRING([Credit Serial Numbers], 
				  0,
				  CHARINDEX([source_certificate_number], [Credit Serial Numbers]) - 1
		) + ''-'' + [source_certificate_number] + ''-'' + [certificate_seq_to]  [Certificate Serial Numbers To]   
	  , volume							[Volume]    
	  , price							[price]
...
INSERT INTO ixp_import_data_mapping(ixp_rules_id, dest_table_id, source_column_name, dest_column, column_function, column_aggregation, repeat_number, where_clause ,udf_field_id)   SELECT @ixp_rules_id_new, it.ixp_tables_id, 'ri.[Generator]', ic.ixp_columns_id, NULL, NULL, 0, NULL, NULL 
									   ...
									   WHERE it.ixp_tables_name = 'ixp_rec_inventory' UNION ALL  SELECT @ixp_rules_id_new, it.ixp_tables_id, '', ic.ixp_columns_id, '''MIRECS''', 'Max', 0, NULL, NULL 
									   FROM ixp_tables it 
									   INNER JOIN ixp_tables it2 ON it2.ixp_tables_name = 'ixp_rec_inventory'
