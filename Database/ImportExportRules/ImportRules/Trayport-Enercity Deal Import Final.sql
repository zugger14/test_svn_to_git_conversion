...
FROM [final_process_table] temp
INNER JOIN mv90_dst d2 ON CAST(d2.date AS DATE) = CAST(temp.[deal_date] AS DATE) AND d2.insert_delete = ''i'' AND d2.dst_group_value_id = 102201
INNER JOIN generic_mapping_values gmv 
	ON gmv.mapping_table_id = @mapping_table_id 
	AND gmv.clm1_value = CASE WHEN temp.curve_id IN (''NCG L - WEST EEX'',''NCG L - EAST EEX'',''THE L West (Hour) EEX'',''THE L East (Hour) EEX'') THEN IIF(CHARINDEX('' '',RTRIM(LTRIM(temp.term_start))) = 0, RTRIM(LTRIM(temp.term_start)),SUBSTRING(RTRIM(LTRIM(temp.term_start)), CHARINDEX('' '',RTRIM(LTRIM(temp.term_start))) + 1 , LEN(RTRIM(LTRIM(temp.term_start))))) ELSE temp.term_start END 
	AND gmv.clm3_value = CAST(DATEPART(HH, dbo.FNAGetLOCALTime(CAST(temp.[deal_date] AS DATETIME),15)) AS VARCHAR(10)) 
...
INNER JOIN static_data_value sdv
    ON sdv.value_id = gmv.clm4_value
AND gmv.clm3_value = CAST(DATEPART(HH, dbo.FNAGetLOCALTime(CAST(t.[deal_date] AS DATETIME),15)) AS VARCHAR(10))
WHERE gmv.clm1_value = ''WD'' AND clm5_value IS NULL

UPDATE temp
	SET temp.[block_define_id] = sdv.code
FROM [final_process_table] temp
INNER JOIN mv90_dst d1 ON CAST(d1.date AS DATE) = CAST(temp.[deal_date] AS DATE) AND d1.insert_delete = ''d'' AND d1.dst_group_value_id = 102201
INNER JOIN generic_mapping_values gmv
	ON gmv.mapping_table_id = @mapping_table_id
	AND gmv.clm1_value = CASE WHEN temp.curve_id IN (''NCG L - WEST EEX'',''NCG L - EAST EEX'',''THE L West (Hour) EEX'',''THE L East (Hour) EEX'') THEN IIF(CHARINDEX('' '',RTRIM(LTRIM(temp.term_start))) = 0, RTRIM(LTRIM(temp.term_start)),SUBSTRING(RTRIM(LTRIM(temp.term_start)), CHARINDEX('' '',RTRIM(LTRIM(temp.term_start))) + 1 , LEN(RTRIM(LTRIM(temp.term_start))))) ELSE temp.term_start END 
	AND gmv.clm3_value = CAST(DATEPART(HH, dbo.FNAGetLOCALTime(CAST(temp.[deal_date] AS DATETIME),15)) AS VARCHAR(10)) 
...
SET deal_date = CAST(deal_date AS DATE),
		...
		term_end = CAST(CASE    WHEN ISNULL(t1.FirstSequenceItemName,t1.SecondSequenceItemName) = ''Saturday'' AND DATEPART(dw, temp.deal_date) = 7 AND DATEPART(hour,temp.deal_date) < 3
								...
								WHEN ISNULL(t1.FirstSequenceItemName,t1.SecondSequenceItemName) = ''W/END'' AND DATEPART(dw, temp.deal_date) = 7 AND DATEPART(hour,temp.deal_date) < 3
										THEN deal_date
								WHEN ISNULL(t1.FirstSequenceItemName,t1.SecondSequenceItemName) = ''W/END'' AND DATEPART(dw, temp.deal_date) = 7 THEN DATEADD(dd, 8,deal_date)
								WHEN ISNULL(t1.FirstSequenceItemName,t1.SecondSequenceItemName) = ''W/END'' THEN DATEADD(dd, 8-(DATEPART(dw, deal_date)), deal_date)
						  ELSE term_end END
					 AS DATE)
FROM [final_process_table] temp
INNER JOIN [temp_process_table] t1
...
INNER JOIN generic_mapping_values gmv
ON gmv.mapping_table_id = @mapping_table_id
AND gmv.clm1_value = ''WD''
AND gmv.clm3_value = 1
...
END', NULL, 0, NULL, ISNULL(CAST(sdv.value_id AS VARCHAR(200)),'Missing udf - ''' + 'Sleeve' + '''')  
				...
				LEFT JOIN user_defined_fields_template udft ON udft.field_id = sdv.value_id
				WHERE it.ixp_tables_name = 'ixp_source_deal_template' UNION ALL  SELECT @ixp_rules_id_new, it.ixp_tables_id, 'tdi.[OrderID]', ic.ixp_columns_id, NULL, NULL, 0, NULL, ISNULL(CAST(sdv.value_id AS VARCHAR(200)),'Missing udf - ''' + 'OrderID' + '''')  
				FROM ixp_tables it 
				INNER JOIN ixp_tables it2 ON it2.ixp_tables_name = 'ixp_source_deal_template'
				...
				LEFT JOIN static_data_value sdv ON sdv.type_id = 5500 AND sdv.code =  'OrderID'									   
				LEFT JOIN user_defined_fields_template udft ON udft.field_id = sdv.value_id
				WHERE it.ixp_tables_name = 'ixp_source_deal_template'
