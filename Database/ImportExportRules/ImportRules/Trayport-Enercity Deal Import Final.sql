BEGIN 
	BEGIN TRY 
		...
		DECLARE @ixp_rules_id_new INT
			...
			WHERE ixp_rule_hash = '1C668AFD_E3AE_4344_AF42_EB23F4BD8B59'
...
SELECT DISTINCT @set_process_id
	, ''ixp_source_deal_template''
	, ''Missing Value''
	, ''Generic mapping not found for TermCode: '' + temp.term_start + '' AND DealTimeHour: '' + CAST(DATEPART(HH, temp.[deal_date]) AS VARCHAR(10)) + '' for Deal : '' + temp.deal_id
	, ''Error''
FROM [final_process_table] temp
INNER JOIN generic_mapping_values gmv0 ON gmv0.mapping_table_id = @mapping_table_id AND gmv0.clm1_value = temp.term_start
...
LEFT JOIN generic_mapping_values gmv 
	...
	AND gmv.clm1_value = temp.[term_start] 
	AND gmv.clm3_value = CAST(DATEPART(HH, CAST(temp.[deal_date] AS DATETIME)) AS VARCHAR(10)) 
...
INNER JOIN generic_mapping_values gmv
    ...
    AND gmv.clm1_value = CASE WHEN t.[term_start] like ''%weekend%'' 
							...
							OR t.[term_start] like ''%sunday%''
							OR t.[term_start] like ''%saturday%''
							OR t.[term_start] like ''%sun%''
							OR t.[term_start] like ''%sat%''
							THEN ''WkEnd'' 
					   ELSE NULL END
INNER JOIN static_data_value sdv
	ON sdv.value_id = gmv.clm4_value
...
UPDATE t
	SET t.curve_id = ISNULL(spcd.curve_id,t.curve_id)
		,t.template_id = ISNULL(sdht.template_name,t.template_id)
...
FROM [final_process_table] t
INNER JOIN generic_mapping_values gmv
    ON ISNULL(t.sub_book, '''') = ISNULL(gmv.clm3_value, '''')
	AND ISNULL(t.commodity_id, '''') = ISNULL(gmv.clm2_value, '''')
	AND ISNULL(t.trader_id, '''') = ISNULL(gmv.clm1_value, '''')
INNER join generic_mapping_header gmh
	ON gmv.mapping_table_id = gmh.mapping_table_id
...
LEFT JOIN generic_mapping_values gmv 
	...
	AND  ISNULL(gmv.clm1_value,1) = CASE WHEN scm.commodity_id = ''Power'' THEN ISNULL(gmv.clm1_value,1) ELSE t1.[InstName] END
...
--insert into sk(type,name) values(''row_count'', @row_count)
--insert into sk(type,name) values(''hyperlink_row_count'', @hyperlink_row_count)
...
-- END',
					...
					23502,
					1,
					'1C668AFD_E3AE_4344_AF42_EB23F4BD8B59'
					 )
...
INNER JOIN contract_group cg
	ON CAST(cg.contract_id AS VARCHAR(20)) = gmv.clm3_value'
				, after_insert_trigger = 'EXEC('' INSERT INTO process_deal_alert_transfer_adjust(source_deal_header_id, source_deal_detail_id, create_user, create_ts, process_status, process_id)
			   SELECT DISTINCT source_deal_header_id,
					  sdd.deal_detail_id,
					  dbo.FNADBUser(),
					  GETDATE(),
...
INSERT INTO ixp_import_data_mapping(ixp_rules_id, dest_table_id, source_column_name, dest_column, column_function, column_aggregation, repeat_number, where_clause ,udf_field_id)   SELECT @ixp_rules_id_new, it.ixp_tables_id, 'tdi.[TradeID]', ic.ixp_columns_id, NULL, NULL, 0, NULL, NULL 
									   ...
									   WHERE it.ixp_tables_name = 'ixp_source_deal_template' UNION ALL  SELECT @ixp_rules_id_new, it.ixp_tables_id, 'tdi.[DateTime]', ic.ixp_columns_id, NULL, NULL, 0, NULL, NULL 
									   FROM ixp_tables it 
									   INNER JOIN ixp_tables it2 ON it2.ixp_tables_name = 'ixp_source_deal_template'
...
WHEN tdi.[Unit]=''000''''MT'' THEN ''MT'' 
									   ...
									   WHERE it.ixp_tables_name = 'ixp_source_deal_template' UNION ALL  SELECT @ixp_rules_id_new, it.ixp_tables_id, '', ic.ixp_columns_id, '''default_location''', 'Max', 0, NULL, NULL 
									   FROM ixp_tables it 
									   INNER JOIN ixp_tables it2 ON it2.ixp_tables_name = 'ixp_source_deal_template'
									   ...
									   WHERE it.ixp_tables_name = 'ixp_source_deal_template' UNION ALL  SELECT @ixp_rules_id_new, it.ixp_tables_id, '', ic.ixp_columns_id, '''111111''', 'Max', 0, NULL, ISNULL(CAST(sdv.value_id AS VARCHAR(200)),'Missing udf - ''' + 'Product group' + '''')  
				FROM ixp_tables it 
				INNER JOIN ixp_tables it2 ON it2.ixp_tables_name = 'ixp_source_deal_template'
				...
				WHERE it.ixp_tables_name = 'ixp_source_deal_template' UNION ALL  SELECT @ixp_rules_id_new, it.ixp_tables_id, '', ic.ixp_columns_id, '''222222''', 'Max', 0, NULL, ISNULL(CAST(sdv.value_id AS VARCHAR(200)),'Missing udf - ''' + 'Delivery Path' + '''')  
				FROM ixp_tables it 
				INNER JOIN ixp_tables it2 ON it2.ixp_tables_name = 'ixp_source_deal_template'
