IF NOT EXISTS(SELECT 1 FROM ixp_rules ir WHERE ir.ixp_rules_name = 'Alert_Conditions') BEGIN BEGIN TRY BEGIN TRAN
...
INSERT INTO ixp_export_tables (ixp_rules_id, table_id, dependent_table_id, sequence_number, dependent_table_order, repeat_number)  
							   SELECT @ixp_rules_id_new,
								...
								WHERE it.ixp_tables_name = 'ixp_alert_conditions'
...
INSERT INTO #old_relation(ixp_export_relation_id, ixp_rules_id, from_data_source, to_data_source, from_column, to_column, data_source)          SELECT          1288,         @ixp_rules_id_new,         1588,         1587,         'alert_sql_id',         'rules_id',         1588
 ...
 INSERT INTO ixp_data_mapping (ixp_rules_id, table_id, column_name, column_function, column_aggregation, column_filter, insert_type, enable_identity_insert, create_destination_table, source_column, export_folder, export_delim, generate_script, column_alias, main_table )  SELECT @ixp_rules_id_new, 
								         it.ixp_tables_id, 'rules_id', NULL, NULL, NULL, NULL, NULL, NULL, 'ASQ.[alert_sql_id]', '\\lhotse\users\export\', NULL, 'y', 'rules_id', 
										ieds.ixp_export_data_source_id
								FROM ixp_tables it
								LEFT JOIN ixp_exportable_table iet ON iet.ixp_exportable_table_name = 'alert_conditions'
								LEFT JOIN ixp_export_data_source ieds ON ieds.export_table = iet.ixp_exportable_table_id AND ieds.ixp_rules_id = @ixp_rules_id_new AND ieds.export_table_alias = 'AC'
								WHERE it.ixp_tables_name = 'ixp_alert_conditions'
								 UNION ALL  SELECT @ixp_rules_id_new, 
								         it.ixp_tables_id, 'alert_conditions_name', NULL, NULL, NULL, NULL, NULL, NULL, 'AC.[alert_conditions_name]', '\\lhotse\users\export\', NULL, 'y', 'alert_conditions_name', 
										ieds.ixp_export_data_source_id
								FROM ixp_tables it
								LEFT JOIN ixp_exportable_table iet ON iet.ixp_exportable_table_name = 'alert_conditions'
								LEFT JOIN ixp_export_data_source ieds ON ieds.export_table = iet.ixp_exportable_table_id AND ieds.ixp_rules_id = @ixp_rules_id_new AND ieds.export_table_alias = 'AC'
								WHERE it.ixp_tables_name = 'ixp_alert_conditions'
								 UNION ALL  SELECT @ixp_rules_id_new, 
								         it.ixp_tables_id, 'alert_conditions_description', NULL, NULL, NULL, NULL, NULL, NULL, 'AC.[alert_conditions_description]', '\\lhotse\users\export\', NULL, 'y', 'alert_conditions_description', 
										ieds.ixp_export_data_source_id
								FROM ixp_tables it
								LEFT JOIN ixp_exportable_table iet ON iet.ixp_exportable_table_name = 'alert_conditions'
								LEFT JOIN ixp_export_data_source ieds ON ieds.export_table = iet.ixp_exportable_table_id AND ieds.ixp_rules_id = @ixp_rules_id_new AND ieds.export_table_alias = 'AC'
