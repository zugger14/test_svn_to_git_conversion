IF NOT EXISTS(SELECT 1 FROM ixp_rules ir WHERE ir.ixp_rules_name = 'Alert_Rule_Table') BEGIN BEGIN TRY BEGIN TRAN
...
INSERT INTO ixp_export_tables (ixp_rules_id, table_id, dependent_table_id, sequence_number, dependent_table_order, repeat_number)  
							   SELECT @ixp_rules_id_new,
								...
								WHERE it.ixp_tables_name = 'ixp_alert_rule_table'
...
CREATE TABLE #old_relation (ixp_export_relation_id INT, ixp_rules_id INT, from_data_source INT, to_data_source INT, from_column VARCHAR(1000), to_column varchar(1000), data_source INT)
 ...
 INSERT INTO ixp_data_mapping (ixp_rules_id, table_id, column_name, column_function, column_aggregation, column_filter, insert_type, enable_identity_insert, create_destination_table, source_column, export_folder, export_delim, generate_script, column_alias, main_table )  SELECT @ixp_rules_id_new, 
								         it.ixp_tables_id, 'alert_id', NULL, NULL, NULL, NULL, NULL, NULL, 'ART.[alert_id]', '\\lhotse\users\export\', NULL, 'y', 'alert_id', 
										ieds.ixp_export_data_source_id
								FROM ixp_tables it
								LEFT JOIN ixp_exportable_table iet ON iet.ixp_exportable_table_name = 'alert_rule_table'
								LEFT JOIN ixp_export_data_source ieds ON ieds.export_table = iet.ixp_exportable_table_id AND ieds.ixp_rules_id = @ixp_rules_id_new AND ieds.export_table_alias = 'ART'
								WHERE it.ixp_tables_name = 'ixp_alert_rule_table'
								 UNION ALL  SELECT @ixp_rules_id_new, 
								         it.ixp_tables_id, 'table_id', NULL, NULL, NULL, NULL, NULL, NULL, 'ATD.[alert_table_definition_id]', '\\lhotse\users\export\', NULL, 'y', 'table_id', 
										ieds.ixp_export_data_source_id
								FROM ixp_tables it
								LEFT JOIN ixp_exportable_table iet ON iet.ixp_exportable_table_name = 'alert_rule_table'
								LEFT JOIN ixp_export_data_source ieds ON ieds.export_table = iet.ixp_exportable_table_id AND ieds.ixp_rules_id = @ixp_rules_id_new AND ieds.export_table_alias = 'ART'
								WHERE it.ixp_tables_name = 'ixp_alert_rule_table'
								 UNION ALL  SELECT @ixp_rules_id_new, 
								         it.ixp_tables_id, 'root_table_id', NULL, NULL, NULL, NULL, NULL, NULL, 'ART.[root_table_id]', '\\lhotse\users\export\', NULL, 'y', 'root_table_id', 
										ieds.ixp_export_data_source_id
								FROM ixp_tables it
								LEFT JOIN ixp_exportable_table iet ON iet.ixp_exportable_table_name = 'alert_rule_table'
								LEFT JOIN ixp_export_data_source ieds ON ieds.export_table = iet.ixp_exportable_table_id AND ieds.ixp_rules_id = @ixp_rules_id_new AND ieds.export_table_alias = 'ART'
								WHERE it.ixp_tables_name = 'ixp_alert_rule_table'
								 UNION ALL  SELECT @ixp_rules_id_new, 
								         it.ixp_tables_id, 'table_alias', NULL, NULL, NULL, NULL, NULL, NULL, 'ART.[table_alias]', '\\lhotse\users\export\', NULL, 'y', 'table_alias', 
										ieds.ixp_export_data_source_id
								FROM ixp_tables it
								LEFT JOIN ixp_exportable_table iet ON iet.ixp_exportable_table_name = 'alert_rule_table'
								LEFT JOIN ixp_export_data_source ieds ON ieds.export_table = iet.ixp_exportable_table_id AND ieds.ixp_rules_id = @ixp_rules_id_new AND ieds.export_table_alias = 'ART'
