...
DECLARE @value_name VARCHAR(1000) = 'Operator'
DECLARE @value_id INT = 303416
-- insert operator
--static data type
...
BEGIN 
	INSERT INTO user_defined_fields_template(field_name
											...
											, field_id
											, sql_string)
	SELECT @value_id, @value_name, 'd', 'varchar(MAX)', 'n', 'h', 120, @value_id, 'SELECT source_counterparty_id, counterparty_name FROM source_counterparty'
END 
...
SELECT @application_ui_field_id = application_ui_field_id FROM application_ui_template_definition WHERE field_id = 'Operator' AND farrms_field_id = 'Operator' AND default_label = 'Operator'
...
BEGIN
	...
	SELECT 10103000, @udf_field_id, @udf_field_id, @value_name, 'combo', 'int', 'h', 'n', 'EXEC spa_getsourcecounterparty ''s'''
		, 200, 'n',	'n', NULL, 'n',	'n'
		, 'n', 'n', NULL, NULL, NULL, NULL, 'y', 'n', 'y', 'n', NULL, NULL, NULL
	...
	BEGIN
		...
		SELECT @application_group_id, @application_ui_field_id, 'n', 'combo', @udf_template_id, 4
	END
END
