...
IF NOT EXISTS(SELECT 1 FROM formula_editor_parameter WHERE formula_id = -874 AND field_label = 'Curve ID')
BEGIN
	...
	VALUES (-874, 'Curve ID', 'd', '',  'Curve ID','','EXEC spa_GetAllPriceCurveDefinitions @flag = s','0','0','','1','farrms_admin', GETDATE())
END
IF NOT EXISTS(SELECT 1 FROM formula_editor_parameter WHERE formula_id = -874 AND field_label = 'Granularity')
BEGIN
 	INSERT INTO formula_editor_parameter (formula_id, field_label, field_type, default_value, tooltip, field_size, sql_string, is_required, is_numeric, custom_validation, sequence, create_user, create_ts)
