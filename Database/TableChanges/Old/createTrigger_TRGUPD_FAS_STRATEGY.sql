...
IF OBJECT_ID('[dbo].[TRGUPD_FAS_STRATEGY]', 'TR') IS NOT NULL
    DROP TRIGGER [dbo].[TRGUPD_FAS_STRATEGY]
GO

CREATE TRIGGER [dbo].[TRGUPD_FAS_STRATEGY]
ON [dbo].[fas_strategy]
FOR  UPDATE
AS
	
	DECLARE @update_user VARCHAR(500) = dbo.FNADBUser()
	DECLARE @update_ts VARCHAR(100) = GETDATE()
	
	UPDATE fas_strategy
	SET    update_user = @update_user,
	       update_ts = @update_ts
	WHERE  fas_strategy.fas_strategy_id IN (SELECT fas_strategy_id
	                                        FROM   DELETED)
