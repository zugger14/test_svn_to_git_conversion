IF NOT EXISTS (SELECT 1 FROM user_defined_fields_template udft WHERE field_name = -5605)
BEGIN
	...
	SELECT  -5605, N'Scheduled From', N't', N'int', N'n', N'', NULL, NULL, NULL, NULL, N'h', NULL, 30, -5605, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL
END
...
SELECT @udf_template_id = udf_template_id FROM user_defined_fields_template udft WHERE udft.field_name = -5605

IF EXISTS(SELECT 1 FROM maintain_field_template_detail mftd WHERE mftd.field_template_id = @field_template_id
AND mftd.udf_or_system = 'u' AND mftd.field_id = @udf_template_id)
BEGIN
	PRINT 'UDF ''Scheduled From'' already exist.'
	RETURN
END
