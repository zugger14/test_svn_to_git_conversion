...
BEGIN
	...
	SELECT value_id, [type_id], code FROM static_data_value WHERE [type_id] = 5500 AND [code] = 'Source Counterparty'
END
...
IF NOT EXISTS(SELECT 1 FROM generic_mapping_header WHERE mapping_name = 'Trayport Counterparty Mapping')
BEGIN 
	INSERT INTO generic_mapping_header(mapping_name, total_columns_used, system_defined)
	VALUES('Trayport Counterparty Mapping', 2, 0)
END
ELSE
BEGIN
	UPDATE gmh
	SET mapping_name = 'Trayport Counterparty Mapping'
...
SELECT @trayport_source_counterparty = udf_template_id 
FROM user_defined_fields_template 
WHERE Field_id = (SELECT field_id FROM user_defined_fields_template WHERE Field_label = 'Source Counterparty')
...
BEGIN
	...
	SELECT @trayport_mapping_table_id, 'Source Counterparty', @trayport_source_counterparty, 'TRM Counterparty', @trayport_trm_counterparty, '1,2'
END
ELSE
BEGIN
	UPDATE gmd
	SET clm1_label = 'Source Counterparty'
	  , clm1_udf_id = @trayport_source_counterparty
	  , clm2_label = 'TRM Counterparty'
	  , clm2_udf_id = @trayport_trm_counterparty
