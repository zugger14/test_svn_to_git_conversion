...
go
if OBJECT_ID('[report_hourly_position_deal_main]') is null
begin
CREATE TABLE [dbo].[report_hourly_position_deal_main](
/**
...
*/
	...
	[rowid] [int] NOT NULL
	,CONSTRAINT PKC_report_hourly_position_deal_deal_main PRIMARY KEY CLUSTERED (term_start,source_deal_detail_id,period)
	-- As we maintain delta position of the deal for explain position
	--, CONSTRAINT FK_delta_report_hourly_position_financial_main_001 FOREIGN KEY (source_deal_detail_id) REFERENCES dbo.source_deal_detail(source_deal_detail_id) ON DELETE CASCADE
	--, CONSTRAINT FK_delta_report_hourly_position_financial_main_002 FOREIGN KEY (source_deal_header_id) REFERENCES dbo.source_deal_header(source_deal_header_id) ON DELETE CASCADE
	, INDEX indx_report_hourly_position_deal_main_001 ([rowid])
	, INDEX indx_report_hourly_position_deal_main_002 ([source_deal_header_id])
...
ELSE
BEGIN
    PRINT 'Table [report_hourly_position_deal_main] EXISTS'
END
