...
/************************** DELETE FIRST ****************************/
DELETE FROM generic_mapping_values WHERE mapping_table_id IN (
	SELECT mapping_table_id FROM generic_mapping_header WHERE mapping_name = 'Generation Transfer Mapping'
)
DELETE FROM generic_mapping_definition WHERE mapping_table_id IN (
	SELECT mapping_table_id FROM generic_mapping_header WHERE mapping_name = 'Generation Transfer Mapping'
)
DELETE FROM generic_mapping_header WHERE mapping_name = 'Generation Transfer Mapping'

DELETE FROM generic_mapping_values WHERE mapping_table_id IN (
	SELECT mapping_table_id FROM generic_mapping_header WHERE mapping_name = 'Retail Transfer Mapping'
)
DELETE FROM generic_mapping_definition WHERE mapping_table_id IN (
	SELECT mapping_table_id FROM generic_mapping_header WHERE mapping_name = 'Retail Transfer Mapping'
)
DELETE FROM generic_mapping_header WHERE mapping_name = 'Retail Transfer Mapping'

DELETE FROM generic_mapping_values WHERE mapping_table_id IN (
	SELECT mapping_table_id FROM generic_mapping_header WHERE mapping_name = 'Renewable Transfer Mapping'
)
DELETE FROM generic_mapping_definition WHERE mapping_table_id IN (
	SELECT mapping_table_id FROM generic_mapping_header WHERE mapping_name = 'Renewable Transfer Mapping'
)
DELETE FROM generic_mapping_header WHERE mapping_name = 'Renewable Transfer Mapping'
/************************** DELETE FIRST ****************************/

/**********************Transfer Mapping Process **********************************/
IF NOT EXISTS(SELECT 1 FROM static_data_type WHERE [type_id] = 112700)
BEGIN
    INSERT INTO static_data_type([type_id], [type_name], [description], [internal], [is_active], create_user, create_ts)
    VALUES (112700, 'Transfer Mapping Process', 'Transfer Mapping Process', 1, 1, 'farrms_admin', GETDATE())
    PRINT 'Inserted static data type 112700 - Transfer Mapping Process.'
END
ELSE
BEGIN
	PRINT 'Static data type 112700 - Transfer Mapping Process already EXISTS.'
END

SET IDENTITY_INSERT static_data_value ON
IF NOT EXISTS(SELECT 1 FROM static_data_value WHERE value_id = 112700)
BEGIN
    INSERT INTO static_data_value ([type_id], [value_id], [code], [description], [category_id], create_user, create_ts)
    VALUES (112700, 112700, 'Generation', 'Generation', NULL, 'farrms_admin', GETDATE())
    PRINT 'Inserted static data value 112700 - Generation.'
END
ELSE
BEGIN
    PRINT 'Static data value 112700 - Generation already EXISTS.'
END
SET IDENTITY_INSERT static_data_value OFF

SET IDENTITY_INSERT static_data_value ON
IF NOT EXISTS(SELECT 1 FROM static_data_value WHERE value_id = 112701)
BEGIN
    INSERT INTO static_data_value ([type_id], [value_id], [code], [description], [category_id], create_user, create_ts)
    VALUES (112700, 112701, 'Retail', 'Retail', NULL, 'farrms_admin', GETDATE())
    PRINT 'Inserted static data value 112701 - Retail.'
END
ELSE
BEGIN
    PRINT 'Static data value 112701 - Retail already EXISTS.'
END
SET IDENTITY_INSERT static_data_value OFF

SET IDENTITY_INSERT static_data_value ON
IF NOT EXISTS(SELECT 1 FROM static_data_value WHERE value_id = 112702)
BEGIN
    INSERT INTO static_data_value ([type_id], [value_id], [code], [description], [category_id], create_user, create_ts)
    VALUES (112700, 112702, 'Renewable', 'Renewable', NULL, 'farrms_admin', GETDATE())
    PRINT 'Inserted static data value 112702 - Renewable.'
END
ELSE
BEGIN
    PRINT 'Static data value 112702 - Renewable already EXISTS.'
END
SET IDENTITY_INSERT static_data_value OFF

SET IDENTITY_INSERT static_data_value ON
IF NOT EXISTS(SELECT 1 FROM static_data_value WHERE value_id = 112703)
BEGIN
    INSERT INTO static_data_value ([type_id], [value_id], [code], [description], [category_id], create_user, create_ts)
    VALUES (112700, 112703, 'Transportation Capacity', 'Transportation Capacity', NULL, 'farrms_admin', GETDATE())
    PRINT 'Inserted static data value 112703 - Transportation Capacity.'
END
ELSE
BEGIN
    PRINT 'Static data value 112703 - Transportation Capacity already EXISTS.'
END
SET IDENTITY_INSERT static_data_value OFF

SET IDENTITY_INSERT static_data_value ON
IF NOT EXISTS(SELECT 1 FROM static_data_value WHERE value_id = 112704)
BEGIN
    INSERT INTO static_data_value ([type_id], [value_id], [code], [description], [category_id], create_user, create_ts)
    VALUES (112700, 112704, 'Storage', 'Storage', NULL, 'farrms_admin', GETDATE())
    PRINT 'Inserted static data value 112704 - Storage.'
END
ELSE
BEGIN
    PRINT 'Static data value 112704 - Storage already EXISTS.'
END
SET IDENTITY_INSERT static_data_value OFF
/********************** Transfer Mapping Process**********************************/
...
BEGIN
	...
	SELECT 'Transfer Volume Mapping', 18, 0, NULL
END
...
BEGIN
    ...
    PRINT 'Inserted static data value -10000325 - Process Type.'
END
ELSE
BEGIN
    PRINT 'Static data value -10000325 - Process Type already EXISTS.'
END
SET IDENTITY_INSERT static_data_value OFF
...
IF NOT EXISTS (
    SELECT 1
    FROM user_defined_fields_template
    WHERE field_label = 'Process Type'
)
BEGIN
    INSERT INTO user_defined_fields_template (field_name, Field_label, Field_type, data_type, is_required, sql_string, udf_type, sequence, field_size, field_id)
    SELECT sdv.value_id,
           ...
           'NVARCHAR(250)',
           'n',
           'EXEC spa_staticdatavalues @flag = ''h'', @type_id = 112700',
           'h',
           NULL,
           NULL,
           sdv.value_id
    FROM static_data_value sdv
    WHERE sdv.[code] = 'Process Type'
END
...
IF NOT EXISTS (
	...
	FROM generic_mapping_definition
	WHERE mapping_table_id = @mapping_table_id
		AND clm18_label = 'Process Type'
)
BEGIN
	INSERT INTO #temp_generic_mapping_definition(mapping_table_id, clm19_label, clm19_udf_id)
	SELECT @mapping_table_id, 'Process Type', udf_template_id
	FROM user_defined_fields_template udft
	INNER JOIN static_data_value sdv ON sdv.value_id = udft.field_id
		AND sdv.[type_id] = 5500
	WHERE sdv.code = 'Process Type'
...
-- SELECT * FROM generic_mapping_definition WHERE mapping_table_id = 156
-- SELECT * FROM generic_mapping_values WHERE mapping_table_id = 156
INSERT INTO generic_mapping_definition (
	...
	clm17_label,clm17_udf_id,clm18_label,clm18_udf_id,clm19_label,clm19_udf_id
)
