BEGIN TRY
	...
	IF NOT EXISTS (SELECT 1 FROM static_data_value WHERE [type_id] = 5500 AND code = 'Block7')
	BEGIN
		INSERT INTO static_data_value ([type_id], code, [description])
		OUTPUT INSERTED.value_id, INSERTED.[type_id], INSERTED.code
		INTO #insert_output_sdv_external
		SELECT '5500', 'Block7', 'Block7'
	END
	ELSE
	BEGIN
		INSERT INTO #insert_output_sdv_external
	...
	IF NOT EXISTS (SELECT 1 FROM user_defined_fields_template WHERE Field_label = 'Block7')
	BEGIN
		INSERT INTO user_defined_fields_template (field_name, Field_label, Field_type, data_type, is_required, sql_string, udf_type, sequence, field_size, field_id)
		SELECT iose.value_id,
			   'Block7',
			   ...
			   iose.value_id
		FROM #insert_output_sdv_external iose
		WHERE iose.[type_name] = 'Block7'
	END
	ELSE
	BEGIN
		UPDATE user_defined_fields_template
		SET sql_string = 'EXEC spa_source_price_curve_def_maintain ''l'', @is_active = ''y'''
		WHERE Field_label = 'Block7'
	END
	...
	SELECT @Block7 = udf_template_id FROM user_defined_fields_template WHERE Field_label = 'Block7'
	...
	BEGIN
		...
		SET clm2_label = 'Index', clm2_udf_id = @Index, clm1_label = 'Venue', clm1_udf_id = @Venue, clm3_label = 'Block1', clm3_udf_id = @Block1, clm4_label = 'Block2',
			...
			clm8_label = 'Block6', clm8_udf_id = @Block6, clm9_label = 'Block7', clm9_udf_id = @Block7, clm11_label = 'Date', clm11_udf_id = @Date, clm10_label = 'Deal Status',
	...
	BEGIN
		...
		SELECT mapping_table_id, 'Index', @Index, 'Venue', @Venue, 'Block1', @Block1, 'Block2', @Block2,		
			   'Block3', @Block3, 'Block4', @Block4, 'Block5', @Block5, 'Block6', @Block6, 'Block7', @Block7,
