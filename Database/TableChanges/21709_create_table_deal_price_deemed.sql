IF EXISTS (SELECT 1 FROM sys.objects WHERE OBJECT_ID = OBJECT_ID(N'[dbo].[deal_price_deemed]') AND TYPE IN (N'U'))
BEGIN
...
BEGIN
	CREATE TABLE [dbo].[deal_price_deemed] (
		[deal_price_deemed_id] INT IDENTITY(1, 1) CONSTRAINT [pk_deal_price_deemed_id] PRIMARY KEY CLUSTERED WITH (IGNORE_DUP_KEY = OFF) NOT NULL,
...
IF OBJECT_ID('TRGUPD_deal_price_deemed') IS NOT NULL
...
FOR UPDATE
AS
	UPDATE [dbo].[deal_price_deemed]
	SET update_user = dbo.FNADBUser(),
		update_ts = GETDATE()
	FROM deal_price_deemed t
	INNER JOIN DELETED u
		ON t.[deal_price_deemed_id] = u.[deal_price_deemed_id]
GO