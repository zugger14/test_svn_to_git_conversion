...
SET IDENTITY_INSERT static_data_value ON
IF NOT EXISTS(SELECT 1 FROM static_data_value WHERE value_id = -10000363)
BEGIN
    INSERT INTO static_data_value ([type_id], [value_id], [code], [description], [category_id], create_user, create_ts)
    VALUES (5500, -10000363, 'Main Profile', 'Main Profile', NULL, 'farrms_admin', GETDATE())
    PRINT 'Inserted static data value -10000363 - Main Profile.'
END
ELSE
BEGIN
    PRINT 'Static data value -10000363 - Main Profile already EXISTS.'
END
SET IDENTITY_INSERT static_data_value OFF
...
IF NOT EXISTS(SELECT * FROM user_defined_fields_template WHERE field_id = -10000363)
BEGIN
	INSERT INTO user_defined_fields_template(field_name, field_label, Field_type, data_type, is_required, sql_string, udf_type, field_size, field_id)
	SELECT -10000363, 'Main Profile', 'd', 'NVARCHAR(100)', 'y', 'spa_forecast_profile @flag = ''x'', @profile_type_id = 17500', 'h', 100, -10000363
END
...
IF NOT EXISTS( SELECT * FROM generic_mapping_header WHERE mapping_name = 'Retail Power Delta Volume')
BEGIN 
	INSERT INTO generic_mapping_header(mapping_name, total_columns_used, system_defined)
	VALUES('Retail Power Delta Volume', 7, 0)
END
...
SELECT @main_profile = udf_template_id 
FROM user_defined_fields_template 
WHERE Field_id = -10000363
...
BEGIN
	...
	SELECT @mapping_table_id, 'ST Process', @process, 'Sub Book', @sub_book, 'Location', @location
		, 'Main Profile', @main_profile, 'Source Profile 1', @profile1, 'Destination Buy Profile', @dest_buy_profile
...
BEGIN
	...
	SET	clm1_label	= 'Process Type'
		...
		, clm4_label = 'Main Profile'
		, clm4_udf_id = @main_profile
		, clm5_label = 'Source Profile 1'
		, clm5_udf_id = @profile1
