IF EXISTS (SELECT 1 FROM sys.objects WHERE OBJECT_ID = OBJECT_ID(N'[dbo].[deal_price_custom_event]') AND TYPE IN (N'U'))
BEGIN
...
BEGIN
	CREATE TABLE [dbo].[deal_price_custom_event](
		[deal_price_custom_event_id] INT IDENTITY(1, 1) CONSTRAINT [pk_deal_price_custom_event_id] PRIMARY KEY CLUSTERED WITH (IGNORE_DUP_KEY = OFF) NOT NULL,
...
IF OBJECT_ID('TRGUPD_deal_price_custom_event') IS NOT NULL
...
FOR UPDATE
AS
	UPDATE [dbo].[deal_price_custom_event]
	SET update_user = dbo.FNADBUser(),
		update_ts = GETDATE()
	FROM deal_price_custom_event t
	INNER JOIN DELETED u
		ON t.[deal_price_custom_event_id] = u.[deal_price_custom_event_id]