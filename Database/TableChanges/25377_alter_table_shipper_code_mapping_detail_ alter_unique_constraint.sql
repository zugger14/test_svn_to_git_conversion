-- Drop unique constraint (shipper_code_id, effective_date)
IF EXISTS( 
	SELECT 1
...
--Add unique constraint (shipper_code_id, effective_date, location_id)
IF NOT EXISTS( 
	SELECT 1
	FROM INFORMATION_SCHEMA.TABLE_CONSTRAINTS tc
	INNER JOIN INFORMATION_SCHEMA.CONSTRAINT_COLUMN_USAGE ccu ON tc.TABLE_NAME = ccu.TABLE_NAME
...
BEGIN
	...
	*/
	...
	ADD CONSTRAINT UC_shipper_code_mapping_detail UNIQUE (shipper_code_id, effective_date, location_id)
