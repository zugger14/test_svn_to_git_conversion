/****** Object:  UserDefinedFunction [dbo].[FNARAverageYearlyPrice] ******/
IF EXISTS (SELECT * FROM sys.objects WHERE  OBJECT_ID = OBJECT_ID(N'[dbo].[FNARAverageYrlyPrice]') AND TYPE IN (N'FN' ,N'IF' ,N'TF' ,N'FS' ,N'FT'))
    DROP FUNCTION [dbo].[FNARAverageYrlyPrice]
GO

CREATE FUNCTION [dbo].[FNARAverageYrlyPrice]
(
		...
		FROM   (
				   ...
				   FROM   source_deal_detail sdd
						  INNER JOIN source_minor_location sml
							   ON  sdd.location_id = sml.source_minor_location_id AND sdd.source_deal_header_id=@source_deal_header_id
						  ...
						  LEFT  JOIN mv90_data md
							   ON  md.meter_id = smlm.meter_id  AND convert(varchar(7),md.from_date,120)=convert(varchar(7),sdd.term_start,120)
						  ...
						  outer  apply (
								   ...
								   WHERE  rp.source_deal_detail_id = sdd.source_deal_detail_id 
										AND rp.term_start=hb.term_date
								   UNION ALL
								   SELECT sdd.source_deal_detail_id
										 ,sdd.curve_id
										 ,rp.term_start
					...
					OUTER APPLY (
							...
							FROM source_price_curve s WHERE s.source_curve_def_id = @curve_id
										AND CONVERT(VARCHAR(4),s.maturity_date,120) =CONVERT(VARCHAR(4),@maturity_date,120) 
				   ...
				   WHERE  CONVERT(VARCHAR(4),ISNULL(mdh.prod_date ,pos.term_start),120)=CONVERT(VARCHAR(4),@maturity_date,120)		   
