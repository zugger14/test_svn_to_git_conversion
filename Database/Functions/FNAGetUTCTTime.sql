...
IF OBJECT_ID(N'[dbo].FNAGetUTCTTime', N'FN') IS NOT NULL 
	DROP FUNCTION [dbo].FNAGetUTCTTime
GO

/**
	This function Converts Given DateTime to UTC Time. It handles European DST too.
...
CREATE FUNCTION [dbo].[FNAGetUTCTTime] (
	 @Date AS DATETIME, 
	 @TimeZone INT
	)
...
BEGIN
	...
	/*-- TEST CODE
	DECLARE
	 @Date AS DATETIME = '2018-04-15 06:20:00', 
	@TimeZone INT = 15
	--*/
...
-- GET THE DST parameter from the provided datetime
	...
	--SELECT @DSTDT [given_date], @DSTEFFDT [DST_start], @DSTENDDT [DST_end]
	...
	BEGIN
		...
		SET @NEWDT = DATEADD(hh,-1*(@DSTOFFSETHR),@Date)
		SET @NEWDT = DATEADD(mi,-1*(@DSTOFFSETMI),@NEWDT)
	END
	...
	BEGIN
		-- Increase the datetime by the hours and minutes assigned to the timezone
		SET @NEWDT = DATEADD(hh,-1*(@OFFSETHR),@Date)
