IF OBJECT_ID('dbo.FNANextInstanceDate','fn') IS NOT NULL
DROP FUNCTION dbo.FNANextInstanceDate
GO
/*
...
Dated  : 09.July.2009
*/
CREATE FUNCTION dbo.FNANextInstanceDate
(@activityid INT,
@dateFrom   DATETIME,
@alreadyLooped CHAR(1) = 'n'
)
RETURNS DATETIME
--IF OBJECT_ID('dbo.FNANextInstanceDate','p') IS NOT NULL
--DROP PROCEDURE dbo.FNANextInstanceDate
--GO
--CREATE PROCEDURE dbo.FNANextInstanceDate
--@activityid INT,
--@dateFrom   DATETIME,
--@alreadyLooped CHAR(1) = 'n'
AS
BEGIN
	...
	ELSE
	BEGIN
		IF EXISTS (SELECT 'x' FROM dbo.process_risk_controls_activities WHERE risk_control_id = @activityid) AND @dateFrom IS NULL 
		BEGIN
		...
		BEGIN	
			...
			BEGIN
				IF @dateFrom<=@run_end_date --@run_effective_date <= @dateFrom
				BEGIN
					IF @dateFrom<@run_effective_date
						SELECT @lastInstanceDate = @run_effective_date
					ELSE
						SELECT @lastInstanceDate = @dateFrom --CASE WHEN @alreadyLooped = 'n' THEN DATEADD(d,-1,@dateFrom) ELSE @dateFrom END --dbo.FNAgetSQLStandardDate(GETDATE())
				END
				ELSE 					
					RETURN NULL					
			END				
