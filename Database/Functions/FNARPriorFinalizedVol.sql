/****** Object:  UserDefinedFunction [dbo].[FNARPriorFinalizedVol]    Script Date: 02/14/2011 15:43:02 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[FNARPriorFinalizedVol]') AND type in (N'FN', N'IF', N'TF', N'FS', N'FT'))
DROP FUNCTION [dbo].[FNARPriorFinalizedVol]
GO


/****** Object:  UserDefinedFunction [dbo].[FNARPriorFinalizedVol]    Script Date: 02/14/2011 15:43:07 ******/
SET ANSI_NULLS ON
GO
...
CREATE FUNCTION [dbo].[FNARPriorFinalizedVol] (@contract_id INT,@counterparty_id INT,@prod_date DATETIME,@invoice_line_item_id INT,@invoice_type CHAR(1))
RETURNS float AS  
BEGIN 
	DECLARE @value FLOAT

	SELECT 
		@value = SUM(ISNULL(civ.Volume,0))
	FROM 
