...
LEFT JOIN source_system_book_map ssbm
	ON ssbm.source_system_book_id1 = sdh_offset.source_system_book_id1
	AND ssbm.source_system_book_id2 = sdh_offset.source_system_book_id2
	AND ssbm.source_system_book_id3 = sdh_offset.source_system_book_id3
	AND ssbm.source_system_book_id4 = sdh_offset.source_system_book_id4
OUTER APPLY (
	...
	WHERE sdd_offset.source_deal_header_id = sdh_offset.source_deal_header_id
) sdd_offset
OUTER APPLY (
...
INNER JOIN source_counterparty sc ON sc.source_counterparty_id = sdh_offset.counterparty_id
INNER JOIN source_traders st ON st.source_trader_id = sdh_offset.trader_id
LEFT JOIN contract_group cg ON cg.contract_id = sdh_offset.contract_id
WHERE sdh.source_deal_header_id <> sdh_offset.source_deal_header_id
AND sdh_offset.source_deal_header_id IS NOT NULL
