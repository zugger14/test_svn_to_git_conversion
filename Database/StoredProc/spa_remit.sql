...
AS
/*-------------------------Debug Section---------------------
EXEC sys.sp_set_session_context @key = N'DB_USER', @value = 'bibash';
DECLARE	@create_date_from VARCHAR(100) = NULL,
		@create_date_to VARCHAR(100) = NULL,
		@generate_xml INT = 1,
		@generate_uti INT = NULL,
		@report_type VARCHAR(MAX) = NULL,
		@process_id VARCHAR(MAX) = '8f29da56-683c-4132-8ec1-e10b27aa1b45',
		@flag CHAR(1) = 'i',
		@batch_unique_id VARCHAR(1000) = NULL,
		    ...
		    BEGIN
				...
				BEGIN  -- Power does not contain HubCodificationInformation
					...
					BEGIN
						...
						INSERT INTO #temp1(hr,source_deal_header_id,term_date,hr_mult,granularity,volume,price,block_column) 
						SELECT hr,source_deal_header_id,term_date,hr_mult,granularity,volume,price,NULL FROM #tempblock1
						...
						DECLARE @term_date DATETIME DECLARE bond_cursor CURSOR FOR
							...
							SELECT term_date, price, volume
								FROM #temp1 OPEN bond_cursor FETCH NEXT FROM bond_cursor
							INTO @term_date, @next_price, @next_volume WHILE @@FETCH_STATUS = 0
							BEGIN
								IF @timer2 = 0
							BEGIN
								SET @price = @next_price
								SET @volume = @next_volume
								SET @timer2 = 1
							END
							IF ISNULL(@price, -1) = ISNULL(@next_price, -1) and ISNULL(@volume, -1) =	ISNULL(@next_volume, -1)
							BEGIN
								SET @timer1 = @timer1
							END
							ELSE
							BEGIN
								...
								SET @timer1 = @timer1 + 1
							END UPDATE #temp1 
								SET block_column = @timer1 
							WHERE term_date = @term_date 
							FETCH	NEXT	
							FROM bond_cursor
								INTO @term_date, @next_price, @next_volume
							END
							CLOSE bond_cursor;
							DEALLOCATE bond_cursor;

							SELECT [source_deal_header_id],volume,price,hr_mult,granularity 
							, MIN(term_date) AS start_date, DATEADD(mi,15,MAX(term_date)) AS				end_date 
							INTO #temp
							FROM #temp1
							GROUP BY						[source_deal_header_id],volume,price,hr_mult,granularity,block_column
							ORDER BY start_date
...
BEGIN
	...
	BEGIN TRY
		...
		BEGIN
			...
			sub_book_id= MAX(td.sub_book_id),
			-----Parties of Contract
			[ID of the market participant or counterparty]= IIF(MAX(td.header_buy_sell_flag) = 'b', MAX(tcuv.[Sub Code]), MAX(tcuv.[Deal Code])),
			...
			[trader_id_market_participant] = IIF(MAX(td.header_buy_sell_flag) = 'b', MAX(tcuv.[Deal Code]), MAX(tcuv.[Sub Code])),
			...
			IIF( MAX(td.internal_desk_id) IN (17301,17302), ROUND(SUM(tdd.deal_volume)/CAST((DATEDIFF(DAY,MIN(td.entire_term_start), MAX(td.entire_term_end)) +1) AS INT)/24,3) ,
		...
		BEGIN
			...
			SELECT DISTINCT
				   ...
				   [ID of the other market participant or counterparty] = IIF(MAX(td.header_buy_sell_flag) = 'b', MAX(tcuv.[Deal Code]), MAX(tcuv.[Sub Code])),
