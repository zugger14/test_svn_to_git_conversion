...
BEGIN
	...
	BEGIN TRY
		...
		BEGIN
			...
			IIF(MAX(td.term_frequency) = 'm' AND MAX(td.profile_granularity) = 982 AND MAX(td.internal_desk_id) IN (17301,17302), ROUND(SUM(tdd.deal_volume)/CAST((DATEDIFF(DAY,MIN(td.entire_term_start), MAX(td.entire_term_end)) +1) AS INT)/24,3) ,
