...
IF OBJECT_ID(N'[dbo].[spa_rfx_report_default_paramset_dhx]', N'P') IS NOT NULL
    DROP PROCEDURE [dbo].[spa_rfx_report_default_paramset_dhx]
GO
...
-- ===========================================================================================================
CREATE PROCEDURE [dbo].[spa_rfx_report_default_paramset_dhx]
@flag		VARCHAR(10) 
		...
		SET @sql = '
			IF NOT EXISTS(
				...
				CASE WHEN @component_type = 't' THEN
					...
					EXEC spa_rfx_get_dependent_parameters_dhx ''1'' ,''p'', ''' + CAST(@process_id AS VARCHAR(200)) + ''', @sql OUTPUT
