...
select  @process_id='1A2C3D84_C2B1_42C9_949F_7C8B91DB1572',@insert_type=1,@partition_no=1,@user_login_id='farrms_admin',@deal_delete='y'
...
-- delete of orphant deal_detail_id's position
	...
	CREATE INDEX indx_tmp_header_deal_id_del_zzz ON #tmp_header_deal_id_del (source_deal_detail_id)
	...
	BEGIN
		...
		BEGIN
			...
			into #report_hourly_position_old(source_deal_header_id,term_start,deal_date,deal_volume_uom_id,hr1,hr2,hr3,hr4,hr5,hr6,hr7,hr8,hr9,hr10,hr11,hr12,hr13,hr14,hr15,hr16,hr17,hr18,hr19,hr20,hr21,hr22,hr23,hr24,hr25,create_ts,create_user,expiration_date,period,granularity,source_deal_detail_id,rowid) '
			END + ' 
			from report_hourly_position_profile_main s INNER JOIN #tmp_header_deal_id_del h ON s.source_deal_detail_id=h.source_deal_detail_id
	...
	END -- EXISTS (SELECT TOP 1 1 FROM #tmp_header_deal_id_del)
	...
	BEGIN
		...
		SET @st_sql = 'INSERT INTO #tmp_header_deal_id (source_deal_header_id,create_user,granularity,source_deal_detail_id,rowid) 
		...
		from #tmp_header_deal_id sdh inner join source_deal_detail sdd on sdh.source_deal_detail_id=sdd.source_deal_detail_id
	...
	--select * from static_data_value where value_id= 17606
		...
		*n_hrs.no_hours,0) END AS NUMERIC(32,16))'
			...
			FROM  source_deal_header sdh    
				...
				(
					...
					where hbt.dst_group_value_id=isnull(thdi.dst_group_value_id,'+@default_dst_group+') 
						AND hbt.block_define_id=COALESCE(spcd.block_define_id,sdh.block_define_id,' + @baseload_block_define_id + ')
						and hbt.term_date between sdd.term_start and sdd.term_END
						and isnull(spcd.hourly_volume_allocation,17601) IN(17603,17604) and sdd.physical_financial_flag=''f'' and hbt.hol_date is null
		...
		SET @st_sql = @destination_tbl + ' SELECT max(pdd.source_deal_header_id),
			...
			sum(cast(case when pdd.commodity_id=-1 then hb.hr8 else hb.hr2*case when isnull(hb.add_dst_hour,0)=2 then 2.00 else 1.00 end end as numeric(1,0)) *' + @col_exp2_gas + '*' + @col_exp3 + ') AS Hr2,
			sum(cast(case when pdd.commodity_id=-1 then hb.hr9 else hb.hr3*case when isnull(hb.add_dst_hour,0)=3 then 2.00 else 1.00 end end as numeric(1,0)) *' + @col_exp2_gas + '*' + @col_exp3 + ') AS Hr3,
		...
		SET @st_sql3 = '
			...
			sum(cast(case when pdd.commodity_id=-1 then hb1.hr2*case when isnull(hb1.add_dst_hour,0)=2 then 2.00 else 1.00 end else hb.hr20 end as numeric(1,0)) *' + @col_exp2_gas + '*' + @col_exp3 + ') AS Hr20,
			sum(cast(case when pdd.commodity_id=-1 then hb1.hr3*case when isnull(hb1.add_dst_hour,0)=3 then 2.00 else 1.00 end else hb.hr21 end as numeric(1,0)) *' + @col_exp2_gas + '*' + @col_exp3 + ') AS Hr21,
		...
		SET @st_from = @report_hourly_position_deal_main + ' 
			From #tmp_pos_deal_detail pdd
				...
				outer apply (
					select hr1,hr2,hr3,hr4,hr5,hr6,hr7,hr8,hr9,hr10,hr11,hr12,hr13,hr14,hr15,hr16,hr17,hr18,hr19,hr20,hr21,hr22,hr23,hr24,volume_mult,add_dst_hour from hour_block_term hbt  inner join 
						(
				...
				outer apply (
					select hr1,hr2,hr3,hr4,hr5,hr6,hr7,hr8,hr9,hr10,hr11,hr12,hr13,hr14,hr15,hr16,hr17,hr18,hr19,hr20,hr21,hr22,hr23,hr24,volume_mult,add_dst_hour from hour_block_term hbt  
					inner join 
						(
			...
			( select case when pdd.commodity_id=-1 then
				   ...
				   +case when isnull(term_hrs_exp1.add_dst_hour,hb1.add_dst_hour)>0 then 1 else 0 end
			  else 
		...
		SET @col_exp2 = 'cast(CASE WHEN pdd.deal_volume_frequency in (''h'',''x'',''y'') THEN pdd.deal_volume *pdd.conversion_factor*cast(pdd.multiplier*pdd.volume_multiplier2 as numeric(21,16)) ELSE pdd.total_volume/nullif(pdd.no_days*
		isnull(hb_term.volume_mult,term_hrs_exp.term_no_hrs),0) END AS NUMERIC(32,16))'
		...
		WHERE   rhpd.source_deal_header_id IS NULL AND hb.term_date is not null
				and ((pdd.hourly_volume_allocation IN(17603,17604) and  hg.exp_date is not null) or (pdd.hourly_volume_allocation in (17600,17601,17602,17605) )  or pdd.physical_financial_flag=''p'')
	...
	BEGIN
		...
		BEGIN
			...
			FROM source_minor_location sml  
					...
					inner join #tmp_header_deal_id ed  on sdd.source_deal_detail_id=ed.source_deal_detail_id
			...
			CREATE index indx_tmp_location_profile_location_id on #tmp_location_profile_mtj ([source_deal_detail_id])
			CREATE index indx_tmp_location_profile_profile_id on #tmp_location_profile_mtj ([profile_id])
...
--sum yearly fraction for the Power profile type=National profile
	SET @st_sql0 = '
		...
		( 
			SELECT distinct [source_deal_header_id],year(term_start) yr ,block_define_id,block_type
				,profile_id,location_id,curve_id,dst_group_value_id FROM #tmp_profile_header
	...
	' FROM source_deal_header sdh    
		...
		where  spcd.formula_id IS  NULL and  (sdd.fixed_float_leg=''t'') AND ISNULL(sdh.product_id,4101)= 4100	
				and ((isnull(spcd.hourly_volume_allocation,17601) IN(17603,17604) and hg.exp_date is not null) or (isnull(spcd.hourly_volume_allocation,17601)in (17600,17601,17602,17605) )  or sdd.physical_financial_flag=''p'') and hb.term_date is not null
	...
	EXEC spa_print '================================================================================================='
	EXEC spa_print 'Total Volume Insert from sum of hourly volume'
	EXEC spa_print '=================================================================================================='
	-------------------------------------------------------------------------------------------
	...
	* CASE WHEN term.buy_sell_flag = 'b' THEN 1 ELSE -1 END
	...
	UPDATE #tmp_financial_term
		...
		ELSE 'insert  into dbo.[report_hourly_position_financial_main] 
			...
			,[hr1],[hr2],[hr3],[hr4],[hr5],[hr6],[hr7],[hr8],[hr9],[hr10],[hr11],[hr12],[hr13],[hr14],[hr15],[hr16],[hr17],[hr18],[hr19],[hr20],[hr21],[hr22],[hr23],[hr24],[hr25],[create_ts],[create_user],source_deal_detail_id,rowid,granularity,[period],financial_curve_id,expiration_date)' 
			+ CASE WHEN isnull(@maintain_delta, 0) = 0 THEN '' ELSE 
		...
		into #report_hourly_position_financial_main_inserted 
		 ...
		 ,hr17,hr18,hr19,hr20,hr21,hr22,hr23,hr24,hr25,create_ts,create_user,source_deal_detail_id,rowid,granularity,[period],financial_curve_id,expiration_date)
		' ELSE '' END
		END
	END
	SET @st_sql1 = '  
	...
	'
		...
		BEGIN
			...
			BEGIN
				SET @st_sql = 
				'insert into dbo.delta_report_hourly_position_financial_main (as_of_date,source_deal_header_id,term_start,deal_date,deal_volume_uom_id,hr1,hr2,hr3,hr4,hr5,hr6,hr7,hr8,hr9,hr10,hr11,hr12,hr13,hr14,hr15,hr16,hr17,hr18,hr19,hr20,hr21,hr22,hr23,hr24,hr25,create_ts,create_user,delta_type,expiration_date,source_deal_detail_id,rowid,granularity,[period],financial_curve_id,expiration_date)
				select getdate() as_of_date
					,isnull(o.source_deal_header_id,n.source_deal_header_id),isnull(o.term_start,n.term_start),isnull(o.deal_date,n.deal_date),isnull(o.deal_volume_uom_id,n.deal_volume_uom_id),
					isnull(n.hr1,0)-isnull(o.hr1,0) hr1,isnull(n.hr2,0)-isnull(o.hr2,0) hr2,isnull(n.hr3,0)-isnull(o.hr3,0) hr3,isnull(n.hr4,0)-isnull(o.hr4,0) hr4,isnull(n.hr5,0)-isnull(o.hr5,0) hr5,isnull(n.hr6,0)-isnull(o.hr6,0) hr6,isnull(n.hr7,0)-isnull(o.hr7,0) hr7,isnull(n.hr8,0)-isnull(o.hr8,0) hr8,isnull(n.hr9,0)-isnull(o.hr9,0) hr9,isnull(n.hr10,0)-isnull(o.hr10,0) hr10 ,isnull(n.hr11,0)-isnull(o.hr11,0) hr11
					,isnull(n.hr12,0)-isnull(o.hr12,0) hr12,isnull(n.hr13,0)-isnull(o.hr13,0) hr13,isnull(n.hr14,0)-isnull(o.hr14,0) hr14,isnull(n.hr15,0)-isnull(o.hr15,0) hr15,isnull(n.hr16,0)-isnull(o.hr16,0) hr16,isnull(n.hr17,0)-isnull(o.hr17,0) hr17,isnull(n.hr18,0)-isnull(o.hr18,0) hr18,isnull(n.hr19,0)-isnull(o.hr19,0) hr19,isnull(n.hr20,0)-isnull(o.hr20,0) hr20,isnull(n.hr21,0)-isnull(o.hr21,0) hr21
					...
					,isnull(o.create_ts,getdate()),isnull(o.create_user,dbo.fnadbuser()),' 
			+ CASE WHEN ISNULL(@orginal_insert_type, 0) = 0 THEN '17404' ELSE '17403' END + 
						' delta_type ,isnull(o.expiration_date,n.expiration_date),isnull(o.source_deal_detail_id,n.source_deal_detail_id),isnull(o.rowid,n.rowid),isnull(o.granularity,n.granularity),isnull(o.[period],n.[period],n.financial_curve_id,n.expiration_date)
