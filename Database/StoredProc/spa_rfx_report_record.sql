IF OBJECT_ID(N'[dbo].[spa_rfx_report_record]', N'P') IS NOT NULL
    DROP PROCEDURE [dbo].[spa_rfx_report_record]
GO

-- ===========================================================================================================
-- Author: padhikari@pioneersolutionsglobal.com
-- Create date: 2012-09-12
-- Description: Select Operation for report record
 
-- Params:
-- @flag					CHAR	- Operation flag
-- @reports_id				INT		- Report ID 
-- EXEC spa_rfx_report_record 's'
-- ===========================================================================================================
-- ===========================================================================================================
-- Modified By: sligal@pioneersolutionsglobal.com
-- Purpose: For adapting privilege system, export handling
 
-- Added Params:
-- @call_from VARCHAR(100) - call from to distinguish from my report and other
-- ===========================================================================================================
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [dbo].[spa_rfx_report_record]
	@flag CHAR(1) = NULL,
	@report_paramset_id varchar(5000) = NULL,
...
BEGIN
	...
	inner join dbo.SplitCommaSeperatedValues(@report_paramset_id) scsv on scsv.item = rps.report_paramset_id
	WHERE 1=1 --rps.report_paramset_id = @report_paramset_id
		AND rpm.hidden <> 1
	GROUP BY dsc.name
...
IF @flag = 'x'
BEGIN
--SET @sql = 'SELECT DISTINCT rps.report_paramset_id [Report Paramset ID] 
--					, r.report_id [Report ID]
--					, r.is_system [Is System]
--					, MAX(r.name) [Report Name]
--					, MAX(r.description) [Description]
--					, MAX(rp.[name]) [Report Page], MAX(rps.[name]) [Paramset]
--					--treat no widget as TEXTBOX (1)
--					, SUM(CASE WHEN ISNULL(rpm.hidden, -1) <> 1 AND (ISNULL(dsc.widget_id, 1) IN (3, 4, 5))  THEN 1 ELSE 0 END) [Tree Filter Required]
--					, SUM(CASE WHEN ISNULL(rpm.hidden, -1) <> 1 AND (ISNULL(dsc.widget_id, 1) NOT IN (3, 4, 5)) THEN 1 ELSE 0 END) [Other Filter Required]
--					--generate RDS_<root_dataset_alias>:<root_dataset_id> in csv format for RDL paramset binding
--					, dbo.FNARFXGenerateReportItemsCombined(MAX(rp.report_page_id)) [Items Combined]
--					, rps.paramset_hash [Paramset Hash]
--			FROM report r 
--			LEFT JOIN report_page rp ON  rp.report_id = r.report_id
--			INNER JOIN report_paramset rps ON  rps.page_id = rp.report_page_id
--				AND r.[owner] = ''' + CAST(@report_owner AS VARCHAR(500)) + '''
--			LEFT JOIN report_dataset_paramset rdp ON rdp.paramset_id = rps.report_paramset_id
--			LEFT JOIN report_param rpm ON rdp.report_dataset_paramset_id = rpm.dataset_paramset_id 
--			LEFT JOIN data_source_column dsc ON dsc.data_source_column_id = rpm.column_id
--			WHERE 1 = 1' 
...
--	BEGIN
--		SET @sql = @sql + ' And r.is_system = ''' + @is_system + ''''
--	END
--	SET @sql = @sql + ' GROUP BY rps.report_paramset_id, r.report_id, r.is_system, rps.paramset_hash'
	
--	SET @sql = @sql + ' UNION '
	
--	SET @sql = @sql + 'SELECT DISTINCT rps.report_paramset_id [Report Paramset ID] , r.report_id ,r.is_system, MAX(r.name) [Report Name], MAX(r.description) [Descriptionstem]
--							  , MAX(rp.[name]) [Report Page], MAX(rps.[name]) [Paramset]
--							  --treat no widget as TEXTBOX (1)
--							  , SUM(CASE WHEN ISNULL(rpm.hidden, -1) <> 1 AND (ISNULL(dsc.widget_id, 1) IN (3, 4, 5))  THEN 1 ELSE 0 END) tree_filter_required
--							  , SUM(CASE WHEN ISNULL(rpm.hidden, -1) <> 1 AND (ISNULL(dsc.widget_id, 1) NOT IN (3, 4, 5)) THEN 1 ELSE 0 END) other_filter_required
...
--						LEFT JOIN report_page rp ON  rp.report_id = r.report_id
--						INNER JOIN report_paramset rps ON  rps.page_id = rp.report_page_id
--							AND (rps.report_status_id = 2 AND r.[owner] <> ''' + CAST(@report_owner AS VARCHAR(500)) + ''')							
--						LEFT JOIN report_dataset_paramset rdp ON rdp.paramset_id = rps.report_paramset_id
--						LEFT JOIN report_param rpm ON rdp.report_dataset_paramset_id = rpm.dataset_paramset_id 
--						LEFT JOIN data_source_column dsc ON dsc.data_source_column_id = rpm.column_id
--						WHERE 1 = 1' 
