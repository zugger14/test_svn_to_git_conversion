IF OBJECT_ID(N'[dbo].[spa_deal_pricing_detail_provisional]', N'P') IS NOT NULL
  DROP PROCEDURE [dbo].spa_deal_pricing_detail_provisional
...
CREATE PROCEDURE [dbo].[spa_deal_pricing_detail_provisional]
	@flag CHAR(1),
...
SELECT @flag='s',@mode='normal',@xml_process_id='2D9AFC14_2063_488E_B5B1_603DAD9F28FC',@source_deal_detail_id='223443'
----------------------------------------------------*/
...
BEGIN
	EXEC spa_Errorhandler -1, 'Deal Provisional Pricing Detail', 'spa_deal_pricing_provisional_detail', 'Error', 'No details to apply price for these terms and legs.', @xml_process_id
	RETURN
END
...
BEGIN
	EXEC ('
		...
		SELECT 1
		FROM adiha_process.dbo.provisional_pricing_xml_' + @user_name + '_' + @xml_process_id + '
...
BEGIN
	...
	BEGIN
		...
		--Quality Grid
		SELECT @new_xml +=  '<deal_price_qualities><deal_price_quality deal_price_quality_id="' + CAST(deal_price_quality_provisional_id AS VARCHAR(10)) + '" attribute="' + ISNULL(CAST(attribute AS VARCHAR(10)), '') + '" operator="' + ISNULL(CAST(operator AS VARCHAR(10)), '') + '", numeric_value="' + ISNULL(dbo.FNARemoveTrailingZero(numeric_value), '') + '" text_value="' + ISNULL(text_value, '') + '" uom="' + ISNULL(CAST(uom AS VARCHAR(10)), '') + '" basis="' + ISNULL(CAST(basis AS VARCHAR(10)), '') + '" /></deal_price_qualities>'
		FROM deal_price_quality_provisional 
		WHERE source_deal_detail_id = @source_deal_detail_id
	...
	BEGIN
		...
		SET @xml_process_table = 'adiha_process.dbo.provisional_pricing_xml_' + @user_name + '_' + @xml_process_id
	...
	SET @sql = '
		...
		THEN 
			INSERT(flag, source_deal_detail_id, xml_value, apply_to_xml, is_apply_to_all, call_from, process_id) 
			VALUES(''m'', s.source_deal_detail_id, s.xml_value, s.apply_to_xml, s.is_apply_to_all, s.call_from, s.process_id)
		WHEN MATCHED 
		THEN 
			UPDATE 
			SET flag = ''m'',
	...
	EXEC spa_Errorhandler 0, 'Deal Pricing Detail', 'spa_deal_pricing_detail_provisional', 'Success', 'Changes have been saved successfully.', @xml_process_id
	RETURN
END
...
BEGIN
	...
	BEGIN
		...
		BEGIN TRAN
			...
			DELETE ddfu  
			FROM deal_price_adjustment_provisional ddfu
			...
			FROM deal_price_type_provisional dpt
			LEFT JOIN #deal_price_type t 
				ON dpt.deal_price_type_provisional_id = t.deal_price_type_id
				AND t.deal_price_type_id IS NOT NULL 
			WHERE dpt.source_deal_detail_id = @source_deal_detail_id
				AND t.deal_price_type_id IS NULL

			DELETE ddfu  
			FROM deal_detail_formula_udf_provisional ddfu
			LEFT JOIN #deal_price_type t 
				ON ddfu.deal_price_type_id = t.deal_price_type_id
				AND t.deal_price_type_id IS NOT NULL 
			WHERE ddfu.source_deal_detail_id = @source_deal_detail_id
			...
			UPDATE sdd
				SET pricing_type = NULLIF(dp.pricing_aggregation,''),
					...
					settlement_currency = dp.settlement_currency,
			...
			DELETE dpq
			FROM deal_price_quality_provisional dpq
			LEFT JOIN #deal_price_quality t
				ON dpq.deal_price_quality_provisional_id = t.deal_price_quality_id
			WHERE t.deal_price_quality_id IS NULL
				AND dpq.source_deal_detail_id = @source_deal_detail_id	

			INSERT INTO deal_price_quality_provisional(
				source_deal_detail_id
				, attribute
				, operator
				, numeric_value
			...
			INSERT INTO  deal_price_type_provisional (
			...
			--ALTER TABLE deal_price_deemed_provisional DROP CONSTRAINT FK__deal_pric__deal___1B895577
			...
			SELECT 
				...
				,NULLIF(dpt.[deal_price_type_provisional_id], '')		
			...
			SELECT dpd.source_deal_detail_id,
					dpd.udf_template_id,
					dpd.udf_value,
					dpt.deal_price_type_provisional_id,
					dpd.formula_id	
			FROM deal_price_type_provisional dpt
			INNER JOIN #deal_detail_formula_udf dpd
				ON (dpt.description = dpd.rid or dpt.deal_price_type_provisional_id = dpd.deal_price_type_id)
			
			DELETE f 
			FROM deal_price_adjustment_provisional f
			INNER JOIN #deal_price_adjustment t
				ON f.deal_price_type_id = t.deal_price_type_id

			INSERT INTO deal_price_adjustment_provisional( 
				source_deal_detail_id,
				udf_template_id,
				udf_value,
				deal_price_type_id,
			...
			FROM deal_price_type_provisional dpt
			INNER JOIN #deal_price_type sub
				ON dpt.deal_price_type_provisional_id = sub.deal_price_type_id 
			...
			EXEC spa_Errorhandler 0,
							'Deal Priceing Detail',
							'spa_deal_pricing_detail_provisional',
							'Success',
...
BEGIN
	...
	BEGIN
		SELECT @grid_json = ISNULL(@grid_json + ',' , '') 
							+  '{id:' + CAST(deal_price_type_provisional_id AS VARCHAR(10)) 
							+ ', data:["' + CAST(deal_price_type_provisional_id AS VARCHAR(10)) 
							+ '","' + CAST(price_type_id AS VARCHAR(10))  
							+ '","' + ISNULL(description, '') + '"]}'
													
		FROM deal_price_type_provisional 
		WHERE source_deal_detail_id = @source_deal_detail_id 
	...
	BEGIN
		SELECT	@quality_grid_json = ISNULL(@quality_grid_json + ',' , '')
									+  '{id:' + CAST(deal_price_quality_provisional_id AS VARCHAR(10)) 
									+ ', data:["' 
									+ CAST(deal_price_quality_provisional_id AS VARCHAR(10)) 
	...
	BEGIN
		SELECT @deemed_form_json = ISNULL(@deemed_form_json + ',', '') 
								   + '"' + ISNULL(NULLIF(CAST(price_type AS VARCHAR(100)), 0), rid) + '_' + ISNULL(NULLIF(CAST(dpt.deal_price_type_provisional_id AS VARCHAR(100)), 0), rid)
								   ...
								   + CASE WHEN dpd.price_type IN (103600) THEN ',"fixed_price":"' + ISNULL(CAST(dpd.fixed_price AS VARCHAR(50)), '') + '"' ELSE '' END 
								   ...
								   + CASE WHEN dpd.price_type IN (103604) THEN ',"fixed_cost":"' + ISNULL(CAST(dpd.fixed_cost AS VARCHAR(50)), '')  + '"' ELSE '' END
	...
	BEGIN
		...
		SELECT @deemed_form_json = ISNULL(@deemed_form_json + ',', '') 
								   ...
								   + ',"volume_uom":"' + ISNULL(CAST(dpd.uom AS VARCHAR(50)), '') + '"'								   
								   + CASE WHEN dpt.price_type_id IN (103600) THEN ',"fixed_price":"' + ISNULL(CAST(dpd.fixed_price AS VARCHAR(50)), '') + '"' ELSE '' END 
								   ...
								   + CASE WHEN dpt.price_type_id IN (103604) THEN ',"fixed_cost":"' + ISNULL(CAST(dpd.fixed_cost AS VARCHAR(50)), '')  + '"' ELSE '' END
	...
	BEGIN
		SELECT @predefined_formula_form_json = ISNULL(@predefined_formula_form_json + ',', '') +
									 '"' + CAST(dpt.deal_price_type_provisional_id AS VARCHAR(10)) 
									+ '":{"deal_price_type_id":"' + CAST(dpt.deal_price_type_provisional_id AS VARCHAR(10)) 
									+ '","price_type":"' + CAST( dpt.price_type_id AS VARCHAR(10)) 
									+ '","price_type_description":"' + ISNULL(dpt.description, '')  
									+ '","formula_id":"' + ISNULL(CAST(sub.formula_id AS VARCHAR(10)), '')  
									+ '", "formula_fields" : {' + STUFF((SELECT ', ' + '"UDF___' +  CAST(udf_template_id AS VARCHAR(10)) + '" : "' + udf_value + '"'
									   FROM deal_detail_formula_udf_provisional ddfu 
									   WHERE ddfu.deal_price_type_id = dpt.deal_price_type_provisional_id 
									  FOR XML PATH('')), 1, 2, '') + '}}'
		FROM deal_price_type_provisional dpt
		OUTER APPLY (SELECT DISTINCT formula_id FROM deal_detail_formula_udf u WHERE u.deal_price_type_id = dpt.deal_price_type_provisional_id ) sub
		WHERE dpt.source_deal_detail_id = @source_deal_detail_id
	...
	BEGIN
		SELECT @price_adjustment_form_json = ISNULL(@price_adjustment_form_json + ',', '') +
									...
									+ '", "adjustment_fields" : {' + STUFF((SELECT ', ' + '"UDF___' +  CAST(udf_template_id AS VARCHAR(10)) + '" : "' + udf_value + '"'
									  ...
									  FOR XML PATH('')), 1, 2, '') + '}}'
		FROM deal_price_type_provisional dpt
		OUTER APPLY (SELECT DISTINCT formula_id FROM deal_price_adjustment u WHERE u.deal_price_type_id = dpt.deal_price_type_provisional_id ) sub
		WHERE dpt.source_deal_detail_id = @source_deal_detail_id
...
BEGIN
    IF EXISTS(
        SELECT top 1 1 FROM deal_price_type_provisional WHERE source_deal_detail_id = @source_deal_detail_id 
    ) 
    BEGIN
...
BEGIN 
    ...
    IF EXISTS (
        SELECT TOP 1 1 FROM source_deal_header sdh 
            ...
            INNER JOIN deal_price_type_provisional dpt ON dpt.source_deal_detail_id = sdd.source_deal_detail_id
...
BEGIN
	DECLARE @deal_pricing_table VARCHAR(300)
 	...
 	SET @deal_pricing_table = dbo.FNAProcessTableName('deal_provisional_pricing_table', @user_name, @process_id)	
 	
	SET @sql = '
		DECLARE @indexed_price_id INT = 103601
							
		INSERT INTO deal_price_type_provisional (source_deal_detail_id, price_type_id, priority)
...
BEGIN
	...
	EXEC [dbo].[spa_deal_pricing_detail_provisional] @flag = 't', @source_deal_detail_id = @first_detail_id, @mode = 'fetch', @ids_to_apply_price = @first_detail_id, @output = @process_id_out OUTPUT

	SELECT @sql = ISNULL(@sql + ';', '') + 'EXEC [dbo].[spa_deal_pricing_detail_provisional] @flag = ''t'', @xml_process_id = ''' + @process_id_out + ''', @source_deal_detail_id = ' + CAST(source_deal_detail_id AS VARCHAR(10)) + ', @mode = ''fetch'', @ids_to_apply_price = ' + CAST(source_deal_detail_id AS VARCHAR(10)) + ';'
	FROM source_deal_detail 
	WHERE source_deal_header_id = @source_deal_detail_id
...
BEGIN
	...
	EXEC('
		...
		INNER JOIN adiha_process.dbo.provisional_pricing_xml_' + @user_name + '_' + @xml_process_id + ' p
