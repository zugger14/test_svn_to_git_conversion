...
/****** Object:  StoredProcedure [dbo].[spa_Get_Risk_Control_Activities_approve]  Script Date: 10/22/2008 11:49:38 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[spa_Get_Risk_Control_Activities_approve]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[spa_Get_Risk_Control_Activities_approve]
...
CREATE PROC [dbo].[spa_Get_Risk_Control_Activities_approve]
...
r.risk_control_reminder_id, k.as_of_date
into #reminder1
from #tempControls a INNER JOIN
process_risk_controls_reminders r on r.risk_control_id = a.risk_control_id
left outer join process_risk_controls_reminders_acknowledge k on k.risk_control_reminder_id = r.risk_control_reminder_id 
and k.as_of_date = a.as_of_date 
where k.as_of_date is null
group by a.risk_control_id, r.risk_control_reminder_id, k.as_of_date
...
select r1.risk_control_id, r1.risk_control_reminder_id, r1.Remind
into #reminder
from
...
CREATE TABLE #tempControls_test (
	[subsidiary_id] [varchar] (50) COLLATE DATABASE_DEFAULT NOT NULL ,
	[Subsidiary] [varchar] (50) COLLATE DATABASE_DEFAULT  NOT NULL ,
	[perform_role_id] [int] NOT NULL ,
	[PerformRole] [varchar] (50) COLLATE DATABASE_DEFAULT  NOT NULL ,
	[approve_role_id] [int] NOT NULL ,
	[ApproveRole] [varchar] (50) COLLATE DATABASE_DEFAULT  NOT NULL ,
    ...
    [proof] [varchar] (100) COLLATE DATABASE_DEFAULT NULL,
    [action] [varchar] (100) COLLATE DATABASE_DEFAULT  
...
SET @sql_stmt = 'INSERT #tempControls_test
	...
	case when (''' + @unapporved_flag + ''' = ''R'' AND #reminder.risk_control_id is not null) then #reminder.risk_control_reminder_id else #tempControls.approve_role_id end approve_role_id,
...
-- 	  case when (isnull(prca.monetary_value, #tempControls.monetary_value) is not null) then '' (Monetary Value: ''+ cast(isnull(prca.monetary_value, #tempControls.monetary_value) as varchar) + '')''else '''' end control_activity,
    ...
    ''<IMG SRC="./adiha_pm_html/process_controls/doc.jpg>'' as proof,
    '' '' as action
...
BEGIN
	...
	If upper(@unapporved_flag) = 'R'
		SET @sql_stmt = @sql_stmt + ' AND (CONVERT(DATETIME,''' + @as_of_date + ''', 102) <> CONVERT(DATETIME, #tempControls.as_of_date, 102) AND 
								((prca.control_status IS NULL OR prca.control_status in (725, 726)) AND  #reminder.risk_control_id is not null)) '
	--Not completed		
...
SELECT #tempControls_test.Subsidiary as Org, 
       
       dbo.FNAComplianceHyperlink('a',10181310,#tempControls_test.ApproveRole, cast(#tempControls_test.approve_role_id as varchar),default,default,default,default,default,default) Role,

       #tempControls_test.run_frequency as Frequency,
       #tempControls_test.risk_priority as Priority,
      
	   dbo.FNAComplianceHyperlink('b',10101125,#tempControls_test.control_activity,'''44''',cast(risk_control_id as varchar),default,default,default,default,default) as Activity,	
      
      
       dbo.FNAComplianceHyperlink('b',10181310,#tempControls_test.activity_status_desc, cast(risk_control_id as varchar),''''+as_of_date_argument+'''',default,default,default,default,default) as Status,
       #tempControls_test.monetary_penalty as 'Penalty/Fees',
       --('By:'+#tempControls_test.run_by+' '+ '(On:'+cast(#tempControls_test.run_date as varchar) +')' ) as Peformer,
       --('By:'+#tempControls_test.approved_by+' '+ '(On:'+cast(#tempControls_test.approved_date as varchar) +')' ) as Approver,
       --#tempControls_test.Proof as Proof,
       dbo.FNAComplianceHyperlink('c',10101125,'<IMG SRC=''http://gambhir-pc/rec4.1/adiha.php.scripts/adiha_pm_html/process_controls/doc.jpg''>',cast(risk_control_id as varchar),''''+as_of_date_argument+'''','''u''',default,default,default,default) as Proof,
      
       case when (activity_status in (727,728)) then 
       dbo.FNAComplianceHyperlink('c',10101142,#tempControls_test.action +'Approve', cast(#tempControls_test.risk_control_id as varchar),''''+#tempControls_test.as_of_date_argument+'''','4',default,default,default,default)+ ' '+dbo.FNAComplianceHyperlink('c',402,#tempControls_test.action +'Unapprove', cast(#tempControls_test.risk_control_id as varchar), ''''+#tempControls_test.as_of_date_argument+'''','''u-5''',default,default,default,default) 
       when activity_status='726' then 
       dbo.FNAComplianceHyperlink('c',10101142,#tempControls_test.action +'Approve', cast(#tempControls_test.risk_control_id as varchar),''''+#tempControls_test.as_of_date_argument+'''','4',default,default,default,default)
       when activity_status='729' then 
       dbo.FNAComplianceHyperlink('c',10101125,#tempControls_test.action +'Unapprove', cast(#tempControls_test.risk_control_id as varchar), ''''+#tempControls_test.as_of_date_argument+'''','''u-5''',default,default,default,default) 
       else
       #tempControls_test.action end as Action
