IF  EXISTS (SELECT * FROM sys.objects WHERE OBJECT_ID = OBJECT_ID(N'[dbo].[spa_rfx_deploy_rdl_as_job]') AND TYPE IN (N'P', N'PC'))
	DROP PROCEDURE [dbo].[spa_rfx_deploy_rdl_as_job]
...
/**
	Deploy RDL file using SQL Job which uses CLR functionality for RDL deployment
	Parameters
	...
	@target_folder	:	SSRS Folder name where RDL is to be deployed
*/

CREATE PROCEDURE [dbo].[spa_rfx_deploy_rdl_as_job] 
	@proc_desc VARCHAR (1000)
	,@user_login_id VARCHAR(50)
	,@job_subsystem VARCHAR(100)='CmdExec' -- SSIS for creating job SSIS package
	,@report_page_id INT
	,@report_name VARCHAR(1024)
	,@target_folder VARCHAR(128) = NULL 
AS
...
SET NOCOUNT ON
	...
	SET @rdl_cmd_failed = 'EXEC ' + @db_name + '.dbo.spa_rfx_report_page ''p'', NULL, ' + CAST(@report_page_id as varchar(100)) 
						+ ',NULL,NULL,NULL,NULL,NULL,NULL,NULL,''' + @user_name + ''',''' + @proc_desc  + ''',''' + @desc + ''''

	SET @rdl_cmd_success = 'EXEC ' + @db_name + '.dbo.spa_rfx_report_page ''q'', NULL, ' + CAST(@report_page_id as varchar(100)) 
	...
	BEGIN
		EXEC [dbo].[spa_manage_memcache] @flag = 'd', @key_prefix = @memcache_key, @source_object = 'spa_rfx_deploy_rdl_as_job'
	END		
	...
	BEGIN
		...
		DECLARE @deploy_cmd VARCHAR(1024) = 'EXEC [spa_deploy_rdl_using_clr] ''' + @report_name + ''',''Gemerated ' + @report_name + ' via SQL Job'' ,''''' 
		IF @target_folder IS NOT NULL
			SET @deploy_cmd = 'EXEC [spa_deploy_rdl_using_clr] ''' + @report_name + ''',''Gemerated ' + @report_name + ' via SQL Job'',''' + @target_folder + ''''
		
		EXEC msdb.dbo.sp_add_jobstep @job_name = @rdl_job_name,
				@step_id = 1,
				@step_name = 'Step1 : Deploy RDL to ReportServer',
				@subsystem = 'TSQL',-- @job_subsystem, --'TSQL',				
				@on_success_action =4, 
