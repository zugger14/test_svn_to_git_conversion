 ...
 IF OBJECT_ID(N'[dbo].[spa_view_expected_return]', N'P') IS NOT NULL
   DROP PROCEDURE [dbo].spa_view_expected_return
 ...
 CREATE PROCEDURE [dbo].[spa_view_expected_return]
 	@flag CHAR(1) = Null,
 	@xml XML= NULL,
...
/*******************************************1st Paging Batch END**********************************************/
 ...
 DECLARE @process_table VARCHAR(500) = dbo.FNAProcessTableName('view_expected_return',dbo.FNADBUser(),@process_id)
...
BEGIN
	SET @as_of_date_from = (SELECT CAST(dbo.FNAGetSQLStandardDate(MAX(as_of_date)) AS VARCHAR(20)) 
								FROM expected_return er
								INNER JOIN source_price_curve_def spcd ON er.curve_id = spcd.source_curve_def_id 
								inner JOIN static_data_value sdv ON sdv.value_id = er.curve_source_value_id
								INNER JOIN dbo.SplitCommaSeperatedValues(@source_price_curve) a ON er.curve_id = a.item
								INNER JOIN dbo.SplitCommaSeperatedValues(@curve_source_value) b ON er.curve_source_value_id = b.item
							)
...
END
 ...
 BEGIN		
	...
	IF @as_of_date_from IS NULL
	BEGIN
		SET @as_of_date_from = (SELECT CAST(dbo.FNAGetSQLStandardDate(MAX(as_of_date)) AS VARCHAR(20)) FROM expected_return)
		SET @as_of_date_to = (SELECT @as_of_date_from)
		IF @as_of_date_to IS NULL
		BEGIN
			SET @as_of_date_to = (SELECT @as_of_date_from)
	...
	END
 	...
 	BEGIN
		...
		END
 		...
 		IF @tenor_from =''
 		BEGIN
 			SELECT @tenor_from =CONVERT(DATETIME,MIN(term),103) FROM expected_return er INNER JOIN #source_price_curve_list spcl 
 			ON er.curve_id = spcl.price_curve_id 
 			WHERE 
		...
		--IF NOT EXISTS(SELECT 1 FROM expected_return 
		...
		--END
 		...
 		FROM dbo.FNATermBreakdownDST(LEFT(@granularity,1),@tenor_from,@tenor_to,ISNULL(@time_zone,102200))
 	...
 	SET @sql = ' INSERT INTO #price_curve_column_header(source_curve_def_id,curve_name,curve_value,as_of_date,maturity_date,curve_source_value_id,column_header)
 				SELECT 
 					...
 					spcd.curve_name,
					' + CASE WHEN NULLIF(@round_value,'') IS NOT NULL THEN 'ROUND(er.~value~, ' + @round_value+ ')' ELSE 'er.~value~' END + ' as curve_value,
 					Convert(VARCHAR(12),er.as_of_date,101),
 					Convert(VARCHAR(12),er.~term~,101),
 					er.curve_source_value_id,
 					Convert(VARCHAR(12),dbo.FNADateFormat(er.as_of_date),101)'+'+'+''''+'::'+''''+'+'+'spcd.curve_name'+'+'+''''+'::'+''''+'+'+'sdv.code AS column_header					
 			FROM 
 				source_price_curve_def spcd 
 				INNER JOIN #source_price_curve_list spcl
 					ON spcd.source_curve_def_id = spcl.price_curve_id
 					and isnull(spcd.derive_on_calculation,''n'')=''~y_n~''
 				INNER JOIN ~expected_return~ er 
 					ON er.~curve_id~ = spcl.price_curve_id
 					and er.curve_source_value_id IN('+isnull(@curve_source_value,'4500')+')
 				LEFT JOIN static_data_value sdv ON er.curve_source_value_id = sdv.value_id AND sdv.type_id = 10007
 				...
 				LEFT JOIN holiday_group hg ON hg.hol_group_value_id =spcd.exp_calendar_id
 					AND hg.hol_date = er.~term~ AND hg.exp_date = er.as_of_date
 				LEFT JOIN static_data_value sdv2 ON sdv2.value_id = hg.hol_group_value_id AND sdv2.type_id = 10017
 								
 				'
 		SET @where_sql = @where_sql +CASE WHEN @as_of_date_from IS NOT NULL  THEN ' AND er.as_of_date >= '+''''+ @as_of_date_from +'''' ELSE '' END
 										+CASE  WHEN @as_of_date_to IS NOT NULL THEN ' AND er.as_of_date <= '+''''+ @as_of_date_to +''''   ELSE '' END
 										+CASE  WHEN @tenor_from IS NOT NULL THEN ' AND er.~term~ >= '+''''+ @tenor_from  +'''' ELSE '' END	  
 										+CASE  WHEN @tenor_to IS NOT NULL THEN ' AND er.~term~ <= '+''''+ @tenor_to +''''  ELSE '' END
 		...
 		set @sql=REPLACE(@sql,'~expected_return~','expected_return')
 		...
 		SET @pivot_query_sql1 = 'SELECT * INTO '+ @process_table + ' 
 			...
 			CASE WHEN c.name = ''Maturity Date'' THEN ''a_1''
 					ELSE ''a_5'' END a2
 			FROM adiha_process.sys.[columns] c WITH(NOLOCK) INNER JOIN adiha_process.sys.tables t WITH(NOLOCK) on t.object_id = c.object_id   where t.name  ='+''''+@table_name+''') a1 WHERE name <> ''row_ord'''
 	...
 	BEGIN
 			...
 			IF OBJECT_ID(N''tempdb..#temp_table'') IS NOT NULL
 				...
 				'SELECT c.name, '+''''+@process_id+'''' +' as process_id,c.column_id,
 										CASE WHEN c.name = ''Maturity Date'' THEN ''a_1''
 											 ELSE ''a_5'' END a2
 										FROM adiha_process.sys.[columns] c WITH(NOLOCK) INNER JOIN adiha_process.sys.tables t WITH(NOLOCK) on t.object_id = c.object_id   where t.name  ='+''''+@table_name+''''
...
BEGIN
 	...
 	IF @grid_xml IS NOT NULL
 			BEGIN
 				...
 				--SELECT * FROM #temp_price_curve
 					...
 					SELECT 
 						...
 						a.value,
 						CASE WHEN er.expected_return_id IS NULL THEN ''i'' ELSE ''u'' END insert_update,
 						a.forward_settle
 					...
 					LEFT JOIN expected_return er ON dbo.FNADateFormat(er.term) = dbo.FNADateFormat(a.maturity_date) 
 					AND dbo.FNADateFormat(er.as_of_date) = dbo.FNADateFormat(a.as_of_date) 
 					AND er.curve_source_value_id = sdv.value_id
 					AND er.curve_id = spcd.source_curve_def_id
 					'
					--print @query_insert
 					EXEC(@query_insert)
 					--SELECT * FROM #temp_price_curve
					...
					UPDATE er
					SET er.as_of_date = CONVERT(VARCHAR,t.as_of_date,103),
 						er.curve_source_value_id = t.curve_source_value_id,
 						er.curve_id = t.curve_id,
 						er.term = CONVERT(VARCHAR,t.maturity_date,103),
 						er.value = NULLIF(t.value,''),
 						er.update_ts = getDate(),
						er.update_user = dbo.FNADBUser()	
 					FROM #temp_price_curve t 
 					INNER JOIN expected_return er ON dbo.FNADateFormat(er.term) = dbo.FNADateFormat(t.maturity_date)
 						AND	dbo.FNADateFormat(er.as_of_date) = dbo.FNADateFormat(t.as_of_date)
 						AND er.curve_id = t.curve_id
 						AND er.curve_source_value_id = t.curve_source_value_id
 					WHERE t.insert_update = 'u'	

 					INSERT INTO expected_return(as_of_date,curve_source_value_id,curve_id,term,value,granularity,create_user,create_ts)
 					SELECT 
 			...
 			BEGIN
 				...
 				SET @query_insert = @query_insert+ 'DELETE er 
 				FROM
 				'+ @delete_xml_table_name + ' a
 				INNER JOIN source_price_curve_def spcd on spcd.curve_name =  a.source_price_curve
				INNER JOIN static_data_value sdv On sdv.code = a.curve_source AND sdv.type_id = 10007
 				LEFT JOIN expected_return er ON 
 						dbo.FNADateFormat(er.term) = dbo.FNADateFormat(a.maturity_date)
 						AND	dbo.FNADateFormat(er.as_of_date) = dbo.FNADateFormat(a.as_of_date)
						AND er.curve_source_value_id = sdv.value_id
 				'
 			--print(@query_insert)
 	...
 	BEGIN CATCH
 		...
 		BEGIN
 				...
 				BEGIN
 					...
 					SET @table_name = 'expected_return'
 		...
 		ELSE 
 			...
 			EXEC spa_ErrorHandler -1, 'Process Form Data', 
 						'spa_view_expected_return', 'Error', 
 						@desc, ''
 		END
 	END CATCH
END
...
BEGIN
	...
	SET @select_sql = 'SELECT 
		er.expected_return_id [ID], 
		spcd.curve_id [Curve ID],
		spcd.curve_name [Curve Name],
		sdv.code [Curve Source], 
		dbo.FNADateFormat(er.as_of_date) [As of Date], 
		dbo.FNADateFormat(er.term) [Maturity Date],
		er.value [Curve Value]'+ @str_batch_table +'
	FROM expected_return er
	INNER JOIN source_price_curve_def spcd ON er.curve_id = spcd.source_curve_def_id 
	LEFT JOIN static_data_value sdv ON sdv.value_id = er.curve_source_value_id
	'

	IF @source_price_curve IS NOT NULL AND @source_price_curve <> ''
		SET @select_sql += ' INNER JOIN dbo.SplitCommaSeperatedValues(''' + @source_price_curve + ''') a ON er.curve_id = a.item'

	IF @curve_source_value IS NOT NULL  AND @curve_source_value <> ''
		SET @select_sql += ' INNER JOIN dbo.SplitCommaSeperatedValues(''' + @curve_source_value + ''') b ON er.curve_source_value_id = b.item'
	
	SET @select_sql += ' WHERE 1 =1 '
	
	IF @as_of_date_from IS NOT NULL  AND @as_of_date_from <> ''
		SET @select_sql += ' AND er.as_of_date >= ''' + @as_of_date_from + ''''

	IF @as_of_date_to IS NOT NULL  AND @as_of_date_to <> ''
		SET @select_sql += ' AND er.as_of_date <= ''' + @as_of_date_to + ''''

	IF @tenor_from IS NOT NULL  AND @tenor_from <> ''
		SET @select_sql += ' AND er.term >= ''' + @tenor_from + ''''

	IF @tenor_to IS NOT NULL  AND @tenor_to <> ''
		SET @select_sql += ' AND er.term <= ''' + @tenor_to + ''''

	SET @select_sql += ' AND DATEPART(d, er.term) = 1' -- Retrieve data of first day.
...
BEGIN
	...
	SELECT @str_batch_table = dbo.FNABatchProcess ('c', @batch_process_id, @batch_report_param, 
				   GETDATE(), 'spa_view_expected_return', 'View Expected Return') --TODO: modify sp and report name
