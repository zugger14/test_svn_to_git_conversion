...
IF OBJECT_ID(N'[dbo].[spa_rfx_report_page_tablix_dhx]', N'P') IS NOT NULL
    DROP PROCEDURE [dbo].[spa_rfx_report_page_tablix_dhx]
GO
...
GO
/**
	CRUD operations for report page tablix. created for new report manager dhx. currently in use.
	Parameters
	@flag					: Flag
	@report_page_tablix_id	: Report Page Tablix Id
	@root_dataset_id		: Root Dataset Id
	@tablix_name			: Tablix Name
	@process_id				: Process Id
	@width					: Width of tablix
	@height					: Height of tablix
	@left					: Left position of tablix
	@page_id				: Page Id
	@top					: Top position of tablix
	@xml_column				: Tablix column informations in XML format
	@group_mode				: Grouping Mode
	@border_style			: Border Style
	@xml_header				: Tablix header information in XML format
	@page_break				: Page Break
	@type_id				: Tablix type Id
	@cross_summary			: Cross Summary
	@no_header				: No Header
	@export_table_name		: Export Table Name
	@is_global				: Is Global
*/
CREATE PROCEDURE [dbo].[spa_rfx_report_page_tablix_dhx]
    @flag						CHAR(1),
...
DECLARE @user_name                       VARCHAR(50),
		...
		@rfx_validation_table			 VARCHAR(200),
		@issue_column_id				 VARCHAR(1000)
...
SET @rfx_validation_table = dbo.FNAProcessTableName('report_validation_table', @user_name, @process_id)
...
BEGIN
	 SET @sql = 'SELECT col.column_id,
						...
						case when rel.is_connected is null then ds.alias else rd.[alias] end + ''.'' + data_col.name as column_real_name_pivot,
				...
				outer apply (
					...
					from ' + @rfx_report_dataset + ' rd1 
					where rd1.root_dataset_id = rd.report_dataset_id or (rd1.report_dataset_id = rd.report_dataset_id and rd1.root_dataset_id is not null)
				) rel
				--OUTER APPLY(
...
BEGIN
	...
	BEGIN
		...
		END	
			...
			DECLARE @actual_function_name VARCHAR(1000)
			DECLARE @source_id INT

			DECLARE @sql_cmd NVARCHAR(2000)

			SET @sql_cmd = 'SELECT @source_id = rd.source_id FROM ' + @rfx_report_dataset + ' rd WHERE rd.report_dataset_id = ' + CAST(@root_dataset_id AS VARCHAR(10))
			EXECUTE sp_executesql @sql_cmd, N'@source_id INT OUTPUT', @source_id = @source_id OUTPUT
			
			SELECT @columns =  @columns + '[' + dsc.name + '] ' + rd.name + ', '
			FROM data_source_column dsc
			INNER JOIN report_datatype rd
				ON dsc.datatype_id = rd.report_datatype_id
			WHERE dsc.source_id = @source_id

			IF OBJECT_ID(@rfx_validation_table) IS NOT NULL
				EXEC('drop table '+ @rfx_validation_table)

			EXEC ('CREATE TABLE ' + @rfx_validation_table + '( ' + @columns + ')')


			DECLARE cur_status CURSOR LOCAL FOR
			SELECT REPLACE(functionName, rd.alias + '.', '') functionName, functionName actual_function_name
			FROM #rfx_tablix_column rtc
			INNER JOIN report_dataset rd
				ON rtc.datasetid = rd.report_dataset_id  
			WHERE NULLIF(functionname, '') IS NOT NULL

			OPEN cur_status;

			FETCH NEXT FROM cur_status INTO @functionName, @actual_function_name
			WHILE @@FETCH_STATUS = 0
			BEGIN

				SET @issue_column_id = @actual_function_name

				SELECT @sql =' SET NOEXEC ON SELECT ' +  @functionName + ' FROM ' + @rfx_validation_table
				EXEC(@sql)
			...
			UPDATE #rfx_tablix_column SET SortingColumn = NULL WHERE  SortingColumn IN ('','undefined')	
	...
	END
		...
		IF nullif(@report_page_tablix_id,'') IS NOT NULL
		BEGIN
				...
				SET @sql +=		'	--DELETE FROM ' + @rfx_report_tablix_column + ' WHERE tablix_id = ' + CAST(@report_page_tablix_id AS VARCHAR) + '
						...
						WHEN MATCHED THEN	
							UPDATE SET rtc.tablix_id = ' + CAST(@report_page_tablix_id AS VARCHAR) + ',
									   ...
									   rtc.sorting_column = tmp_tablix_col.SortingColumn,
			...
			BEGIN			
				SET @sql += '			SET @tablix_id = IDENT_CURRENT(''' + @rfx_report_page_tablix + ''')
						...
						INSERT INTO ' + @rfx_report_tablix_column + ' (
							...
							font, font_size, font_style, text_align, text_color, default_sort_order, sorting_column, default_sort_direction,
						...
						SELECT @tablix_id, ColumnID, Placement, Aggregation,SQLAggregation, Subtotal, FunctionName, ColumnAlias, SortLinkHeader, Rounding, ThousandSeperator,
							   Font, FontSize, FontStyle, TextAlign, TextColor, SortPriority, SortingColumn, SortTo,
		...
		BEGIN
			...
			* */
			...
			WHILE @@FETCH_STATUS = 0   
			BEGIN
				EXEC spa_rfx_report_default_paramset_dhx 'd', 't', @user_name, @process_id, @old_root_dataset_id
				FETCH NEXT FROM cur_get_rootdataset_before_update INTO @old_root_dataset_id
			END
			...
			* */ 
			EXEC spa_rfx_report_default_paramset_dhx 'i', 't', @user_name, @process_id, @root_dataset_id
		END
		
		SET @tablix_id_return = IDENT_CURRENT(@rfx_report_page_tablix)
		DECLARE @recommendation VARCHAR(2000) = CAST(@tablix_id_return AS VARCHAR(5)) + ',' + @tablix_name
			
		EXEC spa_ErrorHandler 0, 'Reporting FX', 'spa_rfx_report_page_tablix_dhx', 'Success', 'Data succesfully updated.', @tablix_id_return
	END TRY
	BEGIN CATCH
		...
		SET @DESC1 = 'Invalid Function: ''' + @issue_column_id + '''.'
		
		SELECT @err_no1 = ERROR_NUMBER()
	 
		EXEC spa_ErrorHandler 1, 'Reporting FX', 'spa_rfx_report_page_tablix_dhx', 'Error', @DESC1, @issue_column_id
	END CATCH
END
...
BEGIN
	BEGIN TRY
		...
		* */
			...
			SET @sql = 'DELETE FROM ' + @rfx_report_tablix_header + ' WHERE tablix_id = ' + CAST(@report_page_tablix_id AS VARCHAR)
			EXEC(@sql)
			...
			* */ 
			...
			EXEC spa_ErrorHandler 0, 'Reporting FX', 'spa_rfx_report_page_tablix_dhx', 'Success', 'Data successfully deleted.', @process_id
			COMMIT
	END TRY
	BEGIN CATCH
		...
		EXEC spa_ErrorHandler 0, 'Reporting FX', 'spa_rfx_report_page_tablix_dhx', 'Failed', 'Fail to delete data.', @process_id
	END CATCH	
END
