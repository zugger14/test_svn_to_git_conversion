IF OBJECT_ID('spa_calculate_options_using_R') IS NOT NULL
	DROP PROC dbo.spa_calculate_options_using_R
GO
CREATE PROCEDURE [dbo].[spa_calculate_options_using_R]
	@input_table_name VARCHAR(250),
	@output_table_name VARCHAR(100)
AS
/*
DECLARE @input_table_name VARCHAR(250)='adiha_process.dbo.option_param_farrms_admin_16F4421C_641F_46AF_86C0_69C7CC52CA28'
DECLARE @output_table_name VARCHAR(250) ='adiha_process.dbo.options_prem_detail_farrms_admin_16F4421C_641F_46AF_86C0_69C7CC52CA28' 
--*/
BEGIN
	...
	SET @sql_check  = 'SELECT @re = 1 FROM '+@input_table_name + ' WHERE Method = ''<<value>>'' AND IDT =''<<IDT>>'' AND option_excercise_type =''<<oet>>'''
	--print('v_bs')
--spa_calculate_options_Vanilla_BS76
	SET @sql = '
		...
		VanilaOption <- function(F1, K, v, r, tau, cp,Method,row_id,Attribute_type){ 
		 ...
		 #  v_theta <- ifelse(cp == "c", ((-F1*dnorm(d1)*v)/(2*sqrt(tau))- r*K* exp(-r*tau) * pnorm(d2)),((-F1*dnorm(d1)*v)/(2*sqrt(tau))-r*K* exp(-r*tau) * pnorm(-d2)))
		  ...
		  v_ops <- data.frame(row_id,v_options,v_delta1,v_gamma,v_vega,v_theta,v_Rho,v_delta2,v_gamma2,v_vega2,v_theta2,v_rho2,Method,Attribute_type,Option_data[,12],Option_data[,13])
	    ...
	    S1,S2,X,V1,V2,R,T,CallPutFlag, Method,row_id,Attribute_type,option_excercise_type,IDT
		FROM '+@input_table_name+' 
		WHERE Method = ''''45500'''' AND IDT = ''''2'''' AND option_excercise_type = ''''e'''' ;''
...
--spa_calculate_options_Vanilla_EUMC
	...
	option_EU <- data.frame(Option_data[,10],EU_MC[,1],EU_MC[,2],EU_MC[,4],EU_MC[,5],EU_MC[,6],EU_MC[,7],EU_MC[,3],EU_gamma2,EU_vega2,EU_theta2,EU_rho2,
	Option_data[,9],Option_data[,11],Option_data[,12],Option_data[,13])                       
	Option_data_out <- option_EU''     
	,@input_data_1 =N''SELECT  
		S1,S2,X,V1,V2,R,T,CallPutFlag, Method,row_id,Attribute_type,option_excercise_type,IDT
		FROM '+@input_table_name+' 
		WHERE Method = ''''45502'''' AND IDT = ''''2'''' AND option_excercise_type = ''''e'''' ;''
		,@input_data_1_name = N'' Option_data''
		,@output_data_1_name = N'' Option_data_out'' '
	SET @r  = 0 
...
SET @sql = '
	...
	option_US <- data.frame(Option_data[,10],US_MC[,1],US_MC[,2],US_MC[,4],US_MC[,5],US_MC[,6],US_MC[,7],US_MC[,3],US_gamma2,US_vega2,US_theta2,US_rho2,Option_data[,9],Option_data[,11],
	                         Option_data[,12],Option_data[,13])                       
	Option_data_out <- option_US''
	,@input_data_1 =N''SELECT  
		S1,S2,X,V1,V2,R,T,CallPutFlag, Method,row_id,Attribute_type,option_excercise_type,IDT
		FROM '+@input_table_name+' 
		WHERE Method = ''''45502'''' AND IDT = ''''2'''' AND option_excercise_type = ''''a'''' ;''
		,@input_data_1_name = N'' Option_data''
		,@output_data_1_name = N'' Option_data_out'' '
	SET @r  = 0 
	SET @sqsl = REPLACE(@sql_check,'<<value>>','45502')
	...
	---- Calculate spread options using Kirk approximation method
		...
		KirkSpreadOption <- function(row_id,F1,F2,K,v1,v2,r,tau,rho,cp,Method){ 
			...
			kirk_ops  <- data.frame(row_id,k_options,k_delta_1,k_gamma_1,k_vega_1,k_theta_1,k_rho_1,k_delta_2,k_gamma_2,k_vega_2,k_theta_2,k_rho_2,Method,Option_data[,12],Option_data[,13],Option_data[,14]);
			return(kirk_ops)
		}
		#----------------------Performing simulations-------------------
		...
		Option_data_out <- vspopt''
	    ,@input_data_1 =N''SELECT  
		S1,S2,X,V1,V2,R,T,C,CallPutFlag,Method,row_id,Attribute_type,option_excercise_type,IDT
		FROM '+@input_table_name+' 
		WHERE Method = ''''45503'''' AND IDT = ''''3'''' AND option_excercise_type = ''''e'''' ;''
		,@input_data_1_name = N'' Option_data''
		,@output_data_1_name = N'' Option_data_out'' '
	SET @r  = 0 
	SET @sqsl = REPLACE(@sql_check,'<<value>>','45503')
	SET @sqsl = REPLACE(@sqsl,'<<IDT>>','3')
	SET @sqsl = REPLACE(@sqsl,'<<oet>>','e')
		EXEC sp_executesql @sqsl,@param,@re =@r  OUT
		IF @r = 1 
		BEGIN
	...
	SET @sql = '
		...
		option_EU <- data.frame(Option_data[,11],EU_MC[,1],EU_MC[,2],EU_MC[,3],EU_MC[,4], EU_MC[,5],EU_MC[,6],
								EU_MC[,7], EU_MC[,8],EU_MC[,9], EU_MC[,10],EU_MC[,11],Option_data[,10],Option_data[,12],Option_data[,13],Option_data[,14])
		#----------------------Performing simulations-------------------
		Option_data_out <- option_EU''
				,@input_data_1 =N''SELECT  
				S1,S2,X,V1,V2,R,T,C,CallPutFlag,Method,row_id,Attribute_type,option_excercise_type,IDT
				FROM '+@input_table_name+' 
				WHERE Method = ''''45502'''' AND IDT = ''''3'''' AND option_excercise_type = ''''e'''';''
				,@input_data_1_name = N'' Option_data''
				,@output_data_1_name = N'' Option_data_out'' '
			SET @r  = 0 
			SET @sqsl = REPLACE(@sql_check,'<<value>>','45502')
	...
	SET @sql = '
			...
			npt <- nrow(Option_data);
			#----------------------------------------------------
			Simulation_US_Spark_spread <- function (n,d,s0_1,s0_2,K,sigma1,sigma2,r,M_T,corrl,cp1)
			{
			  library(Matrix)
			  library(stats)
			  dt <- M_T/d
				rho1 <- 0.999
				correlation_matrix <- matrix(c(1,rep(rho1,2),1),nrow=2)
				L <- t(chol(correlation_matrix))
				z <- matrix(rnorm(n*2),nrow=2)
				z <- L%*%z
				s1.t <- s0_1*exp((r-1/2*sigma1^2)*M_T + sigma1*(M_T^0.5)*z[1,])
				s2.t <- s0_2*exp((r-1/2*sigma2^2)*M_T + sigma2*(M_T^0.5)*z[2,])
				...
				z <- matrix(rnorm(n*2),nrow=2) 
				z <- L%*%z
				mean1 <- (log(s0_1)+ii*log(s1.t))/(ii+1)
				vol1 <- (ii*dt/(ii+1))^0.5*z[1,]
				s1.t_1 <- exp(mean1+sigma1*vol1)
    
				mean2 <- (log(s0_2)+ii*log(s2.t))/(ii+1)
				...
				se.c <- sd(payoff.c)/sqrt(n)
				#------------------------------------
				mcg5  <- US_greeks_cal(s0_1,s0_2,s1.t,s2.t,sigma1,sigma2,r,rho1,M_T1=dt,z,cp1)
				mcg1 <- c(usprice.c,mcg5)
				return(mcg1)
			}
            US_greeks_cal <- function(s0_1,s0_2,s1.t,s2.t,sigma1,sigma2,r,rho1,M_T1,z,cp)
				{
				  del <- 1
				  del1 <- 1
				 ...
				 # v1 <- v; v2<-v
				  ...
				  # calculation of delta1 and delta 2
				  s11 <- (s0_1+del)*exp((r-1/2*sigma1^2)*M_T1 + sigma1*(M_T1^0.5)*z[1,])
				  s12 <- (s0_1-del)*exp((r-1/2*sigma1^2)*M_T1 + sigma1*(M_T1^0.5)*z[1,])
  
				  s21 <- (s0_2+del1)*exp((r-1/2*sigma2^2)*M_T1 + sigma2*(M_T1^0.5)*z[2,])
				  s22 <- (s0_2-del1)*exp((r-1/2*sigma2^2)*M_T1 + sigma2*(M_T1^0.5)*z[2,])
				  ...
				  payoff_s11 <- mean(exp(-r*M_T1)*CC11); payoff_s12 <- mean(exp(-r*M_T1)*CC12);
				  payoff_s21 <- mean(exp(-r*M_T1)*CC21); payoff_s22 <- mean(exp(-r*M_T1)*CC22);
				  payoffs <-mean(exp(-r*M_T1)*CC)
				 ...
				 # v1 <- sigma1; v2<- sigma2 
				  ...
				  sv11 <- (s0_1)*exp((r-1/2*(v1+del)^2)*M_T1 + (v1+del)*(M_T1^0.5)*z[1,])
				  sv12 <- (s0_1)*exp((r-1/2*(v1-del)^2)*M_T1 + (v1-del)*(M_T1^0.5)*z[1,])
  
				  sv21 <- (s0_2)*exp((r-1/2*(v2+del1)^2)*M_T1 + (v2+del1)*(M_T1^0.5)*z[2,])
				  sv22 <- (s0_2)*exp((r-1/2*(v2-del1)^2)*M_T1 + (v2-del1)*(M_T1^0.5)*z[2,])
				  ...
				  payoff_sv11 <- mean(exp(-r*M_T1)*CCv11); payoff_sv12 <- mean(exp(-r*M_T1)*CCv12);
				  payoff_sv21 <- mean(exp(-r*M_T1)*CCv21); payoff_sv22 <- mean(exp(-r*M_T1)*CCv22);
				  ...
				  del <- 1/365
				  #v1 <- v; v2<- v
				  st11 <- (s0_1)*exp((r-1/2*v1^2)*(M_T1+del) + v1 *((M_T1+del)^0.5)*z[1,])
				  st12 <- (s0_1)*exp((r-1/2*v1^2)*(M_T1-del) + v1 *((M_T1-del)^0.5)*z[1,])
  
				  st21 <- (s0_2)*exp((r-1/2*v2^2)*(M_T1+del) + v2*((M_T1+del)^0.5)*z[2,])
				  st22 <- (s0_2)*exp((r-1/2*v2^2)*(M_T1-del) + v2*((M_T1-del)^0.5)*z[2,])
				  ...
				  payoff_st11 <- mean(exp(-r*(M_T1+del))*CCt11); payoff_st12 <- mean(exp(-r*(M_T1-del))*CCt12);
				  payoff_st21 <- mean(exp(-r*(M_T1+del))*CCt21); payoff_st22 <- mean(exp(-r*(M_T1-del))*CCt22);
  
				  mc_t1 <- (payoff_st11-payoff_st12)/(2*del*365)
				  mc_t2 <- (payoff_st21-payoff_st22)/(2*del*365)
  
				  # calculation of Rho
				  del <- 0.01
				  #v1 <- v; v2<- v 
  
				  sr11 <- (s0_1)*exp(((r+del)-1/2*v1^2)*(M_T1) + v1 *((M_T1)^0.5)*z[1,])
				  sr12 <- (s0_1)*exp(((r-del)-1/2*v1^2)*(M_T1) + v1 *((M_T1)^0.5)*z[1,])
  
				  sr21 <- (s0_2)*exp(((r+del)-1/2*v2^2)*(M_T1) + v2*((M_T1)^0.5)*z[2,])
				  sr22 <- (s0_2)*exp(((r-del)-1/2*v2^2)*(M_T1) + v2*((M_T1)^0.5)*z[2,])
				  ...
				  payoff_sr11 <- mean(exp(-(r+del)*(M_T1))*CCr11); payoff_sr12 <- mean(exp(-(r-del)*(M_T1))*CCr12);
				  payoff_sr21 <- mean(exp(-(r+del)*(M_T1))*CCr21); payoff_sr22 <- mean(exp(-(r-del)*(M_T1))*CCr22);
  
				  mc_r1 <- (payoff_sr11-payoff_sr12)/(2*del*100)
				  mc_r2 <- (payoff_sr21-payoff_sr22)/(2*del*100)
		...
		for (ik in 1:npt)
		{
		  s0_1 <- Option_data[ik,1]; s0_2 <- Option_data[ik,2]; K <- Option_data[ik,3]; sigma1 <- Option_data[ik,4]
		  sigma2 <- Option_data[ik,5]; r <- Option_data[ik,6]; M_T <- Option_data[ik,7]; corrl <- Option_data[ik,8]
		  cp1 <- Option_data[ik,9]
		  USoption <- Simulation_US_Spark_spread (n,d,s0_1,s0_2,K,sigma1,sigma2,r,M_T,corrl,cp1)
		  US_MC[ik,] <- USoption
		}
		option_US <- data.frame(Option_data[,11],US_MC[,1],US_MC[,2],US_MC[,3],US_MC[,4], US_MC[,5],US_MC[,6],
								US_MC[,7], US_MC[,8],US_MC[,9], US_MC[,10],US_MC[,11],Option_data[,10],Option_data[,12],Option_data[,13],Option_data[,14])
		#----------------------Performing simulations-------------------
		Option_data_out <- option_US''
				,@input_data_1 =N''SELECT  
				S1,S2,X,V1,V2,R,T,C,CallPutFlag,Method,row_id,Attribute_type,option_excercise_type,IDT
				FROM '+@input_table_name+' 
				WHERE Method = ''''45502'''' AND IDT = ''''3'''' AND option_excercise_type = ''''a'''';''
				,@input_data_1_name = N'' Option_data''
				,@output_data_1_name = N'' Option_data_out'' '
			SET @r  = 0 
			SET @sqsl = REPLACE(@sql_check,'<<value>>','45502')
		...
		-- Bionomial methods 
		SET @sql_check  = 'SELECT @re = 1 FROM '+@input_table_name + ' WHERE Method = ''<<value>>'' AND IDT =''<<IDT>>'' '
	...
	# Accumulating Results
	 option_BT <- data.frame(Option_data[,10],BT[,1],BT[,2],BT[,3],bt_v,BT[,4],bt_r,bt_kd,bt_gamma2,bt_vega2,bt_theta2,bt_rho2,Method,Option_data[,11],Option_data[,12],Option_data[,13])
	...
	SET @sql = '
			...
			ExerciseType <- Option_data[ik,13]
			#------------------------------------------
			...
			bt_sp_opt <- data.frame(Option_data[,11],BT,bt_d1,bt_g1,
							bt_v1,bt_t,bt_r,bt_d2,bt_g2,bt_v2,bt_t2,bt_r2,Option_data[,10],Option_data[,12],Option_data[,13],Option_data[,14])
