...
/*--------------Debug Section------------
EXEC sys.sp_set_session_context @key = N'DB_USER', @value = 'bijan.karki';
...
--grid_value_id
	...
	IF OBJECT_ID('tempdb..#temp_max_date_settlement') IS NOT NULL
		DROP TABLE #temp_max_date_settlement
	
	CREATE TABLE #temp_max_date_settlement (
		source_deal_header_id INT,
		settlement_as_of_date DATETIME,
		term_end DATETIME,
		leg int
	)
	...
	BEGIN
		...
		SELECT source_deal_header_id, MAX(pnl_as_of_date) pnl_as_of_date FROM source_deal_pnl_detail GROUP BY source_deal_header_id
	...
	INSERT INTO #temp_max_date_settlement (source_deal_header_id, settlement_as_of_date, leg, term_end)
		SELECT source_deal_header_id, MAX(as_of_date), leg, term_end
		FROM source_deal_settlement GROUP BY source_deal_header_id, leg, term_end
	CREATE NONCLUSTERED INDEX idx_temp_max_date_stlmt_as_of_date ON #temp_max_date_settlement (settlement_as_of_date)
	CREATE NONCLUSTERED INDEX idx_temp_max_date_stlmt_source_deal_header_id ON #temp_max_date_settlement (source_deal_header_id)
	END	
	...
	BEGIN		
		...
		BEGIN
			SET @sql_select += '
							...
							COALESCE(ds.settlement_amount,fbds.settlement_amount, dp.und_pnl_set) deal_value,
							CASE
								WHEN cfv.finalized = ''y'' THEN ''Final'' 
								ELSE ''Estimate''						
							END estimate_final,
	...
	BEGIN
		SET @sql_from +=  '
						  LEFT JOIN (
							...
							INNER JOIN #temp_max_date_settlement tmpstlmt 
								ON tmpstlmt.settlement_as_of_date = sds.as_of_date
								AND tmpstlmt.source_deal_header_id = sds.source_deal_header_id
								AND tmpstlmt.leg = sds.leg
								AND tmpstlmt.term_end = sds.term_end
							GROUP BY sds.source_deal_header_id
						  ) ds ON ds.source_deal_header_id = dh.source_deal_header_id
						  LEFT JOIN (
							SELECT ifbs.source_deal_header_id, 
								   sum(ifbs.[value]) settlement_amount
							FROM index_fees_breakdown_settlement ifbs
							INNER JOIN #temp_max_date_pnl tmpstlmt 
								ON tmpstlmt.source_deal_header_id = ifbs.source_deal_header_id
								AND tmpstlmt.pnl_as_of_date = ifbs.term_start
							WHERE ifbs.field_id in (-10000368, -10000369)
							GROUP BY ifbs.source_deal_header_id
						 ) fbds ON fbds.source_deal_header_id = dh.source_deal_header_id
						  OUTER APPLY (
		                    SELECT ISNULL(MAX(cfv.finalized), ''n'') finalized,
		                    	   MAX(cfv.calc_id) calc_id
