IF OBJECT_ID(N'[dbo].[spa_rfx_init_dhx]', N'P') IS NOT NULL
    DROP PROCEDURE [dbo].[spa_rfx_init_dhx]
GO
...
-- Description: Init Logic saves all the temporary information of the report to respective temporary tables named (created to use on new report manager dhx, currently in use)
...
-- Sample Use
-- 1. Adding New Report		:: EXEC [spa_rfx_init_dhx] 'c'
-- 2. Updating New report	:: EXEC [spa_rfx_init_dhx] 'e', NULL,4
-- ===========================================================================================================
CREATE PROCEDURE [dbo].[spa_rfx_init_dhx]
	@flag CHAR(1),
...
set nocount on
	...
	--error log table for report manager
	DECLARE @rfx_err_log VARCHAR(1000) = dbo.FNAProcessTableName('rfx_err_log', @user_name, @process_id)
	if OBJECT_ID(@rfx_err_log) is not null
		exec('drop table ' + @rfx_err_log)
	set @sql = '
	create table ' + @rfx_err_log + ' (
		store_proc varchar(500)  null,
	...
	BEGIN
	    ...
	    --print 'Clone Reporting schemas to Process DB :: ' + @sql
	    EXEC (@sql)
	    
	    IF @@ERROR <> 0
	        EXEC spa_ErrorHandler @@ERROR, 'Reporting FX', 'spa_rfx_init_dhx', 'DB Error', 'Cloned Reporting schemas to Process DB.', ''
	    ELSE
	        EXEC spa_ErrorHandler 0, 'Reporting FX', 'spa_rfx_init_dhx', 'Success', 'Cloned Reporting schemas to Process DB.', @process_id
	END
	...
	BEGIN try
	    SET @sql = '----------------------------------------------------------------------------------
					...
					INSERT INTO ' + @rfx_report_chart_column + '(report_chart_column_id, chart_id, column_id, placement, column_order, alias, create_user, create_ts, update_user, update_ts, dataset_id, functions, aggregation, default_sort_order, sorting_column, default_sort_direction, custom_field, render_as_line) 
					SELECT rcc.report_chart_column_id, rcc.chart_id, rcc.column_id, rcc.placement, rcc.column_order, rcc.alias, rcc.create_user, rcc.create_ts, rcc.update_user, rcc.update_ts, rcc.dataset_id, rcc.functions, rcc.aggregation, rcc.default_sort_order, rcc.sorting_column, rcc.default_sort_direction, rcc.custom_field, rcc.render_as_line
		...
		SET @sql_1_1 = 			
					...
					INSERT INTO ' + @rfx_report_paramset + ' (report_paramset_id , page_id, [name], paramset_hash, create_user, create_ts, update_user, update_ts, report_status_id, export_report_name, export_location, output_file_format, delimiter, xml_format, compress_file, report_header, category_id)
					SELECT rp.report_paramset_id, rp.page_id, rp.[name], rp.paramset_hash, rp.create_user, rp.create_ts, rp.update_user, rp.update_ts, rp.report_status_id, rp.export_report_name, rp.export_location, rp.output_file_format, rp.delimiter, rp.xml_format, rp.compress_file, rp.report_header, rp.category_id
	 ...
	 SET @sql_2 = '
	    ...
	    --print 'Clone Reporting schemas to Process DB :: ' 
	    --print @sql 
	    --print @sql_1
	    --print @sql_1_1
	    --print @sql_2
	    ...
	    declare @recommendation varchar(2000) = @process_id + ',' + @report_id
	    EXEC spa_ErrorHandler 0, 'Reporting FX', 'spa_rfx_init_dhx', 'Success', 'Cloned Existing Report schemas and data to Process DB.', @recommendation
	END TRY
	BEGIN CATCH
		DECLARE @catch_err_msg VARCHAR(100)
		SET @catch_err_msg = ERROR_MESSAGE()
		EXEC spa_ErrorHandler 1, 'Reporting FX', 'spa_rfx_init_dhx', 'DB Error', 'Cloned Existing Report schemas and data to Process DB.', @catch_err_msg
	END CATCH
	...
	BEGIN
	    ...
	    --print 'Deleted Scehmas From Process DB :: ' + @sql
	    EXEC (@sql)
	    
		/* DROP ALL PROCESS TABLES RELATED TO PROCESS ID START */
		declare @ptable_name_c varchar(1000) = null
		
		/* cursor existence check, if exists and empty close and deallocate */
		IF (SELECT CURSOR_STATUS('global','cur_del_process_tables')) >= -1
		BEGIN
			IF (SELECT CURSOR_STATUS('global','cur_del_process_tables')) > -1
			BEGIN
				CLOSE cur_del_process_tables
			END
			DEALLOCATE cur_del_process_tables
		END
		...
		fetch next from cur_del_process_tables into @ptable_name_c
		while @@FETCH_STATUS = 0
		begin
			exec('DROP TABLE adiha_process.dbo.' + @ptable_name_c)
			fetch next from cur_del_process_tables into @ptable_name_c
		end
		close cur_del_process_tables
		deallocate cur_del_process_tables

		/* DROP ALL PROCESS TABLES RELATED TO PROCESS ID END */

	    IF @@ERROR <> 0
	        EXEC spa_ErrorHandler @@ERROR, 'Reporting FX', 'spa_rfx_init_dhx', 'DB Error', 'Error in Delete Schemas From Process DB.', ''
	    ELSE
	        EXEC spa_ErrorHandler 0, 'Reporting FX', 'spa_rfx_init_dhx', 'Success', 'Deleted schemas from Process DB.', @process_id
	END
GO
