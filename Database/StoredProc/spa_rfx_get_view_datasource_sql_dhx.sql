...
IF  EXISTS (SELECT * FROM sys.objects WHERE OBJECT_ID = OBJECT_ID(N'[dbo].spa_rfx_get_view_datasource_sql_dhx') AND TYPE IN (N'P', N'PC'))
	DROP PROCEDURE [dbo].spa_rfx_get_view_datasource_sql_dhx
GO
...
--@criteria					VARCHAR(5000)  : criteria of the source
...
CREATE PROCEDURE [dbo].spa_rfx_get_view_datasource_sql_dhx
...
--/* ------------------------------------------------TEST SCRIPT------------------------------------------------*/
/*
DECLARE @datasource_sql				VARCHAR(MAX)='select * 
--[__batch_report__]
from {ARR1} a'
	, @data_source_process_id	VARCHAR(200)='99_csv_99'
	, @validate					BIT	= 0
	, @criteria					VARCHAR(5000) = 'as_of_date=2017-12-01,counterparty_id=NULL'
	, @paramset_id				INT = NULL
	, @with_criteria            CHAR(1)= 'y'
	, @final_sql				VARCHAR(MAX)  
	, @required_cols			VARCHAR(MAX)  
	, @process_id				VARCHAR(50)='C5C64655_58DF_4FCC_9DF4_7FA628E37F73'
...
begin try
	...
	*/
	...
	BEGIN 
		...
		--if call from csv generation then take info as above block(validate=1)
		if @data_source_process_id = '99_csv_99'
		begin
			SET @sql = '
			INSERT INTO #temp_map (start_index,view_name, source_id, Alias)
			SELECT pos_alias_name.n
					,''{'' + rrd.alias +''}''
					, source_id	
					, alias		
	...
	BEGIN
		...
		BEGIN
			EXEC spa_rfx_handle_data_source_dhx
				...
				@data_source_tsql=@data_source_tsql		
				, @data_source_alias=@data_source_alias		
				, @criteria=@criteria				
				, @data_source_process_id=@data_source_process_id
				, @handle_single_line_sql=0
				, @validate=0		
...
begin catch
	...
	select ''spa_rfx_get_view_datasource_sql_dhx'', ''' +  replace(@err_msg,'''','''''') + ''', getdate()
	'
	exec(@sql)
end catch
