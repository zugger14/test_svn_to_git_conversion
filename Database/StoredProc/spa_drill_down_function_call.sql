...
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[spa_drill_down_function_call]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[spa_drill_down_function_call]
...
CREATE PROCEDURE [dbo].[spa_drill_down_function_call] @formula varchar(8000),@formula_stmt VARCHAR(8000) OUTPUT
AS
...
select  @formula_stmt=REPLACE(REPLACE(@new_formula,'dbo.FNAMIN','Min'),'dbo.FNAMax','Max')
