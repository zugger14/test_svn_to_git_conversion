...
IF OBJECT_ID('[dbo].[spa_ems_gen_input]','p') IS NOT NULL
DROP PROCEDURE [dbo].[spa_ems_gen_input]
GO

CREATE PROCEDURE [dbo].[spa_ems_gen_input]
@flag char(1),
...
if @term_start is null
	select @term_start=min(term_start) from ems_gen_input where generator_id=@generator_id
if @term_end is null
	select @term_end=max(term_end) from ems_gen_input where generator_id=@generator_id
...
if @input_value not between @min_input_value and @max_input_value
	begin
		select 'Error' ErrorCode, 'EmissionInput' Module, 
				'spa_ems_gen_input' Area, 'DB Error' Status, 
	...
	begin
		...
		begin
			...
			set @clm1='char'+cast(@i  as varchar)
				...
				else if @static_data_type=13000
		...
		--,sdv2.code 
		from ems_gen_input esc inner join ems_source_input esi on esc.ems_input_id=esi.ems_source_input_id
...
--		
	...
	begin	
		...
		input_value Value,s1.uom_name UOM
		...
		left join static_data_value sdv2 on sdv2.value_id=esc.forecast_type
		 WHERE 1 = 1'
		 IF @ems_input_id IS NOT NULL
			SET @sql = @sql + ' AND ems_input_id = ' + cast(@ems_input_id as varchar)
		 IF @generator_id IS NOT NULL
			SET @sql = @sql + ' AND generator_id = ' + cast(@generator_id as varchar)
			
		
		IF @term_start IS NOT NULL
			SET @sql = @sql + ' AND term_start >= ''' + cast(@term_start as varchar) + ''''
		IF @term_end IS NOT NULL
			SET @sql = @sql + ' AND term_end <= ''' + cast(@term_end as varchar) + ''''
			
		SET @sql = @sql + ' or esi.constant_value=''y'''
				 
		IF @estimate_type IS NOT NULL
			SET @sql = @sql + ' AND estimate_type = ''' + cast(@estimate_type as varchar)+''''
			
		SET @sql = @sql + ' Order by term_start'
...
BEGIN
	...
	egi.uom_id,egi.generator_id,ems_input_id,input_name,egi.forecast_type,rg.technology,rg.fuel_value_id	
	from ems_gen_input  egi join ems_source_input esi 
...
BEGIN
	...
	select @i_term_start=dbo.FNADateFormat(term_start),@i_term_end=dbo.FNADateFormat(term_end),@i_name=input_name from ems_gen_input g join ems_source_input t on g.ems_input_id=t.ems_source_input_id
