...
IF OBJECT_ID(N'[dbo].[spa_rfx_save_dhx]', N'P') IS NOT NULL
    DROP PROCEDURE [dbo].[spa_rfx_save_dhx]
GO
...
-- Description: Save operations for New reporting Form. created for new report manager dhx. currently in use.
 
-- Params:
-- @process_id CHAR(1) - Operation ID

-- Sample Use :: EXEC [spa_rfx_save_dhx] '06205D01_C778_4D98_96A0_AF2FB281DFA4'
-- ===========================================================================================================
CREATE PROCEDURE [dbo].[spa_rfx_save_dhx]
	@process_id VARCHAR(50)
AS


--declare @process_id VARCHAR(50) = 'B8592D33_07FD_4BEA_934F_2E09A9E63BFE'
set nocount on
...
BEGIN TRY
	...
	*************************************************************************/
	...
	--print 'Report to delete:' + ISNULL(CAST(@old_report_id AS VARCHAR(10)), 'NULL')
	-- End
	...
	* */		
	...
	--print 'Old process table report_id: ' + ISNULL(CAST(@report_id_adiha_process AS VARCHAR(10)), 'NULL')	
	
	SET @sql = 'INSERT INTO report([name], [owner], is_system, report_hash, [description], category_id, is_mobile, is_excel, is_powerbi, is_custom_report)	
				OUTPUT ' + CAST(@report_id_adiha_process AS VARCHAR(10)) + ', INSERTED.report_id INTO #report_map
				SELECT [name], [owner], is_system, [report_hash], [description], category_id, is_mobile, is_excel, is_powerbi, is_custom_report
				FROM  ' + @rfx_report + ''
	--print @sql
	EXEC(@sql)
	...
	SET @sql = 'INSERT INTO report_paramset(
					page_id, 
					[name], 
					paramset_hash, 
					report_status_id, 
				...
				SELECT pm.page_id_main, 
					rp.[name], 
					rp.paramset_hash,
					rp.report_status_id, 
					ISNULL(cuh.create_user, dbo.FNADBUser()),
					rp.export_report_name,
					rp.export_location,
	...
	SET @sql = 'INSERT INTO report_tablix_column(tablix_id, dataset_id, column_id, 
					...
					font, font_size, font_style, text_align, text_color, default_sort_order, sorting_column, default_sort_direction, background, column_order, custom_field, render_as, column_template, negative_mark, currency, date_format, cross_summary_aggregation, mark_for_total, sql_aggregation, subtotal)
				SELECT tm.tablix_id_main, dm.dataset_id_main, rrtc.column_id, 
						...
						rrtc.font, rrtc.font_size, rrtc.font_style, rrtc.text_align, rrtc.text_color, rrtc.default_sort_order, nullif(rrtc.sorting_column,''NULL''), rrtc.default_sort_direction, rrtc.background, rrtc.column_order, rrtc.custom_field, rrtc.render_as, rrtc.column_template, rrtc.negative_mark, rrtc.currency, rrtc.date_format, rrtc.cross_summary_aggregation, rrtc.mark_for_total, rrtc.sql_aggregation, rrtc.subtotal
	...
	SET @sql = 'INSERT INTO report_chart_column(chart_id, dataset_id, column_id, placement, column_order, alias, functions, aggregation, default_sort_order, sorting_column, default_sort_direction, custom_field, render_as_line)
				SELECT cm.chart_id_main, dm.dataset_id_main, rrpc.column_id, rrpc.placement, rrpc.column_order, rrpc.alias, rrpc.functions, rrpc.aggregation, rrpc.default_sort_order, nullif(rrpc.sorting_column,''NULL''), rrpc.default_sort_direction, rrpc.custom_field, rrpc.render_as_line
	...
	**/
	...
	--print 'Save Report to DataTables from ProcessDB :: '	
	---- delete power_bi_reports
	SET @sql ='
		DELETE pbr FROM power_bi_report pbr
		left join report_paramset rp on rp.paramset_hash = pbr.source_report
		left join report_page rpg on rp.page_id = rpg.report_page_id
	...
	COMMIT TRAN
	declare @recommendation varchar(2000) = cast(@new_report_id as varchar(10)) + ',' + @process_id
	EXEC spa_ErrorHandler 0, 'Reporting FX', 'spa_rfx_save_dhx', 'Success', 'Saved Report to Data Tables from Process DB.', @recommendation	
END TRY
BEGIN CATCH
	IF @@TRANCOUNT > 0
		ROLLBACK TRAN
	...
	--EXEC spa_ErrorHandler 1, 'Reporting FX', 'spa_rfx_save_dhx', 'DB Error', 'Error on Saving Report to Data Tables from Process DB.', @new_report_id
	EXEC spa_ErrorHandler 1, 'Reporting FX', 'spa_rfx_save_dhx', 'DB Error', @sql, @new_report_id
END CATCH
