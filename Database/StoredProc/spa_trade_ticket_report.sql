...
BEGIN
	SET @sql_Select = '
		SELECT 			
			...
			dbo.FNARemoveTrailingZeroes(AVG(sdd.fixed_price)) [Fixed Price],
			...
			[dbo].[FNARemoveTrailingZeroes](AVG(sdd.deal_volume)) [Volume],
			CONVERT(varchar, CAST([dbo].[FNARemoveTrailingZeroes](SUM(sdd.total_volume)) AS MONEY), 1) [Total Volume],			
            ...
            dbo.FNARemoveTrailingZeroes(AVG(sdd.fixed_price)) * CAST([dbo].[FNARemoveTrailingZeroes](SUM(sdd.total_volume)) AS MONEY) [Notional Value]
			' 
SET @sql_Select2 = ' FROM 
		source_deal_header sdh 
		INNER JOIN source_deal_detail sdd ON sdh.source_deal_header_id=sdd.source_deal_header_id 
