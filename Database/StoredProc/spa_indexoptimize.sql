IF EXISTS (SELECT * FROM   sys.objects WHERE  OBJECT_ID = OBJECT_ID(N'[dbo].[spa_indexoptimize]')AND TYPE IN (N'P', N'PC'))
    DROP PROCEDURE [dbo].[spa_indexoptimize]
GO
SET ANSI_NULLS ON
GO
...
/**
	...
	Parameters
	@Databases:
		Select databases. The keywords SYSTEM_DATABASES, USER_DATABASES and ALL_DATABASES are supported. 
		Value					Description
	...
	@FragmentationMedium: Specify index maintenance operations to be performed on a medium-fragmented index.
		...
		INDEX_REORGANIZE,INDEX_REBUILD_ONLINE,INDEX_REBUILD_OFFLINE	Reorganize index. Rebuild index online if reorganizing is															not supported on an index. Rebuild index offline if																	reorganizing and online rebuilding are not supported on an															index.
		NULL	Do not perform index maintenance.					This is the default for a low-fragmented index.
	@FragmentationHigh: Specify index maintenance operations to be performed on a high-fragmented index. Options are same as above.
	@FragmentationLevel1: Set the lower limit, as a percentage, for medium fragmentation. The default is 5%. This is based on Microsoft's recommendation
	@FragmentationLevel2: Set the lower limit, as a percentage, for high fragmentation. The default is 30%. This is based on Microsoft's recommendation
	@MinNumberOfPages: TBD
	@MaxNumberOfPages: TBD
	@SortInTempdb:Use tempdb for sort operations when rebuilding indexes.
		Value	Description
	...
	@PadIndex:Apply the percentage of free space that is specified by the FillFactor parameter to the intermediate-level pages of the index.
		Value	Description
		Y		Apply the percentage of free space that is specified by the FillFactor parameter to the intermediate-level pages of the index.
	...
	@LOBCompaction: Compact pages that contain large object (LOB) columns, when reorganizing indexes.
		Value	Description
		Y		Compact pages that contain large object (LOB) columns, when reorganizing indexes. This is the default.
		N		Do not compact pages that contain large object (LOB) columns, when reorganizing indexes.
	...
	@OnlyModifiedStatistics:Update only statistics if any rows have been modified since the most recent statistics update.
		Value	Description
		Y		Update only statistics if any rows have been modified since the most recent statistics update.
		N		Update statistics regardless of any rows have been modfied or not.
	@StatisticsModificationLevel:TBD
	@StatisticsSample: Indicate, as a percentage, how much of a table is gathered when updating statistics. A value of 100 is equivalent to a full scan. If no value is specified, SQL Server automatically computes the required sample
	@StatisticsResample: Update statistics with the most recent sample.
		Value	Description
	...
	@PartitionLevel:Maintain partitioned indexes on the partition level. If this parameter is set to Y, the fragmentation level and page count is checked for each partition. The appropriate index maintenance (rebuild or reorganize) is then performed for each partition.
		Value	Description
	...
	@MSShippedObjects:TBD
	@Indexes: Select indexes. If this parameter is not specified, all indexes are selected. The ALL_INDEXES keyword is supported. 
	@TimeLimit: Set the time, in seconds, after which no commands are executed. Default the time is not limited.
	@Delay: Set the delay, in seconds, between index commands. Default there is no delay.
	@WaitAtLowPriorityMaxDuration:TBD
	@WaitAtLowPriorityAbortAfterWait:TBD
	@Resumable:TBD
	@AvailabilityGroups:TBD
	@LockTimeout:TBD
	@LockMessageSeverity:TBD
	@StringDelimiter:TBD
	@DatabaseOrder:TBD
	@DatabasesInParallel:TBD
...
CREATE PROCEDURE [dbo].[spa_indexoptimize]
...
BEGIN
  ...
  IF EXISTS (SELECT * FROM sys.objects objects INNER JOIN sys.schemas schemas ON objects.[schema_id] = schemas.[schema_id] WHERE objects.[type] = 'P' AND schemas.[name] = 'dbo' AND objects.[name] = 'spa_commandexecute' AND OBJECT_DEFINITION(objects.[object_id]) NOT LIKE '%@DatabaseContext%')
  BEGIN
    INSERT INTO @Errors ([Message], Severity, [State])
