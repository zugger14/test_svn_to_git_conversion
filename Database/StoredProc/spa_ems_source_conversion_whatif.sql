...
/****** Object:  StoredProcedure [dbo].[spa_ems_source_conversion_whatif]    Script Date: 06/17/2009 21:24:59 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[spa_ems_source_conversion_whatif]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[spa_ems_source_conversion_whatif]
/****** Object:  StoredProcedure [dbo].[spa_ems_source_conversion_whatif]    Script Date: 06/17/2009 21:25:04 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO

--spa_ems_source_conversion_whatif 's',4
--exec spa_ems_source_conversion_whatif 's',23,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL, NULL,NULL,NULL,1180
CREATE PROCEDURE [dbo].[spa_ems_source_conversion_whatif]
@flag char(1),
...
--	begin
		...
		Alter table #temp add Criteria varchar(100) null
		...
		Alter table #temp add [Shift Value] varchar(100) null	
		Alter table #temp add [Source Input] varchar(100) null	
		Alter table #temp add [Source Factor] varchar(100) null	
		Alter table #temp add Source varchar(100) null	
		Alter table #temp add Comments varchar(100) null	
		if @clm_name<>''
			set @clm_name=@clm_name +','
		set @sql='insert #temp
		select esc.conversion_id,criteria.code, '+ @clm_name +' cast(esc.char_factor as varchar)+'' '' + ISNULL(s1.uom_name,'''') + 
		case when (esc.ems_conversion_type_id IN (1182, 1183,1189)) then '''' else  '' of '' + 
		spcd.curve_name end +ISNULL(''/''+s2.uom_name,''''),sdv.code,dbo.fnadateformat(esc.effective_date),
		esc.value_shift [ShiftValueBy],
		esi.input_name as [SourceInput],
		cast(esc1.char_factor as varchar)+'' '' + ISNULL(s3.uom_name,'''') + 
		case when (esc1.ems_conversion_type_id IN (1182, 1183,1189)) then '''' else  '' of '' + 
		spcd1.curve_name end  +ISNULL(''/''+s4.uom_name,'''') as [SourceFactor],
		source.code,esc.conversion_comment 
		from ems_source_conversion_whatif esc '+ @join1 +'
		...
		left outer join static_data_value criteria on criteria.value_id=esc.criteria_id
		left outer join ems_source_input esi on esi.ems_source_input_id=esc.source_input_id
		left outer join ems_source_conversion esc1 on esc.source_conversion_id=esc1.conversion_id
		left outer  join source_price_curve_def spcd1 on spcd1.source_curve_def_id=esc1.emission_type 
		left outer join source_uom s3 on s3.source_uom_id=esc1.uom_id
		left outer join source_uom s4 on s4.source_uom_id=esc1.uom_id_by

		where 1=1'
		+ case when @ems_input_id is not null then ' And esc.ems_source_input_id=' +cast(@ems_input_id as varchar) else '' end
...
-- 		'spa_ems_source_conversion_whatif' Area, 'DB Error' Status, 
...
BEGIN
	...
	select conversion_id,char1,char2,char3,char4,char5,char6,char7,char8,char9,char10,char_factor,
		uom_id,	emission_type,	uom_id_by ,ems_conversion_type_id, dbo.FNADateFormat(effective_date),source,conversion_comment,criteria_id,
		value_shift,source_input_id,source_conversion_id
	from ems_source_conversion_whatif where conversion_id=@conversion_id
END

ELSE IF @flag='i'
BEGIN
	Insert into ems_source_conversion_whatif(
		ems_source_input_id,
		char1,
		char2,
		char3,
		...
		effective_date,
		source,generator_id,conversion_comment,criteria_id,value_shift,source_input_id,source_conversion_id
	)
...
BEGIN
	...
	delete from ems_source_conversion_whatif 
