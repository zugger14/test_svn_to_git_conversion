 ...
 * Time: 3/4/2017 3:08:19 AM
 ************************************************************/

IF EXISTS (
       SELECT 1
       FROM   sys.objects
       WHERE  OBJECT_ID = OBJECT_ID(N'[dbo].[spa_view_recovery_rate]')
              AND TYPE IN (N'P', N'PC')
   )
    DROP PROCEDURE [dbo].[spa_view_recovery_rate]
...
CREATE PROC [dbo].[spa_view_recovery_rate]	
	@flag				CHAR(1) = NULL,
	@xml				XML = NULL,
...
/*
--# testing
DROP TABLE #recovery_rating_list
DROP TABLE #recovery_rating_header
DROP TABLE #recovery_rating_column_header
...
DECLARE @process_table VARCHAR(500) = dbo.FNAProcessTableName('view_recovery_rate', dbo.FNADBUser(), @process_id)
...
BEGIN
    ...
    BEGIN
        ...
        CREATE TABLE #recovery_rating_list
        (
        ...
        CREATE TABLE #recovery_rating_header
        (
        	rowID              INT NOT NULL IDENTITY(1, 1),
        	debt_rating_id     VARCHAR(MAX) COLLATE DATABASE_DEFAULT ,
        	rate_column        VARCHAR(MAX) COLLATE DATABASE_DEFAULT ,
        	months_column      VARCHAR(MAX) COLLATE DATABASE_DEFAULT 
        )  
        
        CREATE TABLE #recovery_rating_column_header
        (
        	rowId              INT NOT NULL IDENTITY(1, 1),
        	debt_rating_id     INT,
		...
		BEGIN
			...
			IF NOT EXISTS (SELECT 1 
							FROM default_recovery_rate drr 
								INNER JOIN dbo.SplitCommaSeperatedValues(@debt_rating) rrl ON drr.debt_rating = rrl.Item 
							WHERE drr.effective_date >= @as_of_date_from
							AND drr.effective_date <= @as_of_date_to)
				BEGIN  -- IF DATA NOT FOUND IN SAME DATE THEN, DATA < AS_OF_DATE_FROM
					SET @as_of_date_from = (SELECT CAST(dbo.FNAGetSQLStandardDate(MAx(drr.effective_date)) AS VARCHAR(20)) 
											FROM default_recovery_rate drr 
												INNER JOIN dbo.SplitCommaSeperatedValues(@debt_rating) rrl ON drr.debt_rating = rrl.Item
		...
		BEGIN
			SET @as_of_date_from = (
									SELECT CAST(dbo.FNAGetSQLStandardDate(MAX(drr.effective_date)) AS VARCHAR(20)) 
									FROM default_recovery_rate drr 
										INNER JOIN dbo.SplitCommaSeperatedValues(@debt_rating) rrl On drr.debt_rating = rrl.Item
								   )
		END  
        ...
        BEGIN
            INSERT INTO #recovery_rating_list
              (
                debt_rating_id
              )
        ...
        SET @sql = '
					...
					SELECT sdv.value_id debt_rating_id,
						   sdv.code + ''::Rate'' + ''::'' + CAST(sdv.value_id AS VARCHAR(20)) 
						   column_header,
						   ''r'' AS [type]
					FROM   #recovery_rating_list rrl
						   INNER JOIN static_data_value sdv
        ...
        SET @sql = 
            'INSERT INTO #recovery_rating_column_header(debt_rating_id, debt_rating, value,effective_date,[type],column_header)
			SELECT 
				rrl.debt_rating_id,
				sdv.code,
				drr.rate,
				drr.effective_date,
				''r'' as [type],
				sdv.code+''::''+''Rate'' AS column_header
			FROM
				#recovery_rating_list rrl
				INNER JOIN default_recovery_rate drr On drr.debt_rating = rrl.debt_rating_id 
				INNER JOIN static_data_value sdv On sdv.value_id = drr.debt_rating
			WHERE 1=1 '
            + CASE 
                   WHEN @as_of_date_from IS NOT NULL THEN 
                        ' AND drr.effective_date >= ' + '''' + @as_of_date_from 
                        + ''''
                   ELSE ''
              END
            + CASE 
        ...
        FROM   #temp_header_list thl
               LEFT JOIN #recovery_rating_column_header rrch
...
BEGIN
    BEGIN TRY
    	...
    	BEGIN
    	    ...
    	    CREATE TABLE #temp_recovery_rate
    	    (
    	    ...
    	    SET @query_insert = 
    	        '    	    
    	    INSERT INTO #temp_recovery_rate
    	      (
    	        effective_date,
    	        debt_rating,
    	        [recovery],
    	    ...
    	    SELECT CONVERT(VARCHAR, a.effective_date, 101) [effective_date],
    	           ...
    	           a.rate [rate],
    	           CASE 
    	                WHEN drr.id IS NULL THEN ''i''
    	                ELSE ''u''
    	           END [insert_update]
    	    FROM ' + @grid_xml_table_name +
    	        ' a
    	           LEFT JOIN default_recovery_rate drr
    	                ON  a.effective_date = drr.effective_date
    	                AND a.debt_rating = drr.debt_rating'
    	    ...
    	    --SELECT * FROM #temp_recovery_rate
    	    
    	    UPDATE drr
    	    SET    drr.effective_date = trr.effective_date,
    	           drr.debt_rating = trr.debt_rating,
    	           drr.[recovery] = 'n',
    	           drr.months = NULLIF(trr.months, ''),
    	           drr.rate = NULLIF(trr.rate, '')
    	    FROM   #temp_recovery_rate trr
    	           INNER JOIN default_recovery_rate drr
    	                ON  trr.effective_date = drr.effective_date
    	                AND trr.debt_rating = drr.debt_rating
    	    WHERE  trr.insert_update = 'u'
    	    
    	    INSERT INTO default_recovery_rate
    	      (
    ...
    BEGIN CATCH
    	...
    	BEGIN
    	    ...
    	    BEGIN
    	        SET @table_name = 'default_recovery_rate'
    	        SET @err = 
...
BEGIN
	...
	BEGIN
        ...
        '
		SELECT drr.id AS [ID],
			   dbo.FNADateFormat(drr.effective_date) AS [Effective Date],
			   sdv.code AS [Debt Rating],
			   drr.[recovery] AS [Recovery],
			   drr.rate AS [Rate],
			   drr.months AS [Month]'+ @str_batch_table +'
		FROM   default_recovery_rate drr
		INNER JOIN static_data_value sdv ON drr.debt_rating = sdv.value_id'
	END
	ELSE
	BEGIN
		SET @select_sql = 
		...
		SELECT drr.id AS [ID],
			   ...
			   sdv.code AS [Debt Rating],
			   drr.[recovery] AS [Recovery],
			   drr.rate AS [Rate]'+ @str_batch_table +'
		FROM   default_recovery_rate drr
		INNER JOIN static_data_value sdv ON drr.debt_rating = sdv.value_id'
	END				
    ...
    BEGIN
        SET @select_sql += 'AND drr.effective_date BETWEEN ''' + @as_of_date_from 
            + ''' AND ''' + @as_of_date_to + ''''
    END
...
BEGIN
	...
	SELECT @str_batch_table = dbo.FNABatchProcess ('c', @batch_process_id, @batch_report_param, 
				   GETDATE(), 'spa_view_recovery_rate', 'View Recovery Rate') --TODO: modify sp and report name
