...
IF OBJECT_ID(N'[dbo].[spa_rfx_report_page_dhx]', N'P') IS NOT NULL
    DROP PROCEDURE [dbo].[spa_rfx_report_page_dhx]
GO
...
-- Description: Add/Update Operations for Report Pages
 
-- Params:
-- @flag					CHAR	- Operation flag
-- @process_id				VARCHAR - Operation ID
-- @report_page_id	INT		- Report Page ID
-- @report_id				INT		- Report ID
-- @name			VARCHAR	- Report Page Name
-- @layout INT				- Predefined layout types					
-- @report_hash				VARCHAR	- Report Unique HASH Key
-- @width					INT		- Width
-- @height					INT 	- Height	

-- Sample Use :: EXEC spa_rfx_report_page_dhx 'i', 'E35252A5_992D_44C2_B240_848D3149AA68', NULL, 1, 'First Report', 1, '234232323232323232322', 20, 30
-- Sample Use :: EXEC spa_rfx_report_page_dhx 'u', 'E35252A5_992D_44C2_B240_848D3149AA68', 1, 1, 'First Report1', 1, '1111111111111111111', 10, 20
-- Sample Use :: EXEC spa_rfx_report_page_dhx 'a', 'E35252A5_992D_44C2_B240_848D3149AA68', 1
-- Sample Use :: EXEC spa_rfx_report_page_dhx 's', 'E35252A5_992D_44C2_B240_848D3149AA68', NULL
-- ===========================================================================================================
CREATE PROCEDURE [dbo].[spa_rfx_report_page_dhx]
	@flag				CHAR(1),                             
...
BEGIN
	...
	BEGIN
		...
		BEGIN
			EXEC spa_ErrorHandler -1, 'New Reporting FX', 'spa_rfx_report_page_dhx', 'DB Error', 'Page name already used.', ''
			 RETURN
		END
	END
	...
	IF @@ERROR <> 0
	    EXEC spa_ErrorHandler @@ERROR, 'Reporting FX', 'spa_rfx_report_page_dhx', 'DB Error', 'Fail to insert data.', ''
	ELSE
	begin
		DECLARE @report_page_id_tmp INT, @report_paramset_id_tmp int
		SET @report_page_id_tmp = IDENT_CURRENT(@rfx_report_page);
		
		SET @sql = 'INSERT INTO '+@rfx_report_paramset+' ([page_id],[name],paramset_hash, report_status_id) VALUES ('+CAST(@report_page_id_tmp AS VARCHAR(10))+',''' + @name + ''', '''+ dbo.FNAGetNewID() +''', 1)'
		
		----print(@sql)
		EXEC(@sql)
	    SET @report_paramset_id_tmp = IDENT_CURRENT(@rfx_report_paramset);
		declare @return_val varchar(100) = cast(@report_page_id_tmp as varchar(5)) + ',' + cast(@report_paramset_id_tmp as varchar(10)) + ',' + @process_id + ',i'
	    EXEC spa_ErrorHandler 0, 'Reporting FX', 'spa_rfx_report_page_dhx', 'Success', 'Data successfully inserted.', @return_val
	end
END
...
BEGIN
    ...
    EXEC (@sql)
    declare @recommendation_u varchar(1000) = CAST(@report_page_id AS VARCHAR(10)) + ',,' + @process_id + ',u'
	IF @@ERROR <> 0
	    EXEC spa_ErrorHandler @@ERROR, 'Reporting FX', 'spa_rfx_report_page_dhx', 'DB Error', 'Fail to update data.', ''
	ELSE
	    EXEC spa_ErrorHandler 0, 'Reporting FX', 'spa_rfx_report_page_dhx', 'Success', 'Data successfully updated.', @recommendation_u    
END
...
BEGIN
	BEGIN TRY 
		...
		EXEC spa_ErrorHandler 0, 'Reporting FX', 'spa_rfx_report_page_dhx', 'Success', 'Data succesfully deleted.', @process_id
	...
	BEGIN CATCH
		...
		EXEC spa_ErrorHandler @edit_error_no, 'Reporting FX', 'spa_rfx_report_page_dhx', @edit_error_desc, 'Fail to delete data.', ''
	END CATCH
END
IF @flag = 'r'
BEGIN
...
IF @flag = 'c'
BEGIN	
	--Query to validate when page is saved without saving paramset with required parameters.
    ...
    IF NOT EXISTS(SELECT 1 FROM #temp_paramset)  
		EXEC spa_ErrorHandler 0, 'New Reporting FX', 'spa_rfx_report_page_dhx', 'DB Error', 'Please save Paramset to continue.', ''
	ELSE
		EXEC spa_ErrorHandler 0, 'New Reporting FX', 'spa_rfx_report_page_dhx', 'Success', 'Paramset already saved', ''
