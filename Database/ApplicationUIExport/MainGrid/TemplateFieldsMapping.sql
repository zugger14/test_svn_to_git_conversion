 BEGIN
			...
			INSERT INTO #temp_all_grids(old_grid_id, grid_name, fk_table, fk_column, load_sql, grid_label, grid_type, grouping_column, edit_permission, delete_permission, split_at)
									...
									SELECT 130,'TemplateFieldsMapping',NULL,NULL,'EXEC(''SET NOCOUNT ON SELECT deal_fields_mapping_id as [id], sdht.template_name, sc.counterparty_id FROM deal_fields_mapping dfm LEFT JOIN source_deal_header_template sdht ON sdht.template_id = dfm.template_id LEFT JOIN source_counterparty sc ON sc.source_counterparty_id = dfm.counterparty_id  ORDER BY sdht.template_name'')',NULL,'g',NULL,'10106410','10106411',NULL
			...
			FROM #temp_all_grids
							...
							INSERT INTO #temp_all_grids_columns(grid_id, column_name, column_label, field_type, sql_string, is_editable, is_required, column_order, is_hidden, fk_table, fk_column, is_unique, column_width, sorting_preference, validation_rule, column_alignment, browser_grid_id, allow_multi_select, rounding)
												...
												SELECT 130,'template_id','Template','combo','EXEC spa_getDealTemplate @flag=''s''','y','y','2','n',NULL,NULL,NULL,'180','str',NULL,'left', NULL,'n',NULL UNION ALL 
												SELECT 130,'counterparty_id','Counterparty','combo','EXEC spa_source_counterparty_maintain @flag = ''c'', @is_active = ''y'', @not_int_ext_flag = ''b''','y','n','7','n',NULL,NULL,NULL,'180','str',NULL,'left', NULL,'n',NULL UNION ALL 
												SELECT 130,'deal_type_id','Deal Type','combo','EXEC spa_source_deal_type_maintain @flag=''x''','y','n','3','n',NULL,NULL,NULL,'180','str',NULL,'left', NULL,'n',NULL UNION ALL 
												SELECT 130,'commodity_id','Commodity','combo','EXEC spa_source_commodity_maintain @flag=''a''','y','n','4','n',NULL,NULL,NULL,'180','str',NULL,'left', NULL,'n',NULL UNION ALL 
												SELECT 130,'trader_id','Trader','combo','EXEC spa_source_traders_maintain ''x''','y','n','8','n',NULL,NULL,NULL,'180','str',NULL,'left', NULL,'n',NULL UNION ALL 
												SELECT 130,'deal_fields_mapping_id','ID','ro_int',NULL,'n','n','1','y',NULL,NULL,NULL,'80','int',NULL,'left', NULL,'n',NULL
