BEGIN
			...
			INSERT INTO #temp_all_grids(old_grid_id, grid_name, fk_table, fk_column, load_sql, grid_label, grid_type, grouping_column, edit_permission, delete_permission, split_at)
									...
									SELECT 5286,'Setup_dst',NULL,NULL,'EXEC spa_dst_setup_hours ''g''','Setup DST','g',NULL,NULL,NULL,NULL
			...
			FROM #temp_all_grids
							...
							INSERT INTO #temp_all_grids_columns(grid_id, column_name, column_label, field_type, sql_string, is_editable, is_required, column_order, is_hidden, fk_table, fk_column, is_unique, column_width, sorting_preference, validation_rule, column_alignment)
												...
												SELECT 5286,'id','ID','ro',NULL,'n','n','1','y',NULL,NULL,'y','100','int',NULL,'left' UNION ALL 
												SELECT 5286,'year','year','combo','EXEC spa_dst_setup_hours @flag = ''r''','y','y','2','n',NULL,NULL,'n','200','str','NotEmpty','left' UNION ALL 
												SELECT 5286,'insert_delete','DST','combo','EXEC spa_dst_setup_hours @flag=''m''','y','y','3','n',NULL,NULL,'n','250','str','NotEmpty','left' UNION ALL 
												SELECT 5286,'effective_date','Date','dhxCalendarA',NULL,'y','n','4','n',NULL,NULL,'n','250','date','NotEmpty','left' UNION ALL 
												SELECT 5286,'hour','Hour','combo','EXEC spa_dst_setup_hours @flag=''h''','y','y','5','n',NULL,NULL,'n','150','int','NotEmpty','left' UNION ALL
												SELECT 5286,'dst_group_value_id','DST Group Value','combo','exec [spa_StaticDataValues] @flag = ''h'' , @type_id= ''102200'', @license_not_to_static_value_id=''102202''','y','y','6','n',NULL,NULL,'n','150','int','NotEmpty','left'
