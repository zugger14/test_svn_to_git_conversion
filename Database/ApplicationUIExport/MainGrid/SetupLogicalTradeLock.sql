 BEGIN
			...
			INSERT INTO #temp_all_grids(old_grid_id, grid_name, fk_table, fk_column, load_sql, grid_label, grid_type, grouping_column, edit_permission, delete_permission, split_at)
									...
									SELECT 240,'setup_logical_trade_lock',NULL,NULL,'EXEC spa_deal_lock_setup ''g''','Setup Logical Trade Lock','g',NULL,NULL,NULL,NULL
			...
			FROM #temp_all_grids
							...
							INSERT INTO #temp_all_grids_columns(grid_id, column_name, column_label, field_type, sql_string, is_editable, is_required, column_order, is_hidden, fk_table, fk_column, is_unique, column_width, sorting_preference, validation_rule, column_alignment, browser_grid_id, allow_multi_select, rounding)
												...
												SELECT 240,'role_id','Role Name','combo','SELECT  role_id AS [value], role_name AS [text]	FROM application_security_role WHERE 1=1
					AND role_id NOT IN (SELECT 
											role_id
										FROM  
											batch_process_notifications
										WHERE 1=1 
										
											AND role_id IS NOT NULL)','y','y','2','n',NULL,NULL,NULL,'305','str',NULL,'left', NULL,'n',NULL UNION ALL 
												SELECT 240,'deal_type_id','Deal Type','combo','exec spa_setup_logical_trade_lock ''s''','y','y','3','n',NULL,NULL,NULL,'303','str',NULL,'left', NULL,'n',NULL UNION ALL 
												SELECT 240,'hour','Hour','combo','Declare @Hour int
Declare @table1 table(value int,[text] int )

set @Hour=0

while(@Hour<=24)
	Begin
	Insert into @table1(value,[text])
	select @Hour,@Hour

Set @Hour=@Hour+1
end

Select * from @table1 order by value asc','y','y','4','n',NULL,NULL,NULL,'303','str',NULL,'left', NULL,'n',NULL UNION ALL 
												SELECT 240,'minute','Minute','combo','Declare @min int
Declare @table1 table(value int,[text] int )
...
Select * from @table1 order by value asc','y','y','5','n',NULL,NULL,NULL,'303','str',NULL,'left', NULL,'n',NULL UNION ALL 
												SELECT 240,'id','ID','ro_int',NULL,'n','y','1','y',NULL,NULL,'y','303','int',NULL,'left', NULL,'n',NULL
