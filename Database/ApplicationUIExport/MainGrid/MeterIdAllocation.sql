BEGIN
	...
	INSERT INTO #temp_all_grids(old_grid_id, grid_name, fk_table, fk_column, load_sql, grid_label, grid_type, grouping_column, edit_permission, delete_permission)
							...
							SELECT 47,'meter_id_allocation',NULL,NULL,'EXEC(''SELECT allocation_id [Allocation ID], meter_id [Meter ID], convert(varchar(10),production_month,120) [Production Month],gre_per [gre per],gre_volume [gre volume] FROM meter_id_allocation where meter_id = <ID>'')','Meter Allocation','g',NULL,'10103010','10103010'
	...
	FROM #temp_all_grids
					...
					INSERT INTO #temp_all_grids_columns(grid_id, column_name, column_label, field_type, sql_string, is_editable, is_required, column_order, is_hidden, fk_table, fk_column, is_unique, column_width, sorting_preference, validation_rule)
							
										SELECT 47,'production_month','Production Month','dhxCalendarA',NULL,'n','y','3',NULL,NULL,NULL,NULL,'200','str','NotEmpty' UNION ALL 
										SELECT 47,'gre_per','Third Party %','ed',NULL,'n','y','4',NULL,NULL,NULL,NULL,'200','str',NULL UNION ALL 
										SELECT 47,'gre_volume','Third Party Volume','ed',NULL,'n','y','5',NULL,NULL,NULL,NULL,'200','str',NULL UNION ALL 
										SELECT 47,'meter_id','Meter ID','ro',NULL,'n','y','2','y','meter_id','meter_id',NULL,'200','str',NULL UNION ALL 
										SELECT 47,'allocation_id','Allocation ID','ro',NULL,'n','y','1','y',NULL,NULL,'y','200','str',NULL
							
					UPDATE tagc
					SET tagc.grid_id = @grid_id
					FROM #temp_all_grids_columns tagc
						
					INSERT INTO adiha_grid_columns_definition(grid_id, column_name, column_label, field_type, sql_string, is_editable, is_required, column_order, is_hidden, fk_table, fk_column, is_unique, column_width, sorting_preference, validation_rule)
					SELECT	tagc.grid_id,
							tagc.column_name,
							tagc.column_label,
							tagc.field_type,
