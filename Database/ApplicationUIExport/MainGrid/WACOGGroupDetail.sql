BEGIN TRY
	...
	SELECT 427,'wacog_group_detail',NULL,NULL,'EXEC [dbo].[spa_wacog_group] @flag=''x'', @wacog_group_id =<ID>','WACOG Detail','g',NULL,20007401,20007402,NULL
 
	 UPDATE tag
	 SET tag.new_grid_id = agd.grid_id
	 FROM #temp_all_grids tag
	 INNER JOIN adiha_grid_definition AS agd ON agd.grid_name = tag.grid_name
	 ...
	 BEGIN
		...
		INNER JOIN #temp_all_grids AS tag ON tag.new_grid_id = agd.grid_id
	END
	...
	SELECT 427,'wacog_group_detail_id','ID','ro',NULL,'y','y','1','y',NULL,NULL,'y','175',NULL,NULL,'left', NULL,'n' UNION ALL 
	SELECT 427,'wacog_group_id','WACOG Group','ro',NULL,'n','n','2','y','wacog_group','wacog_group_id',NULL,'175',NULL,NULL,'left', NULL,'n' UNION ALL 
	SELECT 427,'template_id','Template','combo','EXEC spa_getDealTemplate @flag=s','y','y','3','n',NULL,NULL,NULL,'175',NULL,NULL,'left', NULL,'n' UNION ALL 
	SELECT 427,'source_deal_type_id','Deal Type','combo','EXEC spa_source_deal_type_maintain @flag=x','y','n','4','n',NULL,NULL,NULL,'175',NULL,NULL,'left', NULL,'n' UNION ALL 
	SELECT 427,'charge_type_id','Charge Type','combo','SELECT udf_template_id val, field_label lab FROM user_defined_fields_template WHERE NULLIF(internal_field_type, '''') IS NOT NULL
														UNION SELECT value_id val, code lab FROM static_data_value WHERE value_id = -10019','y','n','5','n',NULL,NULL,NULL,'175',NULL,NULL,'left', NULL,'y' UNION ALL 
	SELECT 427,'leg','Leg','ed',NULL,'y','y','7','n',NULL,NULL,NULL,'175',NULL,NULL,'left', NULL,'n'
	...
	SELECT tagc.grid_id, tagc.column_name, tagc.column_label, tagc.field_type, tagc.sql_string, tagc.is_editable, tagc.is_required, tagc.column_order, tagc.is_hidden, tagc.fk_table,
		   tagc.fk_column, tagc.is_unique, tagc.column_width, tagc.sorting_preference, tagc.validation_rule, tagc.column_alignment, tagc.browser_grid_id, tagc.allow_multi_select
	FROM #temp_all_grids_columns tagc
