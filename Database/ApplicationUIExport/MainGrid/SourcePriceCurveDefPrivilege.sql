 BEGIN
			...
			INSERT INTO #temp_all_grids(old_grid_id, grid_name, fk_table, fk_column, load_sql, grid_label, grid_type, grouping_column, edit_permission, delete_permission, split_at)
									...
									SELECT 77,'source_price_curve_def_privilege',NULL,NULL,'EXEC spa_source_price_curve_def_privilege @flag = ''g'', @source_curve_def_id = <ID>','Privilege','g',NULL,'10102610','10102610',NULL
			...
			FROM #temp_all_grids
							...
							INSERT INTO #temp_all_grids_columns(grid_id, column_name, column_label, field_type, sql_string, is_editable, is_required, column_order, is_hidden, fk_table, fk_column, is_unique, column_width, sorting_preference, validation_rule, column_alignment, browser_grid_id, allow_multi_select, rounding)
												...
												SELECT 77,'sub_entity_id','Subsidiary','combo','SELECT -1,''All'' UNION ALL SELECT entity_id, entity_name FROM portfolio_hierarchy WHERE entity_type_value_id = 525','y','y','2','n',NULL,NULL,'n','200','str','NotEmpty','left', NULL,'n',NULL UNION ALL 
												SELECT 77,'role_id','Visibility','combo','SELECT NULL [role_id], ''ALL'' [role_name] UNION SELECT role_id, ''Role - '' + role_name [role_name] from application_security_role ORDER BY [role_name]','y','y','3',NULL,NULL,NULL,'n','200','str','NotEmpty','left', NULL,'n',NULL UNION ALL 
												SELECT 77,'id','P_ID','ro_int',NULL,'n','y','1','y',NULL,NULL,'y','100','int',NULL,'left', NULL,'n',NULL UNION ALL 
												SELECT 77,'source_curve_def_id','ID','ro_int',NULL,'n','y','2','y','source_price_curve_def','source_curve_def_id','n','100','int',NULL,'left', NULL,'n',NULL
