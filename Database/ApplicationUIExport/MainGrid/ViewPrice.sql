 BEGIN
			...
			INSERT INTO #temp_all_grids(old_grid_id, grid_name, fk_table, fk_column, load_sql, grid_label, grid_type, grouping_column, edit_permission, delete_permission, split_at)
									...
									SELECT 96,'view_price','','','SELECT	sdv.code AS [curve_type],spcd.curve_name AS [curve_name],sdv1.code AS [granularity],spcd.source_curve_def_id AS [id],su.uom_id AS [uom_id] FROM source_price_curve_def spcd INNER JOIN static_data_value sdv ON spcd.source_curve_type_value_id = sdv.value_id LEFT JOIN static_data_value sdv1 ON spcd.Granularity = sdv1.value_id INNER JOIN source_uom su ON spcd.uom_id = su.source_uom_id',NULL,'t','curve_type,curve_name',NULL,NULL,NULL
			...
			FROM #temp_all_grids
							...
							INSERT INTO #temp_all_grids_columns(grid_id, column_name, column_label, field_type, sql_string, is_editable, is_required, column_order, is_hidden, fk_table, fk_column, is_unique, column_width, sorting_preference, validation_rule, column_alignment, browser_grid_id, allow_multi_select, rounding)
												...
												SELECT 96,'granularity','Granularity','ro',NULL,'y','y','2','n',NULL,NULL,NULL,'150',NULL,NULL,'left', NULL,'n',NULL UNION ALL 
												SELECT 96,'uom','UOM','ro',NULL,'y','y','4','n',NULL,NULL,NULL,'150',NULL,NULL,'right', NULL,'n',NULL UNION ALL 
												SELECT 96,'id','Source Curve Def ID','ro_int',NULL,'y','y','3','y',NULL,NULL,NULL,'150','int',NULL,'left', NULL,'n',NULL UNION ALL 
												SELECT 96,'curve_type','Curve Type/ Name','tree',NULL,'n','y','1','n',NULL,NULL,NULL,'150',NULL,NULL,'left', NULL,'n',NULL
