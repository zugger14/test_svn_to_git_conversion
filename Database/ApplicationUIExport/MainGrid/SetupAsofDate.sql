 BEGIN
			...
			INSERT INTO #temp_all_grids(old_grid_id, grid_name, fk_table, fk_column, load_sql, grid_label, grid_type, grouping_column, edit_permission, delete_permission, split_at)
									...
									SELECT 281,'SetupAsOfDate',NULL,NULL,'EXEC [spa_as_of_date] ''s''','As of Date','g',NULL,NULL,NULL,NULL
			...
			FROM #temp_all_grids
							...
							INSERT INTO #temp_all_grids_columns(grid_id, column_name, column_label, field_type, sql_string, is_editable, is_required, column_order, is_hidden, fk_table, fk_column, is_unique, column_width, sorting_preference, validation_rule, column_alignment, browser_grid_id, allow_multi_select, rounding)
												...
												SELECT 281,'as_of_date','As of date','combo','SELECT 1 AS [id],''Custom As of Date'' AS [value] UNION SELECT 2 AS [id],''First Day of the Month'' AS [value] UNION SELECT 3 AS [id],''Last Day of the Month'' AS [value] UNION SELECT 4 AS [id],''Day Before Run Date'' AS [value] UNION SELECT 5 AS [id],''Custom Days Before Run Date'' AS [value] UNION SELECT 6 AS [id],''Last Business Day of the Month'' AS [value] UNION SELECT 7 AS [id],''First Business Day of the Month'' AS [value] UNION SELECT 8 AS [id],''Last Business Day of Previous Month'' AS [value]','n','y','4','n',NULL,NULL,'n','150','date',NULL,'left', NULL,'n',NULL UNION ALL 
												SELECT 281,'module','Module','ro',NULL,'n','y','3','n',NULL,NULL,'n','80','str',NULL,'left', NULL,'n',NULL UNION ALL 
												SELECT 281,'screens','Menu','ro',NULL,'n','y','2','n',NULL,NULL,'n','150','str',NULL,'left', NULL,'n',NULL UNION ALL 
												SELECT 281,'as_of_date_id','ID','ro_int',NULL,'n','y','1','y',NULL,NULL,'n','150','int',NULL,'left', NULL,'n',NULL
