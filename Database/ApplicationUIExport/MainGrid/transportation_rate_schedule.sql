 BEGIN
			...
			FROM #temp_all_grids
							...
							INSERT INTO #temp_all_grids_columns(grid_id, column_name, column_label, field_type, sql_string, is_editable, is_required, column_order, is_hidden, fk_table, fk_column, is_unique, column_width, sorting_preference, validation_rule, column_alignment)
												...
												SELECT 76,'rate_type_id','Charge Type','combo','select value_id,code from static_data_value sdv INNER JOIN static_data_category sdc ON sdv.category_id = sdc.category_id where sdv.type_id=5500 AND sdc.category_name = ''Rate Schedule Code''','y','y','3','n',NULL,NULL,NULL,'200','str','NotEmpty','left' UNION ALL 
												...
												SELECT 76,'uom_id','UOM','combo','SELECT source_uom_id, uom_id FROM source_uom','y','y','6','n',NULL,NULL,NULL,'205','str','NotEmpty','right'
