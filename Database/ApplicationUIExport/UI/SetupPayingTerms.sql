...
BEGIN
	BEGIN TRY
		...
		WHERE aut.application_function_id = '20017000' AND auf.application_function_id IS NULL
		UNION ALL
		SELECT 
		...
		WHERE auf.application_function_id = '20017000'  AND auf.application_function_id IS NOT NULL
		...
		FROM 
			...
			WHERE aut.application_function_id = '20017000' AND auf.application_function_id IS NOT NULL
		...
		*/		
		...
		IF EXISTS(SELECT 1 FROM application_ui_template aut WHERE aut.application_function_id = '20017000')
		BEGIN				
			...
			WHERE autd.application_function_id = '20017000'
				
			-- DELETE SCRIPT STARTS HERE
				
			EXEC spa_application_ui_template 'd', 20017000
		...
		SELECT 476,'payment_details',NULL,NULL,'EXEC spa_paying_terms @flag= ''s'', @payment_terms_id = <ID>','Payment Details','g',NULL,'20017001','20017002',NULL
		...
		IF NOT EXISTS(SELECT 1 FROM application_ui_template_definition autd WHERE autd.application_function_id = '20017000') 
		BEGIN 
			...
			VALUES('20017000','','','','settings','',' ',' ','','0','n','n','','n','n','n','n','y','n','n','n',NULL,NULL,NULL,NULL)
						
			INSERT INTO application_ui_template_definition (application_function_id, field_id, farrms_field_id, default_label, field_type, data_type, header_detail, system_required, sql_string, field_size, is_disable, is_hidden, default_value, insert_required, data_flag, update_required, has_round_option, blank_option, is_primary, is_udf, is_identity, text_row_num, hyperlink_function, char_length, open_ui_function_id) 
			OUTPUT INSERTED.application_ui_field_id, INSERTED.field_id, INSERTED.field_type
			INTO #temp_new_template_definition (new_definition_id, field_id, field_type)
			VALUES('20017000','payment_terms_id','payment_terms_id',' ID','input','int','h','n',NULL,NULL,'y','n',NULL,'n','n','n','n','n','y','n','y',NULL,NULL,NULL,NULL)
		...
		EXEC spa_application_ui_template_audit @flag='d', @application_function_id='20017000'
	COMMIT 
	END TRY
	BEGIN CATCH
		IF @@TRANCOUNT > 0
	...
	SELECT @memcache_key = CASE WHEN aut.is_report = 'y' 
							THEN @db + '_RptList' + ',' + @db + '_RptStd_' + '20017000'  
							ELSE @db + '_UI_' + '20017000'
						END 
	FROM application_ui_template AS aut
	WHERE aut.application_function_id = 20017000
