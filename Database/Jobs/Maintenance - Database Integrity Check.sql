/* This script is for daily DBCC Integrity check */
...
SET @job_name = @db_name + N' - ' + @job_category + N' - Database Integrity Check'
...
EXECUTE dbo.spa_DatabaseIntegrityCheck
@Databases = @db_name,
@log_to_table = ''Y'''
...
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=@job_name, 
		...
		@notify_level_page=0, 
		@delete_level=0, 
		@description=N'Database Integrity Check', 
		@category_name=@job_category, 
		@owner_login_name=@owner_name, @job_id = @jobId OUTPUT
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback

EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'Run Database Integrity Check', 
		@step_id=1, 
...
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
EXEC @ReturnCode = msdb.dbo.sp_add_jobschedule @job_id=@jobId, @name=N'Run every week', 
		@enabled=1, 
		@freq_type=8, 
		@freq_interval=1, 
		@freq_subday_type=1, 
