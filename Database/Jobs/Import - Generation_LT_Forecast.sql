...
DECLARE @job_name NVARCHAR(500) = @job_db_name + N' - ' + @job_category + N' - Generation_LT_Forecast'

-- batch notification
DECLARE @role_id INT , @csv_file_path VARCHAR(5000)
IF NOT EXISTS(SELECT * FROM application_security_role WHERE role_name = 'Generation Import Notification LT')
BEGIN
	INSERT INTO application_security_role(role_name, role_description, role_type_value_id)
	SELECT 'Generation Import Notification LT', 'Generation Import Notification LT', 2
END

SELECT @role_id = role_id FROM application_security_role WHERE ROLE_NAME = 'Generation Import Notification LT'
SELECT @csv_file_path = document_path+'\temp_note' from connection_string

IF NOT EXISTS(SELECT * FROM batch_process_notifications WHERE process_id = '@ef42e2330j02')
BEGIN
...
DECLARE @command1 NVARCHAR(4000) = '
		...
		WHERE ir.ixp_rule_hash = ''C0B86957_4E39_41C5_9E4C_3A0D62B6C289''
		...
		BEGIN
			...
			EXEC spa_run_sp_with_dynamic_params @parameter, ''@ef42e2330j02'', NULL
		END
		'
