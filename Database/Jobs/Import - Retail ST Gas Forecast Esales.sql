...
DECLARE @job_name NVARCHAR(500) = @job_db_name + N' - ' + @job_category + N' - Retail ST Gas Forecast Esales'

-- batch notification
DECLARE @role_id INT , @csv_file_path VARCHAR(5000)
IF NOT EXISTS(SELECT * FROM application_security_role WHERE role_name = 'Retail Import Notification ST E-Sales')
BEGIN
	INSERT INTO application_security_role(role_name, role_description, role_type_value_id)
	SELECT 'Retail Import Notification ST E-Sales', 'Retail Import Notification ST E-Sales', 2
END

SELECT @role_id = role_id FROM application_security_role WHERE ROLE_NAME = 'Retail Import Notification ST E-Sales'
SELECT @csv_file_path = document_path+'\temp_note' from connection_string

IF NOT EXISTS(SELECT * FROM batch_process_notifications WHERE process_id = '@ef42e2330j06')
BEGIN
...
DECLARE @command1 NVARCHAR(4000) = '
		...
		WHERE ir.ixp_rule_hash = ''E938426C_0EB0_4979_92AB_694FC387BC7B''
		...
		BEGIN
			...
			EXEC spa_run_sp_with_dynamic_params @parameter, ''@ef42e2330j06'', NULL
		END
		'
