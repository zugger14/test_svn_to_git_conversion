...
SET @job_name = @db_name + N' - Calculate - Position Generation_Sell_Back_Tennet'
...
BEGIN
	SELECT @alert_sql_id = alert_sql_id FROM alert_sql WHERE alert_sql_name = ''Sell Back to Generation''

	SELECT @event_trigger_id = e.event_trigger_id
	FROM event_trigger e
	INNER JOIN module_events m ON m.module_events_id = e.modules_event_id
	WHERE e.alert_id = @alert_sql_id
		AND m.workflow_name = ''Sell Back to Generation''

	SELECT @workflow_group_id = id
	FROM workflow_schedule_task
	WHERE [text] = ''Sell Back to Generation''
