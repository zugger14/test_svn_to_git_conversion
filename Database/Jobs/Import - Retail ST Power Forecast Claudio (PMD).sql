...
DECLARE @job_name NVARCHAR(500) = @job_db_name + N' - ' + @job_category + N' - Retail ST Power Forecast Claudio (PMD)'

-- batch notification
DECLARE @role_id INT , @csv_file_path VARCHAR(5000)
IF NOT EXISTS(SELECT * FROM application_security_role WHERE role_name = 'Retail Import Notification ST Claudio')
BEGIN
	INSERT INTO application_security_role(role_name, role_description, role_type_value_id)
	SELECT 'Retail Import Notification ST Claudio', 'Retail Import Notification ST Claudio', 2
END

SELECT @role_id = role_id FROM application_security_role WHERE ROLE_NAME = 'Retail Import Notification ST Claudio'
SELECT @csv_file_path = document_path+'\temp_note' from connection_string

IF NOT EXISTS(SELECT * FROM batch_process_notifications WHERE process_id = '@ef42e2330j04')
BEGIN
...
DECLARE @command1 NVARCHAR(4000) = '
		...
		WHERE ir.ixp_rule_hash = ''A668E7E6_0124_40D6_A861_9C8078B92B17''
		...
		BEGIN
			...
			EXEC spa_run_sp_with_dynamic_params @parameter, ''@ef42e2330j04'', NULL
		END
		'
