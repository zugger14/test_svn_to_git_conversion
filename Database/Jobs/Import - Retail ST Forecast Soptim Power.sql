...
DECLARE @job_name NVARCHAR(500) = @job_db_name + N' - ' + @job_category + N' - Retail ST Forecast Soptim Power'

-- batch notification
DECLARE @role_id INT , @csv_file_path VARCHAR(5000)
IF NOT EXISTS(SELECT * FROM application_security_role WHERE role_name = 'Retail Import Notification ST Soptim')
BEGIN
	INSERT INTO application_security_role(role_name, role_description, role_type_value_id)
	SELECT 'Retail Import Notification ST Soptim', 'Retail Import Notification ST Soptim', 2
END

SELECT @role_id = role_id FROM application_security_role WHERE ROLE_NAME = 'Retail Import Notification ST Soptim'
SELECT @csv_file_path = document_path+'\temp_note' from connection_string

IF NOT EXISTS(SELECT * FROM batch_process_notifications WHERE process_id = '@ef42e2330j08')
BEGIN
...
DECLARE @command1 NVARCHAR(4000) = '
		...
		WHERE ir.ixp_rule_hash = ''2EA2B3B5_265B_4BE2_B8EF_9705E4B8F679''
		...
		BEGIN
			...
			EXEC spa_run_sp_with_dynamic_params @parameter, ''@ef42e2330j08'', NULL
		END
		'
...
EXEC @ReturnCode = msdb.dbo.sp_add_jobschedule @job_id=@jobId, @name=N'Daily - 14:00-15:00', 
		@enabled=1, 
		@freq_type=4, 
		@freq_interval=1, 
		@freq_subday_type=4, 
