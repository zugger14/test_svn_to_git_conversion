...
DECLARE @job_name NVARCHAR(500) = @job_db_name + N' - ' + @job_category + N' - Complex Gas Hour Shaped Data'
...
IF NOT EXISTS(SELECT * FROM batch_process_notifications WHERE process_id = 'zef42e2330j17' AND role_id = @role_id)
BEGIN
...
BEGIN
	INSERT INTO application_security_role(role_name, role_description, role_type_value_id)
	SELECT 'Enercity Traders', 'Enercity Traders', 4
END
...
DECLARE @command1 NVARCHAR(4000) = '
		...
		WHERE ir.ixp_rule_hash = ''F224E702_7357_4EC5_BDE4_544D80D32E9D''
		...
		BEGIN
			...
			EXEC spa_run_sp_with_dynamic_params @parameter, ''zef42e2330j17'', NULL
		END
		'
