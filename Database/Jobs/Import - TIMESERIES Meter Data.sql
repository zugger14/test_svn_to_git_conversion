...
DECLARE @job_name NVARCHAR(500) = @job_db_name + N' - ' + @job_category + N' - TIMESERIES Meter Data'
...
IF NOT EXISTS(SELECT * FROM batch_process_notifications WHERE process_id = 'zef42e2330j14')
BEGIN
...
BEGIN
	...
	WHERE process_id = 'zef42e2330j14'
END
...
DECLARE @command1 NVARCHAR(4000) = '
		...
		BEGIN
			...
			EXEC spa_run_sp_with_dynamic_params @parameter, ''zef42e2330j14'', NULL
		END
		'
...
EXEC @ReturnCode = msdb.dbo.sp_add_jobschedule @job_id=@jobId, @name=N'Tuesdays- 11-13', 
		@enabled=1, 
		@freq_type=8, 
		@freq_interval=4, 
		@freq_subday_type=4, 
