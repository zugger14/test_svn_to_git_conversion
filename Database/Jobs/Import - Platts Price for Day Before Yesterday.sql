...
SET @job_name = @db_name + N' - ' + @job_category + N' - Platts Price for Day Before Yesterday'
...
SET @command =  N'/FILE "' + @ssis_path + '" /CHECKPOINTING OFF /SET "\Package.Connections[OLE_CONN_MainDB].Properties[UserName]";"'+ @owner_name +'" /SET "\Package.Variables[User::ps_setupFileName].Properties[Value]";platts /SET "\Package.Variables[User::PS_dateOption].Properties[Value]";2 /REPORTING E'
...
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=@job_name, 
		...
		@notify_level_page=0, 
		@delete_level=0, 
		@description=N'Import Platts Price for Day Before Yesterday', 
		@category_name=@job_category, 
		@owner_login_name=@owner_name, @job_id = @jobId OUTPUT
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback

EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'Run Import Platts Price for Day Before Yesterday', 
		@step_id=1, 
