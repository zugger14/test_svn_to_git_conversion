...
DECLARE @step1_cmd NVARCHAR(2000)

SET @db_name = DB_NAME()
SET @owner_name = dbo.FNAAppAdminID()
SET @job_category = N'Import'
SET @job_name = @db_name + N' - ' + @job_category + N' - Data Enhancement for Accrual'
...
SET @step1_cmd = N'
	...
	GO
EXEC [dbo].[spa_import_data_enhancement_for_accrual] '''''
...
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=@job_name, 
		...
		@notify_level_page=0, 
		@delete_level=0, 
		@description=N'Starts Import process to fill missing allocation actual data.', 
		@category_name=@job_category, 
		@owner_login_name=@owner_name, @job_id = @jobId OUTPUT
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback

EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'Run Fill Missing Allocation Actual Data', 
		@step_id=1, 
...
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
EXEC @ReturnCode = msdb.dbo.sp_add_jobschedule @job_id=@jobId, @name=N'FirstDayOfMonth', 
		@enabled=1, 
		@freq_type=16, 
		@freq_interval=1, 
		@freq_subday_type=1, 
