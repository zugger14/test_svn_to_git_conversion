...
DECLARE @job_name NVARCHAR(500) = @job_db_name + N' - ' + @job_category + N' - VKW_Forecast Volume'

-- batch notification
DECLARE @role_id INT , @csv_file_path VARCHAR(5000)
IF NOT EXISTS(SELECT * FROM application_security_role WHERE role_name = 'Renewable Import Notification Role')
BEGIN
	INSERT INTO application_security_role(role_name, role_description, role_type_value_id)
	SELECT 'Renewable Import Notification Role', 'Renewable Import Notification Role', 2
END

SELECT @role_id = role_id FROM application_security_role WHERE ROLE_NAME = 'Renewable Import Notification Role'
SELECT @csv_file_path = document_path+'\temp_note' from connection_string

IF NOT EXISTS(SELECT * FROM batch_process_notifications WHERE process_id = '@ef42e2330j01')
BEGIN
...
DECLARE @command1 NVARCHAR(4000) = '
		...
		WHERE ir.ixp_rule_hash = ''F7245D78_7A42_45A4_99AE_0B3A513D2CEF''
		...
		BEGIN
			...
			EXEC spa_run_sp_with_dynamic_params @parameter, ''@ef42e2330j01'', NULL
		END
		'
