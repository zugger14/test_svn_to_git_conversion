...
SET @job_category = N'Remit Feedback Capture'
SET @job_name = @db_name + N' - ' + @job_category + N' - Nordpool'
SET @name = N'Remit Feedback Capture - Nordpool'
SET @command = N' EXEC spa_nordpool_exporter @flag = ''t'''
BEGIN TRANSACTION
...
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'Step 1', 
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [Job fail]    Script Date: 12/30/2019 3:45:14 PM ******/
...
EXEC spa_NotificationUserByRole 22, @process_id, ''BatchReport'', ''Scheduler failed to run for process Remit Feedback Capture. Please contact technical support'', ''e'', ''Remit Feedback Capture'', 0', 
		@database_name=@db_name, 
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
EXEC @ReturnCode = msdb.dbo.sp_update_job @job_id = @jobId, @start_step_id = 1
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
EXEC @ReturnCode = msdb.dbo.sp_add_jobschedule @job_id=@jobId, @name=N'Remit Feedback Capture', 
		@enabled=1, 
		@freq_type=4, 
		@freq_interval=1, 
		@freq_subday_type=1, 
