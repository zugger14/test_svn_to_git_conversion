...
DECLARE @job_name NVARCHAR(500) = @job_db_name + N' - ' + @job_category + N' - External Forecasts'

-- batch notification
DECLARE @role_id INT , @csv_file_path VARCHAR(5000)
IF NOT EXISTS(SELECT * FROM application_security_role WHERE role_name = 'OP_and_Trader_Import_Notification_Role ST')
BEGIN
	INSERT INTO application_security_role(role_name, role_description, role_type_value_id)
	SELECT 'OP_and_Trader_Import_Notification_Role ST', 'OP_and_Trader_Import_Notification_Role ST', 2
END

SELECT @role_id = role_id FROM application_security_role WHERE ROLE_NAME = 'OP_and_Trader_Import_Notification_Role ST'
SELECT @csv_file_path = document_path+'\temp_note' from connection_string

IF NOT EXISTS(SELECT * FROM batch_process_notifications WHERE process_id = '@ef42e2330j10')
BEGIN
...
DECLARE @command1 NVARCHAR(4000) = '
		...
		WHERE ir.ixp_rule_hash = ''DCE7A7FB_D633_4D79_9158_2571681F0625''
		...
		BEGIN
			...
			EXEC spa_run_sp_with_dynamic_params @parameter, ''@ef42e2330j10'', NULL
		END
		'
