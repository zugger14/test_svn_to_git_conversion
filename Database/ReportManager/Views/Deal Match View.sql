BEGIN TRY
	...
	declare @new_ds_alias varchar(10) = 'DMV'
	/** IF DATA SOURCE ALIAS ALREADY EXISTS ON DESTINATION, RAISE ERROR **/
	if exists(select top 1 1 from data_source where alias = 'DMV' and name <> 'Deal Match View')
	begin
		select top 1 @new_ds_alias = 'DMV' + cast(s.n as varchar(5))
		from seq s
		left join data_source ds on ds.alias = 'DMV' + cast(s.n as varchar(5))
	...
	IF NOT EXISTS (SELECT 1 
	           FROM data_source 
	           WHERE [name] = 'Deal Match View'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
	AND NOT EXISTS (SELECT 1 FROM map_function_category WHERE [function_name] = 'Deal Match View' AND '106500' = '106501') 
	BEGIN
		INSERT INTO data_source([type_id], [name], [alias], [description], [tsql], report_id, system_defined,category)
		SELECT TOP 1 1 AS [type_id], 'Deal Match View' AS [name], @new_ds_alias AS ALIAS, 'Deal Match View' AS [description],null AS [tsql], @report_id_data_source_dest AS report_id,'1' AS [system_defined]
			,'106500' AS [category]
	END

	UPDATE data_source
	SET alias = @new_ds_alias, description = 'Deal Match View'
	, [tsql] = CAST('' AS VARCHAR(MAX)) + 'DECLARE @_link_id_from VARCHAR(20)
	,@_link_id_to VARCHAR(20)
	,@_effective_date_from VARCHAR(20)
	,@_effective_date_to VARCHAR(20)
	,@_deal_date_from VARCHAR(20)
	,@_deal_date_to VARCHAR(20)
	,@_source_deal_header_id VARCHAR(MAX)
	,@_sql_string VARCHAR(MAX)
	,@_link_ids VARCHAR(MAX)
	,@_assignment_type VARCHAR(MAX)

--SET @_effective_date_from = ''2020-03-17''
--SET @_effective_date_to = ''2020-03-17''

--SET @_source_deal_header_id = 253156

IF ''@link_id_from'' <> ''NULL''
	SET @_link_id_from = ''@link_id_from''
IF ''@link_id_to'' <> ''NULL''
	SET @_link_id_to = ''@link_id_to''
IF ''@effective_date_from'' <> ''NULL''
	SET @_effective_date_from = ''@effective_date_from''
IF ''@effective_date_to'' <> ''NULL''
	SET @_effective_date_to = ''@effective_date_to''
...
IF ''@assignment_type'' <> ''NULL''
    SET @_assignment_type = ''@assignment_type''
...
SELECT @_link_ids = COALESCE(@_link_ids + '','', '''') + CAST(mh.link_id AS VARCHAR(10))
FROM matching_header mh
INNER JOIN matching_detail md ON mh.link_id = md.link_id
WHERE md.source_deal_header_id = @_source_deal_header_id
GROUP BY mh.link_id

SET @_link_ids = CASE WHEN @_source_deal_header_id IS NOT NULL AND @_link_ids IS NULL THEN ''-1'' ELSE @_link_ids END

SET @_sql_string = ''
SELECT mh.link_id link_id_from
	,ISNULL(mh.link_description, mh.link_id) link_description
	,mh.link_effective_date effective_date_from
	,ssbm.[sub_name] [Subsidiary]
	,ssbm.[stra_name] [Strategy]
	,ssbm.[book_name] [Book]
	,ssbm.logical_name [Sub_Book]
	,ssbm.[sub_id] sub_id
	,ssbm.[stra_id] stra_id
	,ssbm.[book_id] book_id
	,ssbm.[sub_book_id]
	...
	,sco.commodity_name [Commodity]
	,CASE 
		WHEN sdd.buy_sell_flag = ''''b''''
			THEN ''''Buy''''
		ELSE ''''Sell''''
		END [Buy_Sell]
	,sdht.template_name [Template]
	,CASE 
		WHEN sdh.physical_financial_flag = ''''p''''
			THEN ''''Physical''''
		ELSE ''''Financial''''
		END [Physical_Financial]
	,sdv_deal_status.code [Deal_Status]
	,sdv_confirm_status.code [Confirm_Status]
	...
	,sdh.description4 [Description 4]
	,spcd.curve_name [Index]
	,CAST(sdd.total_volume as NUMERIC(10,4))[Total_Volume]
	,md.assigned_vol [Matched_Volume]
	,(sdd.deal_volume - ISNULL(assignedVol.vol, assignedVol1.vol)) [Remaining_Volume]
	,su.uom_name [UOM]
	,CASE 
		WHEN sdd.deal_volume_frequency = ''''t''''
			THEN ''''Term''''
		WHEN sdd.deal_volume_frequency = ''''d''''
			THEN ''''Daily''''
		WHEN sdd.deal_volume_frequency = ''''m''''
			THEN ''''Monthly''''
		END [Volume_Frequency]
	,su_position.uom_name [Position_UOM]
	,dbo.FNARemoveTrailingZero(ROUND(sdd.fixed_price, 4)) [Fixed_Price]
	,scu.currency_name [Currency]
	,mh.create_ts [Create_TS]
	,mh.create_user [Create_User]
	,mh.update_ts [Update_TS]
	,mh.update_user [Update_User]
	,'' + CASE WHEN @_link_id_to IS NOT NULL THEN @_link_id_to ELSE ''NULL'' END + '' [link_id_to]
	,'' + CASE WHEN @_effective_date_to IS NOT NULL THEN @_effective_date_to ELSE ''NULL'' END + '' [effective_date_to]
	,st.source_trader_id trader_id
	,sdh.counterparty_id
	,sdh.commodity_id
	,sdd.curve_id
	,mh.match_status match_status_id
	,sdv_match_status.code match_status
	,sdh.deal_date deal_date_from
	,rg.code generator
	,ISNULL(jur.code, jur1.code) AS jurisdiction
	,ISNULL(tier.code, tier1.code) AS tier
	, CONCAT(SUBSTRING(gc.gis_certificate_number_from, 1, LEN(gc.gis_certificate_number_from)-CHARINDEX(''''-'''',REVERSE(gc.gis_certificate_number_from))),''''-'''',mhdi.sequence_from) AS certificate_from
	, CONCAT(SUBSTRING(gc.gis_certificate_number_to, 1, LEN(gc.gis_certificate_number_to)-CHARINDEX(''''-'''',REVERSE(gc.gis_certificate_number_to))),''''-'''',mhdi.sequence_to) AS certificate_to
	,mhdi.sequence_from sequence_from
	,mhdi.sequence_to sequence_to
	,CAST('' + CASE WHEN @_deal_date_to IS NOT NULL THEN @_deal_date_to ELSE ''NULL'' END + '' AS DATETIME) [deal_date_to]
	,sdd.deal_volume
...
CROSS APPLY (
	SELECT md1.link_id
		,md1.source_deal_header_id
		,md1.source_deal_detail_id
		,SUM(md1.assigned_vol) assigned_vol
	FROM matching_header_detail_info md1
	WHERE md1.link_id = mh.link_id
	GROUP BY md1.link_id
		,md1.source_deal_header_id
		,md1.source_deal_detail_id
	...
	SELECT md1.link_id
		,md1.source_deal_header_id_from source_deal_header_id
		,md1.source_deal_detail_id_from source_deal_detail_id
		,SUM(md1.assigned_vol) assigned_vol
	FROM matching_header_detail_info md1
	WHERE md1.link_id = mh.link_id
	GROUP BY md1.link_id
		,md1.source_deal_header_id_from
		,md1.source_deal_detail_id_from
	) md
...
INNER JOIN source_deal_detail sdd ON sdd.source_deal_header_id = md.source_deal_header_id
	AND sdd.source_deal_detail_id = md.source_deal_detail_id
INNER JOIN #books ssbm ON ssbm.source_system_book_id1 = sdh.source_system_book_id1
	AND ssbm.source_system_book_id2 = sdh.source_system_book_id2
	AND ssbm.source_system_book_id3 = sdh.source_system_book_id3
	AND ssbm.source_system_book_id4 = sdh.source_system_book_id4
CROSS APPLY(SELECT SUM(mhdi.assigned_vol) vol 
			FROM matching_header_detail_info mhdi
			WHERE md.source_deal_detail_id = mhdi.source_deal_detail_id 
		AND mhdi.link_id <= md.link_id) assignedVol
CROSS APPLY(SELECT SUM(mhdi.assigned_vol) vol 
			FROM matching_header_detail_info mhdi
			WHERE md.source_deal_detail_id = mhdi.source_deal_detail_id_from 
			AND mhdi.link_id <= md.link_id) assignedVol1
...
INNER JOIN source_commodity sco ON sco.source_commodity_id = sdh.commodity_id
INNER JOIN source_deal_header_template sdht ON sdht.template_id = sdh.template_id
INNER JOIN static_data_value sdv_deal_status ON sdv_deal_status.value_id = sdh.deal_status
LEFT JOIN static_data_value sdv_confirm_status ON sdv_confirm_status.value_id = sdh.confirm_status_type
...
LEFT JOIN source_uom su_position ON su_position.source_uom_id = sdd.position_uom
LEFT JOIN source_currency scu ON scu.source_currency_id = sdd.fixed_price_currency_id
LEFT JOIN static_data_value sdv_match_status ON sdv_match_status.value_id = mh.match_status
	AND sdv_match_status.type_id = 27200
LEFT JOIN rec_generator rg ON rg.generator_id = sdh.generator_id
LEFT JOIN matching_header_detail_info mhdi ON mhdi.link_id = mh.link_id
	AND mhdi.source_deal_detail_id_from = md.source_deal_detail_id
LEFT JOIN gis_certificate gc ON gc.source_deal_header_id = mhdi.source_deal_detail_id_from
		AND gc.state_value_id = mhdi.state_value_id
		AND gc.tier_type = mhdi.tier_value_id
LEFT JOIN static_data_value jur ON jur.value_id = gc.state_value_id
	AND jur.type_id = 10002
LEFT JOIN static_data_value tier ON tier.value_id = gc.tier_type
LEFT JOIN static_data_value jur1 ON jur1.value_id = mhdi.state_value_id
	AND jur1.type_id = 10002
LEFT JOIN static_data_value tier1 ON tier1.value_id = mhdi.tier_value_id
WHERE 1 = 1 ''
IF (NULLIF(@_link_id_from, '''') IS NOT NULL AND NULLIF(@_link_id_to, '''') IS NULL)
BEGIN
	SET @_sql_string = @_sql_string + '' AND mh.link_id = '' + CAST(@_link_id_from AS VARCHAR(10))
END
IF (NULLIF(@_link_id_from, '''') IS NOT NULL AND NULLIF(@_link_id_to, '''') IS NOT NULL)
BEGIN
	SET @_sql_string = @_sql_string + '' AND mh.link_id BETWEEN '' + CAST(@_link_id_from AS VARCHAR(10)) + '' AND '' + CAST(@_link_id_to AS VARCHAR(10))
END
IF (NULLIF(@_link_id_from, '''') IS NULL AND NULLIF(@_link_id_to, '''') IS NOT NULL)
BEGIN
	SET @_sql_string = @_sql_string + '' AND mh.link_id < '' + CAST(@_link_id_to AS VARCHAR(10))
END
IF (NULLIF(@_effective_date_from, '''') IS NOT NULL AND NULLIF(@_effective_date_to, '''') IS NULL)
BEGIN
	SET @_sql_string = @_sql_string + '' AND mh.link_effective_date = '''''' + @_effective_date_from + ''''''''
END
IF (NULLIF(@_effective_date_from, '''') IS NOT NULL AND NULLIF(@_effective_date_to, '''') IS NOT NULL)
BEGIN
	SET @_sql_string = @_sql_string + '' AND mh.link_effective_date BETWEEN '''''' + @_effective_date_from + '''''' AND '''''' + @_effective_date_to + ''''''''
END
IF (NULLIF(@_effective_date_from, '''') IS NULL AND NULLIF(@_effective_date_to, '''') IS NOT NULL)
BEGIN
	SET @_sql_string = @_sql_string + '' AND mh.link_effective_date < '''''' + @_effective_date_to + ''''''''
END
IF (NULLIF(@_deal_date_from, '''') IS NOT NULL AND NULLIF(@_deal_date_to, '''') IS NULL)
BEGIN
	SET @_sql_string = @_sql_string + '' AND sdh.deal_date = '''''' + @_deal_date_from + ''''''''
END
IF (NULLIF(@_deal_date_from, '''') IS NOT NULL AND NULLIF(@_deal_date_to, '''') IS NOT NULL)
BEGIN
	SET @_sql_string = @_sql_string + '' AND sdh.deal_date BETWEEN '''''' + @_deal_date_from + '''''' AND '''''' + @_deal_date_to + ''''''''
END
IF (NULLIF(@_deal_date_from, '''') IS NULL AND NULLIF(@_deal_date_to, '''') IS NOT NULL)
BEGIN
	SET @_sql_string = @_sql_string + '' AND sdh.deal_date < '''''' + @_deal_date_to + ''''''''
END
IF NULLIF(@_source_deal_header_id, '''') IS NOT NULL AND NULLIF(@_link_ids, '''') IS NOT NULL
BEGIN
	SET @_sql_string = @_sql_string + '' AND mh.link_id IN ('' + @_link_ids + '')''
END
IF NULLIF (@_assignment_type, '''') IS NOT NULL
BEGIN 
    SET @_sql_string = @_sql_string + '' AND mh.assignment_type IN ('' + @_assignment_type + '')''
END

EXEC (@_sql_string)', report_id = @report_id_data_source_dest,
	system_defined = '1'
	,category = '106500' 
	WHERE [name] = 'Deal Match View'
		AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1)
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'commodity_id' AS [name], 'Commodity ID' AS ALIAS, 0 AS reqd_param, 1 AS widget_id, 4 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, 1 AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Deal Match View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'curve_id' AS [name], 'Index ID' AS ALIAS, 0 AS reqd_param, 7 AS widget_id, 4 AS datatype_id, 'browse_curve' AS param_data_source, NULL AS param_default_value, 1 AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Deal Match View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'Description 4' AS [name], 'Description 4' AS ALIAS, 0 AS reqd_param, 1 AS widget_id, 5 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, 1 AS append_filter, NULL  AS tooltip,0 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Deal Match View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'effective_date_from' AS [name], 'Effective Date From' AS ALIAS, 1 AS reqd_param, 6 AS widget_id, 2 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, 0 AS append_filter, NULL  AS tooltip,4 AS column_template, 0 AS key_column, 0 AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Deal Match View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'effective_date_to' AS [name], 'Effective Date To' AS ALIAS, 1 AS reqd_param, 6 AS widget_id, 4 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, 0 AS append_filter, NULL  AS tooltip,4 AS column_template, 0 AS key_column, 0 AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Deal Match View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	IF EXISTS (SELECT 1 
	           ...
	           INNER JOIN data_source ds on ds.data_source_id = dsc.source_id 
	           WHERE ds.[name] = 'Deal Match View'
	            AND dsc.name =  'link_description'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
	BEGIN
		UPDATE dsc  
		SET alias = 'Link Description'
			   , reqd_param = 0, widget_id = 1, datatype_id = 5, param_data_source = NULL, param_default_value = NULL, append_filter = 1, tooltip = NULL, column_template = 0, key_column = 0, required_filter = NULL
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'link_description' AS [name], 'Link Description' AS ALIAS, 0 AS reqd_param, 1 AS widget_id, 5 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, 1 AS append_filter, NULL  AS tooltip,0 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Deal Match View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'link_id_from' AS [name], 'Link ID From' AS ALIAS, 1 AS reqd_param, 1 AS widget_id, 4 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, 0 AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, 0 AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Deal Match View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'link_id_to' AS [name], 'Link ID To' AS ALIAS, 1 AS reqd_param, 1 AS widget_id, 4 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, 0 AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, 0 AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Deal Match View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'trader_id' AS [name], 'Trader ID' AS ALIAS, 0 AS reqd_param, 1 AS widget_id, 4 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, 1 AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Deal Match View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'source_deal_header_id' AS [name], 'Deal ID' AS ALIAS, 0 AS reqd_param, 1 AS widget_id, 4 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, 1 AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, 0 AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Deal Match View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'Buy_Sell' AS [name], 'Buy Sell' AS ALIAS, 0 AS reqd_param, 1 AS widget_id, 5 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, 1 AS append_filter, NULL  AS tooltip,0 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Deal Match View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'Fixed_Price' AS [name], 'Fixed Price' AS ALIAS, 0 AS reqd_param, 1 AS widget_id, 5 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, 1 AS append_filter, NULL  AS tooltip,0 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Deal Match View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	IF EXISTS (SELECT 1 
	           ...
	           INNER JOIN data_source ds on ds.data_source_id = dsc.source_id 
	           WHERE ds.[name] = 'Deal Match View'
	            AND dsc.name =  'Matched_Volume'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
	BEGIN
		UPDATE dsc  
		SET alias = 'Matched Volume'
			   , reqd_param = 0, widget_id = 1, datatype_id = 4, param_data_source = NULL, param_default_value = NULL, append_filter = 1, tooltip = NULL, column_template = 2, key_column = 0, required_filter = NULL
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'Matched_Volume' AS [name], 'Matched Volume' AS ALIAS, 0 AS reqd_param, 1 AS widget_id, 4 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, 1 AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Deal Match View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'Remaining_Volume' AS [name], 'Remaining Volume' AS ALIAS, 0 AS reqd_param, 1 AS widget_id, 3 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, 1 AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Deal Match View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'match_status' AS [name], 'Match Status' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 5 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,0 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Deal Match View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		UPDATE dsc  
		SET alias = 'Match Status ID'
			   , reqd_param = NULL, widget_id = 2, datatype_id = 4, param_data_source = 'SELECT value_id,' + CHAR(10) + '' + CHAR(9) + '   code ' + CHAR(10) + 'FROM static_data_value' + CHAR(10) + 'WHERE type_id = 27200', param_default_value = NULL, append_filter = NULL, tooltip = NULL, column_template = 2, key_column = 0, required_filter = NULL
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
		WHERE ds.[name] = 'Deal Match View'
	...
	IF EXISTS (SELECT 1 
	           ...
	           INNER JOIN data_source ds on ds.data_source_id = dsc.source_id 
	           WHERE ds.[name] = 'Deal Match View'
	            AND dsc.name =  'certificate_from'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
	BEGIN
		UPDATE dsc  
		SET alias = 'Certificate From'
			   , reqd_param = NULL, widget_id = 1, datatype_id = 5, param_data_source = NULL, param_default_value = NULL, append_filter = NULL, tooltip = NULL, column_template = 0, key_column = 0, required_filter = NULL
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'certificate_from' AS [name], 'Certificate From' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 5 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,0 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Deal Match View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	IF EXISTS (SELECT 1 
	           ...
	           INNER JOIN data_source ds on ds.data_source_id = dsc.source_id 
	           WHERE ds.[name] = 'Deal Match View'
	            AND dsc.name =  'certificate_to'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
	BEGIN
		UPDATE dsc  
		SET alias = 'Certificate To'
			   , reqd_param = NULL, widget_id = 1, datatype_id = 5, param_data_source = NULL, param_default_value = NULL, append_filter = NULL, tooltip = NULL, column_template = 0, key_column = 0, required_filter = NULL
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'certificate_to' AS [name], 'Certificate To' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 5 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,0 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Deal Match View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'sequence_from' AS [name], 'Sequence From' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 4 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Deal Match View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'sequence_to' AS [name], 'Sequence To' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 4 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Deal Match View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		UPDATE dsc  
		SET alias = 'Assignment Type'
			   , reqd_param = NULL, widget_id = 2, datatype_id = 4, param_data_source = 'exec spa_StaticDataValues @flag =''h'', @type_id =''10013'',  @value_ids = ''5146,10013''', param_default_value = NULL, append_filter = NULL, tooltip = NULL, column_template = 2, key_column = 0, required_filter = 0
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
		WHERE ds.[name] = 'Deal Match View'
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'assignment_type' AS [name], 'Assignment Type' AS ALIAS, NULL AS reqd_param, 2 AS widget_id, 4 AS datatype_id, 'exec spa_StaticDataValues @flag =''h'', @type_id =''10013'',  @value_ids = ''5146,10013''' AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, 0 AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Deal Match View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
