BEGIN TRY
	...
	declare @new_ds_alias varchar(10) = 'WFCN'
	/** IF DATA SOURCE ALIAS ALREADY EXISTS ON DESTINATION, RAISE ERROR **/
	if exists(select top 1 1 from data_source where alias = 'WFCN' and name <> 'Counterparty')
	begin
		select top 1 @new_ds_alias = 'WFCN' + cast(s.n as varchar(5))
		from seq s
		left join data_source ds on ds.alias = 'WFCN' + cast(s.n as varchar(5))
	...
	AND NOT EXISTS (SELECT 1 FROM map_function_category WHERE [function_name] = 'Counterparty' AND '106502' = '106501') 
	BEGIN
		INSERT INTO data_source([type_id], [name], [alias], [description], [tsql], report_id, system_defined,category)
		SELECT TOP 1 1 AS [type_id], 'Counterparty' AS [name], @new_ds_alias AS ALIAS, 'Workflow Counterparty View' AS [description],null AS [tsql], @report_id_data_source_dest AS report_id,'0' AS [system_defined]
			,'106502' AS [category]
	END

	UPDATE data_source
	SET alias = @new_ds_alias, description = 'Workflow Counterparty View'
	, [tsql] = CAST('' AS VARCHAR(MAX)) + 'SELECT * 
--[__batch_report__]
FROM WF_Counterparty', report_id = @report_id_data_source_dest,
	system_defined = '0'
	,category = '106502' 
	WHERE [name] = 'Counterparty'
		AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1)
	...
	BEGIN
		...
		SELECT  @new_data_source_id = data_source_id FROM data_source WHERE [name] = 'Counterparty'

		IF NOT EXISTS (SELECT 1 FROM alert_table_definition atd INNER JOIN data_source ds ON atd.logical_table_name = ds.name WHERE ds.name = 'Counterparty')
		BEGIN
			INSERT INTO alert_table_definition (logical_table_name, physical_table_name, data_source_id, is_action_view, primary_column)
			SELECT 'Counterparty','WF_Counterparty',@new_data_source_id,'y','source_counterparty_id'
		END
		ELSE 
		BEGIN
			...
			SET data_source_id = @new_data_source_id,
				is_action_view = 'y',
				primary_column = 'source_counterparty_id',
				physical_table_name  = 'WF_Counterparty'
			WHERE logical_table_name = 'Counterparty'
		END
		IF NOT EXISTS (
		SELECT 1
		...
		BEGIN
			...
			SELECT 20602 , atd.alert_table_definition_id, 1 
	...
	IF EXISTS (SELECT 1 
	           ...
	           WHERE ds.[name] = 'Counterparty'
	            AND dsc.name =  'analyst'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
	BEGIN
		UPDATE dsc  
		SET alias = 'Analyst'
			   , reqd_param = NULL, widget_id = 2, datatype_id = 5, param_data_source = 'SELECT user_login_id,user_f_name _ADD_ '' ''_ADD_ user_l_name AS [name] FROM application_users ORDER BY user_f_name', param_default_value = NULL, append_filter = NULL, tooltip = NULL, column_template = 0, key_column = 0, required_filter = NULL
	...
	BEGIN
		...
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'analyst' AS [name], 'Analyst' AS ALIAS, NULL AS reqd_param, 2 AS widget_id, 5 AS datatype_id, 'SELECT user_login_id,user_f_name _ADD_ '' ''_ADD_ user_l_name AS [name] FROM application_users ORDER BY user_f_name' AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,0 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Counterparty'
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'counterparty_desc' AS [name], 'Counterparty Desc' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 5 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,0 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Counterparty'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		SET alias = 'Counterparty Status'
			   , reqd_param = NULL, widget_id = 2, datatype_id = 4, param_data_source = 'SELECT value_id, code FROM static_data_value WHERE [type_id] = 101500 ORDER BY 2', param_default_value = NULL, append_filter = NULL, tooltip = NULL, column_template = 2, key_column = 0, required_filter = NULL
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
		WHERE ds.[name] = 'Counterparty'
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'customer_duns_number' AS [name], 'Customer Duns Number' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 5 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,0 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Counterparty'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		SET alias = 'Int Ext Flag'
			   , reqd_param = NULL, widget_id = 2, datatype_id = 1, param_data_source = 'SELECT ''i'' id, ''Internal'' value UNION SELECT ''e'', ''External'' UNION SELECT ''b'', ''Broker'' UNION SELECT ''c'', ''Clearing''', param_default_value = NULL, append_filter = NULL, tooltip = NULL, column_template = 0, key_column = 0, required_filter = NULL
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
		WHERE ds.[name] = 'Counterparty'
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'int_ext_flag' AS [name], 'Int Ext Flag' AS ALIAS, NULL AS reqd_param, 2 AS widget_id, 1 AS datatype_id, 'SELECT ''i'' id, ''Internal'' value UNION SELECT ''e'', ''External'' UNION SELECT ''b'', ''Broker'' UNION SELECT ''c'', ''Clearing''' AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,0 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Counterparty'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	IF EXISTS (SELECT 1 
	           ...
	           INNER JOIN data_source ds on ds.data_source_id = dsc.source_id 
	           WHERE ds.[name] = 'Counterparty'
	            AND dsc.name =  'netting_parent_counterparty_id'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
	BEGIN
		UPDATE dsc  
		SET alias = 'Netting Parent Counterparty Id'
