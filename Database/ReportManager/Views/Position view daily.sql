BEGIN TRY
	...
	IF EXISTS (SELECT 1 
	           FROM data_source 
	           WHERE [name] = 'Position View Daily'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
			
	BEGIN
		UPDATE data_source
		SET alias = 'PVD', description = ''
...
WHERE vw.expiration_date > ''@as_of_date'' AND vw.term_start > ''@as_of_date''', report_id = @report_id_data_source_dest 
		WHERE [name] = 'Position View Daily'
			AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1)
	END	
	...
	BEGIN
		...
		SELECT TOP 1 1 AS [type_id], 'Position View Daily' AS [name], 'PVD' AS ALIAS, '' AS [description],'IF OBJECT_ID(N''tempdb..#books'') IS NOT NULL
	DROP TABLE #books
IF OBJECT_ID(N''tempdb..#term_date'') IS NOT NULL
	DROP TABLE #term_date
...
LEFT JOIN source_book sb4 ON sb4.source_book_id = vw.source_system_book_id4
LEFT JOIN static_data_value sdv_block ON sdv_block.value_id  = sdh.block_define_id
WHERE vw.expiration_date > ''@as_of_date'' AND vw.term_start > ''@as_of_date''' AS [tsql], @report_id_data_source_dest AS report_id
	END 
	...
	IF EXISTS (SELECT 1 
	           ...
	           WHERE ds.[name] = 'Position View Daily'
