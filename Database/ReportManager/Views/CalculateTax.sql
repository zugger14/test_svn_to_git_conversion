...
SELECT a.source_deal_header_id, a.value tax_value
into #temp_tax_value
 FROM (select distinct source_deal_header_id,udf_template_id from''+ @_farrms_calc_process_table+'') a
OUTER APPLY (
...
FROM '' + @_farrms_calc_process_table + '' f inner join #temp_tax_value ttv on ttv.source_deal_header_id=f.source_deal_header_id
