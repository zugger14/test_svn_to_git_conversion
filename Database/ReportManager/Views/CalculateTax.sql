...
--SET @_source_deal_header_id = 520214 --227305 
--SET @_charge_type = ''-10019''
--SET @_tax_type = ''e''  -- vat, energy
--SET @_price = ''p'' 
...
OUTER APPLY (
	...
	AND term_start = @_prod_date_from
) tax_amt
CROSS APPLY(
...
OUTER APPLY (
	...
	WHERE source_deal_header_id = @_source_deal_header_id
	AND term_start = @_prod_date_from
...
AND ttr.document_type = ISNULL(NULLIF(ttr_both.document_type, ''a''), ttr_inv_rem.document_type)', report_id = @report_id_data_source_dest,
	system_defined = '0'
	,category = '106501' 
	WHERE [name] = 'CalculateTax'
		AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1)
