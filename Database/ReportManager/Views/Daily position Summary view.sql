BEGIN TRY
	...
	declare @new_ds_alias varchar(10) = 'dpsv1'
	/** IF DATA SOURCE ALIAS ALREADY EXISTS ON DESTINATION, RAISE ERROR **/
	if exists(select top 1 1 from data_source where alias = 'dpsv1' and name <> 'Daily position Summary view')
	begin
		select top 1 @new_ds_alias = 'dpsv1' + cast(s.n as varchar(5))
		from seq s
		left join data_source ds on ds.alias = 'dpsv1' + cast(s.n as varchar(5))
	...
	IF NOT EXISTS (SELECT 1 
	           FROM data_source 
	           WHERE [name] = 'Daily Position Summary View'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
	AND NOT EXISTS (SELECT 1 FROM map_function_category WHERE [function_name] = 'Daily Position Summary View' AND '106500' = '106501') 
	BEGIN
		INSERT INTO data_source([type_id], [name], [alias], [description], [tsql], report_id, system_defined,category)
		SELECT TOP 1 1 AS [type_id], 'Daily Position Summary View' AS [name], @new_ds_alias AS ALIAS, 'Daily Position Summary View' AS [description],null AS [tsql], @report_id_data_source_dest AS report_id,'0' AS [system_defined]
			,'106500' AS [category]
	END

	UPDATE data_source
	SET alias = @new_ds_alias, description = 'Daily Position Summary View'
...
DECLARE @_summary_option VARCHAR(6) =''d00s''
	, @_group_by CHAR(1) = ''s'' 
...
EXEC dbo.spa_position_report @_summary_option = @_summary_option, 
	...
	IF EXISTS (SELECT 1 
	           ...
	           WHERE ds.[name] = 'Daily Position Summary View'
