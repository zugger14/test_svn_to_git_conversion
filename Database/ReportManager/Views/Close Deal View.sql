BEGIN TRY
	...
	declare @new_ds_alias varchar(10) = 'CDV1'
	/** IF DATA SOURCE ALIAS ALREADY EXISTS ON DESTINATION, RAISE ERROR **/
	if exists(select top 1 1 from data_source where alias = 'CDV1' and name <> 'Close Deal View')
	begin
		select top 1 @new_ds_alias = 'CDV1' + cast(s.n as varchar(5))
		from seq s
		left join data_source ds on ds.alias = 'CDV1' + cast(s.n as varchar(5))
	...
	IF NOT EXISTS (SELECT 1 
	           FROM data_source 
	           WHERE [name] = 'Close Deal View'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
	AND NOT EXISTS (SELECT 1 FROM map_function_category WHERE [function_name] = 'Close Deal View' AND '106500' = '106501') 
	BEGIN
		INSERT INTO data_source([type_id], [name], [alias], [description], [tsql], report_id, system_defined,category)
		SELECT TOP 1 1 AS [type_id], 'Close Deal View' AS [name], @new_ds_alias AS ALIAS, 'Close Deal View' AS [description],null AS [tsql], @report_id_data_source_dest AS report_id,'1' AS [system_defined]
			,'106500' AS [category]
	END

	UPDATE data_source
	SET alias = @new_ds_alias, description = 'Close Deal View'
	, [tsql] = CAST('' AS VARCHAR(MAX)) + 'DECLARE @_sql VARCHAR(MAX)

DECLARE @_deal_type_id VARCHAR(1000)

DECLARE @_term_start VARCHAR(50)



DECLARE @_product_id VARCHAR(500) = NULL
...
SET @_sql = ''SELECT 
	...
	,case when sdh.header_buy_sell_flag=''''b'''' then ''''Buy'''' Else ''''Sell'''' End [Buy Sell]
	...
	,mdv.term_start [term_end]

	, sdv.value_id product_id
	...
	INNER JOIN match_deal_volume mdv ON sdd.source_deal_detail_id = CASE WHEN sdd.buy_sell_flag = ''''b'''' THEN buy_source_deal_detail_id ELSE sell_source_deal_detail_id END
	...
	INNER JOIN user_defined_deal_fields uddf
	   ...
	   AND ISNUMERIC(ISNULL(NULLIF(uddf.udf_value,'''' ''''),1)) = 1
	...
	LEFT JOIN static_data_value sdv ON sdv.value_id = CAST(uddf.udf_value AS FLOAT)

		AND sdv.type_id = 108100

	INNER JOIN source_uom su ON su.source_uom_id = sdd.position_uom
	...
	WHERE uddft.field_label = ''''Margin Product'''' 

	''



	IF @_deal_type_id IS NOT NULL



	SET @_sql += '' AND sdt.source_deal_type_id IN ('' + @_deal_type_id  + '')''	



	IF @_term_start IS NOT NULL

	SET @_sql += '' AND mdv.term_start >= '''''' + @_term_start + ''''''''



	IF @_term_end IS NOT NULL

	SET @_sql += '' AND mdv.term_start < = '''''' + @_term_end + ''''''''
	...
	SET @_sql += '' AND sdv.value_id IN ('' + @_product_id  + '')''	
	...
	BEGIN
		...
		SET alias = 'Deal Type Id'
			   , reqd_param = NULL, widget_id = 2, datatype_id = 4, param_data_source = 'exec spa_source_deal_type_maintain ''x''', param_default_value = NULL, append_filter = NULL, tooltip = NULL, column_template = 2, key_column = 0, required_filter = 1
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
		WHERE ds.[name] = 'Close Deal View'
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'match_vol' AS [name], 'Match Vol' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 3 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Close Deal View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		SET alias = 'Product Id'
			   , reqd_param = NULL, widget_id = 2, datatype_id = 4, param_data_source = 'EXEC spa_StaticDataValues ''h'', ''108100''', param_default_value = NULL, append_filter = NULL, tooltip = NULL, column_template = 2, key_column = 0, required_filter = 0
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
		WHERE ds.[name] = 'Close Deal View'
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'Buy Sell' AS [name], 'Buy Sell' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 5 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,0 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Close Deal View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
