BEGIN TRY
	...
	IF NOT EXISTS (SELECT 1 
	           FROM data_source 
	           WHERE [name] = 'ContractualFixedPrice'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
	AND NOT EXISTS (SELECT 1 FROM map_function_category WHERE [function_name] = 'ContractualFixedPrice' AND '106501' = '106501') 
	BEGIN
		INSERT INTO data_source([type_id], [name], [alias], [description], [tsql], report_id, system_defined,category)
		SELECT TOP 1 1 AS [type_id], 'ContractualFixedPrice' AS [name], 'cfv' AS ALIAS, 'ContractualFixedPrice' AS [description],null AS [tsql], @report_id_data_source_dest AS report_id,'0' AS [system_defined]
			,'106501' AS [category]
	END

	UPDATE data_source
	SET alias = 'cfv', description = 'ContractualFixedPrice'
...
--SET @_source_deal_header_id = 13865 

IF OBJECT_ID(''tempdb..#temp_deal_details'') IS NOT NULL
    DROP TABLE #temp_deal_details

CREATE TABLE #temp_deal_details(row_id INT, deal_id VARCHAR(100),source_deal_detail_id INT, fixed_price FLOAT, total_volume FLOAT, tv_x_fp FLOAT)

INSERT INTO  #temp_deal_details(row_id,deal_id, source_deal_detail_id, fixed_price, total_volume, tv_x_fp)
SELECT	
		ROW_NUMBER() OVER(ORDER BY source_deal_detail_id ASC) AS row_id,
		source_deal_header_id, 
		source_deal_detail_id, 
		fixed_price, 
		total_volume,
		fixed_price * total_volume 
FROM source_deal_detail 
WHERE  source_deal_header_id = @_source_deal_header_id


SELECT @_max_row_count = MAX(row_id) FROM #temp_deal_details

IF @_max_row_count = 1
BEGIN
	SELECT @_fixed_price = fixed_price FROM #temp_deal_details
END
ELSE
BEGIN 
	SELECT @_fixed_price = SUM(tv_x_fp) / SUM(total_volume) 
	FROM #temp_deal_details
END  

 
SELECT		TOP 1
			tdd.deal_id source_deal_header_id, 
			tdd.source_deal_detail_id,  
			1 [hour], 
			2 [mins], 
			@_fixed_price [value]
--[__batch_report__]  
...
FROM #temp_deal_details tdd
 ', report_id = @report_id_data_source_dest,
	...
	IF EXISTS (SELECT 1 
	           ...
	           WHERE ds.[name] = 'ContractualFixedPrice'
	            AND dsc.name =  'hour'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
	BEGIN
		UPDATE dsc  
	...
	BEGIN
		...
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'ContractualFixedPrice'
	...
	FROM data_source_column dsc 
	INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
		AND ds.[name] = 'ContractualFixedPrice'
		AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1)
