BEGIN TRY
	...
	declare @new_ds_alias varchar(10) = 'CSEV'
	/** IF DATA SOURCE ALIAS ALREADY EXISTS ON DESTINATION, RAISE ERROR **/
	if exists(select top 1 1 from data_source where alias = 'CSEV' and name <> 'Contract Setup Extract View')
	begin
		select top 1 @new_ds_alias = 'CSEV' + cast(s.n as varchar(5))
		from seq s
		left join data_source ds on ds.alias = 'CSEV' + cast(s.n as varchar(5))
	...
	IF NOT EXISTS (SELECT 1 
	           FROM data_source 
	           WHERE [name] = 'Contract Setup Extract View'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
	AND NOT EXISTS (SELECT 1 FROM map_function_category WHERE [function_name] = 'Contract Setup Extract View' AND '106500' = '106501') 
	BEGIN
		INSERT INTO data_source([type_id], [name], [alias], [description], [tsql], report_id, system_defined,category)
		SELECT TOP 1 1 AS [type_id], 'Contract Setup Extract View' AS [name], @new_ds_alias AS ALIAS, 'Standard Contract Setup Extract View' AS [description],null AS [tsql], @report_id_data_source_dest AS report_id,'1' AS [system_defined]
			,'106500' AS [category]
	END

	UPDATE data_source
	SET alias = @new_ds_alias, description = 'Standard Contract Setup Extract View'
...
DECLARE @_header_physical_financial_flag VARCHAR(20)
DECLARE @_header_buy_sell_flag VARCHAR(20)
...
DECLARE @_counterparty_type VARCHAR(20)
DECLARE @_deal_lock VARCHAR(20)
...
DECLARE @_detail_phy_fin_flag VARCHAR(100)
DECLARE @_formula_curve_id VARCHAR(100)
...
SET @_header_physical_financial_flag = nullif(isnull(@_header_physical_financial_flag,nullif(''@header_physical_financial_flag'', replace(''@_header_physical_financial_flag'',''@_'',''@''))),''null'')
SET @_header_buy_sell_flag = nullif(isnull(@_header_buy_sell_flag,nullif(''@header_buy_sell_flag'', replace(''@_header_buy_sell_flag'',''@_'',''@''))),''null'')
...
SET @_detail_phy_fin_flag = nullif(isnull(@_detail_phy_fin_flag,nullif(''@detail_phy_fin_flag'', replace(''@_detail_phy_fin_flag'',''@_'',''@''))),''null'')
SET @_curve_id = nullif(isnull(@_curve_id,nullif(''@curve_id'', replace(''@_curve_id'',''@_'',''@''))),''null'')
...
SET @_counterparty_type = nullif(isnull(@_counterparty_type,nullif(''@counterparty_type'', replace(''@_counterparty_type'',''@_'',''@''))),''null'')
SET @_deal_lock = nullif(isnull(@_deal_lock,nullif(''@deal_lock'', replace(''@_deal_lock'',''@_'',''@''))),''null'')
SET @_block_type = nullif(isnull(@_block_type,nullif(''@block_type'', replace(''@_block_type'',''@_'',''@''))),''null'')
SET @_legal_entity = nullif(isnull(@_legal_entity,nullif(''@legal_entity'', replace(''@_legal_entity'',''@_'',''@''))),''null'')
SET @_deal_id = nullif(isnull(@_deal_id,nullif(''@deal_id'', replace(''@_deal_id'',''@_'',''@''))),''null'')
SET @_block_define_id = nullif(isnull(@_block_define_id,nullif(''@block_define_id'', replace(''@_block_define_id'',''@_'',''@''))),''null'')


--set @_source_deal_header_id = 1499
--SELECT @_as_of_date = GETDATE() 
...
SELECT books.stra_id stra_id,
       ...
       sdd.leg [leg],
       getdate() [term_start], --sdd.term_start [term_start],
       getdate() [term_end],--sdd.term_end [term_end],
       ...
       getdate() [contract_expiration_date],--sdd.contract_expiration_date [contract_expiration_date],
       sdd.price_adder2,
	   ...
	   sdh.broker_id,
	   1 term_start_day, --DAY(sdd.term_start) term_start_day,
	   1 term_start_month, --MONTH(sdd.term_start) term_start_month,
	   1 term_start_month_name,  --DATENAME(m,sdd.term_start) term_start_month_name,
	   1 term_start_month_quarter, --DATEPART(q,sdd.term_start) term_start_month_quarter,
	   1 term_start_year, --YEAR(sdd.term_start) term_start_year,	   
...
#temp_final
 ', report_id = @report_id_data_source_dest,
	...
	IF EXISTS (SELECT 1 
	           ...
	           WHERE ds.[name] = 'Contract Setup Extract View'
	            AND dsc.name =  'sub_id'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
	BEGIN
		UPDATE dsc  
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'formula_curve_id' AS [name], 'Indexed On' AS ALIAS, 1 AS reqd_param, 2 AS widget_id, 4 AS datatype_id, 'SELECT spcd.source_curve_def_id, spcd.curve_name' + CHAR(10) + 'FROM source_price_curve_def spcd' + CHAR(10) + 'INNER JOIN (SELECT DISTINCT formula_curve_id FROM source_deal_detail WHERE formula_curve_id IS NOT NULL) sdd' + CHAR(10) + 'ON sdd.formula_curve_id = spcd.source_curve_def_id' + CHAR(10) + 'ORDER BY spcd.curve_name' AS param_data_source, NULL AS param_default_value, 0 AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, 0 AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Contract Setup Extract View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'pricing_period' AS [name], 'Pricing Period' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 5 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,0 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Contract Setup Extract View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'pricing_index' AS [name], 'Pricing Index' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 5 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,0 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Contract Setup Extract View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
