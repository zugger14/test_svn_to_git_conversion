BEGIN TRY
	...
	@_source_deal_header_id VARCHAR(MAX) = ''@source_deal_header_id'', --268021,
	@_flag CHAR(1) = ''v'',
...
BEGIN
	SET @_sql1 = ''
		 SELECT dbo.FNAConvertTZAwareDateFormat(GETDATE(),1) [Date], '''''' + isnull(@_company,'''') +'''''' company_name,
		 CASE 
	...
	,CASE 
	   ...
	   SET  @_sql2 = ''
				...
				,CASE 
					...
					,MAX(CASE WHEN scom_am.commodity_name IN (''''Natural Gas'''', ''''Gas'''')
					 ...
					 ,CASE
						...
						WHEN LEFT(sddh_shp.hr, 2) = ''''24'''' and MAX(sdh.commodity_id) = -1 THEN CAST(''''18'''' AS decimal(4,2))
		 ...
		 FROM source_deal_header sdh 
			...
			OUTER APPLY( SELECT volume, price, term_date, hr, --RIGHT(''''0'''' + CAST(LEFT(hr,2) - 1 as varchar(2)) + RIGHT(hr,3), 5) hr_from
								CASE WHEN sdh.profile_granularity = 987 AND RIGHT(hr,2) = ''''00'''' 
										  THEN RIGHT( ''''0'''' + CAST( LEFT(hr,2) - 1 as varchar(2)),2) + '''':'''' + ''''45''''
									  WHEN sdh.profile_granularity = 987 AND RIGHT(hr,2) <> ''''00'''' 
										  THEN CAST(LEFT(hr,2) as varchar(2)) + '''':'''' + RIGHT( ''''0'''' + CAST(RIGHT(hr,2) - 15 as varchar(2)), 2)
