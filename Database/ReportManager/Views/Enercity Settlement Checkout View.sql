...
--LEFT JOIN match_group_shipment mgs ON mgs.match_group_shipment_id = mgh.match_group_shipment_id AND mgs.match_group_shipment_id = sco.shipment_id


--LEFT JOIN match_group mg ON mg.match_group_id = mgs.match_group_id


--LEFT JOIN ticket_match tm ON tm.match_group_detail_id = mgd.match_group_detail_id  AND tm.ticket_match_id=sco.ticket_id
...
SELECT MAX(stmt_invoice_id) stmt_invoice_id
...
, IIF(NULLIF(MAX(vat_percentage), '''''''') IS NOT NULL , CONCAT(CAST(cast(MAX(vat_percentage) as Numeric(28, 10)) AS VARCHAR(250)) + ''''%'''', MAX(currency)), NULL) vat
...
EXEC (@_sql + @_sql1 + @_sql2 + @_sql3 + @_sql4 + @_sql5 + @_sql6) ', report_id = @report_id_data_source_dest,
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'vat' AS [name], 'Vat' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 5 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,0 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Enercity Settlement Checkout View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
