...
DECLARE @_stmt_invoice_id  VARCHAR(100) --= 2332
...
LEFT JOIN counterparty_contacts AS ccs ON CCS.counterparty_id = fs_counterparty.source_counterparty_id  AND ccs.contact_type = -32203
...
LEFT JOIN source_deal_detail sdd_d ON sdd_d.source_deal_header_id = sdh.source_deal_header_id
        AND sdd_d.term_start BETWEEN si.prod_date_from and si.prod_date_to
        AND sdd_d.term_end BETWEEN si.prod_date_from and si.prod_date_to
        LEFT JOIN static_data_value sdv_pli on sdv_pli.value_id = sco.pnl_line_item_id
LEFT JOIN match_group_detail mgd ON mgd.source_deal_detail_id = sdd.source_deal_detail_id  AND sco.accrual_or_final = ''''f''''
LEFT JOIN match_group_header mgh ON mgh.match_group_header_id = mgd.match_group_header_id
LEFT JOIN static_data_value sdv3 ON sdv3.value_id = sco.invoicing_charge_type_id 
...
SELECT 
    MAX(taf.stmt_invoice_id) stmt_invoice_id
        ...
        , CAST(MAX(ABS(taf.volume)) as Numeric(28, 10)) total_volume
        , CAST(MAX(taf.amount) As Numeric(28, 10)) net_total
...
FROM #temp_all_final taf 
  OUTER APPLY (
    ...
    OUTER APPLY (
    SELECT pvt.[Broker Fees by Deal Date] [Broker_Fees_by_Deal_Date],source_deal_header_id,term_start,term_end,as_of_date,index_fees_id
    FROM (
        SELECT ifbs.field_name
            , ifbs.[value]          
            , ifbs.source_deal_header_id
        ...
        FROM index_fees_breakdown_settlement ifbs
        INNER JOIN #temp_all_final taf ON ifbs.source_deal_header_id = taf.source_deal_header_id
            AND taf.term_start between  ifbs.term_start and ifbs.term_end
            AND taf.term_end between  ifbs.term_start and ifbs.term_end
        AND ifbs.as_of_date = taf.as_of_date
        WHERE field_name NOT IN (''''Positive Price Commodity VAT'''',''''Negative Price Commodity VAT'''',''''Commodity Energy Tax'''')
        ) x
    PIVOT(MAX(x.[value]) FOR x.field_name IN ([Broker Fees by Deal Date])) pvt
    ) cett  
OUTER APPLY (
...
select stmt_invoice_id, as_of_date, to_as_of_date,  prod_date_from, prod_date_to,   settlement_date,    counterparty_name,  counterparty_accounting_code,   counterparty_id,    contract_name,  contract_id,    charge_type,    charge_type_id, 
    ...
    counterparty_external_value, secondary_cc_address1,  secondary_cc_address2,  secondary_cc_city, secondary_cc_zip, secondary_cc_country,  Accrual_Final_Reversal, update_ts_from, update_ts_to,   create_ts_from, create_ts_to,   primary_counterparty_country,   primary_counterparty_bank_currency_id,  primary_counterparty_bank_currency, primary_counterparty_bank_currency_name
    , CONCAT(CAST(SUM(total_volume) AS VARCHAR(250)) + '''' '''', uom) total_volume
    , CONCAT(CAST(SUM(net_total) AS VARCHAR(250)) + '''' '''', currency) net_total
    , (Vat) as vat 
    --,CASE WHEN settlement_amount <0 THEN -1* (ISNULL(ABS(vat),0) +ABS(settlement_amount) + ISNULL(SUM(abs(commodity_energy_tax_value)), 0)) ELSE 1 * (ISNULL(ABS(vat),0) +ABS(settlement_amount) + ISNULL(SUM(abs(commodity_energy_tax_value)), 0)) END gross_total
    ,CASE WHEN CAST(SUM(settlement_amount) as Numeric(28, 10)) <0 THEN -1* (ISNULL(ABS(vat),0) +ABS(CAST(SUM(settlement_amount) as Numeric(28, 10))) + ISNULL(SUM(abs(commodity_energy_tax_value)), 0)) ELSE 1 * (ISNULL(ABS(vat),0) +ABS(CAST(SUM(settlement_amount) as Numeric(28, 10))) + ISNULL(SUM(abs(commodity_energy_tax_value)), 0)) END gross_total
    , void_status, SUM(abs(commodity_energy_tax_value)) commodity_energy_tax_value
    --[__batch_report__]   
FROM  #tempt_last_finals
GROUP BY stmt_invoice_id, as_of_date, to_as_of_date,  prod_date_from, prod_date_to,   settlement_date, counterparty_name,  counterparty_accounting_code,   counterparty_id, contract_name,  contract_id, charge_type, charge_type_id, uom, deal_volume
    , currency, price, invoice_number, invoice_type, invoice_status, invoice_notes, invoice_subject, cash_received, cash_receive_variance_amount, cash_received_date, charge_type_alias, receive_pay, accounting_status, invoice_date, invoice_payment_date, invoice_template, lock_status, source_deal_header_id, payment_date_from, payment_date_to, pnl_line_item, deal_reference, Leg, invoicing_charge_type, Payment_Dr_GL_Code, Payment_Cr_GL_Code, Payment_Dr_GL_Name, Payment_Cr_GL_Name, Debit_GL_Number, Credit_GL_Number, Debit_account_name, Credit_account_name, payment_status, book, strategy, subsidary, sub_book, subsidiary_accounting_code, strategy_accounting_code, book_accounting_code, sub_book_accounting_code, location_name, location_accounting_code, location_group, commodity, commodity_accounting_code, commodity_description, accounting_receivable_id, accounting_payable_id, [index], template_name, deal_type_name, trader, country, region
    --, term_start, term_end
    , term_start_year_month, actual_forward, term_quarter
    --, pnl_date
    , physical_financial_flag, invoice_id, counterparty_description, counterparty_contact, counterparty_address1, counterparty_address2, counterparty_city, counterparty_state, counterparty_zip, counterparty_phone, counterparty_email, counterparty_fax, primary_counterparty, primary_counterparty_description, primary_counterparty_contact_name, primary_counterparty_address1, primary_counterparty_address2, primary_counterparty_city, primary_counterparty_state, primary_counterparty_zip, primary_counterparty_contact_telephone, primary_counterparty_email_address, primary_counterparty_fax, primary_bank_name, primary_account_name, primary_account_no, primary_iban, primary_swift_no, primary_reference, internal_deal_subtype_value_id, deal_date, primary_counterparty_bank_address1, primary_counterparty_bank_address2, accounting_month, accrual_or_final, Deal_Charge_Type_ID, Calc_Type, reversal_stmt_checkout_id, show_accounting_month, counterparty_country_name, counterparty_region_name, secondary_counterparty_id, secondary_counterparty_name, source_commodity_id, commodity_name, vat_percentage 
    , vat_remarks, vat, price_description
    , counterparty_external_value, secondary_cc_address1, secondary_cc_address2, secondary_cc_city, secondary_cc_zip, secondary_cc_country, Accrual_Final_Reversal, update_ts_from, update_ts_to, create_ts_from, create_ts_to, primary_counterparty_country, primary_counterparty_bank_currency_id, primary_counterparty_bank_currency, primary_counterparty_bank_currency_name
    --, total_volume, net_total
    ,void_status
''
