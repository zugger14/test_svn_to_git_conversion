...
FROM #temp_all_final taf 
  OUTER APPLY (
	...
	FROM (
		...
		AND ifbs.as_of_date = taf.as_of_date
		WHERE field_name NOT IN (''''Positive Price Commodity VAT'''',''''Negative Price Commodity VAT'''',''''Negative Price Commodity'''',''''Positive Price Commodity'''',''''Commodity Energy Tax'''')
		) x
