BEGIN TRY
	...
	IF EXISTS (SELECT 1 
	           FROM data_source 
	           WHERE [name] = 'Position View Hourly'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
			
	BEGIN
		UPDATE data_source
		SET alias = 'PVH', description = ''
...
DECLARE @_term_start_to datetime,@_sql VARCHAR(MAX)
...
IF ''@period_from'' <> ''NULL'' 
	SET @_term_start_from = dbo.FNAGetTermStartDate(''m'', @_as_of_date, ''@period_from'')

IF ''@period_to'' <> ''NULL''
	SET @_term_start_to = dbo.FNAGetTermENDDate(''m'', @_as_of_date, ''@period_to'')
...
CREATE INDEX ix_pt_book on #books(fas_book_id ) INCLUDE (source_system_book_id1,source_system_book_id2,source_system_book_id3,source_system_book_id4)
...
CREATE TABLE #temp_report_hourly_position_breakdown (block_define_id INT, term_start DATETIME, term_end DATETIME)

SET @_sql = ''
INSERT INTO	#temp_report_hourly_position_breakdown(block_define_id,term_start,term_end)  
SELECT distinct isnull(spcd.block_define_id,300501) block_define_id,s.term_start,s.term_end 
...
WHERE 1=1
AND (''''NULL'''' = ''''NULL'''' OR s.source_deal_header_id IN (NULL)) ''
+		
CASE 
...
WHERE 1=1
AND s.deal_date<='''''' + @_to_as_of_date + ''''''  AND s.expiration_date > '''''' + @_as_of_date + '''''' AND s.term_start > '''''' + @_as_of_date + ''''''
AND (''''NULL'''' = ''''NULL'''' OR s.source_deal_header_id IN (NULL)) ''
+		
CASE 
...
DECLARE @_sql1 VARCHAR(max), @_sql2 VARCHAR(max), @_sql3 VARCHAR(max)
...
--END
CASE 
	WHEN @_term_start_from IS NULL AND @_term_start_to IS NULL THEN '' AND hb.term_date > '''''' + @_as_of_date + '''''' ''
...
IF (@_block_type_group_id IS NOT NULL)	
	SET @_sql = ''INSERT INTO #temp_block_type_group_table(block_type_group_id, block_type_id, block_name, hourly_block_id)
	...
	ELSE 
	SET @_sql =''INSERT INTO #temp_block_type_group_table(block_type_group_id, block_type_id, block_name, hourly_block_id)
				SELECT NULL block_type_group_id, NULL block_type_id, ''''Baseload'''' block_name, 300501 hourly_block_id''
								
PRINT (@_sql)				
EXEC (@_sql + '' OPTION (maxdop 1)'')					

CREATE INDEX ix_pt_tpt on #temp_position_table(expiration_date,location_id, curve_id, fas_book_id) INCLUDE (source_system_book_id1,source_system_book_id2,source_system_book_id3,source_system_book_id4)
CREATE INDEX ix_pt_block on #temp_block_type_group_table(hourly_block_id,block_type_id,block_type_group_id) INCLUDE (block_name)
...
INNER JOIN source_price_curve_def spcd WITH (NOLOCK) ON spcd.source_curve_def_id = vw.curve_id  
...
LEFT JOIN  hour_block_term hb WITH (NOLOCK)  ON hb.block_define_id=COALESCE(grp.hourly_block_id,300501) AND  hb.block_type=COALESCE(grp.block_type_id,12000) and vw.term_start=hb.term_date
...
LEFT JOIN source_commodity com  WITH (NOLOCK) ON com.source_commodity_id=spcd.commodity_id 
...
LEFT JOIN source_deal_header sdh WITH (NOLOCK) ON sdh.source_deal_header_id = vw.source_deal_header_id
LEFT JOIN static_data_value sdv_deal_staus WITH (NOLOCK) ON sdv_deal_staus.value_id = vw.deal_status_id
...
WHERE vw.expiration_date > @_as_of_date AND vw.term_start > @_as_of_date
OPTION (maxrecursion 32767)

IF OBJECT_ID(N''tempdb..#final_temp_position_value'') IS NOT NULL
	DROP TABLE #final_temp_position_value
 
 CREATE INDEX ix_pt_test1 ON #temp_hourly_position 
 (deal_date, source_deal_header_id) INCLUDE
...
SELECT as_of_date,
       ...
       index_id AS [curve_id],
		 CASE WHEN unpvt.Hours_from=25 
			THEN dst.[hour] 
			ELSE unpvt.Hours_from 
		 END Hours,
		((CASE WHEN deal_date <=@_as_of_date THEN unpvt.volume_from ELSE 0 END) - CASE 
			WHEN dst.[hour] is not null and unpvt.Hours_from=dst.[hour] 
			THEN isnull(unpvt.dst_25,0) 
			ELSE 0 
		END) prior_volume,
		(CASE 
			WHEN term_start > @_to_as_of_date AND expiration_date > @_to_as_of_date 
		...
		(CASE 
			...
			ELSE 0 
		END) volume,
		  CASE WHEN unpvt.Hours_from=25 THEN 1 ELSE 0 END is_dst,
		  sub_book_id,proxy_curve_id3,		 
		commodity_id,
		...
		(CASE 
			...
			THEN isnull(unpvt.dst_25,0) 
			ELSE 0 
		END) -  ((CASE WHEN deal_date <=@_as_of_date THEN unpvt.volume_from ELSE 0 END) - CASE 
			WHEN dst.[hour] is not null and unpvt.Hours_from=dst.[hour] 
			THEN isnull(unpvt.dst_25,0) 
			ELSE 0 
		END) volume_change
FROM #temp_hourly_position thp
UNPIVOT (volume_from FOR Hours_from IN (thp.[01] , thp.[02] , thp.[03] , thp.[04] , thp.[05] , thp.[06] , thp.[07] 
...
WHERE (unpvt.Hours_from<25 or (unpvt.Hours_from=25 and dst.[hour] is not null))
AND 1 = CASE @_invalid_date_range WHEN 0 THEN 1 ELSE 2 END', report_id = @report_id_data_source_dest 
		WHERE [name] = 'Position View Hourly'
			AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1)
	END	
	...
	BEGIN
		...
		SELECT TOP 1 1 AS [type_id], 'Position View Hourly' AS [name], 'PVH' AS ALIAS, '' AS [description],'IF OBJECT_ID(N''tempdb..#books'') IS NOT NULL
	DROP TABLE #books
IF OBJECT_ID(N''tempdb..#term_date'') IS NOT NULL
	DROP TABLE #term_date
...
LEFT JOIN mv90_dst dst ON dst.[date] = unpvt.term_start and insert_delete = ''i'' 
WHERE (unpvt.Hours_from<25 or (unpvt.Hours_from=25 and dst.[hour] is not null))
AND 1 = CASE @_invalid_date_range WHEN 0 THEN 1 ELSE 2 END' AS [tsql], @report_id_data_source_dest AS report_id
	END 
	...
	IF EXISTS (SELECT 1 
	           ...
	           WHERE ds.[name] = 'Position View Hourly'
