...
/*************************************View: 'Settlement PNL View' START*************************************/
BEGIN TRY
		BEGIN TRAN
	

	declare @new_ds_alias varchar(10) = 'SPNVN'
	/** IF DATA SOURCE ALIAS ALREADY EXISTS ON DESTINATION, RAISE ERROR **/
	if exists(select top 1 1 from data_source where alias = 'SPNVN' and name <> 'Settlement PNL View')
	begin
		select top 1 @new_ds_alias = 'SPNVN' + cast(s.n as varchar(5))
		from seq s
		left join data_source ds on ds.alias = 'SPNVN' + cast(s.n as varchar(5))
	...
	IF NOT EXISTS (SELECT 1 
	           FROM data_source 
	           WHERE [name] = 'Settlement PNL View'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
	AND NOT EXISTS (SELECT 1 FROM map_function_category WHERE [function_name] = 'Settlement PNL View' AND '106500' = '106501') 
	BEGIN
		INSERT INTO data_source([type_id], [name], [alias], [description], [tsql], report_id, system_defined,category)
		SELECT TOP 1 1 AS [type_id], 'Settlement PNL View' AS [name], @new_ds_alias AS ALIAS, '' AS [description],null AS [tsql], @report_id_data_source_dest AS report_id,'0' AS [system_defined]
			,'106500' AS [category]
	END
...
DECLARE @_accounting_month VARCHAR(10) 

DECLARE @_sub_id VARCHAR(1000) = NULL
...
SET @_accrual_or_final = NULLIF( isnull(@_accrual_or_final,NULLIF(''@accrual_or_final'', replace(''@_accrual_or_final'',''@_'',''@''))),''null'')

SET @_accounting_month = NULLIF( isnull(@_accounting_month,NULLIF(''@accounting_month'', replace(''@_accounting_month'',''@_'',''@''))),''null'')



--SET @_as_of_date =''2019-5-01''

--SET @_prod_date_from = ''2019-03-01''

--SET @_prod_date_to = ''2019-04-30''

--SET @_source_deal_header_id = ''48533''

--SET @_counterparty_id =''9130''

-- select * from source_counterparty where counterparty_name like ''%shell%''

-- select * from stmt_checkout where source_deal_detail_id=274419 and accrual_or_final=''f''

-- select * from vwIndexFeesBreakdownStmt where index_fees_id=23329
...
CREATE TABLE #books (
       ...
       IF @_sub_id IS NOT NULL   
              ...
              SET @_sql = @_sql + '' AND stra.parent_entity_id IN  ( ''+ @_sub_id + '') ''              

       IF @_stra_id IS NOT NULL   

              SET @_sql = @_sql + '' AND (stra.entity_id IN(''  + @_stra_id + '' ))''           

       IF @_book_id IS NOT NULL   

              SET @_sql = @_sql + '' AND (book.entity_id IN(''   + @_book_id + '')) ''   

       IF @_sub_book_id IS NOT NULL

              SET @_sql = @_sql + '' AND ssbm.book_deal_type_map_id IN ('' + @_sub_book_id + '' ) ''
...
SELECT 
	...
	MAX(bk.sub_book_id) sub_book_id,

	MAX(bk.book_id) book_id, 

	MAX(bk.stra_id) stra_id,

	MAX(bk.sub_id) sub_id,

	MAX(bk.sub_book) sub_book,

	MAX(bk.book) book, 

	MAX(bk.strategy) strategy, 

	MAX(bk.subsidiary) subsidiary,	

	MAX(sco.as_of_date) [as_of_date],

	MAX(sco.as_of_date) [to_as_of_date],

	MAX(sco.term_start) [prod_date_from],

	MAX(sco.term_end) [prod_date_to],
	...
	sco.field_id [charge_type_id],

	SUM(sco.volume) [volume],

	MAX(su.uom_name) [uom],

	SUM(sco.value)  [amount],

	MAX(cur.currency_name) [currency],

	CAST(dbo.FNARemoveTrailingZeroes(ROUND(MAX(ISNULL(sco.price,ABS(sco.value/NULLIF(sco.volume,0)))), 4)) AS DECIMAL(10,4))  [price], 
	...
	MAX(adgc2.debit_gl_number) [debit_gl_no.],

	MAX(adgc2.credit_gl_number) [credit_gl_no.],
	...
	sdh.deal_id [deal_reference_id],

	MAX(si.accounting_month) accounting_month

--[__batch_report__]

FROM vwIndexFeesBreakdownStmt sco

OUTER APPLY (SELECT MAX(as_of_date) mx_as_of_date FROM vwIndexFeesBreakdownStmt sco1 

				WHERE sco.source_deal_detail_id = sco1.source_deal_detail_id

					AND sco.field_id = sco1.field_id

					AND sco.term_start = sco1.term_start

			) sco1

INNER JOIN source_deal_header sdh ON sdh.source_deal_header_id = sco.source_deal_header_id
...
INNER JOIN source_counterparty sc_si ON sc_si.source_counterparty_id = sdh.counterparty_id

OUTER APPLY(SELECT si.invoice_number,si.invoice_date,si.invoice_type,si.payment_date,si.invoice_status,si.is_voided,si.is_finalized,sc.accrual_or_final,sc.accounting_month FROM stmt_checkout sc INNER JOIN  stmt_invoice_detail sid ON sc.stmt_invoice_detail_id = sid.stmt_invoice_detail_id INNER  JOIN stmt_invoice si ON sid.stmt_invoice_id = si.stmt_invoice_id WHERE index_fees_id = CASE WHEN sco.field_id=-5500 THEN sco.source_deal_settlement_id ELSE sco.index_fees_id END AND sc.accrual_or_final=''''f'''') si

LEFT JOIN static_data_value charge_type ON charge_type.value_id = sco.field_id
...
SET @_sql1 = '' OUTER APPLY (SELECT MAX(acm.stmt_account_code_mapping_id) stmt_account_code_mapping_id FROM stmt_account_code_mapping acm 

		WHERE ISNULL(acm.buy_sell_flag,sdd.buy_sell_flag) = sdd.buy_sell_flag
		...
		AND COALESCE(acm.counterparty_type,sc.int_ext_flag,''''e'''') = ISNULL(sc.int_ext_flag,''''e'''')

		) acm

LEFT JOIN stmt_account_code_chargetype acc ON acc.stmt_account_code_mapping_id = acm.stmt_account_code_mapping_id

		AND (acc.deal_charge_type_id = sco.field_id OR acc.contract_charge_type_id = sco.Contract_Charge_Type_ID)
...
LEFT JOIN adjustment_default_gl_codes adgc ON adgc.default_gl_id = CASE WHEN ISNULL(si.accrual_or_final,''''a'''') = ''''f'''' THEN acg.final_gl ELSE acg.estimate_gl END

LEFT JOIN gl_system_mapping gsm ON gsm.gl_number_id = CASE WHEN sco.value >= 0 THEN adgc.debit_gl_number ELSE adgc.debit_gl_number_minus END 

LEFT JOIN gl_system_mapping gsm1 ON gsm1.gl_number_id = CASE WHEN sco.value >= 0 THEN adgc.credit_gl_number ELSE adgc.credit_gl_number_minus END 
...
WHERE 1 = 1 AND sco.as_of_date = sco1.mx_as_of_date '' +
...
CASE WHEN @_charge_type_id IS NULL THEN '''' ELSE '' AND sco.field_id IN('' + @_charge_type_id + '') '' END +
...
CASE WHEN @_accounting_month IS NULL THEN '''' ELSE '' AND ISNULL(CONVERT(VARCHAR(10),si.accounting_month,120),''''''+@_accounting_month+'''''')=''''''+@_accounting_month+'''''''' END 

+ '' GROUP BY sco.term_start, si.invoice_date, acc.invoicing_charge_type_id, si.invoice_number, sdh.deal_id, si.invoice_type,sdd.Leg, cg.contract_name,si.accrual_or_final,

sco.field_id,sco.as_of_date''
...
EXEC (@_sql+@_sql1)', report_id = @report_id_data_source_dest,
	...
	IF EXISTS (SELECT 1 
	           ...
	           WHERE ds.[name] = 'Settlement PNL View'
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'credit_gl_no.' AS [name], 'Credit Gl No.' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 4 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Settlement PNL View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'debit_gl_no.' AS [name], 'Debit Gl No.' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 4 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Settlement PNL View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'deal_reference_id' AS [name], 'Deal Reference Id' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 5 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,0 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Settlement PNL View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
