BEGIN TRY
	...
	declare @new_ds_alias varchar(10) = 'MSDV'
	/** IF DATA SOURCE ALIAS ALREADY EXISTS ON DESTINATION, RAISE ERROR **/
	if exists(select top 1 1 from data_source where alias = 'MSDV' and name <> 'MTM Settlement Detail View')
	begin
		select top 1 @new_ds_alias = 'MSDV' + cast(s.n as varchar(5))
		from seq s
		left join data_source ds on ds.alias = 'MSDV' + cast(s.n as varchar(5))
	...
	IF NOT EXISTS (SELECT 1 
	           FROM data_source 
	           WHERE [name] = 'MTM Settlement Detail View'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
	AND NOT EXISTS (SELECT 1 FROM map_function_category WHERE [function_name] = 'MTM Settlement Detail View' AND '106500' = '106501') 
	BEGIN
		INSERT INTO data_source([type_id], [name], [alias], [description], [tsql], report_id, system_defined,category)
		SELECT TOP 1 1 AS [type_id], 'MTM Settlement Detail View' AS [name], @new_ds_alias AS ALIAS, '' AS [description],null AS [tsql], @report_id_data_source_dest AS report_id,'0' AS [system_defined]
			,'106500' AS [category]
	END

	UPDATE data_source
	SET alias = @new_ds_alias, description = ''
	, [tsql] = CAST('' AS VARCHAR(MAX)) + '--settlement mega view by tou
...
DECLARE @_from_as_of_date VARCHAR(50)	--=''2019-03-04''
DECLARE @_to_as_of_date VARCHAR(50)		--=''2020-06-20''
DECLARE @_term_start VARCHAR(50)		--=''2017-01-01''
DECLARE @_term_end VARCHAR(50)			--=''2020-12-31''
DECLARE @_deal_status_group CHAR(1)
DECLARE @_deal_date_from VARCHAR(50), @_deal_date_to VARCHAR(50), @_source_counterparty_id VARCHAR(max), @_source_deal_type_id VARCHAR(max), @_deal_sub_type_type_id VARCHAR(max), @_physical_financial_flag VARCHAR(20), @_contract_id VARCHAR(max), @_trader_id VARCHAR(max), @_block_definition_id VARCHAR(50), @_location_id VARCHAR(max), @_index_id VARCHAR(max), @_source_deal_header_id VARCHAR(50),
	--=''82077'' ,
	@_commodity_id VARCHAR(max), @_period_from VARCHAR(50), @_period_to VARCHAR(50), @_buy_sell_flag VARCHAR(20), @_deal_ref_id VARCHAR(50), @_deal_status_id VARCHAR(50), @_confirm_status_id VARCHAR(50), @_pnl_source_value_id VARCHAR(50), @_internal_portfolio_id VARCHAR(max)
...
SET @_internal_portfolio_id = nullif(isnull(@_internal_portfolio_id, nullif(''@internal_portfolio_id'', replace(''@_internal_portfolio_id'', ''@_'', ''@''))), ''null'')
...
WHERE 1 = 1 
--AND (''@sub_id'' = ''NULL'' OR sub.entity_id IN (@sub_id)) AND (''@stra_id'' = ''NULL'' OR stra.entity_id IN (@stra_id)) AND (''@book_id'' = ''NULL'' OR book.entity_id IN (@book_id)) AND (''@sub_book_id'' = ''NULL'' OR ssbm.book_deal_type_map_id IN (@sub_book_id))
IF ''@from_as_of_date'' = ''NULL'' AND ''@to_as_of_date'' <> ''NULL''
	SELECT @_from_as_of_date = Max(CONVERT(CHAR(10), sdp.pnl_as_of_date, 126))
	FROM dbo.source_deal_pnl_tou sdp
...
CREATE TABLE #final_values (
	sub_id INT, stra_id INT, book_id INT, subsidiary VARCHAR(500) COLLATE database_default NULL, strategy VARCHAR(500) COLLATE database_default NULL, book VARCHAR(500) COLLATE database_default NULL, sub_book VARCHAR(500) COLLATE database_default NULL, source_deal_header_id INT, deal_ref_id VARCHAR(500) COLLATE database_default NULL, deal_date_from DATETIME, deal_date_to DATETIME, trader_id INT, trader_name VARCHAR(500) COLLATE database_default NULL, counterparty_name VARCHAR(500), parent_counterparty_name VARCHAR(500), contract_id INT, contract_name VARCHAR(500) COLLATE database_default NULL, source_deal_type_id INT, deal_type_name VARCHAR(500) COLLATE database_default NULL, deal_sub_type_type_id INT, deal_sub_type_name VARCHAR(500) COLLATE database_default NULL, product_id INT, commodity_id INT, product_desc VARCHAR(500) COLLATE database_default NULL, block_definition VARCHAR(500) COLLATE database_default NULL, buy_sell_flag VARCHAR(500) COLLATE database_default NULL, term_start DATETIME, term_end DATETIME, country_id INT, country VARCHAR(500) COLLATE database_default NULL, grid 
	VARCHAR(500) COLLATE database_default NULL, region_id INT, region VARCHAR(500) COLLATE database_default NULL, source_major_location_id INT, location_group VARCHAR(500) COLLATE database_default NULL, location_id INT, location_name VARCHAR(500) COLLATE database_default NULL, location_description VARCHAR(500) COLLATE database_default NULL, from_as_of_date DATETIME, amount FLOAT, und_intrinsic_pnl FLOAT, und_extrinsic_pnl FLOAT, dis_pnl FLOAT, dis_intrinsic_pnl FLOAT, dis_extrinisic_pnl FLOAT, market_value FLOAT, contract_value FLOAT, dis_market_value FLOAT, dis_contract_value FLOAT, discount_factor FLOAT, discount_amount FLOAT, commodity VARCHAR(500) COLLATE database_default NULL, deal_volume FLOAT, volume_uom VARCHAR(500) COLLATE database_default NULL, template_name VARCHAR(500) COLLATE database_default NULL, curve_name VARCHAR(500) COLLATE database_default NULL, index_tou VARCHAR(500) COLLATE database_default NULL, currency VARCHAR(500) COLLATE database_default NULL, formula VARCHAR(500) COLLATE database_default NULL, total_volume FLOAT, position_uom VARCHAR(500) COLLATE 
	database_default NULL, cva FLOAT, dva FLOAT, market_price FLOAT, formula_price FLOAT, net_price FLOAT, physical_financial_flag VARCHAR(10) COLLATE database_default NULL, block_definition_id INT, leg INT, source_counterparty_id INT, counterparty_id VARCHAR(500), index_id INT, credit_adjusted_mtm FLOAT, discounted_credit_adjusted_mtm FLOAT, term_start_day INT, term_start_month VARCHAR(500) COLLATE database_default NULL, term_start_month_name VARCHAR(500), term_start_year INT, term_start_year_month VARCHAR(500) COLLATE database_default NULL, term_quarter VARCHAR(500) COLLATE database_default NULL, to_as_of_date DATETIME, sub_book_id INT, transaction_type INT, transaction_type_name VARCHAR(500) COLLATE database_default NULL, period_from VARCHAR(500) COLLATE database_default NULL, period_to VARCHAR(500) COLLATE database_default NULL, probability FLOAT, credit_reserve FLOAT, dis_credit_reserve FLOAT, fair_value FLOAT, dis_fair_value FLOAT, model_reserve INT, actual_forward CHAR(1) COLLATE database_default NULL, pnl_conversion_factor FLOAT, pnl_adjustment_value FLOAT, 
	internal_deal_type_id INT, internal_deal_type VARCHAR(500) COLLATE database_default NULL, internal_deal_sub_type_id INT, internal_deal_sub_type VARCHAR(500) COLLATE database_default NULL, fixed_cost FLOAT, fixed_price FLOAT, formula_value FLOAT, price_adder FLOAT, price_multiplier FLOAT, strike_price FLOAT, und_pnl_set FLOAT, formula_curve_id INT, formula_curve_name VARCHAR(500) COLLATE database_default NULL, internal_desk_id INT, internal_desk VARCHAR(500) COLLATE database_default NULL, internal_portfolio_id INT, internal_portfolio VARCHAR(500) COLLATE database_default NULL, broker_id INT, broker_name VARCHAR(500), type_of_entity_id INT, type_of_entity VARCHAR(500) COLLATE database_default NULL, int_ext_flag_id CHAR, int_ext_flag VARCHAR(500) COLLATE database_default NULL, deal_status_id INT, deal_status VARCHAR(500) COLLATE database_default NULL, confirm_status_id INT, confirm_status VARCHAR(500) COLLATE database_default NULL, pnl_source_value_id INT, pnl_source_name VARCHAR(500) COLLATE database_default NULL, description1 VARCHAR(500) COLLATE database_default NULL, 
	description2 VARCHAR(500) COLLATE database_default NULL, description3 VARCHAR(500) COLLATE database_default NULL, description4 VARCHAR(500) COLLATE database_default NULL, counterparty_id2 INT, counterparty_name2 VARCHAR(500), ext_deal_id VARCHAR(500) COLLATE database_default NULL, reference VARCHAR(500) COLLATE database_default NULL, create_by VARCHAR(500) COLLATE database_default NULL, create_ts DATETIME, update_by VARCHAR(500) COLLATE database_default NULL, update_ts DATETIME, broker_fixed_cost NUMERIC, confirm_date DATETIME, sub_book_group1_id INT, sub_book_group2_id INT, sub_book_group3_id INT, sub_book_group4_id INT, sub_book_group1 VARCHAR(500) COLLATE database_default NULL, sub_book_group2 VARCHAR(500) COLLATE database_default NULL, sub_book_group3 VARCHAR(500) COLLATE database_default NULL, sub_book_group4 VARCHAR(500) COLLATE database_default NULL, book_identifier1_id INT, book_identifier2_id INT, book_identifier3_id INT, book_identifier4_id INT, book_identifier1 VARCHAR(500) COLLATE database_default NULL, book_identifier2 VARCHAR(500) COLLATE database_default 
	NULL, book_identifier3 VARCHAR(500) COLLATE database_default NULL, book_identifier4 VARCHAR(500) COLLATE database_default NULL, deal_price FLOAT, float_price FLOAT, volume FLOAT, close_reference_id INT, charge_type_id INT, charge_type VARCHAR(500) COLLATE database_default NULL, agg_term VARCHAR(500) COLLATE database_default NULL, recovery_rate FLOAT, fv_level VARCHAR(500) COLLATE database_default NULL, current_non_current VARCHAR(500) COLLATE database_default NULL, deal_category_id INT, deal_category VARCHAR(500) COLLATE database_default NULL, internal_counterparty_id INT, internal_couterparty VARCHAR(500) COLLATE database_default NULL, template_id INT, volume_profile VARCHAR(500) COLLATE database_default NULL, price_adder2 FLOAT, volume_multiplier FLOAT, volume_multiplier2 FLOAT, debt_rating VARCHAR(500) COLLATE database_default NULL, volume_frequency VARCHAR(500) COLLATE database_default NULL, currency_id INT, is_expired VARCHAR(10) COLLATE database_default NULL, entire_term_start DATETIME, entire_term_end DATETIME, contract_exipration_date DATETIME, structured_id 
	VARCHAR(500) COLLATE database_default NULL, payment_date DATETIME, pricing_type INT, must_run_volume FLOAT, dispatch_volume FLOAT, must_run_market_value FLOAT, must_run_contract_value FLOAT, dispatch_market_value FLOAT, dispatch_contract_value FLOAT, cash_flow FLOAT, pnl_date DATETIME, tou_id INT, tou_name VARCHAR(200), mw_position DECIMAL(36,16), option_excercise_type CHAR(1), option_type CHAR(1), strike_granularity INT,
	premium FLOAT, delta FLOAT, gamma FLOAT, vega FLOAT, theta FLOAT, rho FLOAT, delta2 FLOAT, market_value_id VARCHAR(100), market_value_desc VARCHAR(100), granularity VARCHAR(100),
	exp_calendar_id INT, exp_calendar VARCHAR(100), Holiday_calendar_id INT, Holiday_calendar VARCHAR(100), proxy_source_curve_def_id INT, proxy_curve VARCHAR(100), settlement_curve_id INT, settlement_curve VARCHAR(100), 
	Block_define_id INT, Block_name VARCHAR(100), udf_block_group_id INT, block_group_name VARCHAR(100)
	)
SET @_sql_select1 = 
	''INSERT INTO #final_values
SELECT DISTINCT books.sub_id sub_id,
...
ISNULL(sdpt.pnl_as_of_date, sdpd.pnl_as_of_date) from_as_of_date,
CAST(COALESCE(sdpt.und_pnl, sdpd.und_pnl, sdst.settlement_amount, 0) AS FLOAT) amount,
CAST(COALESCE(sdpt.und_intrinsic_pnl, sdpd.und_intrinsic_pnl, 0) AS FLOAT) und_intrinsic_pnl,
CAST(COALESCE(sdpt.und_extrinsic_pnl, sdpd.und_extrinsic_pnl, 0) AS FLOAT) und_extrinsic_pnl,
CAST(COALESCE(sdpt.dis_pnl, sdpd.dis_pnl, 0) AS FLOAT) dis_pnl,
CAST(COALESCE(sdpt.dis_intrinsic_pnl, sdpd.dis_intrinsic_pnl, 0) AS FLOAT) dis_intrinsic_pnl,
CAST(COALESCE(sdpt.dis_extrinisic_pnl, sdpd.dis_extrinisic_pnl,0) AS FLOAT) dis_extrinisic_pnl,
CAST(COALESCE(sdpt.market_value, sdpd.market_value, 0) AS FLOAT) market_value,
CAST(COALESCE(sdpt.contract_value, sdpd.contract_value, sdst.contract_value,0) AS FLOAT) contract_value,
CAST(COALESCE(sdpt.dis_market_value, sdpd.dis_market_value, 0) AS FLOAT) dis_market_value,
CAST(COALESCE(sdpt.dis_contract_value, sdpd.dis_contract_value, 0) AS FLOAT) dis_contract_value,
CAST(COALESCE(sdpt.discount_factor, sdpd.discount_factor, 0) AS FLOAT) discount_factor,
COALESCE(sdpt.und_pnl, sdpd.und_pnl, 0) - COALESCE(sdpt.dis_pnl, sdpd.dis_pnl, 0) [discount_amount],
...
fe.formula [formula],
COALESCE(sdpt.deal_volume, sdpd.deal_volume, sdst.volume, 0) [total_volume],
puom.uom_name AS [position_uom],
CAST(ISNULL(sdc.cva,0) AS FLOAT) [CVA],CAST(ISNULL(sdc.dva,0) AS FLOAT) [DVA],
CAST(COALESCE(sdpt.curve_value, sdpd.curve_value, 0) AS FLOAT) [market_price], 
CAST(COALESCE(sdpt.formula_value, sdpd.formula_value, 0) AS FLOAT) [formula_price],
CAST(COALESCE(sdpt.price, sdpd.price, sdst.net_price, 0) AS FLOAT) [net_price],
...
COALESCE(sdpt.und_pnl, sdpd.und_pnl, 0) - (COALESCE(sdpt.und_pnl, sdpd.und_pnl, 0)* ISNULL(dp.probability, 0) * (1 - ISNULL(drr.rate, 0))) [credit_adjusted_mtm],
COALESCE(sdpt.dis_pnl, sdpd.dis_pnl, 0) - (COALESCE(sdpt.dis_pnl, sdpd.dis_pnl, 0)* ISNULL(dp.probability, 0) * (1 - ISNULL(drr.rate, 0))) [discounted_credit_adjusted_mtm],
...
COALESCE(sdpt.pnl_as_of_date, sdpd.pnl_as_of_date, 0) to_as_of_date,
books.[sub_book_id], books.[transaction_type],
books.[transaction_type_name],
''''@_period_from'''' [period_from],
''''@_period_to'''' [period_to],
ISNULL(dp.probability, 0) [probability], COALESCE(sdpt.und_pnl, sdpd.und_pnl, 0) * ISNULL(dp.probability, 0) * (1 - ISNULL(drr.rate, 0)) [credit_reserve],
COALESCE(sdpt.dis_pnl, sdpd.dis_pnl, 0)* ISNULL(dp.probability, 0) * (1 - ISNULL(drr.rate, 0)) [dis_credit_reserve],
COALESCE(sdpt.und_pnl, sdpd.und_pnl, 0) - (COALESCE(sdpt.und_pnl, sdpd.und_pnl, 0)* ISNULL(dp.probability, 0) * (1 - ISNULL(drr.rate, 0))) [fair_value], 
COALESCE(sdpt.dis_pnl, sdpd.dis_pnl, 0) - (COALESCE(sdpt.dis_pnl, sdpd.dis_pnl, 0)* ISNULL(dp.probability, 0) * (1 - ISNULL(drr.rate, 0))) [dis_fair_value],
0 [model_reserve],
--CASE WHEN ISNULL(sdpt.source_deal_header_id, sdpd.source_deal_header_id) IS NULL THEN ''''a'''' ELSE ''''f'''' END AS actual_forward,
CASE WHEN sdst.term_start IS NOT NULL THEN ''''a'''' ELSE ''''f'''' END AS actual_forward,
ISNULL(sdpt.pnl_conversion_factor, sdpd.pnl_conversion_factor) [pnl_conversion_factor],
ISNULL(sdpt.pnl_adjustment_value, sdpd.pnl_adjustment_value) [pnl_adjustment_value],
ISNULL(sdpt.internal_deal_type_value_id, sdpd.internal_deal_type_value_id) [internal_deal_type_id],
idtst.internal_deal_type_subtype_type internal_deal_type,
ISNULL(sdpt.internal_deal_subtype_value_id, sdpd.internal_deal_subtype_value_id) [internal_deal_sub_type_id],
idtst1.internal_deal_type_subtype_type internal_deal_sub_type,
ISNULL(sdpt.fixed_cost, sdpd.fixed_cost) [fixed_cost],
CAST(COALESCE(sdpt.fixed_price, sdpd.fixed_price,sdd.fixed_price, 0) AS FLOAT) fixed_price,
COALESCE(sdpt.formula_value, sdpd.formula_value, 0) [formula_value],
COALESCE(sdpt.price_adder, sdpd.price_adder, sdd.price_adder) price_adder, 
COALESCE(sdpt.price_multiplier, sdpd.price_multiplier, 0) [price_multiplier], 
COALESCE(sdpt.strike_price, sdpd.strike_price, 0) [strike_price],
COALESCE(sdpt.und_pnl_set, sdpd.und_pnl_set, 0) [und_pnl_set],
...
CoALESCE(sdpt.pnl_source_value_id, sdpd.pnl_source_value_id, 4500) [pnl_source_value_id],
...
CASE WHEN COALESCE(sdpt.deal_volume, sdpd.deal_volume, sdst.volume, 0) = 0 THEN COALESCE(sdpt.contract_value, sdpd.contract_value, sdst.contract_value, 0) ELSE abs(COALESCE(sdpt.contract_value, sdpd.contract_value, sdst.contract_value, 0) / COALESCE(sdpt.deal_volume, sdpd.deal_volume, sdst.volume, 1)) END deal_price,
0 float_price,
COALESCE(sdst.volume, sdpt.deal_volume, sdpd.deal_volume) Volume,
''
SET @_sql_select2 = '' sdh.close_reference_id,
...
CASE WHEN ((DATEPART(year, sdd.term_start) - DATEPART(year, '''''' + Cast(@_from_as_of_date AS VARCHAR(100)) + '''''')) * 12 + (DATEPART(month, sdd.term_start) - DATEPART(month, '''''' + Cast(@_from_as_of_date AS VARCHAR(100)) + 
	''''''))) > 12 THEN ''''Non-Current'''' ELSE ''''Current''''
END [current_non_current],
...
ISNULL(sdpt.price_adder2, sdpd.price_adder2) [price_adder2],
ISNULL(sdpt.volume_multiplier, sdpd.volume_multiplier) [volume_multiplier],
ISNULL(sdpt.volume_multiplier2, sdpd.volume_multiplier2) [volume_multiplier2],
...
COALESCE(sdst.currency_id, sdpt.pnl_currency_id, sdpd.pnl_currency_id) currency_id,
CASE WHEN sdd.contract_expiration_date < '''''' + Cast(@_from_as_of_date AS VARCHAR(100)) + 
	'''''' THEN ''''Yes'''' ELSE ''''No'''' END is_expired,
...
sdh.pricing_type,
ISNULL(sdpt.must_run_volume, sdpd.must_run_volume) [must_run_volume],
ISNULL(sdpt.dispatch_volume, sdpd.dispatch_volume) [dispatch_volume],
ISNULL(sdpt.must_run_market_value, sdpd.must_run_market_value) [must_run_market_value],
ISNULL(sdpt.must_run_contract_value, sdpd.must_run_contract_value) [must_run_contract_value],
ISNULL(sdpt.dispatch_market_value, sdpd.dispatch_market_value) [dispatch_market_value],
ISNULL(sdpt.dispatch_contract_value, sdpd.dispatch_contract_value) [dispatch_contract_value],
CAST(ISNULL(sdpt.und_pnl_set, sdpd.und_pnl_set) AS FLOAT) cash_flow,
COALESCE(dbo.FNAInvoiceDueDate(sdd.term_start, cg.pnl_date, cg.holiday_calendar_id,10),sdd.term_end) [pnl_date],
ISNULL(sdpt.tou_id, sdst.tou_id) [tou_id],
sdv.code [tou_name],
COALESCE(sdst.mw_position, sdpt.mw_position, sdpd.mw_position) mw_position,
sdh.option_excercise_type,
sdh.option_type,
sdd.strike_granularity,
ISNULL(sdpdo.premium, 0),
ISNULL(sdpdo.delta, 0),
ISNULL(sdpdo.gamma, 0),
ISNULL(sdpdo.vega, 0),
ISNULL(sdpdo.theta, 0),
ISNULL(sdpdo.rho, 0),
ISNULL(sdpdo.delta2, 0),
spcd.market_value_id,
sdv_md.code market_value_desc,
sdv_g.code granularity,
spcd.exp_calendar_id,
sdv_ec.code exp_calendar,
spcd.holiday_calendar_id,
sdv_hc.code holiday_calendar,
spcd.proxy_source_curve_def_id,
spcd_pc.curve_id proxy_curve,
spcd.settlement_curve_id,
spcd_sc.curve_id settlement_curve,
spcd.Block_define_id,
sdv_bn.code Block_name,
spcd.udf_block_group_id,
sdv_bgn.code block_group_name
''
SET @_sql_from1 = ''
FROM source_deal_header sdh '' + CASE WHEN @_deal_status_group = ''o'' THEN '' INNER JOIN deal_status_group dsg ON dsg.status_value_id = sdh.deal_status '' WHEN @_deal_status_group = ''u'' THEN '' INNER JOIN (SELECT value_id FROM static_data_value WHERE type_id = 5600 EXCEPT SELECT status_value_id FROM deal_status_group) unofficial_status ON unofficial_status.value_id = sdh.deal_status '' ELSE '''' END + ''
INNER JOIN source_deal_detail sdd ON sdh.source_deal_header_id = sdd.source_deal_header_id
OUTER APPLY( SELECT ISNULL(sdstt.term_start, sds.term_start) term_start, 
					ISNULL(sdstt.volume, sds.volume) volume, 
					sdstt.tou_id, 
					ISNULL(sdstt.contract_value, sds.contract_value) contract_value,
					ISNULL(sdstt.net_price, sds.net_price) net_price,
					ISNULL(sdstt.settlement_amount, sds.settlement_amount) settlement_amount,
					ISNULL(sdstt.mw_position, sds.mw_position) mw_position,
					ISNULL(sdstt.settlement_currency_id, sds.settlement_currency_id) currency_id					
			 FROM source_deal_settlement_tou sdstt
			 INNER JOIN source_deal_settlement sds 
				ON sds.source_deal_header_id = sdstt.source_deal_header_id
			 WHERE sdstt.source_deal_header_id = sdh.source_deal_header_id 
				AND sds.term_start = sds.term_start
				AND sdstt.term_start < CAST('''''' + @_from_as_of_date + '''''' AS DATETIME)
				AND sdd.term_start = sdstt.term_start
                                AND MONTH(sdstt.term_start) < MONTH(CAST('''''' + @_from_as_of_date + '''''' AS DATETIME))
) sdst
...
LEFT JOIN source_deal_pnl_tou sdpt ON sdpt.source_deal_header_id = sdh.source_deal_header_id
	AND '' + CASE WHEN @_to_as_of_date IS NULL THEN ''sdpt.pnl_as_of_date = CAST('''''' + @_from_as_of_date + '''''' AS DATETIME)'' ELSE ''sdpt.pnl_as_of_date BETWEEN CAST('''''' + @_from_as_of_date + '''''' AS DATETIME) AND CAST('''''' + @_to_as_of_date + '''''' AS DATETIME) '' END + CASE WHEN @_term_start IS NULL AND @_term_end IS NULL THEN '''' WHEN @_term_end IS NULL THEN '' AND sdpt.term_start >= CAST('''''' + Cast(@_term_start AS VARCHAR(100)) + '''''' AS DATETIME)'' WHEN @_term_start IS NULL 
				THEN '' AND sdpt.term_start <= CAST('''''' + Cast(@_term_end AS VARCHAR(100)) + '''''' AS DATETIME)'' ELSE '' AND sdpt.term_start BETWEEN CAST('''''' + Cast(@_term_start AS VARCHAR(100)) + '''''' as DATETIME)
	AND CAST('''''' + Cast(@_term_end AS VARCHAR(100)) + ''''''as DATETIME)'' END + 
		'' AND sdd.term_start =sdpt.term_start
	AND sdd.term_end =sdpt.term_end
	AND sdpt.Leg = sdd.leg
	--and sdst.tou_id = sdpt.tou_id
LEFT JOIN source_deal_pnl_detail_options sdpdo ON sdpdo.source_deal_header_id = sdh.source_deal_header_id
	AND '' + CASE WHEN @_to_as_of_date IS NULL THEN ''sdpdo.as_of_date = CAST('''''' + @_from_as_of_date + '''''' AS DATETIME)'' ELSE ''sdpdo.as_of_date BETWEEN CAST('''''' + @_from_as_of_date + '''''' AS DATETIME) AND CAST('''''' + @_to_as_of_date + '''''' AS DATETIME) '' END + CASE WHEN @_term_start IS NULL AND @_term_end IS NULL THEN '''' WHEN @_term_end IS NULL THEN '' AND sdpdo.term_start >= CAST('''''' + Cast(@_term_start AS VARCHAR(100)) + '''''' AS DATETIME)'' WHEN @_term_start IS NULL 
			THEN '' AND sdpdo.term_start <= CAST('''''' + Cast(@_term_end AS VARCHAR(100)) + '''''' AS DATETIME)'' ELSE '' AND sdpdo.term_start BETWEEN CAST('''''' + Cast(@_term_start AS VARCHAR(100)) + '''''' as DATETIME)
	AND CAST('''''' + Cast(@_term_end AS VARCHAR(100)) + ''''''as DATETIME)'' END + ''
	AND sdd.term_start = sdpdo.term_start
OUTER APPLY( SELECT sdv.code
			 FROM static_data_value sdv
			 INNER JOIN static_data_type sdt
				 ON sdt.type_id = sdv.type_id
			 WHERE ISNULL(sdpt.tou_id, sdst.tou_id) = sdv.value_id
				 AND sdt.type_name = ''''Hourly Block''''	
) sdv
INNER JOIN #books books ON books.source_system_book_id1 = sdh.source_system_book_id1 AND
books.source_system_book_id2 = sdh.source_system_book_id2 AND
books.source_system_book_id3 = sdh.source_system_book_id3 AND
books.source_system_book_id4 = sdh.source_system_book_id4
...
LEFT JOIN source_price_curve_def AS spcd_pc ON spcd_pc.source_curve_def_id = spcd.proxy_source_curve_def_id
LEFT JOIN source_price_curve_def AS spcd_sc ON spcd_sc.source_curve_def_id = spcd.settlement_curve_id
LEFT JOIN static_data_value sdv_g ON sdv_g.value_id = spcd.granularity
LEFT JOIN static_data_value sdv_bn ON sdv_bn.value_id = spcd.block_define_id
LEFT JOIN static_data_value sdv_bgn ON sdv_bgn.value_id = spcd.udf_block_group_id
LEFT JOIN static_data_value sdv_hc ON sdv_hc.value_id = spcd.holiday_calendar_id
LEFT JOIN static_data_value sdv_ec ON sdv_ec.value_id = spcd.exp_calendar_id
LEFT JOIN static_data_value sdv_md ON sdv_md.value_id = spcd.market_value_desc
LEFT JOIN source_price_curve_def AS formula_curve ON formula_curve.source_curve_def_id = sdd.formula_curve_id
LEFT JOIN internal_deal_type_subtype_types idtst ON idtst.internal_deal_type_subtype_id = ISNULL(sdpt.internal_deal_type_value_id, sdpd.internal_deal_type_value_id)
LEFT JOIN internal_deal_type_subtype_types idtst1 ON idtst1.internal_deal_type_subtype_id = ISNULL(sdpt.internal_deal_subtype_value_id, sdpd.internal_deal_subtype_value_id)
...
LEFT JOIN static_data_value sdv_ctgy ON sdv_ctgy.value_id = sdh.deal_category_value_id
...
LEFT JOIN static_data_value pnl_sour on pnl_sour.value_id = COALESCE(sdpt.pnl_source_value_id, sdpd.pnl_source_value_id, 4500)
LEFT JOIN source_deal_header_template sdht ON sdh.template_id = sdht.template_id
LEFT JOIN source_commodity sc2 ON sc2.source_commodity_id = ISNULL(sdh.commodity_id,spcd.commodity_id)
...
LEFT JOIN counterparty_contract_address cca On cca.counterparty_id = sdh.counterparty_id AND cca.contract_id = cg.contract_id OUTER APPLY( SELECT probability FROM default_probability
WHERE id IN (SELECT MAX(id) FROM default_probability WHERE effective_date <= '''''' 
	+ Cast(@_from_as_of_date AS VARCHAR(100)) + '''''' AND debt_rating = cci.Risk_rating
)
) dp
OUTER APPLY (
...
OUTER APPLY (
			...
			WHERE sdc.Source_Counterparty_ID = sdh.counterparty_id
				AND sdc.source_deal_header_id = ISNULL(sdpt.source_deal_header_id, sdpd.source_deal_header_id)
				AND sdc.as_of_date = ISNULL(sdpt.pnl_as_of_date, sdpd.pnl_as_of_date)
...
OUTER APPLY(SELECT TOP 1 CASE WHEN MONTH(sdd.term_start) < MONTH('''''' + @_from_as_of_date + '''''')
								AND YEAR(sdd.term_start)<= YEAR('''''' + @_from_as_of_date + '''''') 
							  ...
							  WHEN DATEPART(q,'''''' + @_from_as_of_date + '''''') = DATEPART(q,sdd.term_start)
								AND YEAR('''''' + @_from_as_of_date + '''''') = YEAR(sdd.term_start) 
									THEN convert(varchar(4),sdd.term_start,120) +''''-''''+ ''''M'''' + CAST(DATEDIFF(m,'''''' + @_from_as_of_date + '''''',sdd.term_start) AS VARCHAR) +'''' ''''+ ''''('''' + UPPER(LEFT(DATENAME(MONTH,dateadd(MONTH, MONTH(sdd.term_start),-1)),3)) + '''')'''' WHEN YEAR('''''' + @_from_as_of_date + '''''') = YEAR(sdd.term_start) THEN convert(varchar(4),sdd.term_start,120) + ''''-''''+ ''''Q'''' + CAST(DATEPART(q,sdd.term_start) AS VARCHAR)
							  ELSE CAST(YEAR(sdd.term_start) AS VARCHAR)
						END agg_term
			FROM portfolio_mapping_tenor
) ag_t ''
SET @_sql_from4 = 
	''
OUTER APPLY ( SELECT MAX(effective_date) effective_date FROM fv_report_group_deal
			  WHERE source_deal_header_id = ISNULL(sdpt.source_deal_header_id, sdpd.source_deal_header_id) 
			  AND ISNULL(sdpt.term_start, sdpd.term_start)  = term_start 
			  AND effective_date <= ISNULL(sdpt.Pnl_as_of_date, sdpd.Pnl_as_of_date)
) fv_d_mx
OUTER APPLY ( 
			SELECT fv_level_value_id ,MAX(effective_date) effective_date FROM fv_report_group_deal
			WHERE source_deal_header_id = ISNULL(sdpt.source_deal_header_id, sdpd.source_deal_header_id) 
				AND ISNULL(sdpt.term_start, sdpd.term_start) = term_start 
...
OUTER APPLY( SELECT MAX(effective_date) effective_date 
			 ...
			 WHERE source_curve_def_id = ISNULL(sdpt.curve_id, sdpd.curve_id) 
				AND ISNULL(sdpt.term_start, sdpd.term_start) BETWEEN DATEADD(MM, from_no_of_months, effective_date) AND DATEADD(MM, to_no_of_months, effective_date)
				AND effective_date <= ISNULL(sdpt.Pnl_as_of_date, sdpd.Pnl_as_of_date)
) P_fv_mx
OUTER APPLY ( SELECT fv_reporting_group_id, MAX(effective_date) effective_date 
			  FROM price_curve_fv_mapping
			  WHERE source_curve_def_id = ISNULL(sdpt.curve_id, sdpd.curve_id) 
				  AND ISNULL(sdpt.term_start, sdpd.term_start) BETWEEN DATEADD(MM, from_no_of_months, effective_date) AND DATEADD(MM, to_no_of_months, effective_date) 
...
LEFT JOIN source_currency scur ON scur.source_currency_id = COALESCE(sdst.currency_id, sdpt.pnl_currency_id, sdpd.pnl_currency_id)
...
--SET @_sql_whr = '' WHERE 1 = 1 AND sdst.term_start = sdd.term_start'' + CASE WHEN @_deal_date_from IS NULL AND @_deal_date_to IS NULL THEN '''' WHEN @_deal_date_to IS NULL THEN '' AND sdh.deal_date = CAST('''''' + @_deal_date_from + '''''' AS DATETIME)'' WHEN @_deal_date_from IS NULL THEN '' AND sdh.deal_date <= CAST('''''' + @_deal_date_to + '''''' AS DATETIME)'' ELSE '' AND sdh.deal_date BETWEEN CAST('''''' + @_deal_date_from + '''''' AS DATETIME) AND CAST('''''' + @_deal_date_to + '''''' AS DATETIME)'' END + CASE WHEN @_source_counterparty_id IS NULL THEN '''' ELSE '' AND sdh.counterparty_id IN ('' + @_source_counterparty_id + '')'' END + CASE WHEN @_source_deal_type_id IS NULL THEN '''' ELSE '' AND sdh.source_deal_type_id IN ('' + @_source_deal_type_id + '')'' END + CASE WHEN @_deal_sub_type_type_id IS NULL THEN '''' ELSE '' AND sdh.deal_sub_type_type_id IN ('' + @_deal_sub_type_type_id + '')'' END + CASE WHEN @_physical_financial_flag IS NULL THEN '''' ELSE '' AND sdd.physical_financial_flag = '''''' + @_physical_financial_flag + '''''''' END + CASE WHEN @_contract_id IS NULL THEN '''' ELSE '' AND sdh.contract_id IN ('' + @_contract_id + '')'' END + CASE WHEN @_trader_id IS NULL THEN '''' ELSE 
SET @_sql_whr = '' WHERE 1 = 1 and sdd.term_start =  ISNULL(sdst.term_start, sdpt.term_start)'' + CASE WHEN @_deal_date_from IS NULL AND @_deal_date_to IS NULL THEN '''' WHEN @_deal_date_to IS NULL THEN '' AND sdh.deal_date = CAST('''''' + @_deal_date_from + '''''' AS DATETIME)'' WHEN @_deal_date_from IS NULL THEN '' AND sdh.deal_date <= CAST('''''' + @_deal_date_to + '''''' AS DATETIME)'' ELSE '' AND sdh.deal_date BETWEEN CAST('''''' + @_deal_date_from + '''''' AS DATETIME) AND CAST('''''' + @_deal_date_to + '''''' AS DATETIME)'' END + CASE WHEN @_source_counterparty_id IS NULL THEN '''' ELSE '' AND sdh.counterparty_id IN ('' + @_source_counterparty_id + '')'' END + CASE WHEN @_source_deal_type_id IS NULL THEN '''' ELSE '' AND sdh.source_deal_type_id IN ('' + @_source_deal_type_id + '')'' END + 
+ CASE WHEN @_internal_portfolio_id IS NULL THEN '''' ELSE '' AND sdh.internal_portfolio_id IN ('' + @_internal_portfolio_id + '')'' END+ CASE WHEN @_template_id IS NULL THEN '''' ELSE '' AND sdh.template_id IN ('' + @_template_id + '')'' END 
+ CASE WHEN @_deal_sub_type_type_id IS NULL THEN '''' ELSE '' AND sdh.deal_sub_type_type_id IN ('' + @_deal_sub_type_type_id + '')'' END + CASE WHEN @_physical_financial_flag IS NULL THEN '''' ELSE '' AND sdd.physical_financial_flag = '''''' + @_physical_financial_flag + '''''''' END + CASE WHEN @_contract_id IS NULL THEN '''' ELSE '' AND sdh.contract_id IN ('' + @_contract_id + '')'' END + CASE WHEN @_trader_id IS NULL THEN '''' ELSE 
			'' AND sdh.trader_id IN ('' + @_trader_id + '')'' END + CASE WHEN @_block_definition_id IS NULL THEN '''' ELSE '' AND sdh.block_define_id = '' + @_block_definition_id END + CASE WHEN @_location_id IS NULL THEN '''' ELSE '' AND sdd.location_id IN ('' + @_location_id + '')'' END + CASE WHEN @_index_id IS NULL THEN '''' ELSE '' AND sdd.curve_id IN ('' + @_index_id + '')'' END + CASE WHEN @_source_deal_header_id IS NULL THEN '''' ELSE '' AND sdh.source_deal_header_id IN ('' + @_source_deal_header_id + '')'' END + CASE WHEN @_commodity_id IS NULL THEN '''' ELSE '' AND sdh.commodity_id IN ('' + @_commodity_id + '')'' END + CASE WHEN @_buy_sell_flag IS NULL THEN '''' ELSE '' AND sdh.header_buy_sell_flag = '''''' + @_buy_sell_flag + '''''''' END + CASE WHEN @_deal_ref_id IS NULL THEN '''' ELSE '' AND sdh.deal_id = '''''' + @_deal_ref_id + '''''''' END + CASE WHEN @_deal_status_id IS NULL THEN '''' ELSE '' AND sdh.deal_status IN ('' + @_deal_status_id + '')'' END + CASE WHEN @_confirm_status_id IS NULL THEN '''' ELSE '' AND sdh.confirm_status_type IN ('' + @_confirm_status_id + '')'' END + CASE WHEN @_pnl_source_value_id IS NULL THEN '''' ELSE '' AND (ISNULL(sdpt.pnl_source_value_id, sdpd.pnl_source_value_id) = '' + 
			@_pnl_source_value_id + '' OR ISNULL(sdpt.pnl_source_value_id, sdpd.pnl_source_value_id) IS NULL) '' END + CASE WHEN @_term_start IS NULL THEN '''' ELSE '' AND sdd.term_start>='''''' + CONVERT(VARCHAR(10), CAST(@_term_start AS DATE), 120) + '''''''' END + CASE WHEN @_term_end IS NULL THEN '''' ELSE '' AND sdd.term_end<='''''' + CONVERT(VARCHAR(10), CAST(@_term_end AS DATE), 120) + '''''''' END
...
--print @_sql_from4
--print @_sql_whr
EXEC (@_sql_select1 + @_sql_select2 + @_sql_from1 + @_sql_from2 + @_sql_from3 + @_sql_from4 + @_sql_whr)
SELECT fv.source_deal_header_id, term_start, term_end, leg, sds1.settlement_amount_s amount, Isnull(sds1.market_value_s, 0) market_value, sds1.contract_value_s contract_value, sds1.volume_s total_volume, sds1.float_price_s market_price, sds1.float_price_s formula_price, sds1.net_price_s net_price, sds1.deal_price_s deal_price, sds1.float_price_s float_price, sds1.volume_s Volume, sds1.charge_type_id AS [charge_type_id], ''Commodity'' AS [charge_type], sds1.settlement_currency_id currency_id, sds1.payment_date, sds1.currency, ''a'' actual_forward, fv.pnl_date, fv.tou_id, fv.tou_name
INTO #settlement_value -- select * from #settlement_value
FROM #final_values fv
CROSS APPLY (
	SELECT Max(as_of_date) as_of_date
	FROM source_deal_settlement_tou
	WHERE source_deal_header_id = fv.source_deal_header_id AND term_start = fv.term_start AND leg = fv.leg --and set_type=''''s''''
		AND term_start <= @_from_as_of_date AND as_of_date <= @_from_as_of_date
	) aod
CROSS APPLY (
	SELECT volume volume_s, deal_price deal_price_s, net_price net_price_s, float_price float_price_s, market_value market_value_s, contract_value Contract_value_s, settlement_amount settlement_amount_s, settlement_currency_id, payment_date, - 5500 charge_type_id, scur.currency_name currency
	FROM source_deal_settlement_tou a
	LEFT JOIN source_currency scur ON scur.source_currency_id = a.settlement_currency_id
	WHERE source_deal_header_id = fv.source_deal_header_id AND term_start = fv.term_start AND term_end = fv.term_end AND leg = fv.leg AND as_of_date = aod.as_of_date
	) sds1
LEFT JOIN settlement_charge_type_exclude scte ON Isnull(scte.charge_type_id, - 1) = sds1.charge_type_id AND ISNULL(scte.counterparty_id, - 1) = fv.source_counterparty_id AND ISNULL(scte.internal_counterparty_id, fv.internal_counterparty_id) = fv.internal_counterparty_id AND ISNULL(scte.contract_id, fv.contract_id) = fv.contract_id AND ISNULL(scte.commodity_id, fv.commodity_id) = fv.commodity_id AND ISNULL(scte.deal_type_id, fv.source_deal_type_id) = fv.source_deal_type_id AND COALESCE(scte.deal_sub_type_id, fv.deal_sub_type_type_id, - 1) = Isnull(fv.deal_sub_type_type_id, - 1) AND COALESCE(scte.internal_deal_type_id, fv.internal_deal_type_id, - 1) = Isnull(fv.internal_deal_type_id, - 1) AND COALESCE(scte.internal_deal_sub_type_id, fv.internal_deal_sub_type_id, - 1) = Isnull(fv.internal_deal_sub_type_id, - 1) AND COALESCE(scte.product_id, fv.product_id, - 1) = Isnull(fv.product_id, - 1) AND COALESCE(scte.pricing_type, fv.pricing_type, - 1) = Isnull(fv.pricing_type, - 1)
WHERE scte.charge_type_id IS NULL
--### Get Contract Settlement Values
SELECT source_counterparty_id, contract_id, entire_term_start term_start, entire_term_end term_end, commodity_id, Max(agg_term) agg_term
INTO #cs_group_info
FROM #final_values
GROUP BY source_counterparty_id, contract_id, entire_term_start, entire_term_end, commodity_id
SELECT source_counterparty_id, commodity_id, contract_id, term_start entire_term_start, term_end entire_term_end, civv1.charge_type_id, Cast(source_counterparty_id AS VARCHAR) + ''-'' + Cast(contract_id AS VARCHAR) + ''-'' + Cast(civv1.charge_type_id AS VARCHAR) reference, civv1.charge_type charge_type, civv1.value amount, civv1.volume total_volume, civv1.volume Volume, agg_term, civv1.curreny_id, civv1.payment_date, civv1.currency_name currency
INTO #contract_fees
FROM #cs_group_info fv
CROSS APPLY (
	SELECT Max(as_of_date) as_of_date
	FROM calc_invoice_volume_variance
	WHERE counterparty_id = fv.source_counterparty_id AND contract_id = fv.contract_id AND prod_date = fv.term_start AND as_of_date <= @_from_as_of_date
	) civv
CROSS APPLY (
	SELECT civ.invoice_line_item_id charge_type_id, sdv.code charge_type, civ.volume volume, civ.value value, civ.uom_id uom_id, su.uom_name uom, sc.source_currency_id curreny_id, sc.currency_name currency_name, civvt.payment_date payment_date
	...
	LEFT JOIN contract_group_detail cgd ON cgd.contract_id = cg.contract_id AND cgd.include_cash_flow = ''y''
	LEFT JOIN contract_charge_type cct ON cct.contract_charge_type_id = cg.contract_charge_type_id
	LEFT JOIN contract_charge_type_detail cctd ON cctd.contract_charge_type_id = cct.contract_charge_type_id AND cctd.include_cash_flow = ''y''
...
SELECT sv.source_deal_header_id, sv.term_start, sv.term_end, sv.leg, COALESCE(ifbs.fees, sv.amount, 0) amount, COALESCE(ifbs.fees, sv.amount, 0) contract_value, ifbs.price net_price, ifbs.actual_forward, COALESCE(sv.amount, fv.und_pnl_set, 0) und_pnl_set, ifbs.price deal_price, COALESCE(ifbs.volume, sv.volume) Volume, COALESCE(ifbs.field_id, sv.charge_type_id) charge_type_id, COALESCE(ifbs.field_name, sv.charge_type) charge_type, COALESCE(ifbs.currency_id, sv.currency_id) currency_id, scur.currency_name currency
INTO #deal_fees_set -- select * from #deal_fees_set
	-- select ifbs.*
FROM #settlement_value sv
LEFT JOIN #final_values fv ON fv.source_deal_header_id = sv.source_deal_header_id AND fv.term_start = sv.term_start AND fv.term_end = sv.term_end AND fv.leg = sv.leg AND fv.charge_type_id = sv.charge_type_id AND fv.charge_type = ''Commodity'' --and coalesce(fv.cash_flow,sv.amount,fv.amount) IS NOT NULL
CROSS APPLY (
	SELECT Max(as_of_date) as_of_date
	FROM index_fees_breakdown_settlement
	WHERE source_deal_header_id = sv.source_deal_header_id AND term_start = sv.term_start AND Isnull(leg, 1) = Isnull(sv.leg, 1)
		 AND as_of_date = Isnull(fv.from_as_of_date, @_from_as_of_date) AND field_id > 0
	) ifbs_mx
CROSS APPLY (
	SELECT volume, value fees, field_id, field_name, price, contract_value, ''a'' actual_forward, fee_currency_id currency_id
	FROM index_fees_breakdown_settlement
	WHERE source_deal_header_id = sv.source_deal_header_id AND term_start = sv.term_start AND Isnull(leg, 1) = Isnull(sv.leg, 1) AND as_of_date = ifbs_mx.as_of_date AND field_id > 0
	) ifbs
...
SELECT fv.source_deal_header_id, fv.term_start, fv.term_end, fv.leg, COALESCE(ifb.value, fv.amount, 0) amount, COALESCE(ifb.value, fv.amount, 0) contract_value, ifb.price net_price, ''f'' actual_forward, COALESCE(ifb.value, fv.amount, fv.und_pnl_set, 0) und_pnl_set, ifb.price deal_price, COALESCE(ifb.volume, fv.volume) Volume, COALESCE(ifb.field_id, fv.charge_type_id) charge_type_id, COALESCE(ifb.field_name, fv.charge_type) charge_type, COALESCE(ifb.fee_currency_id, fv.currency_id) currency_id, scur.currency_name currency
INTO #deal_fees
FROM #final_values fv
...
LEFT JOIN source_currency scur ON scur.source_currency_id = COALESCE(ifb.fee_currency_id, fv.currency_id)
...
INSERT INTO #final_values (
	...
	broker_fixed_cost, confirm_date, sub_book_group1_id, sub_book_group2_id, sub_book_group3_id, sub_book_group4_id, sub_book_group1, sub_book_group2, sub_book_group3, sub_book_group4, book_identifier1_id, book_identifier2_id, book_identifier3_id, book_identifier4_id, book_identifier1, book_identifier2, book_identifier3, book_identifier4, deal_price, float_price, Volume, close_reference_id, charge_type_id, charge_type, agg_term, recovery_rate, fv_level, current_non_current, deal_category_id, deal_category, internal_counterparty_id, internal_couterparty, template_id, volume_profile, price_adder2, volume_multiplier, volume_multiplier2, debt_rating, volume_frequency, currency_id, is_expired, entire_term_start, entire_term_end, contract_exipration_date, structured_id, payment_date, pricing_type, must_run_volume, dispatch_volume, must_run_market_value, must_run_contract_value, dispatch_market_value, dispatch_contract_value, pnl_date, cash_flow
	)
SELECT fv.sub_id, fv.stra_id, fv.book_id, fv.subsidiary, fv.strategy, fv.book, fv.sub_book, fv.source_deal_header_id, fv.deal_ref_id, fv.deal_date_from, fv.deal_date_to, fv.trader_id, fv.trader_name, fv.counterparty_name, fv.parent_counterparty_name, fv.contract_id, fv.[contract_name], fv.source_deal_type_id, fv.deal_type_name, fv.deal_sub_type_type_id, fv.deal_sub_type_name, fv.product_id, fv.commodity_id, fv.product_desc, fv.block_definition, fv.buy_sell_flag, fv.term_start, fv.term_end, fv.country_id, fv.country, fv.grid, fv.region_id, fv.region, fv.source_major_location_id, fv.location_group, fv.location_id, fv.location_name, fv.location_description, fv.from_as_of_date, df.Amount, fv.und_intrinsic_pnl, fv.und_extrinsic_pnl, fv.dis_pnl, fv.dis_intrinsic_pnl, fv.dis_extrinisic_pnl, fv.market_value, df.contract_value, fv.dis_market_value, fv.dis_contract_value, fv.discount_factor, fv.discount_amount, fv.commodity, fv.deal_volume, fv.volume_uom, fv.template_name, fv.curve_name, fv.index_tou, df.currency, fv.formula, fv.total_volume, fv.
	position_uom, fv.cva, fv.dva, fv.market_price, fv.formula_price, df.net_price, fv.physical_financial_flag, fv.block_definition_id, fv.leg, fv.source_counterparty_id, fv.counterparty_id, fv.index_id, fv.credit_adjusted_mtm, fv.discounted_credit_adjusted_mtm, fv.term_start_day, fv.term_start_month, fv.term_start_month_name, fv.term_start_year, fv.term_start_year_month, fv.term_quarter, fv.to_as_of_date, fv.sub_book_id, fv.transaction_type, fv.transaction_type_name, fv.period_from, fv.period_to, fv.probability, fv.credit_reserve, fv.dis_credit_reserve, fv.fair_value, fv.dis_fair_value, fv.model_reserve, df.actual_forward, fv.pnl_conversion_factor, fv.pnl_adjustment_value, fv.internal_deal_type_id, fv.internal_deal_type, fv.internal_deal_sub_type_id, fv.internal_deal_sub_type, fv.fixed_cost, fv.fixed_price, fv.formula_value, fv.price_adder, fv.price_multiplier, fv.strike_price, df.und_pnl_set, fv.formula_curve_id, fv.formula_curve_name, fv.internal_desk_id, fv.internal_desk, fv.internal_portfolio_id, fv.internal_portfolio, fv.broker_id
	, fv.broker_name, fv.type_of_entity_id, fv.type_of_entity, fv.int_ext_flag_id, fv.int_ext_flag, fv.deal_status_id, fv.deal_status, fv.confirm_status_id, fv.confirm_status, fv.pnl_source_value_id, fv.pnl_source_name, fv.description1, fv.description2, fv.description3, fv.description4, fv.counterparty_id2, fv.counterparty_name2, fv.ext_deal_id, fv.reference, fv.create_by, fv.create_ts, fv.update_by, fv.update_ts, fv.broker_fixed_cost, fv.confirm_date, fv.sub_book_group1_id, fv.sub_book_group2_id, fv.sub_book_group3_id, fv.sub_book_group4_id, fv.sub_book_group1, fv.sub_book_group2, fv.sub_book_group3, fv.sub_book_group4, fv.book_identifier1_id, fv.book_identifier2_id, fv.book_identifier3_id, fv.book_identifier4_id, fv.book_identifier1, fv.book_identifier2, fv.book_identifier3, fv.book_identifier4, df.deal_price, fv.float_price, df.Volume, fv.close_reference_id, df.charge_type_id, df.charge_type, fv.agg_term, fv.recovery_rate, fv.fv_level, fv.current_non_current, fv.deal_category_id, fv.deal_category, fv.internal_counterparty_id, fv.
	internal_couterparty, fv.template_id, fv.volume_profile, fv.price_adder2, fv.volume_multiplier, fv.volume_multiplier2, fv.debt_rating, fv.volume_frequency, df.currency_id, fv.is_expired, fv.entire_term_start, fv.entire_term_end, fv.contract_exipration_date, fv.structured_id, fv.payment_date, fv.pricing_type, fv.must_run_volume, fv.dispatch_volume, fv.must_run_market_value, fv.must_run_contract_value, fv.dispatch_market_value, fv.dispatch_contract_value, fv.pnl_date, df.Amount cash_flow
FROM #final_values fv
INNER JOIN (
	SELECT *
	FROM #deal_fees_set
	...
	SELECT *
	FROM #deal_fees
	) df ON fv.source_deal_header_id = df.source_deal_header_id AND fv.term_start = df.term_start AND fv.term_end = df.term_end AND fv.leg = df.leg
--order by 1
--INSERT INTO #final_values (
...
--	sub_book_group1, sub_book_group2, sub_book_group3, sub_book_group4, book_identifier1_id, book_identifier2_id, book_identifier3_id, book_identifier4_id, book_identifier1, book_identifier2, book_identifier3, book_identifier4, deal_price, float_price, Volume, close_reference_id, agg_term, recovery_rate, fv_level, current_non_current, deal_category_id, deal_category, internal_counterparty_id, internal_couterparty, template_id, volume_profile, price_adder2, volume_multiplier, volume_multiplier2, debt_rating, volume_frequency, currency_id, is_expired, entire_term_start, entire_term_end, contract_exipration_date, structured_id, payment_date, pricing_type, must_run_volume, dispatch_volume, must_run_market_value, must_run_contract_value, dispatch_market_value, dispatch_contract_value, charge_type_id, charge_type, actual_forward, currency, term_start, term_end, und_pnl_set, market_value, contract_value, Amount, cash_flow, pnl_date--, tou_id, tou_name
--	)
--SELECT DISTINCT fv.sub_id, fv.stra_id, fv.book_id, fv.subsidiary, fv.strategy, fv.book, fv.sub_book, fv.source_deal_header_id, fv.deal_ref_id, fv.deal_date_from, fv.deal_date_to, fv.trader_id, fv.trader_name, fv.counterparty_name, fv.parent_counterparty_name, fv.contract_id, fv.contract_name, fv.source_deal_type_id, fv.deal_type_name, fv.deal_sub_type_type_id, fv.deal_sub_type_name, fv.product_id, fv.commodity_id, fv.product_desc, fv.block_definition, fv.buy_sell_flag, fv.country_id, fv.country, fv.grid, fv.region_id, fv.region, fv.source_major_location_id, fv.location_group, fv.location_id, fv.location_name, fv.location_description, fv.from_as_of_date, fv.und_intrinsic_pnl, fv.und_extrinsic_pnl, fv.dis_pnl, fv.dis_intrinsic_pnl, fv.dis_extrinisic_pnl,
--	--isnull(sv.market_value ,fv.market_value) market_value ,
--	--isnull(sv.contract_value,fv.contract_value) contract_value ,
--	fv.dis_market_value, fv.dis_contract_value, fv.discount_factor, fv.discount_amount, fv.commodity, fv.deal_volume, fv.volume_uom, fv.template_name, fv.curve_name, fv.index_tou, fv.formula, isnull(sv.total_volume, fv.total_volume) total_volume, fv.position_uom, fv.cva, fv.dva, isnull(sv.market_price, fv.market_price) market_price, isnull(sv.formula_price, fv.formula_price) formula_price, isnull(sv.net_price, fv.net_price) net_price, fv.physical_financial_flag, fv.block_definition_id, fv.leg, fv.source_counterparty_id, fv.counterparty_id, fv.index_id, fv.credit_adjusted_mtm, fv.discounted_credit_adjusted_mtm, fv.term_start_day, fv.term_start_month, fv.term_start_month_name, fv.term_start_year, fv.term_start_year_month, fv.term_quarter, fv.to_as_of_date, fv.sub_book_id, fv.transaction_type, fv.transaction_type_name, fv.period_from, fv.period_to, fv.probability, fv.credit_reserve, fv.dis_credit_reserve, fv.fair_value, fv.dis_fair_value, fv.model_reserve, fv.pnl_conversion_factor, fv.pnl_adjustment_value, fv.internal_deal_type_id, fv.
--	internal_deal_type, fv.internal_deal_sub_type_id, fv.internal_deal_sub_type, fv.fixed_cost, fv.fixed_price, fv.formula_value, fv.price_adder, fv.price_multiplier, fv.strike_price, fv.formula_curve_id, fv.formula_curve_name, fv.internal_desk_id, fv.internal_desk, fv.internal_portfolio_id, fv.internal_portfolio, fv.broker_id, fv.broker_name, fv.type_of_entity_id, fv.type_of_entity, fv.int_ext_flag_id, fv.int_ext_flag, fv.deal_status_id, fv.deal_status, fv.confirm_status_id, fv.confirm_status, fv.pnl_source_value_id, fv.pnl_source_name, fv.description1, fv.description2, fv.description3, fv.description4, fv.counterparty_id2, fv.counterparty_name2, fv.ext_deal_id, fv.reference, fv.create_by, fv.create_ts, fv.update_by, fv.update_ts, fv.broker_fixed_cost, fv.confirm_date, fv.sub_book_group1_id, fv.sub_book_group2_id, fv.sub_book_group3_id, fv.sub_book_group4_id, fv.sub_book_group1, fv.sub_book_group2, fv.sub_book_group3, fv.sub_book_group4, fv.book_identifier1_id, fv.book_identifier2_id, fv.book_identifier3_id, fv.book_identifier4_id, fv.
--	book_identifier1, fv.book_identifier2, fv.book_identifier3, fv.book_identifier4, isnull(sv.deal_price, fv.deal_price) deal_price, isnull(sv.float_price, fv.float_price) float_price, isnull(sv.Volume, fv.Volume) Volume, fv.close_reference_id, fv.agg_term, fv.recovery_rate, fv.fv_level, fv.current_non_current, fv.deal_category_id, fv.deal_category, fv.internal_counterparty_id, fv.internal_couterparty, fv.template_id, fv.volume_profile, fv.price_adder2, fv.volume_multiplier, fv.volume_multiplier2, fv.debt_rating, fv.volume_frequency, isnull(sv.currency_id, fv.currency_id) currency_id, fv.is_expired, fv.entire_term_start, fv.entire_term_end, fv.contract_exipration_date, fv.structured_id, isnull(sv.payment_date, fv.payment_date) payment_date, fv.pricing_type, fv.must_run_volume, fv.dispatch_volume, fv.must_run_market_value, fv.must_run_contract_value, fv.dispatch_market_value, fv.dispatch_contract_value, fv.charge_type_id, fv.charge_type, isnull(sv.actual_forward, fv.actual_forward) actual_forward, isnull(sv.currency, fv.currency) 
--	currency, fv.term_start, fv.term_end, fv.und_pnl_set, isnull(sv.market_value, fv.market_value) market_value, isnull(sv.contract_value, fv.contract_value) contract_value, isnull(sv.amount, fv.amount) Amount, coalesce(sv.amount, fv.cash_flow, fv.amount) cash_flow, sv.pnl_date--, fv.tou_id, fv.tou_name
--FROM #settlement_value sv
...
SELECT  fv.sub_id, fv.stra_id, fv.book_id, fv.subsidiary, fv.strategy, fv.book, fv.sub_book, fv.source_deal_header_id, sdd.source_deal_detail_id, ISNULL(fv.deal_ref_id, fv.reference) deal_ref_id, fv.deal_date_from, fv.deal_date_to, fv.trader_id, fv.trader_name, fv.counterparty_name, fv.parent_counterparty_name, fv.contract_id, fv.contract_name, fv.source_deal_type_id, fv.deal_type_name, fv.deal_sub_type_type_id, fv.deal_sub_type_name, fv.product_id, fv.commodity_id, fv.product_desc, fv.block_definition, fv.buy_sell_flag, fv.country_id, fv.country, fv.grid, fv.region_id, fv.region, fv.source_major_location_id, fv.location_group, fv.location_id, fv.location_name, fv.location_description, fv.from_as_of_date, fv.und_intrinsic_pnl, fv.und_extrinsic_pnl, fv.dis_pnl, fv.dis_intrinsic_pnl, fv.dis_extrinisic_pnl
	--isnull(sv.market_value ,fv.market_value) market_value,
	--isnull(sv.contract_value,fv.contract_value) contract_value,
	, fv.dis_market_value, fv.dis_contract_value, fv.discount_factor, fv.discount_amount, fv.commodity, fv.deal_volume, fv.volume_uom, fv.template_name, fv.curve_name, fv.index_tou, fv.formula, fv.total_volume, fv.position_uom, fv.cva, fv.dva, fv.market_price, fv.formula_price, fv.net_price, fv.physical_financial_flag, fv.block_definition_id, fv.leg, fv.source_counterparty_id, fv.counterparty_id, fv.index_id, fv.credit_adjusted_mtm, fv.discounted_credit_adjusted_mtm, fv.term_start_day, fv.term_start_month, fv.term_start_month_name, fv.term_start_year, fv.term_start_year_month, fv.term_quarter, fv.to_as_of_date, fv.sub_book_id, fv.transaction_type, fv.transaction_type_name, fv.period_from, fv.period_to, fv.probability, fv.credit_reserve, fv.dis_credit_reserve, fv.fair_value, fv.dis_fair_value, fv.model_reserve, fv.pnl_conversion_factor, fv.pnl_adjustment_value, fv.internal_deal_type_id, fv.internal_deal_type, fv.internal_deal_sub_type_id, fv.internal_deal_sub_type, fv.fixed_cost, fv.fixed_price, fv.formula_value, fv.price_adder, fv.
	price_multiplier, fv.strike_price, fv.formula_curve_id, fv.formula_curve_name, fv.internal_desk_id, fv.internal_desk, fv.internal_portfolio_id, fv.internal_portfolio, fv.broker_id, fv.broker_name, fv.type_of_entity_id, fv.type_of_entity, fv.int_ext_flag_id, fv.int_ext_flag, fv.deal_status_id, fv.deal_status, fv.confirm_status_id, fv.confirm_status, fv.pnl_source_value_id, fv.pnl_source_name, fv.description1, fv.description2, fv.description3, fv.description4, fv.counterparty_id2, fv.counterparty_name2, fv.ext_deal_id, fv.reference, fv.create_by, fv.create_ts, fv.update_by, fv.update_ts, fv.broker_fixed_cost, fv.confirm_date, fv.sub_book_group1_id, fv.sub_book_group2_id, fv.sub_book_group3_id, fv.sub_book_group4_id, fv.sub_book_group1, fv.sub_book_group2, fv.sub_book_group3, fv.sub_book_group4, fv.book_identifier1_id, fv.book_identifier2_id, fv.book_identifier3_id, fv.book_identifier4_id, fv.book_identifier1, fv.book_identifier2, fv.book_identifier3, fv.book_identifier4, fv.deal_price, fv.float_price, fv.Volume, fv.close_reference_id, 
	fv.agg_term, fv.recovery_rate, fv.fv_level, fv.current_non_current, fv.deal_category_id, fv.deal_category, fv.internal_counterparty_id, fv.internal_couterparty, fv.template_id, fv.volume_profile, fv.price_adder2, fv.volume_multiplier, fv.volume_multiplier2, fv.debt_rating, fv.volume_frequency, fv.currency_id, fv.is_expired, fv.entire_term_start, fv.entire_term_end, fv.contract_exipration_date, fv.structured_id, fv.payment_date, fv.pricing_type, fv.must_run_volume, fv.dispatch_volume, fv.must_run_market_value, fv.must_run_contract_value, fv.dispatch_market_value, fv.dispatch_contract_value, fv.charge_type_id, fv.charge_type, 
	fv.actual_forward, fv.currency, fv.term_start, fv.term_end, fv.und_pnl_set, fv.market_value, fv.contract_value, fv.Amount, fv.cash_flow, fv.pnl_date, CONVERT(VARCHAR(7), fv.pnl_date, 120) [PNL_year_month], CONVERT(VARCHAR(7), fv.payment_date, 120) [cash_flow_year_month], ISNULL(@_deal_status_group, '''') [deal_status_group], fv.tou_id, fv.tou_name, fv.mw_position, fv.option_excercise_type, fv.option_type, 
	fv.strike_granularity, fv.premium, fv.delta, fv.gamma, fv.vega, fv.theta, fv.rho, fv.delta2, fv.market_value_id, fv.market_value_desc, fv.granularity, fv.exp_calendar_id, 
	fv.exp_calendar, fv.holiday_calendar_id, fv.holiday_calendar, fv.proxy_source_curve_def_id, fv.proxy_curve, fv.settlement_curve_id, fv.settlement_curve, fv.block_define_id, 
	fv.block_name, fv.udf_block_group_id, fv.block_group_name
INTO #tmp_settlement_detail
FROM #final_values fv
--LEFT JOIN settlement_charge_type_exclude scte ON scte.charge_type_id = fv.charge_type_id
--WHERE scte.id IS NULL
INNER JOIN source_deal_detail sdd ON sdd.source_deal_header_id = fv.source_deal_header_id-- AND sdd.term_start = fv.term_start AND sdd.term_end = fv.term_end 
AND sdd.leg = fv.leg
where sdd.term_start = fv.term_start
...
FROM (
	...
	FROM #tmp_settlement_detail
	) tsd
OUTER APPLY (
	SELECT SUM(match_vol) AS match_vol, MIN(buy_outstanding_vol) AS buy_outstanding_vol, MIN(buy_outstanding_vol) AS b_outstanding_vol
	...
	HAVING MIN(buy_outstanding_vol) > 0
	) mdv
OUTER APPLY (
	SELECT SUM(match_vol) AS match_vol, MIN(sell_outstanding_vol) AS sell_outstanding_vol, MIN(sell_outstanding_vol) AS s_outstanding_vol
	FROM match_deal_volume mdv
	WHERE mdv.sell_source_deal_detail_id = tsd.source_deal_detail_id
	GROUP BY mdv.sell_source_deal_detail_id
	HAVING MIN(sell_outstanding_vol) > 0
	) mdv1
OUTER APPLY (
...
SELECT tsd.sub_id
	...
	,ISNULL(tsd.deal_ref_id, tsd.reference) deal_ref_id
	,tsd.deal_date_from
	,tsd.deal_date_to
	,tsd.trader_id
	,tsd.trader_name
	,tsd.counterparty_name
	,tsd.parent_counterparty_name
	,tsd.contract_id
	,tsd.contract_name
	,tsd.source_deal_type_id
	,tsd.deal_type_name
	,tsd.deal_sub_type_type_id
	,tsd.deal_sub_type_name
	,tsd.product_id
	,tsd.commodity_id
	,tsd.product_desc
	,tsd.block_definition
	,tsd.buy_sell_flag
	,tsd.country_id
	,tsd.country
	,tsd.grid
	,tsd.region_id
	,tsd.region
	,tsd.source_major_location_id
	,tsd.location_group
	,tsd.location_id
	,tsd.location_name
	,tsd.location_description
	,ISNULL(tsd.from_as_of_date, tsd.to_as_of_date) [from_as_of_date]
	,tsd.und_intrinsic_pnl
	,tsd.und_extrinsic_pnl
	,tsd.dis_pnl
	,tsd.dis_intrinsic_pnl
	,tsd.dis_extrinisic_pnl
	--isnull(sv.market_value ,tsd.market_value)  market_value,
	--isnull(sv.contract_value,tsd.contract_value) contract_value,
	...
	,tsd.discount_amount
	,tsd.commodity
	,(tsd.deal_volume * ISNULL(t.match_ratio, 1)) deal_volume
	,tsd.volume_uom
	,tsd.template_name
	,tsd.curve_name
	,tsd.index_tou
	,tsd.formula
	,(tsd.total_volume * ISNULL(t.match_ratio, 1)) total_volume
	,tsd.position_uom
	,tsd.cva
	,tsd.dva
	,tsd.market_price
	,tsd.formula_price
	,tsd.net_price
	,tsd.physical_financial_flag
	,tsd.block_definition_id
	,tsd.leg
	...
	,tsd.term_start_day
	,tsd.term_start_month
	,tsd.term_start_month_name
	,tsd.term_start_year
	,tsd.term_start_year_month
	,tsd.term_quarter
	,tsd.to_as_of_date
	,tsd.sub_book_id
	,tsd.transaction_type
	,tsd.transaction_type_name
	,tsd.period_from
	,tsd.period_to
	,tsd.probability
	,tsd.credit_reserve
	...
	,tsd.internal_deal_type
	,tsd.internal_deal_sub_type_id
	,tsd.internal_deal_sub_type
	,tsd.fixed_cost
	,tsd.fixed_price
	,tsd.formula_value
	,tsd.price_adder
	...
	,tsd.formula_curve_name
	,tsd.internal_desk_id
	,tsd.internal_desk
	,tsd.internal_portfolio_id
	,tsd.internal_portfolio
	,tsd.broker_id
	,tsd.broker_name
	,tsd.type_of_entity_id
	,tsd.type_of_entity
	,tsd.int_ext_flag_id
	,tsd.int_ext_flag
	,tsd.deal_status_id
	,tsd.deal_status
	,tsd.confirm_status_id
	,tsd.confirm_status
	...
	,tsd.description3
	,tsd.description4
	,tsd.counterparty_id2
	,tsd.counterparty_name2
	,tsd.ext_deal_id
	,tsd.reference
	,tsd.create_by
	,tsd.create_ts
	,tsd.update_by
	,tsd.update_ts
	,tsd.broker_fixed_cost
	,tsd.confirm_date
	,tsd.sub_book_group1_id
	,tsd.sub_book_group2_id
	,tsd.sub_book_group3_id
	,tsd.sub_book_group4_id
	,tsd.sub_book_group1
	,tsd.sub_book_group2
	,tsd.sub_book_group3
	,tsd.sub_book_group4
	,tsd.book_identifier1_id
	,tsd.book_identifier2_id
	,tsd.book_identifier3_id
	,tsd.book_identifier4_id
	,tsd.book_identifier1
	,tsd.book_identifier2
	,tsd.book_identifier3
	,tsd.book_identifier4
	,tsd.deal_price
	,tsd.float_price
	,tsd.Volume
	,tsd.close_reference_id
	,tsd.agg_term
	,tsd.recovery_rate
	,tsd.fv_level
	,tsd.current_non_current
	,tsd.deal_category_id
	,tsd.deal_category
	,tsd.internal_counterparty_id
	,tsd.internal_couterparty
	,tsd.template_id
	,tsd.volume_profile
	,tsd.price_adder2
	...
	,tsd.debt_rating
	,tsd.volume_frequency
	,tsd.currency_id
	,tsd.is_expired
	,tsd.entire_term_start
	,tsd.entire_term_end
	,tsd.contract_exipration_date
	,tsd.structured_id
	,tsd.payment_date
	,tsd.pricing_type
	,tsd.must_run_volume
	...
	,CONVERT(VARCHAR(7), tsd.payment_date, 120) [cash_flow_year_month]
	,ISNULL(@_deal_status_group,'''') [deal_status_group]
	,tsd.tou_id
	,tsd.tou_name
	,tsd.mw_position
	,CASE WHEN tsd.option_excercise_type = ''a'' THEN ''American'' 
		   WHEN tsd.option_excercise_type = ''e'' THEN ''European'' 
		   WHEN tsd.option_excercise_type = ''O'' THEN ''Asian''  
	 ELSE tsd.option_excercise_type END [option_excercise_type]
	,CASE WHEN tsd.option_type = ''c'' THEN ''CALL'' WHEN tsd.option_type = ''p'' THEN ''PUT'' ELSE tsd.option_type END [option_type]
	,tsd.strike_granularity
	,tsd.premium
	,tsd.delta
	,tsd.gamma
	,tsd.vega
	...
	,tsd.delta2
	,tsd.market_value_id
	,tsd.market_value_desc
	,tsd.granularity
	,tsd.exp_calendar_id
	,tsd.exp_calendar
	,tsd.holiday_calendar_id
	,tsd.holiday_calendar
	,tsd.proxy_source_curve_def_id
	,tsd.proxy_curve
	,tsd.settlement_curve_id
	,tsd.settlement_curve
	,tsd.block_define_id
	,tsd.block_name
	,tsd.udf_block_group_id
	,tsd.block_group_name
--[__batch_report__] 
FROM #tmp_settlement_detail tsd
OUTER APPLY (
	SELECT source_deal_detail_id, match_ratio, ''a'' actual_forward
	FROM #tmp_matched_ratio tmr
	WHERE tmr.source_deal_detail_id = tsd.source_deal_detail_id AND match_ratio > 0 --AND ((tsd.buy_sell_flag = ''b'' AND tmr.b_outstanding_vol > 0) OR
		--(tsd.buy_sell_flag = ''s'' AND tmr.s_outstanding_vol > 0))
	UNION ALL
	SELECT source_deal_detail_id, outstanding_ratio, ''f'' actual_forward
	FROM #tmp_matched_ratio tmr
	WHERE tmr.source_deal_detail_id = tsd.source_deal_detail_id AND outstanding_ratio > 0 --AND ((tsd.buy_sell_flag = ''b'' AND tmr.b_outstanding_vol > 0) OR
		--(tsd.buy_sell_flag = ''s'' AND tmr.s_outstanding_vol > 0))
	) t
OUTER APPLY (
	SELECT *
	FROM #tmp_matched_ratio tmr
	WHERE tmr.source_deal_detail_id = tsd.source_deal_detail_id
	) t1', report_id = @report_id_data_source_dest,
	system_defined = '0'
	,category = '106500' 
	WHERE [name] = 'MTM Settlement Detail View'
		AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1)
	...
	IF EXISTS (SELECT 1 
	           ...
	           WHERE ds.[name] = 'MTM Settlement Detail View'
	            AND dsc.name =  'actual_forward'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
	BEGIN
		UPDATE dsc  
		SET alias = 'Actual Forward'
			   , reqd_param = NULL, widget_id = 2, datatype_id = 5, param_data_source = 'SELECT ''a'',''Actual''UNION SELECT ''f'', ''Forward''', param_default_value = NULL, append_filter = NULL, tooltip = NULL, column_template = 0, key_column = 1, required_filter = NULL
	...
	BEGIN
		...
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'actual_forward' AS [name], 'Actual Forward' AS ALIAS, NULL AS reqd_param, 2 AS widget_id, 5 AS datatype_id, 'SELECT ''a'',''Actual''UNION SELECT ''f'', ''Forward''' AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,0 AS column_template, 1 AS key_column, NULL AS required_filter				
		FROM sys.objects o
	...
	BEGIN
		...
		SET alias = 'Block Definition Id'
			   , reqd_param = NULL, widget_id = 2, datatype_id = 4, param_data_source = 'EXEC spa_StaticDataValues @flag = ''h'', @type_id = 10018', param_default_value = NULL, append_filter = NULL, tooltip = NULL, column_template = 2, key_column = 0, required_filter = 0
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
		WHERE ds.[name] = 'MTM Settlement Detail View'
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'broker_fixed_cost' AS [name], 'Broker Fixed Cost' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 3 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'MTM Settlement Detail View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		UPDATE dsc  
		SET alias = 'Buy Sell'
			   , reqd_param = NULL, widget_id = 2, datatype_id = 5, param_data_source = 'SELECT ''b'' as value,''Buy'' as Code' + CHAR(10) + 'UNION' + CHAR(10) + 'SELECT ''s'' as value, ''Sell'' as Code', param_default_value = NULL, append_filter = NULL, tooltip = NULL, column_template = 0, key_column = 0, required_filter = 0
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
		WHERE ds.[name] = 'MTM Settlement Detail View'
	...
	BEGIN
		...
		SET alias = 'Charge Type Id'
			   , reqd_param = NULL, widget_id = 2, datatype_id = 4, param_data_source = 'EXEC spa_StaticDataValues @flag = ''h'', @type_id = 5500', param_default_value = NULL, append_filter = NULL, tooltip = NULL, column_template = 2, key_column = 0, required_filter = NULL
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
		WHERE ds.[name] = 'MTM Settlement Detail View'
	...
	BEGIN
		...
		SET alias = 'Commodity Id'
			   , reqd_param = NULL, widget_id = 2, datatype_id = 4, param_data_source = 'exec spa_source_commodity_maintain ''a''', param_default_value = NULL, append_filter = NULL, tooltip = NULL, column_template = 2, key_column = 1, required_filter = 0
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
		WHERE ds.[name] = 'MTM Settlement Detail View'
	...
	IF EXISTS (SELECT 1 
	           ...
	           INNER JOIN data_source ds on ds.data_source_id = dsc.source_id 
	           WHERE ds.[name] = 'MTM Settlement Detail View'
	            AND dsc.name =  'contract_exipration_date'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
	BEGIN
		UPDATE dsc  
		SET alias = 'Contract Exipration Date'
			   , reqd_param = NULL, widget_id = 6, datatype_id = 2, param_data_source = NULL, param_default_value = NULL, append_filter = NULL, tooltip = NULL, column_template = 4, key_column = 0, required_filter = NULL
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
	...
	BEGIN
		...
		SET alias = 'Contract ID'
			   , reqd_param = NULL, widget_id = 7, datatype_id = 4, param_data_source = 'browse_contract_counterparty', param_default_value = NULL, append_filter = NULL, tooltip = NULL, column_template = 2, key_column = 1, required_filter = 0
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
		WHERE ds.[name] = 'MTM Settlement Detail View'
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'counterparty_id' AS [name], 'Counterparty ID' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 5 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,0 AS column_template, 1 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'MTM Settlement Detail View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'country_id' AS [name], 'Country Id' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 4 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'MTM Settlement Detail View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'credit_adjusted_mtm' AS [name], 'Credit Adjusted Mtm' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 3 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'MTM Settlement Detail View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'credit_reserve' AS [name], 'Credit Reserve' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 3 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'MTM Settlement Detail View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		SET alias = 'Currency Id'
			   , reqd_param = NULL, widget_id = 2, datatype_id = 4, param_data_source = 'exec spa_source_currency_maintain ''p''', param_default_value = NULL, append_filter = NULL, tooltip = NULL, column_template = 2, key_column = 0, required_filter = NULL
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
		WHERE ds.[name] = 'MTM Settlement Detail View'
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'currency_id' AS [name], 'Currency Id' AS ALIAS, NULL AS reqd_param, 2 AS widget_id, 4 AS datatype_id, 'exec spa_source_currency_maintain ''p''' AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'MTM Settlement Detail View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'current_non_current' AS [name], 'Current Non Current' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 5 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,0 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'MTM Settlement Detail View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'cva' AS [name], 'Cva' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 3 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'MTM Settlement Detail View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		SET alias = 'Deal Category Id'
			   , reqd_param = NULL, widget_id = 2, datatype_id = 4, param_data_source = 'EXEC spa_StaticDataValues @flag = ''h'', @type_id = 475', param_default_value = NULL, append_filter = NULL, tooltip = NULL, column_template = 2, key_column = 0, required_filter = NULL
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
		WHERE ds.[name] = 'MTM Settlement Detail View'
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'deal_category_id' AS [name], 'Deal Category Id' AS ALIAS, NULL AS reqd_param, 2 AS widget_id, 4 AS datatype_id, 'EXEC spa_StaticDataValues @flag = ''h'', @type_id = 475' AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'MTM Settlement Detail View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		SET alias = 'Deal Sub Type Type Id'
			   , reqd_param = NULL, widget_id = 2, datatype_id = 4, param_data_source = 'exec spa_source_deal_type_maintain ''x'',@sub_type=''y''', param_default_value = NULL, append_filter = NULL, tooltip = NULL, column_template = 2, key_column = 0, required_filter = 0
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
		WHERE ds.[name] = 'MTM Settlement Detail View'
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'debt_rating' AS [name], 'Debt Rating' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 5 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,0 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'MTM Settlement Detail View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'dis_credit_reserve' AS [name], 'Dis Credit Reserve' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 3 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'MTM Settlement Detail View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'dis_fair_value' AS [name], 'Dis Fair Value' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 3 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'MTM Settlement Detail View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'discounted_credit_adjusted_mtm' AS [name], 'Discounted Credit Adjusted Mtm' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 3 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'MTM Settlement Detail View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'dva' AS [name], 'Dva' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 3 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'MTM Settlement Detail View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'float_price' AS [name], 'Float Price' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 3 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'MTM Settlement Detail View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'index_tou' AS [name], 'Index Tou' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 5 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,0 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'MTM Settlement Detail View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'internal_counterparty_id' AS [name], 'Internal Counterparty Id' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 4 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'MTM Settlement Detail View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'internal_couterparty' AS [name], 'Internal Couterparty' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 5 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,0 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'MTM Settlement Detail View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'internal_deal_sub_type' AS [name], 'Internal Deal Sub Type' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 5 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,0 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'MTM Settlement Detail View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'internal_deal_sub_type_id' AS [name], 'Internal Deal Sub Type Id' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 4 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'MTM Settlement Detail View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'internal_deal_type_id' AS [name], 'Internal Deal Type Id' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 4 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'MTM Settlement Detail View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'is_expired' AS [name], 'Is Expired' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 5 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,0 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'MTM Settlement Detail View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'model_reserve' AS [name], 'Model Reserve' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 4 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'MTM Settlement Detail View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		SET alias = 'Physical Financial'
			   , reqd_param = NULL, widget_id = 2, datatype_id = 5, param_data_source = 'SELECT ''p'' as value,''Physical'' as Code' + CHAR(10) + 'UNION' + CHAR(10) + 'SELECT ''f'' as value, ''Financial'' as Code', param_default_value = NULL, append_filter = NULL, tooltip = NULL, column_template = 0, key_column = 0, required_filter = 0
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
		WHERE ds.[name] = 'MTM Settlement Detail View'
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'pnl_adjustment_value' AS [name], 'Pnl Adjustment Value' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 3 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'MTM Settlement Detail View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'pnl_conversion_factor' AS [name], 'Pnl Conversion Factor' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 3 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'MTM Settlement Detail View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		SET alias = 'Pnl Source Value Id'
			   , reqd_param = NULL, widget_id = 2, datatype_id = 4, param_data_source = 'EXEC spa_StaticDataValues @flag = ''h'', @type_id = 10007', param_default_value = NULL, append_filter = NULL, tooltip = NULL, column_template = 2, key_column = 0, required_filter = 0
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
		WHERE ds.[name] = 'MTM Settlement Detail View'
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'pnl_source_value_id' AS [name], 'Pnl Source Value Id' AS ALIAS, NULL AS reqd_param, 2 AS widget_id, 4 AS datatype_id, 'EXEC spa_StaticDataValues @flag = ''h'', @type_id = 10007' AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, 0 AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'MTM Settlement Detail View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'pricing_type' AS [name], 'Pricing Type' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 4 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'MTM Settlement Detail View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'probability' AS [name], 'Probability' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 3 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'MTM Settlement Detail View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'product_id' AS [name], 'Product Id' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 4 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'MTM Settlement Detail View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'recovery_rate' AS [name], 'Recovery Rate' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 3 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'MTM Settlement Detail View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'region_id' AS [name], 'Region Id' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 4 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'MTM Settlement Detail View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'source_major_location_id' AS [name], 'Source Major Location Id' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 4 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,2 AS column_template, 1 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'MTM Settlement Detail View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'structured_id' AS [name], 'Structured Id' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 5 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,0 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'MTM Settlement Detail View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		SET alias = 'Term Start'
			   , reqd_param = NULL, widget_id = 6, datatype_id = 2, param_data_source = NULL, param_default_value = NULL, append_filter = NULL, tooltip = NULL, column_template = 4, key_column = 1, required_filter = 0
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
		WHERE ds.[name] = 'MTM Settlement Detail View'
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'term_start' AS [name], 'Term Start' AS ALIAS, NULL AS reqd_param, 6 AS widget_id, 2 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,4 AS column_template, 1 AS key_column, 0 AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'MTM Settlement Detail View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'transaction_type' AS [name], 'Transaction Type' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 4 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,2 AS column_template, 1 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'MTM Settlement Detail View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		UPDATE dsc  
		SET alias = 'Transaction Type Name'
			   , reqd_param = NULL, widget_id = 1, datatype_id = 5, param_data_source = NULL, param_default_value = NULL, append_filter = NULL, tooltip = NULL, column_template = 0, key_column = 0, required_filter = NULL
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'transaction_type_name' AS [name], 'Transaction Type Name' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 5 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,0 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'MTM Settlement Detail View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'tou_id' AS [name], 'Tou Id' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 4 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'MTM Settlement Detail View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'tou_name' AS [name], 'Tou Name' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 5 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,0 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'MTM Settlement Detail View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'delta2' AS [name], 'Delta2' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 3 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'MTM Settlement Detail View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'gamma' AS [name], 'Gamma' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 3 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'MTM Settlement Detail View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'premium' AS [name], 'Premium' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 3 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'MTM Settlement Detail View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'rho' AS [name], 'Rho' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 3 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'MTM Settlement Detail View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'theta' AS [name], 'Theta' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 3 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'MTM Settlement Detail View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'vega' AS [name], 'Vega' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 3 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'MTM Settlement Detail View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	IF EXISTS (SELECT 1 
	           ...
	           INNER JOIN data_source ds on ds.data_source_id = dsc.source_id 
	           WHERE ds.[name] = 'MTM Settlement Detail View'
	            AND dsc.name =  'exp_calendar_id'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
	BEGIN
		UPDATE dsc  
		SET alias = 'Exp Calendar Id'
			   , reqd_param = NULL, widget_id = 1, datatype_id = 4, param_data_source = NULL, param_default_value = NULL, append_filter = NULL, tooltip = NULL, column_template = 2, key_column = 0, required_filter = NULL
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'exp_calendar_id' AS [name], 'Exp Calendar Id' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 4 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'MTM Settlement Detail View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'granularity' AS [name], 'Granularity' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 5 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'MTM Settlement Detail View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'holiday_calendar_id' AS [name], 'Holiday Calendar Id' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 4 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'MTM Settlement Detail View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	IF EXISTS (SELECT 1 
	           ...
	           INNER JOIN data_source ds on ds.data_source_id = dsc.source_id 
	           WHERE ds.[name] = 'MTM Settlement Detail View'
	            AND dsc.name =  'market_value_desc'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
	BEGIN
		UPDATE dsc  
		SET alias = 'Market Value Desc'
			   , reqd_param = NULL, widget_id = 1, datatype_id = 5, param_data_source = NULL, param_default_value = NULL, append_filter = NULL, tooltip = NULL, column_template = 2, key_column = 0, required_filter = NULL
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'market_value_desc' AS [name], 'Market Value Desc' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 5 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'MTM Settlement Detail View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	IF EXISTS (SELECT 1 
	           ...
	           INNER JOIN data_source ds on ds.data_source_id = dsc.source_id 
	           WHERE ds.[name] = 'MTM Settlement Detail View'
	            AND dsc.name =  'market_value_id'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
	BEGIN
		UPDATE dsc  
		SET alias = 'Market Value Id'
			   , reqd_param = NULL, widget_id = 1, datatype_id = 5, param_data_source = NULL, param_default_value = NULL, append_filter = NULL, tooltip = NULL, column_template = 0, key_column = 0, required_filter = NULL
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'market_value_id' AS [name], 'Market Value Id' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 5 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,0 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'MTM Settlement Detail View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	IF EXISTS (SELECT 1 
	           ...
	           INNER JOIN data_source ds on ds.data_source_id = dsc.source_id 
	           WHERE ds.[name] = 'MTM Settlement Detail View'
	            AND dsc.name =  'proxy_source_curve_def_id'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
	BEGIN
		UPDATE dsc  
		SET alias = 'Proxy Source Curve Def Id'
			   , reqd_param = NULL, widget_id = 1, datatype_id = 4, param_data_source = NULL, param_default_value = NULL, append_filter = NULL, tooltip = NULL, column_template = 2, key_column = 0, required_filter = NULL
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'proxy_source_curve_def_id' AS [name], 'Proxy Source Curve Def Id' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 4 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'MTM Settlement Detail View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	IF EXISTS (SELECT 1 
	           ...
	           INNER JOIN data_source ds on ds.data_source_id = dsc.source_id 
	           WHERE ds.[name] = 'MTM Settlement Detail View'
	            AND dsc.name =  'settlement_curve_id'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
	BEGIN
		UPDATE dsc  
		SET alias = 'Settlement Curve Id'
			   , reqd_param = NULL, widget_id = 1, datatype_id = 4, param_data_source = NULL, param_default_value = NULL, append_filter = NULL, tooltip = NULL, column_template = 2, key_column = 0, required_filter = NULL
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'settlement_curve_id' AS [name], 'Settlement Curve Id' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 4 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'MTM Settlement Detail View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	IF EXISTS (SELECT 1 
	           ...
	           INNER JOIN data_source ds on ds.data_source_id = dsc.source_id 
	           WHERE ds.[name] = 'MTM Settlement Detail View'
	            AND dsc.name =  'udf_block_group_id'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
	BEGIN
		UPDATE dsc  
		SET alias = 'Udf Block Group Id'
			   , reqd_param = NULL, widget_id = 1, datatype_id = 4, param_data_source = NULL, param_default_value = NULL, append_filter = NULL, tooltip = NULL, column_template = 2, key_column = 0, required_filter = NULL
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'udf_block_group_id' AS [name], 'Udf Block Group Id' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 4 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'MTM Settlement Detail View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	IF EXISTS (SELECT 1 
	           ...
	           INNER JOIN data_source ds on ds.data_source_id = dsc.source_id 
	           WHERE ds.[name] = 'MTM Settlement Detail View'
	            AND dsc.name =  'block_group_name'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
	BEGIN
		UPDATE dsc  
		SET alias = 'Block Group Name'
			   , reqd_param = NULL, widget_id = 1, datatype_id = 5, param_data_source = NULL, param_default_value = NULL, append_filter = NULL, tooltip = NULL, column_template = 0, key_column = 0, required_filter = NULL
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'block_group_name' AS [name], 'Block Group Name' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 5 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,0 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'MTM Settlement Detail View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	IF EXISTS (SELECT 1 
	           ...
	           INNER JOIN data_source ds on ds.data_source_id = dsc.source_id 
	           WHERE ds.[name] = 'MTM Settlement Detail View'
	            AND dsc.name =  'exp_calendar'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
	BEGIN
		UPDATE dsc  
		SET alias = 'Exp Calendar'
			   , reqd_param = NULL, widget_id = 1, datatype_id = 5, param_data_source = NULL, param_default_value = NULL, append_filter = NULL, tooltip = NULL, column_template = 0, key_column = 0, required_filter = NULL
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'exp_calendar' AS [name], 'Exp Calendar' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 5 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,0 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'MTM Settlement Detail View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'holiday_calendar' AS [name], 'Holiday Calendar' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 5 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,0 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'MTM Settlement Detail View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'proxy_curve' AS [name], 'Proxy Curve' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 5 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,0 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'MTM Settlement Detail View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	IF EXISTS (SELECT 1 
	           ...
	           INNER JOIN data_source ds on ds.data_source_id = dsc.source_id 
	           WHERE ds.[name] = 'MTM Settlement Detail View'
	            AND dsc.name =  'settlement_curve'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
	BEGIN
		UPDATE dsc  
		SET alias = 'Settlement Curve'
			   , reqd_param = NULL, widget_id = 1, datatype_id = 5, param_data_source = NULL, param_default_value = NULL, append_filter = NULL, tooltip = NULL, column_template = 0, key_column = 0, required_filter = NULL
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'settlement_curve' AS [name], 'Settlement Curve' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 5 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,0 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'MTM Settlement Detail View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
