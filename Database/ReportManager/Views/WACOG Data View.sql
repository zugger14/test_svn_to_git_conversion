BEGIN TRY
	...
	declare @new_ds_alias varchar(10) = 'WDV'
	/** IF DATA SOURCE ALIAS ALREADY EXISTS ON DESTINATION, RAISE ERROR **/
	if exists(select top 1 1 from data_source where alias = 'WDV' and name <> 'WACOG Data View')
	begin
		select top 1 @new_ds_alias = 'WDV' + cast(s.n as varchar(5))
		from seq s
		left join data_source ds on ds.alias = 'WDV' + cast(s.n as varchar(5))
	...
	IF NOT EXISTS (SELECT 1 
	           FROM data_source 
	           WHERE [name] = 'WACOG Data View'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
	AND NOT EXISTS (SELECT 1 FROM map_function_category WHERE [function_name] = 'WACOG Data View' AND '106500' = '106501') 
	BEGIN
		INSERT INTO data_source([type_id], [name], [alias], [description], [tsql], report_id, system_defined,category)
		SELECT TOP 1 1 AS [type_id], 'WACOG Data View' AS [name], @new_ds_alias AS ALIAS, 'WACOG Data' AS [description],null AS [tsql], @report_id_data_source_dest AS report_id,'0' AS [system_defined]
			,'106500' AS [category]
	END

	UPDATE data_source
	SET alias = @new_ds_alias, description = 'WACOG Data'
	, [tsql] = CAST('' AS VARCHAR(MAX)) + 'DECLARE @_process_id VARCHAR(100) = dbo.FNAGetNewID(), 
...
SELECT udf_template_id charge_type_id, sdv.value_id, field_label charge_type
INTO #charge_type
FROM user_defined_fields_template u
LEFT JOIN static_data_value sdv ON sdv.code = u.field_label AND sdv.type_id = 5500
WHERE NULLIF(internal_field_type, '''') IS NOT NULL
UNION ALL 
SELECT value_id, value_id, code 
FROM static_data_value 
WHERE value_id = -10019

IF OBJECT_ID(''tempdb..#volume_frequency'') IS NOT NULL
	DROP TABLE #volume_frequency

CREATE TABLE #volume_frequency (
	id CHAR(1) COLLATE DATABASE_DEFAULT,
	code VARCHAR(100) COLLATE DATABASE_DEFAULT
)

INSERT INTO #volume_frequency
EXEC spa_getVolumeFrequency NULL, NULL

SET @_sql = ''
	SELECT sdh.source_deal_header_id, sdd.source_deal_detail_id, sdh.deal_id ref_id, cwg.term, sc.counterparty_name counterparty, cg.contract_name contract, ct.charge_type,
		sdht.template_name template, sdt.source_deal_type_name deal_type, wg.wacog_group_name wacog_group, wg.wacog_group_id, sdh.counterparty_id, sdh.contract_id,
		sdh.template_id, sdh.source_deal_type_id deal_type_id, ct.charge_type_id, ct.value_id charge_type_id1, st.trader_name trader, sdh.trader_id, cwg.wacog, ISNULL(ifbs.value, sds.settlement_amount) charge_type_value,
		...
		vf.code frequency,
		IIF(sdh.physical_financial_flag = ''''p'''', ''''Physical'''', ''''Financial'''') physical_financial,
		sdd.leg,
		IIF(-10019 IN (ct.charge_type_id, ct.value_id), sds.volume, NULL) volume
	--[__batch_report__]	
	FROM '' + @_process_table + '' a
	INNER JOIN calculate_wacog_group cwg ON cwg.wacog_group_id = a.wacog_group_id
	...
	INNER JOIN source_deal_settlement sds ON sdh.source_deal_header_id = sds.source_deal_header_id AND sdd.leg = sds.leg
	LEFT JOIN index_fees_breakdown_settlement ifbs ON ifbs.source_deal_header_id = sds.source_deal_header_id AND ifbs.leg = sdd.leg
	LEFT JOIN #charge_type ct ON ISNULL(ifbs.field_id, -10019) IN (ct.charge_type_id, ct.value_id)
	...
	LEFT JOIN #volume_frequency vf ON vf.id = sdd.deal_volume_frequency
	WHERE NOT EXISTS (
		SELECT 1 
		FROM wacog_group_detail 
		WHERE charge_type_id IN (ct.charge_type_id, ct.value_id)
			AND source_deal_type_id = sdh.source_deal_type_id
			AND template_id = sdht.template_id
	)	
''
...
SET @_sql += ''
	GROUP BY sdh.source_deal_header_id, sdh.deal_id, cwg.term, sc.counterparty_name, cg.contract_name, ct.charge_type, sdht.template_name, sco.commodity_name, sdh.header_buy_sell_flag, sdd.leg,
			 sdt.source_deal_type_name, wg.wacog_group_name, wg.wacog_group_id, sdh.counterparty_id, sdh.contract_id, sdh.template_id, sml.location_name, spcd.curve_name, vf.code, sds.volume,
			 sdh.source_deal_type_id, ifbs.field_id, st.trader_name, sdh.trader_id, cwg.wacog, ifbs.value, ct.charge_type_id, ct.value_id, sds.settlement_amount, sdh.physical_financial_flag, sdd.source_deal_detail_id
''
...
EXEC(@_sql)', report_id = @report_id_data_source_dest,
	...
	BEGIN
		...
		SET alias = 'Charge Type Id'
			   , reqd_param = NULL, widget_id = 2, datatype_id = 4, param_data_source = 'SELECT udf_template_id, field_label FROM user_defined_fields_template WHERE udf_type = ''d'' AND NULLIF(internal_field_type, '''') IS NOT NULL', param_default_value = NULL, append_filter = NULL, tooltip = NULL, column_template = 2, key_column = 0, required_filter = 0
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
		WHERE ds.[name] = 'WACOG Data View'
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'deal_type_id' AS [name], 'Deal Type Id' AS ALIAS, NULL AS reqd_param, 2 AS widget_id, 4 AS datatype_id, 'EXEC spa_source_deal_type_maintain @flag=x' AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, 0 AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'WACOG Data View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	IF EXISTS (SELECT 1 
	           ...
	           INNER JOIN data_source ds on ds.data_source_id = dsc.source_id 
	           WHERE ds.[name] = 'WACOG Data View'
	            AND dsc.name =  'wacog_group'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
	BEGIN
		UPDATE dsc  
		SET alias = 'Wacog Group'
			   , reqd_param = NULL, widget_id = 1, datatype_id = 5, param_data_source = NULL, param_default_value = NULL, append_filter = NULL, tooltip = NULL, column_template = 0, key_column = 0, required_filter = NULL
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'wacog_group' AS [name], 'Wacog Group' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 5 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,0 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'WACOG Data View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		SET alias = 'Wacog Group Id'
			   , reqd_param = NULL, widget_id = 9, datatype_id = 4, param_data_source = 'SELECT wacog_group_id, wacog_group_name FROM wacog_group', param_default_value = NULL, append_filter = NULL, tooltip = NULL, column_template = 2, key_column = 0, required_filter = 0
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
		WHERE ds.[name] = 'WACOG Data View'
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'wacog_group_id' AS [name], 'Wacog Group Id' AS ALIAS, NULL AS reqd_param, 9 AS widget_id, 4 AS datatype_id, 'SELECT wacog_group_id, wacog_group_name FROM wacog_group' AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, 0 AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'WACOG Data View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		UPDATE dsc  
		SET alias = 'Trader ID'
			   , reqd_param = NULL, widget_id = 2, datatype_id = 4, param_data_source = 'EXEC spa_getsourcetrader @flag = b', param_default_value = NULL, append_filter = NULL, tooltip = NULL, column_template = 2, key_column = 0, required_filter = 0
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
		WHERE ds.[name] = 'WACOG Data View'
	...
	IF EXISTS (SELECT 1 
	           ...
	           INNER JOIN data_source ds on ds.data_source_id = dsc.source_id 
	           WHERE ds.[name] = 'WACOG Data View'
	            AND dsc.name =  'charge_type_value'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
	BEGIN
		UPDATE dsc  
		SET alias = 'Charge Type Value'
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'charge_type_value' AS [name], 'Charge Type Value' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 3 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'WACOG Data View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	IF EXISTS (SELECT 1 
	           ...
	           INNER JOIN data_source ds on ds.data_source_id = dsc.source_id 
	           WHERE ds.[name] = 'WACOG Data View'
	            AND dsc.name =  'charge_type_id1'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
	BEGIN
		UPDATE dsc  
		SET alias = 'Charge Type Id1'
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'charge_type_id1' AS [name], 'Charge Type Id1' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 4 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'WACOG Data View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'frequency' AS [name], 'Frequency' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 5 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,0 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'WACOG Data View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
