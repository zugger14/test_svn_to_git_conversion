BEGIN TRY
	...
	declare @new_ds_alias varchar(10) = 'TTEV'
	/** IF DATA SOURCE ALIAS ALREADY EXISTS ON DESTINATION, RAISE ERROR **/
	if exists(select top 1 1 from data_source where alias = 'TTEV' and name <> 'Trade Ticket Extract View')
	begin
		select top 1 @new_ds_alias = 'TTEV' + cast(s.n as varchar(5))
		from seq s
		left join data_source ds on ds.alias = 'TTEV' + cast(s.n as varchar(5))
	...
	AND NOT EXISTS (SELECT 1 FROM map_function_category WHERE [function_name] = 'Trade Ticket Extract View' AND '106500' = '106501') 
	BEGIN
		INSERT INTO data_source([type_id], [name], [alias], [description], [tsql], report_id, system_defined,category)
		SELECT TOP 1 1 AS [type_id], 'Trade Ticket Extract View' AS [name], @new_ds_alias AS ALIAS, 'Standard Trade Ticket Extract View' AS [description],null AS [tsql], @report_id_data_source_dest AS report_id,'1' AS [system_defined]
			,'106500' AS [category]
	END

	UPDATE data_source
	SET alias = @new_ds_alias, description = 'Standard Trade Ticket Extract View'
...
SET @_sql = ''  SELECT 			
		   ...
		   MAX(sdd.option_strike_price) [Option Strike Price],
			...
			SUM(sdd.total_volume) [Total Volume],			
