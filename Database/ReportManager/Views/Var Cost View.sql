...
BEGIN TRY
	...
	IF NOT EXISTS (SELECT 1 
	           FROM data_source 
	           WHERE [name] = 'Var Cost View'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
	BEGIN
		INSERT INTO data_source([type_id], [name], [alias], [description], [tsql], report_id)
		SELECT TOP 1 1 AS [type_id], 'Var Cost View' AS [name], 'VCMV' AS ALIAS, 'Standard Var Cost View' AS [description],null AS [tsql], @report_id_data_source_dest AS report_id
	END

	UPDATE data_source
	SET alias = 'VCMV', description = 'Standard Var Cost View'
	, [tsql] = CAST('' AS VARCHAR(MAX)) + 'declare 
    @_term_start varchar(20) =null
    ,@_term_end varchar(20) =null
    ,@_location_id varchar(1000) 
    ,@_hr_from varchar(20)=null
    ,@_hr_to varchar(20)=null,
...
set @_st=''select  
st.location_id
,sml.Location_Name Generator
,sdv.code GenType
,sdv1.code FuelType
,convert(varchar(10),term_hr,120) term_start
,convert(varchar(10),term_hr,120) term_end
,convert(varchar(8),term_hr,120)+''''01''''  Term_month
,convert(varchar(5),term_hr,120)+''''01-01''''      Term_year
,convert(varchar(5),term_hr,120)+ case when datepart(month,term_hr)<=3 then ''''01''''
                                         when datepart(month,term_hr) between 4 and 6 then ''''02''''
                                         when datepart(month,term_hr) between 7 and 9 then ''''03''''
                                         when datepart(month,term_hr) between 10 and 12 then ''''04''''
else ''''01'''' end +''''-01''''  Term_qtr
,datepart(hour,term_hr) hr_from
,datepart(hour,term_hr) hr_to
,round(st.unit_value,0) MW
,st.fuel Fuel
,st.fuel_amount      FuelCost
,st.om1       OnM1
,st.om2       OnM2
,st.om3       OnM3
,st.om4       OnM4
,st.total_cost Total$
,st.avg_cost AvgCost
,st.total_cost/st.unit_value [$/mwh]
,st.inc_cost IncrCost
,st.generator_config_value_id generator_config_value_id
,isnull(st.overlap_fuel_value_id,st.fuel_value_id) fuel_value_id
...
from process_short_term_generation_unit_cost st 
       ...
       left join static_data_value sdv on sdv.value_id=st.generator_config_value_id
       left join static_data_value sdv1 on sdv1.value_id=isnull(st.overlap_fuel_value_id,st.fuel_value_id)
where (nullif(generator_config_value_id,-1) is not null or  (nullif(generator_config_value_id,-1) is null ) and isnull(nullif(st.is_mix,0),st.is_default)=1)''+case when @_term_start is null then '''' else '' and st.term_hr>=''''''+@_term_start+'''''''' end
...
+case when @_hr_from is null then '''' else '' and datepart(hour,st.term_hr)>=''+@_hr_from end
+case when @_hr_to is null then '''' else '' and datepart(hour,st.term_hr)<=''+@_hr_to end
+case when @_generator_config_value_id is null then '''' else '' and st.generator_config_value_id =''+@_generator_config_value_id end
