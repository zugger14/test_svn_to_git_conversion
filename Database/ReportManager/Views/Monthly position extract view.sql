BEGIN TRY
	...
	declare @new_ds_alias varchar(10) = 'mpev'
	/** IF DATA SOURCE ALIAS ALREADY EXISTS ON DESTINATION, RAISE ERROR **/
	if exists(select top 1 1 from data_source where alias = 'mpev' and name <> 'Monthly position extract view')
	begin
		select top 1 @new_ds_alias = 'mpev' + cast(s.n as varchar(5))
		from seq s
		left join data_source ds on ds.alias = 'mpev' + cast(s.n as varchar(5))
	...
	IF NOT EXISTS (SELECT 1 
	           FROM data_source 
	           WHERE [name] = 'Monthly Position Extract View'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
	AND NOT EXISTS (SELECT 1 FROM map_function_category WHERE [function_name] = 'Monthly Position Extract View' AND '106500' = '106501') 
	BEGIN
		INSERT INTO data_source([type_id], [name], [alias], [description], [tsql], report_id, system_defined,category)
		SELECT TOP 1 1 AS [type_id], 'Monthly Position Extract View' AS [name], @new_ds_alias AS ALIAS, 'Monthly Position Extract View' AS [description],null AS [tsql], @report_id_data_source_dest AS report_id,'0' AS [system_defined]
			,'106500' AS [category]
	END

	UPDATE data_source
	SET alias = @new_ds_alias, description = 'Monthly Position Extract View'
...
DECLARE @_summary_option VARCHAR(6) =''m00d''
	, @_group_by CHAR(1) = ''d'' 
...
EXEC dbo.spa_position_report @_summary_option = @_summary_option, 
	...
	IF EXISTS (SELECT 1 
	           ...
	           WHERE ds.[name] = 'Monthly Position Extract View'
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'term_date' AS [name], 'Term Date' AS ALIAS, NULL AS reqd_param, 6 AS widget_id, 5 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,4 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Monthly Position Extract View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
