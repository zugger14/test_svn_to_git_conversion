...
''
	...
	SELECT distinct book.entity_id,ssbm.source_system_book_id1,ssbm.source_system_book_id2,ssbm.source_system_book_id3,ssbm.source_system_book_id4 fas_book_id
		,sub.timezone_id,book.entity_name [book],stra.entity_id [stra_id],stra.entity_name [strat]
...
SELECT actual_term_to_start, actual_term_to_end, IIF('' + ISNULL(@_commodity_id,'''''''''''') + '' = 123 , Position * 4, Position) Position, position_uom [Position UOM], 
	external_id [External ID/Time Series ID],'''''' + @_as_of_date + '''''' as_of_date , '''''''' sub_id, '''''''' stra_id, '''''''' book_id, '''''''' subbook_id, '''''''' sub_book_id,	
...
EXEC (@_Sql + @_Sql2)', report_id = @report_id_data_source_dest,
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'subbook_id' AS [name], 'Subbook Id' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 5 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Nomination Data Summary View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
