...
SET @_commodity_id  =-1
--SET @_as_of_date=''2022-06-01''
--SET @_term_start=''2022-06-01''
--SET @_term_end=''2022-06-01''
----SET @_location_id=2853
...
CREATE TABLE #get_position_data
(position NUMERIC(20,8), term_date DATE , hours INT, location_id INT, [location] varchar(500))

EXEC (''Insert into #get_position_data
SELECT SUM(position)Poistion, term_date , hours, location_id, location
FROM ''+@_position_process_table+''
Group by term_date , hours, location_id, location
Order by term_date , hours'')

Create table #location_temp
(source_minor_location_id int)

exec (''Insert into #location_temp
select source_minor_location_id from source_minor_location
where source_minor_location_id in (''+ @_location_id +'')'')

select tm.* ,dv.from_location
INTO #temp_case_pipeline
from #temp_mdq_data tm
INNER JOIN delivery_path dv on dv.path_id = tm.path_id
INNER JOIN  #location_temp lm on ISNULL(lm.source_minor_location_id, dv.from_location) = dv.from_location

select pos.term_date, pip.[gas_hr] as hours ,pos.location_id location_id, [location], SUM(pos.position) Position, pip.path_id, pip.path_name ,sum(pip.available_volume)  [aviliable volume], sum(pip.total_volume) [Total Volume], NULL as_of_date, NULL term_start, NULL term_end
--[__batch_report__]
FROM #get_position_data pos
INNER JOIN  #temp_case_pipeline pip ON pos.term_date = pip.gas_hr_term and pos.Hours = pip.gas_hr and pip.from_location = pos.location_id
GROUP BY pos.term_date, pip.[gas_hr],pos.location_id,pip.path_id, pip.path_name ,[location]
ORDER BY location_id,pip.path_name,pos.term_date,gas_hr
', report_id = @report_id_data_source_dest,
	...
	BEGIN
		UPDATE dsc  
		SET alias = 'Term End'
			   , reqd_param = NULL, widget_id = 6, datatype_id = 4, param_data_source = NULL, param_default_value = NULL, append_filter = NULL, tooltip = NULL, column_template = 2, key_column = 0, required_filter = 0
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
		WHERE ds.[name] = 'Hourly Position Pipeline View'
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'term_end' AS [name], 'Term End' AS ALIAS, NULL AS reqd_param, 6 AS widget_id, 4 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, 0 AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Hourly Position Pipeline View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'term_start' AS [name], 'Term Start' AS ALIAS, NULL AS reqd_param, 6 AS widget_id, 4 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, 0 AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Hourly Position Pipeline View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
