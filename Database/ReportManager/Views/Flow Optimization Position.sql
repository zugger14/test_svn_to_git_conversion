BEGIN TRY
	...
	declare @new_ds_alias varchar(10) = 'fwp'
	/** IF DATA SOURCE ALIAS ALREADY EXISTS ON DESTINATION, RAISE ERROR **/
	if exists(select top 1 1 from data_source where alias = 'fwp' and name <> 'Flow Optimization Position')
	begin
		select top 1 @new_ds_alias = 'fwp' + cast(s.n as varchar(5))
		from seq s
		left join data_source ds on ds.alias = 'fwp' + cast(s.n as varchar(5))
	...
	IF NOT EXISTS (SELECT 1 
	           FROM data_source 
	           WHERE [name] = 'Flow Optimization Position'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
	AND NOT EXISTS (SELECT 1 FROM map_function_category WHERE [function_name] = 'Flow Optimization Position' AND '106500' = '106501') 
	BEGIN
		INSERT INTO data_source([type_id], [name], [alias], [description], [tsql], report_id, system_defined,category)
		SELECT TOP 1 1 AS [type_id], 'Flow Optimization Position' AS [name], @new_ds_alias AS ALIAS, '' AS [description],null AS [tsql], @report_id_data_source_dest AS report_id,'0' AS [system_defined]
			,'106500' AS [category]
	END

	UPDATE data_source
	SET alias = @new_ds_alias, description = ''
	, [tsql] = CAST('' AS VARCHAR(MAX)) + 'DECLARE @_term_start DATETIME =  ''@term_start'' --''2019-11-11'' --  
DECLARE @_term_end DATETIME
DECLARE @_source_minor_location_id VARCHAR(MAX) = NULL
DECLARE @_source_major_location_id VARCHAR(4000) = NULL
DECLARE @_uom_id VARCHAR(100) --= ''6''
DECLARE @_source_deal_type_id VARCHAR(2000) --= ''1171''
DECLARE @_header_buy_sell_flag VARCHAR(10) --= ''s''
DECLARE @_pipeline_id VARCHAR(2000) --= ''s''
IF ''@uom_id'' <> ''NULL''
	SET @_uom_id = ''@uom_id''
...
IF ''@source_minor_location_id'' <> ''NULL''
	SET @_source_minor_location_id = ''@source_minor_location_id''
IF ''@source_major_location_id'' <> ''NULL''
	SET @_source_major_location_id = ''@source_major_location_id''
IF ''@pipeline_id'' <> ''NULL''
	SET @_pipeline_id = ''@pipeline_id''
IF ''@term_end'' <> ''NULL''
	SET @_term_end = ''@term_end''
--select @_term_start=''2019-11-11'',@_source_minor_location_id='''',@_source_deal_type_id='''',@_header_buy_sell_flag='''', @_source_major_location_id='''', @_pipeline_id=''''
--set all location group id if location group filter null
IF NULLIF(@_source_major_location_id, '''') IS NULL 
BEGIN
	SELECT @_source_major_location_id = STUFF(
		(SELECT DISTINCT '','' + CAST(sml.source_major_location_id AS VARCHAR(10))
		FROM source_major_location sml 
		FOR XML PATH(''''))
		,1,1,'''')
END
--set all locations of location group if location filter is null
IF NULLIF(@_source_minor_location_id, '''') IS NULL AND NULLIF(@_source_major_location_id, '''') IS NOT NULL
BEGIN
	SELECT @_source_minor_location_id = STUFF(
		(SELECT DISTINCT '','' + CAST(sml.source_minor_location_id AS VARCHAR(10))
		FROM source_minor_location sml 
		INNER JOIN dbo.SplitCommaSeperatedValues(@_source_major_location_id) scsv
...
DECLARE @_transportation_template_name VARCHAR(200) = ''Transportation NG''
DECLARE @_transportation_template_id INT
SELECT @_transportation_template_id = template_id  
FROM source_deal_header_template 
WHERE template_name = @_transportation_template_name
...
DECLARE @_proxy_locs VARCHAR(2000)
DECLARE @_child_proxy_locs VARCHAR(4000)
SELECT @_proxy_locs = STUFF(
							(	SELECT DISTINCT '',''  + CAST(sml.proxy_location_id AS VARCHAR)
								FROM source_minor_location sml 
								INNER JOIN dbo.SplitCommaSeperatedValues(@_source_minor_location_id) scsv 
									ON scsv.item = sml.source_minor_location_id
								WHERE sml.proxy_location_id IS NOT NULL
								FOR XML PATH(''''))
						, 1, 1, '''')
SELECT @_child_proxy_locs = STUFF(
									(	SELECT DISTINCT '','' + CAST(sml.source_minor_location_id AS VARCHAR(10))
										FROM source_minor_location sml
										INNER JOIN dbo.SplitCommaSeperatedValues(@_source_minor_location_id + ISNULL('','' + @_proxy_locs, '''')) scsv 
											ON scsv.item = sml.proxy_location_id
										LEFT JOIN dbo.SplitCommaSeperatedValues(@_source_minor_location_id) m 
											ON sml.source_minor_location_id = m.item
										WHERE m.item IS NULL	
										FOR XML PATH('''')
										)
								,1,1,'''')
--deal term breakdown insert
BEGIN					
	...
	INNER JOIN source_deal_header sdh (NOLOCK) ON sdh.source_deal_header_id = dd.source_deal_header_id 
		AND dd.term_start BETWEEN CASE WHEN sdh.term_frequency = ''m'' THEN DATEADD(m, DATEDIFF(m, 0, @_term_start), 0) ELSE @_term_start END  AND ISNULL(@_term_end, @_term_start) -- NOT required condition AS tm.term_start IS needed to filter
		AND dd.physical_financial_flag=''p''
	INNER JOIN #books bk ON bk.sub_book_id = sdh.sub_book
	INNER JOIN source_minor_location sml ON sml.source_minor_location_id = dd.location_id
	CROSS APPLY (
		SELECT scsv1.item, NULL [proxy_record]
		FROM dbo.SplitCommaSeperatedValues(@_source_minor_location_id) scsv1 
		where scsv1.item = dd.location_id 
		GROUP BY scsv1.item
		UNION ALL
		SELECT scsv2.item, ''parent_proxy'' [proxy_record]
		FROM dbo.SplitCommaSeperatedValues(@_proxy_locs) scsv2 
		WHERE scsv2.item = dd.location_id 
			AND scsv2.item NOT IN (SELECT s.item FROM dbo.SplitCommaSeperatedValues(@_source_minor_location_id) s)
		GROUP BY scsv2.item
		UNION ALL
		SELECT scsv3.item, ''child_proxy'' [proxy_record] 
		FROM dbo.SplitCommaSeperatedValues(@_child_proxy_locs) scsv3
		WHERE scsv3.item = dd.location_id 
			AND scsv3.item NOT IN (SELECT s.item FROM dbo.SplitCommaSeperatedValues(@_source_minor_location_id) s)
		GROUP BY scsv3.item
	) scsv
	OUTER APPLY (
		...
		WHERE dd.term_end >= DATEADD(DAY, n - 1, dd.term_start) --AND dd.term_start <> dd.term_end
			AND dd.term_start BETWEEN CASE WHEN sdh.term_frequency = ''m'' THEN DATEADD(m, DATEDIFF(m, 0, @_term_start), 0) ELSE @_term_start END  AND ISNULL(@_term_end, @_term_start)
			AND dd.physical_financial_flag=''p''
	) tm
	LEFT JOIN optimizer_header oh ON oh.transport_deal_id = dd.source_deal_header_id
	WHERE  tm.term_start BETWEEN @_term_start 
		AND ISNULL(@_term_end, @_term_start)
		AND dd.physical_financial_flag=''p''
		--AND dd.Leg=	CASE WHEN sdh.template_id = @_transportation_template_id					
		--				THEN 
...
END
--store deal detail udf [priority]
BEGIN
	...
	WHERE  1=1 
		...
		AND (sdd.term_start BETWEEN @_term_start AND ISNULL(@_term_end, @_term_start))
END
SET @_sql = CAST('''' AS VARCHAR(MAX)) + ''
SELECT CAST(tm.term_start AS DATE) [term_start]
	, dd.source_deal_header_id
	--, COALESCE(
	--	MAX(sml_parent_proxy.location_name)
	--	, MAX(ISNULL(NULLIF(from_loc.location_id, from_loc.location_name) + ''''-'''', '''''''') + from_loc.location_name + ISNULL('''' ['''' + mjr.location_name + '''']'''', ''''''''))
	--) [proxy_location_name]
	--, MAX(ISNULL(NULLIF(from_loc.location_id, from_loc.location_name) + ''''-'''', '''''''') + from_loc.location_name + ISNULL('''' ['''' + mjr.location_name + '''']'''', '''''''')) [location_name
	, COALESCE(
		  MAX(sml_parent_proxy.location_name)
		, MAX(from_loc.location_name)
	) [proxy_location_name]
	, MAX(from_loc.location_name) [location_name]
	, MAX(CASE 
			WHEN sdht.template_name = '''''' + @_transportation_template_name + ''''''
				THEN CASE 
			...
			ELSE CASE 
					...
					END
			END) [group]
	, MAX(sdh.description1) [nomination_group]
	...
	, CAST(SUM(hp.position) AS NUMERIC(10, 0)) [position]
	, IIF(
		MAX(sdht.template_name) = '''''' + @_transportation_template_name + ''''''
		, NULL
		, CAST((SUM(hp.position) - COALESCE(MAX(opt_supply.volume_used), MAX(opt_demand.volume_used) * -1, 0)) AS NUMERIC(20,0))
		)  [available_volume]
	, MAX(uom.uom_name) [uom_name]
	, CAST(MAX(tm.term_end) AS DATE) [term_end]
	, MAX(from_loc.source_minor_location_id) [source_minor_location_id]
	, MAX(uom.source_uom_id) [uom_id]
	, ''''@sub_id'''' [sub_id]
	, ''''@stra_id'''' [stra_id]
	, ''''@book_id'''' [book_id]
	, ''''@sub_book_id'''' [sub_book_id]
	, IIF(MAX(dd.buy_sell_flag) = ''''s'''', -1, 1) * ABS(CAST(MAX(dd.actual_volume) AS NUMERIC(10, 0)))	[actual_volume]
	, IIF(MAX(dd.buy_sell_flag) = ''''s'''', -1, 1) * ABS(CAST(COALESCE(MAX(dd.schedule_volume), SUM(hp.position)) AS NUMERIC(10, 0))) [schedule_volume]
	, IIF(MAX(dd.buy_sell_flag) = ''''s'''', -1, 1) * ABS(CAST(COALESCE(MAX(dd.actual_volume), MAX(dd.schedule_volume), SUM(hp.position)) AS NUMERIC(10, 0))) [best_available_volume]
	, MAX(sdt.source_deal_type_id) [source_deal_type_id]
	, MAX(sdh.header_buy_sell_flag) [header_buy_sell_flag]
	, MAX(mjr.source_major_location_id) [source_major_location_id]
	, MAX(mjr.location_name) [location_group]
	, MAX(loc_pipeline.counterparty_name) [pipeline]
	, MAX(loc_pipeline.source_counterparty_id) [pipeline_id]
	, NULL [receipt_actual]
	, NULL [delivery_actual]
	, NULL [receipt_scheduled]
	, NULL [delivery_scheduled]
	, NULL [receipt_position]
	, NULL [delivery_position]
	, NULL [receipt_best_available_volume]
	, NULL [delivery_best_available_volume]
	, NULL [fuel]
	, NULL [imbalance]
...
INNER JOIN #deal_term_breakdown tm
	...
	AND tm.term_start = dd.term_start --and dd.source_deal_header_id=759
...
LEFT JOIN source_counterparty loc_pipeline
	ON loc_pipeline.source_counterparty_id = from_loc.pipeline
LEFT JOIN contract_group cg
	ON cg.contract_id = sdh.contract_id
LEFT JOIN source_uom uom
	ON uom.source_uom_id = '' + ISNULL(NULLIF(@_uom_id,''''),''dd.deal_volume_uom_id'') + ''
...
WHERE 1 = 1
	AND (
		dd.location_id IN ('' + @_source_minor_location_id + '')
		OR (
			from_loc.proxy_location_id IN ('' + @_source_minor_location_id + '')
	...
	'' + ISNULL('' AND sdt.source_deal_type_id IN ('' + NULLIF(@_source_deal_type_id, '''') + '')'', '''') + ''
	'' + ISNULL('' AND sdh.header_buy_sell_flag = '''''' + NULLIF(@_header_buy_sell_flag, '''') + '''''''', '''') + ''
	'' + ISNULL('' AND loc_pipeline.source_counterparty_id IN ('' + NULLIF(@_pipeline_id, '''') + '')'', '''') + ''
GROUP BY dd.source_deal_header_id
	...
	--, dd.source_deal_detail_id
UPDATE #tmp_result
SET	  [receipt_actual]		= [actual_volume]
	, [receipt_scheduled]	= [schedule_volume]
	, [receipt_position]	= [position]
WHERE [template_name] = '''''' + @_transportation_template_name + ''''''
	AND leg = 1
UPDATE #tmp_result
SET	  [delivery_actual]		= [actual_volume]
	, [delivery_scheduled]	= [schedule_volume]
	, [delivery_position]	= [position]
WHERE [template_name] = '''''' + @_transportation_template_name + ''''''
	AND leg = 2
UPDATE #tmp_result
SET   [receipt_best_available_volume] = COALESCE([receipt_actual], [receipt_scheduled], [receipt_position])
	, [delivery_best_available_volume] = COALESCE([delivery_actual], [delivery_scheduled], [delivery_position])
WHERE [template_name] = '''''' + @_transportation_template_name + ''''''
UPDATE tr
SET [fuel] = (ABS(ISNULL(tr.[receipt_scheduled], 0)) - ABS(ISNULL(tr1.[delivery_scheduled], 0))) * -1
FROM #tmp_result tr
INNER JOIN #tmp_result tr1
	ON tr1.source_deal_header_id = tr.source_deal_header_id
	AND tr1.term_start = tr.term_start
	AND tr1.leg = 2
WHERE tr.[template_name] = '''''' + @_transportation_template_name + ''''''
	AND tr.leg = 1
UPDATE tr
SET [imbalance] = ABS(ISNULL(tr.[receipt_best_available_volume], 0)) + tr.[fuel] - ABS(ISNULL(tr1.[delivery_best_available_volume], 0))
FROM #tmp_result tr
INNER JOIN #tmp_result tr1
	ON tr1.source_deal_header_id = tr.source_deal_header_id
	AND tr1.term_start = tr.term_start
...
EXEC(@_sql)', report_id = @report_id_data_source_dest,
	...
	IF EXISTS (SELECT 1 
	           ...
	           WHERE ds.[name] = 'Flow Optimization Position'
	            AND dsc.name =  'group'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
	BEGIN
		UPDATE dsc  
	...
	BEGIN
		...
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Flow Optimization Position'
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'location_name' AS [name], 'Location' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 5 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,0 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Flow Optimization Position'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	IF EXISTS (SELECT 1 
	           ...
	           INNER JOIN data_source ds on ds.data_source_id = dsc.source_id 
	           WHERE ds.[name] = 'Flow Optimization Position'
	            AND dsc.name =  'nomination_group'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
	BEGIN
		UPDATE dsc  
		SET alias = 'Nomination Group'
			   , reqd_param = NULL, widget_id = 1, datatype_id = 5, param_data_source = NULL, param_default_value = NULL, append_filter = NULL, tooltip = NULL, column_template = 0, key_column = 0, required_filter = NULL
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'nomination_group' AS [name], 'Nomination Group' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 5 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,0 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Flow Optimization Position'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	IF EXISTS (SELECT 1 
	           ...
	           INNER JOIN data_source ds on ds.data_source_id = dsc.source_id 
	           WHERE ds.[name] = 'Flow Optimization Position'
	            AND dsc.name =  'proxy_location_name'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
	BEGIN
		UPDATE dsc  
		SET alias = 'Proxy Location'
			   , reqd_param = NULL, widget_id = 1, datatype_id = 5, param_data_source = NULL, param_default_value = NULL, append_filter = NULL, tooltip = NULL, column_template = 0, key_column = 0, required_filter = NULL
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'proxy_location_name' AS [name], 'Proxy Location' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 5 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,0 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Flow Optimization Position'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'uom_name' AS [name], 'UOM' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 5 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,0 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Flow Optimization Position'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	IF EXISTS (SELECT 1 
	           ...
	           INNER JOIN data_source ds on ds.data_source_id = dsc.source_id 
	           WHERE ds.[name] = 'Flow Optimization Position'
	            AND dsc.name =  'best_available_volume'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
	BEGIN
		UPDATE dsc  
	...
	BEGIN
		...
		SET alias = 'Header Buy Sell Flag'
			   , reqd_param = NULL, widget_id = 2, datatype_id = 5, param_data_source = 'select ''b'',''Buy'' union select ''s'',''Sell''', param_default_value = NULL, append_filter = NULL, tooltip = NULL, column_template = 0, key_column = 0, required_filter = 0
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
		WHERE ds.[name] = 'Flow Optimization Position'
	...
	BEGIN
		...
		SET alias = 'Source Deal Type Id'
			   , reqd_param = NULL, widget_id = 2, datatype_id = 4, param_data_source = 'EXEC spa_source_deal_type_maintain @flag=''x''', param_default_value = NULL, append_filter = NULL, tooltip = NULL, column_template = 2, key_column = 0, required_filter = 0
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
		WHERE ds.[name] = 'Flow Optimization Position'
	...
	BEGIN
		...
		SET alias = 'Source Major Location Id'
			   , reqd_param = NULL, widget_id = 2, datatype_id = 4, param_data_source = 'exec spa_source_major_location ''x''', param_default_value = NULL, append_filter = NULL, tooltip = NULL, column_template = 2, key_column = 0, required_filter = 0
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
		WHERE ds.[name] = 'Flow Optimization Position'
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'source_major_location_id' AS [name], 'Source Major Location Id' AS ALIAS, NULL AS reqd_param, 2 AS widget_id, 4 AS datatype_id, 'exec spa_source_major_location ''x''' AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, 0 AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Flow Optimization Position'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	IF EXISTS (SELECT 1 
	           ...
	           INNER JOIN data_source ds on ds.data_source_id = dsc.source_id 
	           WHERE ds.[name] = 'Flow Optimization Position'
	            AND dsc.name =  'delivery_actual'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
	BEGIN
		UPDATE dsc  
		SET alias = 'Delivery Actual'
			   , reqd_param = NULL, widget_id = 1, datatype_id = 4, param_data_source = NULL, param_default_value = NULL, append_filter = NULL, tooltip = NULL, column_template = 2, key_column = 0, required_filter = NULL
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'delivery_actual' AS [name], 'Delivery Actual' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 4 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Flow Optimization Position'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	IF EXISTS (SELECT 1 
	           ...
	           INNER JOIN data_source ds on ds.data_source_id = dsc.source_id 
	           WHERE ds.[name] = 'Flow Optimization Position'
	            AND dsc.name =  'delivery_position'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
	BEGIN
		UPDATE dsc  
		SET alias = 'Delivery Position'
			   , reqd_param = NULL, widget_id = 1, datatype_id = 4, param_data_source = NULL, param_default_value = NULL, append_filter = NULL, tooltip = NULL, column_template = 2, key_column = 0, required_filter = NULL
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'delivery_position' AS [name], 'Delivery Position' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 4 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Flow Optimization Position'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	IF EXISTS (SELECT 1 
	           ...
	           INNER JOIN data_source ds on ds.data_source_id = dsc.source_id 
	           WHERE ds.[name] = 'Flow Optimization Position'
	            AND dsc.name =  'delivery_scheduled'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
	BEGIN
		UPDATE dsc  
		SET alias = 'Delivery Scheduled'
			   , reqd_param = NULL, widget_id = 1, datatype_id = 4, param_data_source = NULL, param_default_value = NULL, append_filter = NULL, tooltip = NULL, column_template = 2, key_column = 0, required_filter = NULL
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'delivery_scheduled' AS [name], 'Delivery Scheduled' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 4 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Flow Optimization Position'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'fuel' AS [name], 'Fuel' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 4 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Flow Optimization Position'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	IF EXISTS (SELECT 1 
	           ...
	           INNER JOIN data_source ds on ds.data_source_id = dsc.source_id 
	           WHERE ds.[name] = 'Flow Optimization Position'
	            AND dsc.name =  'receipt_actual'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
	BEGIN
		UPDATE dsc  
		SET alias = 'Receipt Actual'
			   , reqd_param = NULL, widget_id = 1, datatype_id = 4, param_data_source = NULL, param_default_value = NULL, append_filter = NULL, tooltip = NULL, column_template = 2, key_column = 0, required_filter = NULL
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'receipt_actual' AS [name], 'Receipt Actual' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 4 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Flow Optimization Position'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	IF EXISTS (SELECT 1 
	           ...
	           INNER JOIN data_source ds on ds.data_source_id = dsc.source_id 
	           WHERE ds.[name] = 'Flow Optimization Position'
	            AND dsc.name =  'receipt_position'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
	BEGIN
		UPDATE dsc  
		SET alias = 'Receipt Position'
			   , reqd_param = NULL, widget_id = 1, datatype_id = 4, param_data_source = NULL, param_default_value = NULL, append_filter = NULL, tooltip = NULL, column_template = 2, key_column = 0, required_filter = NULL
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'receipt_position' AS [name], 'Receipt Position' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 4 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Flow Optimization Position'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	IF EXISTS (SELECT 1 
	           ...
	           INNER JOIN data_source ds on ds.data_source_id = dsc.source_id 
	           WHERE ds.[name] = 'Flow Optimization Position'
	            AND dsc.name =  'receipt_scheduled'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
	BEGIN
		UPDATE dsc  
		SET alias = 'Receipt Scheduled'
			   , reqd_param = NULL, widget_id = 1, datatype_id = 4, param_data_source = NULL, param_default_value = NULL, append_filter = NULL, tooltip = NULL, column_template = 2, key_column = 0, required_filter = NULL
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'receipt_scheduled' AS [name], 'Receipt Scheduled' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 4 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Flow Optimization Position'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		UPDATE dsc  
		SET alias = 'Pipeline ID'
			   , reqd_param = NULL, widget_id = 9, datatype_id = 4, param_data_source = 'SELECT source_counterparty_id, counterparty_name FROM source_counterparty sc WHERE sc.type_of_entity = -10021 ORDER BY 2', param_default_value = NULL, append_filter = NULL, tooltip = NULL, column_template = 2, key_column = 0, required_filter = 0
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
		WHERE ds.[name] = 'Flow Optimization Position'
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'pipeline_id' AS [name], 'Pipeline ID' AS ALIAS, NULL AS reqd_param, 9 AS widget_id, 4 AS datatype_id, 'SELECT source_counterparty_id, counterparty_name FROM source_counterparty sc WHERE sc.type_of_entity = -10021 ORDER BY 2' AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, 0 AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Flow Optimization Position'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	IF EXISTS (SELECT 1 
	           ...
	           INNER JOIN data_source ds on ds.data_source_id = dsc.source_id 
	           WHERE ds.[name] = 'Flow Optimization Position'
	            AND dsc.name =  'delivery_best_available_volume'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
	BEGIN
		UPDATE dsc  
		SET alias = 'Delivery Best Available Volume'
			   , reqd_param = NULL, widget_id = 1, datatype_id = 4, param_data_source = NULL, param_default_value = NULL, append_filter = NULL, tooltip = NULL, column_template = 2, key_column = 0, required_filter = NULL
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'delivery_best_available_volume' AS [name], 'Delivery Best Available Volume' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 4 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Flow Optimization Position'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'imbalance' AS [name], 'Imbalance' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 4 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Flow Optimization Position'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	IF EXISTS (SELECT 1 
	           ...
	           INNER JOIN data_source ds on ds.data_source_id = dsc.source_id 
	           WHERE ds.[name] = 'Flow Optimization Position'
	            AND dsc.name =  'receipt_best_available_volume'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
	BEGIN
		UPDATE dsc  
		SET alias = 'Receipt Best Available Volume'
			   , reqd_param = NULL, widget_id = 1, datatype_id = 4, param_data_source = NULL, param_default_value = NULL, append_filter = NULL, tooltip = NULL, column_template = 2, key_column = 0, required_filter = NULL
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'receipt_best_available_volume' AS [name], 'Receipt Best Available Volume' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 4 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Flow Optimization Position'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	FROM data_source_column dsc 
	INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
		AND ds.[name] = 'Flow Optimization Position'
		AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1)
