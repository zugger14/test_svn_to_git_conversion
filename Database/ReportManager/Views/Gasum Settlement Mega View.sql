BEGIN TRY
	...
	declare @new_ds_alias varchar(10) = 'GSMV'
	/** IF DATA SOURCE ALIAS ALREADY EXISTS ON DESTINATION, RAISE ERROR **/
	if exists(select top 1 1 from data_source where alias = 'GSMV' and name <> 'Gasum Settlement Mega View')
	begin
		select top 1 @new_ds_alias = 'GSMV' + cast(s.n as varchar(5))
		from seq s
		left join data_source ds on ds.alias = 'GSMV' + cast(s.n as varchar(5))
	...
	IF NOT EXISTS (SELECT 1 
	           FROM data_source 
	           WHERE [name] = 'Gasum Settlement Mega View'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
	AND NOT EXISTS (SELECT 1 FROM map_function_category WHERE [function_name] = 'Gasum Settlement Mega View' AND '106500' = '106501') 
	BEGIN
		INSERT INTO data_source([type_id], [name], [alias], [description], [tsql], report_id, system_defined,category)
		SELECT TOP 1 1 AS [type_id], 'Gasum Settlement Mega View' AS [name], @new_ds_alias AS ALIAS, 'Standard Gasum Settlement Mega View' AS [description],null AS [tsql], @report_id_data_source_dest AS report_id,'1' AS [system_defined]
			,'106500' AS [category]
	END

	UPDATE data_source
	SET alias = @new_ds_alias, description = 'Standard Gasum Settlement Mega View'
	, [tsql] = CAST('' AS VARCHAR(MAX)) + '--Gasum Settlement Mega View
...
DECLARE @_deal_date_from VARCHAR(50), @_deal_date_to VARCHAR(50), @_source_counterparty_id VARCHAR(max), @_source_deal_type_id VARCHAR(max), @_deal_sub_type_type_id VARCHAR(max), @_physical_financial_flag VARCHAR(20), @_contract_id VARCHAR(max), @_trader_id VARCHAR(max), @_block_definition_id VARCHAR(MAX), @_location_id VARCHAR(max), @_index_id VARCHAR(max), @_source_deal_header_id VARCHAR(50),	 
	...
	@_commodity_id VARCHAR(max), @_period_from VARCHAR(50), @_period_to VARCHAR(50), @_buy_sell_flag VARCHAR(20), @_deal_ref_id VARCHAR(50), @_deal_status_id VARCHAR(MAX), @_confirm_status_id VARCHAR(MAX), @_pnl_source_value_id VARCHAR(MAX)
...
CREATE TABLE #final_values (
	...
	VARCHAR(500) COLLATE database_default NULL, payment_date DATETIME, pricing_type INT, must_run_volume FLOAT, dispatch_volume FLOAT, must_run_market_value FLOAT, must_run_contract_value FLOAT, dispatch_market_value FLOAT, dispatch_contract_value FLOAT, cash_flow FLOAT, pnl_date DATETIME
	)
SET @_sql_select1 = 
	...
	CAST(sdpd.und_pnl_set AS FLOAT) amount,
	...
	ISNULL(sdpd.und_pnl,0) - (ISNULL(sdpd.und_pnl,0)* ISNULL(dp.probability, 0) * (1 - ISNULL(drr.rate, 0))) [credit_adjusted_mtm],
	...
	ISNULL(dp.probability, 0) [probability], ISNULL(sdpd.und_pnl, 0) * ISNULL(dp.probability, 0) * (1 - ISNULL(drr.rate, 0)) [credit_reserve],
	ISNULL(sdpd.dis_pnl, 0)* ISNULL(dp.probability, 0) * (1 - ISNULL(drr.rate, 0)) [dis_credit_reserve],
	sdpd.und_pnl - (ISNULL(sdpd.und_pnl, 0)* ISNULL(dp.probability, 0) * (1 - ISNULL(drr.rate, 0))) [fair_value], sdpd.dis_pnl - (ISNULL(sdpd.dis_pnl, 0)* ISNULL(dp.probability, 0) * (1 - ISNULL(drr.rate, 0))) [dis_fair_value],
	0 [model_reserve],
	...
	sdpd.formula_value,
	ISNULL(sdpd.price_adder, sdd.price_adder) price_adder, sdpd.price_multiplier, sdpd.strike_price, sdpd.und_pnl_set,
...
SET @_sql_select2 = '' sdh.close_reference_id,
	...
	sdpd.pnl_currency_id currency_id,
...
SELECT fv.sub_id, fv.stra_id, fv.book_id, fv.subsidiary, fv.strategy, fv.book, fv.sub_book, fv.source_deal_header_id, fv.deal_ref_id, fv.deal_date_from, fv.deal_date_to, fv.trader_id, fv.trader_name, fv.counterparty_name, fv.parent_counterparty_name, fv.contract_id, fv.contract_name, fv.source_deal_type_id, fv.deal_type_name, fv.deal_sub_type_type_id, fv.deal_sub_type_name, fv.product_id, fv.commodity_id, fv.product_desc, fv.block_definition, fv.buy_sell_flag, fv.country_id, fv.country, fv.grid, fv.region_id, fv.region, fv.source_major_location_id, fv.location_group, fv.location_id, fv.location_name, fv.location_description, fv.from_as_of_date, fv.und_intrinsic_pnl, fv.und_extrinsic_pnl, fv.dis_pnl, fv.dis_intrinsic_pnl, fv.dis_extrinisic_pnl,
	...
	currency, fv.term_start, fv.term_end, fv.und_pnl_set, isnull(sv.market_value, fv.market_value) market_value, isnull(sv.contract_value, fv.contract_value) contract_value, isnull(sv.amount, fv.amount) Amount, coalesce(sv.amount, fv.cash_flow, fv.amount) cash_flow, sv.pnl_date
FROM #settlement_value sv
...
SELECT fv.sub_id, fv.stra_id, fv.book_id, fv.subsidiary, fv.strategy, fv.book, fv.sub_book, fv.source_deal_header_id, sdd.source_deal_detail_id, ISNULL(fv.deal_ref_id, fv.reference) deal_ref_id, fv.deal_date_from, fv.deal_date_to, fv.trader_id, fv.trader_name, fv.counterparty_name, fv.parent_counterparty_name, fv.contract_id, fv.contract_name, fv.source_deal_type_id, fv.deal_type_name, fv.deal_sub_type_type_id, fv.deal_sub_type_name, fv.product_id, fv.commodity_id, fv.product_desc, fv.block_definition, fv.buy_sell_flag, fv.country_id, fv.country, fv.grid, fv.region_id, fv.region, fv.source_major_location_id, fv.location_group, fv.location_id, fv.location_name, fv.location_description, fv.from_as_of_date, fv.und_intrinsic_pnl, fv.und_extrinsic_pnl, fv.dis_pnl, fv.dis_intrinsic_pnl, fv.dis_extrinisic_pnl
	...
	fv.actual_forward, fv.currency, fv.term_start, fv.term_end, fv.und_pnl_set, fv.market_value, fv.contract_value, fv.Amount, fv.cash_flow, fv.pnl_date, CONVERT(VARCHAR(7), fv.pnl_date, 120) [PNL_year_month], CONVERT(VARCHAR(7), fv.payment_date, 120) [cash_flow_year_month], ISNULL(@_deal_status_group, '''') [deal_status_group]
INTO #tmp_settlement_detail
FROM #final_values fv
INNER JOIN source_deal_detail sdd ON sdd.source_deal_header_id = fv.source_deal_header_id AND sdd.term_start = fv.term_start AND sdd.term_end = fv.term_end AND sdd.leg = fv.leg
...
SELECT tsd.sub_id
	...
	CASE WHEN tsd.charge_type_id = -5500
		THEN CASE WHEN tsd.actual_forward = ''a''
				THEN ifbs.volume
				ELSE ifb.volume
				END
		ELSE tsd.volume
	END * ISNULL(t.match_ratio, 1) [total_volume_new],
	CASE WHEN tsd.charge_type_id = -5500
		THEN CASE WHEN tsd.actual_forward = ''a''
				THEN ifbs.value
				ELSE ifb.value
				END
		ELSE tsd.contract_value
	END * ISNULL(t.match_ratio, 1) [contract_value_new],
	CASE WHEN tsd.charge_type_id = -5500
		THEN CASE WHEN tsd.actual_forward = ''a''
				THEN ifbs.price
				ELSE ifb.price
	...
	CASE WHEN tsd.charge_type_id = -5500
		THEN CASE WHEN tsd.actual_forward = ''a''
				THEN ifbs.field_name
				ELSE ifb.field_name
				END
		ELSE NULL
	END [pricing_index_new],
	CASE WHEN tsd.charge_type_id = -5500
		THEN CASE WHEN tsd.actual_forward = ''a''
				THEN sdv_ifbs.code
				ELSE sdv_ifb.code
...
LEFT JOIN index_fees_breakdown_settlement ifbs
	ON ifbs.source_deal_header_id = tsd.source_deal_header_id
		AND ifbs.term_start = tsd.term_start
		AND ifbs.leg = tsd.leg
		AND ifbs.contract_mkt_flag = ''c''
		AND 1= (CASE WHEN tsd.charge_type_id = -5500 AND tsd.actual_forward = ''a'' THEN 1 ELSE 0 END)
		AND ifbs.as_of_date = Isnull(tsd.from_as_of_date, @_from_as_of_date)
LEFT JOIN index_fees_breakdown ifb
	ON ifb.source_deal_header_id = tsd.source_deal_header_id 
		AND ifb.term_start = tsd.term_start
		AND ISNULL(ifb.leg, 1) = ISNULL(tsd.leg, 1)
		AND ifb.contract_mkt_flag = ''c''
		AND ifb.as_of_date = Isnull(tsd.from_as_of_date, @_from_as_of_date)
		AND 1= (CASE WHEN tsd.charge_type_id = -5500 AND tsd.actual_forward = ''f'' THEN 1 ELSE 0 END) 
LEFT JOIN source_price_curve_def spcd_ifbs
	ON spcd_ifbs.curve_name = ifbs.field_name
LEFT JOIN static_data_value sdv_ifbs 
	ON sdv_ifbs.value_id = spcd_ifbs.index_group
LEFT JOIN source_price_curve_def spcd_ifb
	ON spcd_ifb.curve_name = ifb.field_name
LEFT JOIN static_data_value sdv_ifb 
	ON sdv_ifb.value_id = spcd_ifb.index_group
OUTER APPLY (
...
OUTER APPLY (
	...
	IF EXISTS (SELECT 1 
	           ...
	           WHERE ds.[name] = 'Gasum Settlement Mega View'
	...
	BEGIN
		...
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Gasum Settlement Mega View'
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'deal_status' AS [name], 'Deal Status' AS ALIAS, 0 AS reqd_param, 1 AS widget_id, 5 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, 1 AS append_filter, NULL  AS tooltip,0 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Gasum Settlement Mega View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		SET alias = 'Product ID'
			   , reqd_param = 0, widget_id = 9, datatype_id = 4, param_data_source = 'EXEC spa_source_product @flag=g', param_default_value = NULL, append_filter = 1, tooltip = NULL, column_template = 2, key_column = 0, required_filter = 0
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
		WHERE ds.[name] = 'Gasum Settlement Mega View'
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'product_id' AS [name], 'Product ID' AS ALIAS, 0 AS reqd_param, 9 AS widget_id, 4 AS datatype_id, 'EXEC spa_source_product @flag=g' AS param_data_source, NULL AS param_default_value, 1 AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, 0 AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Gasum Settlement Mega View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	IF EXISTS (SELECT 1 
	           ...
	           INNER JOIN data_source ds on ds.data_source_id = dsc.source_id 
	           WHERE ds.[name] = 'Gasum Settlement Mega View'
	            AND dsc.name =  'contract_value_new'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
	BEGIN
		UPDATE dsc  
		SET alias = 'Contract Value New'
			   , reqd_param = NULL, widget_id = 1, datatype_id = 3, param_data_source = NULL, param_default_value = NULL, append_filter = NULL, tooltip = NULL, column_template = 2, key_column = 0, required_filter = NULL
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'contract_value_new' AS [name], 'Contract Value New' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 3 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Gasum Settlement Mega View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	IF EXISTS (SELECT 1 
	           ...
	           INNER JOIN data_source ds on ds.data_source_id = dsc.source_id 
	           WHERE ds.[name] = 'Gasum Settlement Mega View'
	            AND dsc.name =  'deal_price_new'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
	BEGIN
		UPDATE dsc  
		SET alias = 'Deal Price New'
			   , reqd_param = NULL, widget_id = 1, datatype_id = 3, param_data_source = NULL, param_default_value = NULL, append_filter = NULL, tooltip = NULL, column_template = 2, key_column = 0, required_filter = NULL
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'deal_price_new' AS [name], 'Deal Price New' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 3 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Gasum Settlement Mega View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	IF EXISTS (SELECT 1 
	           ...
	           INNER JOIN data_source ds on ds.data_source_id = dsc.source_id 
	           WHERE ds.[name] = 'Gasum Settlement Mega View'
	            AND dsc.name =  'index_group_new'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
	BEGIN
		UPDATE dsc  
		SET alias = 'Index Group New'
			   , reqd_param = NULL, widget_id = 1, datatype_id = 5, param_data_source = NULL, param_default_value = NULL, append_filter = NULL, tooltip = NULL, column_template = 0, key_column = 0, required_filter = NULL
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'index_group_new' AS [name], 'Index Group New' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 5 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,0 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Gasum Settlement Mega View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	IF EXISTS (SELECT 1 
	           ...
	           INNER JOIN data_source ds on ds.data_source_id = dsc.source_id 
	           WHERE ds.[name] = 'Gasum Settlement Mega View'
	            AND dsc.name =  'pricing_index_new'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
	BEGIN
		UPDATE dsc  
		SET alias = 'Pricing Index New'
			   , reqd_param = NULL, widget_id = 1, datatype_id = 5, param_data_source = NULL, param_default_value = NULL, append_filter = NULL, tooltip = NULL, column_template = 0, key_column = 0, required_filter = NULL
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'pricing_index_new' AS [name], 'Pricing Index New' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 5 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,0 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Gasum Settlement Mega View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	IF EXISTS (SELECT 1 
	           ...
	           INNER JOIN data_source ds on ds.data_source_id = dsc.source_id 
	           WHERE ds.[name] = 'Gasum Settlement Mega View'
	            AND dsc.name =  'total_volume_new'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
	BEGIN
		UPDATE dsc  
		SET alias = 'Total Volume New'
			   , reqd_param = NULL, widget_id = 1, datatype_id = 3, param_data_source = NULL, param_default_value = NULL, append_filter = NULL, tooltip = NULL, column_template = 2, key_column = 0, required_filter = NULL
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'total_volume_new' AS [name], 'Total Volume New' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 3 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Gasum Settlement Mega View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	FROM data_source_column dsc 
	INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
		AND ds.[name] = 'Gasum Settlement Mega View'
		AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1)
