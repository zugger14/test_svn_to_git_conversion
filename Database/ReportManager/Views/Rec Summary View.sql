BEGIN TRY
	...
	declare @new_ds_alias varchar(10) = 'RecSv'
	/** IF DATA SOURCE ALIAS ALREADY EXISTS ON DESTINATION, RAISE ERROR **/
	if exists(select top 1 1 from data_source where alias = 'RecSv' and name <> 'Rec Summary View')
	begin
		select top 1 @new_ds_alias = 'RecSv' + cast(s.n as varchar(5))
		from seq s
		left join data_source ds on ds.alias = 'RecSv' + cast(s.n as varchar(5))
	...
	IF NOT EXISTS (SELECT 1 
	           FROM data_source 
	           WHERE [name] = 'Rec Summary View'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
	AND NOT EXISTS (SELECT 1 FROM map_function_category WHERE [function_name] = 'Rec Summary View' AND '106500' = '106501') 
	BEGIN
		INSERT INTO data_source([type_id], [name], [alias], [description], [tsql], report_id, system_defined,category)
		SELECT TOP 1 1 AS [type_id], 'Rec Summary View' AS [name], @new_ds_alias AS ALIAS, 'Standard Rec Summary View' AS [description],null AS [tsql], @report_id_data_source_dest AS report_id,'1' AS [system_defined]
			,'106500' AS [category]
	END

	UPDATE data_source
	SET alias = @new_ds_alias, description = 'Standard Rec Summary View'
...
INNER JOIN source_system_book_map ssbm
	...
	ON  ssbm.fas_book_id = book.[entity_id]
		...
		--and ssbm.book_deal_type_map_id=27
...
CREATE TABLE #temp_mdv_source_deal_header(
	...
	header_buy_sell_flag NVARCHAR(1) COLLATE DATABASE_DEFAULT,

	transaction_classification VARCHAR(10) COLLATE DATABASE_DEFAULT,
...
(source_deal_detail_id int, facility_name varchar(1000) COLLATE DATABASE_DEFAULT,  unit_id varchar(1000) COLLATE DATABASE_DEFAULT, Generation_state int, technology int, contract_expiration_date date, gis_certificate_number_from varchar(1000) COLLATE DATABASE_DEFAULT, gis_certificate_number_to varchar(1000) COLLATE DATABASE_DEFAULT)
...
SET @_sql = ''
	...
	INSERT INTO #temp_mdv_source_deal_header (source_deal_header_id, header_buy_sell_flag, transaction_classification,gen_state_value_id,technology)

	SELECT DISTINCT sdh.source_deal_header_id, sdh.header_buy_sell_flag,

	CASE WHEN sdh.product_classification = 107400 AND sdh.header_buy_sell_flag = ''''s'''' THEN ''''Target'''' WHEN NULLIF(sdh.product_classification, '''''''') IS NULL AND sdh.header_buy_sell_flag = ''''s'''' THEN ''''Sell'''' WHEN NULLIF(sdh.product_classification, '''''''') IS NULL AND sdh.header_buy_sell_flag = ''''b'''' THEN IIF(sdh.generator_id IS NULL, ''''Buy'''', ''''Gen'''') END
	...
	WHERE 1 = 1  and sdh.is_environmental = ''''y'''''' +
...
SET @_sql = ''
	...
	INSERT INTO #temp_mdv_source_deal_detail (source_deal_header_id, source_deal_detail_id, is_volume_left, match_status,gen_state_value_id,technology)
...
CASE WHEN @_deal_detail_status_id IS NOT NULL THEN '' AND sdd.status = '' + @_deal_detail_status_id + '''' ELSE '''' END +

CASE WHEN @_delivery_date_from IS NOT NULL THEN '' AND CAST(sdd.delivery_date AS DATE) >= '''''' + @_delivery_date_from + '''''''' ELSE '''' END +

CASE WHEN @_delivery_date_from IS NOT NULL THEN '' AND CAST(sdd.delivery_date AS DATE) <= '''''' + @_delivery_date_to + '''''''' ELSE '''' END
...
--SELECT tmsdh.source_deal_detail_id,  NULL as facility_name, NULL as unit_id, NULL as Generation_state, NULL as technology, min(contract_expiration_date), max(gis_certificate_number_from), max(gis_certificate_number_to)
...
---- CASE WHEN @_facility_name IS NOT NULL THEN '' AND gis.name IN ('' + @_facility_name + '')'' ELSE '''' END +

---- CASE WHEN @_technology IS NOT NULL THEN '' AND gis.technology IN ('' + @_technology + '')'' ELSE '''' END +

-- --CASE WHEN @_generation_state IS NOT NULL THEN '' AND gis.Generation_state IN ('' + @_generation_state + '')'' ELSE '''' END +
...
SELECT max(books.stra_id) stra_id,

max(books.sub_id) sub_id,

max(books.book_id) book_id,

max(books.sub_name) sub,

max(books.stra_name) stra,

max(books.book_name) book,

max(sb1.source_book_id) book_identifier1_id,

max(sb2.source_book_id)  book_identifier2_id,

max(sb3.source_book_id)  book_identifier3_id,

max(sb4.source_book_id) book_identifier4_id,
...
CAST(max(sdh.deal_date) as DATE) deal_date,
...
max(sdty.source_deal_type_name) + CASE WHEN max(ssds.source_system_id) = 2 THEN '''' ELSE ''.'' + max(ssds.source_system_name) END deal_sub_type_name,

CASE WHEN max(sdh.physical_financial_flag) = ''p'' THEN ''Physical'' ELSE ''Financial'' END header_physical_financial_flag,

max(sintp.code) internal_portfolio_name,

max(sdh.internal_portfolio_id) internal_portfolio_id,	   	   

max(st.source_trader_id) [trader_id],

max(sc_broker.counterparty_name) [broker_name],

max(sdh.[create_user]) create_user,

max(sdh.[create_ts]) create_ts,

max(sdh.[update_user]) update_user,

max(sdh.[update_ts]) update_ts,

max(books.logical_name) sub_book,

CAST(max(sdh.entire_term_start) AS DATE) entire_term_start,

CAST (max(sdh.entire_term_end) AS DATE) entire_term_end,

max(sdh.internal_deal_type_value_id) internal_deal_type_value_id,

max(idtdt.internal_deal_type_subtype_type) internal_deal_type,

max(sdh.internal_deal_subtype_value_id) internal_deal_subtype_value_id,

max(idtsdt.internal_deal_type_subtype_type) internal_deal_subtype,

max(sdht.template_name) [template],

max(sdht.template_id) template_id,

CASE WHEN max(sdh.header_buy_sell_flag) = ''b'' THEN ''Buy'' WHEN max(sdh.header_buy_sell_flag) = ''s'' THEN ''Sell'' ELSE max(sdh.header_buy_sell_flag) END header_buy_sell_flag,

max(sc.source_counterparty_id) [counterparty_id],

max(sdv_deal_status.value_id) [deal_status_id],

max(sdv_deal_status.code) [deal_status],
...
max(sdv_profile.code) [internal_desk_name],

max(books.[sub_book_id]) sub_book_id,	   
...
ISNULL(max(sml.Location_Name), max(spcd.curve_name)) [location],

max(sml.source_minor_location_id) [location_id],

CASE WHEN sdd.buy_sell_flag = ''b'' THEN ''Buy'' ELSE ''Sell'' END [buy_sell_flag],

CASE WHEN sdd.buy_sell_flag= ''b'' THEN sum(sdd.deal_volume) ELSE (sum(sdd.deal_volume) * - 1) END [volume],

max(su.uom_name) [volume_uom],	 

CASE WHEN sdd.buy_sell_flag = ''b'' THEN sum(sdd.total_volume) ELSE (sum(sdd.total_volume) * - 1) END [total_volume],

max(su_pos.uom_name) [position_uom],

COALESCE(max(sdd.position_uom),max(spcd.display_uom_id), max(spcd.uom_id)) [position_uom_id],

CASE WHEN max(fe.formula_source_type) = ''u'' THEN dbo.FNAFormulaFormat(max(fes.formula_sql), ''r'') ELSE dbo.FNAFormulaFormat(max(fe.formula), ''r'') END [formula],

CASE WHEN max(fe.formula) IS NOT NULL THEN NULL ELSE max(spcd_formula.curve_name) END [formula_curve],

	

CASE WHEN max(sdd.fixed_float_leg) = ''t'' THEN ''Float'' WHEN max(sdd.fixed_float_leg) = ''f'' THEN ''Fixed'' ELSE NULL END [fixed_float],

	   

CASE WHEN max(sdd.physical_financial_flag) = ''p'' THEN ''Physical'' ELSE ''Financial'' END [physical_financial_flag],

CASE WHEN max(sdd.deal_volume_frequency) = ''h'' THEN ''Hourly'' WHEN max(sdd.deal_volume_frequency) = ''d'' THEN ''Daily'' WHEN max(sdd.deal_volume_frequency) = ''m'' THEN ''Monthly'' WHEN max(sdd.deal_volume_frequency) = ''t'' THEN ''Term'' WHEN max(sdd.deal_volume_frequency) = ''a'' THEN ''Annually'' WHEN max(sdd.deal_volume_frequency) = ''x'' THEN ''15 Minutes'' WHEN max(sdd.deal_volume_frequency) = ''y'' THEN ''30 Minutes'' END [volume_frequency],

	  

CASE WHEN sdd.buy_sell_flag = ''b'' THEN sum(sdd.contractual_volume) ELSE (sum(sdd.contractual_volume) * - 1) END [contractual_volume],

max(sdd.contractual_uom_id) contractual_uom_id,

max(su_cont.uom_name) [contractual_uom_name],

CASE WHEN sdd.buy_sell_flag = ''b'' THEN sum(sdd.actual_volume) ELSE (sum(sdd.actual_volume) * - 1) END [actual_volume],	

max(sdd.deal_detail_description) deal_detail_description,

CASE WHEN sdd.buy_sell_flag = ''b'' THEN max(mgd.bookout_split_volume) ELSE (max(mgd.bookout_split_volume) * - 1) END [match_volume],

max(sdg.source_deal_groups_name) [deal_group_name],

max(sdd.status) [deal_detail_status_id],

max(sdv_dds.code) [deal_detail_status],

max(sdh.block_type) block_type,

max(sdh.deal_reference_type_id) deal_reference_type_id,	   

CASE max(sdh.term_frequency) WHEN ''m'' THEN ''Monthly'' WHEN ''q'' THEN ''Quarterly'' WHEN ''h'' THEN ''Hourly'' WHEN ''s'' THEN ''Semi-Annually'' WHEN ''a'' THEN ''Annually'' WHEN ''d'' THEN ''Daily'' WHEN ''z'' THEN ''term'' END [term_frequency_name],

max(sdh.term_frequency) term_frequency,

max(sdh.internal_counterparty) internal_counterparty,

max(sc_internal.counterparty_name) [internal_counterparty_name],	   	   	   	  	   

CASE WHEN sdd.buy_sell_flag = ''b'' THEN sum(sdd.schedule_volume) ELSE (sum(sdd.schedule_volume) * - 1) END [schedule_volume],
...
DAY(max(sdd.term_start)) term_start_day,

MONTH(max(sdd.term_start)) term_start_month,

DATENAME(m,max(sdd.term_start)) term_start_month_name,

DATEPART(q,max(sdd.term_start)) term_start_month_quarter,

YEAR(max(sdd.term_start)) term_start_year,

max(books.sub_book_group1) sub_book_group1_id,

max(books.sub_book_group2) sub_book_group2_id,

max(books.sub_book_group3) sub_book_group3_id,

max(books.sub_book_group4) sub_book_group4_id,
...
CASE WHEN max(sdd.status) = 25008 THEN ''Forward'' WHEN max(sdd.status) = 25002 THEN ''Expired'' WHEN max(sdd.status) IN (25003, 25004) THEN ''Actual'' END forward_actual,

max(sdv_pc.code) product_classification,

transaction_classification,

CASE WHEN max(temp_sdd.match_status) = ''u'' THEN ''Unmatched'' WHEN max(temp_sdd.match_status) = ''m'' THEN ''Matched'' WHEN max(temp_sdd.match_status) = ''p'' THEN ''Partially Matched'' END match_status,

CASE WHEN max(sdh.match_type) = ''m'' THEN ''Generation Month'' WHEN max(sdh.match_type) = ''y'' THEN ''Vintage Year'' WHEN max(sdh.match_type) = ''f'' THEN ''FIFO'' WHEN max(sdh.match_type) = ''g'' THEN ''Generation Year'' END match_type,

max(rg.code) generator,

max(technology.code) technology,

max(rg.name) facility_name,

max(gen_state.code) generation_state,

max(rg.owner) facility_owner,

CASE WHEN sdd.buy_sell_flag = ''b'' THEN sum(sdd.volume_left) ELSE (sum(sdd.volume_left) * - 1) END volume_left,

max(sdd.delivery_date) delivery_date_from,	

max(sdd.delivery_date_to) delivery_date_to,

max(sdd.actual_delivery_date) actual_delivery_date,

CASE WHEN max(sdh.is_environmental) = ''y'' THEN ''yes'' ELSE ''No'' END is_environmental,
...
max(sdd.vintage) AS vintage_id,

max(sdv_def_tier.code) default_tier,

ISNULL(max(sdv_vin.code), Year(max(sdd.term_start))) AS vintage_name
...
LEFT JOIN static_data_value sdv_vi ON sdv_vi.value_id = @_vintage_id

	AND sdv_vi.code = YEAR(sdd.term_start)

LEFT JOIN static_data_value sdv_vin ON sdv_vin.value_id = ISNULL(sdd.vintage, sdv_vi.value_id)
...
LEFT JOIN static_data_value ship ON ship.value_id = mgs.shipment_status AND ship.type_id = 47000	
...
LEFT JOIN dbo.SplitCommaSeperatedValues(@_default_tier_id) t ON t.item = COALESCE(sdh.reporting_tier_id, sdh.tier_value_id, rg.tier_type)

LEFT JOIN dbo.SplitCommaSeperatedValues(@_default_jurisdiction_id) s ON s.item = COALESCE(sdh.reporting_jurisdiction_id, sdh.state_value_id,rg.state_value_id)

LEFT JOIN dbo.SplitCommaSeperatedValues(@_is_environmental) e ON e.item = sdh.is_environmental

LEFT JOIN dbo.SplitCommaSeperatedValues(@_transaction_classification) tc ON tc.item = temp_sdh.transaction_classification

--LEFT JOIN #temp_gis_certificate_data gisc on gisc.source_deal_detail_id = sdd.source_deal_detail_id 

LEFT JOIN dbo.SplitCommaSeperatedValues(@_generation_state) gse ON gse.item = COALESCE(temp_sdd.gen_state_value_id, ''-1'')

LEFT JOIN dbo.SplitCommaSeperatedValues(@_technology) tech ON tech.item = COALESCE(temp_sdd.technology, ''-1'')
...
WHERE ((sdh.deal_date <= CAST(@_as_of_date AS DATETIME) AND sdd.term_end >= CAST(@_as_of_date AS DATETIME)) OR @_as_of_date IS NULL)
	...
	AND IIF(@_match_status IS NULL, ''-1'', temp_sdd.match_status) = IIF(@_match_status IS NULL, ''-1'', j.item)

	AND IIF(@_default_tier_id IS NULL, ''-1'', COALESCE(sdh.reporting_tier_id, sdh.tier_value_id, rg.tier_type)) = IIF(@_default_tier_id IS NULL, ''-1'', t.item)

	AND IIF(@_vintage_id IS NULL, ''-1'', sdv_vin.value_id) = IIF(@_vintage_id IS NULL, ''-1'', @_vintage_id)

	AND IIF(@_default_jurisdiction_id IS NULL, ''-1'', COALESCE(sdh.reporting_jurisdiction_id, sdh.state_value_id,rg.state_value_id)) = IIF(@_default_jurisdiction_id IS NULL, ''-1'', s.item)

	AND IIF(@_is_environmental IS NULL, ''-1'', ISNULL(sdh.is_environmental, ''n'')) = IIF(@_is_environmental IS NULL, ''-1'', e.item)

	AND IIF(@_transaction_classification IS NULL, ''-1'', ISNULL(temp_sdh.transaction_classification, ''-1'')) = IIF(@_transaction_classification IS NULL, ''-1'', tc.item)

	AND IIF(@_generation_state IS NULL, ''-1'', COALESCE(temp_sdd.gen_state_value_id,''-1'')) = IIF(@_generation_state IS NULL, ''-1'', gse.item)

	AND IIF(@_technology IS NULL, ''-1'', COALESCE(temp_sdd.technology,''-1'')) = IIF(@_technology IS NULL, ''-1'', tech.item)

	group by 

	ISNULL(sdv_vin.code, Year((sdd.term_start))),

	sdv_def_jurisdiction.code,

	COALESCE(sdh.reporting_tier_id, sdh.tier_value_id, rg.tier_type),

	COALESCE(sdh.reporting_jurisdiction_id, sdh.state_value_id,rg.state_value_id),

	transaction_classification,

	sdd.buy_sell_flag', report_id = @report_id_data_source_dest,
	system_defined = '1'
	,category = '106500' 
	WHERE [name] = 'Rec Summary View'
		AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1)
	...
	IF EXISTS (SELECT 1 
	           ...
	           WHERE ds.[name] = 'Rec Summary View'
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'create_ts_from' AS [name], 'Created Date From' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 5 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,0 AS column_template, 0 AS key_column, 0 AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Rec Summary View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'create_ts_to' AS [name], 'Created DateTo' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 5 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,0 AS column_template, 0 AS key_column, 0 AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Rec Summary View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'curve_id' AS [name], 'Index ID' AS ALIAS, NULL AS reqd_param, 7 AS widget_id, 4 AS datatype_id, 'browse_curve' AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, 0 AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Rec Summary View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		SET alias = 'Environmental'
			   , reqd_param = NULL, widget_id = 2, datatype_id = 5, param_data_source = 'SELECT ''y'' id, ''Yes'' code UNION ALL SELECT ''n'', ''No''', param_default_value = 'y', append_filter = NULL, tooltip = NULL, column_template = 0, key_column = 0, required_filter = 0
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
		WHERE ds.[name] = 'Rec Summary View'
	...
	BEGIN
		...
		SET alias = 'Match Status'
			   , reqd_param = NULL, widget_id = 2, datatype_id = 5, param_data_source = 'SELECT ''m'' id, ''Matched'' code UNION ALL SELECT ''p'', ''Partially Matched'' UNION ALL SELECT ''u'', ''Unmatched''', param_default_value = NULL, append_filter = NULL, tooltip = NULL, column_template = 0, key_column = 0, required_filter = 0
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
		WHERE ds.[name] = 'Rec Summary View'
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'position_uom_id' AS [name], 'Position UOM' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 4 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Rec Summary View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		SET alias = 'Product Classification'
			   , reqd_param = NULL, widget_id = 2, datatype_id = 5, param_data_source = 'SELECT value_id,' + CHAR(10) + '' + CHAR(9) + '   code' + CHAR(10) + 'FROM static_data_value ' + CHAR(10) + 'WHERE type_id = 107400', param_default_value = NULL, append_filter = NULL, tooltip = NULL, column_template = 0, key_column = 0, required_filter = 0
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
		WHERE ds.[name] = 'Rec Summary View'
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'product_classification' AS [name], 'Product Classification' AS ALIAS, NULL AS reqd_param, 2 AS widget_id, 5 AS datatype_id, 'SELECT value_id,' + CHAR(10) + '' + CHAR(9) + '   code' + CHAR(10) + 'FROM static_data_value ' + CHAR(10) + 'WHERE type_id = 107400' AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,0 AS column_template, 0 AS key_column, 0 AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Rec Summary View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'source_deal_type_id' AS [name], 'Deal Type ID' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 4 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, 0 AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Rec Summary View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		SET alias = 'Technology'
			   , reqd_param = NULL, widget_id = 2, datatype_id = 5, param_data_source = 'SELECT value_id,' + CHAR(10) + '' + CHAR(9) + '   code' + CHAR(10) + 'FROM static_data_value ' + CHAR(10) + 'WHERE type_id = 10009', param_default_value = NULL, append_filter = NULL, tooltip = NULL, column_template = 0, key_column = 0, required_filter = 0
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
		WHERE ds.[name] = 'Rec Summary View'
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'technology' AS [name], 'Technology' AS ALIAS, NULL AS reqd_param, 2 AS widget_id, 5 AS datatype_id, 'SELECT value_id,' + CHAR(10) + '' + CHAR(9) + '   code' + CHAR(10) + 'FROM static_data_value ' + CHAR(10) + 'WHERE type_id = 10009' AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,0 AS column_template, 0 AS key_column, 0 AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Rec Summary View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		UPDATE dsc  
		SET alias = 'Trader ID'
			   , reqd_param = NULL, widget_id = 2, datatype_id = 4, param_data_source = 'SELECT source_trader_id, trader_name FROM dbo.source_traders  ORDER BY trader_name', param_default_value = NULL, append_filter = NULL, tooltip = NULL, column_template = 2, key_column = 0, required_filter = 0
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
		WHERE ds.[name] = 'Rec Summary View'
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'update_ts_from' AS [name], 'Updated Date From' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 5 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,0 AS column_template, 0 AS key_column, 0 AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Rec Summary View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'update_ts_to' AS [name], 'Updated Date To' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 5 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,0 AS column_template, 0 AS key_column, 0 AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Rec Summary View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		SET alias = 'Vintage ID'
			   , reqd_param = NULL, widget_id = 2, datatype_id = 5, param_data_source = 'SELECT value_id,     code  FROM static_data_value WHERE type_id = 10092', param_default_value = NULL, append_filter = NULL, tooltip = NULL, column_template = 2, key_column = 0, required_filter = 0
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
		WHERE ds.[name] = 'Rec Summary View'
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'vintage_name' AS [name], 'Vintage' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 5 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,0 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Rec Summary View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		SET alias = 'Deal Detail Status ID'
			   , reqd_param = NULL, widget_id = 2, datatype_id = 4, param_data_source = 'SELECT value_id,' + CHAR(10) + '' + CHAR(9) + '   code' + CHAR(10) + 'FROM static_data_value ' + CHAR(10) + 'WHERE value_id IN (25003, 25004, 25002, 25008)', param_default_value = NULL, append_filter = NULL, tooltip = NULL, column_template = 2, key_column = 0, required_filter = 0
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
		WHERE ds.[name] = 'Rec Summary View'
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'deal_detail_status_id' AS [name], 'Deal Detail Status ID' AS ALIAS, NULL AS reqd_param, 2 AS widget_id, 4 AS datatype_id, 'SELECT value_id,' + CHAR(10) + '' + CHAR(9) + '   code' + CHAR(10) + 'FROM static_data_value ' + CHAR(10) + 'WHERE value_id IN (25003, 25004, 25002, 25008)' AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, 0 AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Rec Summary View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		SET alias = 'Default Jurisdiction ID'
			   , reqd_param = NULL, widget_id = 2, datatype_id = 4, param_data_source = 'EXEC spa_staticDataValues @flag=h, @type_id=10002', param_default_value = NULL, append_filter = NULL, tooltip = NULL, column_template = 2, key_column = 0, required_filter = 0
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
		WHERE ds.[name] = 'Rec Summary View'
	...
	BEGIN
		...
		SET alias = 'Default Tier ID'
			   , reqd_param = NULL, widget_id = 2, datatype_id = 4, param_data_source = 'EXEC spa_staticDataValues @flag=h, @type_id=15000', param_default_value = NULL, append_filter = NULL, tooltip = NULL, column_template = 2, key_column = 0, required_filter = 0
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
		WHERE ds.[name] = 'Rec Summary View'
	...
	BEGIN
		...
		SET alias = 'Generation State'
			   , reqd_param = NULL, widget_id = 2, datatype_id = 5, param_data_source = 'SELECT value_id,' + CHAR(10) + '' + CHAR(9) + '   code' + CHAR(10) + 'FROM static_data_value ' + CHAR(10) + 'WHERE type_id = 10016', param_default_value = NULL, append_filter = NULL, tooltip = NULL, column_template = 0, key_column = 0, required_filter = 0
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
		WHERE ds.[name] = 'Rec Summary View'
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'generation_state' AS [name], 'Generation State' AS ALIAS, NULL AS reqd_param, 2 AS widget_id, 5 AS datatype_id, 'SELECT value_id,' + CHAR(10) + '' + CHAR(9) + '   code' + CHAR(10) + 'FROM static_data_value ' + CHAR(10) + 'WHERE type_id = 10016' AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,0 AS column_template, 0 AS key_column, 0 AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Rec Summary View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		UPDATE dsc  
		SET alias = 'Generator'
			   , reqd_param = NULL, widget_id = 2, datatype_id = 5, param_data_source = 'SELECT generator_id,' + CHAR(10) + '' + CHAR(9) + '   code' + CHAR(10) + 'FROM rec_generator ' + CHAR(10) + 'ORDER BY code', param_default_value = NULL, append_filter = NULL, tooltip = NULL, column_template = 0, key_column = 0, required_filter = 0
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
		WHERE ds.[name] = 'Rec Summary View'
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'header_buy_sell_flag' AS [name], 'Buy/Sell' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 5 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,0 AS column_template, 0 AS key_column, 0 AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Rec Summary View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
