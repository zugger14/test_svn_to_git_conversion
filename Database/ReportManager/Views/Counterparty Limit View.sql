BEGIN TRY
	...
	declare @new_ds_alias varchar(10) = 'SCLV'
	/** IF DATA SOURCE ALIAS ALREADY EXISTS ON DESTINATION, RAISE ERROR **/
	if exists(select top 1 1 from data_source where alias = 'SCLV' and name <> 'Counterparty Limit View')
	begin
		select top 1 @new_ds_alias = 'SCLV' + cast(s.n as varchar(5))
		from seq s
		left join data_source ds on ds.alias = 'SCLV' + cast(s.n as varchar(5))
	...
	IF NOT EXISTS (SELECT 1 
	           FROM data_source 
	           WHERE [name] = 'Counterparty Limit View'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
	AND NOT EXISTS (SELECT 1 FROM map_function_category WHERE [function_name] = 'Counterparty Limit View' AND '106500' = '106501') 
	BEGIN
		INSERT INTO data_source([type_id], [name], [alias], [description], [tsql], report_id, system_defined,category)
		SELECT TOP 1 1 AS [type_id], 'Counterparty Limit View' AS [name], @new_ds_alias AS ALIAS, 'Standard Counterparty Credit Limit View' AS [description],null AS [tsql], @report_id_data_source_dest AS report_id,'1' AS [system_defined]
			,'106500' AS [category]
	END

	UPDATE data_source
	SET alias = @new_ds_alias, description = 'Standard Counterparty Credit Limit View'
...
SET @_sql = ''SELECT scl.source_counterparty_id [counterparty_id]

		  ,scl.counterparty_id [counterparty_code]

		  ,scl.counterparty_name [counterparty_name]

		  ,ccl.internal_counterparty_id [internal_counterparty_id]

		  ,cptyi.counterparty_name [internal_counterparty_name]

		  ,cptyi.counterparty_id [internal_counterparty_code]

		  ,ccl.contract_id [contract_id]

		  ,cg.contract_name [contract]

		  ,ccl.credit_limit [limit_provided]

		  ,ccl.credit_limit_to_us [limit_received]

		  ,ccl.max_threshold [max_threshold_percentage]

		  ,ccl.min_threshold [min_threshold_percentage]

		  ,ccl.effective_Date [effective_date]

		  ,DAY(ccl.effective_Date) [effective_date_day]

		  ,MONTH(ccl.effective_Date) [effective_date_month]

		  ,CONVERT(VARCHAR(7) ,ccl.effective_Date ,20) [effective_date_year_month]

		  ,YEAR(ccl.effective_Date) [effective_date_year]
		...
		END + '' [as_of_date]
		  ...
		  ,cca.threshold_provided [threshold_amount_provided]

		  ,cca.threshold_received [Threshold Amount Received]
	...
	FROM   counterparty_credit_limits ccl
		   ...
		   LEFT JOIN source_currency sc
				...
				ON  sc.source_currency_id = ccl.currency_id

		   INNER JOIN counterparty_credit_info cci

				ON  cci.counterparty_id = ccl.counterparty_id

		   INNER JOIN source_counterparty scl

				ON  scl.source_counterparty_id = ccl.Counterparty_id

		   LEFT JOIN contract_group cg

				ON  cg.contract_id = ccl.contract_id

		   LEFT JOIN source_counterparty cptyi

				ON  cptyi.source_counterparty_id = ccl.internal_counterparty_id

			LEFT JOIN static_data_value acc 

				ON acc.value_id=cci.account_status

			LEFT JOIN counterparty_contract_address cca ON cca.counterparty_id = ccl.counterparty_id

				AND ISNULL(cca.contract_id, 0) = ISNULL(ccl.contract_id, 0) 

				AND ISNULL(cca.internal_counterparty_id, 0) = ISNULL(ccl.internal_counterparty_id, 0)

	WHERE  1 = 1'' + CASE 

		WHEN @_as_of_date IS NOT NULL

			THEN '' AND ISNULL(ccl.effective_date, ''''1900-01-01'''') <= '''''' + @_as_of_date + ''''''''
		...
		WHEN @_counterparty_id IS NOT NULL
			...
			THEN '' AND scl.source_counterparty_id IN ('' + @_counterparty_id + '')''
		...
		WHEN @_internal_counterparty_id IS NOT NULL
			...
			THEN '' AND ccl.internal_counterparty_id IN ('' + @_internal_counterparty_id + '')''
		...
		WHEN @_contract_id IS NOT NULL
			...
			THEN '' AND ccl.contract_id IN ('' + @_contract_id + '')''

		ELSE ''''

		END + CASE 

		WHEN @_account_status_id IS NOT NULL

			THEN '' AND cci.account_status IN ('' + @_account_status_id + '')''
...
EXEC (@_sql)', report_id = @report_id_data_source_dest,
	...
	IF EXISTS (SELECT 1 
	           ...
	           WHERE ds.[name] = 'Counterparty Limit View'
	...
	BEGIN
		...
		SET alias = 'Internal Counterparty Id'
			   , reqd_param = 1, widget_id = 9, datatype_id = 4, param_data_source = 'SELECT source_counterparty_id, counterparty_name' + CHAR(10) + '  FROM source_counterparty WHERE int_ext_flag=''i''', param_default_value = NULL, append_filter = 0, tooltip = NULL, column_template = 2, key_column = 1, required_filter = 0
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
		WHERE ds.[name] = 'Counterparty Limit View'
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'internal_counterparty_id' AS [name], 'Internal Counterparty Id' AS ALIAS, 1 AS reqd_param, 9 AS widget_id, 4 AS datatype_id, 'SELECT source_counterparty_id, counterparty_name' + CHAR(10) + '  FROM source_counterparty WHERE int_ext_flag=''i''' AS param_data_source, NULL AS param_default_value, 0 AS append_filter, NULL  AS tooltip,2 AS column_template, 1 AS key_column, 0 AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Counterparty Limit View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'effective_date_day' AS [name], 'Effective Date Day' AS ALIAS, 0 AS reqd_param, 1 AS widget_id, 4 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, 1 AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Counterparty Limit View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'effective_date_month' AS [name], 'Effective Date Month' AS ALIAS, 0 AS reqd_param, 1 AS widget_id, 4 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, 1 AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Counterparty Limit View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'effective_date_year' AS [name], 'Effective Date Year' AS ALIAS, 0 AS reqd_param, 1 AS widget_id, 4 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, 1 AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Counterparty Limit View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'effective_date_year_month' AS [name], 'Effective Date Year Month' AS ALIAS, 0 AS reqd_param, 1 AS widget_id, 5 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, 1 AS append_filter, NULL  AS tooltip,0 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Counterparty Limit View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	IF EXISTS (SELECT 1 
	           ...
	           INNER JOIN data_source ds on ds.data_source_id = dsc.source_id 
	           WHERE ds.[name] = 'Counterparty Limit View'
	            AND dsc.name =  'Threshold Amount Received'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
	BEGIN
		UPDATE dsc  
		SET alias = 'Threshold Amount Received'
			   , reqd_param = 0, widget_id = 1, datatype_id = 3, param_data_source = NULL, param_default_value = NULL, append_filter = 1, tooltip = NULL, column_template = 2, key_column = 0, required_filter = NULL
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'Threshold Amount Received' AS [name], 'Threshold Amount Received' AS ALIAS, 0 AS reqd_param, 1 AS widget_id, 3 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, 1 AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Counterparty Limit View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	IF EXISTS (SELECT 1 
	           ...
	           INNER JOIN data_source ds on ds.data_source_id = dsc.source_id 
	           WHERE ds.[name] = 'Counterparty Limit View'
	            AND dsc.name =  'threshold_amount_provided'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
	BEGIN
		UPDATE dsc  
		SET alias = 'Threshold Amount Provided'
			   , reqd_param = 0, widget_id = 1, datatype_id = 3, param_data_source = NULL, param_default_value = NULL, append_filter = 1, tooltip = NULL, column_template = 2, key_column = 0, required_filter = NULL
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'threshold_amount_provided' AS [name], 'Threshold Amount Provided' AS ALIAS, 0 AS reqd_param, 1 AS widget_id, 3 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, 1 AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Counterparty Limit View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		SET alias = 'Account Status ID'
			   , reqd_param = 0, widget_id = 9, datatype_id = 4, param_data_source = 'EXEC spa_StaticDataValues @flag=''h'', @type_id=10082', param_default_value = NULL, append_filter = 1, tooltip = NULL, column_template = 2, key_column = 0, required_filter = 0
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
		WHERE ds.[name] = 'Counterparty Limit View'
