BEGIN TRY
	...
	declare @new_ds_alias varchar(10) = 'PWFPV'
	/** IF DATA SOURCE ALIAS ALREADY EXISTS ON DESTINATION, RAISE ERROR **/
	if exists(select top 1 1 from data_source where alias = 'PWFPV' and name <> '15 Mins Position with Fixed Price View')
	begin
		select top 1 @new_ds_alias = 'PWFPV' + cast(s.n as varchar(5))
		from seq s
		left join data_source ds on ds.alias = 'PWFPV' + cast(s.n as varchar(5))
	...
	IF NOT EXISTS (SELECT 1 
	           FROM data_source 
	           WHERE [name] = '15 Mins Position with Fixed Price View'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
	AND NOT EXISTS (SELECT 1 FROM map_function_category WHERE [function_name] = '15 Mins Position with Fixed Price View' AND '106500' = '106501') 
	BEGIN
		INSERT INTO data_source([type_id], [name], [alias], [description], [tsql], report_id, system_defined,category)
		SELECT TOP 1 1 AS [type_id], '15 Mins Position with Fixed Price View' AS [name], @new_ds_alias AS ALIAS, '' AS [description],null AS [tsql], @report_id_data_source_dest AS report_id,'0' AS [system_defined]
			,'106500' AS [category]
	END
...
DECLARE @_sub_id VARCHAR(MAX) = NULL
    ...
    , @_as_of_date VARCHAR(10) = NULL --''2020-09-06''

    , @_source_deal_header_id VARCHAR(1000) = NULL --''43671, 174035''
    ...
    , @_term_start VARCHAR(10) = NULL --''2021-01-01''

    , @_term_end VARCHAR(10) = NULL --''2021-01-01''

    , @_deal_type VARCHAR(500) = NULL

    , @_deal_sub_type VARCHAR(500) = NULL
    ...
    , @_physical_financial_flag NCHAR(6)=''p''
    ...
    ,@_reporting_group1 VARCHAR(1000)=50000367
...
*/
    ...
    DECLARE 
	  ...
	  @_summary_option VARCHAR(6) =''x06d''   ---x=15 Mins,  (@_summary_option= ...s, @_group_by=s or @_summary_option= ...d, @_group_by=d)
...
', report_id = @report_id_data_source_dest,
	...
	IF EXISTS (SELECT 1 
	           ...
	           WHERE ds.[name] = '15 Mins Position with Fixed Price View'
	...
	BEGIN
		...
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = '15 Mins Position with Fixed Price View'
	...
	FROM data_source_column dsc 
	INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
		AND ds.[name] = '15 Mins Position with Fixed Price View'
		AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1)
