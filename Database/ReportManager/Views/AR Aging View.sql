BEGIN TRY
	...
	IF NOT EXISTS (SELECT 1 
	           FROM data_source 
	           WHERE [name] = 'AR Aging View'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
	BEGIN
		INSERT INTO data_source([type_id], [name], [alias], [description], [tsql], report_id)
		SELECT TOP 1 1 AS [type_id], 'AR Aging View' AS [name], 'SARR' AS ALIAS, 'Standard AR Aging View' AS [description],null AS [tsql], @report_id_data_source_dest AS report_id
	END

	UPDATE data_source
	SET alias = 'SARR', description = 'Standard AR Aging View'
...
where civv.prod_date <= ''@as_of_date''

AND (''NULL'' = ''@counterparty_id'' OR sc.source_counterparty_id IN (@counterparty_id))
...
MAX((CASE
	...
	WHEN ISNULL(invoice_due_date,''1900-01-01'') >= ''@as_of_date'' THEN '' Current''

	WHEN ABS(datediff(day, ''@as_of_date'',invoice_due_date)) BETWEEN 1 AND 30 THEN ''1-30PD''

	WHEN ABS(datediff(day, ''@as_of_date'',invoice_due_date)) BETWEEN 31 AND 60 THEN ''31-60PD''

	WHEN ABS(datediff(day, ''@as_of_date'',invoice_due_date)) BETWEEN 61 AND 90 THEN ''61-90PD''
...
as_of_date,counterparty,counterparty_id,parent_counterparty,
...
WHERE  gross_exposure > 0 and amount > 0

order by as_of_date desc', report_id = @report_id_data_source_dest 
	WHERE [name] = 'AR Aging View'
		AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1)
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'invoice_id' AS [name], 'Invoice ID' AS ALIAS, 0 AS reqd_param, 1 AS widget_id, 4 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, 1 AS append_filter, NULL  AS tooltip,2 AS column_template, 1 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'AR Aging View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
