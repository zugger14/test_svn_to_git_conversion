BEGIN TRY
	...
	declare @new_ds_alias varchar(10) = 'fmpvss'
	/** IF DATA SOURCE ALIAS ALREADY EXISTS ON DESTINATION, RAISE ERROR **/
	if exists(select top 1 1 from data_source where alias = 'fmpvss' and name <> '15 Mins position detail with shipper codes')
	begin
		select top 1 @new_ds_alias = 'fmpvss' + cast(s.n as varchar(5))
		from seq s
		left join data_source ds on ds.alias = 'fmpvss' + cast(s.n as varchar(5))
	...
	IF NOT EXISTS (SELECT 1 
	           FROM data_source 
	           WHERE [name] = '15 Mins Position Detail with Shipper Codes'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
	AND NOT EXISTS (SELECT 1 FROM map_function_category WHERE [function_name] = '15 Mins Position Detail with Shipper Codes' AND '106500' = '106501') 
	BEGIN
		INSERT INTO data_source([type_id], [name], [alias], [description], [tsql], report_id, system_defined,category)
		SELECT TOP 1 1 AS [type_id], '15 Mins Position Detail with Shipper Codes' AS [name], @new_ds_alias AS ALIAS, '15 Mins Position Detail with Shipper Codes' AS [description],null AS [tsql], @report_id_data_source_dest AS report_id,'0' AS [system_defined]
			,'106500' AS [category]
	END

	UPDATE data_source
	SET alias = @new_ds_alias, description = '15 Mins Position Detail with Shipper Codes'
...
x04d: Volume Imbalance Report (Shipper Codes and Reporting Group1)

*/

    DECLARE 

	@_summary_option VARCHAR(6) =''x04d''   ---x=15 Mins,  (@_summary_option= ...s, @_group_by=s or @_summary_option= ...d, @_group_by=d)
...
EXEC dbo.spa_position_report 
	...
	IF EXISTS (SELECT 1 
	           ...
	           WHERE ds.[name] = '15 Mins Position Detail with Shipper Codes'
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'buy_sell_flag' AS [name], 'Buy Sell Flag' AS ALIAS, NULL AS reqd_param, 2 AS widget_id, 5 AS datatype_id, 'SELECT ''b'' Value, ''Buy'' Label UNION SELECT ''s'', ''Sell''' AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,0 AS column_template, 0 AS key_column, 0 AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = '15 Mins Position Detail with Shipper Codes'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
