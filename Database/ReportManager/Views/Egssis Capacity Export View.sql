BEGIN TRY
	...
	declare @new_ds_alias varchar(10) = 'ECE'
	/** IF DATA SOURCE ALIAS ALREADY EXISTS ON DESTINATION, RAISE ERROR **/
	if exists(select top 1 1 from data_source where alias = 'ECE' and name <> 'Egssis Capacity Export View')
	begin
		select top 1 @new_ds_alias = 'ECE' + cast(s.n as varchar(5))
		from seq s
		left join data_source ds on ds.alias = 'ECE' + cast(s.n as varchar(5))
	...
	IF NOT EXISTS (SELECT 1 
	           FROM data_source 
	           WHERE [name] = 'Egssis Capacity Export View'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
	AND NOT EXISTS (SELECT 1 FROM map_function_category WHERE [function_name] = 'Egssis Capacity Export View' AND '106500' = '106501') 
	BEGIN
		INSERT INTO data_source([type_id], [name], [alias], [description], [tsql], report_id, system_defined,category)
		SELECT TOP 1 1 AS [type_id], 'Egssis Capacity Export View' AS [name], @new_ds_alias AS ALIAS, 'Egssis Capacity Export' AS [description],null AS [tsql], @report_id_data_source_dest AS report_id,'0' AS [system_defined]
			,'106500' AS [category]
	END

	UPDATE data_source
	SET alias = @new_ds_alias, description = 'Egssis Capacity Export'
...
SET @_convert_to_utc = nullif(isnull(@_convert_to_utc, nullif(''NULL'', replace(''@_convert_to_utc'', ''@_'', ''@''))), ''null'')
...
--CASE WHEN @_physical_financial_flag IS NOT NULL THEN '' WHERE vw.physical_financial_flag = '''''' + @_physical_financial_flag + '''''''' ELSE '''' END end
	...
	SET @_sql_final = ''
		...
		CREATE TABLE #temp_deal_udf_values (
			...
			[Direction] VARCHAR(100) COLLATE DATABASE_DEFAULT,
			[Capacity Type] VARCHAR(100) COLLATE DATABASE_DEFAULT
		)
		INSERT INTO #temp_deal_udf_values (source_deal_header_id, [Direction], [Capacity Type])
		SELECT source_deal_header_id, 
			CASE WHEN [Direction] = 1 THEN ''''Entry''''
				 WHEN [Direction] = 2 THEN ''''Exit''''
				 ELSE ''''''''
			END	
			,CASE WHEN [Capacity Type] = 1 THEN ''''Interruptible''''
				 WHEN [Capacity Type] = 2 THEN ''''Operational''''
				 WHEN [Capacity Type] = 3 THEN ''''Conditional''''
				 WHEN [Capacity Type] = 4 THEN ''''Firm''''
				 WHEN [Capacity Type] = 5 THEN ''''FuelGasThreshold''''
				 ELSE ''''''''	 
			END
		FROM (
			SELECT sdh.source_deal_header_id, udft.field_label, uddf.udf_value
			FROM (SELECT DISTINCT source_deal_header_id FROM #tmp_position_detail) sdh
		...
		PIVOT (MAX(a.udf_value) FOR a.Field_label IN ([Direction], [Capacity Type])) AS p
		SELECT  MAX(location) location
			  , MAX(location_group) location_type
			  , MAX(tpd.customer_duns_number) [counterparty]
			  , MAX([Deal Volume]) [value]
			  ...
			  , MAX(convert_to_uom_id) [convert_to_uom_id]
			  , '''''''' [booking_reference]
			  , MAX(tduv.[Capacity Type]) [type]
			  , MAX(tduv.[Direction]) [direction]
			  , MAX(postion_uom) [unit]
			  , MAX(tpd.location_id) [location_id]
			  , tpd.source_deal_header_id [source_deal_header_id]
		...
		LEFT JOIN #temp_deal_udf_values tduv
			ON tduv.source_deal_header_id = tpd.source_deal_header_id
		OUTER APPLY (SELECT customer_duns_number 
					 FROM source_counterparty
					 WHERE counterparty_id = ''''Gasum AB''''  --Hardcorded for demo
		) sc_dns
		GROUP BY tpd.source_deal_header_id''
...
exec(
	...
	IF EXISTS (SELECT 1 
	           ...
	           WHERE ds.[name] = 'Egssis Capacity Export View'
	...
	BEGIN
		...
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Egssis Capacity Export View'
	...
	IF EXISTS (SELECT 1 
	           ...
	           INNER JOIN data_source ds on ds.data_source_id = dsc.source_id 
	           WHERE ds.[name] = 'Egssis Capacity Export View'
	            AND dsc.name =  'booking_reference'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
	BEGIN
		UPDATE dsc  
		SET alias = 'Booking Reference'
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'booking_reference' AS [name], 'Booking Reference' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 5 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,0 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Egssis Capacity Export View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		SET alias = 'Convert To Uom Id'
			   , reqd_param = NULL, widget_id = 2, datatype_id = 4, param_data_source = 'exec [spa_source_uom_maintain] ''s''', param_default_value = NULL, append_filter = NULL, tooltip = NULL, column_template = 2, key_column = 0, required_filter = 0
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
		WHERE ds.[name] = 'Egssis Capacity Export View'
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'direction' AS [name], 'Direction' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 5 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,0 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Egssis Capacity Export View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		SET alias = 'Convert To Utc'
			   , reqd_param = NULL, widget_id = 2, datatype_id = 5, param_data_source = 'SELECT ''y'' [value], ''Yes'' [code] UNION' + CHAR(10) + 'SELECT ''n'', ''No''', param_default_value = NULL, append_filter = NULL, tooltip = NULL, column_template = 0, key_column = 0, required_filter = NULL
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
		WHERE ds.[name] = 'Egssis Capacity Export View'
	...
	FROM data_source_column dsc 
	INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
		AND ds.[name] = 'Egssis Capacity Export View'
		AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1)
