...
physical_financial_flag varchar(2),hr1 FLOAT,hr2 FLOAT,hr3 FLOAT,hr4 FLOAT,hr5 FLOAT,hr6 FLOAT,hr7 FLOAT,hr8 FLOAT,
...
source_system_book_id4 INT,expiration_date DATETIME,is_fixedvolume VARCHAR(2),deal_status_id INT,deal_volume_frequency VARCHAR(2),term_end DATETIME)
...
LEFT JOIN	mv90_dst dst ON dst.[date] = unpvt.term_start and insert_delete = 'i' 
	WHERE unpvt.Hours<25 or (unpvt.Hours=25 and dst.[hour] is not null)
*/
BEGIN TRY
		BEGIN TRAN
	...
	BEGIN
		...
		, [tsql] = CAST('' AS VARCHAR(MAX)) + 'Please Copy the view code from the export file', report_id = @report_id_data_source_dest 
		WHERE [name] = 'Position View Hourly With Block Type'
			AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1)
	END	
	ELSE
	BEGIN
		...
		SELECT TOP 1 1 AS [type_id], 'Position View Hourly With Block Type' AS [name], 'PVHWBT' AS ALIAS, '' AS [description],'Please Copy the view code from the export file' AS [tsql], @report_id_data_source_dest AS report_id
	END 
	...
	BEGIN
		...
		SET alias = 'User Defined Block ID'
			   , reqd_param = 1, widget_id = 2, datatype_id = 4, param_data_source = 'select value_id,code from static_data_value where type_id = 15001', param_default_value = NULL, append_filter = 0, tooltip = NULL, column_template = 2, key_column = 0
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
		WHERE ds.[name] = 'Position View Hourly With Block Type'
	...
	BEGIN
		...
		SET alias = 'Commodity ID'
			   , reqd_param = 0, widget_id = 2, datatype_id = 4, param_data_source = 'SELECT source_commodity_id,commodity_id' + CHAR(10) + ' FROM  source_commodity', param_default_value = NULL, append_filter = 1, tooltip = NULL, column_template = 2, key_column = 0
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
		WHERE ds.[name] = 'Position View Hourly With Block Type'
