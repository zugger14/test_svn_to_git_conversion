BEGIN TRY
	...
	declare @new_ds_alias varchar(10) = 'EEEV'
	/** IF DATA SOURCE ALIAS ALREADY EXISTS ON DESTINATION, RAISE ERROR **/
	if exists(select top 1 1 from data_source where alias = 'EEEV' and name <> 'Egssis Entries Exporter View')
	begin
		select top 1 @new_ds_alias = 'EEEV' + cast(s.n as varchar(5))
		from seq s
		left join data_source ds on ds.alias = 'EEEV' + cast(s.n as varchar(5))
	...
	IF NOT EXISTS (SELECT 1 
	           FROM data_source 
	           WHERE [name] = 'Egssis Entries Exporter View'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
	AND NOT EXISTS (SELECT 1 FROM map_function_category WHERE [function_name] = 'Egssis Entries Exporter View' AND '106500' = '106501') 
	BEGIN
		INSERT INTO data_source([type_id], [name], [alias], [description], [tsql], report_id, system_defined,category)
		SELECT TOP 1 1 AS [type_id], 'Egssis Entries Exporter View' AS [name], @new_ds_alias AS ALIAS, 'Egssis Entries Exporter View' AS [description],null AS [tsql], @report_id_data_source_dest AS report_id,'0' AS [system_defined]
			,'106500' AS [category]
	END

	UPDATE data_source
	SET alias = @new_ds_alias, description = 'Egssis Entries Exporter View'
	, [tsql] = CAST('' AS VARCHAR(MAX)) + 'SET NOCOUNT ON
DECLARE @_summary_option CHAR(6) = null --''m'' --''d'' -------  ''d'' Detail, ''h'' =hourly,''x''/''y'' = 15/30 minute, q=quatar, a=annual
	,@_sub_id VARCHAR(1000) = NULL
	,@_stra_id VARCHAR(1000) = NULL
	,@_book_id VARCHAR(1000) = null
	,@_subbook_id VARCHAR(1000) = null--''3736''
	,@_as_of_date VARCHAR(20) = null--''2019-04-02''
	...
	,@_forecast_overrides_quantity CHAR(1)
	,@_convert_to_utc CHAR(1)
...
SET @_forecast_overrides_quantity = nullif(isnull(@_forecast_overrides_quantity, nullif(''@forecast_overrides_quantity'', replace(''@_forecast_overrides_quantity'', ''@_'', ''@''))), ''null'')
SET @_convert_to_utc = nullif(isnull(@_convert_to_utc, nullif(''@convert_to_utc'', replace(''@_convert_to_utc'', ''@_'', ''@''))), ''null'')
...
SET @_forecast_overrides_quantity = isnull(@_forecast_overrides_quantity, ''n'')
--hardcorded for demo
SELECT @_proxy_curve_view = ''n'',
	@_physical_financial_flag = NULL
--hardcorded for demo
...
WHERE user_login_id = @_user_login_id
...
CREATE TABLE #temp_deals ( source_deal_header_id int,physical_financial CHAR(1))
--print @_term_start
--print @_as_of_date
...
WHERE (
		fas_deal_type_value_id IS NULL
		OR fas_deal_type_value_id BETWEEN 400
			AND 401
		)  '' 
...
SET @_Sql_Select = ''
	INSERT INTO #temp_deals (source_deal_header_id,physical_financial)
SELECT sdh.source_deal_header_id,sdh.physical_financial_flag
...
WHERE 1=1  '' --sdh.source_deal_type_id <> 1177
		...
		WHEN @_deal_type IS NOT NULL
			THEN '' and sdh.source_deal_type_id ='' + @_deal_type
		ELSE ''''
		END + CASE 
		WHEN @_deal_sub_type IS NOT NULL
			THEN '' and sdh.deal_sub_type_type_id ='' + @_deal_sub_type
		ELSE ''''
		END + CASE 
...
FROM (
	SELECT DISTINCT tz.dst_group_value_id
		,isnull(spcd.block_define_id, nullif(@_baseload_block_define_id, ''NULL'')) block_define_id
		,s.term_start
		,s.term_end
	...
	+ CASE WHEN  @_deal_status IS NULL AND @_source_deal_header_id IS NULL THEN 
		...
		ELSE '''' END +''
			...
			left join dbo.source_deal_detail sdd on sdd.source_deal_header_id=vw.source_deal_header_id 
				and CASE WHEN vw.physical_financial_flag=''''f'''' and vw.is_fixedvolume=''''y'''' then COALESCE(sdd.formula_curve_id,lpi.curve_id,sdd.curve_id) else ISNULL(lpi.curve_id,sdd.curve_id) end=vw.curve_id 
...
begin
	set @_volume_clm=''
		...
		,   hb1.block_name
		, sdv_block_group.code [user_defined_block] ,sdv_block_group.value_id [user_defined_block_id],hb1.block_type_group_id ''
	SET @_rhpb_0= '' 
		select *,''
		...
		OUTER APPLY ( 
			SELECT SUM(hbt.volume_mult) hr_nos 
			FROM hour_block_term hbt 
			WHERE hbt.block_define_id =  p.block_define_id --AND YEAR(hbt.term_date) =  YEAR(p.term)   
				AND EOMONTH(hbt.term_date) =  EOMONTH(p.term)     
 		) hbt_sum 
		UNPIVOT
			(Volume for Hours IN
...
SELECT 
	...
	sml.Location_Description location,
   ...
   --from demo4
   (''+CASE WHEN @_convert_to_uom_id IS NOT NULL THEN '' cast(isnull(uc.conversion_factor,1) as numeric(21,16))*vw.volume'' else ''vw.volume'' end +'')/
...
INTO #tmp_position_detail
	FROM ''
		+case when  @_summary_option IN (''h'',''x'',''y'') then	''#unpvt''
			...
			''
			(
				SELECT *,[term] term_date,null [hr_nos] FROM #tmp_pos_detail_power
				union all 
				SELECT *,[term] term_date,null [hr_nos] FROM #tmp_pos_detail_gas
				UNION ALL
				SELECT *, NULL [hr_nos] from  #tmp_pos_detail_torage
				'' +  CASE WHEN @_include_storage = ''y'' THEN '' UNION ALL SELECT *, NULL [hr_nos] FROM  #injection_withdrawal_deals''
				ELSE '''' END + ''
			)''
		end +'' vw ''
SET @_rhpba1 = ''		
...
--CASE WHEN @_physical_financial_flag IS NOT NULL THEN '' WHERE vw.physical_financial_flag = '''''' + @_physical_financial_flag + '''''''' ELSE '''' END end
	...
	SET @_rhpb5 = ''
		UPDATE tp SET tp.position = tp.position * CASE WHEN tp.leg = 1 THEN abs(sdpdo.DELTA) ELSE abs(sdpdo.DELTA2) END,
			tp.best_avial_volume = tp.best_avial_volume * CASE WHEN tp.leg = 1 THEN abs(sdpdo.DELTA) ELSE abs(sdpdo.DELTA2) END,
			tp.delta = CASE WHEN tp.leg = 1 THEN sdpdo.DELTA ELSE sdpdo.DELTA2 END
		FROM #tmp_position_detail tp
		INNER JOIN source_deal_pnl_detail_options sdpdo ON sdpdo.source_deal_header_id = tp.source_deal_header_id
			AND sdpdo.as_of_date = tp.as_of_date
			AND sdpdo.term_start = tp.term_start '' 
	--from demo4
	SET @_sql_final = ''
		...
		UPDATE tpd
		SET term_end =  CASE WHEN tz.TIMEZONE_NAME = ''''(GMT +3:00) Eastern European Time'''' THEN DATEADD(HOUR, [hour], tpd.term_end)
							 WHEN tz.TIMEZONE_NAME = ''''(GMT +1:00 hour) Brussels, Copenhagen, Madrid, Paris'''' THEN DATEADD(HOUR, [hour] - 1, tpd.term_end)
							 ELSE tpd.term_end
						END
		FROM #tmp_position_detail tpd
		INNER JOIN source_minor_location sml
				ON sml.source_minor_location_id = tpd.[location_id]
		LEFT JOIN time_zones tz
			ON tz.TIMEZONE_ID = sml.time_zone

		SELECT ISNULL(MAX(sdv.code),'''''''') [grid]
		      ,CASE WHEN  MAX(location) = ''''21Z0000000000252'''' THEN ''''Border''''
			   ELSE ''''Hub'''' END [contract_type]
			  , CASE WHEN  MAX(location) = ''''21Y000000000078W'''' THEN ''''21X-FI-A-A0A0A-G''''
					 WHEN  MAX(location) = ''''21Y---A001A003-5'''' THEN ''''DS000068''''
				ELSE MAX(sc_dns.customer_duns_number) END [zone]
			  , MAX(location) location
			  , MAX(location_group) location_type
			  , CASE WHEN  MAX(location) = ''''21Y000000000078W'''' THEN ''''21X-FI-A-A0A0A-G''''
					 WHEN  MAX(location) = ''''21Y---A001A003-5'''' THEN ''''DS000068''''
				ELSE MAX(sc_dns.customer_duns_number) END [internal_shipper]
			  , MAX(tpd.customer_duns_number) [counterparty]
			  , '' +CASE WHEN  @_summary_option IN (''h'') THEN CASE WHEN ISNULL(@_convert_to_utc,''n'') = ''y'' THEN ''IIF(MAX(sml.time_zone) IS NOT NULL,dbo.FNAGetUTCTTime(term_end,MAX(sml.time_zone)),term_end)'' ELSE ''term_end'' END
				ELSE ''term_end'' END +'' [gas_day]
			  , '''''' + CASE WHEN @_forecast_overrides_quantity = ''n'' THEN ''false'' ELSE ''true'' END + '''''' [forecast_overrides_quantity]
			  , SUM(position) [value]
			  ...
			  , MAX(summary_option) [summary_option]
			  , '''''' + ISNULL(@_convert_to_utc,''n'') + '''''' [convert_to_utc]
			  , MAX(tz.TIMEZONE_NAME) timezone_name
		--[__batch_report__]
		FROM #tmp_position_detail tpd
		INNER JOIN source_minor_location sml
				ON sml.source_minor_location_id = tpd.[location_id]
		LEFT JOIN static_data_value sdv
			ON sdv.value_id = sml.grid_value_id
			AND sdv.type_id = 18000
		LEFT JOIN time_zones tz
			ON tz.TIMEZONE_ID = sml.time_zone
		OUTER APPLY (SELECT customer_duns_number 
					 FROM source_counterparty
					 WHERE counterparty_id = ''''Gasum OY''''  --Hardcorded for demo
		) sc_dns
		GROUP BY tpd.counterparty_id,term_end'' 
		+CASE WHEN  @_summary_option IN (''h'') THEN '',[hour]''
				ELSE '''' END +''
		''
...
WHERE 1 = 1
	AND CAST(td.movement_date_time AS DATE) <= '''''' + CONVERT(VARCHAR(10), @_as_of_date, 120) + ''''''
...
'' 
ELSE ''emply'' END 
SET @_storage_deals = ''SELECT sdd.source_deal_detail_id,ISNULL(lpi.curve_id,sdd.curve_id) source_curve_def_id,
ISNULL(sdd.detail_commodity_id,sdh.commodity_id) commodity_id,CONVERT(VARCHAR(10),sdd.term_start,120) term,0 Period,''''n'''' is_fixedvolume,sdd.physical_financial_flag,sdd.deal_volume_uom_id source_uom_id,su.uom_name UOM,sdh.counterparty_id,sdd.location_id,NULL fas_book_id,NULL proxy_curve_id,0 breakdown, NULL block_define_id,
...
--select (@_rhpb+@_volume_clm+@_commodity_str+ ''; 
--	''+@_rhpb+@_volume_clm+ @_commodity_str1 +'';
--	''+@_storage_deals+'';
--	''+@_injection_withdrawal_deals+ '';
--	''+@_rhpb_0 +@_rhpb1 +@_rhpb2+@_rhpba1+@_rhpb3+@_rhpb4+@_rhpb5+@_sql_final) for xml path('''')
exec(
	...
	IF EXISTS (SELECT 1 
	           ...
	           WHERE ds.[name] = 'Egssis Entries Exporter View'
	...
	BEGIN
		...
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'counterparty' AS [name], 'Counterparty' AS ALIAS, 0 AS reqd_param, 1 AS widget_id, 5 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, 1 AS append_filter, NULL  AS tooltip,0 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Egssis Entries Exporter View'
	...
	IF EXISTS (SELECT 1 
	           ...
	           INNER JOIN data_source ds on ds.data_source_id = dsc.source_id 
	           WHERE ds.[name] = 'Egssis Entries Exporter View'
	            AND dsc.name =  'forecast_overrides_quantity'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
	BEGIN
		UPDATE dsc  
		SET alias = 'Forecast Overrides Quantity'
			   , reqd_param = NULL, widget_id = 2, datatype_id = 5, param_data_source = 'SELECT ''y'' [value], ''True'' [code] UNION' + CHAR(10) + 'SELECT ''n'', ''False''', param_default_value = NULL, append_filter = NULL, tooltip = NULL, column_template = 0, key_column = 0, required_filter = 0
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
		WHERE ds.[name] = 'Egssis Entries Exporter View'
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'forecast_overrides_quantity' AS [name], 'Forecast Overrides Quantity' AS ALIAS, NULL AS reqd_param, 2 AS widget_id, 5 AS datatype_id, 'SELECT ''y'' [value], ''True'' [code] UNION' + CHAR(10) + 'SELECT ''n'', ''False''' AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,0 AS column_template, 0 AS key_column, 0 AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Egssis Entries Exporter View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	IF EXISTS (SELECT 1 
	           ...
	           INNER JOIN data_source ds on ds.data_source_id = dsc.source_id 
	           WHERE ds.[name] = 'Egssis Entries Exporter View'
	            AND dsc.name =  'gas_day'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
	BEGIN
		UPDATE dsc  
		SET alias = 'Gas Day'
			   , reqd_param = NULL, widget_id = 1, datatype_id = 5, param_data_source = NULL, param_default_value = NULL, append_filter = NULL, tooltip = NULL, column_template = 4, key_column = 0, required_filter = NULL
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
		WHERE ds.[name] = 'Egssis Entries Exporter View'
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'gas_day' AS [name], 'Gas Day' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 5 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,4 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Egssis Entries Exporter View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	IF EXISTS (SELECT 1 
	           ...
	           INNER JOIN data_source ds on ds.data_source_id = dsc.source_id 
	           WHERE ds.[name] = 'Egssis Entries Exporter View'
	            AND dsc.name =  'internal_shipper'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
	BEGIN
		UPDATE dsc  
		SET alias = 'Internal Shipper'
			   , reqd_param = NULL, widget_id = 1, datatype_id = 5, param_data_source = NULL, param_default_value = NULL, append_filter = NULL, tooltip = NULL, column_template = 0, key_column = 0, required_filter = NULL
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'internal_shipper' AS [name], 'Internal Shipper' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 5 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,0 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Egssis Entries Exporter View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'location_type' AS [name], 'Location Type' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 5 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,0 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Egssis Entries Exporter View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'zone' AS [name], 'Zone' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 5 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,0 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Egssis Entries Exporter View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		SET alias = 'Summary Option'
			   , reqd_param = NULL, widget_id = 2, datatype_id = 5, param_data_source = 'SELECT ''d'' [value], ''Daily'' [code] UNION' + CHAR(10) + 'SELECT ''h'', ''Hourly''', param_default_value = NULL, append_filter = NULL, tooltip = NULL, column_template = 0, key_column = 0, required_filter = 0
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
		WHERE ds.[name] = 'Egssis Entries Exporter View'
	...
	IF EXISTS (SELECT 1 
	           ...
	           INNER JOIN data_source ds on ds.data_source_id = dsc.source_id 
	           WHERE ds.[name] = 'Egssis Entries Exporter View'
	            AND dsc.name =  'convert_to_utc'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
	BEGIN
		UPDATE dsc  
		SET alias = 'Convert To Utc'
			   , reqd_param = NULL, widget_id = 2, datatype_id = 5, param_data_source = 'SELECT ''y'' [value], ''Yes'' [code] UNION' + CHAR(10) + 'SELECT ''n'', ''No''', param_default_value = NULL, append_filter = NULL, tooltip = NULL, column_template = 0, key_column = 0, required_filter = 0
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
		WHERE ds.[name] = 'Egssis Entries Exporter View'
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'convert_to_utc' AS [name], 'Convert To Utc' AS ALIAS, NULL AS reqd_param, 2 AS widget_id, 5 AS datatype_id, 'SELECT ''y'' [value], ''Yes'' [code] UNION' + CHAR(10) + 'SELECT ''n'', ''No''' AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,0 AS column_template, 0 AS key_column, 0 AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Egssis Entries Exporter View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	IF EXISTS (SELECT 1 
	           ...
	           INNER JOIN data_source ds on ds.data_source_id = dsc.source_id 
	           WHERE ds.[name] = 'Egssis Entries Exporter View'
	            AND dsc.name =  'timezone_name'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
	BEGIN
		UPDATE dsc  
		SET alias = 'Timezone Name'
			   , reqd_param = NULL, widget_id = 1, datatype_id = 5, param_data_source = NULL, param_default_value = NULL, append_filter = NULL, tooltip = NULL, column_template = 0, key_column = 0, required_filter = NULL
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'timezone_name' AS [name], 'Timezone Name' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 5 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,0 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Egssis Entries Exporter View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	FROM data_source_column dsc 
	INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
		AND ds.[name] = 'Egssis Entries Exporter View'
		AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1)
