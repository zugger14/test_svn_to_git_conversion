BEGIN TRY
	...
	IF EXISTS (SELECT 1 
	           FROM data_source 
	           WHERE [name] = 'Position View Monthly'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
			
	BEGIN
		UPDATE data_source
		SET alias = 'pvm', description = ''
...
DECLARE @_baseload_block_id INT
SELECT @_baseload_block_id = value_id FROM static_data_value where type_id=10018 AND code = ''base load''
IF OBJECT_ID(N''tempdb..#temp_report_hourly_position_breakdown'') IS NOT NULL
	DROP TABLE #temp_report_hourly_position_breakdown
	
select 
distinct isnull(spcd.block_define_id,@_baseload_block_id) block_define_id,s.term_start,s.term_end 
...
from #temp_report_hourly_position_breakdown a
		...
		and term_date between a.term_start  and a.term_end --and term_date>2014-12-31
) hb
...
--------------
CROSS APPLY(SELECT MAX(deal_volume_frequency) deal_volume_frequency,MAX(term_end) term_end FROM source_deal_detail 
...
where  isnull(spcd.hourly_volume_allocation,17601) IN(17603,17604) and hbt.block_define_id=COALESCE(spcd.block_define_id,@_baseload_block_id)	and  hbt.block_type=COALESCE(spcd.block_type,12000) and hbt.term_date between s.term_start  and s.term_END) term_hrs_exp
left join #term_date hb on hb.block_define_id=isnull(spcd.block_define_id,@_baseload_block_id) and hb.term_start = s.term_start
...
SELECT * INTO #temp_position_table FROM (
	...
	union ALL
	SELECT * FROM #temp_hourly_position_breakdown	--where source_deal_header_id = 355 	
) pos
...
SELECT CAST(''@as_of_date'' AS DATETIME) as_of_date, 
       ...
       MAX(sdv_deal_staus.code) deal_status,
       ...
       MAX(sb2.source_book_name) group2,
       MAX(sb3.source_book_name) group3,
       MAX(sb4.source_book_name) group4,
       MAX(ssbm.logical_name) AS sub_book,
       SUM(vw.Hr1 + vw.Hr2 + vw.Hr3 + vw.Hr4 + vw.Hr5 + vw.Hr6 + vw.Hr7
                  + vw.Hr8 + vw.Hr9 + vw.Hr10 + vw.Hr11 + vw.Hr12 + vw.Hr13 
           + vw.Hr14 + vw.Hr15 + vw.Hr16 + vw.Hr17 + vw.Hr18 + vw.Hr19  
           + vw.Hr20 + vw.Hr21 + vw.Hr22 + vw.Hr23 + vw.Hr24) volume,
       MAX(su_uom.uom_name) uom,
       MAX(su_pos_uom.uom_name) [position_uom],
       MAX(spcd_monthly_index.curve_name + CASE 
                           WHEN sssd.source_system_id = 2 THEN ''''
                           ELSE ''.'' + sssd.source_system_name
                      END) AS [proxy_curve2],
        MAX(su_uom_proxy2.uom_name) [proxy2_position_uom],
		MAX(spcd_proxy_curve3.curve_name + CASE 
                           WHEN sssd2.source_system_id = 2 THEN ''''
                           ELSE ''.'' + sssd2.source_system_name
                      END) AS [proxy_curve3],
        MAX(su_uom_proxy3.uom_name) [proxy3_position_uom],
		MAX(sdv_block.code) [block_definition],
		MAX(CASE WHEN vw.deal_volume_frequency = ''h'' THEN ''Hourly''
		...
		MAX(spcd_proxy_curve_def.curve_name) [proxy_curve],
		MAX(su_uom_proxy_curve_def.uom_name) [proxy_curve_position_uom],
		MAX(YEAR(vw.term_start)) term_year,
		MAX(vw.term_end) term_end,
		MAX(sc.source_counterparty_id) counterparty_id,
		sml.source_minor_location_id location_id,  
		MAX(su_uom_proxy_curve.uom_name) proxy_index_position_uom,
		MAX(spcd.source_curve_def_id) [curve_id],
...
GROUP BY 
com.commodity_name, vw.physical_financial_flag, 
sml.Location_Name,sml.source_minor_location_id, spcd.curve_name, YEAR(vw.term_start), 
MONTH(vw.term_start), vw.source_deal_header_id	--aggregate data in monthly level', report_id = @report_id_data_source_dest 
		WHERE [name] = 'Position View Monthly'
			AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1)
	END	
	...
	BEGIN
		...
		SELECT TOP 1 1 AS [type_id], 'Position View Monthly' AS [name], 'pvm' AS ALIAS, '' AS [description],'IF OBJECT_ID(N''tempdb..#books'') IS NOT NULL
	DROP TABLE #books
IF OBJECT_ID(N''tempdb..#term_date'') IS NOT NULL
	DROP TABLE #term_date
...
com.commodity_name, vw.physical_financial_flag, 
sml.Location_Name,sml.source_minor_location_id, spcd.curve_name, YEAR(vw.term_start), 
MONTH(vw.term_start), vw.source_deal_header_id	--aggregate data in monthly level' AS [tsql], @report_id_data_source_dest AS report_id
	END 
	...
	IF EXISTS (SELECT 1 
	           ...
	           WHERE ds.[name] = 'Position View Monthly'
