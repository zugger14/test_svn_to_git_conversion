BEGIN TRY
	...
	declare @new_ds_alias varchar(10) = 'SARPV'
	/** IF DATA SOURCE ALIAS ALREADY EXISTS ON DESTINATION, RAISE ERROR **/
	if exists(select top 1 1 from data_source where alias = 'SARPV' and name <> 'At Risk PFE View')
	begin
		select top 1 @new_ds_alias = 'SARPV' + cast(s.n as varchar(5))
		from seq s
		left join data_source ds on ds.alias = 'SARPV' + cast(s.n as varchar(5))
	...
	IF NOT EXISTS (SELECT 1 
	           FROM data_source 
	           WHERE [name] = 'At Risk PFE View'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
	AND NOT EXISTS (SELECT 1 FROM map_function_category WHERE [function_name] = 'At Risk PFE View' AND '106500' = '106501') 
	BEGIN
		INSERT INTO data_source([type_id], [name], [alias], [description], [tsql], report_id, system_defined,category)
		SELECT TOP 1 1 AS [type_id], 'At Risk PFE View' AS [name], @new_ds_alias AS ALIAS, 'Standard At Risk PFE View' AS [description],null AS [tsql], @report_id_data_source_dest AS report_id,'1' AS [system_defined]
			,'106500' AS [category]
	END

	UPDATE data_source
	SET alias = @new_ds_alias, description = 'Standard At Risk PFE View'
...
BEGIN
	...
	SET @_sql = ''SELECT 
		...
		CASE WHEN @_criteria_id IS NOT NULL THEN '' AND criteria_id IN('' + @_criteria_id + '')'' ELSE '''' END +

		CASE WHEN @_counterparty_id IS NOT NULL THEN '' AND counterparty_id IN('' + @_counterparty_id + '')'' ELSE '''' END +
...
IF OBJECT_ID(N''tempdb..#tmp_pfe_result'') IS NOT NULL

    DROP TABLE #tmp_pfe_result 


CREATE TABLE #tmp_pfe_result(
	...
	counterparty_name NVARCHAR(250) COLLATE DATABASE_DEFAULT,

	counterparty_id	INT,

	measurement_approach NVARCHAR(100) COLLATE DATABASE_DEFAULT,

	confidence_interval	NVARCHAR(20) COLLATE DATABASE_DEFAULT,

	fixed_exposure NUMERIC(36,2),

	current_forward_exposure NUMERIC(36,2),

	PFE	NUMERIC(36,2),

	total_potential_exposure NUMERIC(36,2),
...
''INSERT INTO #tmp_pfe_result

SELECT

	pr.criteria_id [criteria_id],

	MAX(pr.criteria_name) [criteria_name], 

	pr.as_of_date [as_of_date_from],

	pr.as_of_date [as_of_date_to],

	MAX(pr.counterparty) [counterparty_name],

	pr.counterparty_id [counterparty_id],

	MAX(sdv.code) [measurement_approach],

	MAX(sdv1.code) [confidence_interval],

	ROUND(MAX(pr.fixed_exposure), 2) [fixed_exposure],

	ROUND(MAX(pr.current_exposure), 2) [current_forward_exposure],

    ROUND(MAX(pr.pfe), 2) AS [PFE],

	ROUND(MAX(pr.total_future_exposure), 2) [total_potential_exposure],
...
INNER JOIN var_probability_density vpd ON pr.criteria_id = vpd.var_criteria_id

	AND pr.counterparty_id = vpd.counterparty

	AND pr.as_of_date = vpd.as_of_date

INNER JOIN credit_exposure_detail ced ON vpd.counterparty = ced.netting_counterparty_id	AND ced.as_of_date = vpd.as_of_date
...
+ CASE	WHEN @_as_of_date_to IS NULL THEN '' pr.as_of_date = '''''' + CAST(@_as_of_date_from AS VARCHAR) + '''''''' 

		ELSE '' pr.as_of_date >= '''''' + CAST(@_as_of_date_from AS VARCHAR) + '''''' AND pr.as_of_date <= '''''' + CAST(@_as_of_date_to AS VARCHAR) + '''''''' 

  END 

+ 

CASE WHEN @_criteria_id IS NOT NULL THEN '' AND pr.criteria_id IN('' + @_criteria_id + '')'' ELSE '''' END +

CASE WHEN @_counterparty_id IS NOT NULL THEN '' AND pr.counterparty_id IN('' + @_counterparty_id + '')'' ELSE '''' END +

''

GROUP BY pr.as_of_date, pr.criteria_id, pr.counterparty_id

ORDER BY pr.as_of_date''
...
FROM #tmp_pfe_result', report_id = @report_id_data_source_dest,
	system_defined = '1'
	,category = '106500' 
	WHERE [name] = 'At Risk PFE View'
		AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1)
	...
	IF EXISTS (SELECT 1 
	           ...
	           WHERE ds.[name] = 'At Risk PFE View'
	...
	BEGIN
		...
		SET alias = 'PFE Criteria ID'
			   , reqd_param = 1, widget_id = 9, datatype_id = 4, param_data_source = 'SELECT vmcd.id [value], vmcd.name [label], ''Enable'' as status FROM var_measurement_criteria_detail vmcd' + CHAR(10) + 'WHERE vmcd.measure = 17355 AND vmcd.[active] = ''y''', param_default_value = NULL, append_filter = 0, tooltip = NULL, column_template = 2, key_column = 1, required_filter = 0
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
		WHERE ds.[name] = 'At Risk PFE View'
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'criteria_id' AS [name], 'PFE Criteria ID' AS ALIAS, 1 AS reqd_param, 9 AS widget_id, 4 AS datatype_id, 'SELECT vmcd.id [value], vmcd.name [label], ''Enable'' as status FROM var_measurement_criteria_detail vmcd' + CHAR(10) + 'WHERE vmcd.measure = 17355 AND vmcd.[active] = ''y''' AS param_data_source, NULL AS param_default_value, 0 AS append_filter, NULL  AS tooltip,2 AS column_template, 1 AS key_column, 0 AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'At Risk PFE View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'criteria_name' AS [name], 'PFE Criteria' AS ALIAS, 0 AS reqd_param, 1 AS widget_id, 5 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, 1 AS append_filter, NULL  AS tooltip,0 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'At Risk PFE View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'measurement_approach' AS [name], 'Measurement Approach' AS ALIAS, 0 AS reqd_param, 1 AS widget_id, 5 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, 1 AS append_filter, NULL  AS tooltip,0 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'At Risk PFE View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'total_potential_exposure' AS [name], 'Total Potential Exposure' AS ALIAS, 0 AS reqd_param, 1 AS widget_id, 3 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, 1 AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'At Risk PFE View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
