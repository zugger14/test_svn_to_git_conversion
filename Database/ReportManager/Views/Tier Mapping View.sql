BEGIN TRY
	...
	declare @new_ds_alias varchar(10) = 'TMV'
	/** IF DATA SOURCE ALIAS ALREADY EXISTS ON DESTINATION, RAISE ERROR **/
	if exists(select top 1 1 from data_source where alias = 'TMV' and name <> 'Tier Mapping View')
	begin
		select top 1 @new_ds_alias = 'TMV' + cast(s.n as varchar(5))
		from seq s
		left join data_source ds on ds.alias = 'TMV' + cast(s.n as varchar(5))
	...
	IF NOT EXISTS (SELECT 1 
	           FROM data_source 
	           WHERE [name] = 'Tier Mapping View'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
	AND NOT EXISTS (SELECT 1 FROM map_function_category WHERE [function_name] = 'Tier Mapping View' AND '106500' = '106501') 
	BEGIN
		INSERT INTO data_source([type_id], [name], [alias], [description], [tsql], report_id, system_defined,category)
		SELECT TOP 1 1 AS [type_id], 'Tier Mapping View' AS [name], @new_ds_alias AS ALIAS, 'Tier Mapping View' AS [description],null AS [tsql], @report_id_data_source_dest AS report_id,'1' AS [system_defined]
			,'106500' AS [category]
	END

	UPDATE data_source
	SET alias = @new_ds_alias, description = 'Tier Mapping View'
	, [tsql] = CAST('' AS VARCHAR(MAX)) + 'DECLARE @_jurisdiction_id VARCHAR(1000),
	@_tier_id VARCHAR(1000),
	@_technology_id VARCHAR(1000),
	@_price_index_id VARCHAR(1000),
	@_banking_years_id VARCHAR(1000),
	@_sql VARCHAR(MAX)
...
IF ''@price_index_id'' <> ''NULL''
	SET @_price_index_id = ''@price_index_id''
IF ''@banking_years_id'' <> ''NULL''
	SET @_banking_years_id = ''@banking_years_id''

--select * from state_properties
--select * from static_data_value where value_id = 50000795
--50000795

SET @_sql =  ''
	SELECT sdv_state_val.code [jurisdiction],
		sdv_tier.code [tier],
		sdv_tecno.code [technology],
		sdv_tecno_sub.code [technology_sub_type],
		spcd.curve_name [valuation_index],
		''''@jurisdiction_id'''' [jurisdiction_id],
		''''@tier_id'''' [tier_id],
		''''@technology_id'''' [technology_id],
		''''@price_index_id'''' [price_index_id],
		''''@banking_years_id'''' [banking_years_id],
		spd.banking_years [banking_years],
		spd.effective_date
	--[__batch_report__]
	FROM state_properties sp 
	INNER JOIN state_properties_details spd ON sp.state_value_id = spd.state_value_id
	LEFT JOIN static_data_value sdv_state_val ON sdv_state_val.value_id = ISNULL(spd.state_value_id,-1) AND sdv_state_val.type_id = 10002
	LEFT JOIN static_data_value sdv_tier ON sdv_tier.value_id = ISNULL(spd.tier_id,-1) AND sdv_tier.type_id = 15000
	LEFT JOIN static_data_value sdv_tecno ON sdv_tecno.value_id = ISNULL(spd.technology_id,-1) AND sdv_tecno.type_id = 10009
	LEFT JOIN static_data_value sdv_tecno_sub ON sdv_tecno_sub.value_id = ISNULL(spd.technology_subtype_id,-1) AND sdv_tecno_sub.type_id = 13000
	LEFT JOIN source_price_curve_def spcd ON spcd.source_curve_def_id = ISNULL(spd.price_index,-1)
	WHERE 1=1 ''

IF @_jurisdiction_id IS NOT NULL
	SET @_sql+= '' AND spd.state_value_id IN ('' + @_jurisdiction_id + '') ''
IF @_tier_id IS NOT NULL
	SET @_sql+= '' AND spd.tier_id IN ('' + @_tier_id + '') ''
IF @_technology_id IS NOT NULL
	SET @_sql+= '' AND spd.technology_id IN ('' + @_technology_id + '') ''
IF @_price_index_id IS NOT NULL
	SET @_sql+= '' AND spcd.source_curve_def_id IN ('' + @_price_index_id + '') ''
IF @_banking_years_id IS NOT NULL
	SET @_sql+= '' AND spd.banking_years IN ('' + @_banking_years_id + '') ''
...
EXEC(@_sql)', report_id = @report_id_data_source_dest,
	...
	IF EXISTS (SELECT 1 
	           ...
	           WHERE ds.[name] = 'Tier Mapping View'
	...
	BEGIN
		UPDATE dsc  
		SET alias = 'Price Index Id'
			   , reqd_param = NULL, widget_id = 2, datatype_id = 5, param_data_source = 'SELECT source_curve_def_id,curve_name FROM source_price_curve_def', param_default_value = NULL, append_filter = NULL, tooltip = NULL, column_template = 0, key_column = 0, required_filter = 0
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
		WHERE ds.[name] = 'Tier Mapping View'
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'price_index_id' AS [name], 'Price Index Id' AS ALIAS, NULL AS reqd_param, 2 AS widget_id, 5 AS datatype_id, 'SELECT source_curve_def_id,curve_name FROM source_price_curve_def' AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,0 AS column_template, 0 AS key_column, 0 AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Tier Mapping View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	IF EXISTS (SELECT 1 
	           ...
	           INNER JOIN data_source ds on ds.data_source_id = dsc.source_id 
	           WHERE ds.[name] = 'Tier Mapping View'
	            AND dsc.name =  'banking_years'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
	BEGIN
		UPDATE dsc  
		SET alias = 'Banking Years'
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'banking_years' AS [name], 'Banking Years' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 4 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Tier Mapping View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		UPDATE dsc  
		SET alias = 'Banking Years Id'
			   , reqd_param = NULL, widget_id = 2, datatype_id = 5, param_data_source = 'SELECT DISTINCT banking_years [banking_years_id], banking_years' + CHAR(10) + 'FROM state_properties_details' + CHAR(10) + 'WHERE banking_years IS NOT NULL', param_default_value = NULL, append_filter = NULL, tooltip = NULL, column_template = 0, key_column = 0, required_filter = 0
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
		WHERE ds.[name] = 'Tier Mapping View'
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'banking_years_id' AS [name], 'Banking Years Id' AS ALIAS, NULL AS reqd_param, 2 AS widget_id, 5 AS datatype_id, 'SELECT DISTINCT banking_years [banking_years_id], banking_years' + CHAR(10) + 'FROM state_properties_details' + CHAR(10) + 'WHERE banking_years IS NOT NULL' AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,0 AS column_template, 0 AS key_column, 0 AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Tier Mapping View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
