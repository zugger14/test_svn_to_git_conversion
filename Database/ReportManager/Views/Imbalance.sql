...
SELECT @_source_deal_header_id source_deal_header_id,
    @_leg leg,
     ...
     0 [hour], 
     0 [mins],
     CASE WHEN sdh.header_buy_sell_flag = ''b'' THEN 1 ELSE -1 END * (ISNULL(mdh.meter_volume,0) - ISNULL(pf.profile_volume,0)) [value],
 ...
 --AND sdd.source_deal_detail_id = @_source_deal_detail_id', report_id = @report_id_data_source_dest,
	system_defined = '0'
	,category = '106501' 
	WHERE [name] = 'Imbalance'
		AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1)
