 BEGIN TRY
	...
	declare @new_ds_alias varchar(10) = 'fmpev'
	/** IF DATA SOURCE ALIAS ALREADY EXISTS ON DESTINATION, RAISE ERROR **/
	if exists(select top 1 1 from data_source where alias = 'fmpev' and name <> '15 Mins Position Extract View')
	begin
		select top 1 @new_ds_alias = 'fmpev' + cast(s.n as varchar(5))
		from seq s
		left join data_source ds on ds.alias = 'fmpev' + cast(s.n as varchar(5))
	...
	IF NOT EXISTS (SELECT 1 
	           FROM data_source 
	           WHERE [name] = '15 Mins Position Extract View'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
	AND NOT EXISTS (SELECT 1 FROM map_function_category WHERE [function_name] = '15 Mins Position Extract View' AND '106500' = '106501') 
	BEGIN
		INSERT INTO data_source([type_id], [name], [alias], [description], [tsql], report_id, system_defined,category)
		SELECT TOP 1 1 AS [type_id], '15 Mins Position Extract View' AS [name], @new_ds_alias AS ALIAS, '15 Mins Position Extract View' AS [description],null AS [tsql], @report_id_data_source_dest AS report_id,'0' AS [system_defined]
			,'106500' AS [category]
	END

	UPDATE data_source
	SET alias = @new_ds_alias, description = '15 Mins Position Extract View'
...
*/
    DECLARE 
	  @_summary_option VARCHAR(6) =''x03d''   ---x=15 Mins,  (@_summary_option= ...s, @_group_by=s or @_summary_option= ...d, @_group_by=d)
...
EXEC dbo.spa_position_report 
	...
	IF EXISTS (SELECT 1 
	           ...
	           WHERE ds.[name] = '15 Mins Position Extract View'
