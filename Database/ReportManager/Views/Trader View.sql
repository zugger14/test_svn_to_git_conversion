BEGIN TRY
	...
	IF NOT EXISTS (SELECT 1 
	           FROM data_source 
	           WHERE [name] = 'Trader View'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
	BEGIN
		INSERT INTO data_source([type_id], [name], [alias], [description], [tsql], report_id)
		SELECT TOP 1 1 AS [type_id], 'Trader View' AS [name], 'Tv' AS ALIAS, '' AS [description],null AS [tsql], @report_id_data_source_dest AS report_id
	END

	UPDATE data_source
	SET alias = 'Tv', description = ''
	, [tsql] = CAST('' AS VARCHAR(MAX)) + 'DECLARE @_sql VARCHAR(MAX)

DECLARE @_trader_id VARCHAR(200) 

DECLARE @_user_login_id VARCHAR(200)
...
'' 
 ...
 +  CASE WHEN @_trader_id IS NOT NULL THEN '' AND st.source_trader_id IN(''+@_trader_id+'')'' ELSE '''' END
