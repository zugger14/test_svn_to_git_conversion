 BEGIN TRY
	...
	declare @new_ds_alias varchar(10) = 'dpev1'
	/** IF DATA SOURCE ALIAS ALREADY EXISTS ON DESTINATION, RAISE ERROR **/
	if exists(select top 1 1 from data_source where alias = 'dpev1' and name <> 'Daily Position Extract View')
	begin
		select top 1 @new_ds_alias = 'dpev1' + cast(s.n as varchar(5))
		from seq s
		left join data_source ds on ds.alias = 'dpev1' + cast(s.n as varchar(5))
	...
	IF NOT EXISTS (SELECT 1 
	           FROM data_source 
	           WHERE [name] = 'Daily Position Extract View'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
	AND NOT EXISTS (SELECT 1 FROM map_function_category WHERE [function_name] = 'Daily Position Extract View' AND '106500' = '106501') 
	BEGIN
		INSERT INTO data_source([type_id], [name], [alias], [description], [tsql], report_id, system_defined,category)
		SELECT TOP 1 1 AS [type_id], 'Daily Position Extract View' AS [name], @new_ds_alias AS ALIAS, 'Daily Position Extract View' AS [description],null AS [tsql], @report_id_data_source_dest AS report_id,'0' AS [system_defined]
			,'106500' AS [category]
	END

	UPDATE data_source
	SET alias = @new_ds_alias, description = 'Daily Position Extract View'
...
DECLARE @_sub_id VARCHAR(MAX) = NULL
	...
	, @_as_of_date VARCHAR(20) =NULL--''2020-01-01''
	, @_source_deal_header_id VARCHAR(1000) = NULL--2
	...
	, @_commodity_id VARCHAR(200) = NULL
	...
	, @_term_start VARCHAR(20) = NULL--''2020-01-01''
	, @_term_end VARCHAR(20) = NULL--''2020-12-31''
	, @_deal_type VARCHAR(500) = NULL
	, @_deal_sub_type VARCHAR(200) = NULL
...
*/
DECLARE @_summary_option VARCHAR(6) =''d00d''
	, @_group_by CHAR(1) = ''d'' 
...
EXEC dbo.spa_position_report @_summary_option = @_summary_option, 
	...
	IF EXISTS (SELECT 1 
	           ...
	           WHERE ds.[name] = 'Daily Position Extract View'
	...
	BEGIN
		...
		SET alias = 'Parent Counterparty'
			   , reqd_param = NULL, widget_id = 9, datatype_id = 5, param_data_source = 'select source_counterparty_id,counterparty_id from source_counterparty where int_ext_flag=''i''', param_default_value = NULL, append_filter = NULL, tooltip = NULL, column_template = 0, key_column = 0, required_filter = 0
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
		WHERE ds.[name] = 'Daily Position Extract View'
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'tot_hours' AS [name], 'Tot Hours' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 4 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Daily Position Extract View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
