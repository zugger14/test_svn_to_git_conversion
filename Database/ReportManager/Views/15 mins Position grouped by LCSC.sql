BEGIN TRY
	...
	declare @new_ds_alias varchar(10) = 'fmpgbs'
	/** IF DATA SOURCE ALIAS ALREADY EXISTS ON DESTINATION, RAISE ERROR **/
	if exists(select top 1 1 from data_source where alias = 'fmpgbs' and name <> '15 mins Position grouped by LCSC')
	begin
		select top 1 @new_ds_alias = 'fmpgbs' + cast(s.n as varchar(5))
		from seq s
		left join data_source ds on ds.alias = 'fmpgbs' + cast(s.n as varchar(5))
	...
	IF NOT EXISTS (SELECT 1 
	           FROM data_source 
	           WHERE [name] = '15 mins Position grouped by LCSC'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
	AND NOT EXISTS (SELECT 1 FROM map_function_category WHERE [function_name] = '15 mins Position grouped by LCSC' AND '106500' = '106501') 
	BEGIN
		INSERT INTO data_source([type_id], [name], [alias], [description], [tsql], report_id, system_defined,category)
		SELECT TOP 1 1 AS [type_id], '15 mins Position grouped by LCSC' AS [name], @new_ds_alias AS ALIAS, '15 mins Position grouped by loc-counterparty-shipper codes' AS [description],null AS [tsql], @report_id_data_source_dest AS report_id,'0' AS [system_defined]
			,'106500' AS [category]
	END

	UPDATE data_source
	SET alias = @new_ds_alias, description = '15 mins Position grouped by loc-counterparty-shipper codes'
...
DECLARE @_sub_id VARCHAR(MAX) = NULL
	...
	, @_term_start VARCHAR(20) =NULL--''2021-03-01''
	, @_term_end VARCHAR(20) =NULL--''2021-03-31''
...
*/
    DECLARE 
	@_summary_option VARCHAR(6) =''x05d''   ---x=15 Mins,  (@_summary_option= ...s, @_group_by=s or @_summary_option= ...d, @_group_by=d)
...
EXEC dbo.spa_position_report 
	...
	IF EXISTS (SELECT 1 
	           ...
	           WHERE ds.[name] = '15 mins Position grouped by LCSC'
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'term_end' AS [name], 'Term End' AS ALIAS, NULL AS reqd_param, 6 AS widget_id, 5 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,4 AS column_template, 0 AS key_column, 1 AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = '15 mins Position grouped by LCSC'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'term_start' AS [name], 'Term Start' AS ALIAS, NULL AS reqd_param, 6 AS widget_id, 5 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,4 AS column_template, 0 AS key_column, 1 AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = '15 mins Position grouped by LCSC'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
