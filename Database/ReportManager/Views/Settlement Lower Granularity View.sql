BEGIN TRY
	...
	IF EXISTS (SELECT 1 
	           FROM data_source 
	           WHERE [name] = 'Settlement Lower Granularity View'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
			
	BEGIN
		UPDATE data_source
		SET alias = 'sslgv', description = 'Standard Settlement Lower Granularity View'
...
SELECT sdsb.as_of_date,
       ...
       sdsb.market_value [market_value],
       sdsb.contract_value [contract_value],
       sdsb.leg_set [settlement_value],    
       sdsb.as_of_date  to_as_of_date,   
       CASE WHEN sdsb.volume <> 0 THEN abs(sdsb.leg_mtm /sdsb.volume) ELSE NULL END [net_price] ,
       sdsb.market_price,
       sdsb.contract_price,
       sdsb.volume,
...
FROM   source_deal_settlement_breakdown sdsb
       INNER JOIN source_deal_header sdh
			...
			CASE 
				...
				THEN ''sdsb.as_of_date = ''''@as_of_date'''''' 
				ELSE ''sdsb.as_of_date >= ''''@as_of_date'''' AND sdsb.as_of_date <= ''''@to_as_of_date'''' ''
			END
			+
			''
...
EXEC(@_sql)', report_id = @report_id_data_source_dest 
	...
	BEGIN
		...
		SELECT TOP 1 1 AS [type_id], 'Settlement Lower Granularity View' AS [name], 'sslgv' AS ALIAS, 'Standard Settlement Lower Granularity View' AS [description],'	DECLARE @_from_as_date VARCHAR(50)  
	DECLARE @_to_as_date VARCHAR(50)
	DECLARE @_sql VARCHAR(MAX)
	DECLARE @_term_start  VARCHAR(MAX)
	DECLARE @_term_end  VARCHAR(MAX)
...
EXEC(@_sql)' AS [tsql], @report_id_data_source_dest AS report_id
	...
	IF EXISTS (SELECT 1 
	           ...
	           WHERE ds.[name] = 'Settlement Lower Granularity View'
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'contract_price' AS [name], 'Contract Price' AS ALIAS, 0 AS reqd_param, 1 AS widget_id, 3 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, 1 AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Settlement Lower Granularity View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		UPDATE dsc  
		SET alias = 'Index ID'
			   , reqd_param = 0, widget_id = 2, datatype_id = 4, param_data_source = NULL, param_default_value = NULL, append_filter = 1, tooltip = NULL, column_template = 2, key_column = 0
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
		WHERE ds.[name] = 'Settlement Lower Granularity View'
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'curve_id' AS [name], 'Index ID' AS ALIAS, 0 AS reqd_param, 2 AS widget_id, 4 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, 1 AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Settlement Lower Granularity View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
