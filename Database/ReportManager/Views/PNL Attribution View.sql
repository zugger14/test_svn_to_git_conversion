BEGIN TRY
	...
	declare @new_ds_alias varchar(10) = 'SDDV3'
	/** IF DATA SOURCE ALIAS ALREADY EXISTS ON DESTINATION, RAISE ERROR **/
	if exists(select top 1 1 from data_source where alias = 'SDDV3' and name <> 'PNL Attribution View')
	begin
		select top 1 @new_ds_alias = 'SDDV3' + cast(s.n as varchar(5))
		from seq s
		left join data_source ds on ds.alias = 'SDDV3' + cast(s.n as varchar(5))
	...
	IF NOT EXISTS (SELECT 1 
	           FROM data_source 
	           WHERE [name] = 'PNL Attribution View'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
	AND NOT EXISTS (SELECT 1 FROM map_function_category WHERE [function_name] = 'PNL Attribution View' AND '106500' = '106501') 
	BEGIN
		INSERT INTO data_source([type_id], [name], [alias], [description], [tsql], report_id, system_defined,category)
		SELECT TOP 1 1 AS [type_id], 'PNL Attribution View' AS [name], @new_ds_alias AS ALIAS, 'Standard PNL Attribution View' AS [description],null AS [tsql], @report_id_data_source_dest AS report_id,'1' AS [system_defined]
			,'106500' AS [category]
	END

	UPDATE data_source
	SET alias = @new_ds_alias, description = 'Standard PNL Attribution View'
...
DECLARE @_as_of_date_from VARCHAR(20) -- = ''2015-01-01''
DECLARE @_as_of_date_to VARCHAR(20) -- = ''2018-12-20''
...
DECLARE @_counterparty_id VARCHAR(MAX)
DECLARE @_trader_id VARCHAR(MAX)
...
DECLARE @_deal_id VARCHAR(MAX)
DECLARE @_current_included VARCHAR(10)
DECLARE @_physical_financial_flag CHAR(1)
DECLARE @_buy_sell_flag CHAR(1)
DECLARE @_curve_id VARCHAR(MAX)
DECLARE @_location_id VARCHAR(MAX)
DECLARE @_transaction_type_id VARCHAR(MAX)
DECLARE @_deal_status_group CHAR(1) -- = ''u''
DECLARE @_internal_counterparty_id VARCHAR(MAX)
...
IF ''@current_included'' <> ''NULL''
	SET @_current_included = ''@current_included''
...
IF ''@transaction_type_id'' <> ''NULL''
	SET @_transaction_type_id = ''@transaction_type_id''

IF OBJECT_ID(N''tempdb..#books'') IS NOT NULL
	DROP TABLE #books

IF OBJECT_ID(N''tempdb..#source_deal_header_ids'') IS NOT NULL
	DROP TABLE #source_deal_header_ids	
...
CREATE TABLE #source_deal_header_ids(
	...
	sub_id	INT,
	book_id	INT,
	sub	VARCHAR(1000) COLLATE DATABASE_DEFAULT NULL,
	stra VARCHAR(1000) COLLATE DATABASE_DEFAULT NULL,
	book VARCHAR(1000) COLLATE DATABASE_DEFAULT NULL,
	book_identifier1_id	INT,
	book_identifier2_id	INT,
	book_identifier3_id	INT,
	book_identifier4_id	INT,
	book_identifier1 NVARCHAR(1000) COLLATE DATABASE_DEFAULT NULL,
	book_identifier2 NVARCHAR(1000) COLLATE DATABASE_DEFAULT NULL,
	book_identifier3 NVARCHAR(1000) COLLATE DATABASE_DEFAULT NULL,
	book_identifier4 NVARCHAR(1000) COLLATE DATABASE_DEFAULT NULL,
	sub_book VARCHAR(1000) COLLATE DATABASE_DEFAULT NULL,
	sub_book_id	INT,
	deal_id	NVARCHAR(500) COLLATE DATABASE_DEFAULT NULL,
	deal_date DATETIME,
	counterparty_id	INT,
	counterparty_name NVARCHAR(1000) COLLATE DATABASE_DEFAULT NULL,
	deal_type_name VARCHAR(1000) COLLATE DATABASE_DEFAULT NULL,
	source_deal_type_id	INT,
	commodity_id INT,
	commodity_name VARCHAR(1000) COLLATE DATABASE_DEFAULT NULL,
	trader_id	INT,
	trader_name	NVARCHAR(1000) COLLATE DATABASE_DEFAULT NULL,
	term_start	DATETIME,
	term_end	DATETIME,
	curve_id	INT,
	curve_name	VARCHAR(1000) COLLATE DATABASE_DEFAULT NULL,
	location_id	INT,
	Location_Name	NVARCHAR(1000) COLLATE DATABASE_DEFAULT NULL,
	leg	INT,
	deal_status_id	INT,
	deal_status	NVARCHAR(1000) COLLATE DATABASE_DEFAULT NULL,
	create_user	NVARCHAR(1000) COLLATE DATABASE_DEFAULT NULL,
	create_ts	DATETIME,
	update_user	NVARCHAR(1000) COLLATE DATABASE_DEFAULT NULL,
	update_ts	DATETIME,
	contract_id	INT,
	contract_name	VARCHAR(1000) COLLATE DATABASE_DEFAULT NULL,
	template_id	INT,
	template_name	VARCHAR(1000) COLLATE DATABASE_DEFAULT NULL,
	physical_financial_flag	VARCHAR(1000) COLLATE DATABASE_DEFAULT NULL,
	buy_sell_flag	VARCHAR(1000) COLLATE DATABASE_DEFAULT NULL,
	sub_book_group1_id INT,
	sub_book_group2_id INT,
	sub_book_group3_id INT,
	sub_book_group4_id INT,
	sub_book_group1	NVARCHAR(1000) COLLATE DATABASE_DEFAULT NULL,
	sub_book_group2	NVARCHAR(1000) COLLATE DATABASE_DEFAULT NULL,
	sub_book_group3	NVARCHAR(1000) COLLATE DATABASE_DEFAULT NULL,
	sub_book_group4	NVARCHAR(1000) COLLATE DATABASE_DEFAULT NULL,
	internal_counterparty_id INT,
	internal_counterparty_name NVARCHAR(1000) COLLATE DATABASE_DEFAULT NULL)

SET @_sql = ''INSERT INTO #source_deal_header_ids
	SELECT 
		...
		sdh.template_id,
		sdht.template_name,
		CASE sdh.physical_financial_flag WHEN ''''p'''' THEN ''''Physical'''' ELSE ''''Financial'''' END AS physical_financial_flag,
		CASE sdd.buy_sell_flag WHEN ''''b'''' THEN ''''Buy'''' ELSE ''''Sell'''' END AS buy_sell_flag,
		...
		sdh.internal_counterparty AS internal_counterparty_id,
		isc.counterparty_name AS internal_counterparty_name
	...
	''''
	END + ''
	INNER JOIN #books books ON  sdh.source_system_book_id1 = books.source_system_book_id1
	...
	LEFT JOIN source_counterparty isc ON isc.source_counterparty_id = sdh.internal_counterparty
	...
	LEFT JOIN source_traders st2 ON st2.source_trader_id = sdh.trader_id
	LEFT JOIN source_price_curve_def spcd ON spcd.source_curve_def_id = sdd.curve_id
	LEFT JOIN source_minor_location sml ON sdd.location_id = sml.source_minor_location_id
	LEFT JOIN static_data_value sdv_ds ON sdv_ds.value_id = sdh.deal_status
	...
	WHERE 1=1 '' +
		...
		CASE WHEN @_deal_date_from IS NULL THEN '''' ELSE '' AND sdh.deal_date >= '''''' + @_deal_date_from + '''''''' END + 
		CASE WHEN @_deal_date_to IS NULL THEN '''' ELSE '' AND sdh.deal_date <= '''''' + @_deal_date_to + '''''''' END +
		CASE WHEN @_counterparty_id IS NULL THEN '''' ELSE '' AND sdh.counterparty_id IN ( '' + @_counterparty_id + '')'' END +
		...
		CASE WHEN @_deal_id IS NULL THEN '''' ELSE '' AND sdh.deal_id LIKE ''''%''+ @_deal_id + ''%'''''' END +
		CASE WHEN @_curve_id IS NULL THEN '''' ELSE '' AND sdd.curve_id IN ( '' + @_curve_id + '')'' END +
		CASE WHEN @_location_id IS NULL THEN '''' ELSE '' AND sdd.location_id IN ( '' + @_location_id + '')'' END +
		CASE WHEN @_term_start IS NULL THEN '''' ELSE '' AND sdd.term_start >= '''''' + @_term_start + '''''''' END + 
		CASE WHEN @_term_end IS NULL THEN '''' ELSE '' AND sdd.term_start <= '''''' + @_term_end + '''''''' END +
		CASE WHEN @_physical_financial_flag IS NULL THEN '''' ELSE '' AND sdh.physical_financial_flag = '''''' + @_physical_financial_flag + '''''''' END +
		CASE WHEN @_buy_sell_flag IS NULL THEN '''' ELSE '' AND sdd.buy_sell_flag = '''''' + @_buy_sell_flag + '''''''' END +
...
SET @_sql = ''INSERT INTO #source_deal_header_ids
	...
	INNER JOIN #books books ON  sdh.source_system_book_id1 = books.source_system_book_id1
		...
		AND sdh.source_system_book_id3 = books.source_system_book_id3
		AND sdh.source_system_book_id4 = books.source_system_book_id4
	INNER JOIN delete_source_deal_detail sdd ON sdd.source_deal_header_id = sdh.source_deal_header_id
	INNER JOIN source_book sb1 ON sb1.source_book_id = sdh.source_system_book_id1
	INNER JOIN source_book sb2 ON sb2.source_book_id = sdh.source_system_book_id2
	INNER JOIN source_book sb3 ON sb3.source_book_id = sdh.source_system_book_id3
	INNER JOIN source_book sb4 ON sb4.source_book_id = sdh.source_system_book_id4
	...
	LEFT JOIN static_data_value sdv_sbg3 ON sdv_sbg3.value_id = books.sub_book_group3
	LEFT JOIN static_data_value sdv_sbg4 ON sdv_sbg4.value_id = books.sub_book_group4
	LEFT JOIN  pnl_explain_view pev ON pev.source_deal_header_id = sdh.source_deal_header_id
		AND pev.leg = sdd.leg
		AND pev.term_start = sdd.term_start
	WHERE 1=1 '' +
		CASE WHEN @_source_deal_header_id IS NULL THEN '''' ELSE '' AND sdh.source_deal_header_id IN ( '' + @_source_deal_header_id + '')'' END +
		CASE WHEN @_deal_date_from IS NULL THEN '''' ELSE '' AND sdh.deal_date >= '''''' + @_deal_date_from + '''''''' END + 
		CASE WHEN @_deal_date_to IS NULL THEN '''' ELSE '' AND sdh.deal_date <= '''''' + @_deal_date_to + '''''''' END +
		...
		CASE WHEN @_buy_sell_flag IS NULL THEN '''' ELSE '' AND sdd.buy_sell_flag = '''''' + @_buy_sell_flag + '''''''' END +
		CASE WHEN @_internal_counterparty_id IS NULL THEN '''' ELSE '' AND sdh.internal_counterparty IN ( '' + @_internal_counterparty_id + '')'' END +
		CASE WHEN @_transaction_type_id IS NULL THEN '''' ELSE '' AND pev.transaction_type_id IN ( '' + @_transaction_type_id + '')'' END 
	...
	SELECT
	  pev.as_of_date_from,
	  pev.as_of_date_to,
	  pev.source_deal_header_id,
	  MAX(sdh.stra_id) stra_id,
	  MAX(sdh.sub_id) sub_id,
	  MAX(sdh.book_id) book_id,
	  MAX(sdh.sub) sub,
	  MAX(sdh.stra) stra,
	  MAX(sdh.book) book,
	  MAX(sdh.sub_book_id) sub_book_id,
	  MAX(sdh.sub_book) sub_book,
	  MAX(sdh.book_identifier1_id) book_identifier1_id,
	  MAX(sdh.book_identifier2_id)  book_identifier2_id,
	  MAX(sdh.book_identifier3_id)  book_identifier3_id,
	  MAX(sdh.book_identifier4_id)  book_identifier4_id,
	  MAX(sdh.book_identifier1)  book_identifier1,
	  MAX(sdh.book_identifier2)  book_identifier2,
	  MAX(sdh.book_identifier3)  book_identifier3,
	  MAX(sdh.book_identifier4)  book_identifier4,
	  SUM(CASE WHEN as_of_date_from = @_as_of_date_from THEN 
			begin_mtm
	  ELSE 0 END) begin_mtm,
	  SUM(CASE WHEN as_of_date_to =@_as_of_date_to THEN
		end_mtm 
	  ELSE 0 END) end_mtm,
	  pev.term_start,
	  pev.term_end,
	 sdh.leg,
	 MAX(sdh.deal_status_id) deal_status_id,
	 MAX(sdh.deal_status) deal_status,
	 SUM(pev.new_mtm) new_mtm,
	 SUM(pev.modify_MTM) modify_MTM,
	 SUM(pev.deleted_mtm) deleted_mtm,
	 SUM(pev.delivered_mtm) delivered_mtm,
	 SUM(pev.pnl_Delta1) pnl_Delta1,
	 SUM(pev.pnl_Delta2) pnl_Delta2,
	 SUM(pev.pnl_Gamma1) pnl_Gamma1,
	 SUM(pev.pnl_Gamma2) pnl_Gamma2,
	 SUM(pev.pnl_Vega1) pnl_Vega1,
	 SUM(pev.pnl_Vega2) pnl_Vega2,
	 SUM(pev.pnl_Theta) pnl_Theta,
	 SUM(pev.pnl_Rho) pnl_Rho,
	 SUM(pev.Method) Method,
	 MAX(pev.attribute_type) attribute_type,
	 SUM(pev.price_changed_mtm) price_changed_mtm,
	 SUM(pev.unexplained_mtm)  unexplained_mtm,
	 SUM(pev.total_change_mtm) total_change_mtm,
	 SUM(CASE WHEN pev.as_of_date_from = @_as_of_date_from THEN 
			begin_vol
	  ELSE 0 END) begin_vol,
	  SUM(CASE WHEN pev.as_of_date_to =@_as_of_date_to THEN
			end_vol
	  ELSE 0 END)  end_vol,
	  SUM(pev.new_vol) new_vol,
	  SUM(pev.modify_vol) modify_vol,
	  SUM(pev.deleted_vol) deleted_vol,
	  SUM(pev.delivered_vol) delivered_vol,
	  SUM(pev.unexplained_vol) unexplained_vol,
	  SUM(pev.total_change_vol) total_change_vol,
	  MAX(pev.price_to) price_to,
	  MAX(pev.price_from) price_from,
	  pev.charge_type charge_type_id,
	  MAX(sdv_ct.code) charge_type,
	  MAX(sdh.curve_id) curve_id,
	  MAX(sdh.curve_name) curve_name,
	  MAX(PEV.source_uom_id) source_uom_id,
	  MAX(su.uom_name) uom_name,
	  MAX(pev.source_currency_id) source_currency_id,
	  MAX(sc.currency_name) currency_name ,
	  MAX(sdh.counterparty_id) counterparty_id,
	  MAX(sdh.counterparty_name) counterparty_name,
	  MAX(sdh.deal_id) deal_id,
	  MAX(pev.transaction_type_id) transaction_type_id,
	  MAX(pev.transaction_type_name) transaction_type_name,
	  pev.current_included current_included,
	  MAX(sdh.commodity_id) commodity_id,
	  MAX(sdh.commodity_name) commodity_name,
	  MAX(sdh.trader_id) trader_id,
	  MAX(sdh.trader_name) trader_name,
	  MAX(sdh.location_id) location_id,
	  MAX(sdh.Location_Name) Location_Name,
	  MAX(sdh.source_deal_type_id) source_deal_type_id,
	  MAX(sdh.deal_type_name) deal_type_name,
	  DAY(pev.term_start) term_start_day,
	  CAST(YEAR(pev.term_start) AS VARCHAR(5)) + ''-'' + RIGHT(''0''+ CAST(MONTH(pev.term_start) AS VARCHAR(2)), 2) AS [term_start_year_month],
	  MONTH(pev.term_start) term_start_month,
	  DATENAME(m,pev.term_start) term_start_month_name,
	  DATEPART(q,pev.term_start) term_start_month_quarter,
	  YEAR(pev.term_start) term_start_year,
	  MAX(sdh.contract_id) contract_id,
	  MAX(sdh.contract_name) [contract_name],
	  MAX(sdh.deal_date) deal_date_from,
	  MAX(sdh.deal_date) deal_date_to,
	  MAX(sdh.template_id) template_id,
	  MAX(sdh.template_name) template_name,
	  MAX(ag_t.agg_term) AS agg_term,
	  MAX(sdh.physical_financial_flag) AS physical_financial_flag,
	  MAX(buy_sell_flag) AS buy_sell_flag,
	  MAX(sub_book_group1_id) AS sub_book_group1_id,
	  MAX(sub_book_group2_id) AS sub_book_group2_id,
	  MAX(sub_book_group3_id) AS sub_book_group3_id,
	  MAX(sub_book_group4_id) AS sub_book_group4_id,
	  MAX(sub_book_group1) AS sub_book_group1,
	  MAX(sub_book_group2) AS sub_book_group2,
	  MAX(sub_book_group3) AS sub_book_group3,
	  MAX(sub_book_group4) AS sub_book_group4,
	  MAX(internal_counterparty_id) AS internal_counterparty_id,
	  MAX(internal_counterparty_name) AS internal_counterparty_name,
	  MAX(ISNULL(@_deal_status_group, '''')) AS [deal_status_group]
	--[__batch_report__]
	FROM pnl_explain_view pev
	INNER JOIN  #source_deal_header_ids sdh ON pev.source_deal_header_id = sdh.source_deal_header_id
		AND pev.leg = sdh.leg
		AND pev.term_start = sdh.term_start
	LEFT JOIN static_data_value sdv_ct ON sdv_ct.value_id = pev.charge_type
	LEFT JOIN source_currency sc On sc.source_currency_id = pev.source_currency_id
	LEFT JOIN source_uom su ON su.source_uom_id = pev.source_uom_id
	OUTER APPLY(
				 SELECT TOP 1 
					CASE 
						WHEN pev.term_start<= @_as_of_date_from THEN 
							CAST(YEAR(pev.term_start) AS VARCHAR) + '' - Current Month''
						WHEN DATEDIFF(d,@_as_of_date_from,pev.term_start)=1 THEN 
							''Day - 1 ''+convert(varchar(7),pev.term_start,120)
						WHEN MONTH(@_as_of_date_from) = MONTH(pev.term_start) AND YEAR(@_as_of_date_from) =  YEAR(pev.term_start) THEN  
							''Week - ''+ CAST(DATEDIFF(week,@_as_of_date_from,pev.term_start) AS VARCHAR)+ '' ''+ convert(varchar(7),pev.term_start,120) 
						WHEN DATEPART(q,@_as_of_date_from) =  DATEPART(q,pev.term_start) AND YEAR(@_as_of_date_from) =  YEAR(pev.term_start) THEN 
							''Month - '' + CAST(DATEDIFF(m,@_as_of_date_from,pev.term_start) AS VARCHAR)+'' ''+ convert(varchar(4),pev.term_start,120) 
						WHEN YEAR(@_as_of_date_from) =  YEAR(pev.term_start) THEN 
							''Quarter - '' + CAST(DATEPART(q,pev.term_start) AS VARCHAR) + '' ''+convert(varchar(4),pev.term_start,120) 
						ELSE  
							''Year - '' +CAST(YEAR(pev.term_start) AS VARCHAR) 
					END agg_term FROM portfolio_mapping_tenor
				  ) ag_t
	WHERE pev.as_of_date_from >= @_as_of_date_from AND  pev.as_of_date_to <= @_as_of_date_to
		 AND (@_current_included IS NULL OR pev.current_included = @_current_included)
GROUP BY pev.as_of_date_from,
	pev.as_of_date_to, 
	pev.source_deal_header_id, 
	pev.term_start, 
	pev.term_end, 
	pev.charge_type,
	sdh.leg,
	pev.current_included', report_id = @report_id_data_source_dest,
	system_defined = '1'
	,category = '106500' 
	WHERE [name] = 'PNL Attribution View'
		AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1)
	...
	IF EXISTS (SELECT 1 
	           ...
	           WHERE ds.[name] = 'PNL Attribution View'
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'begin_mtm' AS [name], 'Begin MTM' AS ALIAS, 0 AS reqd_param, 1 AS widget_id, 3 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, 1 AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'PNL Attribution View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'begin_vol' AS [name], 'Begin Volume' AS ALIAS, 0 AS reqd_param, 1 AS widget_id, 3 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, 1 AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'PNL Attribution View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		SET alias = 'Charge Type ID'
			   , reqd_param = 0, widget_id = 9, datatype_id = 4, param_data_source = 'exec spa_StaticDataValues ''h'',@type_id=10019', param_default_value = NULL, append_filter = 1, tooltip = NULL, column_template = 2, key_column = 0, required_filter = NULL
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
		WHERE ds.[name] = 'PNL Attribution View'
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'commodity_name' AS [name], 'Commodity' AS ALIAS, 0 AS reqd_param, 1 AS widget_id, 5 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, 1 AS append_filter, NULL  AS tooltip,0 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'PNL Attribution View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		UPDATE dsc  
		SET alias = 'Current Included'
			   , reqd_param = 1, widget_id = 2, datatype_id = 4, param_data_source = 'SELECT 1 value, ''Yes'' Code' + CHAR(10) + 'UNION' + CHAR(10) + 'SELECT 0,''No''', param_default_value = NULL, append_filter = 0, tooltip = NULL, column_template = 2, key_column = 0, required_filter = 0
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
		WHERE ds.[name] = 'PNL Attribution View'
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'current_included' AS [name], 'Current Included' AS ALIAS, 1 AS reqd_param, 2 AS widget_id, 4 AS datatype_id, 'SELECT 1 value, ''Yes'' Code' + CHAR(10) + 'UNION' + CHAR(10) + 'SELECT 0,''No''' AS param_data_source, NULL AS param_default_value, 0 AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, 0 AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'PNL Attribution View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		SET alias = 'Deal Status ID'
			   , reqd_param = 0, widget_id = 9, datatype_id = 4, param_data_source = 'exec spa_StaticDataValues ''h'',@type_id=5600', param_default_value = NULL, append_filter = 1, tooltip = NULL, column_template = 2, key_column = 0, required_filter = NULL
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
		WHERE ds.[name] = 'PNL Attribution View'
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'deal_status_id' AS [name], 'Deal Status ID' AS ALIAS, 0 AS reqd_param, 9 AS widget_id, 4 AS datatype_id, 'exec spa_StaticDataValues ''h'',@type_id=5600' AS param_data_source, NULL AS param_default_value, 1 AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'PNL Attribution View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'deleted_mtm' AS [name], 'Deleted MTM' AS ALIAS, 0 AS reqd_param, 1 AS widget_id, 3 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, 1 AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'PNL Attribution View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'deleted_vol' AS [name], 'Deleted Volume' AS ALIAS, 0 AS reqd_param, 1 AS widget_id, 3 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, 1 AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'PNL Attribution View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'delivered_mtm' AS [name], 'Delivered MTM' AS ALIAS, 0 AS reqd_param, 1 AS widget_id, 3 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, 1 AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'PNL Attribution View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'delivered_vol' AS [name], 'Delivered Volume' AS ALIAS, 0 AS reqd_param, 1 AS widget_id, 3 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, 1 AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'PNL Attribution View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'end_mtm' AS [name], 'End MTM' AS ALIAS, 0 AS reqd_param, 1 AS widget_id, 3 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, 1 AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'PNL Attribution View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'end_vol' AS [name], 'End Volume' AS ALIAS, 0 AS reqd_param, 1 AS widget_id, 3 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, 1 AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'PNL Attribution View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'modify_MTM' AS [name], 'Modify MTM' AS ALIAS, 0 AS reqd_param, 1 AS widget_id, 3 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, 1 AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'PNL Attribution View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'modify_vol' AS [name], 'Modify Volume' AS ALIAS, 0 AS reqd_param, 1 AS widget_id, 3 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, 1 AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'PNL Attribution View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'new_mtm' AS [name], 'New MTM' AS ALIAS, 0 AS reqd_param, 1 AS widget_id, 3 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, 1 AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'PNL Attribution View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'new_vol' AS [name], 'New Volume' AS ALIAS, 0 AS reqd_param, 1 AS widget_id, 3 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, 1 AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'PNL Attribution View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'price_changed_mtm' AS [name], 'Price Changed MTM' AS ALIAS, 0 AS reqd_param, 1 AS widget_id, 3 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, 1 AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'PNL Attribution View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'price_from' AS [name], 'Price From' AS ALIAS, 0 AS reqd_param, 1 AS widget_id, 3 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, 1 AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'PNL Attribution View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'price_to' AS [name], 'Price To' AS ALIAS, 0 AS reqd_param, 1 AS widget_id, 3 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, 1 AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'PNL Attribution View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'source_currency_id' AS [name], 'Currency ID' AS ALIAS, 0 AS reqd_param, 1 AS widget_id, 4 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, 1 AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'PNL Attribution View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		SET alias = 'Deal Type ID'
			   , reqd_param = 1, widget_id = 9, datatype_id = 4, param_data_source = 'exec spa_source_deal_type_maintain ''x'',@sub_type=''n''', param_default_value = NULL, append_filter = 0, tooltip = NULL, column_template = 2, key_column = 0, required_filter = 0
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
		WHERE ds.[name] = 'PNL Attribution View'
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'source_uom_id' AS [name], 'UOM ID' AS ALIAS, 0 AS reqd_param, 1 AS widget_id, 4 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, 1 AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'PNL Attribution View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'term_start_month' AS [name], 'Term Month' AS ALIAS, 0 AS reqd_param, 1 AS widget_id, 4 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, 1 AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'PNL Attribution View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'term_start_month_quarter' AS [name], 'Term Month Quarter' AS ALIAS, 0 AS reqd_param, 1 AS widget_id, 4 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, 1 AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'PNL Attribution View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'total_change_mtm' AS [name], 'Total Change MTM' AS ALIAS, 0 AS reqd_param, 1 AS widget_id, 3 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, 1 AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'PNL Attribution View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'total_change_vol' AS [name], 'Total Change Volume' AS ALIAS, 0 AS reqd_param, 1 AS widget_id, 3 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, 1 AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'PNL Attribution View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		SET alias = 'Transaction Type ID'
			   , reqd_param = 0, widget_id = 9, datatype_id = 4, param_data_source = 'EXEC spa_StaticDataValues @flag = ''h'', @type_id = 400', param_default_value = NULL, append_filter = 1, tooltip = NULL, column_template = 2, key_column = 0, required_filter = 0
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
		WHERE ds.[name] = 'PNL Attribution View'
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'unexplained_mtm' AS [name], 'Unexplained MTM' AS ALIAS, 0 AS reqd_param, 1 AS widget_id, 3 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, 1 AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'PNL Attribution View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'unexplained_vol' AS [name], 'Unexplained Volume' AS ALIAS, 0 AS reqd_param, 1 AS widget_id, 3 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, 1 AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'PNL Attribution View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'uom_name' AS [name], 'UOM' AS ALIAS, 0 AS reqd_param, 1 AS widget_id, 5 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, 1 AS append_filter, NULL  AS tooltip,0 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'PNL Attribution View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		SET alias = 'Template ID'
			   , reqd_param = 1, widget_id = 9, datatype_id = 4, param_data_source = 'EXEC spa_getDealTemplate @flag=''s''', param_default_value = NULL, append_filter = 0, tooltip = NULL, column_template = 2, key_column = 0, required_filter = 0
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
		WHERE ds.[name] = 'PNL Attribution View'
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'template_id' AS [name], 'Template ID' AS ALIAS, 1 AS reqd_param, 9 AS widget_id, 4 AS datatype_id, 'EXEC spa_getDealTemplate @flag=''s''' AS param_data_source, NULL AS param_default_value, 0 AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, 0 AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'PNL Attribution View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		SET alias = 'Internal Counterparty ID'
			   , reqd_param = 0, widget_id = 9, datatype_id = 4, param_data_source = 'EXEC spa_getsourcecounterparty @flag=''b'', @counterparty_type=''i''', param_default_value = NULL, append_filter = 1, tooltip = NULL, column_template = 2, key_column = 0, required_filter = 0
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
		WHERE ds.[name] = 'PNL Attribution View'
	...
	IF EXISTS (SELECT 1 
	           ...
	           INNER JOIN data_source ds on ds.data_source_id = dsc.source_id 
	           WHERE ds.[name] = 'PNL Attribution View'
	            AND dsc.name =  'attribute_type'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
	BEGIN
		UPDATE dsc  
		SET alias = 'Attribute Type'
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'attribute_type' AS [name], 'Attribute Type' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 5 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,0 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'PNL Attribution View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'Method' AS [name], 'Method' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 3 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'PNL Attribution View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	IF EXISTS (SELECT 1 
	           ...
	           INNER JOIN data_source ds on ds.data_source_id = dsc.source_id 
	           WHERE ds.[name] = 'PNL Attribution View'
	            AND dsc.name =  'pnl_Delta1'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
	BEGIN
		UPDATE dsc  
		SET alias = 'Pnl Delta1'
			   , reqd_param = NULL, widget_id = 1, datatype_id = 3, param_data_source = NULL, param_default_value = NULL, append_filter = NULL, tooltip = NULL, column_template = 2, key_column = 0, required_filter = NULL
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'pnl_Delta1' AS [name], 'Pnl Delta1' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 3 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'PNL Attribution View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	IF EXISTS (SELECT 1 
	           ...
	           INNER JOIN data_source ds on ds.data_source_id = dsc.source_id 
	           WHERE ds.[name] = 'PNL Attribution View'
	            AND dsc.name =  'pnl_Delta2'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
	BEGIN
		UPDATE dsc  
		SET alias = 'Pnl Delta2'
			   , reqd_param = NULL, widget_id = 1, datatype_id = 3, param_data_source = NULL, param_default_value = NULL, append_filter = NULL, tooltip = NULL, column_template = 2, key_column = 0, required_filter = NULL
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'pnl_Delta2' AS [name], 'Pnl Delta2' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 3 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'PNL Attribution View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	IF EXISTS (SELECT 1 
	           ...
	           INNER JOIN data_source ds on ds.data_source_id = dsc.source_id 
	           WHERE ds.[name] = 'PNL Attribution View'
	            AND dsc.name =  'pnl_Gamma1'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
	BEGIN
		UPDATE dsc  
		SET alias = 'Pnl Gamma1'
			   , reqd_param = NULL, widget_id = 1, datatype_id = 3, param_data_source = NULL, param_default_value = NULL, append_filter = NULL, tooltip = NULL, column_template = 2, key_column = 0, required_filter = NULL
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'pnl_Gamma1' AS [name], 'Pnl Gamma1' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 3 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'PNL Attribution View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	IF EXISTS (SELECT 1 
	           ...
	           INNER JOIN data_source ds on ds.data_source_id = dsc.source_id 
	           WHERE ds.[name] = 'PNL Attribution View'
	            AND dsc.name =  'pnl_Gamma2'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
	BEGIN
		UPDATE dsc  
		SET alias = 'Pnl Gamma2'
			   , reqd_param = NULL, widget_id = 1, datatype_id = 3, param_data_source = NULL, param_default_value = NULL, append_filter = NULL, tooltip = NULL, column_template = 2, key_column = 0, required_filter = NULL
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'pnl_Gamma2' AS [name], 'Pnl Gamma2' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 3 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'PNL Attribution View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'pnl_Rho' AS [name], 'Pnl Rho' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 3 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'PNL Attribution View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'pnl_Theta' AS [name], 'Pnl Theta' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 3 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'PNL Attribution View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'pnl_Vega1' AS [name], 'Pnl Vega1' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 3 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'PNL Attribution View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'pnl_Vega2' AS [name], 'Pnl Vega2' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 3 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'PNL Attribution View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
