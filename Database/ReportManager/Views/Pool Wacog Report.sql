...
BEGIN TRY
	...
	FROM report r
	WHERE r.[name] = 'Pool Wacog Report'
	...
	SELECT item
	FROM dbo.splitcommaseperatedvalues('')
	...
	INNER JOIN #del_report_page drp ON drp.report_page_id = rpl.page_id /*********************************************Table and Chart Deletion END *********************************************/ /*********************************************Paramter Deletion START*********************************************/ /* The parameters are deleted unconditionally. (i.e data of the report_param. report_dataset_paramset, report_paramset) */
	...
	DELETE rp
	FROM report_paramset rp
	INNER JOIN #del_report_paramset drp ON drp.report_paramset_id = rp.report_paramset_id /*********************************************Paramter Deletion END*********************************************/ /*********************************************Page Deletion START*********************************************/ /*Delete pages from #del_report_page, that have other paramset defined. These pages shouldnt be deleted*/
	...
	BEGIN
		...
		SELECT TOP 1 'Pool Wacog Report' [name]
			,'farrms_admin' [owner]
			,0 is_system
			,0 is_excel
			,0 is_mobile
			,'EAD0C03E_EDDA_4ED8_B540_C86F9948C09F' report_hash
			,'Pool Wacog Report' [description]
			,CAST(sdv_cat.value_id AS VARCHAR(10)) category_id
		FROM sys.objects o
		LEFT JOIN static_data_value sdv_cat ON sdv_cat.code = NULL
			AND sdv_cat.type_id = - 1
	...
	BEGIN
		...
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Wacog Report'
	...
	SELECT TOP 1 rpage.report_page_id
		,'Pool WACOG Report'
		,'28022DF6_98C6_4493_A203_D91685B85656'
		,1
	FROM sys.objects o
	INNER JOIN report_page rpage ON rpage.[name] = 'Pool Wacog Report'
	INNER JOIN report r ON r.report_id = rpage.report_id
		AND r.[name] = 'Pool Wacog Report'
	...
	SELECT TOP 1 rp.report_paramset_id AS paramset_id
		...
		,'(  wr1.[term_start] >= ''@term_start''  AND wr1.[term_end] <= ''@term_end''  AND wr1.[wacog_group_id] = ''@wacog_group_id'')' AS where_part
		,0
	FROM sys.objects o
	INNER JOIN report_paramset rp ON rp.[name] = 'Pool WACOG Report'
	INNER JOIN report_page rpage ON rpage.report_page_id = rp.page_id
		AND rpage.[name] = 'Pool Wacog Report'
	INNER JOIN report r ON r.report_id = rpage.report_id
		AND r.[name] = 'Pool Wacog Report'
	INNER JOIN report_dataset rd ON rd.report_id = CASE 
			WHEN 'e ' = 'p'
				THEN 29033
			ELSE @report_id_dest
			END
	...
	INNER JOIN report r ON r.report_id = rpage.report_id
		AND r.[name] = 'Pool Wacog Report'
	INNER JOIN report_dataset rd_root ON rd_root.report_id = @report_id_dest
		AND rd_root.[alias] = 'wr1'
	...
	INNER JOIN report_dataset rd ON rd.report_id = r.report_id
		AND rd.[alias] = 'wr1'
	INNER JOIN data_source ds ON ISNULL(NULLIF(ds.report_id, 0), r.report_id) = r.report_id
		AND ds.[name] = 'Wacog Report'
	...
	SELECT TOP 1 rpage.report_page_id AS page_id
		,rd.report_dataset_id AS root_dataset_id
		,'Pool Wacog Report' [name]
		,'8.186666666666667' width
		,'2.6666666666666665' height
		,'0' [top]
		,'0' [left]
		,2 AS group_mode
		,1 AS border_style
	...
	SELECT TOP 1 rpt.report_page_tablix_id tablix_id
		...
		,NULL subtotal
	FROM sys.objects o
	INNER JOIN report_page_tablix rpt ON rpt.[name] = 'Pool Wacog Report'
	INNER JOIN report_page rpage ON rpage.report_page_id = rpt.page_id
		AND rpage.[name] = 'Pool Wacog Report'
	INNER JOIN report r ON r.report_id = rpage.report_id
		AND r.[name] = 'Pool Wacog Report'
	INNER JOIN report_dataset rd ON rd.report_id = r.report_id
...
BEGIN CATCH
	IF @@TRANCOUNT > 0
		ROLLBACK TRANSACTION;--EXEC spa_print 'Error (' + CAST(ERROR_NUMBER() AS VARCHAR(10)) + ') at Line#' + CAST(ERROR_LINE() AS VARCHAR(10)) + ':' + ERROR_MESSAGE() + ''  END CATCH  
