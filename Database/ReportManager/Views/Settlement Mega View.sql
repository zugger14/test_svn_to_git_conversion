BEGIN TRY
	...
	declare @new_ds_alias varchar(10) = 'SSMV12'
	/** IF DATA SOURCE ALIAS ALREADY EXISTS ON DESTINATION, RAISE ERROR **/
	if exists(select top 1 1 from data_source where alias = 'SSMV12' and name <> 'Settlement Mega View')
	begin
		select top 1 @new_ds_alias = 'SSMV12' + cast(s.n as varchar(5))
		from seq s
		left join data_source ds on ds.alias = 'SSMV12' + cast(s.n as varchar(5))
	...
	, @_from_as_of_date VARCHAR(50) --=''2019-12-27''
	, @_to_as_of_date VARCHAR(50) --=''2019-12-27''
	, @_term_start VARCHAR(50) --=''2020-01-01''
	, @_term_end VARCHAR(50) --=''2020-12-31''
	...
	, @_source_deal_header_id VARCHAR(MAX) --= ''172371,172427''
...
SET @_sql_select2 = '' 
		...
		, ISNULL(sdpd.curve_id, sdd.curve_id) curve_id
		, sdpd.inv_cur_id
		, sdpd.deal_cur_id
...
SET @_sql_whr = '' WHERE 1 = 1 ''   
	...
	EXEC (@_sql_select1 + @_sql_select2 + @_sql_from1 + @_sql_from2 + @_sql_whr + '' INSERT INTO #final_values SELECT * FROM #tmp_final_values'')
IF OBJECT_ID(N''tempdb..#settlement_value'') IS NOT NULL
	DROP TABLE #settlement_value
SELECT aod.as_of_date
	...
	, sds1.amount_inv 
	, sds1.amount_deal
INTO #settlement_value -- select * from #final_values
...
CROSS APPLY (SELECT 
			...
			INNER JOIN source_price_curve_def spcd ON spcd.source_curve_def_id = fv.curve_id
			LEFT JOIN holiday_group hg ON hg.hol_group_value_id = spcd.exp_calendar_id
				AND hg.hol_date >= ISNULL(@_term_start, sdh.entire_term_start)
				AND hg.hol_date_to <= ISNULL(@_term_end, sdh.entire_term_end)
			...
			GROUP BY CASE WHEN sdt.deal_type_id = ''Future'' AND idtst.internal_deal_type_subtype_type = ''Physical Future'' AND sdh.physical_financial_flag = ''p'' AND fv.price_curve_granularity = 980 THEN fin_volume_uom ELSE ''1'' END
			) aod
CROSS APPLY (SELECT 
				volume volume_s, 
...
SELECT DISTINCT ISNULL(ifbs_mx.as_of_date, sv.as_of_date) as_of_date
	, fv.source_deal_header_id
	, fv.term_start
	, fv.term_end
	, fv.leg
...
CROSS APPLY (SELECT 
			   ...
			   , field_id 
			   , field_name 
			   , ([value]/ISNULL(NULLIF(volume,0),1)) price 
			   , contract_value 
			   , ''a'' actual_forward 
...
SELECT Isnull(ifbs_mx.as_of_date, sv.from_as_of_date) as_of_date
	, sv.source_deal_header_id
	, ifbs.term_start
	, ifbs.term_end
	, ifbs.leg
	...
	, ''a'' actual_forward
	, null und_pnl_set
	, (ifbs.[value]/ISNULL(NULLIF(ifbs.volume, 0), 1)) deal_price
	, ifbs.volume Volume
...
SELECT source_deal_header_id
	...
	, fv.term_start
	, fv.term_end
	, ISNULL(cf.as_of_date, fv.from_as_of_date)
	, cf.amount
	, cf.volume total_volume
	, fv.source_counterparty_id
	, ISNULL(cf.as_of_date, fv.to_as_of_date)
	, ''a'' AS actual_forward
	, cf.volume Volume
	, cf.agg_term
...
SELECT DISTINCT
 ...
 , fv.term_start 
 , fv.term_end 
 , ISNULL(df.as_of_date, fv.from_as_of_date)
 , df.Amount 
 ...
 , ISNULL(df.as_of_date, fv.to_as_of_date)
...
, fv.product_id 
, fv.commodity_id 
, ISNULL(sv.as_of_date, fv.from_as_of_date) 
, fv.und_intrinsic_pnl 
, fv.und_extrinsic_pnl 
, fv.dis_pnl 
, fv.dis_intrinsic_pnl 
...
, fv.source_counterparty_id 
, fv.discounted_credit_adjusted_mtm 
, ISNULL(sv.as_of_date, fv.to_as_of_date) 
, fv.dis_credit_reserve 
, fv.dis_fair_value 
, fv.pnl_conversion_factor 
, fv.pnl_adjustment_value 
...
LEFT JOIN #final_values fv ON fv.source_deal_header_id = sv.source_deal_header_id AND fv.term_start = sv.term_start AND fv.term_end = sv.term_end AND fv.leg = sv.leg AND (fv.charge_type_id = sv.charge_type_id OR sv.charge_type_id = -10000377)
WHERE sv.charge_type_id IN (-5500, -10000377) AND COALESCE(fv.cash_flow, sv.amount, fv.amount) IS NOT NULL
DELETE #final_values
...
, (CASE WHEN ISNULL(sdd.buy_sell_flag, sdh.header_buy_sell_flag) = ''b'' THEN ''Buy'' ELSE ''Sell'' END ) AS buy_sell_flag
...
, tsd.market_price
, tsd.formula_price
, ABS(tsd.net_price) net_price
, CASE WHEN sdh.physical_financial_flag = ''p'' THEN ''Physical'' ELSE ''Financial'' END [physical_financial_flag]
...
, ABS(tsd.deal_price) AS deal_price
, tsd.float_price
, tsd.Volume
...
, sdh.structured_deal_id AS structured_id
, tsd.pricing_type_id
, tsd.must_run_volume
, tsd.dispatch_volume
...
INTO #tmp_final_data
FROM source_deal_header sdh
INNER JOIN #final_values tsd ON tsd.source_deal_header_id = sdh.source_deal_header_id
...
INNER JOIN source_counterparty AS sc_name ON sc_name.source_counterparty_id = tsd.source_counterparty_id
...
INTO #req_param
FROM #tmp_final_data
WHERE COALESCE(cf_invoice_due_date, pnl_invoice_due_date) IS NOT NULL
...
SELECT DISTINCT
	...
	, sdd.term_start
	, sdd.term_end 
	, dbo.FNAInvoiceDueDate(sdd.term_start, settlement_date, holiday_calendar_id, settlement_days) [pnl_date]
	, dbo.FNAInvoiceDueDate(sdd.term_start, pnl_invoice_due_date, holiday_calendar_id,pnl_payment_days) [payment_date]
	, dbo.FNAInvoiceDueDate(sdd.term_start, cf_invoice_due_date, holiday_calendar_id,cf_payment_days) [cash_flow_year_month]
INTO #tmp_pay_dt
FROM #req_param sdd
...
SELECT 
	...
	, CONVERT(VARCHAR(7), ISNULL(tpd.pnl_date, tfd.term_end), 120) [PNL_year_month] 
--[__batch_report__]  
