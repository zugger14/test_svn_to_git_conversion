 BEGIN TRY
	...
	, @_source_deal_header_id VARCHAR(50) --= ''176,178,180,161''
...
LEFT JOIN #final_values fv ON fv.source_deal_header_id = sv.source_deal_header_id 
...
CROSS APPLY (SELECT 
			...
			AND Isnull(leg, 1) = Isnull(sv.leg, 1)
			AND as_of_date <= Isnull(fv.from_as_of_date, @_from_as_of_date) 
			AND field_id > 0 and internal_type not in (18722,18723,18733,18742,18743)) ifbs_mx
CROSS APPLY (SELECT 
				volume
		...
		AND (as_of_date = ifbs_mx.as_of_date or field_id in (307014,307075)) AND field_id > 0
		  and internal_type not in (18722,18723,18733,18742,18743)) ifbs
...
, tsd.formula
, CASE WHEN sdd.buy_sell_flag = ''Sell'' THEN -1 ELSE 1 END * ABS(tsd.total_volume * ISNULL(t.match_ratio, 1)) total_volume
, puom.uom_name AS [position_uom]
, CAST(ISNULL(sdc.cva,0) AS FLOAT) [CVA]
, CAST(ISNULL(sdc.dva,0) AS FLOAT) [DVA]
, tsd.market_price
, tsd.formula_price
, CASE WHEN sdd.buy_sell_flag = ''Sell'' THEN -1 ELSE 1 END * ABS(tsd.net_price) net_price
