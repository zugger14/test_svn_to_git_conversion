...
FROM #settlement_value sv
	...
	CROSS APPLY (
		SELECT Max(as_of_date) as_of_date FROM index_fees_breakdown_settlement WHERE source_deal_header_id = sv.source_deal_header_id 
			...
			AND field_id > 0 and internal_type not in (18722,18723,18733)
	) ifbs_mx
	CROSS APPLY (
		SELECT volume, 
		...
		AND (as_of_date = ifbs_mx.as_of_date or field_id in (307014,307075)) AND field_id > 0
		  and internal_type not in (18722,18723,18733)
	) ifbs
...
from 
	...
	CROSS APPLY (
		SELECT Max(as_of_date) as_of_date FROM index_fees_breakdown_settlement WHERE source_deal_header_id = sv.source_deal_header_id 
			...
			and internal_type  in (18722,18723,18733)
	) ifbs_mx
	cross join ( VALUES (18722),(18723),(18733)) fee (internal_type)
	outer apply
...
WHERE ifb.field_id IS NOT NULL and ifb.internal_type not in (18722,18723,18733)
