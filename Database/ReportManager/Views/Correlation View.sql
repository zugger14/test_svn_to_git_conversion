BEGIN TRY
	...
	IF NOT EXISTS (SELECT 1 
	           FROM data_source 
	           WHERE [name] = 'Correlation View'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
	BEGIN
		INSERT INTO data_source([type_id], [name], [alias], [description], [tsql], report_id)
		SELECT TOP 1 1 AS [type_id], 'Correlation View' AS [name], 'SCRRV' AS ALIAS, 'Standard Correlation View' AS [description],null AS [tsql], @report_id_data_source_dest AS report_id
	END

	UPDATE data_source
	SET alias = 'SCRRV', description = 'Standard Correlation View'
	...
	@_curve_id_from  VARCHAR(1000),



	@_curve_id_to VARCHAR(1000),



	@_risk_id VARCHAR(1000), 



	@_risk_id1 VARCHAR(1000), 



	@_curve_source_value_id VARCHAR(1000), 



	@_change_indicator CHAR(10)
...
IF OBJECT_ID(N''tempdb..#tmp_curve_correlation'') IS NOT NULL
...
CREATE TABLE #tmp_curve_correlation(
	...
	curve_name_from	VARCHAR(100) COLLATE DATABASE_DEFAULT,



	curve_name_to VARCHAR(100) COLLATE DATABASE_DEFAULT,
	...
	curve_source_name VARCHAR(500) COLLATE DATABASE_DEFAULT,
	...
	term_month_name NVARCHAR(50) COLLATE DATABASE_DEFAULT,
	...
	term_year_month VARCHAR(20) COLLATE DATABASE_DEFAULT,
...
BEGIN
	...
	SET @_sql = ''SELECT 
		...
		CASE WHEN @_curve_id_from IS NOT NULL THEN '' AND curve_id_from IN ('' + @_curve_id_from + '')'' ELSE '''' END +



		CASE WHEN @_curve_id_to IS NOT NULL THEN '' AND curve_id_to IN ('' + @_curve_id_to + '')'' ELSE '''' END +
...
IF @_period_from IS NOT NULL
    ...
    SET @_term_from = dbo.FNAGetTermStartDate(''m'', @_as_of_date_from, @_period_from)
...
IF @_period_to IS NOT NULL
    ...
    SET @_term_to = dbo.FNAGetTermENDDate(''m'', @_as_of_date_from, @_period_to)
...
SET @_sql = ''
	...
	WHERE source_curve_def_id IN ('' + @_curve_id_from + '')''
...
EXEC sp_executesql @_sql, N''@_risk_id NVARCHAR(1000) OUT'', @_risk_id OUT
...
SET @_sql = ''
	...
	WHERE source_curve_def_id IN ('' + @_curve_id_to + '')''
...
EXEC sp_executesql @_sql, N''@_risk_id1 NVARCHAR(1000) OUT'', @_risk_id1 OUT
...
SELECT @_risk_id = ISNULL(@_risk_id, @_curve_id_from), @_risk_id1 = ISNULL(@_risk_id1, @_curve_id_to) 
	...
	SET @_sql = ''INSERT INTO #tmp_curve_correlation
		...
		SELECT
			   ...
			   '''''' + ISNULL(@_change_indicator, ''n'') + '''''', 



			   '' + CASE WHEN @_curve_id_from IS NOT NULL OR @_curve_id_to IS NOT NULL THEN  ''isnull(spcd_org.curve_name,spcd.curve_name) '' ELSE ''spcd.curve_name'' END +'' [curve_name_from],  



			   '' + CASE WHEN @_curve_id_from IS NOT NULL OR @_curve_id_to IS NOT NULL THEN  ''isnull(spcd_org2.curve_name,spcd2.curve_name) '' ELSE ''spcd2.curve_name'' END +'' [curve_name_to],



			   ISNULL(spcd_org.source_curve_def_id,spcd.source_curve_def_id)  [curve_id_from],



			   ISNULL(spcd_org2.source_curve_def_id,spcd2.source_curve_def_id) [curve_id_to],
			   ...
			   cc.curve_source_value_id [curve_source_value_id],
			   ...
			   DAY(term1) [as_of_date_day],



			   MONTH(term1) [as_of_date_month],



			   DATENAME(MONTH, term1) [as_of_date_month_name],



			   YEAR(term1) [as_of_date_year],



			   CAST(YEAR(term1) AS VARCHAR(4)) + ''''-'''' + RIGHT(''''00'''' + CAST(MONTH(term1) AS VARCHAR(2)), 2) 
			   ...
			   cc.as_of_date [as_of_date],



			   cc.as_of_date [to_as_of_date],



			   DATEDIFF(MM, cc.as_of_date, cc.term1) AS period_from,



			   DATEDIFF(MM, cc.as_of_date, cc.term1) AS period_to,
		...
		+ CASE WHEN @_curve_id_from IS NOT NULL THEN '' AND (spcd_org.source_curve_def_id IN('' + @_curve_id_from + '')'' + CASE WHEN @_curve_id_to IS NULL THEN '')'' ELSE '''' END ELSE '''' END  



		+ CASE WHEN @_curve_id_to IS NOT NULL THEN CASE WHEN @_curve_id_from IS NULL THEN '' AND ( '' ELSE  '' or '' END + '' spcd_org.source_curve_def_id IN('' + @_curve_id_to + ''))'' ELSE '''' END +'' 
		...
		+ CASE WHEN @_curve_id_from IS NOT NULL THEN '' AND ( spcd_org2.source_curve_def_id IN('' + @_curve_id_from + '')'' + CASE WHEN @_curve_id_to IS NULL THEN '')'' ELSE '''' END ELSE '''' END  



		+ CASE WHEN @_curve_id_to IS NOT NULL THEN CASE WHEN @_curve_id_from IS NULL THEN '' AND ( '' ELSE  '' or '' END + '' spcd_org2.source_curve_def_id IN('' + @_curve_id_to + ''))'' ELSE '''' END + ''    
		...
		CASE WHEN @_as_of_date_to IS NULL THEN ''cc.as_of_date = '''''' + CAST(@_as_of_date_from AS VARCHAR) + '''''''' ELSE ''cc.as_of_date >= '''''' + CAST(@_as_of_date_from AS VARCHAR) + '''''' AND cc.as_of_date <= '''''' + CAST(@_as_of_date_to AS VARCHAR) + '''''''' END +



		CASE WHEN @_curve_id_from IS NOT NULL THEN '' AND cc.curve_id_from IN( '' + @_risk_id + '', '' + ISNULL(@_risk_id1, @_risk_id) + '')'' ELSE '''' END +



		CASE WHEN @_risk_id1 IS NOT NULL THEN '' AND cc.curve_id_to IN( '' + ISNULL(@_risk_id, @_risk_id1) + '','' + @_risk_id1 + '')'' ELSE '''' END +



		CASE WHEN @_curve_source_value_id IS NOT NULL THEN '' AND cc.curve_source_value_id IN('' + @_curve_source_value_id + '')'' ELSE '''' END +



		CASE WHEN @_term_from IS NOT NULL THEN '' AND cc.term1 >= '''''' + CAST(@_term_from AS VARCHAR) + '''''' AND cc.term2 >= '''''' + CAST(@_term_from AS VARCHAR) + ''''''''  ELSE '''' END +



		CASE WHEN @_term_to IS NOT NULL THEN '' AND cc.term1 <= '''''' + CAST(@_term_to AS VARCHAR) + '''''' AND cc.term2 <= '''''' + CAST(@_term_to AS VARCHAR) + ''''''''  ELSE '''' END
	...
	SELECT * INTO #tmp_final_result FROM #tmp_curve_correlation WHERE 1 = 2 
	...
	IF ISNULL(@_change_indicator, ''n'') = ''n''
		...
		SELECT * FROM #tmp_curve_correlation
	...
	ELSE 
		...
		SELECT
			...
			MAX(vv_cur.curve_name_from) AS curve_name_from,



			MAX(vv_cur.curve_name_to) AS curve_name_to,
			...
			MAX(vv_cur.value) value,



			MAX(vv_prior.value) prior_value,



			(MAX(vv_cur.value) - MAX(vv_prior.value)) AS value_change,
			...
			MAX(vv_cur.curve_source_name) AS curve_source_name,
			...
			MAX(vv_cur.period_from) AS period_from,
		...
		FROM (
			 ...
			 SELECT MAX(crvi.as_of_date_to) as_of_date_to, 



				MIN(crvi.as_of_date_from) as_of_date_from,



			    crvi.curve_id_from, crvi.curve_id_to



			 FROM #tmp_curve_correlation crvi GROUP BY crvi.curve_id_from, crvi.curve_id_to) ca 



		INNER JOIN #tmp_curve_correlation vv_cur ON vv_cur.curve_id_from = ca.curve_id_from 



			AND vv_cur.curve_id_to = ca.curve_id_to



			AND vv_cur.as_of_date_from = ca.as_of_date_from



		INNER JOIN #tmp_curve_correlation vv_prior ON vv_prior.curve_id_from = ca.curve_id_from 



			AND vv_prior.curve_id_to = ca.curve_id_to



			AND vv_prior.as_of_date_to = ca.as_of_date_to



			AND vv_cur.term_from = vv_prior.term_from 



			AND vv_cur.term_to = vv_prior.term_to



		WHERE ca.as_of_date_from = @_as_of_date_from 



			AND ca.as_of_date_to = @_as_of_date_to



		GROUP BY ca.curve_id_from, 
	...
	IF EXISTS (SELECT 1 
	           ...
	           WHERE ds.[name] = 'Correlation View'
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'term_from' AS [name], 'Term From' AS ALIAS, 1 AS reqd_param, 6 AS widget_id, 2 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, 0 AS append_filter, NULL  AS tooltip,4 AS column_template, 1 AS key_column, 0 AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Correlation View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'term_to' AS [name], 'Term To' AS ALIAS, 1 AS reqd_param, 6 AS widget_id, 2 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, 0 AS append_filter, NULL  AS tooltip,4 AS column_template, 0 AS key_column, 0 AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Correlation View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'curve_name_from' AS [name], 'Index From' AS ALIAS, 0 AS reqd_param, 1 AS widget_id, 5 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, 1 AS append_filter, NULL  AS tooltip,0 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Correlation View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'curve_name_to' AS [name], 'Index To' AS ALIAS, 0 AS reqd_param, 1 AS widget_id, 5 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, 1 AS append_filter, NULL  AS tooltip,0 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Correlation View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'prior_value' AS [name], 'Prior Value' AS ALIAS, 0 AS reqd_param, 1 AS widget_id, 3 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, 1 AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Correlation View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
