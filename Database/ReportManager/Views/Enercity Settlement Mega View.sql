...
SELECT   sub_id
		...
		, CASE WHEN (charge_type =''Positive Price Commodity'' OR charge_type =''Commodity'') AND buy_sell_flag = ''Buy'' THEN ABS(Volume)

		  WHEN (charge_type =''Positive Price Commodity'' OR charge_type =''Commodity'') AND buy_sell_flag = ''Sell'' THEN (ABS(Volume)*-1)

		  WHEN (charge_type =''Negative Price Commodity'' OR charge_type = ''Commodity'') AND buy_sell_flag = ''Buy'' THEN ABS(Volume)

		  WHEN (charge_type =''Negative Price Commodity'' OR charge_type = ''Commodity'') AND buy_sell_flag = ''Sell'' THEN (ABS(Volume)*-1) 
		...
		WHEN (charge_type =''Positive Price Commodity'' OR charge_type =''Commodity'') AND buy_sell_flag = ''Buy'' THEN (ABS(IIF(actual_forward= ''f'', IIF(ISNULL(deal_type_name, ''Non'') = ''Physical'', ABS(contract_value), ABS(und_pnl_set) ),ABS(contract_value)))*-1)

		WHEN (charge_type =''Positive Price Commodity'' OR charge_type =''Commodity'') AND buy_sell_flag = ''Sell'' THEN ABS(IIF(actual_forward= ''f'', IIF(ISNULL(deal_type_name, ''Non'') = ''Physical'', ABS(contract_value), ABS(und_pnl_set) ),ABS(contract_value)))

		  

		  WHEN (charge_type =''Negative Price Commodity'' OR charge_type = ''Commodity'') AND buy_sell_flag = ''Buy'' THEN ABS(IIF(actual_forward= ''f'', IIF(ISNULL(deal_type_name, ''Non'') = ''Physical'', ABS(contract_value), ABS(und_pnl_set) ),ABS(contract_value)))

		  WHEN (charge_type =''Negative Price Commodity'' OR charge_type = ''Commodity'') AND buy_sell_flag = ''Sell'' THEN (ABS(IIF(actual_forward= ''f'', IIF(ISNULL(deal_type_name, ''Non'') = ''Physical'', ABS(contract_value), ABS(und_pnl_set) ),ABS(contract_value))) *-1)
		...
		WHEN (charge_type =''Positive Price Commodity'' OR charge_type =''Commodity'') AND buy_sell_flag = ''Buy'' THEN (ABS(IIF(actual_forward= ''f'', IIF(ISNULL(deal_type_name, ''Non'') = ''Physical'', ABS(contract_value), ABS(und_pnl_set) ),ABS(contract_value)))*-1)/ NULLIF((ABS(Volume)*-1),0)


		WHEN (charge_type =''Positive Price Commodity'' OR charge_type =''Commodity'') AND buy_sell_flag = ''Sell'' THEN ABS(IIF(actual_forward= ''f'', IIF(ISNULL(deal_type_name, ''Non'') = ''Physical'', ABS(contract_value), ABS(und_pnl_set) ),ABS(contract_value))) / (NULLIF((ABS(Volume)*-1),0)*-1)


		  

		  WHEN (charge_type =''Negative Price Commodity'' OR charge_type = ''Commodity'') AND buy_sell_flag = ''Buy'' THEN ABS(IIF(actual_forward= ''f'', IIF(ISNULL(deal_type_name, ''Non'') = ''Physical'', ABS(contract_value), ABS(und_pnl_set) ),ABS(contract_value)))/ NULLIF((ABS(Volume)*-1),0) 


		  WHEN (charge_type =''Negative Price Commodity'' OR charge_type = ''Commodity'') AND buy_sell_flag = ''Sell'' THEN (ABS(IIF(actual_forward= ''f'', IIF(ISNULL(deal_type_name, ''Non'') = ''Physical'', ABS(contract_value), ABS(und_pnl_set) ),ABS(contract_value))) *-1)/ (NULLIF((ABS(Volume)*-1),0)*-1)
