BEGIN TRY
	...
	declare @new_ds_alias varchar(10) = 'SCVA'
	/** IF DATA SOURCE ALIAS ALREADY EXISTS ON DESTINATION, RAISE ERROR **/
	if exists(select top 1 1 from data_source where alias = 'SCVA' and name <> 'CVA View')
	begin
		select top 1 @new_ds_alias = 'SCVA' + cast(s.n as varchar(5))
		from seq s
		left join data_source ds on ds.alias = 'SCVA' + cast(s.n as varchar(5))
	...
	AND NOT EXISTS (SELECT 1 FROM map_function_category WHERE [function_name] = 'CVA View' AND '106500' = '106501') 
	BEGIN
		INSERT INTO data_source([type_id], [name], [alias], [description], [tsql], report_id, system_defined,category)
		SELECT TOP 1 1 AS [type_id], 'CVA View' AS [name], @new_ds_alias AS ALIAS, 'Standard CVA View' AS [description],null AS [tsql], @report_id_data_source_dest AS report_id,'1' AS [system_defined]
			,'106500' AS [category]
	END

	UPDATE data_source
	SET alias = @new_ds_alias, description = 'Standard CVA View'
	, [tsql] = CAST('' AS VARCHAR(MAX)) + 'DECLARE @_sql VARCHAR(8000), @_to_as_of_date VARCHAR(10),@_round VARCHAR(100),@_use_simulated_exposure VARCHAR(100)
...
IF ''@use_simulated_exposure'' <> ''NULL''



	SET @_use_simulated_exposure = ''@use_simulated_exposure''
...
SELECT 	s.as_of_date [to_as_of_date],
		...
		s.Source_Counterparty_ID [counterparty_id],
		...
		''+ CASE WHEN @_use_simulated_exposure = ''n'' THEN ''ROUND(SUM(credit_adjustment_mtm), ''+@_round+'')'' ELSE ''0'' END+'' [credit_adjusted_mtm],



		''+ CASE WHEN @_use_simulated_exposure = ''n'' THEN ''ROUND(SUM(adjusted_discounted_mtm), ''+@_round+'')'' ELSE ''0'' END+'' [adjusted_discounted_mtm],
		...
		MAX(s.curve_source_value_id) [curve_source_value_id],



		MAX(sdv.code) [curve_source],



		ROUND(SUM(s.cva), ''+@_round+'') cva,



		''+ CASE WHEN @_use_simulated_exposure = ''n'' THEN ''ROUND(SUM(s.d_cva), ''+@_round+'')'' ELSE ''0'' END +'' d_cva,



		ROUND(SUM(s.dva), ''+@_round+'') dva,



		''+ CASE WHEN @_use_simulated_exposure = ''n'' THEN ''ROUND(SUM(s.d_dva), ''+@_round+'')'' ELSE ''0'' END +'' d_dva,



		MAX(sdv1.code) entity_type,



		ROUND(SUM(s.exposure_to_us), ''+@_round+'') [exposure_to_us],



		ROUND(SUM(s.exposure_to_them), ''+@_round+'') [exposure_to_them],



		''+ CASE WHEN @_use_simulated_exposure = ''n'' THEN ''ROUND(SUM(s.Final_Und_Pnl), ''+@_round+'')'' ELSE ''0'' END +'' mtm,



		s.term_start term_start,



		s.term_start term_end,
		...
		''''''+@_use_simulated_exposure+'''''' use_simulated_exposure,



		MAX(sc_int.source_counterparty_id) internal_counterparty_id,



		MAX(sc_int.counterparty_name) internal_counterparty_name,



		MAX(sc_int.counterparty_id) internal_counterparty_code,
		...
		DAY(s.term_start) [term_start_day],       



		RIGHT(''''0''''+ CAST(MONTH(s.term_start) AS VARCHAR(2)), 2)   [term_start_month],



		DATENAME(m,s.term_start) [term_start_month_name],



		YEAR(s.term_start) [term_start_year],



		CAST(YEAR(s.term_start) AS VARCHAR(5)) + ''''-'''' + RIGHT(''''0''''+ CAST(MONTH(s.term_start) AS VARCHAR(2)), 2)   [term_start_year_month],



		''''Q'''' + CAST(DATEPART(q,s.term_start) AS VARCHAR) [term_quarter]	
...
+ CASE WHEN @_use_simulated_exposure = ''n'' THEN ''source_deal_cva'' ELSE ''source_deal_cva_simulation'' END +'' s
...
INNER JOIN dbo.source_deal_header sdh ON  sdh.source_deal_header_id = s.source_deal_header_id
...
LEFT JOIN static_data_value sdv ON sdv.value_id = s.curve_source_value_id



LEFT JOIN static_data_value sdv1 ON sdv.value_id = sc.type_of_entity
...
LEFT JOIN source_counterparty sc_int ON sc_int.source_counterparty_id = COALESCE(sdh.internal_counterparty,fs.counterparty_id)
...
LEFT JOIN source_currency cur ON cur.source_currency_id = cci.curreny_code
...
+ CASE	WHEN @_to_as_of_date IS NULL THEN ''AND dbo.FNAGetSQLStandardDate(s.as_of_date) = ''''@as_of_date'''''' 



		ELSE '' AND dbo.FNAGetSQLStandardDate(s.as_of_date) >= ''''@as_of_date'''' AND dbo.FNAGetSQLStandardDate(s.as_of_date) <= ''''@to_as_of_date''''  '' 
...
'' GROUP BY
       ...
       s.Source_Counterparty_ID,s.term_start ''
...
EXEC(@_sql)', report_id = @report_id_data_source_dest,
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'credit_adjusted_mtm' AS [name], 'Credit Adjusted MTM' AS ALIAS, 0 AS reqd_param, 1 AS widget_id, 4 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, 1 AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'CVA View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'd_cva' AS [name], 'Discounted CVA' AS ALIAS, 0 AS reqd_param, 1 AS widget_id, 4 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, 1 AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'CVA View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'd_dva' AS [name], 'Discounted DVA' AS ALIAS, 0 AS reqd_param, 1 AS widget_id, 4 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, 1 AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'CVA View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		SET alias = 'Internal Counterparty ID'
			   , reqd_param = 0, widget_id = 9, datatype_id = 4, param_data_source = 'SELECT source_counterparty_id, sc.counterparty_name FROM source_counterparty sc ' + CHAR(10) + 'INNER JOIN counterparty_credit_info cci ON  cci.Counterparty_id = sc.source_counterparty_id ' + CHAR(10) + 'WHERE  int_ext_flag = ''i''', param_default_value = NULL, append_filter = 1, tooltip = NULL, column_template = 2, key_column = 1, required_filter = NULL
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
		WHERE ds.[name] = 'CVA View'
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'mtm' AS [name], 'MTM' AS ALIAS, 0 AS reqd_param, 1 AS widget_id, 4 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, 1 AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'CVA View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'round' AS [name], 'Round' AS ALIAS, 1 AS reqd_param, 1 AS widget_id, 4 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, 0 AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, 0 AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'CVA View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		UPDATE dsc  
		SET alias = 'Use Simulated Exposure'
			   , reqd_param = 1, widget_id = 2, datatype_id = 5, param_data_source = 'Select ''y'' code, ''Yes'' Value UNION' + CHAR(10) + 'SELECT ''n'' code, ''No'' Value', param_default_value = NULL, append_filter = 0, tooltip = NULL, column_template = 0, key_column = 1, required_filter = 1
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
		WHERE ds.[name] = 'CVA View'
	...
	IF EXISTS (SELECT 1 
	           ...
	           INNER JOIN data_source ds on ds.data_source_id = dsc.source_id 
	           WHERE ds.[name] = 'CVA View'
	            AND dsc.name =  'adjusted_discounted_mtm'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
	BEGIN
		UPDATE dsc  
		SET alias = 'Adjusted Discounted MTM'
			   , reqd_param = 0, widget_id = 1, datatype_id = 4, param_data_source = NULL, param_default_value = NULL, append_filter = 1, tooltip = NULL, column_template = 2, key_column = 0, required_filter = NULL
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
