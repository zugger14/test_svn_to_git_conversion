BEGIN TRY
	...
	declare @new_ds_alias varchar(10) = 'SERV'
	/** IF DATA SOURCE ALIAS ALREADY EXISTS ON DESTINATION, RAISE ERROR **/
	if exists(select top 1 1 from data_source where alias = 'SERV' and name <> 'Expected Return View')
	begin
		select top 1 @new_ds_alias = 'SERV' + cast(s.n as varchar(5))
		from seq s
		left join data_source ds on ds.alias = 'SERV' + cast(s.n as varchar(5))
	...
	IF NOT EXISTS (SELECT 1 
	           FROM data_source 
	           WHERE [name] = 'Expected Return View'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
	AND NOT EXISTS (SELECT 1 FROM map_function_category WHERE [function_name] = 'Expected Return View' AND '106500' = '106501') 
	BEGIN
		INSERT INTO data_source([type_id], [name], [alias], [description], [tsql], report_id, system_defined,category)
		SELECT TOP 1 1 AS [type_id], 'Expected Return View' AS [name], @new_ds_alias AS ALIAS, 'Standard Expected Return View' AS [description],null AS [tsql], @report_id_data_source_dest AS report_id,'1' AS [system_defined]
			,'106500' AS [category]
	END

	UPDATE data_source
	SET alias = @new_ds_alias, description = 'Standard Expected Return View'
...
IF OBJECT_ID(N''tempdb..#tmp_expected_return'') IS NOT NULL


	DROP TABLE #tmp_expected_return





	CREATE TABLE #tmp_expected_return(


		change_indicator VARCHAR(1) COLLATE DATABASE_DEFAULT,
		...
		curve_name	VARCHAR(200) COLLATE DATABASE_DEFAULT,
		...
		curve_source_name	NVARCHAR(50) COLLATE DATABASE_DEFAULT,
	...
	SET @_sql = ''INSERT INTO #tmp_expected_return(
		...
		SELECT '''''' + ISNULL(@_change_indicator, ''n'') + '''''',
			   ...
			   DAY(er.term) [term_day],


			   MONTH(er.term) [term_month],


			   dbo.FNAGetMonthName(MONTH(er.term)) [term_month_name],


			   YEAR(er.term) [term_year],


			   CONVERT(VARCHAR(7), er.term, 120) [term_year_month],	   


			   ISNULL(spcd_org.curve_name,spcd.curve_name) [curve_name],


			   er.granularity AS granularity_id,


			   sdv1.code [granularity],


			   er.[value] [value_at_granularity],


			   CASE er.granularity


					WHEN 700 THEN (er.value  * 252) --to calculate annual value


					WHEN 701 THEN (er.value  * 52) -- 52 weeks in a year


					WHEN 703 THEN (er.value  * 12) -- 12 months in a year


					WHEN 704 THEN (er.value  * 4) -- 4 quarters in a year


					WHEN 706 THEN er.value  


					END [annual_value],  


			   er.curve_source_value_id [curve_source_value_id], 
			   ...
			   er.as_of_date [as_of_date],


			   er.as_of_date [to_as_of_date],
			   ...
			   DATEDIFF(MM, er.as_of_date, er.term) AS period_from,


			   DATEDIFF(MM, er.as_of_date, er.term) AS period_to,
		...
		JOIN source_price_curve_def spcd ON spcd.source_curve_def_id = er.curve_id


		JOIN static_data_value sdv ON sdv.value_id = er.curve_source_value_id


		LEFT JOIN static_data_value sdv1 ON sdv1.value_id = er.granularity
		...
		+ CASE WHEN @_as_of_date_to IS NULL THEN '' er.as_of_date = '''''' + CAST(@_as_of_date_from AS VARCHAR) + '''''''' ELSE '' er.as_of_date >= '''''' + CAST(@_as_of_date_from AS VARCHAR) + '''''' AND er.as_of_date <= '''''' + CAST(@_as_of_date_to AS VARCHAR) + '''''''' END +


		CASE WHEN @_curve_id IS NOT NULL THEN '' AND er.curve_id IN('' + @_curve_id + '')'' ELSE '''' END + 


		CASE WHEN @_curve_source_value_id IS NOT NULL THEN '' AND er.curve_source_value_id IN('' + @_curve_source_value_id + '')'' ELSE '''' END +


		'' ORDER BY [curve_name], er.as_of_date, er.term''
	...
	SELECT * INTO #tmp_final_result FROM #tmp_expected_return WHERE 1 = 2 
	...
	IF ISNULL(@_change_indicator, ''n'') = ''n''
		...
		SELECT * FROM #tmp_expected_return
	...
	ELSE 
		...
		SELECT
			...
			MAX(vv_cur.granularity) AS granularity,
			...
			vv_cur.curve_source_value_id AS curve_source_value_id,
			...
			MAX(vv_cur.term_from) AS term_from,


			MAX(vv_cur.term_to) AS term_to,


			MAX(vv_cur.period_from) period_from,


			MAX(vv_cur.period_to) period_to,
		...
		FROM (
			...
			SELECT vvi.curve_id, MAX(vvi.as_of_date_to) as_of_date_to, MIN(vvi.as_of_date_from) as_of_date_from


			FROM #tmp_expected_return vvi
		...
		INNER JOIN #tmp_expected_return vv_cur ON vv_cur.curve_id = ca.curve_id


			AND vv_cur.as_of_date_to = ca.as_of_date_to


		INNER JOIN #tmp_expected_return vv_prior ON vv_prior.curve_id = ca.curve_id


			AND vv_prior.as_of_date_from = ca.as_of_date_from


			AND vv_cur.term_from = vv_prior.term_from


		WHERE ca.as_of_date_from = @_as_of_date_from AND ca.as_of_date_to = @_as_of_date_to


		GROUP BY ca.curve_id, ca.as_of_date_to, ca.as_of_date_from, vv_cur.term_from, vv_cur.curve_source_value_id
	...
	IF EXISTS (SELECT 1 
	           ...
	           WHERE ds.[name] = 'Expected Return View'
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'term_from' AS [name], 'Term From' AS ALIAS, 0 AS reqd_param, 6 AS widget_id, 2 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, 1 AS append_filter, NULL  AS tooltip,4 AS column_template, 1 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Expected Return View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'term_to' AS [name], 'Term To' AS ALIAS, 0 AS reqd_param, 6 AS widget_id, 2 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, 1 AS append_filter, NULL  AS tooltip,4 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Expected Return View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		UPDATE dsc  
		SET alias = 'Value at Granularity'
			   , reqd_param = 0, widget_id = 1, datatype_id = 3, param_data_source = NULL, param_default_value = NULL, append_filter = 1, tooltip = NULL, column_template = 2, key_column = 0, required_filter = NULL
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'value_at_granularity' AS [name], 'Value at Granularity' AS ALIAS, 0 AS reqd_param, 1 AS widget_id, 3 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, 1 AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Expected Return View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
