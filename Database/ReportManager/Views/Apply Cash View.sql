BEGIN TRY
	...
	declare @new_ds_alias varchar(10) = 'ACV'
	/** IF DATA SOURCE ALIAS ALREADY EXISTS ON DESTINATION, RAISE ERROR **/
	if exists(select top 1 1 from data_source where alias = 'ACV' and name <> 'Apply Cash View')
	begin
		select top 1 @new_ds_alias = 'ACV' + cast(s.n as varchar(5))
		from seq s
		left join data_source ds on ds.alias = 'ACV' + cast(s.n as varchar(5))
	...
	IF NOT EXISTS (SELECT 1 
	           FROM data_source 
	           WHERE [name] = 'Apply Cash View'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
	AND NOT EXISTS (SELECT 1 FROM map_function_category WHERE [function_name] = 'Apply Cash View' AND '106500' = '106501') 
	BEGIN
		INSERT INTO data_source([type_id], [name], [alias], [description], [tsql], report_id, system_defined,category)
		SELECT TOP 1 1 AS [type_id], 'Apply Cash View' AS [name], @new_ds_alias AS ALIAS, 'Standard Apply Cash View' AS [description],null AS [tsql], @report_id_data_source_dest AS report_id,'1' AS [system_defined]
			,'106500' AS [category]
	END

	UPDATE data_source
	SET alias = @new_ds_alias, description = 'Standard Apply Cash View'
	, [tsql] = CAST('' AS VARCHAR(MAX)) + 'IF OBJECT_ID(''tempdb..#temp_ar'') IS NOT NULL			
	DROP TABLE #temp_ar

DECLARE @_as_of_date DATE

IF ''@as_of_date'' <> ''NULL''
	SET @_as_of_date = ''@as_of_date''
ELSE
	SELECT TOP 1 @_as_of_date = CONVERT(VARCHAR(10), custom_as_of_date, 121) FROM setup_as_of_date WHERE custom_as_of_date IS NOT NULL
SELECT

civv.as_of_date as_of_date,
sc.counterparty_name counterparty,
sc.source_counterparty_id counterparty_id,
parent_counterparty.counterparty_name parent_counterparty,
parent_counterparty.source_counterparty_id parent_counterparty_id,
sc.type_of_entity counterparty_type,
...
civv.invoice_number,
civv.invoice_id invoice_id,
civv.prod_date term_start,
dbo.FNAInvoiceDueDate((ISNULL((civv.prod_date), GETDATE())),(cg.invoice_due_date),(cg.holiday_calendar_id),(cg.payment_days)) invoice_due_date
,isnull(invoice_amount, civ.value ) invoice_amount
,icr.cash_received
,icr.received_date,
icr.variance_amount
into #temp_ar
...
OUTER APPLY (
SELECT COUNT(civ1.calc_id) calc_id 
from calc_invoice_volume civ1 
where civv.calc_id = civ1.calc_id) t
where civv.prod_date <= @_as_of_date
AND (''NULL'' = ''@source_counterparty_id'' OR sc.source_counterparty_id IN (@source_counterparty_id))

IF OBJECT_ID(''tempdb..#temp_ar2'') IS NOT NULL			
	DROP TABLE #temp_ar2
...
MAX(parent_counterparty_id) parent_counterparty_id,
MAX(counterparty_type) counterparty_type,
MAX(counterparty_code) counterparty_code,
MAX((CASE
	WHEN ISNULL(invoice_due_date,''1900-01-01'') >= @_as_of_date THEN '' Current''
	WHEN ABS(datediff(day, @_as_of_date,invoice_due_date)) BETWEEN 1 AND 30 THEN ''1-30PD''
	WHEN ABS(datediff(day, @_as_of_date,invoice_due_date)) BETWEEN 31 AND 60 THEN ''31-60PD''
	WHEN ABS(datediff(day, @_as_of_date,invoice_due_date)) BETWEEN 61 AND 90 THEN ''61-90PD''
	ELSE ''Over 90PD''
 END)) AS term,
SUM(gross_exposure) gross_exposure,
invoice_number,
MAX(invoice_id) invoice_id,
MAX(term_start) term_start,
invoice_due_date ,
sum(invoice_amount) invoice_amount,
sum(cash_received) cash_received ,
MAX(received_date) received_date,
SUM(variance_amount) variance_amount
into #temp_ar2
FROM #temp_ar
GROUP BY counterparty_id,invoice_number,invoice_due_date

--[__batch_report__] 

SELECT 
as_of_date,counterparty, counterparty_id source_counterparty_id,parent_counterparty,
parent_counterparty_id,counterparty_type,counterparty_code,term,
--(CASE WHEN gross_exposure < 0 THEN 0 ELSE gross_exposure END)
 gross_exposure,
invoice_number,invoice_id,term_start,invoice_due_date,
--(CASE WHEN oa.amount < 0 THEN 0 ELSE oa.amount END)
 amount, invoice_amount,cash_received, received_date, variance_amount
from #temp_ar2 ta
OUTER APPLY(
SELECT SUM(gross_exposure) amount FROM #temp_ar tr
WHERE ta.counterparty_id = tr.counterparty_id
and ta.invoice_number = tr.invoice_number 
) oa
 ', report_id = @report_id_data_source_dest,
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'counterparty_type' AS [name], 'Counterparty Type' AS ALIAS, 0 AS reqd_param, 1 AS widget_id, 4 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, 1 AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Apply Cash View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'invoice_id' AS [name], 'Invoice ID' AS ALIAS, 0 AS reqd_param, 1 AS widget_id, 4 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, 1 AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Apply Cash View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'invoice_due_date' AS [name], 'Invoice Due Date' AS ALIAS, 0 AS reqd_param, 6 AS widget_id, 2 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, 1 AS append_filter, NULL  AS tooltip,4 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Apply Cash View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	IF EXISTS (SELECT 1 
	           ...
	           INNER JOIN data_source ds on ds.data_source_id = dsc.source_id 
	           WHERE ds.[name] = 'Apply Cash View'
	            AND dsc.name =  'invoice_amount'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
	BEGIN
		UPDATE dsc  
		SET alias = 'Invoice Amount'
			   , reqd_param = 0, widget_id = 1, datatype_id = 3, param_data_source = NULL, param_default_value = NULL, append_filter = 1, tooltip = NULL, column_template = 2, key_column = 0, required_filter = NULL
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'invoice_amount' AS [name], 'Invoice Amount' AS ALIAS, 0 AS reqd_param, 1 AS widget_id, 3 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, 1 AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Apply Cash View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	IF EXISTS (SELECT 1 
	           ...
	           INNER JOIN data_source ds on ds.data_source_id = dsc.source_id 
	           WHERE ds.[name] = 'Apply Cash View'
	            AND dsc.name =  'received_date'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
	BEGIN
		UPDATE dsc  
		SET alias = 'Received Date'
			   , reqd_param = 0, widget_id = 6, datatype_id = 2, param_data_source = NULL, param_default_value = NULL, append_filter = 1, tooltip = NULL, column_template = 4, key_column = 0, required_filter = NULL
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'received_date' AS [name], 'Received Date' AS ALIAS, 0 AS reqd_param, 6 AS widget_id, 2 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, 1 AS append_filter, NULL  AS tooltip,4 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Apply Cash View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	IF EXISTS (SELECT 1 
	           ...
	           INNER JOIN data_source ds on ds.data_source_id = dsc.source_id 
	           WHERE ds.[name] = 'Apply Cash View'
	            AND dsc.name =  'variance_amount'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
	BEGIN
		UPDATE dsc  
		SET alias = 'Variance Amount'
			   , reqd_param = 0, widget_id = 1, datatype_id = 3, param_data_source = NULL, param_default_value = NULL, append_filter = 1, tooltip = NULL, column_template = 2, key_column = 0, required_filter = NULL
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'variance_amount' AS [name], 'Variance Amount' AS ALIAS, 0 AS reqd_param, 1 AS widget_id, 3 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, 1 AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Apply Cash View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
