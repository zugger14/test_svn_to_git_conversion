BEGIN TRY
	...
	IF EXISTS (SELECT 1 
	           FROM data_source 
	           WHERE [name] = 'Position Hourly View Excluding Sub Book'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
			
	BEGIN
		UPDATE data_source
		SET alias = 'PVHEWBT', description = ''
...
IF ''@period_to'' <> ''NULL''
    SET @_term_start_to = dbo.FNADateFormat(dbo.FNAGetTermENDDate(''m'', @_temp_date, ''@period_to''))
...
SELECT CAST(@_as_of_date AS DATETIME)     as_of_date,
       ...
       vw.is_fixedvolume, ssbm.fas_deal_sub_type_value_id,
...
FROM   #temp_hourly_position thp
       ...
       INNER JOIN source_deal_detail sdd WITH(NOLOCK)
            ON  unpvt.term_start BETWEEN sdd.term_start AND sdd.term_end AND sdd.source_deal_header_id = unpvt.source_deal_header_id
            AND unpvt.index_id = CASE WHEN unpvt.is_fixedvolume = ''y'' THEN sdd.formula_curve_id ELSE sdd.curve_id END
...
DROP TABLE #temp_block_type_group_table
DROP TABLE #temp_hourly_position
DROP TABLE #temp_report_hourly_position_breakdown', report_id = @report_id_data_source_dest 
		WHERE [name] = 'Position Hourly View Excluding Sub Book'
			AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1)
	END	
	...
	BEGIN
		...
		SELECT TOP 1 1 AS [type_id], 'Position Hourly View Excluding Sub Book' AS [name], 'PVHEWBT' AS ALIAS, '' AS [description],'IF OBJECT_ID(N''tempdb..#books'') IS NOT NULL
    DROP TABLE #books
...
DROP TABLE #temp_block_type_group_table
DROP TABLE #temp_hourly_position
DROP TABLE #temp_report_hourly_position_breakdown' AS [tsql], @report_id_data_source_dest AS report_id
	END 
	...
	IF EXISTS (SELECT 1 
	           ...
	           WHERE ds.[name] = 'Position Hourly View Excluding Sub Book'
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'change_report' AS [name], 'change_report' AS ALIAS, 1 AS reqd_param, 1 AS widget_id, 5 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, 1 AS append_filter, NULL  AS tooltip,0 AS column_template, 0 AS key_column				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Position Hourly View Excluding Sub Book'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'deal_term' AS [name], 'deal_term' AS ALIAS, 0 AS reqd_param, 6 AS widget_id, 2 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, 1 AS append_filter, NULL  AS tooltip,4 AS column_template, 0 AS key_column				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Position Hourly View Excluding Sub Book'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		UPDATE dsc  
		SET alias = 'source_deal_type_id'
			   , reqd_param = 0, widget_id = 2, datatype_id = 4, param_data_source = 'EXEC spa_source_deal_type_maintain @flag=''x''', param_default_value = NULL, append_filter = 1, tooltip = NULL, column_template = 2, key_column = 0
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
		WHERE ds.[name] = 'Position Hourly View Excluding Sub Book'
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'source_deal_type_id' AS [name], 'source_deal_type_id' AS ALIAS, 0 AS reqd_param, 2 AS widget_id, 4 AS datatype_id, 'EXEC spa_source_deal_type_maintain @flag=''x''' AS param_data_source, NULL AS param_default_value, 1 AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Position Hourly View Excluding Sub Book'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
