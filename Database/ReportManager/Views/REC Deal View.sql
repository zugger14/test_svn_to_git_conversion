BEGIN TRY
	...
	declare @new_ds_alias varchar(10) = 'RECDV'
	/** IF DATA SOURCE ALIAS ALREADY EXISTS ON DESTINATION, RAISE ERROR **/
	if exists(select top 1 1 from data_source where alias = 'RECDV' and name <> 'REC Deal View')
	begin
		select top 1 @new_ds_alias = 'RECDV' + cast(s.n as varchar(5))
		from seq s
		left join data_source ds on ds.alias = 'RECDV' + cast(s.n as varchar(5))
	...
	IF NOT EXISTS (SELECT 1 
	           FROM data_source 
	           WHERE [name] = 'REC Deal View'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
	AND NOT EXISTS (SELECT 1 FROM map_function_category WHERE [function_name] = 'REC Deal View' AND '106500' = '106501') 
	BEGIN
		INSERT INTO data_source([type_id], [name], [alias], [description], [tsql], report_id, system_defined,category)
		SELECT TOP 1 1 AS [type_id], 'REC Deal View' AS [name], @new_ds_alias AS ALIAS, 'Standard REC Deal View' AS [description],null AS [tsql], @report_id_data_source_dest AS report_id,'1' AS [system_defined]
			,'106500' AS [category]
	END

	UPDATE data_source
	SET alias = @new_ds_alias, description = 'Standard REC Deal View'
...
Create table #temp_gis_certificate_data(
	...
	unit_id varchar(1000), 
	Generation_state int, 
	technology int, 
	contract_expiration_date date, 
	gis_certificate_number_from varchar(1000), 
	gis_certificate_number_to varchar(1000))
...
SET @_sql = ''
	INSERT INTO #temp_mdv_source_deal_detail (source_deal_header_id, source_deal_detail_id, is_volume_left, match_status,gen_state_value_id,technology, transaction_classification,vintage_id)
...
SELECT books.stra_id stra_id,
	   ...
	   CAST(sdh.entire_term_start AS DATE) entire_term_start,
	   CAST (sdh.entire_term_end AS DATE) entire_term_end,
	   ...
	   CAST(sdd.contract_expiration_date AS DATE) [contract_expiration_date],
	   sdd.price_adder2,
	   ...
	   cast(sdd.settlement_date as DATE) settlement_date,
	   ...
	   sdd.detail_pricing,
	   cast(sdd.pricing_start as DATE) pricing_start,
	   cast(sdd.pricing_end as date) pricing_end,
	   ...
	   CAST(@_as_of_date AS DATE) as_of_date,
	   CAST (@_deal_date_from AS DATE) [deal_date_from],
	   CAST (@_deal_date_to AS DATE) [deal_date_to],
	   ...
	   CAST(sdd.pnl_date AS DATE) pnl_date,
	   ...
	   ISNULL(sdv_vi.code, Year(sdd.term_start)) AS vintage_name,
	   NULL AS [Certificate Expiration Date],	
	   NULL [Certificate Serial Numbers]
--[__batch_report__]
FROM source_deal_header sdh
...
LEFT JOIN static_data_value sdv_vi 
	ON sdv_vi.value_id = temp_sdd.vintage_id
		AND sdv_vi.type_id = 10092
...
LEFT JOIN source_deal_groups sdg ON sdg.source_deal_groups_id = sdd.source_deal_group_id 
	AND sdg.source_deal_header_id = sdh.source_deal_header_id 
...
LEFT JOIN source_deal_type sdty ON sdty.source_deal_type_id = sdh.deal_sub_type_type_id 
	AND ISNULL(sdty.sub_type, ''n'') = ''y''
...
LEFT JOIN dbo.static_data_value sdv_gc ON sdv_gc.value_id = sdd_sc.commodity_group1 
	AND sdv_gc.type_id = 29100
...
LEFT JOIN static_data_value sdv_att1 ON sdv_att1.value_id = caf1.commodity_attribute_value 
	AND sdv_att1.type_id = 43200
...
LEFT JOIN static_data_value sdv_att2 ON sdv_att2.value_id = caf2.commodity_attribute_value 
	AND sdv_att2.type_id = 43200
...
LEFT JOIN static_data_value sdv_att3 ON sdv_att3.value_id = caf3.commodity_attribute_value 
	AND sdv_att3.type_id = 43200
...
LEFT JOIN static_data_value sdv_att4 ON sdv_att4.value_id = caf4.commodity_attribute_value 
	AND sdv_att4.type_id = 43200
...
LEFT JOIN static_data_value sdv_att5 ON sdv_att5.value_id = caf5.commodity_attribute_value 
	AND sdv_att5.type_id = 43200
LEFT JOIN static_data_value sdv_it ON sdv_it.value_id = sdh.inco_terms 
	AND sdv_it.[type_id] = 40200
LEFT JOIN static_data_value sdv_it2 ON sdv_it2.value_id = sdd.detail_inco_terms 
	AND sdv_it2.[type_id] = 40200
...
LEFT JOIN counterparty_contacts cc_scheduler ON cc_scheduler.counterparty_contact_id = sdh.scheduler 
	AND cc_scheduler.contact_type = -32300
LEFT JOIN static_data_value sdv_cy ON sdv_cy.value_id = sdd.crop_year 
	AND sdv_cy.[type_id] = 10092
LEFT JOIN static_data_value sdv_bso ON sdv_bso.value_id = sdd.buyer_seller_option 
	AND sdv_bso.[type_id] = 40400
...
LEFT JOIN static_data_value ship ON ship.value_id = mgs.shipment_status 
	AND ship.type_id = 47000	
LEFT JOIN match_group mg ON mg.match_group_id = mgs.match_group_id
LEFT JOIN static_data_value sdv_dds ON sdv_dds.value_id = sdd.status 
	AND sdv_dds.type_id = 25000
LEFT JOIN static_data_value sdv_deal_cat ON sdv_deal_cat.value_id = sdh.deal_category_value_id 
	AND sdv_deal_cat.[type_id] = 475
LEFT JOIN static_data_value sdv_block_type ON sdv_block_type.value_id = sdh.block_type 
	AND sdv_block_type.[type_id] = 12000
LEFT JOIN static_data_value sdv_deal_ref ON sdv_deal_ref.value_id = sdh.deal_reference_type_id 
	AND sdv_deal_ref.[type_id] = 12500
LEFT JOIN counterparty_contacts cc_trader ON cc_trader.counterparty_contact_id = sdh.counterparty_trader 
	AND cc_trader.contact_type = -32200
...
LEFT JOIN static_data_value sdv_detail_cat ON sdv_detail_cat.value_id = sdd.category 
	AND sdv_detail_cat.[type_id] = 18100
LEFT JOIN static_data_value sdv_cycle ON sdv_cycle.value_id = sdd.cycle 
	AND sdv_cycle.[type_id] = 41000
LEFT JOIN static_data_value sdv_entity ON sdv_entity.value_id = sc.type_of_entity 
	AND sdv_entity.[type_id] = 10020
...
LEFT JOIN static_data_value fuel ON fuel.value_id = rg.fuel_value_id 
	AND fuel.type_id = 10023
LEFT JOIN static_data_value technology ON technology.value_id = rg.technology 
	AND technology.type_id = 10009
LEFT JOIN static_data_value gen_state ON gen_state.value_id = rg.gen_state_value_id 
	AND gen_state.type_id = 10016
LEFT JOIN static_data_value tech_sub ON tech_sub.value_id = rg.classification_value_id 
	AND tech_sub.type_id = 10009
...
LEFT JOIN dbo.SplitCommaSeperatedValues(@_transaction_classification) tc ON tc.item = temp_sdd.transaction_classification
LEFT JOIN #temp_gis_certificate_data gisc on gisc.source_deal_detail_id = sdd.source_deal_detail_id 
LEFT JOIN dbo.SplitCommaSeperatedValues(@_generation_state) gse ON gse.item = COALESCE(temp_sdd.gen_state_value_id,gisc.Generation_state, ''-1'')
LEFT JOIN dbo.SplitCommaSeperatedValues(@_technology) tech ON tech.item = COALESCE(temp_sdd.technology,gisc.technology, ''-1'')
LEFT JOIN static_data_value sdv_certi_genstate ON sdv_certi_genstate.value_id = gisc.Generation_state 
	AND sdv_certi_genstate.type_id = 10016
LEFT JOIN static_data_value sdv_tech_genstate ON sdv_tech_genstate.value_id = gisc.technology 
	AND sdv_tech_genstate.type_id = 10009
WHERE ((sdh.deal_date <= CAST(@_as_of_date AS DATETIME) AND sdd.term_end >= CAST(@_as_of_date AS DATETIME)) OR @_as_of_date IS NULL)
	...
	AND IIF(@_transaction_classification IS NULL, ''-1'', ISNULL(temp_sdd.transaction_classification, ''-1'')) = IIF(@_transaction_classification IS NULL, ''-1'', tc.item)	
	AND IIF(@_generation_state IS NULL, ''-1'', COALESCE(ISNULL(temp_sdd.gen_state_value_id, gisc.Generation_state),''-1'')) = IIF(@_generation_state IS NULL, ''-1'', gse.item)
	AND IIF(@_technology IS NULL, ''-1'', COALESCE(ISNULL(temp_sdd.technology, gisc.technology),''-1'')) = IIF(@_technology IS NULL, ''-1'', tech.item)
', report_id = @report_id_data_source_dest,
	...
	BEGIN
		UPDATE dsc  
		SET alias = 'Fuel Type'
			   , reqd_param = NULL, widget_id = 9, datatype_id = 5, param_data_source = 'SELECT value_id,' + CHAR(10) + '' + CHAR(9) + '   code' + CHAR(10) + 'FROM static_data_value ' + CHAR(10) + 'WHERE type_id = 10023', param_default_value = NULL, append_filter = NULL, tooltip = NULL, column_template = 2, key_column = 0, required_filter = 0
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
		WHERE ds.[name] = 'REC Deal View'
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'physical_financial_flag' AS [name], 'Detail Physical/Financial' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 5 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,0 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'REC Deal View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'Certificate Expiration Date' AS [name], 'Certificate Expiration Date' AS ALIAS, NULL AS reqd_param, 6 AS widget_id, 4 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,4 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'REC Deal View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'Certificate Serial Numbers' AS [name], 'Certificate Serial Numbers' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 4 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'REC Deal View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
