BEGIN TRY
	...
	declare @new_ds_alias varchar(10) = 'SDSV'
	/** IF DATA SOURCE ALIAS ALREADY EXISTS ON DESTINATION, RAISE ERROR **/
	if exists(select top 1 1 from data_source where alias = 'SDSV' and name <> 'Deal Settlement View')
	begin
		select top 1 @new_ds_alias = 'SDSV' + cast(s.n as varchar(5))
		from seq s
		left join data_source ds on ds.alias = 'SDSV' + cast(s.n as varchar(5))
	...
	IF NOT EXISTS (SELECT 1 
	           FROM data_source 
	           WHERE [name] = 'Deal Settlement View'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
	AND NOT EXISTS (SELECT 1 FROM map_function_category WHERE [function_name] = 'Deal Settlement View' AND '106500' = '106501') 
	BEGIN
		INSERT INTO data_source([type_id], [name], [alias], [description], [tsql], report_id, system_defined,category)
		SELECT TOP 1 1 AS [type_id], 'Deal Settlement View' AS [name], @new_ds_alias AS ALIAS, 'Standard Deal Settlement View' AS [description],null AS [tsql], @report_id_data_source_dest AS report_id,'1' AS [system_defined]
			,'106500' AS [category]
	END

	UPDATE data_source
	SET alias = @new_ds_alias, description = 'Standard Deal Settlement View'
...
DECLARE @_from_as_of_date VARCHAR(50) --= ''1999-1-1''


DECLARE @_to_as_of_date VARCHAR(50) --= ''2050-12-31''
...
DECLARE @_deal_status_group CHAR(1) --= ''a''
...
CREATE TABLE #final_values(
	...
	parent_counterparty_name varchar(500) COLLATE DATABASE_DEFAULT ,
	...
	deal_type_name nvarchar(500) COLLATE DATABASE_DEFAULT NULL,


	deal_sub_type_type_id int ,


	deal_sub_type_name nvarchar(500) COLLATE DATABASE_DEFAULT NULL,
	...
	product_desc nvarchar(500) COLLATE DATABASE_DEFAULT NULL,


	buy_sell_flag nvarchar(500) COLLATE DATABASE_DEFAULT NULL,
	...
	country nvarchar(500) COLLATE DATABASE_DEFAULT NULL,


	grid nvarchar(500) COLLATE DATABASE_DEFAULT NULL,


	region_id int ,


	region nvarchar(500) COLLATE DATABASE_DEFAULT NULL,
	...
	location_description nvarchar(500) COLLATE DATABASE_DEFAULT NULL,
	...
	template_name nvarchar(500) COLLATE DATABASE_DEFAULT NULL,
	...
	counterparty_id nvarchar(500) COLLATE DATABASE_DEFAULT ,
	...
	actual_forward varchar(500) COLLATE DATABASE_DEFAULT NULL,  --Set hard coded value
	...
	internal_desk nvarchar(500) COLLATE DATABASE_DEFAULT NULL,


	internal_portfolio_id int ,


	internal_portfolio nvarchar(500) COLLATE DATABASE_DEFAULT NULL,
	...
	type_of_entity nvarchar(500) COLLATE DATABASE_DEFAULT NULL,
	...
	deal_status nvarchar(500) COLLATE DATABASE_DEFAULT NULL,


	confirm_status_id int ,


	confirm_status nvarchar(500) COLLATE DATABASE_DEFAULT NULL,


	description1 nvarchar(500) COLLATE DATABASE_DEFAULT NULL,


	description2 nvarchar(500) COLLATE DATABASE_DEFAULT NULL,


	description3 nvarchar(500) COLLATE DATABASE_DEFAULT NULL,


	description4 nvarchar(500) COLLATE DATABASE_DEFAULT NULL,


	counterparty_id2 int ,


	counterparty_name2 nvarchar(500) COLLATE DATABASE_DEFAULT ,


	ext_deal_id nvarchar(500) COLLATE DATABASE_DEFAULT NULL,


	reference nvarchar(500) COLLATE DATABASE_DEFAULT NULL,


	create_by nvarchar(500) COLLATE DATABASE_DEFAULT NULL,


	create_ts datetime ,


	update_by nvarchar(500) COLLATE DATABASE_DEFAULT NULL,


	update_ts datetime ,


	broker_fixed_cost numeric ,
	...
	book_identifier1 nvarchar(500) COLLATE DATABASE_DEFAULT NULL,


	book_identifier2 nvarchar(500) COLLATE DATABASE_DEFAULT NULL,


	book_identifier3 nvarchar(500) COLLATE DATABASE_DEFAULT NULL,


	book_identifier4 nvarchar(500) COLLATE DATABASE_DEFAULT NULL,
	...
	deal_category  nvarchar(500) COLLATE DATABASE_DEFAULT NULL,


	internal_counterparty_id int,


	internal_couterparty nvarchar(500) COLLATE DATABASE_DEFAULT NULL,


	template_id int,


	volume_profile nvarchar(500) COLLATE DATABASE_DEFAULT NULL,
	...
	is_expired  varchar(10) COLLATE DATABASE_DEFAULT NULL,
	...
	contract_exipration_date datetime,


	structured_id  nvarchar(500) COLLATE DATABASE_DEFAULT NULL,


	payment_date datetime,


	deal_status_group varchar(20) COLLATE DATABASE_DEFAULT,
...
SET @_sql_select1 =''
	...
	SELECT 
		...
		CASE WHEN sdh.physical_financial_flag = ''''f'''' THEN  sdv_index_group.code ELSE sml.location_name END [location_name], 


		sml.location_description,


		aod.as_of_date AS from_as_of_date,


		CAST(COALESCE(sds1.settlement_amount_s,0) AS FLOAT) amount,


		CAST(COALESCE(sds1.market_value_s,0) AS FLOAT) market_value,


		CAST(COALESCE(sds1.Contract_value_s,0) AS FLOAT) contract_value,
		...
		CASE 
			...
			WHEN sdd.buy_sell_flag = ''''s'''' THEN ABS(sds1.volume_s) * (-1)


		ELSE ABS(sds1.volume_s) 
		...
		CAST(sds1.float_price_s AS FLOAT) [market_price],


		sds1.float_price_s AS [formula_price],


		CAST(sds1.net_price_s AS FLOAT) [net_price],
		...
		aod.as_of_date AS to_as_of_date,
		...
		CASE WHEN sds1.settlement_amount_s IS NOT NULL THEN ''''a'''' ELSE ''''f'''' END actual_forward,


		CAST(sdd.fixed_price AS FLOAT) fixed_price,


		sdd.price_adder AS price_adder,
		...
		sds1.deal_price_s AS deal_price, 


		sds1.float_price_s float_price, 


		--sds1.volume_s AS Volume, 
...
SET @_sql_select2 = ''
		...
		CASE WHEN sdd.contract_expiration_date < '''''' + CAST(@_from_as_of_date AS VARCHAR(100)) + '''''' THEN ''''Yes'''' ELSE ''''No'''' END is_expired,
		...
		COALESCE(sds1.payment_date,dbo.FNAInvoiceDueDate(sdd.term_start, ISNULL(cca.invoice_due_date,cg.invoice_due_date), cg.holiday_calendar_id,ISNULL(cca.payment_days,cg.payment_days)),sdd.term_end) [payment_date]
		...
		sdd.actual_volume AS actual_volume,


		sdd.schedule_volume AS schedule_volume
...
SET @_sql_from1 = ''
	...
	LEFT JOIN static_data_value sdv_index_group
		...
		AND sdv_index_group.type_id = 15100
...
SET  @_sql_from2 = '' 
	...
	(
		...
		select max(sds.as_of_date) as_of_date,
		...
		LEFT JOIN holiday_group hg ON hg.hol_group_value_id = spcd.exp_calendar_id
			...
			AND hg.exp_date = sds.as_of_date
		...
		where sds.source_deal_header_id=sdd.source_deal_header_id and 


			sds.term_start=sdd.term_start and sds.leg=sdd.leg --and set_type=''''s''''


			and sds.as_of_date<=''''''+ @_from_as_of_date +''''''


		GROUP BY CASE WHEN sdt.deal_type_id = ''''Future'''' AND idtst.internal_deal_type_subtype_type = ''''Physical Future'''' AND sdh.physical_financial_flag = ''''p'''' AND spcd.granularity = 980 THEN sds.fin_volume_uom ELSE ''''1'''' END
...
outer apply
		...
		(
			...
			select volume volume_s, deal_price deal_price_s, net_price net_price_s, float_price float_price_s, market_value market_value_s, Contract_value Contract_value_s, settlement_amount settlement_amount_s,settlement_currency_id,payment_date from source_deal_settlement
	...
	OUTER APPLY(SELECT TOP 1 CASE WHEN sdd.term_start<=''''''+@_from_as_of_date+'''''' THEN CAST(YEAR(sdd.term_start) AS VARCHAR) + ''''-YTD''''


		--WHEN DATEDIFF(d,''''''+@_from_as_of_date+'''''',sdd.term_start)=1 THEN  ''''Day - 1''''


		--WHEN MONTH(''''''+@_from_as_of_date+'''''') = MONTH(sdd.term_start) AND YEAR(''''''+@_from_as_of_date+'''''') =  YEAR(sdd.term_start) THEN ''''Week - ''''+ CAST(DATEDIFF(week,''''''+@_from_as_of_date+'''''',sdd.term_start) AS VARCHAR)


		WHEN MONTH(''''''+@_from_as_of_date+'''''') = MONTH(sdd.term_start) AND YEAR(''''''+@_from_as_of_date+'''''') =  YEAR(sdd.term_start) THEN  CAST(MONTH(sdd.term_start) AS VARCHAR) +''''-MTD''''


		WHEN DATEDIFF(m,''''''+@_from_as_of_date+'''''',sdd.term_start) <=3 THEN ''''Month - '''' + CAST(DATEDIFF(m,''''''+@_from_as_of_date+'''''',sdd.term_start) AS VARCHAR) 


		WHEN YEAR(''''''+@_from_as_of_date+'''''') =  YEAR(sdd.term_start) THEN ''''Quarter - '''' + CAST(DATEPART(q,sdd.term_start) AS VARCHAR) 
...
SET @_sql_from4 ='' 
		...
		LEFT JOIN source_currency scur ON  scur.source_currency_id =sds1.settlement_currency_id
...
SET @_sql_whr = '' WHERE 1 = 1 '' + CASE 
	...
	'' AND sds1.settlement_amount_s IS NOT NULL '' +


	CASE WHEN @_internal_counterparty_id IS NULL THEN '''' ELSE '' AND sdh.internal_counterparty IN ('' + @_internal_counterparty_id + '')'' END +
	...
	CASE WHEN @_currency_id IS NULL THEN '''' ELSE '' AND sds1.settlement_currency_id IN ('' + @_currency_id + '')'' END  +
...
--PRINT(@_sql_from1+@_sql_from2+@_sql_from3+@_sql_from4+@_sql_whr)
...
SELECT 
	...
	fv.deal_date_to,trader_id,


	fv.trader_name,


	ISNULL(fv.counterparty_name, sc.counterparty_name) counterparty_name,


	fv.parent_counterparty_name,


	ISNULL(fv.contract_id, cg.contract_id) contract_id,


	ISNULL(fv.[contract_name], cg.[contract_name]) [contract_name],
	...
	fv.location_description,


	fv.from_as_of_date,


	COALESCE(ifbs.fees,ifb.fees,fv.amount,0) amount,


	0 market_value,


	COALESCE(ifbs.fees,ifb.fees,fv.amount,0) contract_value,
	...
	COALESCE(ifbs.price,ifb.price) net_price,


	fv.physical_financial_flag,


	fv.leg,


	ISNULL(fv.source_counterparty_id, sc.source_counterparty_id) source_counterparty_id,


	ISNULL(fv.counterparty_id, sc.counterparty_id) counterparty_id,
	...
	fv.term_start_month_name,


	fv.term_start_year,


	fv.term_start_year_month,
	...
	fv.internal_portfolio_id,


	fv.internal_portfolio,
	...
	fv.sub_book_group1_id,


	fv.sub_book_group2_id,


	fv.sub_book_group3_id,


	fv.sub_book_group4_id,
	...
	fv.book_identifier1_id,


	fv.book_identifier2_id,


	fv.book_identifier3_id,


	fv.book_identifier4_id,
	...
	COALESCE(ifbs.price,ifb.price) deal_price,


	0 float_price,


	--COALESCE(ifbs.volume,ifb.volume,fv.Volume) Volume, 


	fv.close_reference_id,


	COALESCE(ifbs.field_id,ifb.field_id,fv.charge_type_id) charge_type_id,


	COALESCE(ifbs.field_name,ifb.field_name, fv.charge_type) charge_type,
...
WHERE source_deal_header_id=fv.source_deal_header_id 
	...
	AND term_start = fv.term_start AND ISNULL(leg, 1) = ISNULL(fv.leg, 1)


	AND as_of_date =  fv.from_as_of_date
...
FROM index_fees_breakdown  


WHERE source_deal_header_id=fv.source_deal_header_id 


	AND field_id = ISNULL(ifbs.field_id,field_id)
...
LEFT JOIN source_counterparty AS sc ON sc.source_counterparty_id = ifb.counterparty_id


LEFT JOIN contract_group cg ON cg.contract_id = ifb.contract_id


WHERE ISNULL(ifbs.field_id,ifb.field_id) IS NOT NULL
...
WHERE 1=1', report_id = @report_id_data_source_dest,
	...
	IF EXISTS (SELECT 1 
	           ...
	           WHERE ds.[name] = 'Deal Settlement View'
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'broker_id' AS [name], 'Broker Id' AS ALIAS, 0 AS reqd_param, 1 AS widget_id, 4 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, 1 AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Deal Settlement View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'broker_name' AS [name], 'Broker Name' AS ALIAS, 0 AS reqd_param, 1 AS widget_id, 5 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, 1 AS append_filter, NULL  AS tooltip,0 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Deal Settlement View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		UPDATE dsc  
		SET alias = 'Buy Sell'
			   , reqd_param = 1, widget_id = 2, datatype_id = 5, param_data_source = 'SELECT ''b'' AS value, ''Buy'' AS label, ''enable'' AS status ' + CHAR(10) + 'UNION ALL' + CHAR(10) + 'SELECT ''s'', ''Sell'', ''enable''', param_default_value = NULL, append_filter = 0, tooltip = NULL, column_template = 0, key_column = 0, required_filter = 0
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
		WHERE ds.[name] = 'Deal Settlement View'
	...
	BEGIN
		...
		SET alias = 'Counterparty ID'
			   , reqd_param = 0, widget_id = 7, datatype_id = 5, param_data_source = 'browse_counterparty', param_default_value = NULL, append_filter = 1, tooltip = NULL, column_template = 2, key_column = 1, required_filter = NULL
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
		WHERE ds.[name] = 'Deal Settlement View'
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'deal_sub_type_name' AS [name], 'Deal Sub Type Name' AS ALIAS, 0 AS reqd_param, 1 AS widget_id, 5 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, 1 AS append_filter, NULL  AS tooltip,0 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Deal Settlement View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'deal_type_name' AS [name], 'Deal Type Name' AS ALIAS, 0 AS reqd_param, 1 AS widget_id, 5 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, 1 AS append_filter, NULL  AS tooltip,0 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Deal Settlement View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'location_name' AS [name], 'Location Name' AS ALIAS, 0 AS reqd_param, 1 AS widget_id, 5 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, 1 AS append_filter, NULL  AS tooltip,0 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Deal Settlement View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		SET alias = 'Template Id'
			   , reqd_param = 0, widget_id = 9, datatype_id = 4, param_data_source = 'EXEC spa_stmt_account_code_mapping @flag =''template_combo''', param_default_value = NULL, append_filter = 1, tooltip = NULL, column_template = 2, key_column = 0, required_filter = 0
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
		WHERE ds.[name] = 'Deal Settlement View'
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'template_name' AS [name], 'Template Name' AS ALIAS, 0 AS reqd_param, 1 AS widget_id, 5 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, 1 AS append_filter, NULL  AS tooltip,0 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Deal Settlement View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'actual_forward' AS [name], 'Actual Forward' AS ALIAS, 0 AS reqd_param, 1 AS widget_id, 5 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, 1 AS append_filter, NULL  AS tooltip,0 AS column_template, 1 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Deal Settlement View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'agg_term' AS [name], 'Agg Term' AS ALIAS, 0 AS reqd_param, 1 AS widget_id, 5 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, 1 AS append_filter, NULL  AS tooltip,0 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Deal Settlement View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'create_by' AS [name], 'Create By' AS ALIAS, 0 AS reqd_param, 1 AS widget_id, 5 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, 1 AS append_filter, NULL  AS tooltip,0 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Deal Settlement View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		SET alias = 'Currency Id'
			   , reqd_param = 0, widget_id = 2, datatype_id = 4, param_data_source = 'EXEC spa_source_currency_maintain ''p''', param_default_value = NULL, append_filter = 1, tooltip = NULL, column_template = 2, key_column = 0, required_filter = 0
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
		WHERE ds.[name] = 'Deal Settlement View'
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'currency_id' AS [name], 'Currency Id' AS ALIAS, 0 AS reqd_param, 2 AS widget_id, 4 AS datatype_id, 'EXEC spa_source_currency_maintain ''p''' AS param_data_source, NULL AS param_default_value, 1 AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, 0 AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Deal Settlement View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		SET alias = 'Deal Category Id'
			   , reqd_param = 0, widget_id = 9, datatype_id = 4, param_data_source = 'EXEC spa_StaticDataValues ''h'', 475', param_default_value = NULL, append_filter = 1, tooltip = NULL, column_template = 2, key_column = 0, required_filter = 0
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
		WHERE ds.[name] = 'Deal Settlement View'
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'deal_ref_id' AS [name], 'Deal Ref Id' AS ALIAS, 1 AS reqd_param, 1 AS widget_id, 5 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, 0 AS append_filter, NULL  AS tooltip,0 AS column_template, 0 AS key_column, 0 AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Deal Settlement View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		SET alias = 'Deal Sub Type Type Id'
			   , reqd_param = 1, widget_id = 9, datatype_id = 4, param_data_source = 'EXEC spa_StaticDataValues @flag = ''h'', @type_id = 1175', param_default_value = NULL, append_filter = 0, tooltip = NULL, column_template = 2, key_column = 0, required_filter = 0
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
		WHERE ds.[name] = 'Deal Settlement View'
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'deal_sub_type_type_id' AS [name], 'Deal Sub Type Type Id' AS ALIAS, 1 AS reqd_param, 9 AS widget_id, 4 AS datatype_id, 'EXEC spa_StaticDataValues @flag = ''h'', @type_id = 1175' AS param_data_source, NULL AS param_default_value, 0 AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, 0 AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Deal Settlement View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'entire_term_end' AS [name], 'Entire Term End' AS ALIAS, 0 AS reqd_param, 6 AS widget_id, 2 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, 1 AS append_filter, NULL  AS tooltip,4 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Deal Settlement View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'entire_term_start' AS [name], 'Entire Term Start' AS ALIAS, 0 AS reqd_param, 6 AS widget_id, 2 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, 1 AS append_filter, NULL  AS tooltip,4 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Deal Settlement View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'formula_curve_id' AS [name], 'Formula Curve Id' AS ALIAS, 0 AS reqd_param, 1 AS widget_id, 4 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, 1 AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Deal Settlement View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'formula_curve_name' AS [name], 'Formula Curve Name' AS ALIAS, 0 AS reqd_param, 1 AS widget_id, 5 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, 1 AS append_filter, NULL  AS tooltip,0 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Deal Settlement View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'from_as_of_date' AS [name], 'From As Of Date' AS ALIAS, 1 AS reqd_param, 6 AS widget_id, 2 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, 0 AS append_filter, NULL  AS tooltip,4 AS column_template, 1 AS key_column, 1 AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Deal Settlement View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		SET alias = 'Internal Counterparty Id'
			   , reqd_param = 0, widget_id = 9, datatype_id = 4, param_data_source = 'EXEC spa_source_counterparty_maintain @flag = ''f'', @int_ext_flag = ''i''', param_default_value = NULL, append_filter = 1, tooltip = NULL, column_template = 2, key_column = 1, required_filter = 0
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
		WHERE ds.[name] = 'Deal Settlement View'
	...
	BEGIN
		...
		SET alias = 'Product Id'
			   , reqd_param = 0, widget_id = 9, datatype_id = 4, param_data_source = 'EXEC spa_StaticDataValues @flag = ''h'', @type_id = 39800', param_default_value = NULL, append_filter = 1, tooltip = NULL, column_template = 2, key_column = 0, required_filter = 0
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
		WHERE ds.[name] = 'Deal Settlement View'
	...
	BEGIN
		UPDATE dsc  
		SET alias = 'Region Id'
			   , reqd_param = 0, widget_id = 9, datatype_id = 4, param_data_source = 'EXEC spa_StaticDataValues @flag = ''h'', @type_id = 11150', param_default_value = NULL, append_filter = 1, tooltip = NULL, column_template = 2, key_column = 0, required_filter = 0
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
		WHERE ds.[name] = 'Deal Settlement View'
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'source_deal_type_id' AS [name], 'Source Deal Type ID' AS ALIAS, 1 AS reqd_param, 9 AS widget_id, 4 AS datatype_id, 'exec spa_source_deal_type_maintain ''x''' AS param_data_source, NULL AS param_default_value, 0 AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, 0 AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Deal Settlement View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'source_major_location_ID' AS [name], 'Source Major Location Id' AS ALIAS, 0 AS reqd_param, 1 AS widget_id, 4 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, 1 AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Deal Settlement View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'structured_id' AS [name], 'Structured Id' AS ALIAS, 0 AS reqd_param, 1 AS widget_id, 5 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, 1 AS append_filter, NULL  AS tooltip,0 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Deal Settlement View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'sub_book_group1' AS [name], 'Sub Book Group1' AS ALIAS, 0 AS reqd_param, 1 AS widget_id, 5 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, 1 AS append_filter, NULL  AS tooltip,0 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Deal Settlement View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'sub_book_group1_id' AS [name], 'Sub Book Group1 Id' AS ALIAS, 0 AS reqd_param, 1 AS widget_id, 4 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, 1 AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Deal Settlement View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'sub_book_group2' AS [name], 'Sub Book Group2' AS ALIAS, 0 AS reqd_param, 1 AS widget_id, 5 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, 1 AS append_filter, NULL  AS tooltip,0 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Deal Settlement View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'sub_book_group2_id' AS [name], 'Sub Book Group2 Id' AS ALIAS, 0 AS reqd_param, 1 AS widget_id, 4 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, 1 AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Deal Settlement View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'sub_book_group3' AS [name], 'Sub Book Group3' AS ALIAS, 0 AS reqd_param, 1 AS widget_id, 5 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, 1 AS append_filter, NULL  AS tooltip,0 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Deal Settlement View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'sub_book_group3_id' AS [name], 'Sub Book Group3 Id' AS ALIAS, 0 AS reqd_param, 1 AS widget_id, 4 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, 1 AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Deal Settlement View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'sub_book_group4' AS [name], 'Sub Book Group4' AS ALIAS, 0 AS reqd_param, 1 AS widget_id, 5 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, 1 AS append_filter, NULL  AS tooltip,0 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Deal Settlement View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'sub_book_group4_id' AS [name], 'Sub Book Group4 Id' AS ALIAS, 0 AS reqd_param, 1 AS widget_id, 4 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, 1 AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Deal Settlement View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'update_by' AS [name], 'Update By' AS ALIAS, 0 AS reqd_param, 1 AS widget_id, 5 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, 1 AS append_filter, NULL  AS tooltip,0 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Deal Settlement View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
