BEGIN TRY
	...
	declare @new_ds_alias varchar(10) = 'SMCV'
	/** IF DATA SOURCE ALIAS ALREADY EXISTS ON DESTINATION, RAISE ERROR **/
	if exists(select top 1 1 from data_source where alias = 'SMCV' and name <> 'MTM by Charge View')
	begin
		select top 1 @new_ds_alias = 'SMCV' + cast(s.n as varchar(5))
		from seq s
		left join data_source ds on ds.alias = 'SMCV' + cast(s.n as varchar(5))
	...
	IF NOT EXISTS (SELECT 1 
	           FROM data_source 
	           WHERE [name] = 'MTM by Charge View'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
	AND NOT EXISTS (SELECT 1 FROM map_function_category WHERE [function_name] = 'MTM by Charge View' AND '106500' = '106501') 
	BEGIN
		INSERT INTO data_source([type_id], [name], [alias], [description], [tsql], report_id, system_defined,category)
		SELECT TOP 1 1 AS [type_id], 'MTM by Charge View' AS [name], @new_ds_alias AS ALIAS, 'Standard MTM by Charge View' AS [description],null AS [tsql], @report_id_data_source_dest AS report_id,'1' AS [system_defined]
			,'106500' AS [category]
	END

	UPDATE data_source
	SET alias = @new_ds_alias, description = 'Standard MTM by Charge View'
...
--SET @_from_as_of_date = ''2016-09-30''
--SET @_to_as_of_date =  ''2016-09-30''
--SET @_source_deal_header_id=47198
	...
	CREATE TABLE #final_values(
		...
		pnl_source_name nvarchar(500) COLLATE DATABASE_DEFAULT NULL,
		...
		sub_book_group1 nvarchar(500) COLLATE DATABASE_DEFAULT NULL,
		sub_book_group2 nvarchar(500) COLLATE DATABASE_DEFAULT NULL,
		sub_book_group3 nvarchar(500) COLLATE DATABASE_DEFAULT NULL,
		sub_book_group4 nvarchar(500) COLLATE DATABASE_DEFAULT NULL,
		...
		internal_counterparty nvarchar(500) COLLATE DATABASE_DEFAULT NULL,
		template_id int,
		...
		structured_deal_id  nvarchar(500) COLLATE DATABASE_DEFAULT NULL,
		payment_date datetime,
		block_definition_id INT,
		block_definition_name nvarchar(100) COLLATE DATABASE_DEFAULT NULL,
		source_deal_groups_name  nvarchar(500) COLLATE DATABASE_DEFAULT NULL,
		reporting_group1 INT, 
	    reporting_group2 INT,
	    reporting_group3 INT,
	    reporting_group4 INT,
		reporting_group5 INT,
		[reporting_group1_name]  nvarchar(500) COLLATE DATABASE_DEFAULT NULL,
		[reporting_group2_name]  nvarchar(500) COLLATE DATABASE_DEFAULT NULL,
		[reporting_group3_name]  nvarchar(500) COLLATE DATABASE_DEFAULT NULL,
		[reporting_group4_name]  nvarchar(500) COLLATE DATABASE_DEFAULT NULL,
		[reporting_group5_name]  nvarchar(500) COLLATE DATABASE_DEFAULT NULL
	)
	SET @_sql_select1 =''
		...
		SELECT 
			...
			fe.formula [formula],               
			(CASE WHEN sdd.buy_sell_flag = ''''s'''' THEN sdpd.deal_volume * (-1) ELSE sdpd.deal_volume END) [total_volume],
	...
	SET @_sql_from1 = '' 
		...
		LEFT JOIN source_deal_pnl_detail sdpd ON sdpd.source_deal_header_id = sdh.source_deal_header_id
				...
				+ 
				CASE 
					WHEN @_to_as_of_date IS NULL THEN ''sdpd.pnl_as_of_date = '''''' + @_from_as_of_date + '''''''' 
					ELSE ''sdpd.pnl_as_of_date  BETWEEN '''''' + @_from_as_of_date + '''''' AND '''''' + @_to_as_of_date + ''''''''
				END
				+					
				CASE 
...
SET @_sql_from4 = '' 
	...
	--PRINT(@_sql_select1+@_sql_select2+@_sql_from1+@_sql_from3+@_sql_from4+@_sql_whr)
	EXEC(@_sql_select1+@_sql_select2+@_sql_from1+@_sql_from3+@_sql_from4+@_sql_whr)
--select market_value, * from #final_values where term_start=''@from_as_of_date''
--drop table ##final_values
SET @_sql = ''
				INSERT INTO #final_values
				SELECT fv.sub_id,
					...
					fv.from_as_of_date,
					COALESCE(ifb.fees,fv.MTM,0) MTM,
					0 und_intrinsic_pnl,
					0 und_extrinsic_pnl,
					COALESCE(ifb.fees,fv.dis_pnl,0) dis_pnl,
					0 dis_intrinsic_pnl,
					0 dis_extrinisic_pnl,
					0 market_value,
					COALESCE(ifb.fees,fv.contract_value,0) contract_value,
					0 dis_market_value,
					0 dis_contract_value,
					0 discount_factor,
					0 discount_amount,
					fv.commodity,
					fv.deal_volume,
					...
					fv.internal_deal_type,
					fv.internal_deal_sub_type_id,
					fv.internal_deal_sub_type,
					0 fixed_cost,
					0 fixed_price,
					0 formula_value,
					1 price_adder,
					...
					fv.entire_term_end,
					fv.contract_expiration_date,
					fv.structured_deal_id,
					fv.payment_date,
					fv.block_definition_id,
					fv.block_definition_name,
					fv.source_deal_groups_name,
					fv.reporting_group1,
					fv.reporting_group2,
					fv.reporting_group3,
					fv.reporting_group4,
			...
			WHERE 1=1 AND ifb.field_id IS NOT NULL
			''			
		    +
			CASE WHEN @_charge_type_id IS NULL THEN '''' ELSE '' AND COALESCE(ifb.field_id,fv.charge_type_id) IN ('' + @_charge_type_id + '')'' END 
EXEC(@_sql)
SET @_sql = ''
				SELECT *, 
						'''''' + ISNULL(@_deal_status_group, ''NULL'') + '''''' [deal_status_group]
				--[__batch_report__]	
				FROM #final_values
				WHERE 1=1
			''
			+
			CASE WHEN @_charge_type_id IS NULL THEN '''' ELSE '' AND charge_type_id IN ('' + @_charge_type_id + '')'' END 
EXEC(@_sql)', report_id = @report_id_data_source_dest,
	...
	IF EXISTS (SELECT 1 
	           ...
	           WHERE ds.[name] = 'MTM by Charge View'
	...
	BEGIN
		...
		SET alias = 'Charge Type ID'
			   , reqd_param = 0, widget_id = 9, datatype_id = 4, param_data_source = 'EXEC spa_StaticDataValues @flag = ''h'', @type_id = 5500', param_default_value = NULL, append_filter = 1, tooltip = NULL, column_template = 2, key_column = 0, required_filter = 0
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
		WHERE ds.[name] = 'MTM by Charge View'
	...
	BEGIN
		...
		SET alias = 'Commodity ID'
			   , reqd_param = 1, widget_id = 9, datatype_id = 4, param_data_source = 'exec spa_source_commodity_maintain ''a''', param_default_value = NULL, append_filter = 0, tooltip = NULL, column_template = 2, key_column = 0, required_filter = 0
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
		WHERE ds.[name] = 'MTM by Charge View'
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'counterparty_id' AS [name], 'Counterparty ID' AS ALIAS, 0 AS reqd_param, 1 AS widget_id, 5 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, 1 AS append_filter, NULL  AS tooltip,0 AS column_template, 1 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'MTM by Charge View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'counterparty_name2' AS [name], 'Counterparty2' AS ALIAS, 0 AS reqd_param, 1 AS widget_id, 5 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, 1 AS append_filter, NULL  AS tooltip,0 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'MTM by Charge View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		SET alias = 'Currency ID'
			   , reqd_param = 0, widget_id = 9, datatype_id = 4, param_data_source = 'EXEC spa_getcurrencyunit ''s''', param_default_value = NULL, append_filter = 1, tooltip = NULL, column_template = 2, key_column = 0, required_filter = 0
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
		WHERE ds.[name] = 'MTM by Charge View'
	...
	BEGIN
		UPDATE dsc  
		SET alias = 'Discounted Contract Value'
			   , reqd_param = 0, widget_id = 1, datatype_id = 3, param_data_source = NULL, param_default_value = NULL, append_filter = 1, tooltip = NULL, column_template = 2, key_column = 0, required_filter = NULL
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
	...
	BEGIN
		UPDATE dsc  
		SET alias = 'Discounted Fair Value'
			   , reqd_param = 0, widget_id = 1, datatype_id = 3, param_data_source = NULL, param_default_value = NULL, append_filter = 1, tooltip = NULL, column_template = 2, key_column = 0, required_filter = NULL
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'dis_fair_value' AS [name], 'Discounted Fair Value' AS ALIAS, 0 AS reqd_param, 1 AS widget_id, 3 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, 1 AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'MTM by Charge View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'discount_amount' AS [name], 'Discounted Amount' AS ALIAS, 0 AS reqd_param, 1 AS widget_id, 3 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, 1 AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'MTM by Charge View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'MTM' AS [name], 'MTM' AS ALIAS, 0 AS reqd_param, 1 AS widget_id, 3 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, 1 AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'MTM by Charge View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'pnl_source_name' AS [name], 'Pnl Source Name' AS ALIAS, 0 AS reqd_param, 1 AS widget_id, 5 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, 1 AS append_filter, NULL  AS tooltip,0 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'MTM by Charge View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'sub_book_group1_id' AS [name], 'Report Group1 ID' AS ALIAS, 0 AS reqd_param, 1 AS widget_id, 4 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, 1 AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'MTM by Charge View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'sub_book_group2_id' AS [name], 'Report Group2 ID' AS ALIAS, 0 AS reqd_param, 1 AS widget_id, 4 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, 1 AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'MTM by Charge View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'sub_book_group3_id' AS [name], 'Report Group3 ID' AS ALIAS, 0 AS reqd_param, 1 AS widget_id, 4 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, 1 AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'MTM by Charge View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'sub_book_group4_id' AS [name], 'Report Group4 ID' AS ALIAS, 0 AS reqd_param, 1 AS widget_id, 4 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, 1 AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'MTM by Charge View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'block_definition_name' AS [name], 'Block Definition' AS ALIAS, 0 AS reqd_param, 1 AS widget_id, 5 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, 1 AS append_filter, NULL  AS tooltip,0 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'MTM by Charge View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'currency_name' AS [name], 'Currency' AS ALIAS, 0 AS reqd_param, 1 AS widget_id, 5 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, 1 AS append_filter, NULL  AS tooltip,0 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'MTM by Charge View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	IF EXISTS (SELECT 1 
	           ...
	           INNER JOIN data_source ds on ds.data_source_id = dsc.source_id 
	           WHERE ds.[name] = 'MTM by Charge View'
	            AND dsc.name =  'source_deal_groups_name'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
	BEGIN
		UPDATE dsc  
		SET alias = 'Source Deal Groups Name'
	...
	BEGIN
		...
		SET alias = 'Deal Status Group'
			   , reqd_param = NULL, widget_id = 2, datatype_id = 5, param_data_source = 'SELECT ''a'', ''All'' UNION' + CHAR(10) + 'SELECT ''o'', ''Official'' UNION' + CHAR(10) + 'SELECT ''u'', ''Unofficial''', param_default_value = NULL, append_filter = NULL, tooltip = NULL, column_template = 2, key_column = 0, required_filter = 1
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
		WHERE ds.[name] = 'MTM by Charge View'
