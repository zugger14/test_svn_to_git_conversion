...
SET @_sql = N''
	...
	, suom.uom_name [uom_name]
	, scu.source_currency_id [currency_id]
	, scu.currency_id [currency_code]
    , scu.currency_name
	...
	, sdv_ct.code no_roc
	, CASE WHEN scom.commodity_id = ''''Power'''' THEN ''''Strom'''' WHEN scom.commodity_id = ''''GAS'''' THEN ''''Gas'''' ELSE '''''''' END product
	, CAST(null AS VARCHAR(20)) netting
	, CAST(null AS VARCHAR(20)) document
	, scom.source_commodity_id commodity_id
	, scom.commodity_id commodity_code
...
SET @_sql1 = N''
   OUTER APPLY (
	...
	WHERE 1 = 1 
 ''
+ CASE WHEN @_counterparty_id IS NOT NULL THEN '' AND  si.counterparty_id IN (''+@_counterparty_id+'') '' ELSE '''' END
+ CASE WHEN @_contract_id IS NOT NULL THEN '' AND  si.contract_id IN (''+@_contract_id+'') '' ELSE '''' END
