BEGIN TRY
		...
		IF EXISTS (SELECT 1 FROM dbo.report WHERE report_hash='55234A21_A9B8_4E42_BB28_4CDCAE71B161')
		BEGIN
			declare @report_id_to_delete int
			select @report_id_to_delete = report_id from report where report_hash = '55234A21_A9B8_4E42_BB28_4CDCAE71B161'
		...
		SELECT TOP 1 'EOD - Copy Missing Price' [name], 'dev_admin' [owner], 0 is_system, 0 is_excel, 0 is_mobile, '55234A21_A9B8_4E42_BB28_4CDCAE71B161' report_hash, 'EOD - Copy Missing Price' [description], CAST(sdv_cat.value_id AS VARCHAR(10)) category_id
		FROM sys.objects o
...
BEGIN TRY
	...
	declare @new_ds_alias varchar(10) = 'cmp1'
	/** IF DATA SOURCE ALIAS ALREADY EXISTS ON DESTINATION, RAISE ERROR **/
	if exists(select top 1 1 from data_source where alias = 'cmp1' and name <> 'EOD - Copy Missing Price')
	begin
		select top 1 @new_ds_alias = 'cmp1' + cast(s.n as varchar(5))
		from seq s
		left join data_source ds on ds.alias = 'cmp1' + cast(s.n as varchar(5))
	...
	IF NOT EXISTS (SELECT 1 
	           FROM data_source 
	           WHERE [name] = 'EOD - Copy Missing Price'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
	AND NOT EXISTS (SELECT 1 FROM map_function_category WHERE [function_name] = 'EOD - Copy Missing Price' AND '106500' = '106501') 
	BEGIN
		INSERT INTO data_source([type_id], [name], [alias], [description], [tsql], report_id, system_defined,category)
		SELECT TOP 1 2 AS [type_id], 'EOD - Copy Missing Price' AS [name], @new_ds_alias AS ALIAS, NULL AS [description],null AS [tsql], @report_id_data_source_dest AS report_id,NULL AS [system_defined]
			,'106500' AS [category]
	END
...
', report_id = @report_id_data_source_dest,
	...
	FROM data_source_column dsc 
	INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
		AND ds.[name] = 'EOD - Copy Missing Price'
		AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1)
...
COMMIT TRAN
	...
	IF OBJECT_ID('tempdb..#data_source_column', 'U') IS NOT NULL
		...
		SELECT TOP 1 ds.data_source_id AS source_id, @report_id_dest AS report_id, 'cmp1' [alias], rd_root.report_dataset_id AS root_dataset_id,0 AS is_free_from, 'NULL' AS relationship_sql
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'EOD - Copy Missing Price'
	...
	SELECT @report_id_dest AS report_id, 'EOD - Copy Missing Price' [name], '55234A21_A9B8_4E42_BB28_4CDCAE71B161' report_hash, 11.5 width,5.5 height
		...
		SELECT TOP 1 rpage.report_page_id, 'EOD- Copy Price', '8132D32C_C7FE_45CB_9DB1_AB4D0ABC27CC', 2,'','','.xlsx',',', 
		-100000,'n','n',0	
		FROM sys.objects o
		...
		SELECT TOP 1 rpage.report_page_id AS page_id, rd.report_dataset_id AS root_dataset_id, 'EOD_ Copy Price_tablix' [name], '5.066666666666666' width, '2.72' height, '0' [top], '0' [left],2 AS group_mode,1 AS border_style,0 AS page_break,1 AS type_id,1 AS cross_summary,2 AS no_header,'' export_table_name, 0 AS is_global
		FROM sys.objects o
		INNER JOIN report_page rpage 
		ON rpage.[name] = 'EOD - Copy Missing Price'
		INNER JOIN report r 
		...
		INNER JOIN report_page_tablix rpt 
			ON rpt.[name] = 'EOD_ Copy Price_tablix'
		INNER JOIN report_page rpage 
		...
		INNER JOIN report r 
			...
			AND r.[name] = 'EOD - Copy Missing Price'
		INNER JOIN report_dataset rd 
			ON rd.report_id = r.report_id AND rd.[alias] = 'cmp1' 	
		INNER JOIN data_source ds 
			ON ISNULL(NULLIF(ds.report_id, 0), r.report_id) = r.report_id	AND ds.[name] = 'EOD - Copy Missing Price' 	
		INNER JOIN data_source_column dsc 
