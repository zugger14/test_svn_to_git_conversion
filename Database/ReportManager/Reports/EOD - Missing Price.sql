BEGIN TRY
		...
		set @report_copy_name = isnull(@report_copy_name, 'Copy of ' + 'EOD - Missing Price')
		

		INSERT INTO report ([name], [owner], is_system, is_excel, is_mobile, report_hash, [description], category_id)
		SELECT TOP 1 'EOD - Missing Price' [name], 'dev_admin' [owner], 0 is_system, 0 is_excel, 0 is_mobile, '0C7562A9_A785_427D_805E_76551048169D' report_hash, 'EOD - Missing Price' [description], CAST(sdv_cat.value_id AS VARCHAR(10)) category_id
		FROM sys.objects o
...
BEGIN TRY
	...
	FROM report r
	WHERE r.[name] = 'EOD - Missing Price'
...
COMMIT TRAN
	...
	SELECT @report_id_dest AS report_id, 'EOD - Missing Price' [name], '0C7562A9_A785_427D_805E_76551048169D' report_hash, 11.5 width,5.5 height
		...
		INNER JOIN report_page rpage 
			on rpage.[name] = 'EOD - Missing Price'
		INNER JOIN report r 
		ON r.report_id = rpage.report_id
			AND r.[name] = 'EOD - Missing Price'
		...
		INNER JOIN report_page rpage 
			...
			AND rpage.[name] = 'EOD - Missing Price'
		INNER JOIN report r 
			ON r.report_id = rpage.report_id
			AND r.[name] = 'EOD - Missing Price'
