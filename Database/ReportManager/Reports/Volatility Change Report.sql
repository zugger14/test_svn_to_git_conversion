BEGIN TRY
		...
		IF EXISTS (SELECT 1 FROM dbo.report WHERE report_hash='72585C9C_5AE3_4097_A2FD_D03FE16CD275')
		BEGIN
			declare @report_id_to_delete int
			select @report_id_to_delete = report_id from report where report_hash = '72585C9C_5AE3_4097_A2FD_D03FE16CD275'
		...
		set @report_copy_name = isnull(@report_copy_name, 'Copy of ' + 'Volatility Change Report')
		

		INSERT INTO report ([name], [owner], is_system, is_excel, is_mobile, report_hash, [description], category_id)
		SELECT TOP 1 'Volatility Change Report' [name], 'farrms_admin' [owner], 1 is_system, 1 is_excel, 0 is_mobile, '72585C9C_5AE3_4097_A2FD_D03FE16CD275' report_hash, 'Standard Volatility Change Report' [description], CAST(sdv_cat.value_id AS VARCHAR(10)) category_id
		FROM sys.objects o
	...
	SELECT @report_id_dest AS report_id, 'Volatility Change Report' [name], '72585C9C_5AE3_4097_A2FD_D03FE16CD275' report_hash, 11.5 width,5.5 height
		...
		SELECT TOP 1 rpage.report_page_id, 'Volatility Change Report', 'D12C2D0C_046D_4611_90A2_86BF42215970', 3,NULL,NULL,NULL,NULL, 
		NULL,'n','n'	
		FROM sys.objects o
		INNER JOIN report_page rpage 
			on rpage.[name] = 'Volatility Change Report'
		INNER JOIN report r 
		ON r.report_id = rpage.report_id
			AND r.[name] = 'Volatility Change Report'
		...
		INNER JOIN report_paramset rp 
			ON rp.[name] = 'Volatility Change Report'
		INNER JOIN report_page rpage 
			ON rpage.report_page_id = rp.page_id
			AND rpage.[name] = 'Volatility Change Report'
		INNER JOIN report r 
			ON r.report_id = rpage.report_id
			AND r.[name] = 'Volatility Change Report'
		...
		SELECT TOP 1 rdp.report_dataset_paramset_id AS dataset_paramset_id, rd.report_dataset_id AS dataset_id , dsc.data_source_column_id AS column_id, 1 AS operator, 'y' AS initial_value, '' AS initial_value2, 1 AS optional, 1 AS hidden,1 AS logical_operator, 8 AS param_order, 0 AS param_depth, NULL AS label
		FROM sys.objects o
		INNER JOIN report_paramset rp 
			ON rp.[name] = 'Volatility Change Report'
		INNER JOIN report_page rpage 
		...
		SELECT TOP 1 rpage.report_page_id AS page_id, rd.report_dataset_id AS root_dataset_id, 'Volatility Change Report_tablix' [name], '7.76' width, '4.32' height, '0.08' [top], '0.10666666666666667' [left],2 AS group_mode,1 AS border_style,0 AS page_break,1 AS type_id,1 AS cross_summary,2 AS no_header,'' export_table_name, 0 AS is_global
		FROM sys.objects o
		INNER JOIN report_page rpage 
		ON rpage.[name] = 'Volatility Change Report'
		INNER JOIN report r 
		...
		INNER JOIN report_page_tablix rpt 
			ON rpt.[name] = 'Volatility Change Report_tablix'
		INNER JOIN report_page rpage 
