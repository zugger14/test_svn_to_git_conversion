BEGIN TRY
		...
		IF EXISTS (SELECT 1 FROM dbo.report WHERE report_hash='40CD2F6C_2F7D_4680_9AC4_1D1B68FF9183')
		BEGIN
			declare @report_id_to_delete int
			select @report_id_to_delete = report_id from report where report_hash = '40CD2F6C_2F7D_4680_9AC4_1D1B68FF9183'
		...
		set @report_copy_name = isnull(@report_copy_name, 'Copy of ' + 'Trade Violation Report')
		

		INSERT INTO report ([name], [owner], is_system, is_excel, is_mobile, report_hash, [description], category_id)
		SELECT TOP 1 'Trade Violation Report' [name], 'farrms_admin' [owner], 0 is_system, 0 is_excel, 0 is_mobile, '40CD2F6C_2F7D_4680_9AC4_1D1B68FF9183' report_hash, '' [description], CAST(sdv_cat.value_id AS VARCHAR(10)) category_id
		FROM sys.objects o
...
BEGIN TRY
	...
	declare @new_ds_alias varchar(10) = 'DVS'
	/** IF DATA SOURCE ALIAS ALREADY EXISTS ON DESTINATION, RAISE ERROR **/
	if exists(select top 1 1 from data_source where alias = 'DVS' and name <> 'Deal Violation SQL')
	begin
		select top 1 @new_ds_alias = 'DVS' + cast(s.n as varchar(5))
		from seq s
		left join data_source ds on ds.alias = 'DVS' + cast(s.n as varchar(5))
	...
	FROM report r
	WHERE r.[name] = 'Trade Violation Report'

	IF NOT EXISTS (SELECT 1 
	           FROM data_source 
	           WHERE [name] = 'Deal Violation SQL'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
	AND NOT EXISTS (SELECT 1 FROM map_function_category WHERE [function_name] = 'Deal Violation SQL' AND '106500' = '106501') 
	BEGIN
		INSERT INTO data_source([type_id], [name], [alias], [description], [tsql], report_id, system_defined,category)
		SELECT TOP 1 2 AS [type_id], 'Deal Violation SQL' AS [name], @new_ds_alias AS ALIAS, NULL AS [description],null AS [tsql], @report_id_data_source_dest AS report_id,NULL AS [system_defined]
			,'106500' AS [category]
	END

	UPDATE data_source
	SET alias = @new_ds_alias, description = NULL
	, [tsql] = CAST('' AS VARCHAR(MAX)) + 'DECLARE @_source_deal_header_id VARCHAR(8000)  --= ''225600,225601,225602''
...
SELECT 
	...
	CASE WHEN MAX(sdd.fixed_price) IS NOT NULL 
		THEN AVG(sdd.fixed_price)
	ELSE COALESCE(MAX(sdd_indxed_price.curve_value),0) + MAX(sdd.price_adder)
	END price,
	SUM(ISNULL(sdd_contract_val.contract_value, sdd_indxed_price.trade_value)) trade_value

	--[__batch_report__] 

FROM source_deal_header sdh 

	INNER JOIN dbo.SplitCommaSeperatedValues(@_source_deal_header_id) sel_deal 
	
	ON  sdh.source_Deal_header_id = sel_deal.item
	...
	OUTER APPLY(
		...
		SELECT (sdd1.total_volume * sdd1.fixed_price) contract_value FROM source_Deal_detail sdd1 WHERE sdd1.source_deal_detail_id = sdd.source_deal_detail_id

	) sdd_contract_val
OUTER APPLY(
SELECT (ISNULL(AVG(spc.curve_value), 0) + ISNULL(sdd.price_adder, 0)) * ISNULL(sdd.total_volume, 0) trade_value, AVG(spc.curve_value) curve_value
	FROM source_price_curve_def spcd
	LEFT JOIN source_Deal_detail sdd1 --where fixed_price IS NULL 
		ON sdd1.formula_curve_id = spcd.source_curve_def_id AND sdd1.formula_curve_id IS NOT NULL
	...
	IF EXISTS (SELECT 1 
	           ...
	           WHERE ds.[name] = 'Deal Violation SQL'
	...
	BEGIN
		...
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Deal Violation SQL'
	...
	IF EXISTS (SELECT 1 
	           ...
	           INNER JOIN data_source ds on ds.data_source_id = dsc.source_id 
	           WHERE ds.[name] = 'Deal Violation SQL'
	            AND dsc.name =  'trade_value'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
	BEGIN
		UPDATE dsc  
		SET alias = 'Trade Value'
			   , reqd_param = NULL, widget_id = 1, datatype_id = 3, param_data_source = NULL, param_default_value = NULL, append_filter = NULL, tooltip = NULL, column_template = 2, key_column = 0, required_filter = NULL
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'trade_value' AS [name], 'Trade Value' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 3 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Deal Violation SQL'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	FROM data_source_column dsc 
	INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
		AND ds.[name] = 'Deal Violation SQL'
		AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1)
...
COMMIT TRAN
	...
	IF OBJECT_ID('tempdb..#data_source_column', 'U') IS NOT NULL
		...
		SELECT TOP 1 ds.data_source_id AS source_id, @report_id_dest AS report_id, 'DVS' [alias], rd_root.report_dataset_id AS root_dataset_id,0 AS is_free_from, 'NULL' AS relationship_sql
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Deal Violation SQL'
	...
	SELECT @report_id_dest AS report_id, 'Trade Violation Report' [name], '40CD2F6C_2F7D_4680_9AC4_1D1B68FF9183' report_hash, 11.5 width,5.5 height
		...
		SELECT TOP 1 rpage.report_page_id, 'Trade Violation Report', '76D659C2_153A_438D_9C9F_9BC378888A7A', 1,'','','.xlsx',',', 
		-100000,'n','n'	
		FROM sys.objects o
		INNER JOIN report_page rpage 
			on rpage.[name] = 'Trade Violation Report'
		INNER JOIN report r 
		ON r.report_id = rpage.report_id
			AND r.[name] = 'Trade Violation Report'
		...
		INNER JOIN report_paramset rp 
			ON rp.[name] = 'Trade Violation Report'
		INNER JOIN report_page rpage 
			ON rpage.report_page_id = rp.page_id
			AND rpage.[name] = 'Trade Violation Report'
		INNER JOIN report r 
			ON r.report_id = rpage.report_id
			AND r.[name] = 'Trade Violation Report'
		...
		SELECT TOP 1 rpage.report_page_id AS page_id, rd.report_dataset_id AS root_dataset_id, 'Trade Violation Report_tablix' [name], '9.066666666666666' width, '3.52' height, '0' [top], '0.13333333333333333' [left],2 AS group_mode,1 AS border_style,0 AS page_break,1 AS type_id,1 AS cross_summary,2 AS no_header,'' export_table_name, 0 AS is_global
		FROM sys.objects o
		INNER JOIN report_page rpage 
		ON rpage.[name] = 'Trade Violation Report'
		INNER JOIN report r 
		...
		INNER JOIN report r 
			...
			AND r.[name] = 'Trade Violation Report'
		INNER JOIN report_dataset rd 
			ON rd.report_id = r.report_id AND rd.[alias] = 'DVS' 	
		INNER JOIN data_source ds 
			ON ISNULL(NULLIF(ds.report_id, 0), r.report_id) = r.report_id	AND ds.[name] = 'Deal Violation SQL' 	
		INNER JOIN data_source_column dsc 
		...
		INSERT INTO report_tablix_column(tablix_id, dataset_id, column_id, placement, column_order, aggregation
					...
					, font_size, font_style, text_align, text_color, background, default_sort_order
					, default_sort_direction, custom_field, render_as, column_template, negative_mark, currency, date_format, cross_summary_aggregation, mark_for_total, sql_aggregation, subtotal)
		SELECT TOP 1 rpt.report_page_tablix_id tablix_id, rd.report_dataset_id dataset_id, dsc.data_source_column_id column_id,1 placement, 5 column_order,NULL aggregation, NULL functions, 'Price' [alias], 1 sortable, -1 rounding, 0 thousand_seperation, 'Tahoma' font, '8' font_size, '0,0,0' font_style, 'Center' text_align, '#000000' text_color, '#ffffff' background, NULL default_sort_order, NULL sort_direction, 0 custom_field, 2 render_as,-1 column_template,0 negative_mark,NULL currency,NULL date_format,-1 cross_summary_aggregation,NULL mark_for_total,NULL sql_aggregation,NULL subtotal
		...
		INSERT INTO report_tablix_column(tablix_id, dataset_id, column_id, placement, column_order, aggregation
					...
					, font_size, font_style, text_align, text_color, background, default_sort_order
					, default_sort_direction, custom_field, render_as, column_template, negative_mark, currency, date_format, cross_summary_aggregation, mark_for_total, sql_aggregation, subtotal)
		SELECT TOP 1 rpt.report_page_tablix_id tablix_id, rd.report_dataset_id dataset_id, dsc.data_source_column_id column_id,1 placement, 6 column_order,NULL aggregation, NULL functions, 'Trade Value' [alias], 1 sortable, -1 rounding, 0 thousand_seperation, 'Tahoma' font, '8' font_size, '0,0,0' font_style, 'Center' text_align, '#000000' text_color, '#ffffff' background, NULL default_sort_order, NULL sort_direction, 0 custom_field, 2 render_as,-1 column_template,0 negative_mark,NULL currency,NULL date_format,-1 cross_summary_aggregation,NULL mark_for_total,NULL sql_aggregation,NULL subtotal
		...
		INNER JOIN data_source ds 
			ON ISNULL(NULLIF(ds.report_id, 0), r.report_id) = r.report_id	AND ds.[name] = 'Deal Violation SQL' 	
		INNER JOIN data_source_column dsc 
			ON dsc.source_id = ds.data_source_id AND dsc.[name] = 'trade_value' 
 ...
 INSERT INTO report_tablix_header(tablix_id, column_id, font, font_size, font_style, text_align, text_color, background, report_tablix_column_id)
	  SELECT TOP 1 
		...
		INNER JOIN report_tablix_column rtc 
			on rtc.tablix_id = rpt.report_page_tablix_id
			--AND rtc.column_id = dsc.data_source_column_id  --This did not handle custom column, got duplicate custom columns during export
			AND rtc.alias = 'Trade Value' --Added to handle custom column. Assumption: alias is unique and NOT NULL
