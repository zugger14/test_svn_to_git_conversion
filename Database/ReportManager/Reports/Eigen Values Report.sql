...
IF EXISTS (SELECT 1 FROM dbo.report WHERE name='Eigen Values Report')
		BEGIN
		EXEC spa_rfx_report 'd', NULL,39267, NULL, NULL, NULL, NULL, NULL
		END
		...
		FROM report r
		WHERE r.[name] = 'Eigen Values Report'
		...
		* */
		...
		BEGIN
			...
			SELECT TOP 1 'Eigen Values Report' [name], 'farrms_admin' [owner], 1 is_system, 1 is_excel, 'C9AC6CD9_A4F9_48B2_94E1_7B0D5C98035E' report_hash, 'Standard Eigen Values Report' [description], CAST(sdv_cat.value_id AS VARCHAR(10)) category_id
			FROM sys.objects o
...
COMMIT TRAN
	...
	IF NOT EXISTS(SELECT 1 FROM report_page rp 
	              WHERE rp.report_id = CASE WHEN 'e ' = 'p' 
											...
											ELSE @report_id_dest 
						               END 
					AND rp.name =  'Standard Eigen Values Report'  
	)
	BEGIN
		...
		SELECT CASE WHEN 'e ' = 'p' 
					Then 39267 
					ELSE @report_id_dest 
		       END  AS report_id, 'Standard Eigen Values Report' [name], 'C9AC6CD9_A4F9_48B2_94E1_7B0D5C98035E' report_hash, 11.5 width,5.5 height
	END 
	

		INSERT INTO report_paramset(page_id, [name], paramset_hash, report_status_id)
		SELECT TOP 1 rpage.report_page_id, 'Eigen Values Report', 'F452AC55_6415_4B03_9D67_4966FE59A774', 3
		FROM sys.objects o
		INNER JOIN report_page rpage 
			on rpage.[name] = 'Standard Eigen Values Report'
		INNER JOIN report r 
		ON r.report_id = rpage.report_id
			AND r.[name] = 'Eigen Values Report'
		...
		INNER JOIN report_paramset rp 
			ON rp.[name] = 'Eigen Values Report'
		INNER JOIN report_page rpage 
			ON rpage.report_page_id = rp.page_id
			AND rpage.[name] = 'Standard Eigen Values Report'
		INNER JOIN report r 
			ON r.report_id = rpage.report_id
			AND r.[name] = 'Eigen Values Report'
		...
		SELECT TOP 1 rpage.report_page_id AS page_id, rd.report_dataset_id AS root_dataset_id, 'Standard Eigen Values Report_tablix' [name], '6.56' width, '3.7866666666666666' height, '0' [top], '0' [left],2 AS group_mode,1 AS border_style,0 AS page_break,2 AS type_id,1 AS cross_summary,2 AS no_header,'' export_table_name, 0 AS is_global
		FROM sys.objects o
		INNER JOIN report_page rpage 
		ON rpage.[name] = 'Standard Eigen Values Report'
		INNER JOIN report r 
		...
		INNER JOIN data_source_column dsc 
			ON dsc.source_id = ds.data_source_id AND dsc.[name] = 'eigen_values' 
