BEGIN TRY
		...
		FROM report r
		WHERE r.[name] = 'Yearly MTM by Book'
		...
		* */
		...
		BEGIN
			...
			SELECT TOP 1 'Yearly MTM by Book' [name], 'farrms_admin' [owner], 0 is_system, '55F8EA5D_A198_4CF7_A805_98607D5C936E' report_hash, 'Yearly MTM by book' [description], CAST(sdv_cat.value_id AS VARCHAR(10)) category_id
			FROM sys.objects o
...
COMMIT TRAN
	...
	IF NOT EXISTS(SELECT 1 FROM report_page rp 
	              WHERE rp.report_id = CASE WHEN 'e ' = 'p' 
											...
											ELSE @report_id_dest 
						               END 
					AND rp.name =  'Yearly MTM by Book'  
	)
	BEGIN
		...
		SELECT CASE WHEN 'e ' = 'p' 
					Then 45352 
					ELSE @report_id_dest 
		       END  AS report_id, 'Yearly MTM by Book' [name], '55F8EA5D_A198_4CF7_A805_98607D5C936E' report_hash, 7 width,9 height
	END 
	

		INSERT INTO report_paramset(page_id, [name], paramset_hash, report_status_id)
		SELECT TOP 1 rpage.report_page_id, 'Yearly MTM by Book', '70DAB351_F344_439E_B267_AE0A2367FC0A', 2
		FROM sys.objects o
		INNER JOIN report_page rpage 
			on rpage.[name] = 'Yearly MTM by Book'
		INNER JOIN report r 
		ON r.report_id = rpage.report_id
			AND r.[name] = 'Yearly MTM by Book'
	

		INSERT INTO report_dataset_paramset(paramset_id, root_dataset_id, where_part, advance_mode)
		SELECT TOP 1 rp.report_paramset_id AS paramset_id, rd.report_dataset_id AS root_dataset_id, '(  MTMV1.[term_start_year] BETWEEN ''@term_start_year'' AND ''@2_term_start_year'')' AS where_part, 0
		FROM sys.objects o
		INNER JOIN report_paramset rp 
			ON rp.[name] = 'Yearly MTM by Book'
		INNER JOIN report_page rpage 
			ON rpage.report_page_id = rp.page_id
			AND rpage.[name] = 'Yearly MTM by Book'
		INNER JOIN report r 
			ON r.report_id = rpage.report_id
			AND r.[name] = 'Yearly MTM by Book'
		...
		INSERT INTO report_param(dataset_paramset_id, dataset_id, column_id, operator,
					initial_value, initial_value2, optional, hidden, logical_operator, param_order, param_depth, label)
		SELECT TOP 1 rdp.report_dataset_paramset_id AS dataset_paramset_id, rd.report_dataset_id AS dataset_id , dsc.data_source_column_id AS column_id, 8 AS operator, '' AS initial_value, '' AS initial_value2, 1 AS optional, 0 AS hidden,1 AS logical_operator, 6 AS param_order, 0 AS param_depth, 'Year' AS label
		FROM sys.objects o
		INNER JOIN report_paramset rp 
			ON rp.[name] = 'Yearly MTM by Book'
		INNER JOIN report_page rpage 
		...
		SELECT TOP 1 rpage.report_page_id AS page_id, rd.report_dataset_id AS root_dataset_id, 'Yearly MTM by Book' [name], '6.986666666666666' width, '9' height, '0' [top], '0.013333333333333334' [left],0 AS group_mode,1 AS border_style,0 AS page_break,2 AS type_id,2 AS cross_summary,2 AS no_header,NULL export_table_name, 1 AS is_global
		FROM sys.objects o
		INNER JOIN report_page rpage 
		ON rpage.[name] = 'Yearly MTM by Book'
		INNER JOIN report r 
		...
		INNER JOIN report_page_tablix rpt 
			ON rpt.[name] = 'Yearly MTM by Book'
		INNER JOIN report_page rpage 
