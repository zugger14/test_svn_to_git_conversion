BEGIN TRY
		...
		IF EXISTS (SELECT 1 FROM dbo.report WHERE report_hash='F946B7DA_1F6D_4537_8A1F_80158670A56F')
		BEGIN
			DECLARE @report_id_to_delete INT
			SELECT @report_id_to_delete = report_id FROM report WHERE report_hash = 'F946B7DA_1F6D_4537_8A1F_80158670A56F'
		...
		set @report_copy_name = isnull(@report_copy_name, 'Copy of ' + 'Hourly Position Report By Book Enercity')
		

		INSERT INTO report ([name], [owner], is_system, is_excel, is_mobile, report_hash, [description], category_id)
		SELECT TOP 1 'Hourly Position Report by Book Enercity' [name], 'dev_admin' [owner], 0 is_system, 1 is_excel, 0 is_mobile, 'F946B7DA_1F6D_4537_8A1F_80158670A56F' report_hash, 'Hourly Position Report by Book Enercity' [description], CAST(sdv_cat.value_id AS VARCHAR(10)) category_id
		FROM sys.objects o
	...
	SELECT @report_id_dest AS report_id, 'Hourly Position Report by Book Enercity' [name], 'F946B7DA_1F6D_4537_8A1F_80158670A56F' report_hash, 11.5 width,5.5 height
		...
		SELECT TOP 1 rpage.report_page_id, 'Hourly Position Report by Book Enercity', 'CFE2E1D0_453B_41AA_8569_C3F63D3E13E2', 3,'','','.csv',';', 
		-100000,'y','n',0	
		FROM sys.objects o
		INNER JOIN report_page rpage 
			on rpage.[name] = 'Hourly Position Report by Book Enercity'
		INNER JOIN report r 
		ON r.report_id = rpage.report_id
			AND r.[name] = 'Hourly Position Report by Book Enercity'
		...
		INNER JOIN report_paramset rp 
			ON rp.[name] = 'Hourly Position Report by Book Enercity'
		INNER JOIN report_page rpage 
			ON rpage.report_page_id = rp.page_id
			AND rpage.[name] = 'Hourly Position Report by Book Enercity'
		INNER JOIN report r 
			ON r.report_id = rpage.report_id
			AND r.[name] = 'Hourly Position Report by Book Enercity'
		...
		SELECT TOP 1 rpage.report_page_id AS page_id, rd.report_dataset_id AS root_dataset_id, 'Enercity Hourly Position Report by Book_tablix' [name], '4' width, '2.6666666666666665' height, '0' [top], '0' [left],2 AS group_mode,1 AS border_style,0 AS page_break,2 AS type_id,2 AS cross_summary,2 AS no_header,'' export_table_name, 0 AS is_global
		FROM sys.objects o
		INNER JOIN report_page rpage 
		ON rpage.[name] = 'Hourly Position Report by Book Enercity'
		INNER JOIN report r 
		...
		INNER JOIN report_page_tablix rpt 
			ON rpt.[name] = 'Enercity Hourly Position Report by Book_tablix'
		INNER JOIN report_page rpage 
