BEGIN TRY
		...
		IF EXISTS (SELECT 1 FROM dbo.report WHERE report_hash='CEE2C459_A485_43C3_A182_FFC44544BF5F')
		BEGIN
			declare @report_id_to_delete int
			select @report_id_to_delete = report_id from report where report_hash = 'CEE2C459_A485_43C3_A182_FFC44544BF5F'
		...
		set @report_copy_name = isnull(@report_copy_name, 'Copy of ' + 'EOD - Derive Prices -Partner')
		

		INSERT INTO report ([name], [owner], is_system, is_excel, is_mobile, report_hash, [description], category_id)
		SELECT TOP 1 'EOD - Derive Prices -Partner' [name], 'dev_admin' [owner], 0 is_system, 0 is_excel, 0 is_mobile, 'CEE2C459_A485_43C3_A182_FFC44544BF5F' report_hash, 'EOD - Derive Prices -Partner' [description], CAST(sdv_cat.value_id AS VARCHAR(10)) category_id
		FROM sys.objects o
...
BEGIN TRY
	...
	declare @new_ds_alias varchar(10) = 'dppsv'
	/** IF DATA SOURCE ALIAS ALREADY EXISTS ON DESTINATION, RAISE ERROR **/
	if exists(select top 1 1 from data_source where alias = 'dppsv' and name <> 'Derive Prices Partner Sql View')
	begin
		select top 1 @new_ds_alias = 'dppsv' + cast(s.n as varchar(5))
		from seq s
		left join data_source ds on ds.alias = 'dppsv' + cast(s.n as varchar(5))
	...
	FROM report r
	WHERE r.[name] = 'EOD - Derive Prices -Partner'

	IF NOT EXISTS (SELECT 1 
	           FROM data_source 
	           WHERE [name] = 'Derive Prices Partner Sql View'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
	AND NOT EXISTS (SELECT 1 FROM map_function_category WHERE [function_name] = 'Derive Prices Partner Sql View' AND '106500' = '106501') 
	BEGIN
		INSERT INTO data_source([type_id], [name], [alias], [description], [tsql], report_id, system_defined,category)
		SELECT TOP 1 2 AS [type_id], 'Derive Prices Partner Sql View' AS [name], @new_ds_alias AS ALIAS, NULL AS [description],null AS [tsql], @report_id_data_source_dest AS report_id,NULL AS [system_defined]
			,'106500' AS [category]
	END
...
--	update source_price_curve_def set formula_id=137 where source_curve_def_id in (7404,7406,7400,7401,7402,7398)
...
SELECT @_tenor_from = CONVERT(VARCHAR(10), DATEADD(YEAR, DATEDIFF(YEAR, 0,  @_as_of_date)+1, 0), 120)
SELECT @_tenor_to = CONVERT(VARCHAR(10), DATEADD(YEAR, DATEDIFF(YEAR, 0,  @_as_of_date)+5, 0), 120)

--SELECT DATEADD(YEAR, DATEDIFF(YEAR, 0,  ''2020-10-15'')+5, 0)
...
', report_id = @report_id_data_source_dest,
	...
	IF EXISTS (SELECT 1 
	           ...
	           WHERE ds.[name] = 'Derive Prices Partner Sql View'
	            AND dsc.name =  'Area'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
	BEGIN
		UPDATE dsc  
	...
	BEGIN
		...
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Derive Prices Partner Sql View'
	...
	FROM data_source_column dsc 
	INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
		AND ds.[name] = 'Derive Prices Partner Sql View'
		AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1)
...
COMMIT TRAN
	...
	IF OBJECT_ID('tempdb..#data_source_column', 'U') IS NOT NULL
		...
		SELECT TOP 1 ds.data_source_id AS source_id, @report_id_dest AS report_id, 'dppsv' [alias], rd_root.report_dataset_id AS root_dataset_id,0 AS is_free_from, 'NULL' AS relationship_sql
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Derive Prices Partner Sql View'
	...
	SELECT @report_id_dest AS report_id, 'EOD - Derive Prices -Partner' [name], 'CEE2C459_A485_43C3_A182_FFC44544BF5F' report_hash, 11.5 width,5.5 height
		...
		SELECT TOP 1 rpage.report_page_id, 'EOD - Derive Prices -Partner', 'DF5A5B0B_ACCD_4C62_B690_5DBC91E7B6C6', 1,'','','.xlsx',',', 
		-100000,'n','n',0	
		FROM sys.objects o
		INNER JOIN report_page rpage 
			on rpage.[name] = 'EOD - Derive Prices -Partner'
		INNER JOIN report r 
		ON r.report_id = rpage.report_id
			AND r.[name] = 'EOD - Derive Prices -Partner'
		...
		INNER JOIN report_paramset rp 
			ON rp.[name] = 'EOD - Derive Prices -Partner'
		INNER JOIN report_page rpage 
			ON rpage.report_page_id = rp.page_id
			AND rpage.[name] = 'EOD - Derive Prices -Partner'
		INNER JOIN report r 
			ON r.report_id = rpage.report_id
			AND r.[name] = 'EOD - Derive Prices -Partner'
		...
		SELECT TOP 1 rpage.report_page_id AS page_id, rd.report_dataset_id AS root_dataset_id, 'EOD_Derive Prices_Partner_tablix' [name], '4' width, '2.6666666666666665' height, '0' [top], '0' [left],2 AS group_mode,1 AS border_style,0 AS page_break,1 AS type_id,1 AS cross_summary,2 AS no_header,'' export_table_name, 0 AS is_global
		FROM sys.objects o
		INNER JOIN report_page rpage 
		ON rpage.[name] = 'EOD - Derive Prices -Partner'
		INNER JOIN report r 
		...
		INNER JOIN report_page_tablix rpt 
			ON rpt.[name] = 'EOD_Derive Prices_Partner_tablix'
		INNER JOIN report_page rpage 
		...
		INNER JOIN report r 
			...
			AND r.[name] = 'EOD - Derive Prices -Partner'
		INNER JOIN report_dataset rd 
			ON rd.report_id = r.report_id AND rd.[alias] = 'dppsv' 	
		INNER JOIN data_source ds 
			ON ISNULL(NULLIF(ds.report_id, 0), r.report_id) = r.report_id	AND ds.[name] = 'Derive Prices Partner Sql View' 	
		INNER JOIN data_source_column dsc 
