...
IF EXISTS (SELECT 1 FROM dbo.report WHERE name='EOD - Derive Power RTC Monthly Curves')
		BEGIN
		EXEC spa_rfx_report 'd', NULL,53270, NULL, NULL, NULL, NULL, NULL
		END
		...
		FROM report r
		WHERE r.[name] = 'EOD - Derive Power RTC Monthly Curves'
		...
		* */
		...
		BEGIN
			...
			SELECT TOP 1 'EOD - Derive Power RTC Monthly Curves' [name], 'farrms_admin' [owner], 0 is_system, 0 is_excel, 0 is_mobile, '7F74F821_A827_458A_B31B_E255642CA310' report_hash, '' [description], CAST(sdv_cat.value_id AS VARCHAR(10)) category_id
			FROM sys.objects o
...
BEGIN TRY
	...
	IF EXISTS (SELECT 1 
	           FROM data_source 
	           WHERE [name] = 'Derive Power RTC Monthly Curves'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
			
	BEGIN
		UPDATE data_source
		SET alias = 'dprmc1', description = NULL
...
', report_id = @report_id_data_source_dest 
	...
	BEGIN
		...
		SELECT TOP 1 2 AS [type_id], 'Derive Power RTC Monthly Curves' AS [name], 'dprmc1' AS ALIAS, NULL AS [description],'DECLARE @_as_of_date VARCHAR(10) = ''@as_of_date''
DECLARE @_curve_ids VARCHAR(MAX) = ''@curve_ids'' 
DECLARE @_tenor_from VARCHAR(10) = NULL
DECLARE @_tenor_to VARCHAR(10) = NULL
...
' AS [tsql], @report_id_data_source_dest AS report_id
	...
	IF EXISTS (SELECT 1 
	           FROM data_source_column dsc 
	           INNER JOIN data_source ds on ds.data_source_id = dsc.source_id 
	           WHERE ds.[name] = 'Derive Power RTC Monthly Curves'
	            AND dsc.name =  'Area'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
	BEGIN
		UPDATE dsc  
	...
	BEGIN
		...
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Derive Power RTC Monthly Curves'
	...
	FROM data_source_column dsc 
	INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
		AND ds.[name] = 'Derive Power RTC Monthly Curves'
		AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1)
...
COMMIT TRAN
	...
	IF OBJECT_ID('tempdb..#data_source_column', 'U') IS NOT NULL
		...
		IF NOT EXISTS(SELECT 1 FROM report_dataset rd WHERE rd.report_id = @report_id_dest AND rd.[alias] =  'dprmc1')
		BEGIN
			INSERT INTO report_dataset (source_id, report_id, [alias], root_dataset_id, is_free_from, relationship_sql)
			SELECT TOP 1 ds.data_source_id AS source_id, @report_id_dest AS report_id, 'dprmc1' [alias], rd_root.report_dataset_id AS root_dataset_id,0 AS is_free_from, 'NULL' AS relationship_sql
			FROM sys.objects o
			INNER JOIN data_source ds ON ds.[name] = 'Derive Power RTC Monthly Curves'
	...
	IF NOT EXISTS(SELECT 1 FROM report_page rp 
	              WHERE rp.report_id = CASE WHEN 'e ' = 'p' 
											...
											ELSE @report_id_dest 
						               END 
					AND rp.name =  'EOD - Derive Power RTC Monthly Curves'  
	)
	BEGIN
		...
		SELECT CASE WHEN 'e ' = 'p' 
					Then 53270 
					ELSE @report_id_dest 
		       END  AS report_id, 'EOD - Derive Power RTC Monthly Curves' [name], '7F74F821_A827_458A_B31B_E255642CA310' report_hash, 11.5 width,5.5 height
	END 
		...
		SELECT TOP 1 rpage.report_page_id, 'EOD - Derive Power RTC Monthly Curves', 'D77FD7BF_D1E5_44E2_B318_0DC379F26307', 1,'','','.xlsx',',', 
		-100000,'n','n'	
		FROM sys.objects o
		INNER JOIN report_page rpage 
			on rpage.[name] = 'EOD - Derive Power RTC Monthly Curves'
		INNER JOIN report r 
		ON r.report_id = rpage.report_id
			AND r.[name] = 'EOD - Derive Power RTC Monthly Curves'
		...
		INNER JOIN report_paramset rp 
			ON rp.[name] = 'EOD - Derive Power RTC Monthly Curves'
		INNER JOIN report_page rpage 
			ON rpage.report_page_id = rp.page_id
			AND rpage.[name] = 'EOD - Derive Power RTC Monthly Curves'
		INNER JOIN report r 
			ON r.report_id = rpage.report_id
			AND r.[name] = 'EOD - Derive Power RTC Monthly Curves'
		...
		SELECT TOP 1 rpage.report_page_id AS page_id, rd.report_dataset_id AS root_dataset_id, 'EOD _ Derive Power RTC Monthly Curves_tablix' [name], '4' width, '2.6666666666666665' height, '0' [top], '0' [left],2 AS group_mode,1 AS border_style,0 AS page_break,1 AS type_id,1 AS cross_summary,2 AS no_header,'' export_table_name, 0 AS is_global
		FROM sys.objects o
		INNER JOIN report_page rpage 
		ON rpage.[name] = 'EOD - Derive Power RTC Monthly Curves'
		INNER JOIN report r 
		...
		INNER JOIN report_page_tablix rpt 
			ON rpt.[name] = 'EOD _ Derive Power RTC Monthly Curves_tablix'
		INNER JOIN report_page rpage 
		...
		INNER JOIN report r 
			...
			AND r.[name] = 'EOD - Derive Power RTC Monthly Curves'
		INNER JOIN report_dataset rd 
			ON rd.report_id = r.report_id AND rd.[alias] = 'dprmc1' 	
		INNER JOIN data_source ds 
			ON ISNULL(NULLIF(ds.report_id, 0), r.report_id) = r.report_id	AND ds.[name] = 'Derive Power RTC Monthly Curves' 	
		INNER JOIN data_source_column dsc 
