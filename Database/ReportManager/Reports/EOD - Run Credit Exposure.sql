...
IF EXISTS (SELECT 1 FROM dbo.report WHERE name='EOD - Run Credit Exposure')
		BEGIN
		EXEC spa_rfx_report 'd', NULL,53284, NULL, NULL, NULL, NULL, NULL
		END
		...
		* */
		...
		BEGIN
			...
			SELECT TOP 1 'EOD - Run Credit Exposure' [name], 'farrms_admin' [owner], 0 is_system, 0 is_excel, 0 is_mobile, '89CD3EE5_876F_40B6_BEB4_C6B757643CFE' report_hash, '' [description], CAST(sdv_cat.value_id AS VARCHAR(10)) category_id
			FROM sys.objects o
...
BEGIN TRY
	...
	BEGIN
		UPDATE data_source
		SET alias = 'rce1', description = NULL
...
', report_id = @report_id_data_source_dest 
	...
	BEGIN
		...
		SELECT TOP 1 2 AS [type_id], 'Run Credit Exposure' AS [name], 'rce1' AS ALIAS, NULL AS [description],'DECLARE @_as_of_date VARCHAR(10) = ''@as_of_date'',
		@_process_id VARCHAR(100) = ''@process_id'',	
		@_counterparty VARCHAR(500) = ''@counterparty''	
...
COMMIT TRAN
	...
	IF OBJECT_ID('tempdb..#data_source_column', 'U') IS NOT NULL
		...
		IF NOT EXISTS(SELECT 1 FROM report_dataset rd WHERE rd.report_id = @report_id_dest AND rd.[alias] =  'rce1')
		BEGIN
	...
	IF NOT EXISTS(SELECT 1 FROM report_page rp 
	              WHERE rp.report_id = CASE WHEN 'e ' = 'p' 
											...
											ELSE @report_id_dest 
						               END 
					AND rp.name =  'EOD - Run Credit Exposure'  
	)
	BEGIN
		...
		SELECT CASE WHEN 'e ' = 'p' 
					Then 53284 
					ELSE @report_id_dest 
		       END  AS report_id, 'EOD - Run Credit Exposure' [name], '89CD3EE5_876F_40B6_BEB4_C6B757643CFE' report_hash, 11.5 width,5.5 height
	END 
		...
		SELECT TOP 1 rpage.report_page_id, 'EOD - Run Credit Exposure', '6744ECB5_B183_4FEF_AC35_E5B96EC77FB5', 1,'','','.xlsx',',', 
		-100000,'n','n'	
		FROM sys.objects o
		...
		SELECT TOP 1 rpage.report_page_id AS page_id, rd.report_dataset_id AS root_dataset_id, 'EOD _ Run Credit Exposure_tablix' [name], '4' width, '2.6666666666666665' height, '0' [top], '0' [left],2 AS group_mode,1 AS border_style,0 AS page_break,1 AS type_id,1 AS cross_summary,2 AS no_header,'' export_table_name, 0 AS is_global
		FROM sys.objects o
		INNER JOIN report_page rpage 
		ON rpage.[name] = 'EOD - Run Credit Exposure'
		INNER JOIN report r 
