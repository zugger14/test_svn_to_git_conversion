BEGIN TRY
		...
		IF EXISTS (SELECT 1 FROM dbo.report WHERE report_hash='86E4C8A9_394B_431E_9C37_F33B9E814720')
		BEGIN
			declare @report_id_to_delete int
			select @report_id_to_delete = report_id from report where report_hash = '86E4C8A9_394B_431E_9C37_F33B9E814720'
		...
		set @report_copy_name = isnull(@report_copy_name, 'Copy of ' + 'EOD - Run Credit Exposure')
		

		INSERT INTO report ([name], [owner], is_system, is_excel, is_mobile, report_hash, [description], category_id)
		SELECT TOP 1 'EOD - Run Credit Exposure' [name], 'dev_admin' [owner], 1 is_system, 0 is_excel, 0 is_mobile, '86E4C8A9_394B_431E_9C37_F33B9E814720' report_hash, 'EOD - Run Credit Exposure' [description], CAST(sdv_cat.value_id AS VARCHAR(10)) category_id
		FROM sys.objects o
...
BEGIN TRY
	...
	declare @new_ds_alias varchar(10) = 'rce1'
	/** IF DATA SOURCE ALIAS ALREADY EXISTS ON DESTINATION, RAISE ERROR **/
	if exists(select top 1 1 from data_source where alias = 'rce1' and name <> 'Run Credit Exposure')
	begin
		select top 1 @new_ds_alias = 'rce1' + cast(s.n as varchar(5))
		from seq s
		left join data_source ds on ds.alias = 'rce1' + cast(s.n as varchar(5))
	...
	FROM report r
	WHERE r.[name] = 'EOD - Run Credit Exposure'

	IF NOT EXISTS (SELECT 1 
	           FROM data_source 
	           WHERE [name] = 'Run Credit Exposure'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
	AND NOT EXISTS (SELECT 1 FROM map_function_category WHERE [function_name] = 'Run Credit Exposure' AND '106500' = '106501') 
	BEGIN
		INSERT INTO data_source([type_id], [name], [alias], [description], [tsql], report_id, system_defined,category)
		SELECT TOP 1 2 AS [type_id], 'Run Credit Exposure' AS [name], @new_ds_alias AS ALIAS, NULL AS [description],null AS [tsql], @report_id_data_source_dest AS report_id,NULL AS [system_defined]
			,'106500' AS [category]
	END
	...
	, [tsql] = CAST('' AS VARCHAR(MAX)) + 'DECLARE @_as_of_date VARCHAR(10) = ''@as_of_date'',
		...
		@_counterparty VARCHAR(500) = ''@counterparty''	
...
IF ''@counterparty'' <> ''NULL''
    SET @_counterparty = ''@counterparty''
 ELSE    
    SET @_counterparty = NULL     
        		
IF OBJECT_ID(''tempdb..#tmp_result_credit_exposer'') IS NOT NULL DROP TABLE #tmp_result_credit_exposer
CREATE TABLE #tmp_result_credit_exposer (
...
EXEC spa_Calc_Credit_Netting_Exposure @_as_of_date, NULL, ''4500'', NULL, NULL, NULL, @_counterparty, ''n'', ''n'', ''n'', 0, ''n'', @_process_id
...
SELECT  
	...
	[Recommendation]
--[__batch_report__] 
FROM #tmp_result_credit_exposer
WHERE 1=1
...
', report_id = @report_id_data_source_dest,
	...
	IF EXISTS (SELECT 1 
	           ...
	           WHERE ds.[name] = 'Run Credit Exposure'
	            AND dsc.name =  'Area'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
	BEGIN
		UPDATE dsc  
	...
	BEGIN
		...
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Run Credit Exposure'
	...
	FROM data_source_column dsc 
	INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
		AND ds.[name] = 'Run Credit Exposure'
		AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1)
...
COMMIT TRAN
	...
	IF OBJECT_ID('tempdb..#data_source_column', 'U') IS NOT NULL
		...
		SELECT TOP 1 ds.data_source_id AS source_id, @report_id_dest AS report_id, 'rce1' [alias], rd_root.report_dataset_id AS root_dataset_id,0 AS is_free_from, 'NULL' AS relationship_sql
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Run Credit Exposure'
	...
	SELECT @report_id_dest AS report_id, 'EOD - Run Credit Exposure' [name], '86E4C8A9_394B_431E_9C37_F33B9E814720' report_hash, 11.5 width,5.5 height
		...
		SELECT TOP 1 rpage.report_page_id, 'EOD - Run Credit Exposure', '6B1B8A82_4391_4B6B_BE33_2A4CF4899E8E', 1,'','','.xlsx',',', 
		-100000,'n','n',NULL	
		FROM sys.objects o
		INNER JOIN report_page rpage 
			on rpage.[name] = 'EOD - Run Credit Exposure'
		INNER JOIN report r 
		ON r.report_id = rpage.report_id
			AND r.[name] = 'EOD - Run Credit Exposure'
		...
		INNER JOIN report_paramset rp 
			ON rp.[name] = 'EOD - Run Credit Exposure'
		INNER JOIN report_page rpage 
			ON rpage.report_page_id = rp.page_id
			AND rpage.[name] = 'EOD - Run Credit Exposure'
		INNER JOIN report r 
			ON r.report_id = rpage.report_id
			AND r.[name] = 'EOD - Run Credit Exposure'
		...
		INNER JOIN report_page_tablix rpt 
			ON rpt.[name] = 'EOD _ Run Credit Exposure_tablix'
		INNER JOIN report_page rpage 
		...
		INNER JOIN report r 
			...
			AND r.[name] = 'EOD - Run Credit Exposure'
		INNER JOIN report_dataset rd 
			ON rd.report_id = r.report_id AND rd.[alias] = 'rce1' 	
		INNER JOIN data_source ds 
			ON ISNULL(NULLIF(ds.report_id, 0), r.report_id) = r.report_id	AND ds.[name] = 'Run Credit Exposure' 	
		INNER JOIN data_source_column dsc 
