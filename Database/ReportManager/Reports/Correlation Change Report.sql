...
IF EXISTS (SELECT 1 FROM dbo.report WHERE name='Correlation Change Report')
		BEGIN
		EXEC spa_rfx_report 'd', NULL,18500, NULL, NULL, NULL, NULL, NULL
		END
		...
		FROM report r
		WHERE r.[name] = 'Correlation Change Report'
		...
		* */
		...
		BEGIN
			...
			SELECT TOP 1 'Correlation Change Report' [name], 'farrms_admin' [owner], 1 is_system, 1 is_excel ,'87901FB8_B2FC_4580_B834_6D38414D257B' report_hash, 'Standard Correlation Change Report' [description], CAST(sdv_cat.value_id AS VARCHAR(10)) category_id
			FROM sys.objects o
...
COMMIT TRAN
	...
	IF OBJECT_ID('tempdb..#data_source_column', 'U') IS NOT NULL
		...
		IF NOT EXISTS(SELECT 1 FROM report_dataset rd WHERE rd.report_id = @report_id_dest AND rd.[alias] =  'CRV1')
		BEGIN
	...
	IF NOT EXISTS(SELECT 1 FROM report_page rp 
	              WHERE rp.report_id = CASE WHEN 'e ' = 'p' 
											...
											ELSE @report_id_dest 
						               END 
					AND rp.name =  'Correlation Change Report'  
	)
	BEGIN
		...
		SELECT CASE WHEN 'e ' = 'p' 
					Then 18500 
					ELSE @report_id_dest 
		       END  AS report_id, 'Correlation Change Report' [name], '87901FB8_B2FC_4580_B834_6D38414D257B' report_hash, 11.5 width,5.5 height
	END 
	

		INSERT INTO report_paramset(page_id, [name], paramset_hash, report_status_id)
		SELECT TOP 1 rpage.report_page_id, 'Correlation Change Report', '86E4E7A8_0251_4FC1_82D4_B28F6B29138C', 3
		FROM sys.objects o
		...
		SELECT TOP 1 rpage.report_page_id AS page_id, rd.report_dataset_id AS root_dataset_id, 'Correlation Change Report_tablix' [name], '6.64' width, '4.373333333333333' height, '0' [top], '0' [left],2 AS group_mode,1 AS border_style,0 AS page_break,2 AS type_id,1 AS cross_summary,2 AS no_header,'' export_table_name, 0 AS is_global
		FROM sys.objects o
		INNER JOIN report_page rpage 
		ON rpage.[name] = 'Correlation Change Report'
		INNER JOIN report r 
