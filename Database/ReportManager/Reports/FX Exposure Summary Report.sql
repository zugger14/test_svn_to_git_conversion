...
IF EXISTS (SELECT 1 FROM dbo.report WHERE name='FX Exposure Summary Report')
		BEGIN
		EXEC spa_rfx_report 'd', NULL,29424, NULL, NULL, NULL, NULL, NULL
		END
		...
		FROM report r
		WHERE r.[name] = 'FX Exposure Summary Report'
		...
		* */
		...
		BEGIN
			...
			SELECT TOP 1 'FX Exposure Summary Report' [name], 'farrms_admin' [owner], 1 is_system, 0 is_excel, 0 is_mobile, '3D727F33_67D5_4CF9_BF58_B66A2139D82A' report_hash, 'FX Exposure Summary Report' [description], CAST(sdv_cat.value_id AS VARCHAR(10)) category_id
			FROM sys.objects o
...
COMMIT TRAN
	...
	IF NOT EXISTS(SELECT 1 FROM report_page rp 
	              WHERE rp.report_id = CASE WHEN 'e ' = 'p' 
											...
											ELSE @report_id_dest 
						               END 
					AND rp.name =  'FX Exposure Summary Report'  
	)
	BEGIN
		...
		SELECT CASE WHEN 'e ' = 'p' 
					Then 29424 
					ELSE @report_id_dest 
		       END  AS report_id, 'FX Exposure Summary Report' [name], '3D727F33_67D5_4CF9_BF58_B66A2139D82A' report_hash, 11.5 width,5.5 height
	END 
	

		INSERT INTO report_paramset(page_id, [name], paramset_hash, report_status_id)
		SELECT TOP 1 rpage.report_page_id, 'FX Exposure Summary Report', '3E4ACCDA_0F43_4D07_A5C2_926F1B2D3770', 1
		FROM sys.objects o
		INNER JOIN report_page rpage 
			on rpage.[name] = 'FX Exposure Summary Report'
		INNER JOIN report r 
		ON r.report_id = rpage.report_id
			AND r.[name] = 'FX Exposure Summary Report'
		...
		INNER JOIN report_paramset rp 
			ON rp.[name] = 'FX Exposure Summary Report'
		INNER JOIN report_page rpage 
			ON rpage.report_page_id = rp.page_id
			AND rpage.[name] = 'FX Exposure Summary Report'
		INNER JOIN report r 
			ON r.report_id = rpage.report_id
			AND r.[name] = 'FX Exposure Summary Report'
		...
		SELECT TOP 1 rpage.report_page_id AS page_id, rd.report_dataset_id AS root_dataset_id, 'FX Exposure Summary Report_tablix' [name], '5.653333333333333' width, '2.92' height, '0' [top], '0' [left],0 AS group_mode,1 AS border_style,0 AS page_break,2 AS type_id,2 AS cross_summary,2 AS no_header,NULL export_table_name, 0 AS is_global
		FROM sys.objects o
		INNER JOIN report_page rpage 
		ON rpage.[name] = 'FX Exposure Summary Report'
		INNER JOIN report r 
		...
		INSERT INTO report_tablix_column(tablix_id, dataset_id, column_id, placement, column_order, aggregation
					...
					, font_size, font_style, text_align, text_color, background, default_sort_order
					, default_sort_direction, custom_field, render_as, column_template, negative_mark, currency, date_format, cross_summary_aggregation, mark_for_total, sql_aggregation, subtotal)
		SELECT TOP 1 rpt.report_page_tablix_id tablix_id, rd.report_dataset_id dataset_id, dsc.data_source_column_id column_id,4 placement, 2 column_order,NULL aggregation, NULL functions, 'Monthly Term' [alias], 1 sortable, NULL rounding, NULL thousand_seperation, 'Tahoma' font, '8' font_size, '0,0,0' font_style, 'Left' text_align, '#000000' text_color, '#ffffff' background, NULL default_sort_order, NULL sort_direction, 0 custom_field, 4 render_as,-1 column_template,NULL negative_mark,NULL currency,0 date_format,NULL cross_summary_aggregation,NULL mark_for_total,NULL sql_aggregation,1 subtotal
