BEGIN TRY
		...
		IF EXISTS (SELECT 1 FROM dbo.report WHERE report_hash='06E5D6CF_8D5B_4C72_9FA0_5C4C96D587B1')
		BEGIN
			declare @report_id_to_delete int
			select @report_id_to_delete = report_id from report where report_hash = '06E5D6CF_8D5B_4C72_9FA0_5C4C96D587B1'
		...
		set @report_copy_name = isnull(@report_copy_name, 'Copy of ' + 'Cash Flow Report by Book and Counterparty')
		
		INSERT INTO report ([name], [owner], is_system, is_excel, is_mobile, report_hash, [description], category_id)
		SELECT TOP 1 'Cash Flow Report by Book and Counterparty' [name], 'dev_admin' [owner], 1 is_system, 1 is_excel, 0 is_mobile, '06E5D6CF_8D5B_4C72_9FA0_5C4C96D587B1' report_hash, 'Cash Flow Report by Book and Counterparty' [description], CAST(sdv_cat.value_id AS VARCHAR(10)) category_id
		FROM sys.objects o
	...
	SELECT @report_id_dest AS report_id, 'Cash Flow Report by Book and Counterparty' [name], '06E5D6CF_8D5B_4C72_9FA0_5C4C96D587B1' report_hash, 11.5 width,5.5 height
		...
		SELECT TOP 1 rpage.report_page_id, 'Cash Flow Report by Book and Counterparty', '36DCBB26_C141_474E_908B_D8705A78CBAB', 3,'','','.xlsx',',', 
		-100000,'n','n'	
		FROM sys.objects o
		INNER JOIN report_page rpage 
			on rpage.[name] = 'Cash Flow Report by Book and Counterparty'
		INNER JOIN report r 
		ON r.report_id = rpage.report_id
			AND r.[name] = 'Cash Flow Report by Book and Counterparty'
		...
		INNER JOIN report_paramset rp 
			ON rp.[name] = 'Cash Flow Report by Book and Counterparty'
		INNER JOIN report_page rpage 
			ON rpage.report_page_id = rp.page_id
			AND rpage.[name] = 'Cash Flow Report by Book and Counterparty'
		INNER JOIN report r 
			ON r.report_id = rpage.report_id
			AND r.[name] = 'Cash Flow Report by Book and Counterparty'
		...
		INSERT INTO report_param(dataset_paramset_id, dataset_id, column_id, operator,
					initial_value, initial_value2, optional, hidden, logical_operator, param_order, param_depth, label)
		SELECT TOP 1 rdp.report_dataset_paramset_id AS dataset_paramset_id, rd.report_dataset_id AS dataset_id , dsc.data_source_column_id AS column_id, 9 AS operator, '' AS initial_value, '' AS initial_value2, 0 AS optional, 1 AS hidden,1 AS logical_operator, 0 AS param_order, 0 AS param_depth, NULL AS label
		FROM sys.objects o
		INNER JOIN report_paramset rp 
			ON rp.[name] = 'Cash Flow Report by Book and Counterparty'
		INNER JOIN report_page rpage 
		...
		SELECT TOP 1 rpage.report_page_id AS page_id, rd.report_dataset_id AS root_dataset_id, 'Cash Flow by Book and Counterparty' [name], '5.946666666666666' width, '2.6666666666666665' height, '0' [top], '0' [left],2 AS group_mode,1 AS border_style,0 AS page_break,2 AS type_id,2 AS cross_summary,2 AS no_header,'' export_table_name, 0 AS is_global
		FROM sys.objects o
		INNER JOIN report_page rpage 
		ON rpage.[name] = 'Cash Flow Report by Book and Counterparty'
		INNER JOIN report r 
		...
		INNER JOIN report_page_tablix rpt 
			ON rpt.[name] = 'Cash Flow by Book and Counterparty'
		INNER JOIN report_page rpage 
		...
		INNER JOIN data_source ds 
			ON ISNULL(NULLIF(ds.report_id, 0), r.report_id) = r.report_id	AND ds.[name] = 'Settlement Mega View' 	
		INNER JOIN data_source_column dsc 
			ON dsc.source_id = ds.data_source_id AND dsc.[name] = 'actual_forward'  INSERT INTO report_tablix_header(tablix_id, column_id, font, font_size, font_style, text_align, text_color, background, report_tablix_column_id)
	  SELECT TOP 1 
