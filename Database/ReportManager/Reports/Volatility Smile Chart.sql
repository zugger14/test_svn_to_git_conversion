...
IF EXISTS (SELECT 1 FROM dbo.report WHERE name='Volatility Smile Chart')
		BEGIN
		EXEC spa_rfx_report 'd', NULL,40459, NULL, NULL, NULL, NULL, NULL
		END
		...
		FROM report r
		WHERE r.[name] = 'Volatility Smile Chart'
		...
		* */
		...
		BEGIN
			...
			SELECT TOP 1 'Volatility Smile Chart' [name], 'farrms_admin' [owner], 0 is_system, '548D40FD_ECB4_476C_8B4C_F7F13300B400' report_hash, 'Volatility Smile Chart' [description], CAST(sdv_cat.value_id AS VARCHAR(10)) category_id
			FROM sys.objects o
...
COMMIT TRAN
	...
	IF OBJECT_ID('tempdb..#data_source_column', 'U') IS NOT NULL
		...
		IF NOT EXISTS(SELECT 1 FROM report_dataset rd WHERE rd.report_id = @report_id_dest AND rd.[alias] =  'SIVV1')
		BEGIN
			INSERT INTO report_dataset (source_id, report_id, [alias], root_dataset_id, is_free_from, relationship_sql)
			SELECT TOP 1 ds.data_source_id AS source_id, @report_id_dest AS report_id, 'SIVV1' [alias], rd_root.report_dataset_id AS root_dataset_id,0 AS is_free_from, 'NULL' AS relationship_sql
			FROM sys.objects o
			INNER JOIN data_source ds ON ds.[name] = 'Implied Volatility View'
	...
	IF NOT EXISTS(SELECT 1 FROM report_page rp 
	              WHERE rp.report_id = CASE WHEN 'e ' = 'p' 
											...
											ELSE @report_id_dest 
						               END 
					AND rp.name =  'Volatility Smile Chart'  
	)
	BEGIN
		...
		SELECT CASE WHEN 'e ' = 'p' 
					Then 40459 
					ELSE @report_id_dest 
		       END  AS report_id, 'Volatility Smile Chart' [name], '548D40FD_ECB4_476C_8B4C_F7F13300B400' report_hash, 12 width,11.69 height
	END 
	

		INSERT INTO report_paramset(page_id, [name], paramset_hash, report_status_id)
		SELECT TOP 1 rpage.report_page_id, 'Volatility Smile Chart', 'BE05864F_7A45_4777_9988_A084AC902DC5', 1
		FROM sys.objects o
		INNER JOIN report_page rpage 
			on rpage.[name] = 'Volatility Smile Chart'
		INNER JOIN report r 
		ON r.report_id = rpage.report_id
			AND r.[name] = 'Volatility Smile Chart'
		...
		INNER JOIN report_paramset rp 
			ON rp.[name] = 'Volatility Smile Chart'
		INNER JOIN report_page rpage 
			ON rpage.report_page_id = rp.page_id
			AND rpage.[name] = 'Volatility Smile Chart'
		INNER JOIN report r 
			ON r.report_id = rpage.report_id
			AND r.[name] = 'Volatility Smile Chart'
		...
		INNER JOIN data_source ds 
			...
			AND ds.[name] = 'Implied Volatility View' 
		...
		SELECT TOP 1 rpage.report_page_id page_id, rd.report_dataset_id root_dataset_id, 'Volatility Smile Chart' [name], 3 [type_id], '7.52' width, '5.16' height, '0.10666666666666667' [top], '0.14666666666666667' [left], 'Strike' [x_axis_caption], 'Implied Volatility' [y_axis_caption],0 [page_break],'{"axes":{"y":{"render_as":"0","column_template":"-1","currency":"","thousand_list":"","rounding":"","date_format":"","font":"Tahoma","font_size":"8","bold_style":0,"italic_style":0,"underline_style":0,"text_color":"#000000"},"z":{"render_as":"0","column_template":"-1","currency":"","thousand_list":"","rounding":"","date_format":"","font":"Tahoma","font_size":"8","bold_style":0,"italic_style":0,"underline_style":0,"text_align":"Left","text_color":"#000000"},"x":{"render_as":"0","column_template":"-1","currency":"","thousand_list":"","rounding":"","date_format":"","font":"Tahoma","font_size":"8","bold_style":0,"italic_style":0,"underline_style":0,"text_color":"#000000"}},"axes_caption":{}}' [chart_properties] 
		FROM sys.objects o
		INNER JOIN report_page rpage
			ON rpage.[name] = 'Volatility Smile Chart'			
		INNER JOIN report r 
		...
		SELECT TOP 1 rpc.report_page_chart_id chart_id, rd.report_dataset_id dataset_id,
		  ...
		  dsc.data_source_column_id column_id,1 placement,1 column_order, 'Implied Volatility' [alias], NULL [functions], 8 aggregation,NULL default_sort_order, NULL default_sort_direction, 0 custom_field, 0 render_as_line
		FROM sys.objects o
		INNER JOIN report_page_chart rpc 
			ON rpc.[name] = 'Volatility Smile Chart'
		INNER JOIN report_page rpage 
		...
		INNER JOIN report_dataset rd 
			...
			AND rd.[alias] = 'SIVV1'	
		INNER JOIN data_source ds 
		ON ISNULL(NULLIF(ds.report_id, 0), r.report_id) = r.report_id AND ds.[name] = 'Implied Volatility View' 	
		INNER JOIN data_source_column dsc 
			ON dsc.source_id = ds.data_source_id  AND dsc.[name] = 'annual_implied_volatility' 
		...
		SELECT TOP 1 rpc.report_page_chart_id chart_id, rd.report_dataset_id dataset_id,
		  ...
		  dsc.data_source_column_id column_id,2 placement,2 column_order, 'Option Name' [alias], NULL [functions], NULL aggregation,NULL default_sort_order, NULL default_sort_direction, 0 custom_field, 0 render_as_line
		FROM sys.objects o
		INNER JOIN report_page_chart rpc 
			ON rpc.[name] = 'Volatility Smile Chart'
		INNER JOIN report_page rpage 
		...
		ON ISNULL(NULLIF(ds.report_id, 0), r.report_id) = r.report_id AND ds.[name] = 'Implied Volatility View' 	
		INNER JOIN data_source_column dsc 
			ON dsc.source_id = ds.data_source_id  AND dsc.[name] = 'option_name' 

		INSERT INTO report_chart_column(chart_id, dataset_id, column_id, placement, column_order, [alias], functions, aggregation, default_sort_order, default_sort_direction, custom_field, render_as_line)
		SELECT TOP 1 rpc.report_page_chart_id chart_id, rd.report_dataset_id dataset_id,
		
		  dsc.data_source_column_id column_id,3 placement,1 column_order, 'Strike' [alias], NULL [functions], NULL aggregation,NULL default_sort_order, NULL default_sort_direction, 0 custom_field, 0 render_as_line
		FROM sys.objects o
		INNER JOIN report_page_chart rpc 
			ON rpc.[name] = 'Volatility Smile Chart'
		INNER JOIN report_page rpage 
		...
		ON ISNULL(NULLIF(ds.report_id, 0), r.report_id) = r.report_id AND ds.[name] = 'Implied Volatility View' 	
		INNER JOIN data_source_column dsc 
			ON dsc.source_id = ds.data_source_id  AND dsc.[name] = 'strike' 
COMMIT 
