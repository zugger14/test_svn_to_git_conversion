BEGIN TRY
		...
		set @report_copy_name = isnull(@report_copy_name, 'Copy of ' + 'position formula')
		

		INSERT INTO report ([name], [owner], is_system, is_excel, is_mobile, report_hash, [description], category_id)
		SELECT TOP 1 'Position Formula' [name], 'dev_admin' [owner], 0 is_system, 0 is_excel, 0 is_mobile, 'A48E12D0_3792_4A72_95BC_9507138D83FA' report_hash, '' [description], CAST(sdv_cat.value_id AS VARCHAR(10)) category_id
		FROM sys.objects o
...
COMMIT TRAN
	...
	SELECT @report_id_dest AS report_id, 'Position Formula' [name], 'A48E12D0_3792_4A72_95BC_9507138D83FA' report_hash, 11.5 width,5.5 height
		...
		INNER JOIN report_page rpage 
			on rpage.[name] = 'Position Formula'
		INNER JOIN report r 
		ON r.report_id = rpage.report_id
			AND r.[name] = 'Position Formula'
		...
		INNER JOIN report_page rpage 
			...
			AND rpage.[name] = 'Position Formula'
		INNER JOIN report r 
			ON r.report_id = rpage.report_id
			AND r.[name] = 'Position Formula'
		...
		INSERT INTO report_tablix_column(tablix_id, dataset_id, column_id, placement, column_order, aggregation
					...
					, font_size, font_style, text_align, text_color, background, default_sort_order
					, default_sort_direction, custom_field, render_as, column_template, negative_mark, currency, date_format, cross_summary_aggregation, mark_for_total, sql_aggregation, subtotal)
		SELECT TOP 1 rpt.report_page_tablix_id tablix_id, rd.report_dataset_id dataset_id, dsc.data_source_column_id column_id,1 placement, 2 column_order,NULL aggregation, NULL functions, 'Location ID' [alias], 1 sortable, NULL rounding, NULL thousand_seperation, 'Tahoma' font, '8' font_size, '0,0,0' font_style, 'Left' text_align, '#000000' text_color, '#ffffff' background, NULL default_sort_order, NULL sort_direction, 0 custom_field, 0 render_as,-1 column_template,NULL negative_mark,NULL currency,NULL date_format,-1 cross_summary_aggregation,NULL mark_for_total,NULL sql_aggregation,NULL subtotal
