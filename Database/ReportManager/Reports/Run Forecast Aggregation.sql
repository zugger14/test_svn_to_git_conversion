BEGIN TRY
		...
		IF EXISTS (SELECT 1 FROM dbo.report WHERE report_hash='04BFBCE4_52F3_4D4E_898D_6F405E82F506')
		BEGIN
			declare @report_id_to_delete int
			select @report_id_to_delete = report_id from report where report_hash = '04BFBCE4_52F3_4D4E_898D_6F405E82F506'
		...
		set @report_copy_name = isnull(@report_copy_name, 'Copy of ' + 'Run Forecast Aggregation')
		

		INSERT INTO report ([name], [owner], is_system, is_excel, is_mobile, report_hash, [description], category_id)
		SELECT TOP 1 'Run Forecast Aggregation' [name], 'dev_admin' [owner], 1 is_system, 0 is_excel, 0 is_mobile, '04BFBCE4_52F3_4D4E_898D_6F405E82F506' report_hash, '' [description], CAST(sdv_cat.value_id AS VARCHAR(10)) category_id
		FROM sys.objects o
...
BEGIN TRY
	...
	declare @new_ds_alias varchar(10) = 'rfavs'
	/** IF DATA SOURCE ALIAS ALREADY EXISTS ON DESTINATION, RAISE ERROR **/
	if exists(select top 1 1 from data_source where alias = 'rfavs' and name <> 'Run Forecast Aggregation View SQL')
	begin
		select top 1 @new_ds_alias = 'rfavs' + cast(s.n as varchar(5))
		from seq s
		left join data_source ds on ds.alias = 'rfavs' + cast(s.n as varchar(5))
	...
	FROM report r
	WHERE r.[name] = 'Run Forecast Aggregation'

	IF NOT EXISTS (SELECT 1 
	           FROM data_source 
	           WHERE [name] = 'Run Forecast Aggregation View SQL'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
	AND NOT EXISTS (SELECT 1 FROM map_function_category WHERE [function_name] = 'Run Forecast Aggregation View SQL' AND '106500' = '106501') 
	BEGIN
		INSERT INTO data_source([type_id], [name], [alias], [description], [tsql], report_id, system_defined,category)
		SELECT TOP 1 2 AS [type_id], 'Run Forecast Aggregation View SQL' AS [name], @new_ds_alias AS ALIAS, NULL AS [description],null AS [tsql], @report_id_data_source_dest AS report_id,NULL AS [system_defined]
			,'106500' AS [category]
	END

	UPDATE data_source
	SET alias = @new_ds_alias, description = NULL
	, [tsql] = CAST('' AS VARCHAR(MAX)) + 'DECLARE @_process_id VARCHAR(100)
DECLARE @_zone VARCHAR(MAX)
DECLARE @_channel VARCHAR(MAX)
DECLARE @_product VARCHAR(MAX)
DECLARE @_as_of_date VARCHAR(MAX)
DECLARE @_committed_uncommitted VARCHAR(MAX) =''b''
...
IF ''@committed_uncommitted'' <> ''NULL''
	SET @_committed_uncommitted = ''@committed_uncommitted''
...
EXEC [dbo].[spa_forecast_aggregation] NULL, @_zone, @_channel, @_product, NULL, NULL, @_as_of_date, @_committed_uncommitted

--INSERT INTO #tmp_result (ErrorCode, Module, Area, Status, Message, Recommendation) 
--select ''Success'', ''Report'', ''Process'', ''success'', ''Demo message'', ''No recomendation''
...
SELECT @_zone [zone],
	   ...
	   @_committed_uncommitted [committed_uncommitted],
	[ErrorCode],
	[Module],
	[Area],
	[Status],
...
WHERE 1=1', report_id = @report_id_data_source_dest,
	...
	IF EXISTS (SELECT 1 
	           ...
	           WHERE ds.[name] = 'Run Forecast Aggregation View SQL'
	            AND dsc.name =  'Area'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
	BEGIN
		UPDATE dsc  
	...
	BEGIN
		...
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Run Forecast Aggregation View SQL'
	...
	BEGIN
		UPDATE dsc  
		SET alias = 'Channel'
			   , reqd_param = NULL, widget_id = 9, datatype_id = 5, param_data_source = 'SELECT DISTINCT [channel] AS id, [channel] AS VALUE FROM udt_customer_deals_header_info', param_default_value = NULL, append_filter = NULL, tooltip = NULL, column_template = 0, key_column = 0, required_filter = 0
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
		WHERE ds.[name] = 'Run Forecast Aggregation View SQL'
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'channel' AS [name], 'Channel' AS ALIAS, NULL AS reqd_param, 9 AS widget_id, 5 AS datatype_id, 'SELECT DISTINCT [channel] AS id, [channel] AS VALUE FROM udt_customer_deals_header_info' AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,0 AS column_template, 0 AS key_column, 0 AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Run Forecast Aggregation View SQL'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		UPDATE dsc  
		SET alias = 'Product'
			   , reqd_param = NULL, widget_id = 9, datatype_id = 5, param_data_source = 'SELECT DISTINCT [product] AS id, [product] AS VALUE FROM udt_customer_deals_header_info', param_default_value = NULL, append_filter = NULL, tooltip = NULL, column_template = 0, key_column = 0, required_filter = 0
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
		WHERE ds.[name] = 'Run Forecast Aggregation View SQL'
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'product' AS [name], 'Product' AS ALIAS, NULL AS reqd_param, 9 AS widget_id, 5 AS datatype_id, 'SELECT DISTINCT [product] AS id, [product] AS VALUE FROM udt_customer_deals_header_info' AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,0 AS column_template, 0 AS key_column, 0 AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Run Forecast Aggregation View SQL'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		UPDATE dsc  
		SET alias = 'Zone'
			   , reqd_param = NULL, widget_id = 9, datatype_id = 5, param_data_source = 'SELECT DISTINCT [zone]' + CHAR(9) + ' AS id, [zone]' + CHAR(9) + '  AS VALUE FROM udt_customer_deals_header_info', param_default_value = NULL, append_filter = NULL, tooltip = NULL, column_template = 0, key_column = 0, required_filter = 0
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
		WHERE ds.[name] = 'Run Forecast Aggregation View SQL'
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'zone' AS [name], 'Zone' AS ALIAS, NULL AS reqd_param, 9 AS widget_id, 5 AS datatype_id, 'SELECT DISTINCT [zone]' + CHAR(9) + ' AS id, [zone]' + CHAR(9) + '  AS VALUE FROM udt_customer_deals_header_info' AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,0 AS column_template, 0 AS key_column, 0 AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Run Forecast Aggregation View SQL'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	IF EXISTS (SELECT 1 
	           ...
	           INNER JOIN data_source ds on ds.data_source_id = dsc.source_id 
	           WHERE ds.[name] = 'Run Forecast Aggregation View SQL'
	            AND dsc.name =  'committed_uncommitted'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
	BEGIN
		UPDATE dsc  
		SET alias = 'Committed Uncommitted'
			   , reqd_param = NULL, widget_id = 2, datatype_id = 5, param_data_source = 'SELECT ''c'' value, ''Committed'' text UNION ALL' + CHAR(10) + 'SELECT ''u'' , ''Uncomitted''', param_default_value = NULL, append_filter = NULL, tooltip = NULL, column_template = 0, key_column = 0, required_filter = 0
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
		WHERE ds.[name] = 'Run Forecast Aggregation View SQL'
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'committed_uncommitted' AS [name], 'Committed Uncommitted' AS ALIAS, NULL AS reqd_param, 2 AS widget_id, 5 AS datatype_id, 'SELECT ''c'' value, ''Committed'' text UNION ALL' + CHAR(10) + 'SELECT ''u'' , ''Uncomitted''' AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,0 AS column_template, 0 AS key_column, 0 AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Run Forecast Aggregation View SQL'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	FROM data_source_column dsc 
	INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
		AND ds.[name] = 'Run Forecast Aggregation View SQL'
		AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1)
...
COMMIT TRAN
	...
	IF OBJECT_ID('tempdb..#data_source_column', 'U') IS NOT NULL
		...
		SELECT TOP 1 ds.data_source_id AS source_id, @report_id_dest AS report_id, 'rfavs' [alias], rd_root.report_dataset_id AS root_dataset_id,0 AS is_free_from, 'NULL' AS relationship_sql
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Run Forecast Aggregation View SQL'
	...
	SELECT @report_id_dest AS report_id, 'Run Forecast Aggregation' [name], '04BFBCE4_52F3_4D4E_898D_6F405E82F506' report_hash, 11.5 width,5.5 height
		...
		SELECT TOP 1 rpage.report_page_id, 'Run Forecast Aggregation', '289FDCD5_239C_498A_BF89_2FD9B31FD353', 3,'','','.xlsx',',', 
		-100000,'n','n'	
		FROM sys.objects o
		INNER JOIN report_page rpage 
			on rpage.[name] = 'Run Forecast Aggregation'
		INNER JOIN report r 
		ON r.report_id = rpage.report_id
			AND r.[name] = 'Run Forecast Aggregation'
		...
		INNER JOIN report_paramset rp 
			ON rp.[name] = 'Run Forecast Aggregation'
		INNER JOIN report_page rpage 
			ON rpage.report_page_id = rp.page_id
			AND rpage.[name] = 'Run Forecast Aggregation'
		INNER JOIN report r 
			ON r.report_id = rpage.report_id
			AND r.[name] = 'Run Forecast Aggregation'
		...
		SELECT TOP 1 rpage.report_page_id AS page_id, rd.report_dataset_id AS root_dataset_id, 'Run Forecast asggregation_tablix' [name], '4' width, '2.6666666666666665' height, '0' [top], '0' [left],2 AS group_mode,1 AS border_style,0 AS page_break,1 AS type_id,1 AS cross_summary,2 AS no_header,'' export_table_name, 0 AS is_global
		FROM sys.objects o
		INNER JOIN report_page rpage 
		ON rpage.[name] = 'Run Forecast Aggregation'
		INNER JOIN report r 
		...
		INNER JOIN report_page_tablix rpt 
			ON rpt.[name] = 'Run Forecast asggregation_tablix'
		INNER JOIN report_page rpage 
		...
		INNER JOIN report r 
			...
			AND r.[name] = 'Run Forecast Aggregation'
		INNER JOIN report_dataset rd 
			ON rd.report_id = r.report_id AND rd.[alias] = 'rfavs' 	
		INNER JOIN data_source ds 
			ON ISNULL(NULLIF(ds.report_id, 0), r.report_id) = r.report_id	AND ds.[name] = 'Run Forecast Aggregation View SQL' 	
		INNER JOIN data_source_column dsc 
