BEGIN TRY
		...
		IF EXISTS (SELECT 1 FROM dbo.report WHERE report_hash='D793A787_F1E1_4612_A1D8_897B73329E3E')
		BEGIN
			declare @report_id_to_delete int
			select @report_id_to_delete = report_id from report where report_hash = 'D793A787_F1E1_4612_A1D8_897B73329E3E'
		...
		set @report_copy_name = isnull(@report_copy_name, 'Copy of ' + 'EOD - Volatility and Correlation Calculation')
		

		INSERT INTO report ([name], [owner], is_system, is_excel, is_mobile, report_hash, [description], category_id)
		SELECT TOP 1 'EOD - Volatility and Correlation Calculation' [name], 'dev_admin' [owner], 1 is_system, 0 is_excel, 0 is_mobile, 'D793A787_F1E1_4612_A1D8_897B73329E3E' report_hash, 'EOD - Volatility and Correlation Calculation' [description], CAST(sdv_cat.value_id AS VARCHAR(10)) category_id
		FROM sys.objects o
...
BEGIN TRY
	...
	declare @new_ds_alias varchar(10) = 'vcc1'
	/** IF DATA SOURCE ALIAS ALREADY EXISTS ON DESTINATION, RAISE ERROR **/
	if exists(select top 1 1 from data_source where alias = 'vcc1' and name <> 'Volatility and Correlation Calculation')
	begin
		select top 1 @new_ds_alias = 'vcc1' + cast(s.n as varchar(5))
		from seq s
		left join data_source ds on ds.alias = 'vcc1' + cast(s.n as varchar(5))
	...
	FROM report r
	WHERE r.[name] = 'EOD - Volatility and Correlation Calculation'

	IF NOT EXISTS (SELECT 1 
	           FROM data_source 
	           WHERE [name] = 'Volatility and Correlation Calculation'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
	AND NOT EXISTS (SELECT 1 FROM map_function_category WHERE [function_name] = 'Volatility and Correlation Calculation' AND '106500' = '106501') 
	BEGIN
		INSERT INTO data_source([type_id], [name], [alias], [description], [tsql], report_id, system_defined,category)
		SELECT TOP 1 2 AS [type_id], 'Volatility and Correlation Calculation' AS [name], @new_ds_alias AS ALIAS, NULL AS [description],null AS [tsql], @report_id_data_source_dest AS report_id,NULL AS [system_defined]
			,'106500' AS [category]
	END
...
DECLARE @_var_id INT, @_spa VARCHAR(5000), @_cor_value CHAR(1)


IF ''@as_of_date'' = ''1900'' 
	BEGIN
		  INSERT INTO #tmp_result_calc_vol_cor_job (ErrorCode, Module, Area, Status, Message, Recommendation) 
...
BEGIN
	BEGIN
		SET @_spa = ''EXEC spa_calc_vol_cor_job '''''' + @_as_of_date + '''''', 4500, '' + CAST(@_var_id AS VARCHAR(10)) + '', '' + CASE WHEN @_process_id IS NOT NULL THEN '''''''' + @_process_id + '''''''' ELSE ''NULL'' END + '', NULL, NULL,NULL, NULL, NULL, ''''n'''', ''''y'''', '''''' + @_cor_value + '''''', NULL,NULL, NULL, NULL, NULL, NULL, NULL,10639, ''''n'''', 1'' + '', '' + CASE WHEN @_process_id IS NOT NULL THEN '''''''' + @_process_id + '''''''' ELSE ''NULL'' END + '',NULL''
		EXEC (@_spa)
	END
FETCH NEXT
FROM @_cur_var_id INTO @_var_id, @_cor_value
...
SELECT TOP 1 
	...
	[Recommendation]
--[__batch_report__] 
FROM  #tmp_result_calc_vol_cor_job ORDER BY [Status] DESC', report_id = @report_id_data_source_dest,
	system_defined = NULL
	,category = '106500' 
	WHERE [name] = 'Volatility and Correlation Calculation'
		AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1)
	...
	IF EXISTS (SELECT 1 
	           ...
	           WHERE ds.[name] = 'Volatility and Correlation Calculation'
	            AND dsc.name =  'Area'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
	BEGIN
		UPDATE dsc  
	...
	FROM data_source_column dsc 
	INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
		AND ds.[name] = 'Volatility and Correlation Calculation'
		AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1)
...
COMMIT TRAN
	...
	IF OBJECT_ID('tempdb..#data_source_column', 'U') IS NOT NULL
		...
		SELECT TOP 1 ds.data_source_id AS source_id, @report_id_dest AS report_id, 'vcc1' [alias], rd_root.report_dataset_id AS root_dataset_id,0 AS is_free_from, 'NULL' AS relationship_sql
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Volatility and Correlation Calculation'
	...
	SELECT @report_id_dest AS report_id, 'EOD - Volatility and Correlation Calculation' [name], 'D793A787_F1E1_4612_A1D8_897B73329E3E' report_hash, 11.5 width,5.5 height
		...
		SELECT TOP 1 rpage.report_page_id, 'EOD - Volatility and Correlation Calculation', 'F80470CE_402C_4555_8744_4D05F9AF8E78', 2,'','','.xlsx',',', 
		-100000,'n','n',0	
		FROM sys.objects o
		INNER JOIN report_page rpage 
			on rpage.[name] = 'EOD - Volatility and Correlation Calculation'
		INNER JOIN report r 
		ON r.report_id = rpage.report_id
			AND r.[name] = 'EOD - Volatility and Correlation Calculation'
		...
		INNER JOIN report_paramset rp 
			ON rp.[name] = 'EOD - Volatility and Correlation Calculation'
		INNER JOIN report_page rpage 
			ON rpage.report_page_id = rp.page_id
			AND rpage.[name] = 'EOD - Volatility and Correlation Calculation'
		INNER JOIN report r 
			ON r.report_id = rpage.report_id
			AND r.[name] = 'EOD - Volatility and Correlation Calculation'
		...
		SELECT TOP 1 rpage.report_page_id AS page_id, rd.report_dataset_id AS root_dataset_id, 'EOD _ Volatility and Correlation Calculation_tablix' [name], '4.773333333333333' width, '3.12' height, '0' [top], '0' [left],2 AS group_mode,1 AS border_style,0 AS page_break,1 AS type_id,1 AS cross_summary,2 AS no_header,'' export_table_name, 0 AS is_global
		FROM sys.objects o
		INNER JOIN report_page rpage 
		ON rpage.[name] = 'EOD - Volatility and Correlation Calculation'
		INNER JOIN report r 
		...
		INNER JOIN report_page_tablix rpt 
			ON rpt.[name] = 'EOD _ Volatility and Correlation Calculation_tablix'
		INNER JOIN report_page rpage 
		...
		INNER JOIN report r 
			...
			AND r.[name] = 'EOD - Volatility and Correlation Calculation'
		INNER JOIN report_dataset rd 
			ON rd.report_id = r.report_id AND rd.[alias] = 'vcc1' 	
		INNER JOIN data_source ds 
			ON ISNULL(NULLIF(ds.report_id, 0), r.report_id) = r.report_id	AND ds.[name] = 'Volatility and Correlation Calculation' 	
		INNER JOIN data_source_column dsc 
