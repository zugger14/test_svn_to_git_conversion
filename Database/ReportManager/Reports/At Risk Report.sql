BEGIN TRY
		...
		IF EXISTS (SELECT 1 FROM dbo.report WHERE report_hash='FDC8517D_AB7A_4465_ACBC_5E9535F7D154')
		BEGIN
			declare @report_id_to_delete int
			select @report_id_to_delete = report_id from report where report_hash = 'FDC8517D_AB7A_4465_ACBC_5E9535F7D154'
	...
	SELECT @report_id_dest AS report_id, 'At Risk Report' [name], 'FDC8517D_AB7A_4465_ACBC_5E9535F7D154' report_hash, 11.5 width,5.5 height
		...
		INNER JOIN report_paramset rp 
			ON rp.[name] = 'At Risk Report'
		INNER JOIN report_page rpage 
			ON rpage.report_page_id = rp.page_id
			AND rpage.[name] = 'At Risk Report'
		INNER JOIN report r 
			ON r.report_id = rpage.report_id
			AND r.[name] = 'At Risk Report'
		...
		INNER JOIN report_page_tablix rpt 
			ON rpt.[name] = 'At Risk Report_tablix'
		INNER JOIN report_page rpage 
