BEGIN TRY
		...
		IF EXISTS (SELECT 1 FROM dbo.report WHERE report_hash='74E89C25_462E_450B_89E9_178DDCCFADF2')
		BEGIN
			declare @report_id_to_delete int
			select @report_id_to_delete = report_id from report where report_hash = '74E89C25_462E_450B_89E9_178DDCCFADF2'
		...
		set @report_copy_name = isnull(@report_copy_name, 'Copy of ' + 'Default Probability Report')
		...
		SELECT TOP 1 ds.data_source_id AS source_id, @report_id_dest AS report_id, 'SDPV1' [alias], rd_root.report_dataset_id AS root_dataset_id,0 AS is_free_from, 'NULL' AS relationship_sql
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Default Probability View'
	...
	SELECT @report_id_dest AS report_id, 'Default Probability Report' [name], '74E89C25_462E_450B_89E9_178DDCCFADF2' report_hash, 11.5 width,5.5 height
		...
		ON r.report_id = rpage.report_id
			AND r.[name] = 'Default Probability Report'
		...
		INNER JOIN report_paramset rp 
			ON rp.[name] = 'Default Probability Report'
		INNER JOIN report_page rpage 
			ON rpage.report_page_id = rp.page_id
			AND rpage.[name] = 'Default Probability Report'
		INNER JOIN report r 
			ON r.report_id = rpage.report_id
			AND r.[name] = 'Default Probability Report'
		...
		INNER JOIN data_source ds 
			...
			AND ds.[name] = 'Default Probability View' 
		...
		SELECT TOP 1 rpage.report_page_id AS page_id, rd.report_dataset_id AS root_dataset_id, 'Standard Default Probability Report_tablix' [name], '6.986666666666666' width, '3.84' height, '0' [top], '0' [left],2 AS group_mode,1 AS border_style,0 AS page_break,1 AS type_id,1 AS cross_summary,2 AS no_header,'' export_table_name, 0 AS is_global
		FROM sys.objects o
		INNER JOIN report_page rpage 
		ON rpage.[name] = 'Default Probability Report'
		INNER JOIN report r 
		...
		INNER JOIN report_page_tablix rpt 
			ON rpt.[name] = 'Standard Default Probability Report_tablix'
		INNER JOIN report_page rpage 
		...
		INNER JOIN report r 
			...
			AND r.[name] = 'Default Probability Report'
		INNER JOIN report_dataset rd 
			ON rd.report_id = r.report_id AND rd.[alias] = 'SDPV1' 	
		INNER JOIN data_source ds 
			ON ISNULL(NULLIF(ds.report_id, 0), r.report_id) = r.report_id	AND ds.[name] = 'Default Probability View' 	
		INNER JOIN data_source_column dsc 
 ...
 INSERT INTO report_tablix_header(tablix_id, column_id, font, font_size, font_style, text_align, text_color, background, report_tablix_column_id)
	  SELECT TOP 1 
		...
		INNER JOIN report_tablix_column rtc 
			on rtc.tablix_id = rpt.report_page_tablix_id
			--AND rtc.column_id = dsc.data_source_column_id  --This did not handle custom column, got duplicate custom columns during export
			AND rtc.alias = 'Probability' --Added to handle custom column. Assumption: alias is unique and NOT NULL
