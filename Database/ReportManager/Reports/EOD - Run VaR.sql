BEGIN TRY
		...
		IF EXISTS (SELECT 1 FROM dbo.report WHERE report_hash='0CE1ECCA_BB6B_47FE_B9C8_8E4B005972D9')
		BEGIN
			declare @report_id_to_delete int
			select @report_id_to_delete = report_id from report where report_hash = '0CE1ECCA_BB6B_47FE_B9C8_8E4B005972D9'
		...
		set @report_copy_name = isnull(@report_copy_name, 'Copy of ' + 'EOD - Run VaR')
		

		INSERT INTO report ([name], [owner], is_system, is_excel, is_mobile, report_hash, [description], category_id)
		SELECT TOP 1 'EOD - Run VaR' [name], 'dev_admin' [owner], 1 is_system, 0 is_excel, 0 is_mobile, '0CE1ECCA_BB6B_47FE_B9C8_8E4B005972D9' report_hash, 'EOD - Run VaR' [description], CAST(sdv_cat.value_id AS VARCHAR(10)) category_id
		FROM sys.objects o
...
BEGIN TRY
	...
	declare @new_ds_alias varchar(10) = 'rv'
	/** IF DATA SOURCE ALIAS ALREADY EXISTS ON DESTINATION, RAISE ERROR **/
	if exists(select top 1 1 from data_source where alias = 'rv' and name <> 'Run VaR')
	begin
		select top 1 @new_ds_alias = 'rv' + cast(s.n as varchar(5))
		from seq s
		left join data_source ds on ds.alias = 'rv' + cast(s.n as varchar(5))
	...
	FROM report r
	WHERE r.[name] = 'EOD - Run VaR'

	IF NOT EXISTS (SELECT 1 
	           FROM data_source 
	           WHERE [name] = 'Run VaR'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
	AND NOT EXISTS (SELECT 1 FROM map_function_category WHERE [function_name] = 'Run VaR' AND '106500' = '106501') 
	BEGIN
		INSERT INTO data_source([type_id], [name], [alias], [description], [tsql], report_id, system_defined,category)
		SELECT TOP 1 2 AS [type_id], 'Run VaR' AS [name], @new_ds_alias AS ALIAS, NULL AS [description],null AS [tsql], @report_id_data_source_dest AS report_id,NULL AS [system_defined]
			,'106500' AS [category]
	END
...
CREATE TABLE #var_measurement_criteria(id INT)

IF @_criteria_id IS NOT NULL
BEGIN
	INSERT INTO #var_measurement_criteria(id)
	SELECT id
...
FOR
SELECT id FROM #var_measurement_criteria
OPEN @_cur_var_id
...
COMMIT TRAN
	...
	IF OBJECT_ID('tempdb..#data_source_column', 'U') IS NOT NULL
		...
		SELECT TOP 1 ds.data_source_id AS source_id, @report_id_dest AS report_id, 'rv' [alias], rd_root.report_dataset_id AS root_dataset_id,0 AS is_free_from, 'NULL' AS relationship_sql
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Run VaR'
	...
	SELECT @report_id_dest AS report_id, 'EOD - Run VaR' [name], '0CE1ECCA_BB6B_47FE_B9C8_8E4B005972D9' report_hash, 11.5 width,5.5 height
		...
		SELECT TOP 1 rpage.report_page_id, 'EOD - Run VaR', '7D4A0616_D9C7_4427_B879_71B12CADFD0E', 2,'','','.xlsx',',', 
		-100000,'n','n',NULL	
		FROM sys.objects o
		INNER JOIN report_page rpage 
			on rpage.[name] = 'EOD - Run VaR'
		INNER JOIN report r 
		ON r.report_id = rpage.report_id
			AND r.[name] = 'EOD - Run VaR'
		...
		INNER JOIN report_paramset rp 
			ON rp.[name] = 'EOD - Run VaR'
		INNER JOIN report_page rpage 
			ON rpage.report_page_id = rp.page_id
			AND rpage.[name] = 'EOD - Run VaR'
		INNER JOIN report r 
			ON r.report_id = rpage.report_id
			AND r.[name] = 'EOD - Run VaR'
		...
		SELECT TOP 1 rpage.report_page_id AS page_id, rd.report_dataset_id AS root_dataset_id, 'EOD _ Run VaR_tablix' [name], '4.613333333333333' width, '3.28' height, '0' [top], '0' [left],2 AS group_mode,1 AS border_style,0 AS page_break,1 AS type_id,1 AS cross_summary,2 AS no_header,'' export_table_name, 0 AS is_global
		FROM sys.objects o
		INNER JOIN report_page rpage 
		ON rpage.[name] = 'EOD - Run VaR'
		INNER JOIN report r 
		...
		INNER JOIN report_page_tablix rpt 
			ON rpt.[name] = 'EOD _ Run VaR_tablix'
		INNER JOIN report_page rpage 
		...
		INNER JOIN report r 
			...
			AND r.[name] = 'EOD - Run VaR'
		INNER JOIN report_dataset rd 
			ON rd.report_id = r.report_id AND rd.[alias] = 'rv' 	
		INNER JOIN data_source ds 
			ON ISNULL(NULLIF(ds.report_id, 0), r.report_id) = r.report_id	AND ds.[name] = 'Run VaR' 	
		INNER JOIN data_source_column dsc 
