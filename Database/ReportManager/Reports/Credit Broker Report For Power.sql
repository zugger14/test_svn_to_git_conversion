BEGIN TRY
		...
		IF EXISTS (SELECT 1 FROM dbo.report WHERE report_hash='280C3B65_C801_46F9_A82F_193D517DF249')
		BEGIN
			declare @report_id_to_delete int
			select @report_id_to_delete = report_id from report where report_hash = '280C3B65_C801_46F9_A82F_193D517DF249'
		...
		set @report_copy_name = isnull(@report_copy_name, 'Copy of ' + 'Credit Broker Report For Power')
		

		INSERT INTO report ([name], [owner], is_system, is_excel, is_mobile, report_hash, [description], category_id)
		SELECT TOP 1 'Credit Broker Report For Power' [name], 'dev_admin' [owner], 0 is_system, 0 is_excel, 0 is_mobile, '280C3B65_C801_46F9_A82F_193D517DF249' report_hash, 'Credit Broker Report For Power' [description], CAST(sdv_cat.value_id AS VARCHAR(10)) category_id
		FROM sys.objects o
	...
	SELECT @report_id_dest AS report_id, 'Credit Broker Report For Power' [name], '280C3B65_C801_46F9_A82F_193D517DF249' report_hash, 11.5 width,5.5 height
		...
		SELECT TOP 1 rpage.report_page_id, 'Broker Notification for Power', 'DF92D84B_04CA_410D_A004_D9FEFBADE626', 2,'','','.csv',',', 
		-100003,'y','n',NULL	
		FROM sys.objects o
		INNER JOIN report_page rpage 
			on rpage.[name] = 'Credit Broker Report For Power'
		INNER JOIN report r 
		ON r.report_id = rpage.report_id
			AND r.[name] = 'Credit Broker Report For Power'
		...
		INNER JOIN report_paramset rp 
			ON rp.[name] = 'Broker Notification for Power'
		INNER JOIN report_page rpage 
			ON rpage.report_page_id = rp.page_id
			AND rpage.[name] = 'Credit Broker Report For Power'
		INNER JOIN report r 
			ON r.report_id = rpage.report_id
			AND r.[name] = 'Credit Broker Report For Power'
		...
		SELECT TOP 1 rpage.report_page_id AS page_id, rd.report_dataset_id AS root_dataset_id, 'Credit Broker Notification Report_tablix' [name], '8.213333333333333' width, '3.6' height, '0.02666666666666667' [top], '0' [left],2 AS group_mode,1 AS border_style,0 AS page_break,2 AS type_id,1 AS cross_summary,2 AS no_header,'' export_table_name, 0 AS is_global
		FROM sys.objects o
		INNER JOIN report_page rpage 
		ON rpage.[name] = 'Credit Broker Report For Power'
		INNER JOIN report r 
		...
		INNER JOIN data_source ds 
			ON ISNULL(NULLIF(ds.report_id, 0), r.report_id) = r.report_id	AND ds.[name] = 'Broker Notification For Power' 	
		INNER JOIN data_source_column dsc 
