BEGIN TRY
		...
		SELECT TOP 1 'EOD - Tranche Deal Update' [name], 'dev_admin' [owner], 1 is_system, 0 is_excel, 0 is_mobile, '51C0FD75_FE86_4AF1_946C_2160F4263FCC' report_hash, '' [description], CAST(sdv_cat.value_id AS VARCHAR(10)) category_id
		FROM sys.objects o
...
BEGIN TRY
	...
	SELECT DISTINCT REPLACE(fe.formula, '' '', '''') formula, sdd.source_deal_header_id,	CASE WHEN fe.formula IS NOT NULL THEN 1 ELSE 0 END formula_applicable 
	...
	(source_deal_header_id int, pricing_index varchar(10) , multiplier float, adder float, negative_adder float)
	...
	dbo.fnagetsplitpart(replace(replace(replace(item,''dbo.FNALagCurve'', ''''),''('',''''),'')'',''''),'','',1),
		dbo.fnagetsplitpart(replace(replace(replace(item,''dbo.FNALagCurve'', ''''),''('',''''),'')'',''''),'','',8) multiplier, 
		NULL adder, 
		CASE WHEN item like ''%dbo.fna%'' THEN NULL ELSE item  END  negative_adder
	from #collect_deal_formula c
	OUTER APPLY dbo.FNASplit(formula, '')-'') s
	INNER JOIN #collect_deal_formula cdf ON cdf.formula = c.formula  
	AND c.source_deal_header_id = cdf.source_deal_header_id
	select Distinct * into #grouping_formula_update  from #grouping_formula
	IF OBJECT_ID(''tempdb..#item'') IS NOT NULL
		DROP table #item
	CREATE TABLE #item
	(source_deal_header_id INT,  pricing_index FLOAT, multiplier FLOAT, adder FLOAT, negative_adder FLOAT)
	INSERT INTO #item
	...
	dbo.fnagetsplitpart(replace(replace(replace(item,''dbo.FNALagCurve'', ''''),''('',''''),'')'',''''),'','',8) multiplier, CASE WHEN item like ''%dbo.fna%'' THEN NULL ELSE item  END adder , NULL negative_adder
	FROM #collect_deal_formula cdf 
		OUTER APPLY dbo.FNASplit(cdf.formula, ''+'')
	UNION ALL 
	SELECT DISTINCT cdf.source_deal_header_id, CASE WHEN item like ''%dbo.fna%''  THEN dbo.fnagetsplitpart(replace(replace(replace(item,''dbo.FNALagCurve'', ''''),''('',''''),'')'',''''),'','',1) Else NULL end Pricing_index,
	dbo.fnagetsplitpart(replace(replace(replace(item,''dbo.FNALagCurve'', ''''),''('',''''),'')'',''''),'','',8) multiplier, NULL as adder, CASE WHEN item like ''%dbo.fna%'' THEN NULL ELSE item  END negative_adder 
	FROM #collect_deal_formula cdf 
		OUTER APPLY dbo.FNASplit(cdf.formula, '')-'')
	ORDER BY cdf.source_deal_header_id
	select source_deal_header_id, max(adder) adder, max(negative_adder) negative_adder into #filter_data from #item group by source_deal_header_id  
	select Distinct * into #item_udpate from #item
		Update iu
		set
		iu.adder = 0
		from #item_udpate iu
		INNER JOIN #filter_data id on iu.source_deal_header_id = id.source_deal_header_id
		where id.negative_adder is NULL and id.adder is NULL
	...
	INNER JOIN #item_udpate i on i.source_deal_header_id = cd.source_deal_header_id 
	...
	(source_deal_header_id int, product float, adder float, negative_adder float)
	...
	SELECT Distinct cdc.source_deal_header_id,(o.multiplier * cdc.udf_value  + case when cdc.component = 1 then isnull(i.adder,-1 * i.negative_adder)  else 0 end ) Product, NULL adder, NULL negative_adder
	...
	LEFT JOIN #item_udpate i on i.source_deal_header_id = cdc.source_deal_header_id-- and i.pricing_index = cdc.udf_value
	...
	INNER JOIN #item_udpate it ON it.source_deal_header_id = c.source_deal_header_id 
	...
	OUTER Apply (
		...
		LEFT JOIN #item_udpate it ON it.source_deal_header_id = c.source_deal_header_id 
		LEFT JOIN #index_data_collection idc on idc.source_deal_header_id = c.source_deal_header_id and idc.Field_label = c.Field_label
	...
	and isnull(p.adder,p.negative_adder) is not null
     UNION ALL 
        SELECT 
	Distinct c.source_deal_header_id, (idc.curve_value * it.multiplier ) Product , (p.adder) adder, NULL negative_adder
	from #component_data_collection c
	...
	UNION ALL 
        SELECT 
	Distinct c.source_deal_header_id, (idc.curve_value * it.multiplier ) Product , NULL as adder , p.negative_adder 
	from #component_data_collection c
	INNER JOIN #index_data_collection idc on idc.source_deal_header_id = c.source_deal_header_id and idc.udf_deal_id = c.udf_deal_id
	INNER JOIN #item_udpate it ON it.source_deal_header_id = c.source_deal_header_id and it.pricing_index = idc.pricing_index
	OUTER Apply (
	...
	WHERE
	 p.source_deal_header_id = c.source_deal_header_id
	and p.negative_adder is not null
        and c.udf_value is null 
	and component = ''1'' and c.field_label = ''Pricing Index 1''
...
WHERE 1=1', report_id = @report_id_data_source_dest,
	...
	SELECT @report_id_dest AS report_id, 'EOD - Tranche Deal Update' [name], '51C0FD75_FE86_4AF1_946C_2160F4263FCC' report_hash, 11.5 width,5.5 height
		...
		SELECT TOP 1 rpage.report_page_id, 'EOD - Tranche Deal Update', '32208EB7_4CF1_4FF8_A4A0_041142041936', 3,'','','.xlsx',',', 
		-100000,'n','n',0	
		FROM sys.objects o
		...
		SELECT TOP 1 rpage.report_page_id AS page_id, rd.report_dataset_id AS root_dataset_id, 'EOD _ Tranche Deal Update_tablix' [name], '5.653333333333333' width, '2.8' height, '0' [top], '0' [left],2 AS group_mode,1 AS border_style,0 AS page_break,1 AS type_id,1 AS cross_summary,2 AS no_header,'' export_table_name, 0 AS is_global
		FROM sys.objects o
		INNER JOIN report_page rpage 
		ON rpage.[name] = 'EOD - Tranche Deal Update'
		INNER JOIN report r 
		...
		INNER JOIN data_source ds 
			ON ISNULL(NULLIF(ds.report_id, 0), r.report_id) = r.report_id	AND ds.[name] = 'EOD - Tranche Deal Update' 	
		INNER JOIN data_source_column dsc 
			ON dsc.source_id = ds.data_source_id AND dsc.[name] = 'ErrorCode'  INSERT INTO report_tablix_header(tablix_id, column_id, font, font_size, font_style, text_align, text_color, background, report_tablix_column_id)
	  SELECT TOP 1 
