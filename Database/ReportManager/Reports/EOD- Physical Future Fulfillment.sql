...
SELECT ''Success'' ErrorCode, ''Deal'' Module, ''physical future fulfillment'' Area, ''Success'' [Status]
				...
				, ''Physical Future Fulfillment successfully completed on '' + CONVERT(NVARCHAR(20),@_as_of_date,120) + '' for '' + CAST(ISNULL(@_deal_count,0) AS NVARCHAR(10)) + '' Deal(s).'' [Message], '''' Recommendation
