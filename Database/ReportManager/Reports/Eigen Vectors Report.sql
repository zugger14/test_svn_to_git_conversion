...
IF EXISTS (SELECT 1 FROM dbo.report WHERE name='Eigen Vectors Report')
		BEGIN
		EXEC spa_rfx_report 'd', NULL,39268, NULL, NULL, NULL, NULL, NULL
		END
		...
		FROM report r
		WHERE r.[name] = 'Eigen Vectors Report'
		...
		* */
		...
		BEGIN
			...
			SELECT TOP 1 'Eigen Vectors Report' [name], 'farrms_admin' [owner], 1 is_system, 1 is_excel, 'C1B2431F_38D9_4D53_B836_D3C6CE0614B5' report_hash, 'Standard Eigen Vectors Report' [description], CAST(sdv_cat.value_id AS VARCHAR(10)) category_id
			FROM sys.objects o
...
COMMIT TRAN
	...
	IF OBJECT_ID('tempdb..#data_source_column', 'U') IS NOT NULL
		...
		IF NOT EXISTS(SELECT 1 FROM report_dataset rd WHERE rd.report_id = @report_id_dest AND rd.[alias] =  'SEDV1')
		BEGIN
			INSERT INTO report_dataset (source_id, report_id, [alias], root_dataset_id, is_free_from, relationship_sql)
			SELECT TOP 1 ds.data_source_id AS source_id, @report_id_dest AS report_id, 'SEDV1' [alias], rd_root.report_dataset_id AS root_dataset_id,0 AS is_free_from, 'NULL' AS relationship_sql
			FROM sys.objects o
			INNER JOIN data_source ds ON ds.[name] = 'Eigen Decomposition View'
	...
	IF NOT EXISTS(SELECT 1 FROM report_page rp 
	              WHERE rp.report_id = CASE WHEN 'e ' = 'p' 
											...
											ELSE @report_id_dest 
						               END 
					AND rp.name =  'Standard Eigen Vectors Report'  
	)
	BEGIN
		...
		SELECT CASE WHEN 'e ' = 'p' 
					Then 39268 
					ELSE @report_id_dest 
		       END  AS report_id, 'Standard Eigen Vectors Report' [name], 'C1B2431F_38D9_4D53_B836_D3C6CE0614B5' report_hash, 11.5 width,5.5 height
	END 
	

		INSERT INTO report_paramset(page_id, [name], paramset_hash, report_status_id)
		SELECT TOP 1 rpage.report_page_id, 'Eigen Vectors Report', 'BE00E6F9_FC0E_453F_87CD_F53A77CB693E', 3
		FROM sys.objects o
		INNER JOIN report_page rpage 
			on rpage.[name] = 'Standard Eigen Vectors Report'
		INNER JOIN report r 
		ON r.report_id = rpage.report_id
			AND r.[name] = 'Eigen Vectors Report'
		...
		INNER JOIN report_paramset rp 
			ON rp.[name] = 'Eigen Vectors Report'
		INNER JOIN report_page rpage 
			ON rpage.report_page_id = rp.page_id
			AND rpage.[name] = 'Standard Eigen Vectors Report'
		INNER JOIN report r 
			ON r.report_id = rpage.report_id
			AND r.[name] = 'Eigen Vectors Report'
		...
		INNER JOIN data_source ds 
			...
			AND ds.[name] = 'Eigen Decomposition View' 
		...
		SELECT TOP 1 rpage.report_page_id AS page_id, rd.report_dataset_id AS root_dataset_id, 'Standard Eigen Vectors Report_tablix' [name], '7.946666666666666' width, '3.9466666666666667' height, '0.02666666666666667' [top], '0' [left],2 AS group_mode,1 AS border_style,0 AS page_break,2 AS type_id,1 AS cross_summary,2 AS no_header,'' export_table_name, 0 AS is_global
		FROM sys.objects o
		INNER JOIN report_page rpage 
		ON rpage.[name] = 'Standard Eigen Vectors Report'
		INNER JOIN report r 
		...
		INNER JOIN report r 
			...
			AND r.[name] = 'Eigen Vectors Report'
		INNER JOIN report_dataset rd 
			ON rd.report_id = r.report_id AND rd.[alias] = 'SEDV1' 	
		INNER JOIN data_source ds 
			ON ISNULL(NULLIF(ds.report_id, 0), r.report_id) = r.report_id	AND ds.[name] = 'Eigen Decomposition View' 	
		INNER JOIN data_source_column dsc 
			ON dsc.source_id = ds.data_source_id AND dsc.[name] = 'eigen_vectors' 
		...
		INNER JOIN data_source ds 
			ON ISNULL(NULLIF(ds.report_id, 0), r.report_id) = r.report_id	AND ds.[name] = 'Eigen Decomposition View' 	
		INNER JOIN data_source_column dsc 
			ON dsc.source_id = ds.data_source_id AND dsc.[name] = 'curve_to_name' 
		...
		INNER JOIN data_source ds 
			ON ISNULL(NULLIF(ds.report_id, 0), r.report_id) = r.report_id	AND ds.[name] = 'Eigen Decomposition View' 	
		INNER JOIN data_source_column dsc 
			ON dsc.source_id = ds.data_source_id AND dsc.[name] = 'curve_from_name' 
