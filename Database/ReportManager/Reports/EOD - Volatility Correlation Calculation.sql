...
IF EXISTS (SELECT 1 FROM dbo.report WHERE name='EOD - Volatility Correlation Calculation')
		BEGIN
		EXEC spa_rfx_report 'd', NULL,53274, NULL, NULL, NULL, NULL, NULL
		END
		...
		FROM report r
		WHERE r.[name] = 'EOD - Volatility Correlation Calculation'
		...
		* */
		...
		BEGIN
			...
			SELECT TOP 1 'EOD - Volatility Correlation Calculation' [name], 'farrms_admin' [owner], 0 is_system, 0 is_excel, 0 is_mobile, '04C8894E_0196_4D73_AE6D_CA68F9370409' report_hash, '' [description], CAST(sdv_cat.value_id AS VARCHAR(10)) category_id
			FROM sys.objects o
...
BEGIN TRY
	...
	IF EXISTS (SELECT 1 
	           FROM data_source 
	           WHERE [name] = 'Volatiliy Correlation Calculation'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
			
	BEGIN
		UPDATE data_source
		SET alias = 'vcc1', description = NULL
		, [tsql] = CAST('' AS VARCHAR(MAX)) + 'DECLARE @_as_of_date VARCHAR(10) = ''@as_of_date'',
		@_curve_id VARCHAR(MAX) = NULL,
...
SET @_term_end = CONVERT(VARCHAR(10), [dbo].[FNAGetFirstLastDayOfMonth](DATEADD(YEAR, 2, @_as_of_date), ''f''), 120)
...
EXEC spa_calc_vol_cor_job @_as_of_date, 4500, NULL, @_process_id,@_curve_id, @_term_start,@_term_end, NULL, NULL, ''n'', ''y'', ''v'', NULL,NULL, NULL, NULL, NULL, NULL, NULL,10639, ''n'' 
...
', report_id = @report_id_data_source_dest 
	...
	BEGIN
		...
		SELECT TOP 1 2 AS [type_id], 'Volatiliy Correlation Calculation' AS [name], 'vcc1' AS ALIAS, NULL AS [description],'DECLARE @_as_of_date VARCHAR(10) = ''@as_of_date'',
		@_curve_id VARCHAR(MAX) = NULL,
		@_term_start VARCHAR(10) = NULL,
		@_term_end VARCHAR(10) = NULL,
		@_process_id VARCHAR(100) = ''@process_id''
...
' AS [tsql], @report_id_data_source_dest AS report_id
	...
	IF EXISTS (SELECT 1 
	           FROM data_source_column dsc 
	           INNER JOIN data_source ds on ds.data_source_id = dsc.source_id 
	           WHERE ds.[name] = 'Volatiliy Correlation Calculation'
	            AND dsc.name =  'Area'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
	BEGIN
		UPDATE dsc  
	...
	BEGIN
		...
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Volatiliy Correlation Calculation'
	...
	FROM data_source_column dsc 
	INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
		AND ds.[name] = 'Volatiliy Correlation Calculation'
		AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1)
...
COMMIT TRAN
	...
	IF OBJECT_ID('tempdb..#data_source_column', 'U') IS NOT NULL
		...
		IF NOT EXISTS(SELECT 1 FROM report_dataset rd WHERE rd.report_id = @report_id_dest AND rd.[alias] =  'vcc1')
		BEGIN
	...
	IF NOT EXISTS(SELECT 1 FROM report_page rp 
	              WHERE rp.report_id = CASE WHEN 'e ' = 'p' 
											...
											ELSE @report_id_dest 
						               END 
					AND rp.name =  'EOD - Volatility Correlation Calculation'  
	)
	BEGIN
		...
		SELECT CASE WHEN 'e ' = 'p' 
					Then 53274 
					ELSE @report_id_dest 
		       END  AS report_id, 'EOD - Volatility Correlation Calculation' [name], '04C8894E_0196_4D73_AE6D_CA68F9370409' report_hash, 11.5 width,5.5 height
	END 
		...
		SELECT TOP 1 rpage.report_page_id, 'EOD - Volatility Correlation Calculation', '511344D7_0262_498C_B9AF_561DB920AABF', 1,'','','.xlsx',',', 
		-100000,'n','n'	
		FROM sys.objects o
		INNER JOIN report_page rpage 
			on rpage.[name] = 'EOD - Volatility Correlation Calculation'
		INNER JOIN report r 
		ON r.report_id = rpage.report_id
			AND r.[name] = 'EOD - Volatility Correlation Calculation'
		...
		INNER JOIN report_paramset rp 
			ON rp.[name] = 'EOD - Volatility Correlation Calculation'
		INNER JOIN report_page rpage 
			ON rpage.report_page_id = rp.page_id
			AND rpage.[name] = 'EOD - Volatility Correlation Calculation'
		INNER JOIN report r 
			ON r.report_id = rpage.report_id
			AND r.[name] = 'EOD - Volatility Correlation Calculation'
		...
		INNER JOIN data_source ds 
			ON ISNULL(NULLIF(ds.report_id, 0), r.report_id) = r.report_id	AND ds.[name] = 'Volatiliy Correlation Calculation' 	
		INNER JOIN data_source_column dsc 
