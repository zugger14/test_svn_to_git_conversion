BEGIN TRY
		...
		IF EXISTS (SELECT 1 FROM dbo.report WHERE report_hash='A41A4F9E_323D_48F9_A475_AA251D3892EA')
		BEGIN
			declare @report_id_to_delete int
			select @report_id_to_delete = report_id from report where report_hash = 'A41A4F9E_323D_48F9_A475_AA251D3892EA'
		...
		set @report_copy_name = isnull(@report_copy_name, 'Copy of ' + 'Power Continuous Nomination Data Export')
		

		INSERT INTO report ([name], [owner], is_system, is_excel, is_mobile, report_hash, [description], category_id)
		SELECT TOP 1 'Power Continuous Nomination Data Export' [name], 'dev_admin' [owner], 0 is_system, 1 is_excel, 0 is_mobile, 'A41A4F9E_323D_48F9_A475_AA251D3892EA' report_hash, 'Power Continuous Nomination Data Export' [description], CAST(sdv_cat.value_id AS VARCHAR(10)) category_id
		FROM sys.objects o
		...
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Export Nomination Power Data Continuous'
	...
	SELECT @report_id_dest AS report_id, 'Power Continuous Nomination Data Export' [name], 'A41A4F9E_323D_48F9_A475_AA251D3892EA' report_hash, 11.5 width,5.5 height
		...
		SELECT TOP 1 rpage.report_page_id, 'Power Continuous Nomination Data Export', '879DD57E_79D8_4591_A9EF_8004F53FEC29', 3,'','','.xlsx',',', 
		-100000,'n','n',0	
		FROM sys.objects o
		INNER JOIN report_page rpage 
			on rpage.[name] = 'Power Continuous Nomination Data Export'
		INNER JOIN report r 
		ON r.report_id = rpage.report_id
			AND r.[name] = 'Power Continuous Nomination Data Export'
		...
		INNER JOIN report_paramset rp 
			ON rp.[name] = 'Power Continuous Nomination Data Export'
		INNER JOIN report_page rpage 
			ON rpage.report_page_id = rp.page_id
			AND rpage.[name] = 'Power Continuous Nomination Data Export'
		INNER JOIN report r 
			ON r.report_id = rpage.report_id
			AND r.[name] = 'Power Continuous Nomination Data Export'
		...
		INNER JOIN data_source ds 
			...
			AND ds.[name] = 'Export Nomination Power Data Continuous' 
		...
		SELECT TOP 1 rpage.report_page_id AS page_id, rd.report_dataset_id AS root_dataset_id, 'Power Continuous Nomination Data Export_tablix' [name], '7.626666666666667' width, '3.7333333333333334' height, '0' [top], '0' [left],2 AS group_mode,1 AS border_style,0 AS page_break,1 AS type_id,1 AS cross_summary,2 AS no_header,'' export_table_name, 0 AS is_global
		FROM sys.objects o
		INNER JOIN report_page rpage 
		ON rpage.[name] = 'Power Continuous Nomination Data Export'
		INNER JOIN report r 
		...
		INNER JOIN report_page_tablix rpt 
			ON rpt.[name] = 'Power Continuous Nomination Data Export_tablix'
		INNER JOIN report_page rpage 
		...
		INNER JOIN data_source ds 
			ON ISNULL(NULLIF(ds.report_id, 0), r.report_id) = r.report_id	AND ds.[name] = 'Export Nomination Power Data Continuous' 	
		INNER JOIN data_source_column dsc 
			ON dsc.source_id = ds.data_source_id AND dsc.[name] = 'startDate' 
		...
		INNER JOIN data_source ds 
			ON ISNULL(NULLIF(ds.report_id, 0), r.report_id) = r.report_id	AND ds.[name] = 'Export Nomination Power Data Continuous' 	
		INNER JOIN data_source_column dsc 
			ON dsc.source_id = ds.data_source_id AND dsc.[name] = 'endDate' 
		...
		INNER JOIN data_source ds 
			ON ISNULL(NULLIF(ds.report_id, 0), r.report_id) = r.report_id	AND ds.[name] = 'Export Nomination Power Data Continuous' 	
		INNER JOIN data_source_column dsc 
			ON dsc.source_id = ds.data_source_id AND dsc.[name] = 'Time series id' 
