BEGIN TRY
		...
		IF EXISTS (SELECT 1 FROM dbo.report WHERE report_hash='66722AEC_AE47_4A13_BC9A_25BD77238ADF')
		BEGIN
			declare @report_id_to_delete int
			select @report_id_to_delete = report_id from report where report_hash = '66722AEC_AE47_4A13_BC9A_25BD77238ADF'
		...
		set @report_copy_name = isnull(@report_copy_name, 'Copy of ' + 'EOD - Copy Prices for Next Date')
		

		INSERT INTO report ([name], [owner], is_system, is_excel, is_mobile, report_hash, [description], category_id)
		SELECT TOP 1 'EOD - Copy Prices for Next Date' [name], 'dev_admin' [owner], 0 is_system, 0 is_excel, 0 is_mobile, '66722AEC_AE47_4A13_BC9A_25BD77238ADF' report_hash, 'EOD - Copy Prices for Next Date' [description], CAST(sdv_cat.value_id AS VARCHAR(10)) category_id
		FROM sys.objects o
...
BEGIN TRY
	...
	declare @new_ds_alias varchar(10) = 'cpfnd'
	/** IF DATA SOURCE ALIAS ALREADY EXISTS ON DESTINATION, RAISE ERROR **/
	if exists(select top 1 1 from data_source where alias = 'cpfnd' and name <> 'copy price for next day')
	begin
		select top 1 @new_ds_alias = 'cpfnd' + cast(s.n as varchar(5))
		from seq s
		left join data_source ds on ds.alias = 'cpfnd' + cast(s.n as varchar(5))
	...
	FROM report r
	WHERE r.[name] = 'EOD - Copy Prices for Next Date'

	IF NOT EXISTS (SELECT 1 
	           FROM data_source 
	           WHERE [name] = 'copy price for next day'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
	AND NOT EXISTS (SELECT 1 FROM map_function_category WHERE [function_name] = 'copy price for next day' AND '106500' = '106501') 
	BEGIN
		INSERT INTO data_source([type_id], [name], [alias], [description], [tsql], report_id, system_defined,category)
		SELECT TOP 1 2 AS [type_id], 'copy price for next day' AS [name], @new_ds_alias AS ALIAS, NULL AS [description],null AS [tsql], @report_id_data_source_dest AS report_id,NULL AS [system_defined]
			,'106500' AS [category]
	END
...
BEGIN TRY
	...
	SET @_sql = ''INSERT INTO #missing_price (source_curve_def_id, Forward_Settle)
				 ...
				 LEFT JOIN source_price_curve spc ON spd.source_curve_def_id = spc.source_curve_def_id AND spc.as_of_date = DATEADD(DAY, 1,'''''' + @_as_of_date + '''''') AND spc.curve_source_value_id = 4500
	...
	SELECT DISTINCT spc.source_curve_def_id
		...
		,DATEADD(DAY, 1, @_as_of_date) as_of_date
...
WHERE 1 = 1', report_id = @report_id_data_source_dest,
	...
	IF EXISTS (SELECT 1 
	           ...
	           WHERE ds.[name] = 'copy price for next day'
	            AND dsc.name =  'Area'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
	BEGIN
		UPDATE dsc  
	...
	BEGIN
		...
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'copy price for next day'
	...
	FROM data_source_column dsc 
	INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
		AND ds.[name] = 'copy price for next day'
		AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1)
...
COMMIT TRAN
	...
	IF OBJECT_ID('tempdb..#data_source_column', 'U') IS NOT NULL
		...
		SELECT TOP 1 ds.data_source_id AS source_id, @report_id_dest AS report_id, 'cpfnd' [alias], rd_root.report_dataset_id AS root_dataset_id,0 AS is_free_from, 'NULL' AS relationship_sql
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'copy price for next day'
	...
	SELECT @report_id_dest AS report_id, 'EOD - Copy Prices for Next Date' [name], '66722AEC_AE47_4A13_BC9A_25BD77238ADF' report_hash, 11.5 width,5.5 height
		...
		SELECT TOP 1 rpage.report_page_id, 'EOD - Copy Prices for Next Date', '92AEB8F0_EFA4_427D_9D70_E1E24212BC78', 2,'','','.xlsx',',', 
		-100000,'n','n',0	
		FROM sys.objects o
		INNER JOIN report_page rpage 
			on rpage.[name] = 'EOD - Copy Prices for Next Date'
		INNER JOIN report r 
		ON r.report_id = rpage.report_id
			AND r.[name] = 'EOD - Copy Prices for Next Date'
		...
		INNER JOIN report_paramset rp 
			ON rp.[name] = 'EOD - Copy Prices for Next Date'
		INNER JOIN report_page rpage 
			ON rpage.report_page_id = rp.page_id
			AND rpage.[name] = 'EOD - Copy Prices for Next Date'
		INNER JOIN report r 
			ON r.report_id = rpage.report_id
			AND r.[name] = 'EOD - Copy Prices for Next Date'
		...
		INNER JOIN report r 
			...
			AND r.[name] = 'EOD - Copy Prices for Next Date'
		INNER JOIN report_dataset rd 
			ON rd.report_id = r.report_id AND rd.[alias] = 'cpfnd' 	
		INNER JOIN data_source ds 
			ON ISNULL(NULLIF(ds.report_id, 0), r.report_id) = r.report_id	AND ds.[name] = 'copy price for next day' 	
		INNER JOIN data_source_column dsc 
