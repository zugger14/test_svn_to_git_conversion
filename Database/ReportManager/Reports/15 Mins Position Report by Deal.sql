BEGIN TRY
		...
		set @report_copy_name = isnull(@report_copy_name, 'Copy of ' + '15 Mins Position Report by Deal')
		

		INSERT INTO report ([name], [owner], is_system, is_excel, is_mobile, report_hash, [description], category_id)
		SELECT TOP 1 '15 Mins Position Report by Deal' [name], 'dev_admin' [owner], 0 is_system, 1 is_excel, 0 is_mobile, '7A4F5AF4_9D1F_4799_841A_F4A74162ABAE' report_hash, '15 Mins Position Report by Deal' [description], CAST(sdv_cat.value_id AS VARCHAR(10)) category_id
		FROM sys.objects o
	...
	SELECT @report_id_dest AS report_id, '15 Mins Position Report by Deal' [name], '7A4F5AF4_9D1F_4799_841A_F4A74162ABAE' report_hash, 11.5 width,5.5 height
		...
		SELECT TOP 1 rpage.report_page_id, '15 Mins Position Report by Deal', '4E391F9F_B672_4256_AD4B_7595D23D00AE', 3,'','','.xlsx',',', 
		-100000,'n','n',0	
		FROM sys.objects o
		INNER JOIN report_page rpage 
			on rpage.[name] = '15 Mins Position Report by Deal'
		INNER JOIN report r 
		ON r.report_id = rpage.report_id
			AND r.[name] = '15 Mins Position Report by Deal'
		...
		INNER JOIN report_paramset rp 
			ON rp.[name] = '15 Mins Position Report by Deal'
		INNER JOIN report_page rpage 
			ON rpage.report_page_id = rp.page_id
			AND rpage.[name] = '15 Mins Position Report by Deal'
		INNER JOIN report r 
			ON r.report_id = rpage.report_id
			AND r.[name] = '15 Mins Position Report by Deal'
