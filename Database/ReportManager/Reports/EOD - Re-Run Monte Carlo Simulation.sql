...
IF EXISTS (SELECT 1 FROM dbo.report WHERE name='EOD - Re-Run Monte Carlo Simulation')
		BEGIN
		EXEC spa_rfx_report 'd', NULL,23958, NULL, NULL, NULL, NULL, NULL
		END
		...
		FROM report r
		WHERE r.[name] = 'EOD - Re-Run Monte Carlo Simulation'
		...
		* */
		...
		BEGIN
			...
			SELECT TOP 1 'EOD - Re-Run Monte Carlo Simulation' [name], 'farrms_admin' [owner], 0 is_system, 0 is_excel, 0 is_mobile, 'DCE4C24F_0C5D_44D5_9DC9_A5438652C907' report_hash, '' [description], CAST(sdv_cat.value_id AS VARCHAR(10)) category_id
			FROM sys.objects o
...
BEGIN TRY
	...
	IF EXISTS (SELECT 1 
	           FROM data_source 
	           WHERE [name] = 'Re-Run Monte Carlo Simulation'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
			
	BEGIN
		UPDATE data_source
		SET alias = 'rrmcs', description = NULL
...
IF OBJECT_ID(''tempdb..#curve_list'') IS NOT NULL 
...
IF OBJECT_ID(''tempdb..#source_price_simulation_delta'') IS NOT NULL 
	DROP TABLE #source_price_simulation_delta
SET @_curve_id = ''''

SELECT distinct source_curve_def_id 
	INTO #source_price_curve_simulation 
	FROM source_price_curve_simulation WHERE run_date = @_as_of_date
 
SELECT distinct source_curve_def_id 
	INTO #source_price_simulation_delta  
	FROM source_price_simulation_delta  WHERE run_date = @_as_of_date

SELECT source_curve_def_id INTO #curve_list
	FROM   source_price_curve_def
	WHERE  curve_id IN (@_curve_id)	


SELECT @_curve_id = ISNULL(@_curve_id + '','', '''') + CAST(cl.source_curve_def_id AS VARCHAR(10)) FROM #curve_list cl 
	LEFT JOIN #source_price_curve_simulation sim ON sim.source_curve_def_id = cl.source_curve_def_id
	LEFT JOIN #source_price_simulation_delta del ON del.source_curve_def_id = cl.source_curve_def_id
	WHERE sim.source_curve_def_id IS NULL OR del.source_curve_def_id  IS NULL
...
CREATE TABLE #tmp_result (
	...
	Recommendation VARCHAR(200) COLLATE DATABASE_DEFAULT 
		...
		BEGIN TRY
			IF NULLIF(@_curve_id,'''') IS NOT NULL
			BEGIN	
			SET @_sql = ''EXEC spa_monte_carlo_simulation'''''' + @_as_of_date + '''''', '''''' 
					+ @_term_start + '''''', '''''' + @_term_end + '''''', 3000, NULL, '''''' + @_curve_id + '''''', NULL, ''''n'''',''''n'''',NULL, '''''' + @_process_id + ''''''''

			EXEC(@_sql)
			END
			
			DECLARE @_start_time1 DATETIME = GETDATE();
			DECLARE @_no_of_running_job1 INT = 0, @_time_out1 INT = 0;			
			
			SELECT @_no_of_running_job1 = COUNT(1) FROM tbl_sims_status WHERE process_id = @_process_id AND sims_status = ''R''	
						
			-- This will loop every 5 mins until there are running job of Monte Carl simulations, or until a timeout of 3 hours.
			WHILE (@_time_out1 = 0 and @_no_of_running_job1 > 0)
			BEGIN
				--five minute delay
				WAITFOR DELAY ''00:05:00'';
				
				SELECT @_no_of_running_job1 = COUNT(1) 
				FROM tbl_sims_status 
				WHERE process_id = @_process_id 
					AND sims_status = ''R''	
					
				--time out after 3 hours
				IF GETDATE() >= DATEADD(MINUTE, 180, @_start_time1) 
				BEGIN 
					SET @_time_out1 = 1 
					PRINT ''time out'' 
				END					
			...
			IF @_time_out1 = 0
				...
				SELECT ''Success'',''Monte Carlo Simulation'',''Monte Carlo Simulation'',''Success'',''Monte Carlo Simulation Re-Run Completed Successfully'',NULL
			ELSE
				INSERT INTO #tmp_result(ErrorCode,Module,Area,Status,Message,Recommendation)
				SELECT ''Error'',''Monte Carlo Simulation'',''Monte Carlo Simulation'',''Error'',''Monte Carlo Simulation Re-Run Failed with Timeout Error.'',NULL
		END TRY			
		BEGIN CATCH										
			INSERT INTO #tmp_result(ErrorCode,Module,Area,Status,Message,Recommendation)
				SELECT ''Error'',''Monte Carlo Simulation'',''Monte Carlo Simulation'',''Error'',''Monte Carlo Simulation Re-Run Failed with Technical Error.'',NULL
		END CATCH
...
WHERE 1=1
 ', report_id = @report_id_data_source_dest 
	...
	BEGIN
		...
		SELECT TOP 1 2 AS [type_id], 'Re-Run Monte Carlo Simulation' AS [name], 'rrmcs' AS ALIAS, NULL AS [description],'
DECLARE @_as_of_date VARCHAR(10) = ''@as_of_date'',
		@_tenor_from VARCHAR(500) = ''@tenor_from'',
		@_tenor_to VARCHAR(500) = ''@tenor_to'',
		@_curve_id VARCHAR(MAX) = ''@curve_id'',
...
WHERE 1=1
 ' AS [tsql], @report_id_data_source_dest AS report_id
	...
	IF EXISTS (SELECT 1 
	           FROM data_source_column dsc 
	           INNER JOIN data_source ds on ds.data_source_id = dsc.source_id 
	           WHERE ds.[name] = 'Re-Run Monte Carlo Simulation'
	            AND dsc.name =  'Area'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
	BEGIN
		UPDATE dsc  
	...
	BEGIN
		...
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Re-Run Monte Carlo Simulation'
	...
	FROM data_source_column dsc 
	INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
		AND ds.[name] = 'Re-Run Monte Carlo Simulation'
		AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1)
...
COMMIT TRAN
	...
	IF OBJECT_ID('tempdb..#data_source_column', 'U') IS NOT NULL
		...
		IF NOT EXISTS(SELECT 1 FROM report_dataset rd WHERE rd.report_id = @report_id_dest AND rd.[alias] =  'rrmcs')
		BEGIN
			INSERT INTO report_dataset (source_id, report_id, [alias], root_dataset_id, is_free_from, relationship_sql)
			SELECT TOP 1 ds.data_source_id AS source_id, @report_id_dest AS report_id, 'rrmcs' [alias], rd_root.report_dataset_id AS root_dataset_id,0 AS is_free_from, 'NULL' AS relationship_sql
			FROM sys.objects o
			INNER JOIN data_source ds ON ds.[name] = 'Re-Run Monte Carlo Simulation'
	...
	IF NOT EXISTS(SELECT 1 FROM report_page rp 
	              WHERE rp.report_id = CASE WHEN 'e ' = 'p' 
											...
											ELSE @report_id_dest 
						               END 
					AND rp.name =  'EOD - Re-Run Monte Carlo Simulation'  
	)
	BEGIN
		...
		SELECT CASE WHEN 'e ' = 'p' 
					Then 23958 
					ELSE @report_id_dest 
		       END  AS report_id, 'EOD - Re-Run Monte Carlo Simulation' [name], 'DCE4C24F_0C5D_44D5_9DC9_A5438652C907' report_hash, 11.5 width,5.5 height
	END 
		...
		SELECT TOP 1 rpage.report_page_id, 'EOD - Re-Run Monte Carlo Simulation', 'A149D3CD_D92F_4B95_882D_D3634E750B93', 1,'','','.xlsx',',', 
		0,'n','n'	
		FROM sys.objects o
		INNER JOIN report_page rpage 
			on rpage.[name] = 'EOD - Re-Run Monte Carlo Simulation'
		INNER JOIN report r 
		ON r.report_id = rpage.report_id
			AND r.[name] = 'EOD - Re-Run Monte Carlo Simulation'
		...
		INNER JOIN report_paramset rp 
			ON rp.[name] = 'EOD - Re-Run Monte Carlo Simulation'
		INNER JOIN report_page rpage 
			ON rpage.report_page_id = rp.page_id
			AND rpage.[name] = 'EOD - Re-Run Monte Carlo Simulation'
		INNER JOIN report r 
			ON r.report_id = rpage.report_id
			AND r.[name] = 'EOD - Re-Run Monte Carlo Simulation'
		...
		INSERT INTO report_param(dataset_paramset_id, dataset_id, column_id, operator,
					initial_value, initial_value2, optional, hidden, logical_operator, param_order, param_depth, label)
		SELECT TOP 1 rdp.report_dataset_paramset_id AS dataset_paramset_id, rd.report_dataset_id AS dataset_id , dsc.data_source_column_id AS column_id, 1 AS operator, '1' AS initial_value, '' AS initial_value2, 0 AS optional, 0 AS hidden,1 AS logical_operator, 2 AS param_order, 0 AS param_depth, NULL AS label
		FROM sys.objects o
		INNER JOIN report_paramset rp 
			ON rp.[name] = 'EOD - Re-Run Monte Carlo Simulation'
		INNER JOIN report_page rpage 
		...
		INSERT INTO report_param(dataset_paramset_id, dataset_id, column_id, operator,
					initial_value, initial_value2, optional, hidden, logical_operator, param_order, param_depth, label)
		SELECT TOP 1 rdp.report_dataset_paramset_id AS dataset_paramset_id, rd.report_dataset_id AS dataset_id , dsc.data_source_column_id AS column_id, 1 AS operator, '24' AS initial_value, '' AS initial_value2, 0 AS optional, 0 AS hidden,1 AS logical_operator, 3 AS param_order, 0 AS param_depth, NULL AS label
		FROM sys.objects o
		INNER JOIN report_paramset rp 
			ON rp.[name] = 'EOD - Re-Run Monte Carlo Simulation'
		INNER JOIN report_page rpage 
		...
		INNER JOIN report r 
			...
			AND r.[name] = 'EOD - Re-Run Monte Carlo Simulation'
		INNER JOIN report_dataset rd 
			ON rd.report_id = r.report_id AND rd.[alias] = 'rrmcs' 	
		INNER JOIN data_source ds 
			ON ISNULL(NULLIF(ds.report_id, 0), r.report_id) = r.report_id	AND ds.[name] = 'Re-Run Monte Carlo Simulation' 	
		INNER JOIN data_source_column dsc 
