BEGIN TRY
		...
		IF EXISTS (SELECT 1 FROM dbo.report WHERE report_hash='0CCA3B11_8BD7_4E4C_B76B_DD99E46C713F')
		BEGIN
			declare @report_id_to_delete int
			select @report_id_to_delete = report_id from report where report_hash = '0CCA3B11_8BD7_4E4C_B76B_DD99E46C713F'
		...
		SELECT TOP 1 'EOD - Run Settlement' [name], 'farrms_admin' [owner], 0 is_system, 0 is_excel, 0 is_mobile, '0CCA3B11_8BD7_4E4C_B76B_DD99E46C713F' report_hash, '' [description], CAST(sdv_cat.value_id AS VARCHAR(10)) category_id
		FROM sys.objects o
...
COMMIT TRAN
	...
	SELECT @report_id_dest AS report_id, 'EOD - Run Settlement' [name], '0CCA3B11_8BD7_4E4C_B76B_DD99E46C713F' report_hash, 11.5 width,5.5 height
		...
		SELECT TOP 1 rpage.report_page_id, 'EOD - Run Settlement', '4EED052D_1C47_4501_BAF2_0C8C1511E578', 1,'','','.xlsx',',', 
		-100000,'n','n'	
		FROM sys.objects o
		...
		SELECT TOP 1 rpage.report_page_id AS page_id, rd.report_dataset_id AS root_dataset_id, 'EOD _ Run Settlement_tablix' [name], '4' width, '2.6666666666666665' height, '0' [top], '0.02666666666666667' [left],2 AS group_mode,1 AS border_style,0 AS page_break,1 AS type_id,1 AS cross_summary,2 AS no_header,'' export_table_name, 0 AS is_global
		FROM sys.objects o
		INNER JOIN report_page rpage 
		ON rpage.[name] = 'EOD - Run Settlement'
		INNER JOIN report r 
