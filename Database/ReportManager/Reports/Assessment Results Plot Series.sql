BEGIN TRY
		...
		IF EXISTS (SELECT 1 FROM dbo.report WHERE report_hash='12478249_30D9_47E2_9500_6F039346B645')
		BEGIN
			declare @report_id_to_delete int
			select @report_id_to_delete = report_id from report where report_hash = '12478249_30D9_47E2_9500_6F039346B645'
		...
		BEGIN
			...
			SELECT TOP 1 'Assessment Results Plot Series' [name], 'farrms_admin' [owner], 1 is_system, 0 is_excel, 0 is_mobile, '12478249_30D9_47E2_9500_6F039346B645' report_hash, 'Standard Assessment Results Plot Series' [description], CAST(sdv_cat.value_id AS VARCHAR(10)) category_id
			FROM sys.objects o
...
BEGIN TRY
	...
	FROM report r
	WHERE r.[name] = 'Assessment Results Plot Series'

	IF NOT EXISTS (SELECT 1 
	           FROM data_source 
	           WHERE [name] = 'Assessment Results Plot Series'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
	BEGIN
		INSERT INTO data_source([type_id], [name], [alias], [description], [tsql], report_id)
		SELECT TOP 1 2 AS [type_id], 'Assessment Results Plot Series' AS [name], 'arps' AS ALIAS, NULL AS [description],null AS [tsql], @report_id_data_source_dest AS report_id
	END

	UPDATE data_source
	SET alias = 'arps', description = NULL
	, [tsql] = CAST('' AS VARCHAR(MAX)) + 'IF OBJECT_ID(''tempdb..#temp_tbl'') IS NOT NULL
DROP TABLE #temp_tbl
CREATE TABLE #temp_tbl(AsOfDate DATETIME, Hedge		FLOAT, Item 	FLOAT)


INSERT INTO #temp_tbl(AsOfDate, Hedge, Item)
EXEC  spa_Get_Assessment_Results_curves_Plot @result_id

SELECT AsOfDate, Hedge, item, [result_id]
 --[__batch_report__]
FROM (
SELECT AsOfDate,	Hedge Hedge,	''hedge'' item, @result_id [result_id]
FROM #temp_tbl
UNION ALL
SELECT AsOfDate,	item,	''Item'', @result_id  [result_id]
FROM #temp_tbl
	...
	IF EXISTS (SELECT 1 
	           ...
	           WHERE ds.[name] = 'Assessment Results Plot Series'
	            AND dsc.name =  'AsOfDate'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
	BEGIN
		UPDATE dsc  
		SET alias = 'Asofdate'
	...
	BEGIN
		...
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'AsOfDate' AS [name], 'Asofdate' AS ALIAS, 0 AS reqd_param, 6 AS widget_id, 2 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, 1 AS append_filter, NULL  AS tooltip,4 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Assessment Results Plot Series'
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'item' AS [name], 'Item' AS ALIAS, 0 AS reqd_param, 1 AS widget_id, 5 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, 1 AS append_filter, NULL  AS tooltip,0 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Assessment Results Plot Series'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	FROM data_source_column dsc 
	INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
		AND ds.[name] = 'Assessment Results Plot Series'
		AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1)
...
COMMIT TRAN
	...
	IF OBJECT_ID('tempdb..#data_source_column', 'U') IS NOT NULL
		...
		IF NOT EXISTS(SELECT 1 FROM report_dataset rd WHERE rd.report_id = @report_id_dest AND rd.[alias] =  'arps')
		BEGIN
			INSERT INTO report_dataset (source_id, report_id, [alias], root_dataset_id, is_free_from, relationship_sql)
			SELECT TOP 1 ds.data_source_id AS source_id, @report_id_dest AS report_id, 'arps' [alias], rd_root.report_dataset_id AS root_dataset_id,0 AS is_free_from, 'NULL' AS relationship_sql
			FROM sys.objects o
			INNER JOIN data_source ds ON ds.[name] = 'Assessment Results Plot Series'
	...
	IF NOT EXISTS(SELECT 1 FROM report_page rp 
	              WHERE rp.report_id = CASE WHEN 'e ' = 'p' 
											...
											ELSE @report_id_dest 
						               END 
					AND rp.name =  'Assessment Results Plot Series'  
	)
	BEGIN
		...
		SELECT CASE WHEN 'e ' = 'p' 
					Then 33434 
					ELSE @report_id_dest 
		       END  AS report_id, 'Assessment Results Plot Series' [name], '12478249_30D9_47E2_9500_6F039346B645' report_hash, 12 width,7.252 height
	END 
		...
		SELECT TOP 1 rpage.report_page_id, 'Assessment Results Plot Series', '4868AA97_2868_4D31_AF11_863DF4CC8B46', 3,'','','.xlsx',',', 
		-100000,'n','n'	
		FROM sys.objects o
		INNER JOIN report_page rpage 
			on rpage.[name] = 'Assessment Results Plot Series'
		INNER JOIN report r 
		ON r.report_id = rpage.report_id
			AND r.[name] = 'Assessment Results Plot Series'
		...
		INNER JOIN report_paramset rp 
			ON rp.[name] = 'Assessment Results Plot Series'
		INNER JOIN report_page rpage 
			ON rpage.report_page_id = rp.page_id
			AND rpage.[name] = 'Assessment Results Plot Series'
		INNER JOIN report r 
			ON r.report_id = rpage.report_id
			AND r.[name] = 'Assessment Results Plot Series'
		...
		SELECT TOP 1 rpage.report_page_id page_id, rd.report_dataset_id root_dataset_id, 'Assessment Results Plot Series' [name], 3 [type_id], '11.973333333333332' width, '7.226666666666667' height, '0.02666666666666667' [top], '0.04' [left], 'As of Date' [x_axis_caption], 'Hedge/Item' [y_axis_caption],0 [page_break],'{"axes":{"y":{"render_as":"2","column_template":"-1","currency":"","thousand_list":"0","rounding":"4","date_format":"","font":"Tahoma","font_size":"8","bold_style":0,"italic_style":0,"underline_style":0,"text_color":"#000000"},"z":{"render_as":"0","column_template":"-1","currency":"","thousand_list":"","rounding":"","date_format":"","font":"Tahoma","font_size":"8","bold_style":0,"italic_style":0,"underline_style":0,"text_align":"Left","text_color":"#000000"},"x":{"render_as":"4","column_template":"-1","currency":"","thousand_list":"","rounding":"","date_format":"0","font":"Tahoma","font_size":"8","bold_style":0,"italic_style":0,"underline_style":0,"text_color":"#000000"}},"axes_caption":{}}' [chart_properties] 
		FROM sys.objects o
		INNER JOIN report_page rpage
			ON rpage.[name] = 'Assessment Results Plot Series'			
		INNER JOIN report r 
		...
		SELECT TOP 1 rpc.report_page_chart_id chart_id, rd.report_dataset_id dataset_id,
		  ...
		  dsc.data_source_column_id column_id,1 placement,1 column_order, 'Hedge/Item' [alias], NULL [functions], 8 aggregation,NULL default_sort_order, NULL default_sort_direction, 0 custom_field, 0 render_as_line
		FROM sys.objects o
		INNER JOIN report_page_chart rpc 
			ON rpc.[name] = 'Assessment Results Plot Series'
		INNER JOIN report_page rpage 
		...
		INNER JOIN report_dataset rd 
			...
			AND rd.[alias] = 'arps'	
		INNER JOIN data_source ds 
		ON ISNULL(NULLIF(ds.report_id, 0), r.report_id) = r.report_id AND ds.[name] = 'Assessment Results Plot Series' 	
		INNER JOIN data_source_column dsc 
		...
		SELECT TOP 1 rpc.report_page_chart_id chart_id, rd.report_dataset_id dataset_id,
		  ...
		  dsc.data_source_column_id column_id,3 placement,1 column_order, 'Asofdate' [alias], NULL [functions], NULL aggregation,NULL default_sort_order, 1 default_sort_direction, 0 custom_field, 0 render_as_line
		FROM sys.objects o
		INNER JOIN report_page_chart rpc 
			ON rpc.[name] = 'Assessment Results Plot Series'
		INNER JOIN report_page rpage 
		...
		ON ISNULL(NULLIF(ds.report_id, 0), r.report_id) = r.report_id AND ds.[name] = 'Assessment Results Plot Series' 	
		INNER JOIN data_source_column dsc 
			ON dsc.source_id = ds.data_source_id  AND dsc.[name] = 'AsOfDate' 
COMMIT 
