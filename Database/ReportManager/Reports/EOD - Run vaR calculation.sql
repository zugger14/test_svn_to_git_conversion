...
IF EXISTS (SELECT 1 FROM dbo.report WHERE name='EOD - Run vaR calculation')
		BEGIN
		EXEC spa_rfx_report 'd', NULL,53275, NULL, NULL, NULL, NULL, NULL
		END
		...
		FROM report r
		WHERE r.[name] = 'EOD - Run vaR calculation'
		...
		* */
		...
		BEGIN
			...
			SELECT TOP 1 'EOD - Run vaR calculation' [name], 'farrms_admin' [owner], 0 is_system, 0 is_excel, 0 is_mobile, 'A83D88B8_4D90_43D8_A659_51F839E7FDF7' report_hash, '' [description], CAST(sdv_cat.value_id AS VARCHAR(10)) category_id
			FROM sys.objects o
...
BEGIN TRY
	...
	IF EXISTS (SELECT 1 
	           FROM data_source 
	           WHERE [name] = 'Run vaR calculation'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
			
	BEGIN
		UPDATE data_source
		SET alias = 'rvc', description = NULL
		, [tsql] = CAST('' AS VARCHAR(MAX)) + 'DECLARE @_as_of_date VARCHAR(10) = ''@as_of_date'', 
		@_process_id VARCHAR(100) = ''@process_id'',
		@_var_criteria_id1     INT,
		@_var_criteria_id2     INT,
		@_var_criteria_id3     INT,
		@_var_criteria_id4     INT,
		@_var_criteria_id5     INT,
		@_var_criteria_id6     INT 
...
SELECT @_var_criteria_id1 = id
FROM   var_measurement_criteria_detail
WHERE  NAME = ''Total NFC Portfolio (1 day)''

SELECT @_var_criteria_id2 = id
FROM   var_measurement_criteria_detail
WHERE  NAME = ''Total NFC Portfolio (3 days)''

SELECT @_var_criteria_id3 = id
FROM   var_measurement_criteria_detail
WHERE  NAME = ''Total FAC Portfolio-Year 1 (10 days)''

SELECT @_var_criteria_id4 = id
FROM   var_measurement_criteria_detail
WHERE  NAME = ''Total FAC Portfolio-Year 1 (HTM)''

SELECT @_var_criteria_id5 = id
FROM   var_measurement_criteria_detail
WHERE  NAME = ''Total FAC Portfolio-Year 2 (10 days)''

SELECT @_var_criteria_id6 = id
FROM   var_measurement_criteria_detail
WHERE  NAME = ''Total FAC Portfolio-Year 2 (HTM)''    
...
EXEC spa_calc_var_simulation_job  @_as_of_date , @_var_criteria_id1 , NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL , @_process_id 

SELECT  
    @_as_of_date as_of_date, 
    @_var_criteria_id1 var_criteria_id,
...
WHERE 1=1
 ...
 ', report_id = @report_id_data_source_dest 
	...
	BEGIN
		...
		SELECT TOP 1 2 AS [type_id], 'Run vaR calculation' AS [name], 'rvc' AS ALIAS, NULL AS [description],'DECLARE @_as_of_date VARCHAR(10) = ''@as_of_date'', 
		@_process_id VARCHAR(100) = ''@process_id'',
		@_var_criteria_id1     INT,
		@_var_criteria_id2     INT,
		@_var_criteria_id3     INT,
...
WHERE 1=1
 ...
 ' AS [tsql], @report_id_data_source_dest AS report_id
	...
	IF EXISTS (SELECT 1 
	           FROM data_source_column dsc 
	           INNER JOIN data_source ds on ds.data_source_id = dsc.source_id 
	           WHERE ds.[name] = 'Run vaR calculation'
	            AND dsc.name =  'Area'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
	BEGIN
		UPDATE dsc  
	...
	BEGIN
		...
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Run vaR calculation'
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'var_criteria_id' AS [name], 'Var Criteria ID' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 4 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,2 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Run vaR calculation'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	FROM data_source_column dsc 
	INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
		AND ds.[name] = 'Run vaR calculation'
		AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1)
...
COMMIT TRAN
	...
	IF OBJECT_ID('tempdb..#data_source_column', 'U') IS NOT NULL
		...
		IF NOT EXISTS(SELECT 1 FROM report_dataset rd WHERE rd.report_id = @report_id_dest AND rd.[alias] =  'rvc')
		BEGIN
			INSERT INTO report_dataset (source_id, report_id, [alias], root_dataset_id, is_free_from, relationship_sql)
			SELECT TOP 1 ds.data_source_id AS source_id, @report_id_dest AS report_id, 'rvc' [alias], rd_root.report_dataset_id AS root_dataset_id,0 AS is_free_from, 'NULL' AS relationship_sql
			FROM sys.objects o
			INNER JOIN data_source ds ON ds.[name] = 'Run vaR calculation'
	...
	IF NOT EXISTS(SELECT 1 FROM report_page rp 
	              WHERE rp.report_id = CASE WHEN 'e ' = 'p' 
											...
											ELSE @report_id_dest 
						               END 
					AND rp.name =  'EOD - Run vaR calculation'  
	)
	BEGIN
		...
		SELECT CASE WHEN 'e ' = 'p' 
					Then 53275 
					ELSE @report_id_dest 
		       END  AS report_id, 'EOD - Run vaR calculation' [name], 'A83D88B8_4D90_43D8_A659_51F839E7FDF7' report_hash, 11.5 width,5.5 height
	END 
		...
		SELECT TOP 1 rpage.report_page_id, 'EOD - Run vaR calculation', 'DD80E143_74FD_4F11_9B20_9FCB488BF34A', 1,'','','.xlsx',',', 
		-100000,'n','n'	
		FROM sys.objects o
		INNER JOIN report_page rpage 
			on rpage.[name] = 'EOD - Run vaR calculation'
		INNER JOIN report r 
		ON r.report_id = rpage.report_id
			AND r.[name] = 'EOD - Run vaR calculation'
		...
		INNER JOIN report_paramset rp 
			ON rp.[name] = 'EOD - Run vaR calculation'
		INNER JOIN report_page rpage 
			ON rpage.report_page_id = rp.page_id
			AND rpage.[name] = 'EOD - Run vaR calculation'
		INNER JOIN report r 
			ON r.report_id = rpage.report_id
			AND r.[name] = 'EOD - Run vaR calculation'
		...
		SELECT TOP 1 rpage.report_page_id AS page_id, rd.report_dataset_id AS root_dataset_id, 'EOD _ Run vaR calculation_tablix' [name], '4' width, '2.6666666666666665' height, '0' [top], '0' [left],2 AS group_mode,1 AS border_style,0 AS page_break,1 AS type_id,1 AS cross_summary,2 AS no_header,'' export_table_name, 0 AS is_global
		FROM sys.objects o
		INNER JOIN report_page rpage 
		ON rpage.[name] = 'EOD - Run vaR calculation'
		INNER JOIN report r 
		...
		INNER JOIN report_page_tablix rpt 
			ON rpt.[name] = 'EOD _ Run vaR calculation_tablix'
		INNER JOIN report_page rpage 
		...
		INNER JOIN report r 
			...
			AND r.[name] = 'EOD - Run vaR calculation'
		INNER JOIN report_dataset rd 
			ON rd.report_id = r.report_id AND rd.[alias] = 'rvc' 	
		INNER JOIN data_source ds 
			ON ISNULL(NULLIF(ds.report_id, 0), r.report_id) = r.report_id	AND ds.[name] = 'Run vaR calculation' 	
		INNER JOIN data_source_column dsc 
