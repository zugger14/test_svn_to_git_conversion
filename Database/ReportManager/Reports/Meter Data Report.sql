...
IF EXISTS (SELECT 1 FROM dbo.report WHERE name='Meter Data Report')
		BEGIN
		EXEC spa_rfx_report 'd', NULL,19691, NULL, NULL, NULL, NULL, NULL
		END
		...
		FROM report r
		WHERE r.[name] = 'Meter Data Report'
		...
		* */
		...
		BEGIN
			...
			SELECT TOP 1 'Meter Data Report' [name], 'farrms_admin' [owner], 1 is_system, 1 is_excel,'9227AE04_2A51_4E8B_92D9_4FA6F14A4699' report_hash, 'Meter Data Report' [description], CAST(sdv_cat.value_id AS VARCHAR(10)) category_id
			FROM sys.objects o
...
COMMIT TRAN
	...
	IF OBJECT_ID('tempdb..#data_source_column', 'U') IS NOT NULL
		...
		IF NOT EXISTS(SELECT 1 FROM report_dataset rd WHERE rd.report_id = @report_id_dest AND rd.[alias] =  'MDDV1')
		BEGIN
	...
	BEGIN
		...
		SELECT CASE WHEN 'e ' = 'p' 
					Then 19691 
					ELSE @report_id_dest 
		       END  AS report_id, 'Meter Data Report' [name], '9227AE04_2A51_4E8B_92D9_4FA6F14A4699' report_hash, 12 width,11.69 height
	END 
	

		INSERT INTO report_paramset(page_id, [name], paramset_hash, report_status_id)
		SELECT TOP 1 rpage.report_page_id, 'Meter Data Report', '6B8EA3F9_8048_4B9D_BCBC_2DE67375F678', 3
		FROM sys.objects o
