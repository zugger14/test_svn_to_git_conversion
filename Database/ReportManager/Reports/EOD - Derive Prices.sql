BEGIN TRY
		...
		IF EXISTS (SELECT 1 FROM dbo.report WHERE report_hash='540E1CB8_F744_4EFF_9C41_F02B3AF86FAD')
		BEGIN
			declare @report_id_to_delete int
			select @report_id_to_delete = report_id from report where report_hash = '540E1CB8_F744_4EFF_9C41_F02B3AF86FAD'
		...
		set @report_copy_name = isnull(@report_copy_name, 'Copy of ' + 'EOD - Derive Prices')
		

		INSERT INTO report ([name], [owner], is_system, is_excel, is_mobile, report_hash, [description], category_id)
		SELECT TOP 1 'EOD - Derive Prices' [name], 'dev_admin' [owner], 1 is_system, 0 is_excel, 0 is_mobile, '540E1CB8_F744_4EFF_9C41_F02B3AF86FAD' report_hash, '' [description], CAST(sdv_cat.value_id AS VARCHAR(10)) category_id
		FROM sys.objects o
...
BEGIN TRY
	...
	declare @new_ds_alias varchar(10) = 'dpsv'
	/** IF DATA SOURCE ALIAS ALREADY EXISTS ON DESTINATION, RAISE ERROR **/
	if exists(select top 1 1 from data_source where alias = 'dpsv' and name <> 'Derive Prices Sql View')
	begin
		select top 1 @new_ds_alias = 'dpsv' + cast(s.n as varchar(5))
		from seq s
		left join data_source ds on ds.alias = 'dpsv' + cast(s.n as varchar(5))
	...
	FROM report r
	WHERE r.[name] = 'EOD - Derive Prices'

	IF NOT EXISTS (SELECT 1 
	           FROM data_source 
	           WHERE [name] = 'Derive Prices Sql View'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
	AND NOT EXISTS (SELECT 1 FROM map_function_category WHERE [function_name] = 'Derive Prices Sql View' AND '106500' = '106501') 
	BEGIN
		INSERT INTO data_source([type_id], [name], [alias], [description], [tsql], report_id, system_defined,category)
		SELECT TOP 1 2 AS [type_id], 'Derive Prices Sql View' AS [name], @new_ds_alias AS ALIAS, NULL AS [description],null AS [tsql], @report_id_data_source_dest AS report_id,NULL AS [system_defined]
			,'106500' AS [category]
	END
...
DECLARE @_curve_ids VARCHAR(MAX) = NULL 
DECLARE @_tenor_from VARCHAR(10) = NULL
DECLARE @_tenor_to VARCHAR(10) = NULL
DECLARE @_process_id VARCHAR(100) = ''@process_id''
DECLARE @_batch_param VARCHAR(MAX)
...
IF ''@curve_ids'' <> ''NULL''
    SET @_curve_ids = ''@curve_ids''
ELSE 
	SELECT @_curve_ids = COALESCE(@_curve_ids + '','', '''') + CAST(source_curve_def_id AS VARCHAR(100))
	FROM source_price_curve_def
	WHERE formula_id IS NOT NULL AND granularity = 980

--SELECT @_tenor_from = CONVERT(VARCHAR(10), DATEADD(DAY, 1, @_as_of_date), 120)
--SELECT @_tenor_to = CONVERT(VARCHAR(10), CAST(YEAR(DATEADD(YEAR, 5, @_as_of_date)) AS VARCHAR(4)) + ''-12-31 23:00:00.000'' , 120)

SELECT @_tenor_from = CONVERT(VARCHAR(10), DATEADD(MONTH, 1 , DATEADD(month, DATEDIFF(MONTH, 0,  @_as_of_date), 0)), 120)
SELECT @_tenor_to = CONVERT(VARCHAR(10),DATEADD(MONTH, 24, @_tenor_from)  , 120)

SET @_batch_param = ''spa_save_derived_curve_value ''''c'''', ''''''+@_curve_ids+'''''', 77, ''''''+@_as_of_date+'''''', ''''''+@_as_of_date+'''''',''''4500'''',''''''+@_tenor_from+'''''' ,''''''+@_tenor_to+''''''''
...
EXEC spa_save_derived_curve_value ''c'', @_curve_ids , 77, @_as_of_date , @_as_of_date, ''4500'' , @_tenor_from ,  @_tenor_to, @_process_id, @_batch_param 

SELECT  
    @_as_of_date as_of_date,
    @_curve_ids curve_ids,
    @_as_of_date from_date,
    @_as_of_date to_date,
...
', report_id = @report_id_data_source_dest,
	...
	IF EXISTS (SELECT 1 
	           ...
	           WHERE ds.[name] = 'Derive Prices Sql View'
	            AND dsc.name =  'Area'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
	BEGIN
		UPDATE dsc  
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'from_date' AS [name], 'From Date' AS ALIAS, NULL AS reqd_param, 6 AS widget_id, 5 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,0 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Derive Prices Sql View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'tenor_from' AS [name], 'Tenor From' AS ALIAS, NULL AS reqd_param, 6 AS widget_id, 5 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,0 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Derive Prices Sql View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'tenor_to' AS [name], 'Tenor To' AS ALIAS, NULL AS reqd_param, 6 AS widget_id, 5 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,0 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Derive Prices Sql View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'to_date' AS [name], 'To Date' AS ALIAS, NULL AS reqd_param, 6 AS widget_id, 5 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,0 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Derive Prices Sql View'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	FROM data_source_column dsc 
	INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
		AND ds.[name] = 'Derive Prices Sql View'
		AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1)
...
COMMIT TRAN
	...
	IF OBJECT_ID('tempdb..#data_source_column', 'U') IS NOT NULL
		...
		SELECT TOP 1 ds.data_source_id AS source_id, @report_id_dest AS report_id, 'dpsv' [alias], rd_root.report_dataset_id AS root_dataset_id,0 AS is_free_from, 'NULL' AS relationship_sql
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Derive Prices Sql View'
	...
	SELECT @report_id_dest AS report_id, 'EOD - Derive Prices' [name], '540E1CB8_F744_4EFF_9C41_F02B3AF86FAD' report_hash, 11.5 width,5.5 height
		...
		SELECT TOP 1 rpage.report_page_id, 'EOD - Derive Prices', '48DC9010_F4A9_45D4_A544_3C3AF7AE172C', 1,'','','.xlsx',',', 
		-100000,'n','n',NULL	
		FROM sys.objects o
		INNER JOIN report_page rpage 
			on rpage.[name] = 'EOD - Derive Prices'
		INNER JOIN report r 
		ON r.report_id = rpage.report_id
			AND r.[name] = 'EOD - Derive Prices'
	

		INSERT INTO report_dataset_paramset(paramset_id, root_dataset_id, where_part, advance_mode)
		SELECT TOP 1 rp.report_paramset_id AS paramset_id, rd.report_dataset_id AS root_dataset_id, '(  dpsv.[Area] = ''@Area'')' AS where_part, 0
		FROM sys.objects o
		INNER JOIN report_paramset rp 
			ON rp.[name] = 'EOD - Derive Prices'
		INNER JOIN report_page rpage 
			ON rpage.report_page_id = rp.page_id
			AND rpage.[name] = 'EOD - Derive Prices'
		INNER JOIN report r 
			ON r.report_id = rpage.report_id
			AND r.[name] = 'EOD - Derive Prices'
		...
		SELECT TOP 1 rdp.report_dataset_paramset_id AS dataset_paramset_id, rd.report_dataset_id AS dataset_id , dsc.data_source_column_id AS column_id, 1 AS operator, '' AS initial_value, '' AS initial_value2, 1 AS optional, 1 AS hidden,0 AS logical_operator, 0 AS param_order, 0 AS param_depth, NULL AS label
		FROM sys.objects o
		INNER JOIN report_paramset rp 
			ON rp.[name] = 'EOD - Derive Prices'
		INNER JOIN report_page rpage 
		...
		SELECT TOP 1 rpage.report_page_id AS page_id, rd.report_dataset_id AS root_dataset_id, 'EOD _ Derive Prices_tablix' [name], '4.4' width, '3.04' height, '0' [top], '0' [left],2 AS group_mode,1 AS border_style,0 AS page_break,1 AS type_id,1 AS cross_summary,2 AS no_header,'' export_table_name, 0 AS is_global
		FROM sys.objects o
		INNER JOIN report_page rpage 
		ON rpage.[name] = 'EOD - Derive Prices'
		INNER JOIN report r 
		...
		INNER JOIN report_page_tablix rpt 
			ON rpt.[name] = 'EOD _ Derive Prices_tablix'
		INNER JOIN report_page rpage 
		...
		INNER JOIN report r 
			...
			AND r.[name] = 'EOD - Derive Prices'
		INNER JOIN report_dataset rd 
			ON rd.report_id = r.report_id AND rd.[alias] = 'dpsv' 	
		INNER JOIN data_source ds 
			ON ISNULL(NULLIF(ds.report_id, 0), r.report_id) = r.report_id	AND ds.[name] = 'Derive Prices Sql View' 	
		INNER JOIN data_source_column dsc 
