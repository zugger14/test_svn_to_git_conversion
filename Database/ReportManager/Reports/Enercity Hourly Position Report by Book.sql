BEGIN TRY
		...
		set @report_copy_name = isnull(@report_copy_name, 'Copy of ' + 'Enercity Hourly Position Report by Book')
		

		INSERT INTO report ([name], [owner], is_system, is_excel, is_mobile, report_hash, [description], category_id)
		SELECT TOP 1 'Enercity Hourly Position Report by Book' [name], 'dev_admin' [owner], 0 is_system, 1 is_excel, 0 is_mobile, 'F946B7DA_1F6D_4537_8A1F_80158670A56F' report_hash, 'Enercity Hourly Position Report by Book' [description], CAST(sdv_cat.value_id AS VARCHAR(10)) category_id
		FROM sys.objects o
	...
	SELECT @report_id_dest AS report_id, 'Enercity Hourly Position Report by Book' [name], 'F946B7DA_1F6D_4537_8A1F_80158670A56F' report_hash, 11.5 width,5.5 height
		...
		SELECT TOP 1 rpage.report_page_id, 'Enercity Hourly Position Report by Book', 'CFE2E1D0_453B_41AA_8569_C3F63D3E13E2', 3,'','','.xlsx',',', 
		-100000,'n','n',0	
		FROM sys.objects o
		INNER JOIN report_page rpage 
			on rpage.[name] = 'Enercity Hourly Position Report by Book'
		INNER JOIN report r 
		ON r.report_id = rpage.report_id
			AND r.[name] = 'Enercity Hourly Position Report by Book'
		...
		INNER JOIN report_paramset rp 
			ON rp.[name] = 'Enercity Hourly Position Report by Book'
		INNER JOIN report_page rpage 
			ON rpage.report_page_id = rp.page_id
			AND rpage.[name] = 'Enercity Hourly Position Report by Book'
		INNER JOIN report r 
			ON r.report_id = rpage.report_id
			AND r.[name] = 'Enercity Hourly Position Report by Book'
