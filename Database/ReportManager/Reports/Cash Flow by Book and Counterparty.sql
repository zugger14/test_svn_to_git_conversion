BEGIN TRY
		...
		BEGIN
			...
			SELECT TOP 1 'Cash Flow by Book and Counterparty' [name], 'farrms_admin' [owner], 0 is_system, 0 is_excel, 0 is_mobile, '06E5D6CF_8D5B_4C72_9FA0_5C4C96D587B1' report_hash, 'Cash Flow by Book and Counterparty' [description], CAST(sdv_cat.value_id AS VARCHAR(10)) category_id
			FROM sys.objects o
	...
	FROM report r
	WHERE r.[name] = 'Cash Flow by Book and Counterparty'
	...
	IF NOT EXISTS(SELECT 1 FROM report_page rp 
	              WHERE rp.report_id = CASE WHEN 'e ' = 'p' 
											...
											ELSE @report_id_dest 
						               END 
					AND rp.name =  'Cash Flow by Book and Counterparty'  
	)
	BEGIN
		...
		SELECT CASE WHEN 'e ' = 'p' 
					Then 36326 
					ELSE @report_id_dest 
		       END  AS report_id, 'Cash Flow by Book and Counterparty' [name], '06E5D6CF_8D5B_4C72_9FA0_5C4C96D587B1' report_hash, 11.5 width,5.5 height
	END 
		...
		SELECT TOP 1 rpage.report_page_id, 'Cash Flow by Book and Counterparty', '36DCBB26_C141_474E_908B_D8705A78CBAB', 2,'','','.xlsx',',', 
		-100000,'n','n'	
		FROM sys.objects o
		INNER JOIN report_page rpage 
			on rpage.[name] = 'Cash Flow by Book and Counterparty'
		INNER JOIN report r 
		ON r.report_id = rpage.report_id
			AND r.[name] = 'Cash Flow by Book and Counterparty'
		...
		INNER JOIN report_paramset rp 
			ON rp.[name] = 'Cash Flow by Book and Counterparty'
		INNER JOIN report_page rpage 
			ON rpage.report_page_id = rp.page_id
			AND rpage.[name] = 'Cash Flow by Book and Counterparty'
		INNER JOIN report r 
			ON r.report_id = rpage.report_id
			AND r.[name] = 'Cash Flow by Book and Counterparty'
		...
		INSERT INTO report_param(dataset_paramset_id, dataset_id, column_id, operator,
					initial_value, initial_value2, optional, hidden, logical_operator, param_order, param_depth, label)
		SELECT TOP 1 rdp.report_dataset_paramset_id AS dataset_paramset_id, rd.report_dataset_id AS dataset_id , dsc.data_source_column_id AS column_id, 1 AS operator, '' AS initial_value, '' AS initial_value2, 1 AS optional, 0 AS hidden,1 AS logical_operator, 20 AS param_order, 0 AS param_depth, 'Deal Sub Type' AS label
		FROM sys.objects o
		INNER JOIN report_paramset rp 
			ON rp.[name] = 'Cash Flow by Book and Counterparty'
		INNER JOIN report_page rpage 
		...
		INSERT INTO report_param(dataset_paramset_id, dataset_id, column_id, operator,
					initial_value, initial_value2, optional, hidden, logical_operator, param_order, param_depth, label)
		SELECT TOP 1 rdp.report_dataset_paramset_id AS dataset_paramset_id, rd.report_dataset_id AS dataset_id , dsc.data_source_column_id AS column_id, 9 AS operator, '' AS initial_value, '' AS initial_value2, 1 AS optional, 0 AS hidden,1 AS logical_operator, 26 AS param_order, 0 AS param_depth, 'Location' AS label
		FROM sys.objects o
		INNER JOIN report_paramset rp 
			ON rp.[name] = 'Cash Flow by Book and Counterparty'
		INNER JOIN report_page rpage 
		...
		SELECT TOP 1 rpt.report_page_tablix_id tablix_id, rd.report_dataset_id dataset_id, dsc.data_source_column_id column_id,3 placement, 0 column_order,NULL aggregation, NULL functions, 'Counterparty' [alias], 1 sortable, NULL rounding, NULL thousand_seperation, 'Tahoma' font, '8' font_size, '0,0,0' font_style, 'Left' text_align, '#000000' text_color, '#ffffff' background, NULL default_sort_order, NULL sort_direction, 0 custom_field, 0 render_as,-1 column_template,NULL negative_mark,NULL currency,NULL date_format,NULL cross_summary_aggregation,NULL mark_for_total,NULL sql_aggregation,NULL subtotal
