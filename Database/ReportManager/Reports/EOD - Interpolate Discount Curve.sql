...
IF EXISTS (SELECT 1 FROM dbo.report WHERE name='EOD - Interpolate Discount Curve')
		BEGIN
		EXEC spa_rfx_report 'd', NULL,53272, NULL, NULL, NULL, NULL, NULL
		END
		...
		FROM report r
		WHERE r.[name] = 'EOD - Interpolate Discount Curve'
		...
		* */
		...
		BEGIN
			...
			SELECT TOP 1 'EOD - Interpolate Discount Curve' [name], 'farrms_admin' [owner], 0 is_system, 0 is_excel, 0 is_mobile, '1B7B42C0_9C6F_4356_8AB3_F3AE18BD8DF9' report_hash, '' [description], CAST(sdv_cat.value_id AS VARCHAR(10)) category_id
			FROM sys.objects o
...
BEGIN TRY
	...
	IF EXISTS (SELECT 1 
	           FROM data_source 
	           WHERE [name] = 'Interpolate Discount Curve'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
			
	BEGIN
		UPDATE data_source
		SET alias = 'idc', description = NULL
		, [tsql] = CAST('' AS VARCHAR(MAX)) + 'DECLARE @_as_of_date VARCHAR(10) = ''@as_of_date'' 
DECLARE @_term_start VARCHAR(10) = NULL
DECLARE @_term_end VARCHAR(10) = NULL
DECLARE @_curve_id VARCHAR(MAX) =  ''@curve_id'',
		@_errorcode VARCHAR(20),
		@_module VARCHAR(200),
		@_area VARCHAR(200),
		@_status VARCHAR(20),
		@_message VARCHAR(500),
		@_recommendation VARCHAR(100)
...
SELECT @_term_start = CONVERT(VARCHAR(10),  @_as_of_date, 120) 
SELECT @_term_end =  CONVERT(VARCHAR(10), DATEADD(DAY, 10962, @_as_of_date), 120)	 

  
EXEC spa_curve_interpolation ''c'',  @_as_of_date, @_as_of_date,@_term_start, @_term_end , @_curve_id ,NULL,''y'',@_errorcode OUTPUT,  @_module OUTPUT , @_area OUTPUT ,  @_status OUTPUT,   @_message OUTPUT,  @_recommendation OUTPUT  
 
IF OBJECT_ID(''tempdb..#final_output'') IS NOT NULL DROP TABLE #final_output

SELECT  
    @_as_of_date as_of_date,
    @_as_of_date as_of_date_to,
...
--[__batch_report__]
FROM 
#final_output', report_id = @report_id_data_source_dest 
		WHERE [name] = 'Interpolate Discount Curve'
			AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1)
	END	
	...
	BEGIN
		...
		SELECT TOP 1 2 AS [type_id], 'Interpolate Discount Curve' AS [name], 'idc' AS ALIAS, NULL AS [description],'DECLARE @_as_of_date VARCHAR(10) = ''@as_of_date'' 
DECLARE @_term_start VARCHAR(10) = NULL
DECLARE @_term_end VARCHAR(10) = NULL
DECLARE @_curve_id VARCHAR(MAX) =  ''@curve_id'',
		@_errorcode VARCHAR(20),
...
--[__batch_report__]
FROM 
#final_output' AS [tsql], @report_id_data_source_dest AS report_id
	END 
	...
	IF EXISTS (SELECT 1 
	           FROM data_source_column dsc 
	           INNER JOIN data_source ds on ds.data_source_id = dsc.source_id 
	           WHERE ds.[name] = 'Interpolate Discount Curve'
	            AND dsc.name =  'Area'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
	BEGIN
		UPDATE dsc  
	...
	BEGIN
		...
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Interpolate Discount Curve'
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'as_of_date_to' AS [name], 'As Of Date To' AS ALIAS, NULL AS reqd_param, 6 AS widget_id, 5 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,0 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Interpolate Discount Curve'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'curve_id' AS [name], 'Curve ID' AS ALIAS, NULL AS reqd_param, 7 AS widget_id, 5 AS datatype_id, 'browse_curve' AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,0 AS column_template, 0 AS key_column, 0 AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Interpolate Discount Curve'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	FROM data_source_column dsc 
	INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
		AND ds.[name] = 'Interpolate Discount Curve'
		AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1)
...
COMMIT TRAN
	...
	IF OBJECT_ID('tempdb..#data_source_column', 'U') IS NOT NULL
		...
		IF NOT EXISTS(SELECT 1 FROM report_dataset rd WHERE rd.report_id = @report_id_dest AND rd.[alias] =  'idc')
		BEGIN
			INSERT INTO report_dataset (source_id, report_id, [alias], root_dataset_id, is_free_from, relationship_sql)
			SELECT TOP 1 ds.data_source_id AS source_id, @report_id_dest AS report_id, 'idc' [alias], rd_root.report_dataset_id AS root_dataset_id,0 AS is_free_from, 'NULL' AS relationship_sql
			FROM sys.objects o
			INNER JOIN data_source ds ON ds.[name] = 'Interpolate Discount Curve'
	...
	IF NOT EXISTS(SELECT 1 FROM report_page rp 
	              WHERE rp.report_id = CASE WHEN 'e ' = 'p' 
											...
											ELSE @report_id_dest 
						               END 
					AND rp.name =  'EOD - Interpolate Discount Curve'  
	)
	BEGIN
		...
		SELECT CASE WHEN 'e ' = 'p' 
					Then 53272 
					ELSE @report_id_dest 
		       END  AS report_id, 'EOD - Interpolate Discount Curve' [name], '1B7B42C0_9C6F_4356_8AB3_F3AE18BD8DF9' report_hash, 11.5 width,5.5 height
	END 
		...
		SELECT TOP 1 rpage.report_page_id, 'EOD - Interpolate Discount Curve', '6596587D_3E00_4805_A7CA_7EDDEA97FDD7', 1,'','','.xlsx',',', 
		-100000,'n','n'	
		FROM sys.objects o
		INNER JOIN report_page rpage 
			on rpage.[name] = 'EOD - Interpolate Discount Curve'
		INNER JOIN report r 
		ON r.report_id = rpage.report_id
			AND r.[name] = 'EOD - Interpolate Discount Curve'
		...
		INNER JOIN report_paramset rp 
			ON rp.[name] = 'EOD - Interpolate Discount Curve'
		INNER JOIN report_page rpage 
			ON rpage.report_page_id = rp.page_id
			AND rpage.[name] = 'EOD - Interpolate Discount Curve'
		INNER JOIN report r 
			ON r.report_id = rpage.report_id
			AND r.[name] = 'EOD - Interpolate Discount Curve'
		...
		SELECT TOP 1 rpage.report_page_id AS page_id, rd.report_dataset_id AS root_dataset_id, 'EOD _ Interpolate Discount Curve_tablix' [name], '4' width, '2.6666666666666665' height, '0' [top], '0' [left],2 AS group_mode,1 AS border_style,0 AS page_break,1 AS type_id,1 AS cross_summary,2 AS no_header,'' export_table_name, 0 AS is_global
		FROM sys.objects o
		INNER JOIN report_page rpage 
		ON rpage.[name] = 'EOD - Interpolate Discount Curve'
		INNER JOIN report r 
		...
		INNER JOIN report_page_tablix rpt 
			ON rpt.[name] = 'EOD _ Interpolate Discount Curve_tablix'
		INNER JOIN report_page rpage 
		...
		INNER JOIN report r 
			...
			AND r.[name] = 'EOD - Interpolate Discount Curve'
		INNER JOIN report_dataset rd 
			ON rd.report_id = r.report_id AND rd.[alias] = 'idc' 	
		INNER JOIN data_source ds 
			ON ISNULL(NULLIF(ds.report_id, 0), r.report_id) = r.report_id	AND ds.[name] = 'Interpolate Discount Curve' 	
		INNER JOIN data_source_column dsc 
