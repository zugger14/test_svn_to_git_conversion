BEGIN TRY
		...
		FROM report r
		WHERE r.[name] = 'Zai net Comparison Report'
		...
		* */
		...
		BEGIN
			...
			SELECT TOP 1 'Zai net Comparison Report' [name], 'farrms_admin' [owner], 0 is_system, 'F296755C_03E8_45A3_846C_056583798512' report_hash, 'Zai net Comparison Report' [description], CAST(sdv_cat.value_id AS VARCHAR(10)) category_id
			FROM sys.objects o
...
COMMIT TRAN
	...
	IF NOT EXISTS(SELECT 1 FROM report_page rp 
	              WHERE rp.report_id = CASE WHEN 'e ' = 'p' 
											...
											ELSE @report_id_dest 
						               END 
					AND rp.name =  'Zai net Comparison Report'  
	)
	BEGIN
		...
		SELECT CASE WHEN 'e ' = 'p' 
					Then 8016 
					ELSE @report_id_dest 
		       END  AS report_id, 'Zai net Comparison Report' [name], 'F296755C_03E8_45A3_846C_056583798512' report_hash, 12 width,9 height
	END 
	

		INSERT INTO report_paramset(page_id, [name], paramset_hash, report_status_id)
		SELECT TOP 1 rpage.report_page_id, 'Zai net Comparison Report', '97BD91D5_513E_4FF7_B58E_38351AD70630', 1
		FROM sys.objects o
		INNER JOIN report_page rpage 
			on rpage.[name] = 'Zai net Comparison Report'
		INNER JOIN report r 
		ON r.report_id = rpage.report_id
			AND r.[name] = 'Zai net Comparison Report'
		...
		INNER JOIN report_paramset rp 
			ON rp.[name] = 'Zai net Comparison Report'
		INNER JOIN report_page rpage 
			ON rpage.report_page_id = rp.page_id
			AND rpage.[name] = 'Zai net Comparison Report'
		INNER JOIN report r 
			ON r.report_id = rpage.report_id
			AND r.[name] = 'Zai net Comparison Report'
		...
		INNER JOIN report_page_tablix rpt 
			ON rpt.[name] = 'Zai net Comparison Report_page1_tab_0'
		INNER JOIN report_page rpage 
		...
		INSERT INTO report_tablix_column(tablix_id, dataset_id, column_id, placement, column_order, aggregation
					...
					, font_size, font_style, text_align, text_color, background, default_sort_order
					, default_sort_direction, custom_field, render_as, column_template, negative_mark, currency, date_format, cross_summary_aggregation, mark_for_total, sql_aggregation)
		SELECT TOP 1 rpt.report_page_tablix_id tablix_id, rd.report_dataset_id dataset_id, dsc.data_source_column_id column_id,1 placement, 2 column_order,13 aggregation, NULL functions, 'Amount' [alias], 1 sortable, 3 rounding, 0 thousand_seperation, 'Tahoma' font, '8' font_size, '0,0,0' font_style, 'Left' text_align, '#000000' text_color, '#ffffff' background, NULL default_sort_order, NULL sort_direction, 0 custom_field, 3 render_as,-1 column_template,0 negative_mark,0 currency,NULL date_format,13 cross_summary_aggregation,NULL mark_for_total,13 sql_aggregation
		FROM sys.objects o
		INNER JOIN report_page_tablix rpt 
			ON rpt.[name] = 'Zai net Comparison Report_page1_tab_0'
		INNER JOIN report_page rpage 
		...
		INSERT INTO report_tablix_column(tablix_id, dataset_id, column_id, placement, column_order, aggregation
					...
					, font_size, font_style, text_align, text_color, background, default_sort_order
					, default_sort_direction, custom_field, render_as, column_template, negative_mark, currency, date_format, cross_summary_aggregation, mark_for_total, sql_aggregation)
		SELECT TOP 1 rpt.report_page_tablix_id tablix_id, rd.report_dataset_id dataset_id, dsc.data_source_column_id column_id,4 placement, 3 column_order,NULL aggregation, 'dbo.fnadateformat(CONVERT(VARCHAR(7),DSV1.term_start,120)+''-01'')' functions, 'Month' [alias], 1 sortable, NULL rounding, NULL thousand_seperation, 'Tahoma' font, '8' font_size, '0,0,0' font_style, 'Left' text_align, '#000000' text_color, '#ffffff' background, NULL default_sort_order, NULL sort_direction, 0 custom_field, 0 render_as,-1 column_template,NULL negative_mark,NULL currency,NULL date_format,NULL cross_summary_aggregation,NULL mark_for_total,NULL sql_aggregation
		FROM sys.objects o
		INNER JOIN report_page_tablix rpt 
			ON rpt.[name] = 'Zai net Comparison Report_page1_tab_0'
		INNER JOIN report_page rpage 
