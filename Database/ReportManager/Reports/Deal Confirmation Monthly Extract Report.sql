BEGIN TRY
		...
		IF EXISTS (SELECT 1 FROM dbo.report WHERE report_hash='FE66C984_F7BE_4AAA_9643_433D22347FE0')
		BEGIN
			DECLARE @report_id_to_delete INT
			SELECT @report_id_to_delete = report_id FROM report WHERE report_hash = 'FE66C984_F7BE_4AAA_9643_433D22347FE0'
		...
		set @report_copy_name = isnull(@report_copy_name, 'Copy of ' + 'Deal Confirmation Monthly Extract Report')
		
		INSERT INTO report ([name], [owner], is_system, is_excel, is_mobile, report_hash, [description], category_id)
		SELECT TOP 1 'Deal Confirmation Monthly Extract Report' [name], 'dev_admin' [owner], 0 is_system, 1 is_excel, 0 is_mobile, 'FE66C984_F7BE_4AAA_9643_433D22347FE0' report_hash, 'Deal Confirmation Monthly Extract Report' [description], CAST(sdv_cat.value_id AS VARCHAR(10)) category_id
		FROM sys.objects o
	...
	declare @new_ds_alias varchar(10) = 'DCMER'
	/** IF DATA SOURCE ALIAS ALREADY EXISTS ON DESTINATION, RAISE ERROR **/
	if exists(select top 1 1 from data_source where alias = 'DCMER' and name <> 'Deal Confirmation Monthly Extract Report')
	begin
		select top 1 @new_ds_alias = 'DCMER' + cast(s.n as varchar(5))
		from seq s
		left join data_source ds on ds.alias = 'DCMER' + cast(s.n as varchar(5))
	...
	FROM report r
	WHERE r.[name] = 'Deal Confirmation Monthly Extract Report'
	IF NOT EXISTS (SELECT 1 
	           FROM data_source 
	           WHERE [name] = 'Deal Confirmation Monthly Extract Report'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
	AND NOT EXISTS (SELECT 1 FROM map_function_category WHERE [function_name] = 'Deal Confirmation Monthly Extract Report' AND '106500' = '106501') 
	BEGIN
		INSERT INTO data_source([type_id], [name], [alias], [description], [tsql], report_id, system_defined,category)
		SELECT TOP 1 2 AS [type_id], 'Deal Confirmation Monthly Extract Report' AS [name], @new_ds_alias AS ALIAS, NULL AS [description],null AS [tsql], @report_id_data_source_dest AS report_id,NULL AS [system_defined]
			,'106500' AS [category]
	END
...
BEGIN
	SET @_sql1 = ''
			...
			,CASE 
				...
				CAST(sdh.deal_date AS DATE) [Trade Date],
				...
				sdd.term_start [Start Date],
				sdd.term_end [End Date],
				CASE 
					WHEN sdd.deal_volume_frequency =''''t'''' 
						THEN CAST(dbo.FNARemoveTrailingZeroes(ISNULL(CASE WHEN ISNULL(sddh.volume, 0.0) = 0.0 THEN NULL ELSE ISNULL(sddh.volume, 0.0) END, 0.0)) as VARCHAR) + '''' '''' +  MAX(su.uom_name) + '''' per term'''' 
					ELSE 
						CASE 
							WHEN CAST(sdd.deal_volume AS decimal(10,2)) IS NULL THEN CAST(0 AS VARCHAR(10)) + '''' '''' + MAX(su.uom_name) + '''' per '''' + dbo.FNAGetFrequencyText(MAX(sdd.deal_volume_frequency), ''''v'''') 
							ELSE CAST(dbo.FNANumberFormat(CAST(sdd.deal_volume AS decimal(10,2)),''''n'''') AS VARCHAR(1000)) + '''' '''' + MAX(su.uom_name) + '''' per '''' + dbo.FNAGetFrequencyText(MAX(sdd.deal_volume_frequency),''''v'''') 
						END 
				END [Quantity] ,
				sdd.total_volume [Total Quantity],			
				ISNULL(MAX(spcd.curve_name), MAX(risk_spcd.curve_name)) AS [Price Index],
				''''1st day of the month'''' [Pricing Date],
				CASE WHEN sdd.price_adder IS NOT NULL
...
END', report_id = @report_id_data_source_dest,
	...
	IF EXISTS (SELECT 1 
	           ...
	           WHERE ds.[name] = 'Deal Confirmation Monthly Extract Report'
	...
	BEGIN
		...
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Deal Confirmation Monthly Extract Report'
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'From Shipper Code' AS [name], 'From Shipper Code' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 5 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,0 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Deal Confirmation Monthly Extract Report'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column, required_filter)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'To Shipper Code' AS [name], 'To Shipper Code' AS ALIAS, NULL AS reqd_param, 1 AS widget_id, 5 AS datatype_id, NULL AS param_data_source, NULL AS param_default_value, NULL AS append_filter, NULL  AS tooltip,0 AS column_template, 0 AS key_column, NULL AS required_filter				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Deal Confirmation Monthly Extract Report'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	FROM data_source_column dsc 
	INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
		AND ds.[name] = 'Deal Confirmation Monthly Extract Report'
		AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1)
	...
	IF OBJECT_ID('tempdb..#data_source_column', 'U') IS NOT NULL
		...
		SELECT TOP 1 ds.data_source_id AS source_id, @report_id_dest AS report_id, 'DCMER' [alias], rd_root.report_dataset_id AS root_dataset_id,0 AS is_free_from, 'NULL' AS relationship_sql
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'Deal Confirmation Monthly Extract Report'
	...
	SELECT @report_id_dest AS report_id, 'Deal Confirmation Monthly Extract Report' [name], 'FE66C984_F7BE_4AAA_9643_433D22347FE0' report_hash, 8.5 width,11 height
		...
		SELECT TOP 1 rpage.report_page_id, 'Deal Confirmation Monthly Extract Report', '8CB802B7_1074_4C2D_965A_419817AF6047', 3,'','','.xlsx',',', 
		-100003,'y','n',0	
		FROM sys.objects o
		INNER JOIN report_page rpage 
			on rpage.[name] = 'Deal Confirmation Monthly Extract Report'
		INNER JOIN report r 
		ON r.report_id = rpage.report_id
			AND r.[name] = 'Deal Confirmation Monthly Extract Report'
		...
		INNER JOIN report_paramset rp 
			ON rp.[name] = 'Deal Confirmation Monthly Extract Report'
		INNER JOIN report_page rpage 
			ON rpage.report_page_id = rp.page_id
			AND rpage.[name] = 'Deal Confirmation Monthly Extract Report'
		INNER JOIN report r 
			ON r.report_id = rpage.report_id
			AND r.[name] = 'Deal Confirmation Monthly Extract Report'
		...
		SELECT TOP 1 rpage.report_page_id AS page_id, rd.report_dataset_id AS root_dataset_id, 'Deal Confirmation Monthly Extract Report_tablix' [name], '7.6' width, '2.6666666666666665' height, '0' [top], '0' [left],2 AS group_mode,1 AS border_style,0 AS page_break,1 AS type_id,1 AS cross_summary,2 AS no_header,'' export_table_name, 0 AS is_global
		FROM sys.objects o
		INNER JOIN report_page rpage 
		ON rpage.[name] = 'Deal Confirmation Monthly Extract Report'
		INNER JOIN report r 
		...
		INNER JOIN report_page_tablix rpt 
			ON rpt.[name] = 'Deal Confirmation Monthly Extract Report_tablix'
		INNER JOIN report_page rpage 
		...
		INNER JOIN report r 
			...
			AND r.[name] = 'Deal Confirmation Monthly Extract Report'
		INNER JOIN report_dataset rd 
			ON rd.report_id = r.report_id AND rd.[alias] = 'DCMER' 	
		INNER JOIN data_source ds 
			ON ISNULL(NULLIF(ds.report_id, 0), r.report_id) = r.report_id	AND ds.[name] = 'Deal Confirmation Monthly Extract Report' 	
		INNER JOIN data_source_column dsc 
		...
		INSERT INTO report_tablix_column(tablix_id, dataset_id, column_id, placement, column_order, aggregation
					...
					, font_size, font_style, text_align, text_color, background, default_sort_order
					, default_sort_direction, custom_field, render_as, column_template, negative_mark, currency, date_format, cross_summary_aggregation, mark_for_total, sql_aggregation, subtotal)
		SELECT TOP 1 rpt.report_page_tablix_id tablix_id, rd.report_dataset_id dataset_id, dsc.data_source_column_id column_id,1 placement, 98 column_order,NULL aggregation, NULL functions, 'Counterparty City Buy' [alias], 1 sortable, NULL rounding, NULL thousand_seperation, 'Tahoma' font, '8' font_size, '0,0,0' font_style, 'Left' text_align, '#000000' text_color, '#ffffff' background, NULL default_sort_order, NULL sort_direction, 0 custom_field, 0 render_as,-1 column_template,NULL negative_mark,NULL currency,NULL date_format,-1 cross_summary_aggregation,NULL mark_for_total,NULL sql_aggregation,NULL subtotal
		...
		INSERT INTO report_tablix_column(tablix_id, dataset_id, column_id, placement, column_order, aggregation
					...
					, font_size, font_style, text_align, text_color, background, default_sort_order
					, default_sort_direction, custom_field, render_as, column_template, negative_mark, currency, date_format, cross_summary_aggregation, mark_for_total, sql_aggregation, subtotal)
		SELECT TOP 1 rpt.report_page_tablix_id tablix_id, rd.report_dataset_id dataset_id, dsc.data_source_column_id column_id,1 placement, 106 column_order,NULL aggregation, NULL functions, 'Counterparty Zip Buy' [alias], 1 sortable, NULL rounding, NULL thousand_seperation, 'Tahoma' font, '8' font_size, '0,0,0' font_style, 'Left' text_align, '#000000' text_color, '#ffffff' background, NULL default_sort_order, NULL sort_direction, 0 custom_field, 0 render_as,-1 column_template,NULL negative_mark,NULL currency,NULL date_format,-1 cross_summary_aggregation,NULL mark_for_total,NULL sql_aggregation,NULL subtotal
		...
		INNER JOIN data_source ds 
			ON ISNULL(NULLIF(ds.report_id, 0), r.report_id) = r.report_id	AND ds.[name] = 'Deal Confirmation Monthly Extract Report' 	
		INNER JOIN data_source_column dsc 
			ON dsc.source_id = ds.data_source_id AND dsc.[name] = 'address1_counterparty'  INSERT INTO report_tablix_header(tablix_id, column_id, font, font_size, font_style, text_align, text_color, background, report_tablix_column_id)
	  SELECT TOP 1 
	 ...
	 INSERT INTO report_tablix_header(tablix_id, column_id, font, font_size, font_style, text_align, text_color, background, report_tablix_column_id)
	  SELECT TOP 1 
		...
		INNER JOIN report_tablix_column rtc 
			on rtc.tablix_id = rpt.report_page_tablix_id
			--AND rtc.column_id = dsc.data_source_column_id  --This did not handle custom column, got duplicate custom columns during export
			AND rtc.alias = 'Counterparty City Buy' --Added to handle custom column. Assumption: alias is unique and NOT NULL
	 INSERT INTO report_tablix_header(tablix_id, column_id, font, font_size, font_style, text_align, text_color, background, report_tablix_column_id)
	  SELECT TOP 1 
			rpt.report_page_tablix_id tablix_id, dsc.data_source_column_id column_id,
			'Tahoma' font,
	 ...
	 INSERT INTO report_tablix_header(tablix_id, column_id, font, font_size, font_style, text_align, text_color, background, report_tablix_column_id)
	  SELECT TOP 1 
		...
		INNER JOIN report_tablix_column rtc 
			on rtc.tablix_id = rpt.report_page_tablix_id
			--AND rtc.column_id = dsc.data_source_column_id  --This did not handle custom column, got duplicate custom columns during export
			AND rtc.alias = 'Counterparty Zip Buy' --Added to handle custom column. Assumption: alias is unique and NOT NULL
	 INSERT INTO report_tablix_header(tablix_id, column_id, font, font_size, font_style, text_align, text_color, background, report_tablix_column_id)
	  SELECT TOP 1 
			rpt.report_page_tablix_id tablix_id, dsc.data_source_column_id column_id,
			'Tahoma' font,
