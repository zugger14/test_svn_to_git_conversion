BEGIN TRY
		...
		IF EXISTS (SELECT 1 FROM dbo.report WHERE report_hash='A66FD430_7FEC_4A98_88F9_37A2C11EC951')
		BEGIN
			declare @report_id_to_delete int
			select @report_id_to_delete = report_id from report where report_hash = 'A66FD430_7FEC_4A98_88F9_37A2C11EC951'
		...
		set @report_copy_name = isnull(@report_copy_name, 'Copy of ' + 'EOD - Book Balancing')
		
		INSERT INTO report ([name], [owner], is_system, is_excel, is_mobile, report_hash, [description], category_id)
		SELECT TOP 1 'EOD - Book Balancing' [name], 'dev_admin' [owner], 1 is_system, 0 is_excel, 0 is_mobile, 'A66FD430_7FEC_4A98_88F9_37A2C11EC951' report_hash, 'EOD - Book Balancing' [description], CAST(sdv_cat.value_id AS VARCHAR(10)) category_id
		FROM sys.objects o
	...
	FROM report r
	WHERE r.[name] = 'EOD - Book Balancing'
...
DECLARE @_process_id VARCHAR(100) 
...
CROSS APPLY(SELECT location_id,term_date,hr,volume 

	FROM #temp_hourly_data_pre b 
...
SELECT ''Success''
	...
	, ''Book balancing completed successfully.''
...
SELECT  
	...
	@_source_subbook_id source_subbook_id,
...
', report_id = @report_id_data_source_dest,
	...
	SELECT @report_id_dest AS report_id, 'EOD - Book Balancing' [name], 'A66FD430_7FEC_4A98_88F9_37A2C11EC951' report_hash, 11.5 width,5.5 height
		...
		SELECT TOP 1 rpage.report_page_id, 'EOD - Book Balancing', '0F5DB095_A433_4405_867C_0DC21D4F0236', 2,'EOD - Book Balancing','','.xlsx',',', 
		-100000,'y','n',0	
		FROM sys.objects o
		INNER JOIN report_page rpage 
			on rpage.[name] = 'EOD - Book Balancing'
		INNER JOIN report r 
		ON r.report_id = rpage.report_id
			AND r.[name] = 'EOD - Book Balancing'
		...
		INNER JOIN report_paramset rp 
			ON rp.[name] = 'EOD - Book Balancing'
		INNER JOIN report_page rpage 
			ON rpage.report_page_id = rp.page_id
			AND rpage.[name] = 'EOD - Book Balancing'
		INNER JOIN report r 
			ON r.report_id = rpage.report_id
			AND r.[name] = 'EOD - Book Balancing'
		...
		INSERT INTO report_param(dataset_paramset_id, dataset_id, column_id, operator,
					initial_value, initial_value2, optional, hidden, logical_operator, param_order, param_depth, label)
		SELECT TOP 1 rdp.report_dataset_paramset_id AS dataset_paramset_id, rd.report_dataset_id AS dataset_id , dsc.data_source_column_id AS column_id, 1 AS operator, '' AS initial_value, '' AS initial_value2, 0 AS optional, 0 AS hidden,1 AS logical_operator, 6 AS param_order, 0 AS param_depth, 'Destination Ref ID' AS label
		FROM sys.objects o
		INNER JOIN report_paramset rp 
			ON rp.[name] = 'EOD - Book Balancing'
		INNER JOIN report_page rpage 
		...
		INSERT INTO report_param(dataset_paramset_id, dataset_id, column_id, operator,
					initial_value, initial_value2, optional, hidden, logical_operator, param_order, param_depth, label)
		SELECT TOP 1 rdp.report_dataset_paramset_id AS dataset_paramset_id, rd.report_dataset_id AS dataset_id , dsc.data_source_column_id AS column_id, 1 AS operator, '' AS initial_value, '' AS initial_value2, 1 AS optional, 0 AS hidden,1 AS logical_operator, 7 AS param_order, 0 AS param_depth, 'Process ID' AS label
		FROM sys.objects o
		INNER JOIN report_paramset rp 
			ON rp.[name] = 'EOD - Book Balancing'
		INNER JOIN report_page rpage 
		...
		INSERT INTO report_param(dataset_paramset_id, dataset_id, column_id, operator,
					initial_value, initial_value2, optional, hidden, logical_operator, param_order, param_depth, label)
		SELECT TOP 1 rdp.report_dataset_paramset_id AS dataset_paramset_id, rd.report_dataset_id AS dataset_id , dsc.data_source_column_id AS column_id, 9 AS operator, '' AS initial_value, '' AS initial_value2, 1 AS optional, 0 AS hidden,1 AS logical_operator, 2 AS param_order, 0 AS param_depth, 'Destination Subbook' AS label
		FROM sys.objects o
		INNER JOIN report_paramset rp 
			ON rp.[name] = 'EOD - Book Balancing'
		INNER JOIN report_page rpage 
		...
		INSERT INTO report_param(dataset_paramset_id, dataset_id, column_id, operator,
					initial_value, initial_value2, optional, hidden, logical_operator, param_order, param_depth, label)
		SELECT TOP 1 rdp.report_dataset_paramset_id AS dataset_paramset_id, rd.report_dataset_id AS dataset_id , dsc.data_source_column_id AS column_id, 9 AS operator, '' AS initial_value, '' AS initial_value2, 0 AS optional, 0 AS hidden,1 AS logical_operator, 5 AS param_order, 0 AS param_depth, 'Location' AS label
		FROM sys.objects o
		INNER JOIN report_paramset rp 
			ON rp.[name] = 'EOD - Book Balancing'
		INNER JOIN report_page rpage 
		...
		INSERT INTO report_param(dataset_paramset_id, dataset_id, column_id, operator,
					initial_value, initial_value2, optional, hidden, logical_operator, param_order, param_depth, label)
		SELECT TOP 1 rdp.report_dataset_paramset_id AS dataset_paramset_id, rd.report_dataset_id AS dataset_id , dsc.data_source_column_id AS column_id, 9 AS operator, '' AS initial_value, '' AS initial_value2, 0 AS optional, 0 AS hidden,0 AS logical_operator, 1 AS param_order, 0 AS param_depth, 'Source Subbook' AS label
		FROM sys.objects o
		INNER JOIN report_paramset rp 
			ON rp.[name] = 'EOD - Book Balancing'
		INNER JOIN report_page rpage 
