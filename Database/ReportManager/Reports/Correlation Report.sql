...
IF EXISTS (SELECT 1 FROM dbo.report WHERE name='Correlation Report')
		BEGIN
		EXEC spa_rfx_report 'd', NULL,37139, NULL, NULL, NULL, NULL, NULL
		END
		...
		FROM report r
		WHERE r.[name] = 'Correlation Report'
		...
		* */
		...
		BEGIN
			...
			SELECT TOP 1 'Correlation Report' [name], 'farrms_admin' [owner], 1 is_system, 1 is_excel, '738E75CC_D73F_40F6_AF95_1803E8F3B394' report_hash, 'Standard Correlation Report' [description], CAST(sdv_cat.value_id AS VARCHAR(10)) category_id
			FROM sys.objects o
...
COMMIT TRAN
	...
	IF NOT EXISTS(SELECT 1 FROM report_page rp 
	              WHERE rp.report_id = CASE WHEN 'e ' = 'p' 
											...
											ELSE @report_id_dest 
						               END 
					AND rp.name =  'Correlation Report'  
	)
	BEGIN
		...
		SELECT CASE WHEN 'e ' = 'p' 
					Then 37139 
					ELSE @report_id_dest 
		       END  AS report_id, 'Correlation Report' [name], '738E75CC_D73F_40F6_AF95_1803E8F3B394' report_hash, 11.5 width,5.5 height
	END 
	

		INSERT INTO report_paramset(page_id, [name], paramset_hash, report_status_id)
		SELECT TOP 1 rpage.report_page_id, 'Correlation Report', '9B6B2EA9_C9C7_407D_9F61_884ECAFABB32', 3
		FROM sys.objects o
		...
		SELECT TOP 1 rpage.report_page_id AS page_id, rd.report_dataset_id AS root_dataset_id, 'Correlation Report_tablix' [name], '5.92' width, '3.36' height, '0' [top], '0' [left],2 AS group_mode,1 AS border_style,0 AS page_break,2 AS type_id,1 AS cross_summary,2 AS no_header,'' export_table_name, 0 AS is_global
		FROM sys.objects o
		INNER JOIN report_page rpage 
		ON rpage.[name] = 'Correlation Report'
		INNER JOIN report r 
