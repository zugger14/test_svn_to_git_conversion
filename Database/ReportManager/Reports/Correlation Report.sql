BEGIN TRY
		...
		IF EXISTS (SELECT 1 FROM dbo.report WHERE report_hash='738E75CC_D73F_40F6_AF95_1803E8F3B394')
		BEGIN
			declare @report_id_to_delete int
			select @report_id_to_delete = report_id from report where report_hash = '738E75CC_D73F_40F6_AF95_1803E8F3B394'
		...
		set @report_copy_name = isnull(@report_copy_name, 'Copy of ' + 'Correlation report')
		

		INSERT INTO report ([name], [owner], is_system, is_excel, is_mobile, report_hash, [description], category_id)
		SELECT TOP 1 'Correlation Report' [name], 'dev_admin' [owner], 1 is_system, 1 is_excel, 0 is_mobile, '738E75CC_D73F_40F6_AF95_1803E8F3B394' report_hash, 'Standard Correlation Report' [description], CAST(sdv_cat.value_id AS VARCHAR(10)) category_id
		FROM sys.objects o
	...
	SELECT @report_id_dest AS report_id, 'Correlation Report' [name], '738E75CC_D73F_40F6_AF95_1803E8F3B394' report_hash, 11.5 width,5.5 height
		...
		SELECT TOP 1 rpage.report_page_id, 'Correlation Report', '9B6B2EA9_C9C7_407D_9F61_884ECAFABB32', 3,NULL,NULL,NULL,NULL, 
		NULL,'n','n',NULL	
		FROM sys.objects o
		INNER JOIN report_page rpage 
			on rpage.[name] = 'Correlation Report'
		INNER JOIN report r 
		ON r.report_id = rpage.report_id
			AND r.[name] = 'Correlation Report'
		...
		INNER JOIN report_paramset rp 
			ON rp.[name] = 'Correlation Report'
		INNER JOIN report_page rpage 
			ON rpage.report_page_id = rp.page_id
			AND rpage.[name] = 'Correlation Report'
		INNER JOIN report r 
			ON r.report_id = rpage.report_id
			AND r.[name] = 'Correlation Report'
		...
		SELECT TOP 1 rpage.report_page_id AS page_id, rd.report_dataset_id AS root_dataset_id, 'Correlation Report_tablix' [name], '6.906666666666666' width, '3.92' height, '0' [top], '0' [left],2 AS group_mode,1 AS border_style,0 AS page_break,2 AS type_id,1 AS cross_summary,2 AS no_header,'' export_table_name, 0 AS is_global
		FROM sys.objects o
		INNER JOIN report_page rpage 
		ON rpage.[name] = 'Correlation Report'
		INNER JOIN report r 
