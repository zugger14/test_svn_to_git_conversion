...
IF EXISTS (SELECT 1 FROM dbo.report WHERE name='EOD - Check Position')
		BEGIN
		EXEC spa_rfx_report 'd', NULL,22924, NULL, NULL, NULL, NULL, NULL
		END
		...
		* */
		...
		BEGIN
			...
			SELECT TOP 1 'EOD - Check Position' [name], 'farrms_admin' [owner], 0 is_system, 0 is_excel, 0 is_mobile, '3E7AD0C0_55DA_4CE1_8E80_FCEB42EC3346' report_hash, '' [description], CAST(sdv_cat.value_id AS VARCHAR(10)) category_id
			FROM sys.objects o
...
BEGIN TRY
	...
	IF EXISTS (SELECT 1 
	           FROM data_source 
	           WHERE [name] = 'EOD - Check Position'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
			
	BEGIN
		UPDATE data_source
		SET alias = 'ecp', description = NULL
...
EXEC spa_pnm_eod_calc_pos_for_deals_no_position @_process_id
...
', report_id = @report_id_data_source_dest 
	...
	BEGIN
		...
		SELECT TOP 1 2 AS [type_id], 'EOD - Check Position' AS [name], 'ecp' AS ALIAS, NULL AS [description],'DECLARE @_as_of_date VARCHAR(10)  = ''@as_of_date'',
		@_process_id VARCHAR(100) = ''@process_id''  
...
' AS [tsql], @report_id_data_source_dest AS report_id
	...
	IF EXISTS (SELECT 1 
	           FROM data_source_column dsc 
	           INNER JOIN data_source ds on ds.data_source_id = dsc.source_id 
	           WHERE ds.[name] = 'EOD - Check Position'
	            AND dsc.name =  'Area'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
	BEGIN
		UPDATE dsc  
	...
	BEGIN
		...
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'EOD - Check Position'
	...
	FROM data_source_column dsc 
	INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
		AND ds.[name] = 'EOD - Check Position'
		AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1)
...
COMMIT TRAN
	...
	IF OBJECT_ID('tempdb..#data_source_column', 'U') IS NOT NULL
		...
		IF NOT EXISTS(SELECT 1 FROM report_dataset rd WHERE rd.report_id = @report_id_dest AND rd.[alias] =  'ecp')
		BEGIN
			INSERT INTO report_dataset (source_id, report_id, [alias], root_dataset_id, is_free_from, relationship_sql)
			SELECT TOP 1 ds.data_source_id AS source_id, @report_id_dest AS report_id, 'ecp' [alias], rd_root.report_dataset_id AS root_dataset_id,0 AS is_free_from, 'NULL' AS relationship_sql
			FROM sys.objects o
			INNER JOIN data_source ds ON ds.[name] = 'EOD - Check Position'
	...
	IF NOT EXISTS(SELECT 1 FROM report_page rp 
	              WHERE rp.report_id = CASE WHEN 'e ' = 'p' 
											...
											ELSE @report_id_dest 
						               END 
					AND rp.name =  'EOD - Check Position'  
	)
	BEGIN
		...
		SELECT CASE WHEN 'e ' = 'p' 
					Then 22924 
					ELSE @report_id_dest 
		       END  AS report_id, 'EOD - Check Position' [name], '3E7AD0C0_55DA_4CE1_8E80_FCEB42EC3346' report_hash, 11.5 width,5.5 height
	END 
		...
		SELECT TOP 1 rpage.report_page_id, 'EOD - Check Position', '2222310A_700C_416F_82D2_535698026724', 1,'','','.xlsx',',', 
		-100000,'n','n'	
		FROM sys.objects o
		...
		SELECT TOP 1 rpage.report_page_id AS page_id, rd.report_dataset_id AS root_dataset_id, 'EOD _ Check Position_tablix' [name], '4' width, '2.6666666666666665' height, '0' [top], '0' [left],2 AS group_mode,1 AS border_style,0 AS page_break,1 AS type_id,1 AS cross_summary,2 AS no_header,'' export_table_name, 0 AS is_global
		FROM sys.objects o
		INNER JOIN report_page rpage 
		ON rpage.[name] = 'EOD - Check Position'
		INNER JOIN report r 
		...
		INNER JOIN report r 
			...
			AND r.[name] = 'EOD - Check Position'
		INNER JOIN report_dataset rd 
			ON rd.report_id = r.report_id AND rd.[alias] = 'ecp' 	
		INNER JOIN data_source ds 
			ON ISNULL(NULLIF(ds.report_id, 0), r.report_id) = r.report_id	AND ds.[name] = 'EOD - Check Position' 	
		INNER JOIN data_source_column dsc 
