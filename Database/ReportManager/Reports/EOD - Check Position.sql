BEGIN TRY
		...
		IF EXISTS (SELECT 1 FROM dbo.report WHERE report_hash='7CD61880_7EB3_44D9_99C1_2CD724E1CF76')
		BEGIN
			declare @report_id_to_delete int
			select @report_id_to_delete = report_id from report where report_hash = '7CD61880_7EB3_44D9_99C1_2CD724E1CF76'
		...
		set @report_copy_name = isnull(@report_copy_name, 'Copy of ' + 'EOD - Check Position')
		

		INSERT INTO report ([name], [owner], is_system, is_excel, is_mobile, report_hash, [description], category_id)
		SELECT TOP 1 'EOD - Check Position' [name], 'dev_admin' [owner], 0 is_system, 0 is_excel, 0 is_mobile, '7CD61880_7EB3_44D9_99C1_2CD724E1CF76' report_hash, 'EOD - Check Position' [description], CAST(sdv_cat.value_id AS VARCHAR(10)) category_id
		FROM sys.objects o
...
BEGIN TRY
	...
	declare @new_ds_alias varchar(10) = 'cp'
	/** IF DATA SOURCE ALIAS ALREADY EXISTS ON DESTINATION, RAISE ERROR **/
	if exists(select top 1 1 from data_source where alias = 'cp' and name <> 'check position')
	begin
		select top 1 @new_ds_alias = 'cp' + cast(s.n as varchar(5))
		from seq s
		left join data_source ds on ds.alias = 'cp' + cast(s.n as varchar(5))
	...
	FROM report r
	WHERE r.[name] = 'EOD - Check Position'

	IF NOT EXISTS (SELECT 1 
	           FROM data_source 
	           WHERE [name] = 'check position'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
	AND NOT EXISTS (SELECT 1 FROM map_function_category WHERE [function_name] = 'check position' AND '106500' = '106501') 
	BEGIN
		INSERT INTO data_source([type_id], [name], [alias], [description], [tsql], report_id, system_defined,category)
		SELECT TOP 1 2 AS [type_id], 'check position' AS [name], @new_ds_alias AS ALIAS, NULL AS [description],null AS [tsql], @report_id_data_source_dest AS report_id,NULL AS [system_defined]
			,'106500' AS [category]
	END
...
BEGIN TRY
    ...
    SET @_sql = N''
        ...
        LEFT JOIN
        (
            SELECT DISTINCT source_deal_header_id FROM report_hourly_position_deal rhpd
            UNION ALL
            SELECT DISTINCT source_deal_header_id FROM report_hourly_position_profile rhpd
            UNION ALL
            SELECT DISTINCT source_deal_header_id FROM report_hourly_position_breakdown rhpd
        ) rhpd
         ON rhpd.source_deal_header_id = sdh.source_deal_header_id
...
WHERE 1=1', report_id = @report_id_data_source_dest,
	...
	BEGIN
		...
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'check position'
...
COMMIT TRAN
	...
	IF OBJECT_ID('tempdb..#data_source_column', 'U') IS NOT NULL
		...
		SELECT TOP 1 ds.data_source_id AS source_id, @report_id_dest AS report_id, 'cp' [alias], rd_root.report_dataset_id AS root_dataset_id,0 AS is_free_from, 'NULL' AS relationship_sql
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'check position'
	...
	SELECT @report_id_dest AS report_id, 'EOD - Check Position' [name], '7CD61880_7EB3_44D9_99C1_2CD724E1CF76' report_hash, 11.5 width,5.5 height
		...
		SELECT TOP 1 rpage.report_page_id, 'EOD - Check Position', 'CE87D57F_F07B_45E9_910A_9010F8CD0BBF', 1,'','','.xlsx',',', 
		-100000,'n','n',NULL	
		FROM sys.objects o
		INNER JOIN report_page rpage 
			on rpage.[name] = 'EOD - Check Position'
		INNER JOIN report r 
		ON r.report_id = rpage.report_id
			AND r.[name] = 'EOD - Check Position'
		...
		INNER JOIN report_paramset rp 
			ON rp.[name] = 'EOD - Check Position'
		INNER JOIN report_page rpage 
			ON rpage.report_page_id = rp.page_id
			AND rpage.[name] = 'EOD - Check Position'
		INNER JOIN report r 
			ON r.report_id = rpage.report_id
			AND r.[name] = 'EOD - Check Position'
		...
		INNER JOIN report r 
			...
			AND r.[name] = 'EOD - Check Position'
		INNER JOIN report_dataset rd 
			ON rd.report_id = r.report_id AND rd.[alias] = 'cp' 	
		INNER JOIN data_source ds 
			ON ISNULL(NULLIF(ds.report_id, 0), r.report_id) = r.report_id	AND ds.[name] = 'check position' 	
		INNER JOIN data_source_column dsc 
