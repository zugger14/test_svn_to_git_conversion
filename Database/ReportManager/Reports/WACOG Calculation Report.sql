...
IF EXISTS (SELECT 1 FROM dbo.report WHERE name='WACOG Calculation Report')
		BEGIN
		EXEC spa_rfx_report 'd', NULL,28399, NULL, NULL, NULL, NULL, NULL
		END
		...
		FROM report r
		WHERE r.[name] = 'WACOG Calculation Report'
		...
		* */
		...
		BEGIN
			...
			SELECT TOP 1 'WACOG Calculation Report' [name], 'farrms_admin' [owner], 0 is_system, 0 is_excel, 0 is_mobile, '8591D75C_0078_4796_AFCF_B1B6D78DB18E' report_hash, '' [description], CAST(sdv_cat.value_id AS VARCHAR(10)) category_id
			FROM sys.objects o
...
BEGIN TRY
	...
	IF EXISTS (SELECT 1 
	           FROM data_source 
	           WHERE [name] = 'WACOQ Sql'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
			
	BEGIN
		UPDATE data_source
		SET alias = 'wacog', description = NULL
		, [tsql] = CAST('' AS VARCHAR(MAX)) + 'DECLARE @_as_of_date DATETIME
	  , @_wacog_group_id INT
...
IF ''@wacog_group_id'' <> NULL
	SET @_wacog_group_id = ''@wacog_group_id''
...
INNER JOIN wacog_group AS wg
ON wg.wacog_group_id = cwg.wacog_group_id
INNER JOIN ( SELECT cwg.wacog_group_id
				 , MAX(cwg.create_ts) create_ts
			FROM calculate_wacog_group AS cwg
			GROUP BY cwg.wacog_group_id
) sub_wg
ON sub_wg.wacog_group_id = cwg.wacog_group_id AND cwg.create_ts = sub_wg.create_ts
WHERE 1 = 1 
''

IF @_as_of_date IS NOT NULL
	SET @_sql += ''CHAR(10)'' + ''AND cwg.as_of_date <= '''''' + @_as_of_date + ''''''''

IF @_wacog_group_id IS NOT NULL
	SET @_sql += ''CHAR(10)'' + ''AND cwg.wacog_group_id = '''''' + @_wacog_group_id + ''''''''
	
IF @_term_start IS NOT NULL
	SET @_sql += ''CHAR(10)'' + ''AND cwg.term >= '''''' + @_term_start + ''''''''

IF @_term_end IS NOT NULL
	SET @_sql += ''CHAR(10)'' + ''AND cwg.term <= '''''' + @_term_end + ''''''''
...
EXEC(@_sql)', report_id = @report_id_data_source_dest 
	...
	BEGIN
		...
		SELECT TOP 1 2 AS [type_id], 'WACOQ Sql' AS [name], 'wacog' AS ALIAS, NULL AS [description],'DECLARE @_as_of_date DATETIME
	  , @_wacog_group_id INT
	  , @_term_start DATETIME
	  , @_term_end DATETIME
	  , @_sql VARCHAR(MAX)
...
EXEC(@_sql)' AS [tsql], @report_id_data_source_dest AS report_id
	...
	BEGIN
		...
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'WACOQ Sql'
	...
	BEGIN
		...
		SET alias = 'Wacog Group ID'
			   , reqd_param = NULL, widget_id = 2, datatype_id = 4, param_data_source = 'SELECT wacog_group_id AS [Value], wacog_group_name AS [Label] FROM wacog_group', param_default_value = NULL, append_filter = NULL, tooltip = NULL, column_template = 2, key_column = 0, required_filter = 0
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
		WHERE ds.[name] = 'WACOQ Sql'
...
COMMIT TRAN
	...
	IF OBJECT_ID('tempdb..#data_source_column', 'U') IS NOT NULL
		...
		IF NOT EXISTS(SELECT 1 FROM report_dataset rd WHERE rd.report_id = @report_id_dest AND rd.[alias] =  'wacog')
		BEGIN
			INSERT INTO report_dataset (source_id, report_id, [alias], root_dataset_id, is_free_from, relationship_sql)
			SELECT TOP 1 ds.data_source_id AS source_id, @report_id_dest AS report_id, 'wacog' [alias], rd_root.report_dataset_id AS root_dataset_id,0 AS is_free_from, 'NULL' AS relationship_sql
			FROM sys.objects o
			INNER JOIN data_source ds ON ds.[name] = 'WACOQ Sql'
	...
	IF NOT EXISTS(SELECT 1 FROM report_page rp 
	              WHERE rp.report_id = CASE WHEN 'e ' = 'p' 
											...
											ELSE @report_id_dest 
						               END 
					AND rp.name =  'WACOG Calculation Report'  
	)
	BEGIN
		...
		SELECT CASE WHEN 'e ' = 'p' 
					Then 28399 
					ELSE @report_id_dest 
		       END  AS report_id, 'WACOG Calculation Report' [name], '8591D75C_0078_4796_AFCF_B1B6D78DB18E' report_hash, 11.5 width,5.5 height
	END 
	

		INSERT INTO report_paramset(page_id, [name], paramset_hash, report_status_id)
		SELECT TOP 1 rpage.report_page_id, 'WACOG Calculation Report', '3DEA9035_B014_4608_B681_B452463EDAAC', 2
		FROM sys.objects o
		INNER JOIN report_page rpage 
			on rpage.[name] = 'WACOG Calculation Report'
		INNER JOIN report r 
		ON r.report_id = rpage.report_id
			AND r.[name] = 'WACOG Calculation Report'
	

		INSERT INTO report_dataset_paramset(paramset_id, root_dataset_id, where_part, advance_mode)
		SELECT TOP 1 rp.report_paramset_id AS paramset_id, rd.report_dataset_id AS root_dataset_id, '(  wacog.[term] BETWEEN ''@term'' 
AND ''@2_term'')' AS where_part, 0
		FROM sys.objects o
		INNER JOIN report_paramset rp 
			ON rp.[name] = 'WACOG Calculation Report'
		INNER JOIN report_page rpage 
			ON rpage.report_page_id = rp.page_id
			AND rpage.[name] = 'WACOG Calculation Report'
		INNER JOIN report r 
			ON r.report_id = rpage.report_id
			AND r.[name] = 'WACOG Calculation Report'
		...
		INSERT INTO report_param(dataset_paramset_id, dataset_id, column_id, operator,
					initial_value, initial_value2, optional, hidden, logical_operator, param_order, param_depth, label)
		SELECT TOP 1 rdp.report_dataset_paramset_id AS dataset_paramset_id, rd.report_dataset_id AS dataset_id , dsc.data_source_column_id AS column_id, 8 AS operator, '' AS initial_value, '' AS initial_value2, 1 AS optional, 0 AS hidden,0 AS logical_operator, 2 AS param_order, 0 AS param_depth, NULL AS label
		FROM sys.objects o
		INNER JOIN report_paramset rp 
			ON rp.[name] = 'WACOG Calculation Report'
		INNER JOIN report_page rpage 
		...
		SELECT TOP 1 rpage.report_page_id AS page_id, rd.report_dataset_id AS root_dataset_id, 'WACOG Calculation Report_tablix' [name], '11.786666666666667' width, '2.6666666666666665' height, '0' [top], '0' [left],2 AS group_mode,1 AS border_style,0 AS page_break,1 AS type_id,1 AS cross_summary,2 AS no_header,'' export_table_name, 0 AS is_global
		FROM sys.objects o
		INNER JOIN report_page rpage 
		ON rpage.[name] = 'WACOG Calculation Report'
		INNER JOIN report r 
		...
		INNER JOIN report_page_tablix rpt 
			ON rpt.[name] = 'WACOG Calculation Report_tablix'
		INNER JOIN report_page rpage 
		...
		INNER JOIN report r 
			...
			AND r.[name] = 'WACOG Calculation Report'
		INNER JOIN report_dataset rd 
			ON rd.report_id = r.report_id AND rd.[alias] = 'wacog' 	
		INNER JOIN data_source ds 
			ON ISNULL(NULLIF(ds.report_id, 0), r.report_id) = r.report_id	AND ds.[name] = 'WACOQ Sql' 	
		INNER JOIN data_source_column dsc 
		...
		INNER JOIN data_source ds 
			ON ISNULL(NULLIF(ds.report_id, 0), r.report_id) = r.report_id	AND ds.[name] = 'WACOQ Sql' 	
		INNER JOIN data_source_column dsc 
			ON dsc.source_id = ds.data_source_id AND dsc.[name] = 'wacog_group_id' 

		INSERT INTO report_tablix_column(tablix_id, dataset_id, column_id, placement, column_order, aggregation
					, functions, [alias], sortable, rounding, thousand_seperation, font
					, font_size, font_style, text_align, text_color, background, default_sort_order
					, default_sort_direction, custom_field, render_as, column_template, negative_mark, currency, date_format, cross_summary_aggregation, mark_for_total, sql_aggregation, subtotal)
		SELECT TOP 1 rpt.report_page_tablix_id tablix_id, rd.report_dataset_id dataset_id, dsc.data_source_column_id column_id,1 placement, 2 column_order,NULL aggregation, NULL functions, 'Wacog Group Name' [alias], 1 sortable, NULL rounding, NULL thousand_seperation, 'Tahoma' font, '8' font_size, '0,0,0' font_style, 'Left' text_align, '#000000' text_color, '#ffffff' background, NULL default_sort_order, NULL sort_direction, 0 custom_field, 0 render_as,-1 column_template,NULL negative_mark,NULL currency,NULL date_format,-1 cross_summary_aggregation,NULL mark_for_total,NULL sql_aggregation,NULL subtotal
		...
		INSERT INTO report_tablix_column(tablix_id, dataset_id, column_id, placement, column_order, aggregation
					...
					, font_size, font_style, text_align, text_color, background, default_sort_order
					, default_sort_direction, custom_field, render_as, column_template, negative_mark, currency, date_format, cross_summary_aggregation, mark_for_total, sql_aggregation, subtotal)
		SELECT TOP 1 rpt.report_page_tablix_id tablix_id, rd.report_dataset_id dataset_id, dsc.data_source_column_id column_id,1 placement, 4 column_order,NULL aggregation, NULL functions, 'Wacog' [alias], 1 sortable, -1 rounding, 0 thousand_seperation, 'Tahoma' font, '8' font_size, '0,0,0' font_style, 'Right' text_align, '#000000' text_color, '#ffffff' background, NULL default_sort_order, NULL sort_direction, 0 custom_field, 2 render_as,-1 column_template,0 negative_mark,NULL currency,NULL date_format,-1 cross_summary_aggregation,NULL mark_for_total,NULL sql_aggregation,NULL subtotal
