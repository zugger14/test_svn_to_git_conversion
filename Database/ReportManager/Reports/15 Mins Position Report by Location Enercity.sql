BEGIN TRY
		...
		set @report_copy_name = isnull(@report_copy_name, 'Copy of ' + '15 Mins Position Report by Location Enercity')
		

		INSERT INTO report ([name], [owner], is_system, is_excel, is_mobile, report_hash, [description], category_id)
		SELECT TOP 1 '15 Mins Position Report by Location Enercity' [name], 'dev_admin' [owner], 0 is_system, 1 is_excel, 0 is_mobile, '8A0B17E9_E5C6_44DB_88A0_02BA21582B4B' report_hash, '15 Mins Position Report by Location Enercity' [description], CAST(sdv_cat.value_id AS VARCHAR(10)) category_id
		FROM sys.objects o
		...
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = '15 Mins Position by Location View'
	...
	SELECT @report_id_dest AS report_id, '15 Mins Position Report by Location Enercity' [name], '8A0B17E9_E5C6_44DB_88A0_02BA21582B4B' report_hash, 11.5 width,5.5 height
		...
		SELECT TOP 1 rpage.report_page_id, '15 Mins Position Report by Location Enercity', '40A90357_EA22_47DA_A388_B18BC3443A55', 3,'','','.xlsx',',', 
		-100000,'n','n',0	
		FROM sys.objects o
		INNER JOIN report_page rpage 
			on rpage.[name] = '15 Mins Position Report by Location Enercity'
		INNER JOIN report r 
		ON r.report_id = rpage.report_id
			AND r.[name] = '15 Mins Position Report by Location Enercity'
		...
		INNER JOIN report_paramset rp 
			ON rp.[name] = '15 Mins Position Report by Location Enercity'
		INNER JOIN report_page rpage 
			ON rpage.report_page_id = rp.page_id
			AND rpage.[name] = '15 Mins Position Report by Location Enercity'
		INNER JOIN report r 
			ON r.report_id = rpage.report_id
			AND r.[name] = '15 Mins Position Report by Location Enercity'
		...
		INNER JOIN data_source ds 
			...
			AND ds.[name] = '15 Mins Position by Location View' 
		...
		INSERT INTO report_param(dataset_paramset_id, dataset_id, column_id, operator,
					initial_value, initial_value2, optional, hidden, logical_operator, param_order, param_depth, label)
		SELECT TOP 1 rdp.report_dataset_paramset_id AS dataset_paramset_id, rd.report_dataset_id AS dataset_id , dsc.data_source_column_id AS column_id, 9 AS operator, '' AS initial_value, '' AS initial_value2, 1 AS optional, 0 AS hidden,1 AS logical_operator, 25 AS param_order, 0 AS param_depth, 'Deal Status' AS label
		FROM sys.objects o
		INNER JOIN report_paramset rp 
			ON rp.[name] = '15 Mins Position Report by Location Enercity'
		INNER JOIN report_page rpage 
		...
		INSERT INTO report_param(dataset_paramset_id, dataset_id, column_id, operator,
					initial_value, initial_value2, optional, hidden, logical_operator, param_order, param_depth, label)
		SELECT TOP 1 rdp.report_dataset_paramset_id AS dataset_paramset_id, rd.report_dataset_id AS dataset_id , dsc.data_source_column_id AS column_id, 9 AS operator, 'a' AS initial_value, '' AS initial_value2, 1 AS optional, 0 AS hidden,1 AS logical_operator, 35 AS param_order, 0 AS param_depth, NULL AS label
		FROM sys.objects o
		INNER JOIN report_paramset rp 
			ON rp.[name] = '15 Mins Position Report by Location Enercity'
		INNER JOIN report_page rpage 
		...
		SELECT TOP 1 rpage.report_page_id AS page_id, rd.report_dataset_id AS root_dataset_id, 'Fifteen Mins Power Position Summary Report_tablix' [name], '6.346666666666667' width, '3.5733333333333332' height, '0' [top], '0' [left],2 AS group_mode,1 AS border_style,0 AS page_break,2 AS type_id,4 AS cross_summary,2 AS no_header,'' export_table_name, 0 AS is_global
		FROM sys.objects o
		INNER JOIN report_page rpage 
		ON rpage.[name] = '15 Mins Position Report by Location Enercity'
		INNER JOIN report r 
		...
		INNER JOIN data_source ds 
			ON ISNULL(NULLIF(ds.report_id, 0), r.report_id) = r.report_id	AND ds.[name] = '15 Mins Position by Location View' 	
		INNER JOIN data_source_column dsc 
