...
BEGIN TRY
	...
	FROM report r
	WHERE r.[name] = 'EOD - Price Verification'

	IF NOT EXISTS (SELECT 1 
	           FROM data_source 
	           WHERE [name] = 'Price Verification'
	...
	, [tsql] = CAST('' AS VARCHAR(MAX)) + 'DECLARE @_as_of_date VARCHAR(10)  = ''@as_of_date'',
		...
		@_curve_id NVARCHAR(4000) = ''@curve_id''
...
EXEC spa_eod_verify_missing_curve ''CHECK'',@_as_of_date,@_process_id,@_curve_id
...
', report_id = @report_id_data_source_dest,
	...
	IF EXISTS (SELECT 1 
	           ...
	           WHERE ds.[name] = 'Price Verification'
	            AND dsc.name =  'Status'
				AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1))
	BEGIN
		UPDATE dsc  
	...
	FROM data_source_column dsc 
	INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
		AND ds.[name] = 'Price Verification'
		AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1)
...
COMMIT TRAN
	...
	SELECT @report_id_dest AS report_id, 'EOD - Price Verification' [name], '54DFBBBD_11F5_4AEF_A7C6_FCDFA0ACDD96' report_hash, 11.5 width,5.5 height
		...
		SELECT TOP 1 rp.report_paramset_id AS paramset_id, rd.report_dataset_id AS root_dataset_id, '(  pv.[process_id] = ''@process_id'')' AS where_part, 0
		FROM sys.objects o
		INNER JOIN report_paramset rp 
			ON rp.[name] = 'EOD - Price Verification'
		INNER JOIN report_page rpage 
		...
		SELECT TOP 1 rpage.report_page_id AS page_id, rd.report_dataset_id AS root_dataset_id, 'EOD _ Price Verification_tablix' [name], '4' width, '2.6666666666666665' height, '0' [top], '0' [left],2 AS group_mode,1 AS border_style,0 AS page_break,1 AS type_id,1 AS cross_summary,2 AS no_header,'' export_table_name, 0 AS is_global
		FROM sys.objects o
		INNER JOIN report_page rpage 
		ON rpage.[name] = 'EOD - Price Verification'
		INNER JOIN report r 
		...
		INNER JOIN report r 
			...
			AND r.[name] = 'EOD - Price Verification'
		INNER JOIN report_dataset rd 
			ON rd.report_id = r.report_id AND rd.[alias] = 'pv' 	
		INNER JOIN data_source ds 
			ON ISNULL(NULLIF(ds.report_id, 0), r.report_id) = r.report_id	AND ds.[name] = 'Price Verification' 	
		INNER JOIN data_source_column dsc 
