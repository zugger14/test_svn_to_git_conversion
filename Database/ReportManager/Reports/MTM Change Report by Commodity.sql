...
IF EXISTS (SELECT 1 FROM dbo.report WHERE name='MTM Change Report By Commodity')
		BEGIN
		EXEC spa_rfx_report 'd', NULL,11684, NULL, NULL, NULL, NULL, NULL
		END
		...
		FROM report r
		WHERE r.[name] = 'MTM Change Report By Commodity'
		...
		* */
		...
		BEGIN
			...
			SELECT TOP 1 'MTM Change Report By Commodity' [name], 'farrms_admin' [owner], 0 is_system, '184A904B_390C_4F15_94C6_19B1F141D590' report_hash, 'Daily MTM Change Report' [description], CAST(sdv_cat.value_id AS VARCHAR(10)) category_id
			FROM sys.objects o
...
IF ''@as_of_date'' = ''NULL''
	SELECT @_as_of_date =  MAX(CONVERT(char(10), sdp.pnl_as_of_date,126)) FROM  dbo.source_deal_pnl sdp WHERE sdp.pnl_as_of_date < ''@to_as_of_date''


SELECT mtm_cur.source_deal_header_id [source_deal_header_id]
	, ca.as_of_date [as_of_date]
	, ca.prior_as_of_date [to_as_of_date]
    , CASE WHEN ca.[prior_date_diff] = 2 THEN 0 ELSE mtm_prior.und_pnl END[prior_mtm]
    , CASE WHEN ca.[prior_date_diff] = 3 THEN 0 ELSE mtm_cur.und_pnl END [cur_mtm]
	, CASE WHEN ca.[prior_date_diff] = 2 THEN (mtm_cur.und_pnl - 0)
			WHEN ca.[prior_date_diff] = 3 THEN (0 - mtm_prior.und_pnl)
			ELSE  mtm_cur.und_pnl - mtm_prior.und_pnl 
	 END [mtm_change]
	, mtm_cur.sub_id [sub_id], mtm_cur.stra_id [stra_id], mtm_cur.book_id [book_id], mtm_cur.sub_book_id 
                     , mtm_cur.commodity, ca.deal_type_name [deal_type_name]
, mtm_cur.deal_id,mtm_cur.term_start,mtm_cur.currency
,mtm_cur.period_from, mtm_cur.period_to,  mtm_cur.physical_financial,mtm_cur.subsidiary,mtm_cur.strategy,mtm_cur.book, mtm_cur.transaction_type
--[__batch_report__]
FROM (
	SELECT mtmv1.sub_id, mtmv1.stra_id, mtmv1.book_id, mtmv1.sub_book_id, mtmv1.source_deal_header_id, MAX(mtmv1.deal_type_name) deal_type_name
	, MAX(mtmv1.as_of_date) as_of_date, MIN(mtmv1.to_as_of_date) prior_as_of_date
	, CASE 
		WHEN MAX(mtmv1.as_of_date) <> MIN(mtmv1.to_as_of_date) AND DATEDIFF(day, MAX(mtmv1.as_of_date), CAST(@_as_of_date AS DATETIME)) < DATEDIFF(day, MIN(mtmv1.as_of_date), MIN(mtmv1.to_as_of_date)) THEN 1 
		WHEN DATEDIFF(day, MAX(mtmv1.as_of_date), CAST(@_as_of_date AS DATETIME)) < DATEDIFF(day, MIN(mtmv1.as_of_date), MIN(mtmv1.to_as_of_date)) THEN 2
		ELSE 3
		END [prior_date_diff]
	FROM {mtmv1} mtmv1
	GROUP BY mtmv1.sub_id, mtmv1.stra_id, mtmv1.book_id, mtmv1.sub_book_id, mtmv1.source_deal_header_id
) ca 
INNER JOIN {mtmv1} mtm_cur ON mtm_cur.source_deal_header_id = ca.source_deal_header_id
	AND mtm_cur.as_of_date = ca.as_of_date
INNER JOIN {mtmv1} mtm_prior ON mtm_prior.source_deal_header_id = ca.source_deal_header_id
	AND mtm_prior.as_of_date = ca.prior_as_of_date AND mtm_prior.term_start = mtm_cur.term_start
--WHERE ca.prior_as_of_date = @_as_of_date  AND ca.as_of_date = ''@to_as_of_date''', report_id = @report_id_data_source_dest 
		WHERE [name] = 'MTM Change SQL'
			AND ISNULL(report_id, -1) =  ISNULL(@report_id_data_source_dest, -1)
	END	
	...
	BEGIN
		...
		SELECT TOP 1 2 AS [type_id], 'MTM Change SQL' AS [name], 'mtmcs' AS ALIAS, NULL AS [description],'DECLARE @_as_of_date VARCHAR(10) = ''@as_of_date''
...
INNER JOIN {mtmv1} mtm_prior ON mtm_prior.source_deal_header_id = ca.source_deal_header_id
	AND mtm_prior.as_of_date = ca.prior_as_of_date AND mtm_prior.term_start = mtm_cur.term_start
--WHERE ca.prior_as_of_date = @_as_of_date  AND ca.as_of_date = ''@to_as_of_date''' AS [tsql], @report_id_data_source_dest AS report_id
	END 
	...
	BEGIN
		UPDATE dsc  
		SET alias = 'Commodity'
			   , reqd_param = 0, widget_id = 2, datatype_id = 5, param_data_source = 'EXEC spa_source_commodity_maintain ''a''', param_default_value = NULL, append_filter = 1, tooltip = NULL, column_template = 0, key_column = 0
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
		WHERE ds.[name] = 'MTM Change SQL'
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'commodity' AS [name], 'Commodity' AS ALIAS, 0 AS reqd_param, 2 AS widget_id, 5 AS datatype_id, 'EXEC spa_source_commodity_maintain ''a''' AS param_data_source, NULL AS param_default_value, 1 AS append_filter, NULL  AS tooltip,0 AS column_template, 0 AS key_column				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'MTM Change SQL'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
	...
	BEGIN
		...
		SET alias = 'Physical / Financial'
			   , reqd_param = 0, widget_id = 2, datatype_id = 5, param_data_source = 'SELECT ''Physical'' [value],''Physical'' [label]' + CHAR(10) + 'union' + CHAR(10) + 'SELECT ''Financial'' [value],''Financial'' [label]', param_default_value = NULL, append_filter = 1, tooltip = NULL, column_template = 0, key_column = 0
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		FROM data_source_column dsc
		INNER JOIN data_source ds ON ds.data_source_id = dsc.source_id 
		WHERE ds.[name] = 'MTM Change SQL'
	...
	BEGIN
		...
		, datatype_id, param_data_source, param_default_value, append_filter, tooltip, column_template, key_column)
		OUTPUT INSERTED.data_source_column_id INTO #data_source_column(column_id)
		SELECT TOP 1 ds.data_source_id AS source_id, 'physical_financial' AS [name], 'Physical / Financial' AS ALIAS, 0 AS reqd_param, 2 AS widget_id, 5 AS datatype_id, 'SELECT ''Physical'' [value],''Physical'' [label]' + CHAR(10) + 'union' + CHAR(10) + 'SELECT ''Financial'' [value],''Financial'' [label]' AS param_data_source, NULL AS param_default_value, 1 AS append_filter, NULL  AS tooltip,0 AS column_template, 0 AS key_column				
		FROM sys.objects o
		INNER JOIN data_source ds ON ds.[name] = 'MTM Change SQL'
			AND ISNULL(ds.report_id , -1) = ISNULL(@report_id_data_source_dest, -1)
		LEFT JOIN report r ON r.report_id = ds.report_id
...
COMMIT TRAN
	...
	IF NOT EXISTS(SELECT 1 FROM report_page rp 
	              WHERE rp.report_id = CASE WHEN 'e ' = 'p' 
											...
											ELSE @report_id_dest 
						               END 
					AND rp.name =  'MTM Change Report by Commodity'  
	)
	BEGIN
		...
		SELECT CASE WHEN 'e ' = 'p' 
					Then 11684 
					ELSE @report_id_dest 
		       END  AS report_id, 'MTM Change Report by Commodity' [name], '184A904B_390C_4F15_94C6_19B1F141D590' report_hash, 8 width,8 height
	END 
	

		INSERT INTO report_paramset(page_id, [name], paramset_hash, report_status_id)
		SELECT TOP 1 rpage.report_page_id, 'MTM Change Report by Commodity', 'DD985BBD_550F_413A_9F87_558034858B54', 3
		FROM sys.objects o
		INNER JOIN report_page rpage 
			on rpage.[name] = 'MTM Change Report by Commodity'
		INNER JOIN report r 
		ON r.report_id = rpage.report_id
			AND r.[name] = 'MTM Change Report By Commodity'
	

		INSERT INTO report_dataset_paramset(paramset_id, root_dataset_id, where_part, advance_mode)
		SELECT TOP 1 rp.report_paramset_id AS paramset_id, rd.report_dataset_id AS root_dataset_id, '(  mtmcs.[deal_type_name] = ''@deal_type_name'' AND mtmcs.[source_deal_header_id] = ''@source_deal_header_id'' AND mtmcs.[commodity] = ''@commodity'' AND mtmcs.[transaction_type] = ''@transaction_type'' AND mtmcs.[physical_financial] = ''@physical_financial'')' AS where_part, 0
		FROM sys.objects o
		INNER JOIN report_paramset rp 
			ON rp.[name] = 'MTM Change Report by Commodity'
		INNER JOIN report_page rpage 
			ON rpage.report_page_id = rp.page_id
			AND rpage.[name] = 'MTM Change Report by Commodity'
		INNER JOIN report r 
			ON r.report_id = rpage.report_id
			AND r.[name] = 'MTM Change Report By Commodity'
		...
		SELECT TOP 1 rdp.report_dataset_paramset_id AS dataset_paramset_id, rd.report_dataset_id AS dataset_id , dsc.data_source_column_id AS column_id, 1 AS operator, '' AS initial_value, '' AS initial_value2, 1 AS optional, 0 AS hidden,0 AS logical_operator, 1 AS param_order, 0 AS param_depth, 'Prior As of Date' AS label
		FROM sys.objects o
		INNER JOIN report_paramset rp 
			ON rp.[name] = 'MTM Change Report by Commodity'
		INNER JOIN report_page rpage 
		...
		SELECT TOP 1 rpage.report_page_id AS page_id, rd.report_dataset_id AS root_dataset_id, 'MTM Change Report by Commodity' [name], '7.906666666666666' width, '7.906666666666666' height, '0.04' [top], '0.06666666666666666' [left],0 AS group_mode,1 AS border_style,0 AS page_break,1 AS type_id,1 AS cross_summary,2 AS no_header,NULL export_table_name, 1 AS is_global
		FROM sys.objects o
		INNER JOIN report_page rpage 
		ON rpage.[name] = 'MTM Change Report by Commodity'
		INNER JOIN report r 
		...
		INNER JOIN report_page_tablix rpt 
			ON rpt.[name] = 'MTM Change Report by Commodity'
		INNER JOIN report_page rpage 
		...
		INSERT INTO report_tablix_column(tablix_id, dataset_id, column_id, placement, column_order, aggregation
					...
					, font_size, font_style, text_align, text_color, background, default_sort_order
					, default_sort_direction, custom_field, render_as, column_template, negative_mark, currency, date_format, cross_summary_aggregation, mark_for_total, sql_aggregation)
		SELECT TOP 1 rpt.report_page_tablix_id tablix_id, rd.report_dataset_id dataset_id, dsc.data_source_column_id column_id,1 placement, 3 column_order,NULL aggregation, 'mtmcs.subsidiary + ''/'' + mtmcs.strategy + ''/'' + mtmcs.book' functions, 'Subsidiary / Strategy / Book' [alias], 1 sortable, NULL rounding, NULL thousand_seperation, 'Tahoma' font, '8' font_size, '0,0,0' font_style, 'Left' text_align, '#000000' text_color, '#ffffff' background, NULL default_sort_order, NULL sort_direction, 0 custom_field, 0 render_as,-1 column_template,NULL negative_mark,NULL currency,NULL date_format,-1 cross_summary_aggregation,NULL mark_for_total,NULL sql_aggregation
		FROM sys.objects o
		INNER JOIN report_page_tablix rpt 
			ON rpt.[name] = 'MTM Change Report by Commodity'
		INNER JOIN report_page rpage 
		...
		INSERT INTO report_tablix_column(tablix_id, dataset_id, column_id, placement, column_order, aggregation
					...
					, font_size, font_style, text_align, text_color, background, default_sort_order
					, default_sort_direction, custom_field, render_as, column_template, negative_mark, currency, date_format, cross_summary_aggregation, mark_for_total, sql_aggregation)
		SELECT TOP 1 rpt.report_page_tablix_id tablix_id, rd.report_dataset_id dataset_id, dsc.data_source_column_id column_id,1 placement, 5 column_order,NULL aggregation, NULL functions, 'Current MTM ($)' [alias], 1 sortable, -1 rounding, 0 thousand_seperation, 'Tahoma' font, '8' font_size, '0,0,0' font_style, 'Left' text_align, '#000000' text_color, '#ffffff' background, NULL default_sort_order, NULL sort_direction, 0 custom_field, 2 render_as,-1 column_template,0 negative_mark,NULL currency,NULL date_format,-1 cross_summary_aggregation,NULL mark_for_total,13 sql_aggregation
		FROM sys.objects o
		INNER JOIN report_page_tablix rpt 
			ON rpt.[name] = 'MTM Change Report by Commodity'
		INNER JOIN report_page rpage 
