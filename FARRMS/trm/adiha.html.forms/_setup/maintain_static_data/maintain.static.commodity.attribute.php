<?php
/**
* Maintain static commodity attribute screen
...
<html>
    ...
    <body>
        ...
        $layout_name = 'commodity_attribute_layout';


        if (isset($_POST['value_id'])) {
            $value_id = get_sanitized_value($_POST['value_id']);
            echo $xml = '<Root><PSRecordset commodity_attribute_id="' . $value_id . '"></PSRecordset></Root>';
        } else {
            $value_id = "null";
            $xml = '<Root><PSRecordset commodity_attribute_id=""></PSRecordset></Root>';
        }
        ...
        $name_space = 'commodity_attribute';
        echo $layout->init_layout($layout_name, '', '1C', $layout_json, $name_space);

        $toolbar_name = 'commodity_attribute_toolbar';
        ...
        //Start of Tabs
        $tab_name = 'commodity_attribute_tabs';
        ...
        $xml_file = "EXEC spa_create_application_ui_json 'j', 10101080, 'commodity_attribute', '$xml' ";
        ...
        $form_name = 'commodity_attribute';
        echo $tab_obj->attach_form($form_name, 'a1', $form_structure_general, $name_space);
        //Grid 
        $grid_name = 'commodity_attribute_quality_grd';
        //echo $tab_obj->attach_grid($grid_name, 'a2');
        echo 'commodity_attribute.commodity_attribute_quality_grd = commodity_attribute.commodity_attribute_tabs.tabs("a2").attachGrid();';


        $grid_quality_obj = new GridTable('commodity_attribute_form');
        ...
        $grid_spa = "EXEC spa_commodity_attribute_form @flag = 's', @commodity_attribute_id = '" . $value_id . "'";
        ...
        <script type="text/javascript">
            $(function () {
                ...
                var delete_grid = '';
                var check_box_json;
                var commodity_attribute_quality_changed_cell_arr = [];
                ...
                grid_toolbar = commodity_attribute.commodity_attribute_tabs.tabs("a2").attachMenu();
                ...
                grid_toolbar.attachEvent('onClick', function (id) {
                    switch (id) {
                        case "add" :
                            ...
                            commodity_attribute.commodity_attribute_quality_grd.addRow(newId, '', '');
                            commodity_attribute.commodity_attribute_quality_grd.cells(newId, 6).setValue(value_id);
                            break;
                        case "delete" :
                            var del_ids = commodity_attribute.commodity_attribute_quality_grd.getSelectedRowId();
                            var values_id = commodity_attribute.commodity_attribute_quality_grd.cells(del_ids, 0).getValue();
                            var static_values_id = commodity_attribute.commodity_attribute_quality_grd.cells(del_ids, 1).getValue();
                            delete_grid += '<GridRow  commodity_attribute_form_id ="' + values_id + '" ></GridRow>';
                            commodity_attribute.commodity_attribute_quality_grd.deleteRow(del_ids);
                            grid_toolbar.setItemDisabled("delete");
                            break;
                        case "excel":
                            commodity_attribute.commodity_attribute_quality_grd.toExcel(php_script_loc_ajax + "components/lib/adiha_dhtmlx/grid-excel-php/generate.php");
                            break;
                        case "pdf":
                            commodity_attribute.commodity_attribute_quality_grd.toPDF(php_script_loc_ajax + "components/lib/adiha_dhtmlx/grid-pdf-php/generate.php");
                            break;
                    }
                });

                // Event after clicking in the
                commodity_attribute.commodity_attribute_toolbar.attachEvent('onClick', function (id) {

                    if (id == 'save') {
                        generalForm = commodity_attribute.commodity_attribute.getForm();
                        ...
                        if (status === false) {
                            ...
                            var source_commodity_attribute_id = value_id;
                            var data_type = generalForm.getItemValue('data_type');
                            data_type = (data_type == '') ? 4071 : data_type;
                            ...
                            form_xml = '<Root function_id="10101080" object_id="' + source_commodity_attribute_id + '" ><FormXML commodity_attribute_id = "' + source_commodity_attribute_id + '" data_type = "' + data_type + '" commodity_name = "' + commodity_name + '" commodity_description = "' + commodity_description + '" ></FormXML>';
                            grid_xml += commodity_attribute.get_changed_data_grid();
                            xml = form_xml + grid_xml + '</Root>';

                            data = {"action": "spa_process_form_data", "xml": xml};
                            var count = commodity_attribute.commodity_attribute_quality_grd.getRowsNum();
                            
                            if (delete_grid != '') {
                                del_msg = "Some data has been deleted from grid. Are you sure you want to save?";
                                result = adiha_post_data("confirm-warning", data, "", "", "commodity_attribute.call_back", "", del_msg);
                            } else {
                                result = adiha_post_data("alert", data, "", "", "commodity_attribute.call_back");
                            }
                        }
                    }
                });

                commodity_attribute.commodity_attribute_quality_grd.attachEvent("onEditCell", function (stage, rId, cInd, nValue, oValue) {

                    if (stage == 2) {
                        if (nValue != oValue && cInd >= 0) {
                            commodity_attribute_quality_changed_cell_arr.push(rId);
                            if (cInd == 1) {
                                commodity_attribute.get_quality_type(nValue, rId);
                            }
                        }
                    }
                    commodity_attribute_quality_changed_cell_arr = commodity_attribute_quality_changed_cell_arr.filter(function (elem, pos) {
                        return commodity_attribute_quality_changed_cell_arr.indexOf(elem) == pos;
                    });
                    return true;
                });

                commodity_attribute.get_changed_data_grid = function () {
                    var i = 0;
                    var grid_xml = '';
                    var store_name = '';
                    grid_xml += '<GridGroup>';
                    if (delete_grid != '') {
                        grid_xml += '<GridDelete grid_id="commodity_attribute_form" grid_label="commodity_attribute_form">' + delete_grid + '</GridDelete>';
                    }
                    commodity_attribute.commodity_attribute_quality_grd.clearSelection();
                    if (commodity_attribute_quality_changed_cell_arr.length > 0) {
                        grid_xml += '<Grid grid_id="commodity_attribute_form">';
                        $.each(commodity_attribute_quality_changed_cell_arr, function (index, value) {
                            grid_xml += "<GridRow ";
                            commodity_attribute.commodity_attribute_quality_grd.forEachCell(value, function (cellObj, ind) {
                                grid_index = commodity_attribute.commodity_attribute_quality_grd.getColumnId(ind);
                ...
                commodity_attribute.call_back = function (result) {
                    if (result[0].errorcode == "Success") {
                        ...
                        refresh_grid(new_id);
                        delete_grid = "";
                        commodity_attribute_quality_changed_cell_arr = [];
                        parent.setup_static_data.special_menu_case(result, code, 'commodity_attribute');
                    }
                }

                commodity_attribute.commodity_attribute_quality_grd.attachEvent("onRowSelect", doOnRowSelected);
...
//            if( has_rights_holiday_block_grid_delete == 1 ) {
//                grid_toolbar.setItemEnabled("delete");
//            }
                }
                function refresh_grid(id) {
                    var sql_param = {
                        "sql": "EXEC spa_commodity_attribute_form @flag = 's', @commodity_attribute_id = '" + value_id + "'"
                    };
                    sql_param = $.param(sql_param);
                    var sql_url = js_data_collector_url + "&" + sql_param;
                    commodity_attribute.commodity_attribute_quality_grd.clearAll();
                    commodity_attribute.commodity_attribute_quality_grd.load(sql_url);
                    var count = commodity_attribute.commodity_attribute_quality_grd.getRowsNum();
                    if (count > 0) {
                        commodity_attribute.commodity_attribute_quality_grd.forEachRow(function (id) {
                            // Looping for cell
                            commodity_attribute.commodity_attribute_quality_grd.forEachCell(id, function (cellObj, ind) {
                                grid_index = commodity_attribute.commodity_attribute_quality_grd.getColumnId(ind);
                                if (grid_index == 'range') {
