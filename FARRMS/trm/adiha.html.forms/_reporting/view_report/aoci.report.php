...
<body> 
    ...
    $layout_name = 'aoci_report_layout';
    $name_space = 'aoci_report';
...
<script type="text/javascript">
    ...
    function load_report_detail(result) {
        ...
        report_ui["report_tabs_" + active_object_id] = aoci_report.aoci_report_layout.cells("a").attachTabbar();
        ...
        var att_obj = aoci_report.aoci_report_layout.cells('a');
    ...
    function get_message(code) {
        switch (code) {
            case 'DATE_VALIDATION':
                return 'As of Date From cannot be greater than As of Date To.';
                
        }
    }
    
    function return_as_of_date() {
        var inner_tab_obj = aoci_report.aoci_report_layout.cells("a").getAttachedObject();
    ...
    function report_parameter(is_batch) {
            ...
            form_obj.forEachItem(function(name) {
                ...
                if (item_type == 'calendar') {
                    ...
                    if (name == 'enable_paging') {
                        ...
                        filter_list.push(form_obj.getItemLabel(name) + '=' + singleQuote(value));   
                    } 
        ...
        var exec_call = 'EXEC spa_Create_AOCI_Report ' +
        ...
        if (is_batch) {
            if (apply_paging == 1) {
                var exec_call = exec_call + '&applied_filters=' + filter_list + '&gen_as_of_date=0&enable_paging=true&np=1';
            } else {
                var exec_call = exec_call + '&applied_filters=' + filter_list + '&gen_as_of_date=0';
            }
        } else {
            if (apply_paging == 1) {
                var exec_call = exec_call + '&applied_filters=' + filter_list + '&enable_paging=true&np=1';
            } else {
