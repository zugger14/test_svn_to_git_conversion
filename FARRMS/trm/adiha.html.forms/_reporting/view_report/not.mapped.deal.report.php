...
//        list($default_as_of_date_to, $default_as_of_date_from) = getDefaultAsOfDate($module_type);
        ...
        $layout_name = 'not_mapped_deal_layout';
        $name_space = 'not_mapped_deal';
        $layout_obj = new AdihaLayout();
        echo $layout_obj->init_layout($layout_name, '', '1C', $layout_json, $name_space);
        $dt_as_of_date_from = date('Y-m-01');
        $dt_as_of_date_to = date('Y-m-t');
    ...
    <script>
        ...
        function load_report_detail(result) {
            ...
            report_ui["report_tabs_" + active_object_id] = not_mapped_deal.not_mapped_deal_layout.cells("a").attachTabbar();
            ...
            // set_default_value(); //set 'deal_date_to' from setup menu 'Setup As of Date'                
            report_ui["form_0"].setItemValue('deal_date_to', '<?php echo $dt_as_of_date_to; ?>');
             
            var att_obj = not_mapped_deal.not_mapped_deal_layout.cells('a');
        ...
        function return_as_of_date() {
            var inner_tab_obj = not_mapped_deal.not_mapped_deal_layout.cells("a").getAttachedObject();                    
            var as_of_date;
            ...
            inner_tab_obj.forEachTab(function(tab){
                var tab_name = tab.getText();
                 ...
                 if(form_obj.isItem('custom_as_of_date_field')) {
                    ...
                    as_of_date = form_obj.getItemValue('deal_date_to', true);
                 }                
             });
        ...
        function report_parameter(is_batch) {
            ...
            inner_tab_obj.forEachTab(function(tab){
                ...
                form_obj.forEachItem(function(name) {
                    ...
                    if (name == 'spa_name') {
                    ...
                    } else if (name == 'deal_date_to') {
                        gen_as_of_date = 1;
                        
                        value = form_obj.getItemValue(name, true);   
                        filter_value = (value != '') ? dates.convert_to_user_format(value) : '';                    
                        as_of_date_to_value = value;
                        
                        if (value != '') {                            
                            as_of_date_field_name = form_obj.getItemValue('deal_date_to', true);  //this will be field_name for as of date
                        ...
                        if (name == 'deal_date_from') {
                            as_of_date_from_value = value
                        } 
                        ...
                        if (name == 'enable_paging') {
                            ...
                            filter_list.push(form_obj.getItemLabel(name) + '="' + value.toUpperCase() + '"');   
                        }   
        ...
        function set_default_value_call_back(return_json) { 
            ...
            inner_tab_obj.forEachTab(function(tab){
                ...
                form_obj = tab.getAttachedObject();
                if (tab_name == 'General') {
                    form_obj.setItemValue('deal_date_from', custom_as_of_date);                              
                }
            })
        }
        }
        ...
        function load_business_day(return_json) { 
            ...
            inner_tab_obj.forEachTab( function(tab) {
                ...
                form_obj = tab.getAttachedObject();
                if (tab_name == 'General') {
                    form_obj.setItemValue('deal_date_from', business_day);                              
                }
            })
        }
    </script>