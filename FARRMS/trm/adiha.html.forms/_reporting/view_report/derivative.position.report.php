...
<html>  
    ...
    <script>
        ...
        function load_report_detail(result) {
            ...
            report_ui["report_tabs_" + active_object_id] = dpr.dpr_layout.cells("a").attachTabbar();
            ...
            for (j = 0; j < result_length; j++) {
                ...
                if (result[j][2]) {
                    ...
                    attach_browse_event(form_name, active_object_id, 'post_browse_event', 'n');
                }
            }
            ...
            report_ui["report_tabs_" + active_object_id].cells('detail_tab_' + result[0][0]).setActive();
            var att_obj = dpr.dpr_layout.cells('a');
            parent.set_apply_filter(att_obj);
            var cmb_obj = report_ui["form_1"].getCombo('transaction_type');
            var cindex = cmb_obj.getIndexByValue('407');
        ...
        function post_browse_event(name) {
            var inner_tab_obj = dpr.dpr_layout.cells("a").getAttachedObject();
            
            inner_tab_obj.forEachTab(function(tab){
                 var tab_name = tab.getText();
                 form_obj = tab.getAttachedObject();
                 if (tab_name == 'Additional') {
                    
                 
                     if(name == 'major_location' && form_obj.isItem('major_location')) {
                        source_major_location_id = form_obj.getItemValue('major_location');
                        //source_major_location_id='60,61';
                        var cmb_param = {
                                        "action": "spa_source_minor_location",
                                        "flag": "o",
                                        "source_major_location_id": source_major_location_id.toString(),
                                        "has_blank_option": "true"
                                    };
            
                        cmb_param = $.param(cmb_param);
                        var url = js_dropdown_connector_url + '&' + cmb_param;
                        var cmb_obj = form_obj.getCombo('location_id'); 
                        cmb_obj.clearAll();
                        cmb_obj.load(url);
                     }
                }                
        ...
        function report_parameter(is_batch) {
            ...
            inner_tab_obj.forEachTab(function(tab){
                ...
                form_obj.forEachItem(function(name) {
                    ...
                    //dates
                    if (name == 'as_of_date_from') {
                        as_of_date_from_value = form_obj.getItemValue('as_of_date_from', true);
                    } else if (name == 'as_of_date') {
                        as_of_date_to_value = form_obj.getItemValue('as_of_date', true);
                    } else if (name == 'tenor_from') {
                        tenor_from_value = form_obj.getItemValue('tenor_from', true);
                    } else if (name == 'tenor_to') {
                        tenor_to_value = form_obj.getItemValue('tenor_to', true);
                    }
            ...
            tenor_from_value = get_static_date_value(tenor_from_value);
            tenor_to_value = get_static_date_value(tenor_to_value);

            var err_msg = '';
            /*Validation for the Date*/
            if (validation_flag == 0) {
                if ((as_of_date_from_value != '') && (as_of_date_from_value > as_of_date_to_value)) {
                    err_msg = '<b>As of Date From</b> should be less than <b>As of Date To</b>.';
                    validation_flag = 1;
                } else if ((tenor_from_value != null) && (tenor_to_value != null) && (tenor_from_value > tenor_to_value)) {
                    err_msg = '<b>Tenor From</b> should be less than <b>Tenor To</b>.';
                    validation_flag = 1;
                }
            }
