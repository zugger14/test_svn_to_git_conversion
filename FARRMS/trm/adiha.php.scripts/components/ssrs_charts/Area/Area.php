 ...
 * 
 *  @par Description
 *  This class handles area chartype : Area
 *  @copyright Pioneer Solutions
 */
	class Area implements iChart {	   
        private $xml_str;
       ...
       /**
		 ...
		 *
		 * @return  string  xml string for area chart
		 */
       private function build_member_hierarchy_xml($name, $identifier) {
        	return '<ChartMembers>
            			<ChartMember>
            				<Group Name="' . $this->name . 'ChartMemberSeries_' . $identifier . '_' . $name . '">
            					<GroupExpressions><GroupExpression>=Fields!' . $name . '.Value</GroupExpression></GroupExpressions>
            				</Group>
		 ...
		 * @param   string  $name        group name
		 * @param   string  $identifier  unique identifier
		 * @param   array  $sort_name   Short Name
		 *
		 * @return  string  xml string for area chart
		 */
		 ...
		 * @param   array $sorts   sorting type
		 *
		 * @return  string   xml string for area chart
		 */
    	   ...
    	   $group_xml = NULL;            
            if (count($groups) > 0) {        
            	...
            	foreach ($groups as $sn => $node) {
					$sorts_sn = ($sorts[$sn] ?? '');
            		if ($sn == 0) {
            			$group_xml .= $this->build_group_hierarchy_xml($node, $sn, $sorts_sn);
            		} else {
            			$xml_partial = $this->build_group_hierarchy_xml($node, $sn);
            			$group_xml = str_replace('<ChartMembers></ChartMembers>', $xml_partial, $group_xml, $sorts_sn); 
            		}
            		if ($total_series == ($sn + 1)) {
			...
			foreach ($series as $val) {			
				if ($count < 1) {			
					$series_xml .= '<ChartSeries Name="' . $val . '">
    									<ChartDataPoints>
    										<ChartDataPoint>
    											<ChartDataPointValues><Y>=Sum(CDbl(Fields!' . $val . '.Value))</Y></ChartDataPointValues>
		 ...
		 * @return  string  xml string for area chart
		 */
		private function build_area_chart(){
		 ...
		 *
		 * @return  string  returns area chart rdl string
		 */
        public function get_chart_rdl() {
			return $this->build_area_chart();
		}	  
	}
