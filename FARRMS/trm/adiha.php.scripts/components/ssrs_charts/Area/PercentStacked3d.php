<?php
	 ...
	 * @access public
	 ...
	 Modification
	 Defaulted to replace Zeros with Blank to avoid "Value was either too large or too small for a Decimal" issue with SSRS 3-D 100% Stacked Area chart.
	 FX CODE USED :
	 =iif(Sum(Fields!' . $val . '.Value)>0,Sum(Fields!' . $val . '.Value),"")
					...
					$series_xml .= '<ChartSeries Name="' . $val . '">
										<ChartDataPoints>
											...
											<ChartDataPointValues>
												<Y>=iif(Sum(Fields!' . $val . '.Value)>0,Sum(Fields!' . $val . '.Value),"")</Y>
											</ChartDataPointValues>
											<ChartDataLabel>
												<Style />
											</ChartDataLabel>
