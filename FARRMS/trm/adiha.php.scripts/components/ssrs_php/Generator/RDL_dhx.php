...
class RDL {
     ...
     */
    public function __construct($report_name = null, $ssrs_config = null, $rdl_type = null) {
        $this->report_name = $report_name . ($rdl_type == 'rdl_preview' ? '_preview' : '');
    ...
    public function set_base($report_height, $report_width, $report_hash) {
		$new_report_hash = str_replace("_","-",$report_hash);
            ...
            'Report' => array(
                ...
                'DataSources' => array(
                    'DataSource' => array(
                        ...
                        'rd:DataSourceID' => $new_report_hash
                    ),
                ),
                'DataSets' => '',
                'Body' => array(
    ...
    public function push_dataset($dataset) {
		...
		if (!is_array($this->arr_rdl['Report']['DataSets']['DataSet'])) {
    ...
    public function push_textbox($textboxes) {
		...
		if (!is_array($this->arr_rdl['Report']['Body']['ReportItems']['Textbox'])) {
    ...
    public function push_tablix(array $tablixes) {
        if(is_array($tablixes) && sizeof($tablixes) > 0){
			...
			if (!is_array($this->arr_rdl['Report']['Body']['ReportItems']['Tablix'])) {
    ...
    public function save_rdl() {
        ...
        $write_status = fwrite($file_handler, $content); //var_dump($write_status);
        fclose($file_handler);
        return $write_status;
    }
    ...
    public function get_job_sql($report_page) {
        return "EXEC [spa_rfx_deploy_rdl_as_job_dhx] 'RDL Deployer', 'farrms_admin', 'TSQL', " . $report_page . ", '$this->rdl_type'" . ", '" .$this->report_name . "'" ;
    }
