<?php
/** 
 *  @brief AdihaStandardForm Standrad form class
 *  
 *  @par Description
 *  This class is used to generate a simple form according to the template defined in template tables.
 *  Basic definition required to create a form should be present in template tables prior to the use of this class.
 *  
...
class AdihaStandardForm {
    ...
    private $key_prefix = 'UI';
    private $cache_data = true;

    private $after_save_function;
    private $before_save_validation;

    private $call_from_combo = false;
    ...
    public function __construct($namespace, $function_id) {
        ...
        if ($_GET['call_from_combo'] ?? '' == 'combo_add') {
            $this->call_from_combo = true;
        }
    }

    /**
     * Define grid for left side of the form, that is defined in adiha_grid_definition.
     * @param  String  $grid_name   Grid Name
     * @param  String  $grid_sp     Grid SP
     * @param  String  $grid_type   type o grid - 'a' for accordion grid, else grid/treegrid]
     * @param  Boolean $report_status True if accordian grid used for Report else false
     * @param  Boolean $auto_adjust Set auto adjust false by default
     * @param  Boolean $auto_hide   Set auto hide false by default
     */
    public function define_grid($grid_name, $grid_sp = null, $grid_type = '', $report_status=false, $sorting_preference = '', $auto_adjust = false, $auto_hide = false) {
    ...
    /**
     * Define width for layout cells.
     * @param  Integer $left_width  Cell of a width
     */
    ...
    /**
     * Define custom functions for save and form load.
     * @param  String $save_function                Name of save function]
     * @param  String $form_load_function           Name of form load function]
     * @param  String $delete_function              Name of left tree delete function]
     * @param  String $form_load_complete_function  Name of function that is returned to after form load by standard form
     * @param  String $before_save_validation       Function to validate before save
     * @param  String $after_save_function          Function to validate after save
     */
    public function define_custom_functions($save_function = null, $form_load_function = null, $delete_function = null,$form_load_complete_function = null, $before_save_validation = null, $after_save_function = null) {
    ...
    /**
     * Controls to modify the default standard form.
     * @param  Boolean $hide_save     Control to hide the default save button in form
     */
    public function define_custom_setting($hide_save = null) {
        $this->hide_save = $hide_save;        
    }
    
    /**
     * Controls to add apply filters in standard form.
     * @param  Boolean $filter_enable        Check the filter is enable or disable
     * @param  Integer $filter_function_id   Set filter function id is empty by default
     * @param  string  $filter_template_name Set filter template name is empty by default
     * @param  string  $filter_group_name    Set filter group name is empty by default
     * @param  boolean $cache_ui             Set Cache to false by default
     */
    public function define_apply_filters($filter_enable, $filter_function_id = '', $filter_template_name = '', $filter_group_name = '', $cache_ui = false) {
        ...
        $this->cache_data = $cache_ui;
    }
    
    /**
     * Show apply filter.
     * @param  Boolean $enable  False - just create the layout, donot load the apply filter
     */
    ...
    /**
     * Add Privilege menu.
     * @param Boolean $privilege_rights    Privilege rights
     * @param String  $win_close_callback  Callback function name
     * @param Integer $value_col_ind       Column Index
     */
    ...
    /**
     * Initialize the form.
     * @param  String  $first_cell_label  Label for first cell 
     * @param  String  $second_cell_label Label for second cell
     * @param  Integer $selected_id       Set empty for selected id by default
     * @param  Integer $farrms_product_id Set empty for selected product id by default
     * @return String                     JS string to initialize the form
     */
    public function init_form($first_cell_label, $second_cell_label = null, $selected_id = '', $farrms_product_id = '') {
        ...
        if ($this->grid_type == 'a') {
            if (!$this->report_status) {
                ...
                $return_string .= $this->layout->show_header("$grid_cell");
            ...
            if ($this->auto_hide) {
                $return_string .=  $this->grid->enable_filter_auto_hide();
            }
            
            if ($this->call_from_combo) {
                $return_string .=  $this->grid->load_grid_data($this->grid_sp, '', $this->auto_adjust, 'function() { ' . $this->name_space . '.create_tab(-1,0,0,0); }');
            ...
            $return_string .=  $this->grid->attach_event('', 'onSelectStateChanged', $this->name_space.'.enable_menu_item');           
        ...
        if ($this->define_apply_filters  && $this->show_apply_filter) {
            $xml_file = "EXEC spa_create_application_ui_json @flag='j', @application_function_id='$this->filter_function_id', @template_name='$this->filter_template_name', @group_name='$this->filter_group_name', @template_type='Filter', @parse_xml='<Root><PSRecordset ".$this->primary_field."=\"NULL\"></PSRecordset></Root>'";
            
            $key_suffx = $this->filter_function_id . '_' . md5($this->filter_group_name);

            $return_value = ($this->cache_data) ? readXMLURLCached($xml_file,true,$this->key_prefix,$key_suffx,false,0,'readXMLURL') : readXMLURL($xml_file);
            ...
            $return_string .= $this->filter_form_obj->load_form_filter($this->name_space, 'filter_form', 'layout', 
                "$filter_cell", $this->filter_function_id, 2, 'layout');
    ...
    /**
     * Generate hex tab id returns PHP equivalent of ord - used in this class to create a numeric tabid for string data.
     * @return String       JS string to generate hex tab id
     */
    ...
    /**
     * Close form - also includes all necessary functions with it.
     * @return String       JS string for closing form
     */
    ...
    /**
     * Attach necessary functions.
     * @return String       JS string for attaching functions
     */
    ...
    /**
     * Grid Menu click functions.
     * @return String JS string for grid menu click
     */
    private function click_grid_menu() {
        ...
        $return_string .= '     var selected_row = ""; ' . "\n";
        ...
        // for grid and tree grid
            ...
            $return_string .= '         var count = select_id.indexOf(",") > -1 ? select_id.split(",").length : 1;'. "\n";
            $return_string .= '         select_id = select_id.indexOf(",") > -1 ? select_id.split(",") : [select_id];'. "\n";
            $return_string .= '         if (select_id != null) {'. "\n";
            $return_string .= '             confirm_messagebox("Are you sure you want to delete?", function() {'. "\n";
            $return_string .= '                 for ( var i = 1; i <= count; i++) {'. "\n";
            $return_string .= '                     var full_id = ' . $this->name_space . '.get_id(' . $this->name_space . '.grid, select_id[i-1]);'. "\n";
            $return_string .= '                     var full_id_split = full_id.split("_");  full_id_split.splice(0,1); var get_id_only = full_id_split.join("_");' . "\n";
            $return_string .= '                     if (get_id_only == "") {'. "\n";
            $return_string .= '                         show_messagebox("Please select child item only.");'. "\n";
            $return_string .= '                         return false;'. "\n";
            $return_string .= '                     } else {'. "\n";
            $return_string .= '                      	if (i == 1) {'. "\n";
            ...
            $return_string .= '         } else {'. "\n";
            $return_string .= '             show_messagebox("Please select a row from grid.");'. "\n";
            $return_string .= '         }'. "\n";
        }
        ...
        if ($this->add_privilege_menu) {
            ...
            $return_string .= '     selected_row = ' . $this->name_space . '.grid.getSelectedRowId();' . "\n";
            ...
            $return_string .= '             if (level == 0) { ' . "\n";
            $return_string .= '                 haschild = ' . $this->name_space . '.grid.getSubItems(selected_row_arr[i])' . "\n";
            $return_string .= '                 if(!haschild) {' . "\n";
            $return_string .= '                     selected_row = selected_row_arr[i];' . "\n";
            $return_string .= '                 }' . "\n";
            $return_string .= '                 break;' . "\n";
            $return_string .= '             }' . "\n";
            ...
            $return_string .= '         }' . "\n";
            $return_string .= '     } else { ' . "\n";
            $return_string .= '         selected_row = selected_row_arr[0];' . "\n";
            $return_string .= '     }' . "\n";
            $return_string .= '     if (col_type == "tree" && level == 0) { ' . "\n";
            $return_string .= '        haschild = ' . $this->name_space . '.grid.getSubItems(selected_row)' . "\n";
            $return_string .= '        if(!haschild) {' . "\n";
            ...
            $return_string .= '         var call_from = 1; var type_id = ' . $this->name_space . '.grid.cells(selected_row, ' . $this->name_space . '.grid.getColIndexById("type_id")).getValue();' . "\n"; 
            $return_string .= '     }' . "\n";
            $return_string .= '     confirm_messagebox("Are you sure you want to Activate ?", function() {' . "\n";
            ...
            $return_string .= '         var call_from = 1; var type_id = ' . $this->name_space . '.grid.cells(selected_row, ' . $this->name_space . '.grid.getColIndexById("type_id")).getValue();' . "\n"; 
            $return_string .= '     }' . "\n";
            $return_string .= '     confirm_messagebox("Are you sure you want to Deactivate ?", function() {' . "\n";
        ...
        $return_string .= '             success_call(return_value[0][4]);' . "\n";
        $return_string .= $this->name_space . '.refresh_grid("", ' . $this->name_space . '.enable_menu_item);' . "\n";
        $return_string .= '         } else {' . "\n";
        $return_string .= '             show_messagebox(return_value[0][4]);' . "\n";
        ...
        $return_string .= '             open_static_data_privilege_window(type_id, value_id);' . "\n";
        $return_string .= $this->name_space .'.refresh_grid()'."\n";                    
        $return_string .= '         } else {' . "\n";
        $return_string .= '             show_messagebox(return_value[0][4]);' . "\n";
        $return_string .= '             return;' . "\n";
        $return_string .= '         } ' . "\n";
        ...
        $return_string .= '             var filter_param = "<FormXML";' . "\n";
        ...
        $return_string .= ' var full_id;';
        $return_string .= ' if (result[0][0] == "Success") { ';
        $return_string .= '     var select_id = ' . $this->name_space .   '.grid.getSelectedRowId();'. "\n";
        $return_string .= '     var count = select_id.indexOf(",") > -1 ? select_id.split(",").length : 1;'. "\n";
        $return_string .= '     select_id = select_id.indexOf(",") > -1 ? select_id.split(",") : [select_id];'. "\n";
        $return_string .= '     for (var i = 0; i < count ; i++) {'. "\n";
        $return_string .= '         full_id = ' . $this->name_space . '.get_id(' . $this->name_space . '.grid, select_id[i]);'. "\n";
        ...
        $return_string .= '     success_call(result[0][4]);' . "\n";
        ...
        $return_string .= '    });' . "\n";
        $return_string .= ' } else {' . "\n";
        $return_string .= '     show_messagebox(result[0][4]);' . "\n";
        $return_string .= ' }' . "\n";
        $return_string .= '};' . "\n";
    ...
    /**
     * Tab toolbar click function.
     * @return String       JS string for toolbar click
     */
    private function click_tab_toolbar() {
        ...
        $return_string .= '             if (previous_text == get_locale_value("New")) {' . "\n";
        ...
        if ($this->call_from_combo) {
            $return_string .= '             parent.combo_data_add_win.callEvent("onWindowSaveCloseEvent", ["onSave", tab_text[1]]);' . "\n";
            $return_string .= '             return;' . "\n";
        }
    ...
    /**
     * Clear all deleted XML in grid.
     * @return String       JS string of deleted xml.
     */
    ...
    /**
     * Refresh tab property.
     * @return String       JS string to refresh tab
     */
    ...
    /**
     *  Opens a tab after insert-save.
     *  @return String JS string to open tab
     */
    private function open_tab() {
        ...
        if ($this->selected_id != '') {
            $return_string .= ' var no_of_tabs = ' . $this->name_space . '.tabbar.getNumberOfTabs();' . "\n";
            $return_string .= ' var prev_id = (no_of_tabs == 0) ? "" : ' . $this->name_space . '.tabbar.getActiveTab();' . "\n";
            $return_string .= ' var system_id = (no_of_tabs == 0) ? "' . $this->selected_id . '" : ' . $this->name_space . '.tabbar.tabs(prev_id).getText();' . "\n";
        } else {
            $return_string .= ' var prev_id = ' . $this->name_space . '.tabbar.getActiveTab();' . "\n";
    ...
    /**
     * Create Tab function.
     * @return String JS string to create tab
     */
    private function create_tab() {
        ...
        $return_string .= '         text = get_locale_value("New");'. "\n";
        $return_string .= '   } else { '. "\n";
    ...
    /**
     * Set form data.
     * @return String JS string to set form data
     */
    ...
    /**
     * Create New Id for tab.
     * @return String JS string to get new id
     */
     ...
     *  </pre>
     */
    public function set_grid_menu_json($json_array, $hide_originals = null) {
     ...
     * @return  String  JS string to enable menu
     */
    private function enable_menu_item() {
         ...
         if (!$this->hide_edit_menu) {
             $return_string .= '     if("' . $this->delete_permission . '" == "true" && id != null){' . "\n";  
         ...
         if ($this->add_privilege_menu) {
             $return_string .= ' if(id != null && id.indexOf(",") == -1) {' . "\n";
    ...
    /**
     * Disable the multiple select in the left side grid.
     */
    public function disable_multiple_select() {
