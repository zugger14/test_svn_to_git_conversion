 ...
 *  
 *  @par Description
 *  This class is used to generate a grid in accordion
 *  Grids should be defined in the template tables
 *  
...
class AccordionGrid extends AdihaAccordion { 
    ...
    /**
     * AccordionGrid constructor - set the table name as defined in template tables.
    ...
    /**
     * Initialize the accordion grid.
     * @param  String $accordion_name Name of the accordion grid
    ...
    public function init_accordion_grid($accordion_name, $namespace) {
    	...
    	$return_string .= $this->load_accordion_grid_data($this->sql_string, $this->grouping_column);
     ...
     * @return String               Data in JSON string
     */
    public function load_accordion_grid_data($grid_sp, $grouping_key) {
        if ($grid_sp != '') {
            ...
            for ($i = 0; $i < count($accordion_item_array); $i++) {
                $json_string .= '{"id":"' . str_replace(' ', '_', $accordion_item_array[$i]) . '","text":get_locale_value("' . $accordion_item_array[$i] . '"),"height":"*"},';
            }
    ...
    public function attach_search_textbox($layout_name, $layout_cell) {
        ...
        $search_text_json = '[
                                    {id:"search_text", type:"buttonInput", value:get_locale_value("Search..."), title:"Search", width:"235"}
                             ]';
        ...
        $html_string .= '   if (search_obj.value == get_locale_value("Search...")) {' . "\n";
        ...
        $html_string .= '       search_obj.value = get_locale_value("Search...");' . "\n";
        $html_string .= '   }' . "\n";
        $html_string .= '});' . "\n";
     ...
     * @return String JS string to filter the accordion
     */
     ...
     * @return String                 JS String equivalent to attach double click
     */
    ...
    /**
     * Attaching single click event for each grid in the accordion grid.
     * @param String  $grid_name      Grid in which event is to be attached
     * @param String  $event_function Single click function name
     * @return String                 JS String equivalent to attach single click
     */
