...
class gridPdfGenerator {
	...
	private function mainParse($xml) {
		...
		if ($xml->attributes()->pagewidth) {
			$this->pageFormat = (string) $xml->attributes()->pagewidth;
		}
	...
	public function printGridPdf($filename) {
		...
		$this->wrapper = new gridPdfWrapper($this->minOffsetTop, $this->minOffsetRight, $this->minOffsetBottom, $this->minOffsetLeft, $this->orientation, $this->fontSize, $this->dpi, $this->lang, $this->pageFormat);
	...
	private function printGridPage($startRow, $pageNumber) {
		...
		$footer_cols_array = (is_array($this->footerColumns) ? $this->footerColumns : array());
		$numRows = floor(($pageHeight - $this->headerHeight*count($this->columns) - $this->headerHeight*count($footer_cols_array))/$this->rowHeight);
