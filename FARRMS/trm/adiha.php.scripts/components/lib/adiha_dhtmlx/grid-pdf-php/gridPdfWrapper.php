...
class gridPdfWrapper {
	...
	private $imgsPath = '/imgs/'; // removed . from ./imgs/  to make PHP 7.0.0 compatible
	...
	public function __construct($offsetTop, $offsetRight, $offsetBottom, $offsetLeft, $orientation='P', $fontSize = 8, $dpi = 72, $lang, $format = 'A4') {
		// pdf-component initialization
		$this->cb = new TCPDFExt($orientation, 'mm', $format, true, 'UTF-8', false); 
	...
	private function footerDrawAfterRows() {
		...
		$yPos = $this->cb->getPageHeight() - $this->offsetBottom - $this->footerHeight*count($footer_cols_array);
	...
	public function gridDraw($rowHeight, $rows, $widths, $startRow, $rowsNum, $scaleOneColor, $scaleTwoColor, $profile) {
		...
		$last = '0';
		$footer_cols_array = (is_array($this->footerColumns) ? $this->footerColumns : array());
		$limitY = $this->cb->getPageHeight() - $this->offsetBottom - count($footer_cols_array)*$this->footerHeight;
