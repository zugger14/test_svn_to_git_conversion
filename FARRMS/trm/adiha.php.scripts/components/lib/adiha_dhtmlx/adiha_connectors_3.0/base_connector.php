...
class Connector {
	...
	public $data_cache;	//Data cache instance
	public $limit=false;
	...
	*/	
	public function __construct($db,$type=false, $item_type=false, $data_type=false, $render_type = false){
		...
		$this->enable_data_caching = $ENABLE_DATA_CACHING;
	...
	*/
	...
	public function render_complex_sql($sql,$id,$fields,$extra=false,$relation_id=false,$key_id=''){
		...
		if ($this->live_update !== false && $this->updating!==false) {
			...
			if ($this->editing){
				...
				if ($this->model && method_exists($this->model, "get")){
					...
					$k = $this->get_cache_key();
					$cachedresult = ($this->data_cache && $k!= '') ? $this->data_cache->get_data($k) : '';
					
					/*if(!empty($cachedresult))
					var_dump($cachedresult);
					else echo 'np data'.$this->request->get_fieldset().$k.$this->app_user_name;
					die();
					*/
					if ($this->names["item_class"] == "ComboDataItem" && !empty($cachedresult)) {
						$this->cached_output_as_xml($cachedresult);
					} else {
	...
	*/
	...
	/*! output fetched data as XML
		@param res
			DB resultset 
	*/
	protected function cached_output_as_xml($rs){
		/*if ($this->names["item_class"] == "ComboDataItem") {
		$a = $this->render_set($res);
		return $a;
		}
		 /**/
		$start="<?xml version='1.0' encoding='".$this->encoding."' ?>".$this->xml_start();
		$end=$this->render_cache_set($rs).$this->xml_end();
	...
	protected function render_cache_set($rs){
		return $this->render->render_cache_data_set($rs, $this->names["item_class"], $this->dload, $this->data_separator, $this->config);
		if ($this->data_cache) {
			$this->data_cache->close_conn();	
		}
