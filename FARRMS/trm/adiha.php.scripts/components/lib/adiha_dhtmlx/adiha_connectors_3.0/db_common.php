	...
	*/
	public function set_sort($field,$order=false){
		if (!$field && !$order)
			...
			/*added if condition to sort grid of price curve as header is send with [](bracked) but is replaced which is causing the query issue on order clause*/
...
class DataConfig{
	...
	*/
	public function init($id,$fields,$extra,$relation){
		...
		$this->extra_user_data = $this->parse($extra,true);
