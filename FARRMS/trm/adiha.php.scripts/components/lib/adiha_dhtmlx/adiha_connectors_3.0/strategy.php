...
class RenderStrategy {
	...
	*/
	public function render_set($res, $name, $dload, $sep, $config){
		...
		if ($ENABLE_DATA_CACHING && $data_cache->cache_data) {	
			$k = $conn->get_cache_key();
			if (!empty($k) && $k != '') {
				$cached_data = $data_cache->set_data($k,$rows);
			}
	...
	public function render_cache_data_set($res, $name, $dload, $sep, $config){
		$output="";
		$index=0;
		$conn = $this->conn;
		//$conn->event->trigger("beforeRenderSet",$conn,$res,$config);		
		
		//return "render_cache_data_set".$conn->data_cache->get_data($k);
		foreach ($res as $key => $value) { 
	        $data = new $name($value,$config,$key);
			if ($data->get_id()===false)
				$data->set_id($conn->uuid());
			$conn->event->trigger("beforeRender",$data);
			$output.=$data->to_xml().$sep;
