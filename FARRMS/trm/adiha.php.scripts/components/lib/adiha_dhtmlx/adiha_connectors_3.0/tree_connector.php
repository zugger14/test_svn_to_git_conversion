...
class TreeDataItem extends DataItem{
	...
	*/
	function to_xml_start(){
		...
		for ($i=1; $i < sizeof($this->config->text); $i++){ 
			$name=$this->config->text[$i]["name"];
			$value = isset($this->data[$name]) ? $this->xmlentities($this->data[$name]) : '';
			if ($value != '')
				$str1.=" ".$name."='".$value."'";
		}
		$str1.=">";

		for ($i=0; $i < sizeof($this->config->extra_user_data); $i++){ 
			$ud_name=$this->config->extra_user_data[$i]["name"];
			$ud_value = isset($this->data[$ud_name]) ? $this->xmlentities($this->data[$ud_name]) : '';
			if ($ud_value != '')
				$str1.="<userdata name='".$ud_name."'><![CDATA[".$ud_value."]]></userdata>";
		}
