...
namespace FARRMSGenericCLR
{
    internal class PlattsPriceImporter : ICLRImporter
    {
        ...
        public ImportStatus ImportData(CLRImportInfo clrImportInfo)
        {
            ...
            //this.SqlConnection = new SqlConnection(@"Data Source=SG-D-SQL01.FARRMS.US,2033;Initial Catalog=TRMTracker_Release;Persist Security Info=True;User ID=dev_admin;password=Admin2929");
        ...
        private DataTable CreateProcessTable()
        {
            ...
            StoredProcedure.ExecuteQuery(createTableSql, this.SqlConnection);
        ...
        private void DownloadFiles()
        {
            ...
            using (
                SftpClient sftpClient = new SftpClient(this.RuleSetting.Url, 22, this.RuleSetting.UserName, this.RuleSetting.Password))
            {
                sftpClient.Connect();
                IEnumerable<SftpFile> files = sftpClient.ListDirectory(this.RemoteDirectory);
                
                foreach (SftpFile sftpFile in files)
                {
                    string localFileName = DateTime.Today.ToString("yyyyMMdd_") + Guid.NewGuid().ToString().ToUpper().Replace("-", "_") + "." + sftpFile.Name;
                    using (Stream stream = File.OpenWrite(this.RuleSetting.DownloadDir + localFileName))
                    {
                        sftpClient.BufferSize = 1024;
                        sftpClient.DownloadFile(this.RemoteDirectory + sftpFile.Name, stream);
                        //this.DownloadedFiles.Add(this.DestinationDirectory + localFileName);
        ...
        void GetFtpConfiguration()
        {
            ...
            string sql =
                "select folder_location , ftp_url,ftp_username, dbo.FNADecrypt(ftp_password) ftp_password from ixp_import_data_source where rules_id =" +
            ...
            using (SqlCommand cmd = new SqlCommand(sql, this.SqlConnection))
            {
                using (SqlDataReader rd = cmd.ExecuteReader())
                {
                    ...
                    this.RuleSetting = new RuleSetting()
                    {
                        ...
                        Url = rd["ftp_url"].ToString(),
                        UserName = rd["ftp_username"].ToString(),
                        Password = rd["ftp_password"].ToString()
                    };
        ...
        private void Update(string asofDate)
        {
            ...
            StoredProcedure.ExecuteQuery(sql, this.SqlConnection);
