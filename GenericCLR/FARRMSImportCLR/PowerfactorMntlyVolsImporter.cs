...
{
    class PowerfactorMntlyVolsImporter : CLRWebImporterBase
    {
        public override ImportStatus ImportData(CLRImportInfo clrImportInfo)
        {
            ...
            try
            {
                ...
                clrImportInfo.WebServiceInfo.RequestBody = clrImportInfo.WebServiceInfo.RequestBody.Replace("<__start_time__>", clrImportInfo.Params[0].paramValue).Replace("<__end_time__>", clrImportInfo.Params[1].paramValue);
                ...
                {
                    request = clrImportInfo.WebServiceInfo.RequestBody.Replace("<__id__>", plant_id);
                    ...
                    webRequest.Headers.Add("Ocp-Apim-Subscription-Key", clrImportInfo.WebServiceInfo.Token);
                ...
                //using (SqlConnection cn = new SqlConnection("Data Source=SG-D-SQL01.FARRMS.US,2033;Initial Catalog=TRMTracker_Release;Persist Security Info=True;User ID=dev_admin;password=Admin2929"))
                {
                    ...
                    using (SqlCommand cmd = new SqlCommand("EXEC spa_process_rec_api_info 'powerfactor_vols', '" + clrImportInfo.RuleID.ToString() + "', '" + processTableName + "', '" + responseFromServer + "'", cn))
                    {
